...
case class KaisarContext(s: Statement) {
  ...
  def reapply(s: Statement): KaisarContext = {
    val x = KaisarContext(s)
  ...
  override def clone: KaisarContext = {
    val x = KaisarContext(s)
    x.ghostStatus = ghostStatus
    x.outer = outer
    x._isElaborationContext = _isElaborationContext
  ...
  def withInverseGhost: KaisarContext = {
    val gc: KaisarContext = this.clone
  ...
  def --(other: KaisarContext): KaisarContext = {
    def fail = throw TransformationException(s"Context $other is not a prefix of $this", node = s)
    if (this.s == other.s) KaisarContext.empty
    else
      (this.s, other.s) match {
        case (s, Triv()) => KaisarContext(s)
        case (Block(ss1), Block(ss2)) =>
          if (ss2.length > ss1.length) fail
          else if (ss2.isEmpty) KaisarContext(Block(ss1))
          else {
            val lrs = ss1.zip(ss2).dropWhile({ case (l, r) => l == r }).map({ case (l, r) => KaisarContext(l).--(KaisarContext(r)) })
            val lrStatement = lrs.foldLeft(KaisarContext.empty)({ case (l, r) => l.:+(r.s) })
            if (ss1.length >= ss2.length)
              lrStatement.:+(Block(ss1.drop(ss2.length)))
            else lrStatement
          }
        case (Block(l), r) if l.nonEmpty  =>
          val diff = KaisarContext(l.head) -- KaisarContext(r)
          if (diff.isEmpty) KaisarContext(Block(l.tail))
          else KaisarContext(KaisarProof.block(diff.s :: l.tail))
        ...
        case (switch: Switch, switchPr: SwitchProgress) if switch == switchPr.switch =>
          ...
          KaisarContext(switch.pats(switchPr.onBranch)._3) -- KaisarContext(switchPr.progress)
        case (bc: BoxChoice, bcPr: BoxChoiceProgress) if bc == bcPr.bc =>
          if (bcPr.onBranch == 0)
            KaisarContext(bc.left) -- KaisarContext(bcPr.progress)
          else
            KaisarContext(bc.right) -- KaisarContext(bcPr.progress)
        case (l, r) => fail
      }
  }

  /** Add s to the "end" of the context. */
  def :+(other: Statement): KaisarContext = {
    s match {
  ...
  def add(x: Ident, fml: Formula): KaisarContext = {
    :+(Assume(x, fml))
  }
  ...
  def programSignature: Map[ProgramConst, Program] = {
    s match {
      ...
      case Block(ss) => ss.flatMap(s => KaisarContext(s).programSignature).toMap
      case BoxChoice(l, r) => KaisarContext(l).programSignature ++ KaisarContext(r).programSignature
      case Switch(sel, pats) => pats.map(_._3).flatMap(KaisarContext(_).programSignature).toMap
      ...
      case ForProgress(fr, prog) => KaisarContext(prog).programSignature
      case BoxChoiceProgress(bl, i, progress) => KaisarContext(progress).programSignature
      case SwitchProgress(bl, i, progress) => KaisarContext(progress).programSignature
      case Phi(s) => KaisarContext(s).programSignature
  ...
  def signature: Map[Function, LetSym] = {
    s match {
      ...
      case Block(ss) => ss.flatMap(s => KaisarContext(s).signature).toMap
      case BoxChoice(l, r) => KaisarContext(l).signature ++ KaisarContext(r).signature
      case Switch(sel, pats) => pats.map(_._3).flatMap(KaisarContext(_).signature).toMap
      ...
      case ForProgress(fr, prog) => KaisarContext(prog).signature
      case BoxChoiceProgress(bl, i, progress) => KaisarContext(progress).signature
      case SwitchProgress(bl, i, progress) => KaisarContext(progress).signature
      case Phi(s) => KaisarContext(s).signature
      case _: Triv | _: Assume | _: Assert | _: Note | _: PrintGoal | _: InverseGhost | _: ProveODE | _: Modify
           | _: Label | _: Match | _: Pragma => Map()
    }
  }
  ...
  def searchAll(cq: ContextQuery, tabooProgramVars: Set[Variable], tabooFactVars: Set[Ident]): ContextResult = {
    ...
    def matched(x: IdentPat, fml: Formula): ContextResult =
      KaisarContext.matchAssume(x, fml, s).admissiblePart(this, tabooProgramVars, tabooFactVars).matchingPart(cq).elaborated(this, cq)
    s match {
  ...
  * @param wantProgramVar search exclusively for unannotated assignments x:=f rather than facts named "x" */
  ...
  private def finishFact(f: Formula, phis: List[Phi]): Formula = destabilize(phis.foldLeft(f)((fml, phi) => KaisarContext.substPhi(phi, fml)))
  ...
  def elaborateStable(pt: KaisarProofTerm): KaisarProofTerm = {
    pt match {
  ...
  def elaborateFunctions(pt: KaisarProofTerm, node: ASTNode): KaisarProofTerm =
    pt match {
  ...
  def ghost(f: Formula): KaisarContext = add(fresh(), f)
}

object KaisarContext {
  ...
  def empty: KaisarContext = KaisarContext(Triv())
