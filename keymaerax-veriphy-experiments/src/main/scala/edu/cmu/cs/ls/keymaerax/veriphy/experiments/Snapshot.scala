...
object Snapshot {
  ...
  def ofContext(c: KaisarContext): Snapshot = {
    ...
    val tf = new TraversalFunction {
      ...
      override def postDiffS(kc: KaisarContext, kce: KaisarContext, s: DiffStatement): DiffStatement = {
        s match {
      ...
      override def postDomS(kc: KaisarContext, kce: KaisarContext, s: DomainStatement): DomainStatement = {
        s match {
    ...
    ProofTraversal.traverse(KaisarContext.empty, KaisarContext.empty, c.s, tf)
    snap
  }
}
