 ...
 * @see [[KaisarProof]] for proof statement constructors
 * @see [[KaisarFileParser]] for concrete file syntax parsing
...
case class TheoremDecl(name: KaisarProof.Ident, inPf: Statement, outPf: Statement = Triv(), conclusion: Formula = True) extends KaisarDecl
/** Check whether given conclusion follows from given proof */
case class ProvesDecl(thmName: KaisarProof.Ident, conclusion: Formula) extends KaisarDecl
/** Display the already-proved conclusion of a proof */
case class ConclusionDecl(thmName: KaisarProof.Ident) extends KaisarDecl
/** Display the synthesized strategy resulting from an already-proved proof. */
case class SynthesizeDecl(thmName: KaisarProof.Ident) extends KaisarDecl
