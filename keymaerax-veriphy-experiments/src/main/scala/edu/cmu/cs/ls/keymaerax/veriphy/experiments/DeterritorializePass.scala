...
import DeterritorializePass.TimeTable
import KaisarProof.{Ident, LabelDef, LabelRef, TimeIdent, TransformationException, getAt}
import ProofTraversal.TraversalFunction
...
object DeterritorializePass {
  ...
  /** The [[TimeTable]] associates each line label with its key information such as snapshot, KaisarContext, definition */
  type TimeTable = Map[TimeIdent, (Snapshot, KaisarContext, LabelDef)]
  ...
  def apply(s: Statement): Statement = {
    ...
    val collectSnapshots = new TraversalFunction {
      override def postS(kc: KaisarContext, kce: KaisarContext, s: Statement): Statement = {
        s match {
    ...
    ProofTraversal.traverse(KaisarContext.empty, KaisarContext.empty, s, collectSnapshots)
...
case class DeterritorializePass(tt: TimeTable) {
  ...
  private def rewind(conDiff: KaisarContext, labelSnap: Snapshot, lr: LabelRef, f: Expression, except: Set[Ident], node: ASTNode): Expression = {
  ...
  private def renameAdmissible(kc: KaisarContext, label: LabelRef, f: Expression, except: Set[Ident], node: ASTNode): Option[Expression] = {
  ...
  def etf(kc: KaisarContext, local: Set[Ident], node: ASTNode = Triv()): ExpressionTraversalFunction = new ExpressionTraversalFunction {
  ...
  private def translate(kc: KaisarContext, t: Term, localVars: List[Ident] = Nil, node: ASTNode = Triv()): Term = {
    ExpressionTraversal.traverse(etf(kc, localVars.toSet, node), t).getOrElse(t)
  }

  /** Translate a formula [[fml]] */
  private def translate(kc: KaisarContext, fml: Formula, localVars: List[Ident], node: ASTNode): Formula = {
    ExpressionTraversal.traverse(etf(kc, localVars.toSet, node), fml).getOrElse(fml)
  }

  /** Translate an expression [[e]] */
  private def translate(kc: KaisarContext, e: Expression, localVars: List[Ident], node: ASTNode): Expression = {
  ...
  private def translatePat(kc: KaisarContext, pat: Term,  node: ASTNode): Term = {
    val boundVars = VariableSets(kc).boundVars
    ExpressionTraversal.traverse(etf(kc, boundVars, node), pat).getOrElse(pat)

  }
  private def translatePat(kc: KaisarContext, pat: Formula, node: ASTNode): Formula = {
    val boundVars = VariableSets(kc).boundVars
    ExpressionTraversal.traverse(etf(kc, boundVars, node), pat).getOrElse(pat)
  }
  ...
  def apply(s: Statement): Statement = {
    val pass = new TraversalFunction {
      ...
      override def preS(kc: KaisarContext, kce: KaisarContext, s: Statement): Option[Statement] = {
        s match {
      ...
      override def postPT(kc: KaisarContext, kce: KaisarContext, pt: KaisarProofTerm): KaisarProofTerm = {
        pt match {
      ...
      override def postSel(kc: KaisarContext, kce: KaisarContext, sel: Selector): Selector = {
        sel match {
      ...
      override def postDomS(kc: KaisarContext,  kce: KaisarContext, ds: DomainStatement): DomainStatement = {
        ds match {
      ...
      override def postDiffS(kc: KaisarContext,  kce: KaisarContext, ds: DiffStatement): DiffStatement = {
        ds match {
    ...
    ProofTraversal.traverse(KaisarContext.empty, KaisarContext.empty, s, pass)
  }
}