...
sealed trait KaisarProofTerm extends ASTNode {
...
case class ProofVar(x: Ident) extends KaisarProofTerm {}
// Looks up all assumptions corresponding to program variable
case class ProgramVar(x: Variable) extends KaisarProofTerm {}
// Looks up assignments corresponding to a program variable
case class ProgramAssignments(x: Variable, onlySSA: Boolean = false) extends KaisarProofTerm {}
...
case class ProofInstance(e: Expression) extends KaisarProofTerm {}
...
case class ProofApp(m: KaisarProofTerm, n: KaisarProofTerm) extends KaisarProofTerm {}
...
case class ForwardSelector(forward: KaisarProofTerm) extends Selector {}
...
case class Note(x: Ident, proof: KaisarProofTerm, var annotation: Option[Formula] = None) extends Statement
...
final case class DomCollection(assumptions: Set[DomAssume], assertions: List[DomAssert],  weakens: Set[DomainStatement], modifiers: Set[DomModify]) {
  def constraints: KaisarContext = {
    val assume = assumptions.foldLeft(KaisarContext.empty)({case (acc, DomAssume(x, f)) => acc.:+(Assume(x, f))})
