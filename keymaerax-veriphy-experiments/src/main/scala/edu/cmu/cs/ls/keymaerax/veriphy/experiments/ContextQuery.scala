 ...
 */
/** The ContextQuery trait gives a query language for inspecting the proof context for facts and assignments.
 * A query-language approach provides compositionality and is especially useful for bulk queries. In turn, support for
 * bulk queries is important both for performance and convenience. For performance, it is important that we do not
 * unintentionally select the same fact twice, and the language approach simplifies fact selection deduplication.
 * Bulk queries also provide convenience when lookup up two facts A and B which are each doubly defined on disjoint
 * proof branches C and D. Naive lookups would yield a weaker result (A_C | A_D) & (B_C | B_D), while we can support
 * the more precise  (A_C & B_C) | (A_D & B_D). This need arises frequently in safety proofs of branching controllers.
 * Queries arise both from the selectors written in a proof (e.g. "x" or "fact") as well as Kaisar's own heuristics.
 * As the scenario warrants, default selectors can look up facts, or assignments which are likely to be relevant.
...
sealed trait ContextQuery {
  /** Required operation.
   * @param x  optional fact identifier
   * @param f fact formula,
   * @param isAssignment  flag indicating whether the fact was induced by an assignment
   * @return whether this query is looking for the given fact */
  ...
  def elaborate(kc: KaisarContext, f: Formula): Formula = f
  /** Optionally elaborate or otherwise post-process only the successful matches */
  def elaborate(kc: KaisarContext, f: Term): Term = f

  def partitionPt: (List[KaisarProofTerm], ContextQuery) = {
    this match {
 ...
 * (user-defined and internal) function symbols appearing in the result. */
case class QElaborate(cq: ContextQuery) extends ContextQuery {
  ...
  override def elaborate(kc: KaisarContext, f: Formula): Formula =
    kc.elaborateStable(kc.elaborateFunctions(f, Triv()))
  override def elaborate(kc: KaisarContext, f: Term): Term =
    kc.elaborateStable(kc.elaborateFunctions(f, Triv()))
}
...
/** Matches all assignments which assign a given variable [[x]]. The subscript of [[x]] is ignored so that a simple search
 * for [[x]] will capture its SSA-variants such as [[x_1]].*/
...
/** Matches only facts which are bound to fact name [[x]]. In the case that [[x]] is bound to different facts on different
 * branches, both facts are returned so that they may be (soundly) disjoined. */
 ...
 * selecting the [[note]]d fact variable. However, having a ProofTerm query would allow us in principle to manage
 * proof terms whose arguments differ across branches. */
case class QProofTerm(pt: KaisarProofTerm) extends ContextQuery {
...
/**  Applies a query [[cq]] under an assignment. Assuming queries are applied to (light) SSA-form contexts, the QPhi
 * is only needed for (inlined) Phi assignments, as they are the only assignments which ever shadow another. */
case class QPhi(phi: Phi, cq: ContextQuery) extends ContextQuery {
  override def matches(x: Option[Ident], fml: Formula, isAssignment: Boolean): Boolean =
    cq.matches(x, KaisarContext.substPhi(phi, fml), isAssignment)

  override def elaborate(kc: KaisarContext, f: Formula): Formula = cq.elaborate(kc, f)
}
