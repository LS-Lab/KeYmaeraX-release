...
object ForwardProofChecker {
  ...
  private def ptToForwardArg(con: KaisarContext, pt: KaisarProofTerm): ForwardArg = {
    pt match {
  ...
  def apply(con: KaisarContext, pt: KaisarProofTerm): Formula = {
    pt match {
      case ProgramVar(x) =>
