...
import edu.cmu.cs.ls.keymaerax.veriphy.experiments.KaisarProof.{Ident, LabelDef}

object KaisarPrettyPrinter {
  ...
  private def ofFml(f:Formula): String = {
    "(" + f.prettyString + ")"
  }

  private def ofProofTerm(term: KaisarProofTerm): String = {
    term match {
  ...
  def full(s: Statement): String = {
    s match {
      ...
      case Switch(scrutinee, pats) =>
        "switch " + scrutinee.map(s => "(" + ofSelector(s) + ")").getOrElse("") + "{\n" + pats.map(ofBranch).mkString + "}\n"
      case ProveODE(ds, dc) =>
        ofODE(ds, dc)
