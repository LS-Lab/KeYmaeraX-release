...
//import edu.cmu.cs.ls.keymaerax.cd//gl.kaisar.KaisarContext._
import KaisarProof.{ElaborationException, Ident, IdentPat}
import ASTNode._
import edu.cmu.cs.ls.keymaerax.core._

class ElaborationPass() {
  var ghostCon: KaisarContext = KaisarContext.empty.asElaborationContext
  ...
  private def freeVarsPT(con: KaisarContext, pt: KaisarProofTerm): Set[Variable] = {
    pt match {
  ...
  private def freeVarsSel(con: KaisarContext, sel: Selector): Set[Variable] = {
    sel match {
  ...
  /** Determine which variables in [[pt]] are fact variables vs. program variables when resolved in KaisarContext [[kc]]. */
  private def collectPt(kc: KaisarContext, pt: KaisarProofTerm): KaisarProofTerm = {
    pt match {
  ...
  private def collectPts(kc: KaisarContext, sel: Selector, goal: Formula): List[Selector] = {
    sel match {
  ...
  private def collectPts(kc: KaisarContext, m: Method, goal: Formula): (List[Selector], Method) = {
    m match {
      case Using(use, m) =>
  ...
  private def collectPts(kc: KaisarContext, dc: DomainStatement): DomainStatement = {
    dc match {
      case DomAnd(l, r) =>
  ...
  def apply(s: Statement): Statement = {
    ...
    val ret = ProofTraversal.traverse(KaisarContext.empty.asElaborationContext, KaisarContext.empty.asElaborationContext,s, new TraversalFunction {
      // Forward proof term conjoining input list of facts
      private def conjoin(xs: List[Ident]): KaisarProofTerm = {
        xs.map(ProofVar).reduce[KaisarProofTerm]{case (l, r) => ProofApp(ProofApp(ProofVar(Variable("andI")), l), r)}
      }
      ...
      private def coerceFormula(e: KaisarProofTerm): KaisarProofTerm = {
        e match {
      ...
      private def coerceTerm(e: KaisarProofTerm): KaisarProofTerm = {
        e match {
      ...
      private def coercePTSorts(pt: KaisarProofTerm): KaisarProofTerm = {
        pt match {
      ...
      override def preS(kc: KaisarContext, kce: KaisarContext, sel: Statement): Option[Statement] = {
        sel match {
          case mod: Modify =>
          ...
          case ProveODE(ds, dc) =>
            ...
            // KaisarContext should include ODE during collection, because we want to know about domain constraint fmls and we may
      ...
      private def elaborateODE(kc: KaisarContext, kce: KaisarContext,  ds: DiffStatement): DiffStatement = {
        ds match {
    ...
    ghostCon = KaisarContext.empty.asElaborationContext
    ret
  }
}
