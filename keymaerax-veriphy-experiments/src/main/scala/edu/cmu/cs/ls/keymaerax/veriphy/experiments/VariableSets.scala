...
object VariableSets {
   ...
   * @param isInverseGhost Was the KaisarContext where the facts were defined enclosed by an inverse-ghost?
   * */
   ...
   * @param isGhost Was the KaisarContext where the program variables were defined enclosed by a ghost?
   * */
   ...
   * @param isInverseGhost Was the KaisarContext where the functions were defined enclosed by an inverse-ghost?
   * */
  ...
  /** Compute static semantic variables sets for a given Kaisar [[statement]], depending on the KaisarContext in which
   * [[statement]] appears
   *
   ...
   */
  def apply(statement: Statement): VariableSets = apply(KaisarContext(statement).withOuter)
  def apply(kc: KaisarContext): VariableSets = kc.s match {
  ...
  /** Compute static semantic variables sets for a given Kaisar differential statement, depending on the KaisarContext in
   * which it appears
   *
   * @param kc Kaisar KaisarContext surrounding statement
   * @param diffStatement The differential Kaisar statement whose variable sets should be computed
   * @return All bound and taboo variables of statement
   */
  def apply(kc: KaisarContext, diffStatement: DiffStatement): VariableSets = diffStatement match {
  ...
  /** Compute static semantic variables sets for a given Kaisar domain statement, depending on the KaisarContext in
   * which it appears
   *
   * @param kc Kaisar KaisarContext surrounding statement
   ...
   * @return All bound and taboo variables of statement
   */
  def apply(kc: KaisarContext, domainStatement: DomainStatement): VariableSets = domainStatement match {
