 ...
 */
/** Structured result of a context lookup. Successful queries can return facts and assignments, while failures are distinguished
 * between those that should propagate or those that should not. When the result differs by which proof branch is
 * "executed," both branches are given.
 *
 *  @author Brandon Bohrer
 * @see [[edu.cmu.cs.ls.keymaerax.cdgl.kaisar.ContextQuery]]
...
sealed trait ContextResult {
  ...
  def admissiblePart(inContext: KaisarContext, tabooProgramVars: Set[Variable], tabooFactVars: Set[Ident]): ContextResult =
    this match {
  ...
  def elaborated(kc: KaisarContext, cq: ContextQuery): ContextResult = {
    this match {
