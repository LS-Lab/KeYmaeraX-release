...
object KaisarKeywordParser {
  ...
  def proofVarApp[_: P]: P[KaisarProofTerm] = (ident ~ ("(" ~ proofTerm.rep(sep =",") ~ ")").?).map({
    case (id, None) => ProofVar(id)
    case (id, Some(args)) => args.foldLeft[KaisarProofTerm](ProofVar(id))({case (acc, pt) => ProofApp(acc, pt)})
  })
  //@TODO: add support for distinct ProofVar terms
  def proofTerm[_: P]: P[KaisarProofTerm] = !reserved ~ (proofInstance | proofVarApp)
