...
import org.keymaerax.hydra.requests.models._
import org.keymaerax.hydra.requests.proofs.ExtractModelSolutionsRequest
...
object Models extends Logging {
  ...
  val modelList: SessionToken => Route = (t: SessionToken) =>
    pathPrefix("models" / "users" / Segment / Segment.?) { (userId, folder) =>
      {
        pathEnd {
          get {
  ...
  val userModel: SessionToken => Route = (t: SessionToken) =>
    userPrefix { userId =>
      {
        pathPrefix("model" / Segment) { modelNameOrId =>
          {
            pathEnd {
              get {
  ...
  val updateUserModel: SessionToken => Route = (t: SessionToken) =>
    userPrefix { userId =>
      pathPrefix("model" / Segment / "update") { modelId =>
        pathEnd {
          post {
            entity(as[String]) { modelJson =>
              val modelData = modelJson
                .parseJson
                .asJsObject
                .fields
                .map({ case (k, v) => k -> v.asInstanceOf[JsString].value })
              completeRequest(
  ...
  val importExampleRepo: SessionToken => Route = (t: SessionToken) =>
    path("models" / "users" / Segment / "importRepo") { userId =>
      {
        pathEnd {
          post {
  ...
  val deleteModel: SessionToken => Route = (t: SessionToken) =>
    userPrefix { userId =>
      pathPrefix("model" / Segment / "delete") { modelId =>
        pathEnd {
          post {
  ...
  val downloadAllModels: SessionToken => Route = (t: SessionToken) =>
    path("models" / "user" / Segment / "downloadAllModels" / Segment) { (userId, proofs) =>
      {
        pathEnd {
          get {
  ...
  val deleteAllModels: SessionToken => Route = (t: SessionToken) =>
    path("models" / "user" / Segment / "delete" / "all") { userId =>
      {
        pathEnd {
          get {
  ...
  val userModel2: SessionToken => Route = (t: SessionToken) =>
    userPrefix { userId =>
      {
        pathPrefix("modelupload" / Segment.?) { modelNameOrId =>
          {
            pathEnd {
              post {
  ...
  val createTemplate: SessionToken => Route = (t: SessionToken) =>
    pathPrefix("models" / "users" / Segment / "templates" / Segment / "create" / Segment) {
      (userId, template, switchingKind) =>
        {
          pathEnd {
            post {
              entity(as[String]) { x =>
                {
                  ...
                  val request = template match {
                    case "controlledstability" => new CreateControlledStabilityTemplateRequest(
                        userId,
                        code,
                        switchingKind,
                        specKind,
  ...
  val guestBrowseArchiveRequest: Route = path("show" / Segments) { archiveUri =>
    pathEnd {
      get {
  ...
  val examples: SessionToken => Route = (t: SessionToken) =>
    path("examples" / "user" / Segment / "all") { userId =>
      pathEnd {
        get {
  ...
  val templates: SessionToken => Route = (t: SessionToken) =>
    path("templates" / "user" / Segment / "all") { userId =>
      pathEnd {
        get {
