...
import org.keymaerax.hydra.responses.modelplex.{
  ModelPlexArtifactResponse,
  ModelPlexCCodeResponse,
  ModelPlexMonitorResponse,
  ModelPlexSandboxResponse,
}
import org.keymaerax.hydra.{DBAbstraction, ErrorResponse, ModelPOJO, RegisteredOnlyRequest, Response, UserRequest}
...
class ModelPlexRequest(
    ...
    modelFml match {
      case Imply(init, Box(prg, _)) => conditionKind match {
          ...
          case "C" =>
            ...
            )(prg, vars, CodeGenerator.getInputs(prg))
            val code = s"""${controller._1}
                          |${controller._2}
                          |
                          |int main() {
                          |  /* control loop stub */
            ...
            conditionKind match {
              ...
              case "C" =>
                ...
                val ctrlMonitorCode = (new CGenerator(
                  new CMonitorGenerator(Symbol("resist"), model.defs),
                  init,
                  model.defs,
                ))(monitorCond, stateVars, ctrlInputs, "Monitor")
  ...
  /**
   * Synthesizes a ModelPlex monitor formula over variables `vars` from the model `modelFml`. Returns the monitor
   * conjecture together with the synthesized monitor, or an error.
   */
  private def createMonitorCondition(
      modelFml: Formula,
    ...
    val mx = ModelPlex.mxSynthesize(monitorKind) &
      ...
      ModelPlex.mxAutoInstantiate(assumptions) & ModelPlex.mxFormatShape(monitorShape)

    val monitorCond =
      try { TactixLibrary.proveBy(modelplexInput, mx) }
      catch {
      ...
      case Left((modelplexConjecture, monitorFml, synthesizeTactic)) => conditionKind match {
          case "dL" =>
            ...
            val entry = ParsedArchiveEntry(
              model.name + " ModelPlex Monitor",
              "theorem",
              "",
              mxProof.conclusion.succ.head.prettyString,
          ...
          case "C" =>
            val inputs = CodeGenerator.getInputs(prg)
            val monitor = (
              new CGenerator(new CMonitorGenerator(Symbol("resist"), model.defs), init, model.defs)
            )(monitorFml, vars, inputs, model.name)
            val code =
              s"""${monitor._1}
                 |${monitor._2}
                 |
          ...
          case "Python" =>
            ...
            val monitor = (new PythonGenerator(
              new PythonMonitorGenerator(Symbol("min"), model.defs),
              init,
              model.defs,
            ))(monitorFml, vars, inputs, model.name)
