...
import org.keymaerax.bellerophon.parser.HackyInlineErrorMsgPrinter
import org.keymaerax.btactics.macros._
import org.keymaerax.btactics.{TactixInit, ToolProvider}
import org.keymaerax.core.{ProverException, Sequent}
import org.keymaerax.hydra._
import org.keymaerax.hydra.responses.proofs.{RunBelleTermResponse, TacticErrorResponse}
import org.keymaerax.parser.{ArchiveParser, ParseException, Parser, UnknownLocation}
 ...
 * to a Tactic */
class RunBelleTermRequest(
    db: DBAbstraction,
    userId: String,
    proofId: String,
    nodeId: String,
    belleTerm: String,
    pos: Option[PositionLocator],
    pos2: Option[PositionLocator] = None,
    inputs: List[BelleTermInput] = Nil,
    consultAxiomInfo: Boolean = true,
  ...
  private def fullExpr(sequent: Sequent): String = {
    val paramStrings: List[String] = inputs.map {
      ...
      case BelleTermInput(value, Some(ListArg(_: FormulaArg))) =>
        ...
        if (values.isEmpty) "\"nil\"" else "\"" + values.mkString("::") + "::nil\""
      case BelleTermInput(value, Some(ListArg(_: VariableArg))) =>
        val values = Parser.parseExpressionList(value).map(_.prettyString)
        if (values.isEmpty) value else "\"" + values.mkString("::") + "::nil\""
    ...
    val (specificTerm: String, adaptedPos: Option[PositionLocator], adaptedPos2: Option[PositionLocator], args) =
      if (consultAxiomInfo) RequestHelper
        ...
        case Right(t: PositionTacticInfo) if t.codeName == "hideL" || t.codeName == "hideR" =>
          pos match {
            case Some(Fixed(pp, None, _)) if pp.isTopLevel =>
              (t.codeName, Some(Fixed(pp, Some(sequent(pp.top)))), None, Nil)
    ...
    if (
      (args match {
        case (_: ListArg) :: Nil => true
        case _ => false
      }) && inputs.isEmpty && adaptedPos.isEmpty
    ) {
      ...
      specificTerm + "(" + paramStrings.mkString(",") + adaptedPos.map("," + _.prettyString).getOrElse("") +
        adaptedPos2.map("," + _.prettyString).getOrElse("") + ")"
    }
  }

  private class TacticPositionError(val msg: String, val pos: org.keymaerax.parser.Location, val inlineMsg: String)
      extends Exception
  ...
  private def executionInfo(ruleName: String): String = ruleName + ": " +
    (ruleName match {
      case "solve" | "ODE" =>
        """
          ...
          case Some(node) if node.goal.isDefined =>
            ...
            try {
              ...
              def interpreter(proofId: Int, startNodeId: Int) = new Interpreter {
                ...
                private val inner = SpoonFeedingInterpreter(
                  proofId,
                  startNodeId,
                  db.createProof,
                  proofSession.defs,
                ...
                if (ruleName == "custom") {
                  ...
                  val startStepIndex = node.id match {
                    ...
                    case _ => throw new Exception(
                        "Unexpected node ID shape " + node.id.toString +
                  ...
                  val taskId = executor.schedule(
                    userId,
                    expr,
                    BelleProvable.labeled(localProvable, node.label.map(_ :: Nil)),
                    interpreter(localProofId, -1),
                  )
                  RunBelleTermResponse(
                    localProofId.toString,
                    "()",
                    taskId,
