...
import org.keymaerax.core.{BaseVariable, Formula, FuncOf, Function, Nothing, Number, Real, Term, Unit, Variable}
import org.keymaerax.hydra.{ModelPOJO, Response, UIKeYmaeraXPrettyPrinter}
...
class TestSynthesisResponse(
    ...
    testCases: List[(String, List[(Map[Term, Term], Option[Number], Number)])],
) extends Response {
  ...
  private def radius(n: BigDecimal): BigDecimal =
    if (maxVariance > 0) minRadius + (maxRadius - minRadius) * (n / maxVariance) else minRadius

  private def scatterData(tc: List[(Map[Term, Term], Option[Number], Number)]) = JsArray(
    tc.zipWithIndex
      .map({ case ((_, safetyMargin, Number(variance)), idx) =>
        JsObject(
          "x" -> JsNumber(idx),
          "y" ->
            (safetyMargin match {
              case Some(Number(sm)) => JsNumber(sm)
  ...
  private def prePostVals(vals: Map[Term, Term]): (JsArray, JsArray, JsArray, JsArray) = {
    ...
    val preJson = pre.map({ case (v, Number(value)) =>
      JsObject("v" -> JsString(v.prettyString), "val" -> JsNumber(value))
    })
    val postJson = post.map({ case (v, Number(value)) =>
      JsObject("v" -> JsString(preByPost(v).prettyString), "val" -> JsNumber(value))
    })
    ...
    (
      JsArray(preJson.toVector),
      JsArray(postJson.toVector),
      JsArray(labels.toVector),
      JsArray(JsArray(preSeries.toVector), JsArray(postSeries.toVector)),
    )
  }

  private def seriesData(data: List[(Map[Term, Term], Option[Number], Number)]): JsArray = JsArray(
    data
      .zipWithIndex
      .map({ case ((vals: Map[Term, Term], safetyMargin, Number(variance)), idx) =>
