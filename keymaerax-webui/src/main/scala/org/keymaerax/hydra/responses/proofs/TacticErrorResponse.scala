...
import org.keymaerax.bellerophon.{BelleThrowable, BelleUnexpectedProofStateError, CompoundCriticalException}
...
class TacticErrorResponse(msg: String, tacticMsg: String, exn: Throwable = null)
  ...
  override def getJson: JsValue = exn match {
    case _: BelleUnexpectedProofStateError =>
      JsObject(super.getJson.asJsObject.fields ++ Map("tacticMsg" -> JsString(tacticMsg)))
    case ex: CompoundCriticalException =>
      val exceptions = flatten(ex)
      val messages = exceptions
        .zipWithIndex
        .map({
      ...
      JsObject(
        super.getJson.asJsObject.fields.filter(_._1 != "textStatus") ++
          Map("textStatus" -> JsString(message), "tacticMsg" -> JsString(tacticMsg))
      )
    case _ => JsObject(super.getJson.asJsObject.fields ++ Map("tacticMsg" -> JsString(tacticMsg)))
  }
