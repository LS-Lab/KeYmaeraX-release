...
/**
  ...
  */
class PythonMonitorGenerator(conjunctionsAs: Symbol, defs: Declaration) extends MonitorGenerator(conjunctionsAs, defs,
  PythonPrettyPrinter, PythonMonitorGenerator.termContainer) {

  /** @inheritdoc */
  override def apply(expr: Expression, stateVars: Set[BaseVariable], inputVars: Set[BaseVariable],
                     modelName: String): (String, String) = generateMonitoredCtrlPythonCode(expr, stateVars)
