...
object KeYmaeraX {
  ...
  def modelplex(options: OptionMap) = {
    ...
    pw.write("/**\n * variables are for the state before the controller run\n * post() function symbols are for the state after the controller run\n * other function symbols are constant\n */\n\n")
    pw.write(output)
    pw.close()
  }
