...
/**
 * A Request should handle all expensive computation as well as all possible side-effects of a request (e.g. updating
 * the database), but should not modify the internal state of the HyDRA server (e.g. do not update the event queue).
 *
...
trait Request extends Logging {
  ...
  final def permission(t: SessionToken): Boolean =
    (t match {
...
abstract class LocalhostOnlyRequest() extends Request {
  override protected def doPermission(t: SessionToken): Boolean =
    !HyDRAServerConfig.isHosted // @todo change this to a literal false prior to deployment.
}
...
object RequestHelper {
    ...
    ProvableInfo.locate(di.canonicalName) match {
      case Some(i) =>
        val (cond, op, key, keyPosString, conclusion, conclusionPos) = i.provable.conclusion.succ.head match {
          case Imply(c, eq @ Equiv(l, r)) if keyPos == PosInExpr(1 :: 0 :: Nil) =>
            (Some(c), OpSpec.op(eq).opcode, l, "1.0", r, "1.1")
          case Imply(c, eq @ Equiv(l, r)) if keyPos == PosInExpr(1 :: 1 :: Nil) =>
            (Some(c), OpSpec.op(eq).opcode, r, "1.1", l, "1.0")
          case bcf: BinaryCompositeFormula if keyPos == PosInExpr(0 :: Nil) =>
            (None, OpSpec.op(bcf).opcode, bcf.left, "0", bcf.right, "1")
          case bcf: BinaryCompositeFormula if keyPos == PosInExpr(1 :: Nil) =>
            (None, OpSpec.op(bcf).opcode, bcf.right, "1", bcf.left, "0")
  ...
  def getSpecificName(
      tacticId: String,
      sequent: Sequent,
      l1: Option[PositionLocator],
      l2: Option[PositionLocator],
      what: DerivationInfo => String,
      session: ProofSession,
  ): String = getSpecificName(tacticId, sequent, l1, l2, session) match {
    case Left(s) => s
    case Right(t) => what(t)
  }
  ...
  def getSpecificName(
      ...
      l2: Option[PositionLocator],
      session: ProofSession,
  ): Either[String, DerivationInfo] = {
    val pos = l1 match {
      case Some(Fixed(p, _, _)) => Some(p)
      case _ => None
    }
    val pos2 = l2 match {
      case Some(Fixed(p, _, _)) => Some(p)
    ...
    tacticId.toLowerCase match {
      case "step" | "stepat" if pos.isDefined && pos2.isEmpty =>
        sequent.sub(pos.get) match {
          case Some(fml: Formula) => UIIndex.theStepAt(fml, pos, Some(sequent), session.defs.substs) match {
      ...
      case "step" | "stepat" if pos.isDefined && pos2.isDefined =>
        sequent.sub(pos.get) match {
          case Some(fml: Formula) => UIIndex.theStepAt(pos.get, pos2.get, sequent) match {
              case Some(step) => Right(step)
              case None => Left(tacticId)
            }
        }
  ...
  def listenerFactory(db: DBAbstraction, session: ProofSession)(
      proofId: Int
  ...
  def updateProofSessionDefinitions(proofSession: ProofSession, node: ProofTreeNode): ProofSession = {
    val symbols = node
      .children
      .flatMap(_.localProvable.subgoals.flatMap(StaticSemantics.symbols))
      .toSet[NamedSymbol]
      .filter({
    ...
    val elaboratedToFns = symbols.filter({
      case Function(n, i, Unit, s, _) => proofSession
          .defs
          .decls
          .exists({
    ...
    val newDefs: Map[Name, Signature] = undefinedSymbols
      .map({
        case fn @ Function(name, index, domain, sort, _) => Name(name, index) -> Signature(
            Some(domain),
            sort,
            collectedArgs.decls.get(Name(fn.name, fn.index)).flatMap(_.arguments),
            Right(None),
            UnknownLocation,
          )
        ...
        case u =>
          Name(u.name, u.index) ->
            Signature(None, u.sort, None, Right(None), UnknownLocation) // cuts may introduce variables
      })
      .toMap
