...
class InMemoryDB extends DBAbstraction {
  ...
  private val configs: scala.collection.mutable.Map[String, ConfigurationPOJO] = scala
    .collection
    .mutable
    .Map(
      "version" -> new ConfigurationPOJO("version", Map("version" -> Version.toString)),
      ...
      "mathematica" -> new ConfigurationPOJO(
        ...
        Map(
          ...
          "jlinkLibDir" ->
            "/Applications/Mathematica.app/Contents/SystemFiles/Links/JLink/SystemFiles/Libraries/MacOSX-x86-64",
        ),
      ),
    )
  ...
  override def getTempUsers: List[UserPOJO] = Nil
  ...
  override def getProofsForModel(modelId: Int): List[ProofPOJO] =
    synchronized { proofs.values.map(_._2).filter(_.modelId.contains(modelId)).toList }
  ...
  override def createProofForModel(
      modelId: Int,
      name: String,
      description: String,
      date: String,
  ...
  override def createProof(provable: ProvableSig): Int = synchronized {
    ...
    provables(provableId) = provable
    proofs(proofId) =
      (provable, ProofPOJO(proofId, None, "", "", "", 0, closed = false, Some(provableId), temporary = true, None))
    proofId
  }
  ...
  override def deleteModel(modelId: Int): Boolean = synchronized {
    proofs
      .filter({ case (_, (_, p)) => p.modelId.contains(modelId) })
      .foreach({ case (_, (_, p)) => deleteProof(p.proofId) })
  ...
  override def getExecutable(executableId: Int): ExecutablePOJO = { getExecutables(List(executableId)).head }
  ...
  def getExecutables(executableIds: List[Int]): List[ExecutablePOJO] =
    synchronized { executables.view.filterKeys(executableIds.contains).values.toList.sortBy(_.executableId) }

  /** Use escape hatch in prover core to create a new Provable */
  override def getProvable(lemmaId: Int): ProvablePOJO = { loadProvables(List(lemmaId)).head }

  def loadProvables(lemmaIds: List[Int]): List[ProvablePOJO] = provables
    .view
    .filterKeys(lemmaIds.contains)
    .map({ case (k, v) => ProvablePOJO(k, v) })
    .toList
    .sortBy(_.provableId)
  ...
  override def agendaItemsForProof(proofId: Int): List[AgendaItemPOJO] = agendaItems
    .values
    .filter(_.proofId == proofId)
    .toList

  override def getAgendaItem(proofId: Int, initialProofNode: ProofTreeNodeId): Option[AgendaItemPOJO] = agendaItems
    .values
    .find(i => i.proofId == proofId && i.initialProofNode == initialProofNode)

  /** Updates an executable step */
  override def updateExecutionStep(executionStepId: Int, step: ExecutionStepPOJO): Unit =
    synchronized { executionSteps(executionStepId) = step }
  ...
  def proofSteps(executionId: Int): List[ExecutionStepPOJO] = synchronized {
    executionSteps
      .values
      .filter(v => v.executionId == executionId && v.status == ExecutionStepStatus.Finished)
      .toList
      .sortBy(_.stepId)
  }
  ...
  private def zipTrace(
      ...
      executables: List[ExecutablePOJO],
      localProvables: List[ProvableSig],
  ): List[ExecutionStep] = {
    (executionSteps, executables, localProvables) match {
      case (step :: steps, _ :: exes, localProvable :: moreProvables) =>
        ...
        ExecutionStep(
          ...
          step.executableId,
          step.userExecuted,
        ) :: zipTrace(steps, exes, moreProvables)
  ...
  override def getPlainOpenSteps(proofId: Int): List[(ExecutionStepPOJO, List[Int])] = {
    ...
    trace
      .map(parent => (parent, trace.filter(s => parent.stepId == s.previousStep).map(_.branchOrder)))
    ...
    steps.find(_.stepId.contains(stepId)) match {
      ...
      case Some(step) =>
        ...
        Some(ExecutionStep(
          ...
          step.previousStep,
          step.executionId,
          () => getProvable(step.localProvableId.get).provable,
          step.branchOrder,
          step.numSubgoals,
          step.numOpenSubgoals,
          successorIds,
  ...
  override def getExecutionTrace(proofId: Int, withProvables: Boolean = true): ExecutionTrace = {
    /* This method has proven itself to be a resource hog, so this implementation attempts to minimize the number of
       DB calls. */
    val steps = proofSteps(proofId)
    if (steps.isEmpty) { ExecutionTrace(proofId.toString, proofId.toString, Nil) }
    else {
