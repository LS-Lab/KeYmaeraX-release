...
import edu.cmu.cs.ls.keymaerax.tactics.{TacticLibrary2, Tactics, TacticLibrary}
...
class KeYmaeraInitializer(env : {val db: DBAbstraction
  ...
  def initialize() {
    ...
    initTactic("keymaerax.default", "TacticLibrary.default", TacticKind.Tactic, env.tacticLibrary.default)
    initTactic("keymaerax.defaultNoArith", "TacticLibrary.defaultNoArith", TacticKind.Tactic, env.tacticLibrary.defaultNoArith)
    ...
    initTactic("dl.diffconstify", "TacticLibrary.diffConstifyT", TacticKind.PositionTactic, TacticLibrary.diffConstifyT)
