...
object CodeGenerator {
  ...
  /**
   * Returns a set of names (excluding names in `exclude` and interpreted functions) that are immutable parameters of
   * the expression `expr`.
   */
  def getParameters(expr: Expression, exclude: Set[BaseVariable]): Set[NamedSymbol] = StaticSemantics
    .symbols(expr)
    .filter({
  ...
  /**
   * Returns a set of names whose values are chosen nondeterministically in the program `expr` (empty if `expr` is not a
   * program).
   */
  ...
  def isInterpreted(f: NamedSymbol): Boolean = f match {
    case Function(_, _, _, _, interpretation) => interpretation.isDefined
    case _ => false
  }
}
...
trait CodeGenerator extends (Expression => (String, String)) {
  ...
  /**
   * Translate expression `expr` into code. Symbols in `stateVars` are treated as mutable states, all others are
   * constant parameters.
   */
  ...
  /**
   * Translate expression `expr` into code. Symbols in `stateVars` are treated as mutable states, all others are
   * constant parameters. `inputVars` is a subset of `stateVars` whose value is nondeterministically chosen in the
   * original program.
   */
  def apply(expr: Expression, stateVars: Set[BaseVariable], inputVars: Set[BaseVariable]): (String, String) =
    apply(expr, stateVars, inputVars, "")

  /**
   * Translate expression `expr` into code. Symbols in `stateVars` are treated as mutable states, all others are
   * constant parameters. `inputVars` is a subset of `stateVars` whose value is nondeterministically chosen in the
   * original program. The model name `modelName` is added to the file header comment.
   */
  def apply(
      expr: Expression,
