...
trait HTMLPrinter {
  ...
  val textTagRewritings = List("&" -> "&amp;", "<" -> "&lt;", ">" -> "&gt;", HTML_OPEN -> "<", HTML_CLOSE -> ">")
  ...
  def htmlElement(tag: String, body: String, clazz: Option[String] = None): String = htmlOpenTag(tag, clazz) + body +
    htmlEndTag(tag)

  /**
   * Replaces KeYmaeraX syntax with HTML characters (e.g., < becomes &lt;) and introduces opening/closing tags HTML
   * syntax.
   */
...
class UIAbbreviatingKeYmaeraXPrettyPrinter extends KeYmaeraXWeightedPrettyPrinter {
  protected override def pp(q: PosInExpr, term: Term): String = term match {
    ...
    case FuncOf(Function(n, i, _, _, Some(_)), arg) =>
      emit(q, n + i.map("_" + _).getOrElse("") + "(" + pp(q ++ 0, arg) + ")")
...
class UIAbbreviatingKeYmaeraXPrettierPrinter(margin: Int) extends KeYmaeraXPrettierPrinter(margin) {
  ...
  override def docOf(term: Term): Doc = term match {
    ...
    case FuncOf(Function(n, i, _, _, Some(_)), c: Pair) =>
      (Doc.text(n + i.map("_" + _).getOrElse("")) + Doc.lineBreak + docOf(c)).grouped
    case FuncOf(Function(n, i, _, _, Some(_)), c) =>
      (Doc.text(n + i.map("_" + _).getOrElse("")) + encloseText("(", Doc.lineBreak + docOf(c), ")").nested(2)).grouped
...
//  }
  override val fullPrinter: (Expression => String) = (e: Expression) =>
    throw new UnsupportedOperationException("UIKeYmaeraXPrettyPrinter.fullPrinter not implemented yet")
}
...
class UIKeYmaeraXAxiomPrettyPrinter extends KeYmaeraXWeightedPrettyPrinter with HTMLPrinter {
  ...
  private def printSpace(s: Space): String = s match {
    case AnyArg =>
      "⦅⦆" // s"(${HTML_OPEN}b$HTML_CLOSE${HTML_OPEN}i$HTML_CLOSE" + "x" + s"$HTML_OPEN/i$HTML_CLOSE$HTML_OPEN/b$HTML_CLOSE)"
  ...
  protected override def pp(q: PosInExpr, term: Term): String = emit(
    ...
    term match {
      ...
      case t: Power => wrapLeft(t, pp(q ++ 0, t.left)) + s"${HTML_OPEN}sup$HTML_CLOSE" +
  ...
  override val fullPrinter: (Expression => String) = (e: Expression) =>
    throw new UnsupportedOperationException("UIKeYmaeraXAxiomPrettyPrinter.fullPrinter not implemented yet")
}
