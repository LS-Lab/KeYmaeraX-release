...
import edu.cmu.cs.ls.keymaerax.hydra.RestApi.{completeRequest, completeResponse, database, standardCompletion, userPrefix}
...
object Proofs {
  ...
  val derivationInfo: SessionToken=>Route = (t : SessionToken) => path("proofs" / "user" / Segment / Segment / Segment / "derivationInfos" / Segment.?) { (userId, proofId, _, axiomId) => { pathEnd {
    get {
