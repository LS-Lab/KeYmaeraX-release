...
object CodeGen {
  ...
  def codegen(entry: ParsedArchiveEntry, interval: Boolean, outputFileName: String, head: String,
    ...
    val output = (new CGenerator(new CMonitorGenerator('resist)))(inputFormula, theVars, Set(), outputFileName)
  ...
  def codegenQuantitative(entry: ParsedArchiveEntry, outputFileName: String, head: String,
    ...
    val monitorCode = (new CGenerator(new CMonitorGenerator('resist)))(monitorProg, monitorStateVars, inputs, "Monitor")
