  ...
  override def resultingResponses(): List[Response] = {
    ...
    ToolProvider.tool(toolName) match {
      ...
      case _ =>
        new ErrorResponse(
          toolName +
            " failed to initialize. Please reselect the desired tool and double-check the configuration paths. Temporarily switched to fallback tools " +
            ToolProvider.tools().map(_.name).mkString(",")
        ) :: Nil
    }
  }
}
