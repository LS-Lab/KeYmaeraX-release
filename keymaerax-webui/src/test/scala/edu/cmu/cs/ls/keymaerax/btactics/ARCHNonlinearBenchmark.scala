...
class ARCHNonlinearBenchmark extends TacticTestBase {
  ...
  for(f <- benchmarks) {
    testNum += 1
    ignore should s"prove ${f.prettyString} (benchmark problem ${testNum})" in withMathematica(_ => {
      proveBy(f.asInstanceOf[Formula], TactixLibrary.implyR(1) & DifferentialTactics.ODE(1)).isProved shouldBe true
    })
  }
}
