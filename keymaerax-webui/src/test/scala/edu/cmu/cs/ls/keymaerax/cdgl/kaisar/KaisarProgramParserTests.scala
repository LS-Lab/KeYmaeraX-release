...
class KaisarProgramParserTests extends TacticTestBase {
  ...
  "id pattern parser" should "parse terminals" in {
    p("", pp.idPat(_)) shouldBe NoPat()
    p("x", pp.idPat(_)) shouldBe VarPat("x")
    p("x:p", pp.idPat(_)) shouldBe VarPat("x", Some(Variable("p")))
    p("*", pp.idPat(_)) shouldBe WildPat()
  }

  it should "parse tuples" in {
    p("(x,y)", pp.idPat(_)) shouldBe TuplePat(List(VarPat("x"), VarPat("y")))
    p("(x,y,z)", pp.idPat(_)) shouldBe TuplePat(List(VarPat("x"), VarPat("y"), VarPat("z")))
  }
  ...
  it should "parse proof instantiation" in {
    p("(X 1)", kp.proofTerm(_)) shouldBe ProofInst(ProofVar("X"), Number(1))
  }

  // selector parser
  "selector parser" should "parse forward selector" in {
    p("(and X Y)", kp.proofTerm(_)) shouldBe ProofApp(ProofApp(ProofVar("and"), ProofVar("X")), ProofVar("Y"))
  }

  it should "parse pattern selector" in {
    p("*", kp.proofTerm(_)) shouldBe PatternSelector(wild)
  }
  // method parser
  "method parser" should "parse terminal methods" in {
    p("rcf", pp.method(_)) shouldBe RCF()
    p("auto", pp.method(_)) shouldBe Auto()
    p("prop", pp.method(_)) shouldBe Prop()
  }
  ...
  it should "parse by-proof" in {
    p("proof show true by auto end", pp.method(_)) shouldBe ByProof(Proof(List[Statement](Assert(NoPat(), True, Auto()))))
  }


  // proof-statement parser
  "proof statement parser" should "parse assumption" in {
    p("?x:true;", pp.statement(_)) shouldBe Assume(VarPat("x"), True)
  }
