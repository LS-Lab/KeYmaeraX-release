...
class ExampleGameTests extends TacticTestBase {
  ...
  val cake: Program = Compose(AssignAny(vx),Compose(Test(cakeTest),
    Compose(Assign(vy, Minus(Number(1), vx)),
  ...
  "cake" should "be safe" in withMathematica { _ =>
    ...
    val M =
      BComposeI(BRandomI(vx,BComposeI(BTestI(cakeTest, BComposeI(BAssignI(Assign(vy, Minus(Number(1), vx)),
        BDualI(OrE(Compare(vx, Number(0.5), QE(Greater(vk, Number(0)), Hyp(2))),
  ...
  it should "be live" in withMathematica { _ =>
    ...
    val M =
      DComposeI(DRandomI(Assign(vx, Number(0.5)),DComposeI(DTestI(McakeTest, DComposeI(DAssignI(Assign(vy, Minus(Number(1), vx)),
        DDualI(
          BChoiceI(BComposeI(BAssignI(Assign(va,vx),BAssignI(Assign(vd,vy), Mqe)))
  ...
  val (vv, v0, v1, vt, vA, vD, vg, x0, x2) = (Variable("v"), Variable("v", Some(0)), Variable("v", Some(1)), Variable("t"), Variable("A"), Variable("D"), Variable("g"), Variable("x", Some(0)), Variable("x", Some(2)))
  val pushPullODE: ODESystem = ODESystem(DifferentialProduct(AtomicODE(DifferentialSymbol(vx), vv), AtomicODE(DifferentialSymbol(vv), Plus(va,vd))))
  val pushPull: Loop =
    Loop(Compose(Choice(Assign(va, vA), Assign(va, Neg(vA))),
  ...
  "push-pull cart" should "be safe" in withMathematica { _ =>
    ...
    val M =
      BRepeatI(QE(j, and(3))
        , BComposeI(
           BChoiceI(
            BAssignI(Assign(va, vA),
            BComposeI(BDualI(DChoiceIR(Assign(vd, vD), DAssignI(Assign(vd, Neg(vD)),
                BSolve(pushPullODE, j, Mqe))))))
          , BAssignI(Assign(va, Neg(vA)),
            BComposeI(BDualI(DChoiceIL(Assign(vd, Neg(vD)), DAssignI(Assign(vd, vD),
                BSolve(pushPullODE, j, Mqe))))))))
        , QE(j2, Hyp(0))
        , pushPull.child)
  ...
  it should "be live" in withMathematica { _ =>
    ...
    val M = DRepeatI(metric,
      QE(j, and(G))
    , DComposeI(
        DChoiceIL(Assign(va, Neg(vA)),
        DAssignI(Assign(va, vA),
            DComposeI(DDualI(
              BChoiceI(BAssignI(Assign(vd, vD), DSolve(pushPullODE, odePost, Mdur, Mdc, Mqe))
