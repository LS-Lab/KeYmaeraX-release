...
class ContextTests extends FlatSpec with Matchers with BeforeAndAfterEach {
  ...
  "Different context construction" should "lead to the same context" in {
    ...
    ctx1 shouldBe ctx2
    ctx3 shouldBe ctx4
    ctx3 shouldBe ctx2 //reverse order requires defining [[GuardedContext.equals]] similar to [[ReplacementContext.equals]]
  }

  "Contexts" should "FEATURE_REQUEST: not fail when applied" in {
    val f = "[x:=2;]x = 0".asFormula
    val ctx1 = Context("[x:=2;]⎵".asFormula)
    val (ctx2, e) = Context.at(f, PosInExpr(1::Nil))
    ctx2(e) shouldBe f
    ctx1(e) shouldBe f
    val (ctx3, e2) = Context.at(f, PosInExpr(1 :: 0 :: Nil))
    val ctx4 = Context("[x:=2;].=0".asFormula)
    ctx3(e2) shouldBe f
    ctx4(e2) shouldBe f
  }
}