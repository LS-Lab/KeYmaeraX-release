...
class Etcs extends TacticTestBase {
  ...
  it should "synthesize a model monitor from essentials" in withMathematica { tool =>
    ...
    foResult.subgoals should have size 1
    foResult.subgoals.head.ante shouldBe empty
    foResult.subgoals.head.succ should contain only "m-z<=v^2/(2*b)+(A/b+1)*(A/2*ep^2+ep*v)&(((ep=0&ep=tpost())&apost()+b=0)&((v=0&vpost()=0)&1/2*b*(ep+-1*0)^2+zpost()=z|(b*ep+vpost()=b*0+v&1/2*b*(ep+-1*0)^2+0*v+zpost()=ep*v+z)&v>0)|(ep>0&apost()+b=0)&(((b*0+v=b*tpost()+vpost()&b*(0+-1*tpost())^2+2*(0*v+-1*tpost()*v+-1*z+zpost())=0)&v>0)&((ep=tpost()&b<=(ep+-1*0)^-1*v|0=tpost())|(tpost() < ep&0 < tpost())&b*tpost()<=b*0+v)|((0=tpost()&v=0)&vpost()=0)&b*(0+-1*tpost())^2+2*zpost()=2*z))|m-z>=v^2/(2*b)+(A/b+1)*(A/2*ep^2+ep*v)&((ep=0&ep=tpost())&((A=0&apost()=0)&((v=0&vpost()=0)&z=zpost()|(v=vpost()&ep*v+z=0*v+zpost())&v>0)|(A=apost()&A>0)&((v=0&vpost()=0)&zpost()=1/2*A*(ep+-1*0)^2+z|(A*ep+v=A*0+vpost()&0*v+zpost()=1/2*A*(ep+-1*0)^2+ep*v+z)&v>0))|ep>0&(((A=0&apost()=0)&((v=0&vpost()=0)&z=zpost()|(v=vpost()&0*v+zpost()=tpost()*v+z)&v>0))&((ep=tpost()|0=tpost())|0 < tpost()&tpost() < ep)|(A=apost()&A>0)&(((v=0&A*0+vpost()=A*tpost())&zpost()=1/2*A*(0+-1*tpost())^2+z|(A*0+vpost()=A*tpost()+v&0*v+zpost()=1/2*A*(0+-1*tpost())^2+tpost()*v+z)&v>0)&(ep=tpost()|0 < tpost()&tpost() < ep)|0=tpost()&((v=0&vpost()=0)&zpost()=1/2*A*(0+-1*tpost())^2+z|(A*0+vpost()=A*tpost()+v&0*v+zpost()=1/2*A*(0+-1*tpost())^2+tpost()*v+z)&v>0))))".asFormula
  }
