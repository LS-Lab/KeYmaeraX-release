...
//case it if 110 until 170 contains it => True
        ...
        case it if 290 until 300 contains it => DifferentialFormula(nextF(vars, n-1, dotTs, dotFs))
  ...
  private val game: Boolean = try {Dual(AssignAny(Variable("x"))); true} catch {case ignore: IllegalArgumentException => false }
    ...
    def nextP(vars : IndexedSeq[Variable], n : Int, dotTs: Boolean = false, dotFs: Boolean = false) : Program = {
        ...
        r match {
          ...
          case it if (190 until 220 contains it) && game => Dual(nextP(vars, n-1, dotTs, dotFs))
