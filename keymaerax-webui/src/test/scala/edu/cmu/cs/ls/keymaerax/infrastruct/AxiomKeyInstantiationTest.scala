...
class AxiomKeyInstantiationTest extends SystemTestBase with BeforeAndAfterAll {
  ...
  it should "instantiate [:=] assign 1" in { matchKey(Ax.assignbAxiom, "[x:=z;]x^2>=9".asFormula) }

  it should "instantiate [:=] assign equality 1" in { matchKey(Ax.assignbeq, "[x:=1;][x:=2;]x>0".asFormula) }

  it should "instantiate [:=] assign equality 2" in { matchKey(Ax.assignbeq, "[x:=1;][{x:=x+1;}*]x>0".asFormula) }

  it should "instantiate [:=] assign equality 3" in { matchKey(Ax.assignbeq, "[z1:=1;]<z1:=*;>1>=1".asFormula) }

  it should "instantiate [:=] assign equality 4" in {
    matchKey(Ax.assignbeq, "[z1:=98+1;][z1:=*;][?true;]true".asFormula)
  }

  it should "instantiate [++]" in { matchKey(Ax.choiceb, "[x:=x+1;++{x:=0;{y'=-2}}]x>=y".asFormula) }

  it should "instantiate DI" in { matchKey(Ax.DIequiv, "[{x'=5&x>9}]x>=10".asFormula) }

  it should "instantiate DC" in { matchKey(Ax.DC, "[{x'=5&x>9}]x>=10".asFormula) }

  it should "instantiate DG" in { matchKey(Ax.DGa, "[{x'=5&x>9}]x>=10".asFormula) }
  ...
  it should "instantiate DG crazy 3" in { matchKey(Ax.DGa, "[{z1'=1&<z2:=z1;>\\exists z1 z1>=z2}]z1>=5".asFormula) }

  it should "instantiate DS&" in { matchKey(Ax.DS, "[{y1'=z0+31&q(y1)}]\\exists z1 z1>y1".asFormula) }
  ...
  it should "instantiate [:=] assign 1" in { matchDirect(Ax.assignbAxiom, "[x:=z;]x^2>=9 <-> z^2>=9".asFormula) }
  it should "instantiate [:=] assign 2" in {
    matchDirect(Ax.assignbAxiom, "[x:=2*x+1;]x^3>=9*x <-> (2*x+1)^3>=9*(2*x+1)".asFormula)
  }
  it should "instantiate [:=] assign 3" in { matchDirect(Ax.assignbAxiom, "[x:=x+1;]x^2>=9 <-> (x+1)^2>=9".asFormula) }
  ...
  it should "instantiate [*]" in {
    matchDirect(
      Ax.iterateb,
      "[{x:=x+1;{x:=0;{y'=-2}}}*]x>=y <-> x>=y & [x:=x+1;{x:=0;{y'=-2}}][{x:=x+1;{x:=0;{y'=-2}}}*]x>=y".asFormula,
    )
  }
  ...
  it should "[:=] self assign" in { matchDirect(Ax.selfassignb, "[x:=x;]x>0 <-> x>0".asFormula) }

  it should "[:=] assign 1" in { matchDirect(Ax.assignbAxiom, "[y:=1;][y:=2;]y>0 <-> [y:=2;]y>0".asFormula) }

  it should "[:=] assign 2" in { matchDirect(Ax.assignbAxiom, "[y:=1;][y:=2+y;]y>0 <-> [y:=2+1;]y>0".asFormula) }

  it should "[:=] assign 3" in { matchDirect(Ax.assignbAxiom, "[y:=y+1;]y>0 <-> y+1>0".asFormula) }
  ...
  it should "[:=] self assign 1" in { matchDirect(Ax.selfassignb, "[x:=x;]x>0<->x>0".asFormula) }
...
//  }
  ...
  "Random Renamed Instance Unification optimistic" should
    "FEATURE_REQUEST: instantiate keys of all axioms to random schematic instantiations" taggedAs TodoTest in
    instantiateRandomKey()
