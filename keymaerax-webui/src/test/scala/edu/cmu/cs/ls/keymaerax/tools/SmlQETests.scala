...
class SmlQETests extends TacticTestBase {
  ...
  abstract class BenchmarkResult {
    ...
    def defaultInfoPrinter: Map[String, String] => String = info =>
      "," + info.getOrElse("logic", "unknown") + "," + info.getOrElse("status", "unknown") + "," +
        info.getOrElse("boundvars", "unknown") + "," + info.getOrElse("freevars", "unknown")
  }
  case class BenchmarkResultEntry(
      name: String,
      index: Long,
      status: String,
      result: String,
  ...
  case class BenchmarkResultList(
      file: String,
      fileIdx: Long,
      overallDuration: Long,
      results: List[BenchmarkResultEntry],
  ) extends BenchmarkResult {
    override def toString: String = results.mkString("\r\n")
    override def toCsv(infoPrinter: Map[String, String] => String = defaultInfoPrinter): String = results
      .map(_.toCsv(infoPrinter))
      .mkString("\r\n")
  }
  ...
  private def listFiles(dir: File, ending: String): List[File] = {
    dir.listFiles().filter(_.isDirectory).flatMap(listFiles(_, ending)).toList ++
      dir
  ...
  private def closed(fml: Formula): Formula = StaticSemantics
    .freeVars(fml)
    .toSet
    .toList
    .foldRight(fml)({ case (v, f) => Forall(v :: Nil, f) })
...
//            }
      ...
      exprs.filter(_.isInstanceOf[Formula]).map(_.asInstanceOf[Formula]) match {
        case Not(fml) :: Nil =>
          ...
          val content = DefaultSMTConverter.generateSMT(fml) match {
            case (varDec, fmlContent) => varDec + "(assert " + fmlContent + ")\n" +
      ...
      (logLines: List[String]) => {
        ...
        if (replIdx + 1 >= logLines.length) BenchmarkResultEntry(_, _, "timeout", "", TIMEOUT * 1000, _)
          ...
          case e if e.startsWith("+++ Error") =>
            BenchmarkResultEntry(_, _, "error", logLines.mkString, TIMEOUT * 1000, _)
    ...
    convertResults(
      ...
      (logLines: List[String]) =>
        logLines.headOption match {
          ...
          case Some(e) if e.startsWith("ERROR") =>
            BenchmarkResultEntry(_, _, "error", logLines.mkString, TIMEOUT * 1000, _)
  ...
  it should "convert SMT-RAT QE results" in {
    convertResults(
      "smtrat",
      (logLines: List[String]) =>
        logLines.find(_.startsWith("Equivalent Quantifier-Free Formula:")) match {
          case None => BenchmarkResultEntry(_, _, "timeout", "", TIMEOUT * 1000, _)
          case Some(r @ "Equivalent Quantifier-Free Formula: true") =>
            BenchmarkResultEntry(_, _, "aproved", "TRUE", TIMEOUT * 1000, _)
          case Some(r @ "Equivalent Quantifier-Free Formula: false") =>
            BenchmarkResultEntry(_, _, "disproved", "FALSE", TIMEOUT * 1000, _)
          case Some(_) => BenchmarkResultEntry(_, _, "undefined", logLines.mkString, TIMEOUT * 1000, _)
        },
    )
  }
          ...
          logLines
            ...
            .map({ case (l, i) =>
              ...
              result match {
                ...
                case "False" =>
                  BenchmarkResultEntry(name, i, "disproved", l, TIMEOUT * 1000, (duration.toDouble * 1000).toLong)
                case "$Aborted" =>
                  BenchmarkResultEntry(name, i, "timeout", l, TIMEOUT * 1000, (duration.toDouble * 1000).toLong)
                case r =>
                  BenchmarkResultEntry(name, i, "simplified", r, TIMEOUT * 1000, (duration.toDouble * 1000).toLong)
              }
            }),
        ),
    )
  ...
  private def runMathematicaExperiments(
      tool: Mathematica,
      ending: String,
      converter: File => (List[Expression], Map[String, String]),
  ): Unit = {
    // @note set working directory in test case configuration
    ...
    for (c <- categories) {
      ...
      Table("filehandle", smtFiles.zipWithIndex: _*).forEvery({ case (smtFile, i) =>
        ...
        val result = out match {
          case Right(r) =>
            val (message, result) =
              if (r.isProved) r.conclusion match {
          ...
          case Left(_: TimeoutException) =>
            BenchmarkResultEntry(smtFile.getName, i, "timeout", "", TIMEOUT * 1000, dur / 1000000, info = infos)
          case Left(ex) => BenchmarkResultEntry(
              smtFile.getName,
              i,
              "error",
              ex.getMessage,
  ...
  private def runCmd(cmd: String, args: Seq[String], timeout: Long = -1): Either[String, (Int, String)] = {
    ...
    try {
      ...
      if (exitVal == 0) Left(result) else Right(exitVal -> result)
