...
class SomeUnifyUSCalculusTests extends TacticTestBase with PrivateMethodTester {
    ...
    UnificationMatch(fact, goal) shouldBe RenUSubst(
      (DifferentialProgramConst("c", Except(y :: Nil)), AtomicODE(DifferentialSymbol(Variable("t")), Number(1))) ::
        ...
        (
          ...
          Forall(Seq(Variable("x")), GreaterEqual(Power(Variable("x"), Number(2)), Number(0))),
        ) :: Nil
    )
  }
  ...
  it should "unify DG with universal postcondition" ignore withTactics {
    ...
    UnificationMatch(fact, goal) shouldBe RenUSubst(
      (DifferentialProgramConst("c", Except(y :: Nil)), AtomicODE(DifferentialSymbol(Variable("t")), Number(1))) ::
        ...
        (UnitFunctional("b", Except(y :: Nil), Real), Number(1)) :: (y, Variable("x")) :: Nil
    )
  }
  ...
  it should "prove unify DG with universal postcondition" ignore withMathematica { qeTool =>
    ...
    val fact: Sequent = Sequent(
      IndexedSeq[Formula](),
      IndexedSeq[Formula]("<{c{|y_|}&q(|y_|)}>p(|y_|)<->\\forall y_ <{c{|y_|},y_'=b(|y_|)&q(|y_|)}>p(|y_|)".asFormula),
    )
    pv.conclusion shouldBe fact
    val sequent: Sequent = Sequent(
      IndexedSeq[Formula](),
  ...
  "UnifyUSCalculus" should "rewrite implications" in withMathematica { qeTool =>
    ...
    val pr = proveBy(
      ...
      DebuggingTactics.print("Initial") &
        ...
        DebuggingTactics
          .print("After <-> Succ contextual CEats") & CEat(impl3, ctx1)(AntePosition(3, 1 :: Nil)) & // Equiv CEat
        ...
        CEat(impl5)(SuccPosition(4, 0 :: Nil)) & DebuggingTactics.print("After succedent non-toplevel CEats") &
        ...
        DebuggingTactics.print("After antecedent non-toplevel CEats in context") & nil, // CEat(impl,ctx)
    )

    (pr.subgoals.head.ante == pr.subgoals.head.succ) shouldBe true
    pr.subgoals.head.ante shouldBe IndexedSeq(
      "F_()^2=0".asFormula,
      "F_()^2=0".asFormula,
      "P()->Q_()^2=0->F_()^2=0".asFormula,
      "Q_()^2=0->F_()^2=0".asFormula,
      "P()->Q_()^2=0->F_()^2=0".asFormula,
    )
