...
class UniformSubstitutionTests extends TacticTestBase with PrivateMethodTester {
  ...
  def fnof(name: String) = FuncOf(Function(name, None, Unit, Real), Nothing)
  ...
  "Uniform substitution of (x,y)(y,x) |-> x-y" should "be y-x" in {
    val s = create(SubstitutionPair(fnof("x"), "y".asTerm), SubstitutionPair(fnof("y"), "x".asTerm))
...
//    val s = FastUSubst(List(SubstitutionPair("x()".asTerm, "y".asTerm), SubstitutionPair(fnof("y"), "t".asTerm)))
...
//    val s = FastUSubst(List(SubstitutionPair("x()".asTerm, "y".asTerm), SubstitutionPair(fnof("y"), "x".asTerm)))
//    a [SubstitutionClashException] should be thrownBy applySubstitutionT(s, Set.empty, Set(V("y")),"x()/y".asTerm)
    // TODO USUbst case
  }
  ...
  "Uniform substitution of (x,y)(f(.),.+1) |-> f(x)" should "be y+1" in {
    ...
    val s = create(
      SubstitutionPair(fnof("x"), "y".asTerm),
      SubstitutionPair(FuncOf(f, DotTerm()), Plus(DotTerm(), Number(1))),
    )
  ...
  "Uniform substitution of f(x)" should "be y+z+1 with (x,y+z)(f(.),.+1)" in {
    val f = Function("f", None, Real, Real)
    val s = create(
      SubstitutionPair(fnof("x"), "y+z".asTerm),
      SubstitutionPair(FuncOf(f, DotTerm()), Plus(DotTerm(), "1".asTerm)),
    )
  ...
  "Uniform substitution of  p(x)" should "be [x:=y+1;]x>0 with (x,y)(p(.),[x:=.+1;]x>0)" in {
    ...
    val s = create(
      ...
      SubstitutionPair(
        PredOf(p, DotTerm()),
        Box(Assign("x".asVariable, Plus(DotTerm(), Number(1))), Greater("x".asTerm, Number(0))),
      ),
    )
  ...
  ignore should "be [x:=y+1;]x>0 with (x,y)(p(x),[x:=x+1;]x>0)" in {
    val p = Function("p", None, Real, Bool)
    val s = create(
      SubstitutionPair("x".asTerm, "y".asTerm),
      SubstitutionPair(
        PredOf(p, "x".asTerm),
        Box(Assign("x".asVariable, Plus("x".asTerm, Number(1))), Greater("x".asTerm, Number(0))),
      ),
    )
  ...
  "Uniform substitution of (x,1)(y,x) |-> g(x)" should "be g(1)" in {
    ...
    val s = create(SubstitutionPair(fnof("x"), Number(1)), SubstitutionPair(fnof("y"), "x".asTerm))
    s(FuncOf(g, fnof("x"))) should be(FuncOf(g, Number(1)))
  }

  "Uniform substitution of (x,1)(y,x) |-> g(x) where {x} is bound" should "not be permitted" in {
    val g = Function("g", None, Real, Bool)
//    val s = FastUSubst(List(SubstitutionPair(fnof("y"), "x".asTerm)))
//    a [SubstitutionClashException] should be thrownBy applySubstitutionT(s, Set.empty, Set(V("x")), FuncOf(g, fnof("y")))
    // TODO USUbst case
  }
  ...
  "Uniform substitution of (x,y)(y,x) |-> g(x)" should "be g(y)" in {
    val g = Function("g", None, Real, Bool)
    val s = create(SubstitutionPair(fnof("x"), "y".asTerm), SubstitutionPair(fnof("y"), "x".asTerm))
    s(FuncOf(g, fnof("x"))) should be(FuncOf(g, "y".asTerm))
  }
  ...
  "Uniform substitution of (x,y)(y,x) |-> f(x)=g(y)" should "be f(y)=g(x)" in {
    ...
    val g = Function("g", None, Real, Bool)
    val s = create(SubstitutionPair(fnof("x"), "y".asTerm), SubstitutionPair(fnof("y"), "x".asTerm))
    s(Equal(FuncOf(f, fnof("x")), FuncOf(g, fnof("y")))) should be(Equal(FuncOf(f, "y".asTerm), FuncOf(g, "x".asTerm)))
  }
  ...
  "Uniform substitution of (x,y) |-> x" should "be y" in {
    val s = create(SubstitutionPair(fnof("x"), "y".asTerm))
    s(fnof("x")) should be("y".asTerm)
  }

  "Uniform substitution of (x,y) |-> x where {y} is bound" should "not be permitted" in {
//    val s = FastUSubst(Seq(new SubstitutionPair(fnof("x"), "y".asTerm)))
//    a [SubstitutionClashException] should be thrownBy applySubstitutionT(s, Set.empty, Set(V("y")),fnof("x"))
    // TODO USUbst case
  }
...
//    val s = FastUSubst(List(SubstitutionPair(fnof("x"), "1".asTerm)))
...
//    val s = FastUSubst(List(SubstitutionPair(fnof("x"), "t".asTerm)))
    // TODO not yet supported, hence exception
...
//      s(Box(Choice(ProgramConst("a"), ProgramConst("b")), PredOf(Function("p", None, Unit, Bool), Nothing)))
  ...
  it should "work when cascaded" in {
    val s = create(
      SubstitutionPair(ProgramConst("a"), "x:=2;".asProgram),
      SubstitutionPair(ProgramConst("b"), "y:=3;".asProgram),
    )
    val t = create(SubstitutionPair(UnitPredicational("p", AnyArg), "x*y>5".asFormula))

    t(s(Box(Choice(ProgramConst("a"), ProgramConst("b")), UnitPredicational("p", AnyArg)))) should be(
  ...
  "Uniform substitution of (y,x+y) |-> x:=y z:=x+y" should "not be permitted" in {
    val s = create(SubstitutionPair(fnof("y"), "x+y".asTerm))
  ...
  "Uniform substitution of (x,1) |-> x:=x+1 z:=x" should "be x:=1+1 z:=x" in {
    val s = create(SubstitutionPair(fnof("x"), "1".asTerm))
  ...
  "Uniform substitution of (a,x:=2),(b,?x>1) |-> [a b]1>0" should "be [x:=2 ?x>1]1>0" in {
    ...
    s(Box(Compose(ProgramConst("a"), ProgramConst("b")), "1>0".asFormula)) should be("[x:=2; ?x>1;]1>0".asFormula)
  }

  "Uniform substitution of (x,1) |-> {x:=1 ++ x:=x+1 ++ z:=x};{x:=1 ++ x:=x+1 ++ z:=x};" should "be {x:=1 ++ x:=1+1 ++ z:=1};{x:=1 ++ x:=x+1 ++ z:=x};" in {
//    val s = FastUSubst(Seq(new SubstitutionPair(fnof("x"), "1".asTerm)))
//    // TODO not yet supported, hence exception
//    a [SubstitutionClashException] should be thrownBy s("{x:=1 ++ x:=x()+1 ++ z:=x()};{x:=1 ++ x:=x+1 ++ z:=x};".asProgram)// should be ("{x:=1 ++ x:=1+1 ++ z:=1};{x:=1 ++ x:=x+1 ++ z:=x};".asProgram)
  ...
  "Uniform substitution of (x(),1)(y(),t) |-> {x:=x+y}*;" should "be {x:=x+y;}*" in {
    val s = create(SubstitutionPair(fnof("x"), "1".asTerm), SubstitutionPair(fnof("y"), "t".asTerm))
  ...
  "Uniform substitution of {x:=x+y}*;" should "not be permitted with (x(),1)(y(),x)" in {
    val s = create(SubstitutionPair(fnof("x"), "1".asTerm), SubstitutionPair(fnof("y"), "x".asTerm))
    a[SubstitutionClashException] should be thrownBy s("{x:=x()+y();}*".asProgram)
  }
  ...
  "Uniform substitution of (x,y)(y,x+y) |-> x:=y" should "be x:=x+y" in {
    val s = create(SubstitutionPair(fnof("x"), "y".asTerm), SubstitutionPair(fnof("y"), "x+y".asTerm))
    s("x:=y();".asProgram) should be("x:=x+y;".asProgram)
  }
...
//    a [SubstitutionClashException] should be thrownBy applySubstitution(s, Set.empty, Set.empty, "t'=x;".asProgram)
  ...
  "Uniform substitution of (x,t) |-> t'=x;" should "not be permitted" in {
    val s = create(SubstitutionPair(fnof("x"), "t".asTerm))
...
//    val s = FastUSubst(Seq(SubstitutionPair("t".asTerm, "1".asTerm), SubstitutionPair(fnof("x"), "y".asTerm)))
...
//    a [SubstitutionClashException] should be thrownBy applySubstitution(s, Set.empty, Set.empty, "t'=x() & x()*y+t+1>0;".asProgram)
  ...
  "Uniform substitution of (x,y)(y,y+1) |-> \\forall x. x>y" should "be forall x. x>y+1" in {
    val s = create(SubstitutionPair(fnof("x"), "y".asTerm), SubstitutionPair(fnof("y"), "y+1".asTerm))
  ...
  "Uniform substitution of (x,y)(p(t),t<1) |-> p(x)" should "be y<1" in {
    ...
    val s = create(
      SubstitutionPair(fnof("x"), "y".asTerm),
      SubstitutionPair(PredOf(p, DotTerm()), Less(DotTerm(), Number(1))),
    )
  ...
  "Uniform substitution of (x,y)(p(t),t>x) |-> p(x)" should "be y>x" in {
    ...
    val s = create(
      SubstitutionPair(fnof("x"), "y".asTerm),
      SubstitutionPair(PredOf(p, DotTerm()), Greater(DotTerm(), "x".asTerm)),
    )
  ...
  "Uniform substitution of (x,1)(y,x) |-> q(x)" should "be q(1)" in {
    ...
    val s = create(SubstitutionPair(fnof("x"), Number(1)), SubstitutionPair(fnof("y"), "x".asTerm))
    s(PredOf(q, fnof("x"))) should be(PredOf(q, Number(1)))
  }

  "Uniform substitution of (x,y)(y,x) |-> q(x)" should "be q(y)" in {
    val q = Function("q", None, Real, Bool)
    val s = create(SubstitutionPair(fnof("x"), "y".asTerm), SubstitutionPair(fnof("y"), "x".asTerm))
    s(PredOf(q, fnof("x"))) should be(PredOf(q, "y".asTerm))
  }
  ...
  "Uniform substitution of (x,y)(y,x) |-> p(x)=q(y)" should "be p(y)=q(x)" in {
    ...
    val q = Function("q", None, Real, Bool)
    val s = create(SubstitutionPair(fnof("x"), "y".asTerm), SubstitutionPair(fnof("y"), "x".asTerm))
    s(Equal(FuncOf(p, fnof("x")), FuncOf(q, fnof("y")))) should be(Equal(FuncOf(p, "y".asTerm), FuncOf(q, "x".asTerm)))
  }
  ...
  "Uniform substitution of (x,y) |-> (y,x+y) |-> x=y" should "be y=y+y" in {
    val s = create(SubstitutionPair(fnof("x"), "y".asTerm))
    val s1 = create(SubstitutionPair(fnof("y"), "x()+y".asTerm))
  ...
  "Uniform substitution of [y:=t;]p(y) <-> p(t)" should "[y:=t;]y>0 <-> z>0" in {
    ...
    val s =
      create(SubstitutionPair(fnof("t"), "z".asTerm), SubstitutionPair(p(DotTerm()), Greater(DotTerm(), Number(0))))
  ...
  "Uniform substitution (p(.) |-> .>0),z |-> 2) of [x:=2y+1;]p(3x+z)" should "[x:=2y+1;]3x+2>0" in {
    def p(x: Term) = PredOf(Function("p", None, Real, Bool), x)
    val s =
      create(SubstitutionPair(fnof("z"), "2".asTerm), SubstitutionPair(p(DotTerm()), Greater(DotTerm(), Number(0))))
  ...
  "Uniform substitution (p(.) |-> .>0),z |-> x+2) of [x:=2y+1;]p(3x+z)" should "throw a clash exception" in {
    def p(x: Term) = PredOf(Function("p", None, Real, Bool), x)
    val s =
      create(SubstitutionPair(fnof("z"), "2+x".asTerm), SubstitutionPair(p(DotTerm()), Greater(DotTerm(), Number(0))))
...
//  "Uniform substitution of p(x)"
  ignore should "alpha rename x to any other variable in modalities" in {
    val s = create(SubstitutionPair(PredOf(Function("p", None, Real, Bool), "x".asTerm), "[x:=2;]x>5".asFormula))
    s(PredOf(Function("p", None, Real, Bool), "y".asTerm)) should be("[y:=2;]y>5".asFormula)
    val t = create(SubstitutionPair(PredOf(Function("p", None, Real, Bool), "x".asTerm), "[x:=y;][x:=z;]x>5".asFormula))
    t(PredOf(Function("p", None, Real, Bool), "y".asTerm)) should be("[y:=z;]y>5".asFormula)
    val u = create(
      SubstitutionPair(PredOf(Function("p", None, Real, Bool), "x".asTerm), "[x:=y;][x:=z;][{x:=x+1;}*;]x>5".asFormula)
    )
    u(PredOf(Function("p", None, Real, Bool), "y".asTerm)) should be("[y:=z;][{y:=y+1;}*;]y>5".asFormula)

    val v = create(SubstitutionPair(PredOf(Function("p", None, Real, Bool), "x".asTerm), "<x:=2;>x>5".asFormula))
  ...
  ignore should "not alpha rename to arbitrary terms in modalities" in {
    val s = create(SubstitutionPair(PredOf(Function("p", None, Real, Bool), "x".asTerm), "[x:=2;]true".asFormula))
   ...
   * Andre's tests
   */
  def sToT(s: String, t: String) =
    create(SubstitutionPair(if (s.endsWith("()")) fnof(s.substring(0, s.length - 2)) else s.asTerm, t.asTerm))
...
//      (sToT("x", "5"), "[{x:=0 ++ z:=z}; a:=x;]1>0".asFormula) ::         // subst(a:=x): x (maybe) bound by {x:=0 ++ ...}
      (
        sToT("b()", "z+1"),
        "[{x:=0; ++ z:=z;}; a:=b();]1>0".asFormula,
      ) :: // subst(a:=b): z free in z+1 but (maybe) bound by {... ++ z:=z}
...
//      (sToT("z", "1"), "[x:=x+1 ++ x:=x+z;z:=z-1]x>z".asFormula) ::       // subst(x>z): z (maybe) bound by {... ++ z:=z+1}
        (
          sToT("z()", "2*x"),
          "[x:=x+1; ++ x:=x+z;z:=z()-1;]x>0".asFormula,
        ) :: // subst(z:=z-1): x free in 2*x but bound by x:=x+z
        (
          sToT("z()", "2*x"),
          "[x:=x+1; ++ x:=x+z();]x>z()".asFormula,
        ) :: // subst(x>z): x free in 2*x but mustbe (and thus maybe) bound by {x:=x+1 ++ x:=x+z;...}
        (
          sToT("a()", "2*x"),
          "[{x:=x+1; ++ y:=5;}; z:=x-a();]x>0".asFormula,
        ) :: // subst(z:=x-a): x free in 2*x but (maybe) bound by {x:=x+1 ++ ...}
        (
          sToT("a()", "2*x"),
          "[{x:=x+1; ++ ?1>0;}; z:=x-a();]x>0".asFormula,
        ) :: // subst(z:=x-a): x free in 2*x but (maybe) bound by {x:=x+1 ++ ...}
//      (sToT("x", "2"), "[{x:=x+1;}*; z:=x+z]1>0".asFormula) ::            // subst(z:=x+z): x (maybe) bound by {x:=x+1}*
        (
          sToT("a()", "x"),
          "[{x:=x+1;}*; z:=a();]1>0".asFormula,
        ) :: // subst(z:=a): x free in x but maybe bound by {x:=x+1}*
        (
          sToT("a()", "x"),
          "[x:=x+1; z:=a();]1>0".asFormula,
        ) :: // subst(z:=a): x free in x but mustbe (and thus maybe) bound by x:=x+1
//      (sToT("x", "a"), "[{x:=x+1;}*; x:=x+1]1>0".asFormula) ::            // subst(x:=x+1): x (maybe) bound by {x:=x+1}*
  ...
  ignore /*"Uniform substitution of mustbe bound"*/ should "be same as input" in {
    // @TODO variable substitution not yet supported
    val cases = (sToT("x", "5"), "[{x:=0; ++ z:=z;}]1>0".asFormula) :: // x no free occurrence
...
//        Nil
    val cases =
      (sToT("z()", "1"), "[x:=x+1; ++ x:=x+z();z:=z()-1;]x>0".asFormula, "[x:=x+1; ++ x:=x+1;z:=1-1;]x>0".asFormula) ::
        (
          sToT("x()", "2"),
          "[x:=x()+1; ++ x:=x()+z;z:=z-1;]x>0".asFormula,
          "[x:=2+1; ++ x:=2+z;z:=z-1;]x>0".asFormula,
        ) ::
        (
          sToT("x()", "2"),
          "[x:=x()+1; ++ x:=x()+z;z:=z-1;]x>z".asFormula,
          "[x:=2+1; ++ x:=2+z;z:=z-1;]x>z".asFormula,
        ) ::
        (sToT("z()", "2"), "[{x:=x+1;}*; z:=x+z();]x>z".asFormula, "[{x:=x+1;}* z:=x+2;]x>z".asFormula) ::
        (sToT("x()", "a"), "[x:=x()+1; z:=a;]1>0".asFormula, "[x:=a+1; z:=a;]1>0".asFormula) ::
        (sToT("x()", "a"), "[x:=x()+1; {x:=x+1;}*]1>0".asFormula, "[x:=a+1; {x:=x+1;}*]1>0".asFormula) ::
        (
          sToT("z()", "-z"),
          "[{x:=x+1; ++ x:=x+z();}; z:=x-1;]x>z".asFormula,
          "[{x:=x+1; ++ x:=x+-z;} z:=x-1;]x>z".asFormula,
        ) ::
        Nil
  ...
  "Substitution in Barcan" should "should illustrate why axiom Barcan is unsound" in {
    ...
    val f = Imply(Forall(x :: Nil, Box(ca, PredOf(p, x))), Box(ca, Forall(x :: Nil, PredOf(p, x))))

    val s = create(
      SubstitutionPair(ca, "x:=0;".asProgram),
    ...
    val t = create(
      SubstitutionPair(ca, "y:=x^2;".asProgram),
  ...
  "Substitution of choice axiom" should "work" in {
    ...
    val s = create(
      SubstitutionPair(ca, "x:=0;".asProgram),
      SubstitutionPair(cb, "y:=1;".asProgram),
      SubstitutionPair(p, "x>=0".asFormula),
    )
    s(f) should be("[x:=0; ++ y:=1;]x>=0 <-> [x:=0;]x>=0 & [y:=1;]x>=0".asFormula)

    val t = create(
      SubstitutionPair(ca, "x:=x+1;".asProgram),
      SubstitutionPair(cb, "x:=0;y:=0;".asProgram),
  ...
  "Substitution of sequence axiom" should "work" in {
    ...
    val u = create(
      SubstitutionPair(ca, "x:=x+1; ++ y:=0;".asProgram),
      SubstitutionPair(cb, "y:=y+1;".asProgram),
      SubstitutionPair(p, "x>=y".asFormula),
    )
  ...
  "Substitution of primed terms" should "work" in {
    ...
    val h = Equal(Differential(Times(f, g)), Plus(Times(Differential(f), g), Times(f, Differential(g))))

    val s = create(SubstitutionPair(f, "x".asVariable), SubstitutionPair(g, "x".asVariable))
    s(h) shouldBe Equal(
      "(x*x)'".asTerm,
      Plus(Times(Differential("x".asVariable), "x".asVariable), Times("x".asVariable, Differential("x".asVariable))),
    )
  }
  ...
  "Substitution of primed formulas" should "work" in {
    ...
    val h = Equiv(DifferentialFormula(Equal(f, g)), Equal(Differential(f), Differential(g)))
  ...
  ignore /*"Substitution of programs where not all branches write the same variables"*/ should "work too" in {
    ...
    s(h) should be(
      "[x:=x^2;][{y:=y+1 ++ {z:=z+y;}*}; z:=x+y*z;]y>x <-> [{y:=y+1 ++ {z:=z+y;}*}; z:=x^2+y*z;]y>x^2".asFormula
    )
  }
  ...
  ignore should "work with a simpler predicate" in {
    ...
    s(h) should be(
      "[x:=x^2;][{y:=y+1 ++ {z:=z+y;}*}; z:=x+y*z;]x>0 <-> [{y:=y+1 ++ {z:=z+y;}*}; z:=x^2+y*z;]x^2>0".asFormula
    )
  }
...
//    val s = FastUSubst(List(SubstitutionPair(fnof("x"), "1".asTerm), SubstitutionPair(fnof("y"), "x".asTerm)))
...
//    val s = FastUSubst(Seq(SubstitutionPair(fnof("x"), "y".asTerm)))
...
//    val s = FastUSubst(Seq(SubstitutionPair(fnof("y"), "x+y".asTerm)))
...
//    val s = FastUSubst(Seq(SubstitutionPair(fnof("x"), "x".asTerm)))
...
//    val s = FastUSubst(Seq(SubstitutionPair("t()".asTerm, "1".asTerm), SubstitutionPair(fnof("x"), "t".asTerm)))
//    a [SubstitutionClashException] should be thrownBy applySubstitution(s, Set.empty, Set(V("t")), "t'=x();".asProgram)
    // TODO USUbst case
  }

  it should "occur on t'=x() & x()*y+t+1>0 when BV={t}" in {
//    val s = FastUSubst(Seq(SubstitutionPair(fnof("x"), "t".asTerm)))
//    a [SubstitutionClashException] should be thrownBy applySubstitution(s, Set.empty, Set(V("t")), "t'=x() & x()*y+t+1>0;".asProgram)
    // TODO USUbst case
  }
  ...
  it should "occur on y() when BV={x}" in {
    val q = Function("q", None, Real, Bool)
//    val s = FastUSubst(Seq(SubstitutionPair(fnof("x"), Number(1)), SubstitutionPair(fnof("y"), "x".asTerm)))
//    a [SubstitutionClashException] should be thrownBy applySubstitutionF(s, Set.empty, Set(V("x")), PredOf(q, fnof("y")))
    // TODO USUbst case
  }
