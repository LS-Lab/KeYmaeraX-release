...
import edu.cmu.cs.ls.keymaerax.tagobjects.{SummaryTest, TodoTest}
...
class ODETests extends TacticTestBase(registerAxTactics = Some("z3")) {
  ...
  it should "prove FM tutorial 4" in withMathematica { _ =>
    withDatabase { db =>
  ...
  it should "prove a double integrator with events" in withMathematica { _ =>
    val s =
      "x()>=0, A()>0, B()>0, y<=x()|(y-x())^2<=2*B()*(i-j), x()>=0, y>=0, A()>0, B()>0, j<=i, y<=x()|(y-x())^2<=4*B()*(i-j) ==> [{j'=y,y'=A(),i'=x()&y>=0&!(y<=x()|(y-x())^2<=2*B()*(i-j))}]((x()>=0&y>=0&A()>0&B()>0&j<=i)&(y<=x()|(y-x())^2<=2*B()*(i-j)))"
        .asSequent
  ...
  it should "prove when invariants are present in negated form" in withQE { _ =>
    val s =
      "x!=ox | y!=oy, (x+w/1-ox)^2+(y-v/1-oy)^2 != v^2+w^2 ==> [{x'=v, y'=w, v'=1*w, w'=-1*v}]!(x=ox & y=oy)".asSequent
    proveBy(s, ODE(1)) shouldBe Symbol("proved")
  }
  ...
  it should "fall back to dI in non-top positions" in withQE { _ =>
    ...
    proveBy("x=1 ==> \\exists y [{x'=y}]x>=1".asSequent, ODE(1, 0 :: Nil)).subgoals.loneElement shouldBe
      "x=1 ==> \\exists y (x>=1&y>=0)".asSequent
    proveBy("x=1 ==> \\forall y (y>=0 -> [{x'=y}]x>=1)".asSequent, ODE(1, 0 :: 1 :: Nil)).subgoals.loneElement shouldBe
      "x=1 ==> \\forall y (y>=0 -> x>=1&y>=0)".asSequent
    proveBy("x=1 ==> \\forall y (y>=0 -> [{x'=y}@invariant(x>=1)]x>=0)".asSequent, ODE(1, 0 :: 1 :: Nil))
      .subgoals
      .loneElement shouldBe "x=1 ==> \\forall y (y>=0 -> x>=1 -> x>=0 & \\forall x (x>=1->y>=0))".asSequent
  }

  it should "prove STTT Example 9b subgoal fast" in withMathematica { _ =>
    proveBy(
      "Kp()=2, Kd()=3, 5/4*(x-xr)^2+(x-xr)*v/2+v^2/4 < ((S()-xm_0)/2)^2, xr=(xm_0+S())/2, v>=0, xm_0<=x, xm=x, 5/4*(x-(xm+S())/2)^2+(x-(xm+S())/2)*v/2+v^2/4 < ((S()-xm)/2)^2\n  ==>  [{x'=v,v'=-Kp()*(x-(xm+S())/2)-Kd()*v&v>=0}]5/4*(x-(xm+S())/2)^2+(x-(xm+S())/2)*v/2+v^2/4 < ((S()-xm)/2)^2"
        .asSequent,
      ODE(1),
    ) shouldBe Symbol("proved")
  }
  ...
  it should "FEATURE_REQUEST: work in existential context" taggedAs TodoTest in withQE { _ =>
    proveBy("x=1 ==> \\exists y [{x'=y}@invariant(x>=1)]x>=0".asSequent, ODE(1, 0 :: Nil)) shouldBe
      Symbol("proved") // or at least not fail in dC
  }

  "Z3" should "prove what's needed by ODE for the Z3 ghost" in withZ3 { _ =>
    the[BelleThrowable] thrownBy
      TactixLibrary.proveBy("\\forall x_0 (x_0>0&true->\\forall x (x>0->-x>=0))".asFormula, QE) should have message
    ...
    TactixLibrary.proveBy(
      "\\forall x_0 (x_0>0->\\exists y_ (true->x_0*y_^2>0&\\forall x \\forall y_ (-x)*y_^2+x*(2*y_^(2-1)*(1/2*y_+0))>=0))"
        .asFormula,
      QE,
    ) shouldBe Symbol("proved")
  }
  ...
  it should "prove a postcondition invariant that requires trying hard" in withMathematica { _ =>
    proveBy("u^2<=v^2+9/2 ==> [{u'=-v+u/4*(1-u^2-v^2),v'=u+v/4*(1-u^2-v^2)}]u^2<=v^2+9/2".asSequent, ODE(1)) shouldBe
      Symbol("proved")
  }

  it should "FEATURE_REQUEST: prove a postcondition invariant that requires trying hard" taggedAs TodoTest in withZ3 {
    _ =>
      proveBy("u^2<=v^2+9/2 ==> [{u'=-v+u/4*(1-u^2-v^2),v'=u+v/4*(1-u^2-v^2)}]u^2<=v^2+9/2".asSequent, ODE(1)) shouldBe
        Symbol("proved")
  }
  ...
  "QE" should "be able to prove the arithmetic subgoal from x'=-x case" in withQE { _ =>
    val f =
      "x>0->(\\exists y_ (true->x*y_^2>0&\\forall x \\forall y_ (-x)*y_^2+x*(2*y_^(2-1)*(1/2*y_+0))>=0))".asFormula
  ...
  it should "prove w>=0&x=0&y=3->[{x'=y,y'=-w^2*x-2*w*y}]w^2*x^2+y^2<=9" in withQE { _ =>
    TactixLibrary.proveBy("w>=0&x=0&y=3->[{x'=y,y'=-w^2*x-2*w*y}]w^2*x^2+y^2<=9".asFormula, implyR(1) & ODE(1)) shouldBe
      Symbol("proved")
  }

  it should "cut to prove x>=0&v>=0&a>=0->[{x'=v,v'=a,a'=a^2}]x>=0" in withQE { _ =>
    TactixLibrary.proveBy("x>=0&v>=0&a>=0->[{x'=v,v'=a,a'=a^2}]x>=0".asFormula, implyR(1) & ODE(1)) shouldBe
      Symbol("proved")
  }

  it should "cut to prove x>=0&v>=0&a>=0&j>=0->[{x'=v,v'=a,a'=j,j'=j^2}]x>=0" in withQE { _ =>
    TactixLibrary.proveBy("x>=0&v>=0&a>=0&j>=0->[{x'=v,v'=a,a'=j,j'=j^2}]x>=0".asFormula, implyR(1) & ODE(1)) shouldBe
      Symbol("proved")
  }
  ...
  it should "DGauto x>0 |- [{x'=2}]x>0" in withMathematica { _ =>
    ...
    proveBy(Sequent(IndexedSeq("x>0".asFormula), IndexedSeq("[{x'=2}]x>0".asFormula)), DGauto(1)) shouldBe
      Symbol("proved")
  }

  it should "DGauto x>b() |- [{x'=2}]x>b()" in withMathematica { _ =>
    proveBy(Sequent(IndexedSeq("x>b()".asFormula), IndexedSeq("[{x'=2}]x>b()".asFormula)), DGauto(1)) shouldBe
      Symbol("proved")
  }
  ...
  it should "split and prove x^3>5 & y>2 -> [{x'=x^3+x^4,y'=5*y+y^2}](x^3>5&y>2)" in withQE { _ =>
    proveBy(
      "x^3>5 & y>2 -> [{x'=x^3+x^4,y'=5*y+y^2}](x^3>5&y>2)".asFormula,
      implyR(1) & boxAnd(1) & andR(1) < (ODE(1), ODE(1)),
    ) shouldBe Symbol("proved")
  }
  ...
  it should "split and on all prove x^3>5 & y>2 -> [{x'=x^3+x^4,y'=5*y+y^2}](x^3>5&y>2)" in withQE { _ =>
    proveBy(
      "x^3>5 & y>2 -> [{x'=x^3+x^4,y'=5*y+y^2}](x^3>5&y>2)".asFormula,
      implyR(1) & boxAnd(1) & andR(1) & onAll(ODE(1)),
    ) shouldBe Symbol("proved")
  }
  ...
  it should "split* and on all prove x^3>5 & y>2 -> [{x'=x^3+x^4,y'=5*y+y^2}](x^3>5&y>2)" in withQE { _ =>
    proveBy(
      "x^3>5 & y>2 -> [{x'=x^3+x^4,y'=5*y+y^2}](x^3>5&y>2)".asFormula,
      implyR(1) & SaturateTactic(onAll(Idioms.?(boxAnd(1) & andR(1)))) & onAll(ODE(1)),
    ) shouldBe Symbol("proved")
  }

  it should "prove x^3>5 & y>2 -> [{x'=x^3+x^4,y'=5*y+y^2}](x^3>5&y>2)" in withQE { _ =>
    proveBy("x^3>5 & y>2 -> [{x'=x^3+x^4,y'=5*y+y^2}](x^3>5&y>2)".asFormula, implyR(1) & ODE(1)) shouldBe
      Symbol("proved")
  }
  ...
  it should "cut to prove x>=0&y>=0&z>=8->[{x'=x^2,y'=4*x,z'=5*y}]z>=8" in withQE { _ =>
    TactixLibrary.proveBy("x>=0&y>=0&z>=8->[{x'=x^2,y'=4*x,z'=5*y}]z>=8".asFormula, implyR(1) & ODE(1)) shouldBe
      Symbol("proved")
  }

  it should "PDEify to prove x=1&y=2&z>=8->[{x'=x^2,y'=4*x,z'=5*y}]z>=8" in withMathematica { _ =>
    // @todo Z3
    TactixLibrary.proveBy("x=1&y=2&z>=8->[{x'=x^2,y'=4*x,z'=5*y}]z>=8".asFormula, implyR(1) & ODE(1)) shouldBe
      Symbol("proved")
  }

  it should "work with nested ODEs" in withQE { _ =>
    proveBy("x>0 -> [{x'=5};{x'=2};{x'=x}]x>0".asFormula, (unfoldProgramNormalize & ODE(1) & dWPlus(1)) * 3) shouldBe
      Symbol("proved")
  }
  ...
  it should "work with solvable maybe bound" in withQE { _ =>
    ...
    result.subgoals.loneElement should
      (be(
        "true, time_=0, x_0=x ==> [{x'=5,time_'=1 & true & time_>=0&x=5*time_+x_0}][{x:=x+3;}* ++ y:=x;](x>0&y>0)"
          .asSequent
      )
      // Z3
      or be(
        "true, time_=0, x_0=x ==> [{x'=5,time_'=1 & true & time_>=0&x=x_0+5*time_}][{x:=x+3;}* ++ y:=x;](x>0&y>0)"
          .asSequent
      ))
  }
  ...
  it should "neither stutter nor fail evolution domain simplification on empty evolution domain constraint with Z3" in
    withZ3 { _ =>
  ...
  it should
    "neither stutter nor fail evolution domain simplification on empty evolution domain constraint with Mathematica" in
    withMathematica { _ =>
  ...
  it should "prove a list of ODEs" in withQE { qeTool =>
    withTemporaryConfig(Map(Configuration.Keys.ODE_TIMEOUT_FINALQE -> "60")) {
      forEvery(list) { (formula, requiredTool) =>
            ...
            try {
              cancelAfter(2.minutes) {
                a[BelleThrowable] should be thrownBy
                  TactixLibrary.proveBy(formula.asFormula, implyR(1) & ODE(1) & onAll(QE) & done)
              }
            } catch {
  ...
  it should "FEATURE_REQUEST: prove a list of ODEs with cuts after improving tactics" taggedAs TodoTest in withQE {
    tool =>
      forEvery(nops) { formula =>
  ...
  it should "use an annotated differential invariant" in withMathematica { _ =>
    ...
    proveBy("x=3 & y=4 ==> [{x'=-y,y'=x}@invariant(x^2+y^2=old(x^2+y^2))]x^2+y^2<=25".asSequent, ODE(1)) shouldBe
      Symbol("proved")
  }
  ...
  it should "interpret implications as differential invariants on multiple occurrences of substituted variable" in
    withMathematica { _ =>
      val g =
  ...
  "liveness" should "prove that exponential diff eqs are unbounded" in withMathematica { _ =>
    ...
    val pr1 = proveBy(texists, implyR(1) & useAt(Ax.DSdnodomain)(1) & chase(1, 0 :: 1 :: Nil) & QE)

    // DG for linear systems
    val xexists = "x0>0 -> <{t'=1,x'=1*x+0}> 0<=x0-x1+x0*t".asFormula
    val pr2 = proveBy(
      xexists,
      implyR(1) & universalGen(Some("x".asVariable), "x".asTerm)(1) & useAt(Ax.DGd, PosInExpr(1 :: Nil))(1) & implyRi &
        by(pr1),
    )
      ...
      prop &
        ...
        (
          useAt(Ax.diamond, PosInExpr(1 :: Nil))(1) & notR(1) & SimplifierV3.fullSimpTac() &
            ...
            (
              useAt(Ax.box, PosInExpr(1 :: Nil))(-6) & notL(-6) & chase(1, 1 :: Nil) &
                implyRi()(AntePos(2), SuccPos(0)) & cohideR(1) & byUS(pr2),
  ...
  "invariant clusters" should "prove example 6 (Kong et. al. HSCC'17)" in withMathematica { _ =>
    ...
    val pr = proveBy(
      ...
      implyR(1) &
        ...
        (
          SaturateTactic(existsL(Symbol("L"))) &
            dC("u1-u3*(x^2-y^2)=0".asFormula)(1) < (dWPlus(1) & QE, dI(Symbol("full"))(1)),
          hideR(1) & QE,
        ),
    )
    // println(pr)
  ...
  "ODE Counterexample" should "disprove a wrong conjecture" in withMathematica { _ =>
    val fml =
      "7.1798*x^3-7.1798*y^3+21.539*x^2+21.539*y^2-3081.9<=0 ->  [{ x'=y^2-2*y, y'=x^2+2*x } ]7.1798*x^3-7.1798*y^3+21.539*x^2+21.539*y^2-3081.9<=0"
        .asFormula
