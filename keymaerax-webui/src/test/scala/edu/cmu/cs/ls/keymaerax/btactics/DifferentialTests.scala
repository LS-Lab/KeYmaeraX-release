...
/**
 * Basic differential equation proving technology tests [[edu.cmu.cs.ls.keymaerax.btactics.DifferentialTactics]],
 * [[edu.cmu.cs.ls.keymaerax.btactics.TactixLibrary.DW]], and [[edu.cmu.cs.ls.keymaerax.btactics.TactixLibrary.DC]]
 * @see
...
class DifferentialTests extends TacticTestBase {
  ...
  it should "keep initial conditions" in withQE { _ =>
    ...
    proveBy(
      ...
      "dx_0^2+dy_0^2=1&x_0^2+y_0^2>0, old=dy_0*x_0-(dx_0-1)*y_0, dx=dx_0&dy=dy_0&dy*x-(dx-1)*y=old " +
        " ==> dx^2+dy^2=1&x^2+y^2>0"
    ).asSequent
  }
  ...
  it should "support box assumptions" in withQE { _ =>
    proveBy(
      """x_0<=m, A()>=0, b()>0, true, t=0,
        ...
        |==>
        |[{x'=v,v'=A(),t_0'=1,time_'=1&(v>=0&t_0<=ep())&time_>=0&t_0=time_+t_1&v=A()*time_+v_0&x=1/2*A()*time_^2+time_*v_0+x_0}]x<=m"""
        .stripMargin
        .asSequent,
      DifferentialTactics.diffWeakenPlus(1),
    ).subgoals.loneElement shouldBe
  ...
  it should "work if not sole formula in succedent" in withQE { _ =>
    proveBy("A>0&A>1, B=1, C=2&D=3, x=4 ==> Blah=1, [{x'=1&x>2}]x>0, Blub=3".asSequent, dW(2))
      .subgoals
      .loneElement shouldBe "x>2 & A>0 & A>1 & B=1 & C=2 & D=3 & !Blah=1 & !Blub=3 ==> x>0".asSequent
    proveBy(
      "A>0&A>1, B=1, C=2&D=3, x=4 ==> Blah=1, [{x'=1&x>2}]x>0, Blub=3".asSequent,
      DifferentialTactics.diffWeakenPlus(2),
    ).subgoals.loneElement shouldBe "A>0&A>1, B=1, C=2&D=3, x_0=4, x>2 ==> Blub=3, Blah=1, x>0".asSequent
  }

  it should "DW in context" in withQE { _ =>
    proveBy("==> [x:=2;][{x'=-3&x>=1}]x>=0".asSequent, dW(1, 1 :: Nil))
      .subgoals
      .loneElement shouldBe "==> [x:=2;]\\forall x (x>=1 -> x>=0)".asSequent
    proveBy("==> [x:=2;][{x'=-3&x>=1}]x>=0".asSequent, DifferentialTactics.diffWeakenPlus(1, 1 :: Nil))
      .subgoals
      .loneElement shouldBe "==> [x:=2;]\\forall x (x>=1 -> x>=0)".asSequent
  }
  ...
  it should "prove single variable" in withQE { _ =>
    proveBy(
      "v>=0 -> [{x'=v}][x':=v;]x'>=0".asFormula,
      Dassignb(1, 1 :: 1 :: Nil) & implyR(1) & abstractionb(1) & QE,
  ...
  it should "assign flat variable" in withTactics {
    val result =
      proveBy("[{x'=v,v'=a}][v':=a;][x':=v;]v'>=0".asFormula, Dassignb(1, 1 :: 1 :: Nil) & Dassignb(1, 1 :: Nil))
  ...
  it should "step into a constified ODE" in withQE { _ =>
    proveByS(
      ...
      _.value should contain theSameElementsAs List(BelleLabels.dIInit, BelleLabels.dIStep),
      "a()~>a".asDeclaration,
  ...
  it should "fail constification in context if ODE consts are bound outside" in withTactics {
    ...
    the[BelleProofSearchControl] thrownBy proveBy(
      "x>=5 ==> [y:=2;x:=x+1;][{x'=y}]x>=y".asSequent,
      DifferentialTactics.diffInd(Symbol("full"))(1, 1 :: Nil),
    ) should
  ...
  it should "work with quantified postconditions" in withMathematica { _ =>
    proveBy("[{x'=3}]\\exists y y<=x".asFormula, dI(auto = Symbol("diffInd"))(1))
  ...
  it should "expand special functions" in withQE { _ =>
    proveBy("[{x'=3}]abs(x)>=2".asFormula, dI(auto = Symbol("diffInd"))(1))
      .subgoals should contain theSameElementsInOrderAs List(
      "true ==> x>=0&x>=2 | x<0&-x>=2".asSequent,
      "true ==> [x':=3;]((x'>=0&x'>=0) & x'<=0&-x'>=0)".asSequent,
    )
    proveBy("[{x'=3}]max(x,4)>=2".asFormula, dI(auto = Symbol("diffInd"))(1))
      .subgoals should contain theSameElementsInOrderAs List(
      "true ==> x>=4&x>=2 | x<4&4>=2".asSequent,
      "true ==> [x':=3;]((x'>=0&x'>=0) & x'<=0&0>=0)".asSequent,
    )
    proveBy("[{x'=3}]min(x,4)<=6".asFormula, dI(auto = Symbol("diffInd"))(1))
      .subgoals should contain theSameElementsInOrderAs List(
  ...
  it should "FEATURE_REQUEST: expand special functions and constify their abbreviation if possible" taggedAs TodoTest in withQE {
    _ =>
      // @todo needs expand outside DConstify(...) with intermediate result between expand and DConstify stored in DB
      proveBy("[{x'=3}]abs(f())>=2".asFormula, dI(auto = Symbol("diffInd"))(1))
        .subgoals should contain theSameElementsInOrderAs List(
  ...
  it should "not be applicable on non-FOL postcondition" in withQE { _ =>
    the[TacticInapplicableFailure] thrownBy proveBy(
      "x>=0, y>=1 ==> [{x'=2}][{x'=5&y>=2} ++ y:=y+1;](x>=0 & y>=2)".asSequent,
      dI()(1),
    ) should
  ...
  it should "work with uninterpreted and interpreted functions" in withMathematica { _ =>
    val defs =
      "loopinv(x,y,v,xo,yo) ~> (v >= 0 & (abs(x-xo) > stopDist(v) | abs(y-yo) > stopDist(v))) :: stopDist(v) ~> v^2/(2*b()) :: nil"
        .asDeclaration
    val s =
  ...
  it should "work as dIRule in existential context" in withMathematica { _ =>
    proveBy("x>0 ==> \\exists y [{x'=-x,y'=1/2*y}]x*y^2=1".asSequent, dI(auto = Symbol("diffInd"))(1, 0 :: Nil))
      .subgoals
      .loneElement shouldBe "x>0 ==> \\exists y (true->x*y^2=1&\\forall x \\forall y [y':=1/2*y;][x':=-x;]x'*y^2+x*(2*y^(2-1)*y')=0)"
      .asSequent
  }

  "odeInvariant" should "prove STTT Example 9b invariant" in withQE { _ =>
    val seq =
      "Kp()=2, Kd()=3, v>=0, xm<=x, xr=(xm+S())/2, 5/4*(x-xr)^2+(x-xr)*v/2+v^2/4 < ((S()-xm)/2)^2, true ==> [{x'=v,v'=-Kp()*(x-xr)-Kd()*v&v>=0&xm<=x}]5/4*(x-(xm+S())/2)^2+(x-(xm+S())/2)*v/2+v^2/4 < ((S()-xm)/2)^2"
        .asSequent
  ...
  it should "derive implications" in withTactics {
    ...
    proveBy("((a=0->b=1)->c=2)'".asFormula, derive(1)).subgoals.loneElement shouldBe "==> (a'=0 & b'=0) & c'=0"
      .asSequent
  }
  ...
  it should "work in context 3" in withQE { _ =>
    val result =
      proveBy("a>1 -> [z:=1;][y:=2;][x:=3;][{x'=2}]x>=0".asFormula, DC("x>0".asFormula)(1, 1 :: 1 :: 1 :: 1 :: Nil))
    result.subgoals should have size 2
  ...
  it should "work in context 4" in withQE { _ =>
    val result = proveBy(
      "a>1 -> b=2|([z:=1;][y:=2;][x:=3;][{x'=2}]x>=0 | c<3)".asFormula,
      DC("x>0".asFormula)(1, 1 :: 1 :: 0 :: 1 :: 1 :: 1 :: Nil),
    )
    result.subgoals should have size 2
  ...
  it should "work in context 6" in withQE { _ =>
    val result = proveBy(
      "a>1 -> b=2 & (c<3|[z:=1;][y:=2;][x:=3;][{x'=2}]x>=0) & d=4".asFormula,
      DC("x>0".asFormula)(1, 1 :: 1 :: 0 :: 1 :: 1 :: 1 :: 1 :: Nil),
    )
    result.subgoals should have size 2
    result.subgoals.head shouldBe "==> a>1 -> b=2 & (c<3|[z:=1;][y:=2;][x:=3;][{x'=2 & true & x>0}]x>=0) & d=4"
      .asSequent
  ...
  it should "auto-generate multiple names for term-ghosts when special function old is used over consecutive cuts" in withQE {
    _ =>
      val result = proveBy(
        "[{x'=2 & x>=0 | y<z}]x>=0".asFormula,
        dC("x>=old(x^2+4)+old(y*z)".asFormula)(1) & Idioms.<(dC("x>old(2+4)".asFormula)(1), nil),
      )
      result.subgoals should have size 3
      result
        .subgoals
        .head shouldBe "old=x^2+4, old_0=y*z, old_1=2+4 ==> [{x'=2 & ((x>=0 | y<z) & x>=old+old_0) & x>old_1}]x>=0"
        .asSequent
      result.subgoals(1) shouldBe "old=x^2+4, old_0=y*z ==> [{x'=2 & (x>=0 | y<z)}]x>=old+old_0".asSequent
      result.subgoals(2) shouldBe "old=x^2+4, old_0=y*z, old_1=2+4 ==> [{x'=2 & (x>=0 | y<z) & x>=old+old_0}]x>old_1"
        .asSequent
  }
  ...
  it should "auto-generate and re-use names" in withQE { _ =>
    ...
    result.subgoals should have size 3
    result.subgoals.head shouldBe "old=x^2+4, old_0=y*z ==> [{x'=2 & ((x>=0 | y<z) & x>=old+old_0+old) & x>old}]x>=0"
      .asSequent
  ...
  it should "already rewrite existing conditions and introduce ghosts when special function old is used" in withQE {
    _ =>
  ...
  it should "cut in single formula with multiple old variables" in withQE { _ =>
    val result =
      proveBy("dx^2+dy^2=1 ==> [{dx'=0,dy'=0}]dx^2+dy^2=1".asSequent, dC("dx=old(dx) & dy=old(dy)".asFormula)(1))
    result.subgoals should have size 2
    result
      .subgoals
      .head shouldBe "dx_0^2+dy_0^2=1, dx_0=dx, dy_0=dy ==> [{dx'=0,dy'=0&true&dx=dx_0&dy=dy_0}]dx^2+dy^2=1".asSequent
  ...
  it should "work when not sole formula in succedent" in withMathematica { _ =>
    ...
    proveBy(
      "t=0  ==> x!=1, [{t'=1,x'=-x&t=0}]x=1, x!=3".asSequent,
      dC("(x-old(x))^2<=2*((x-old(x))*(-x))*(t-0)".asFormula)(2),
    ).subgoals should contain theSameElementsInOrderAs List(
    ...
    proveBy(
      "==> x!=1, t=0 -> [{t'=1,x'=-x&t=0}]x=1, x!=3".asSequent,
      dC("(x-old(x))^2<=2*((x-old(x))*(-x))*(t-0)".asFormula)(2, 1 :: Nil),
    ).subgoals should contain theSameElementsInOrderAs List(
      "==> x!=1, t=0 -> \\forall x_0 (x_0=x -> [{t'=1,x'=-x&t=0&(x-x_0)^2<=2*((x-x_0)*(-x))*(t-0)}]x=1), x!=3"
        .asSequent,
  ...
  it should "cut in multiple formulas" in withQE { _ =>
    val result =
      proveBy("v>=0, x>0 ==> [{x'=v,v'=2}]x>=0".asSequent, dC("v>=0".asFormula :: "x>=old(x)".asFormula :: Nil)(1))
    result.subgoals should have size 3
  ...
  it should "compute the followup position correctly" in withQE { _ =>
    val result =
      proveBy("y=1 ==> [x:=0;][{x'=1,y'=-1}]x>=0".asSequent, dC("x>=old(x) & y<=old(y)".asFormula)(1, 1 :: Nil))
    result.subgoals should have size 2
    result
      .subgoals
      .head shouldBe "y=1 ==> [x:=0;]\\forall y_0 (y_0=y -> \\forall x_0 (x_0=x -> [{x'=1,y'=-1 & true & (x>=x_0 & y<=y_0)}]x>=0))"
      .asSequent
  }
  ...
  it should "cut in single formula with multiple old variables" in withQE { _ =>
    val result =
      proveBy("dx^2+dy^2=1 ==> <{dx'=0,dy'=0}>dx^2+dy^2=1".asSequent, dC("dx=old(dx) & dy=old(dy)".asFormula)(1))
    result.subgoals should have size 2
    result.subgoals(0) shouldBe "dx_0^2+dy_0^2=1, dx_0=dx, dy_0=dy ==> <{dx'=0,dy'=0&true&dx=dx_0&dy=dy_0}>dx^2+dy^2=1"
      .asSequent
    result.subgoals(1) shouldBe "dx_0^2+dy_0^2=1, dx_0=dx, dy_0=dy ==> [{dx'=0,dy'=0}](dx=dx_0&dy=dy_0)".asSequent
  }
  ...
  it should "keep position stable" in withQE { _ =>
    val result =
      proveBy("x>=0 ==> y>=0, [{x'=1 & y>=0 & x>=0}]true, z>1".asSequent, DifferentialTactics.inverseDiffCut(2))
    result.subgoals should have size 2
  ...
  it should "work in context" in withQE { _ =>
    val result =
      proveBy("==> [x:=1;][{x'=1 & y>=0 & x>=0}]true".asSequent, DifferentialTactics.inverseDiffCut(1, 1 :: Nil))
    result.subgoals should have size 2
  ...
  "diffRefine" should "refine at succedent positions" in withQE { _ =>
    val result = proveBy(
      "x>=0 ==> <{x'=v,v'=2}>x>=0, [{x'=x+y,y'=2}]y>=0".asSequent,
  ...
  it should "refine at antecedent positions" in withQE { _ =>
    val result = proveBy(
      "<{x'=v,v'=2}>x>=0, [{x'=x+y,y'=2}]y>=0 ==> x>=0 ".asSequent,
  ...
  it should "cut in single formula with multiple old variables" in withQE { _ =>
    val result = proveBy(
      "dx^2+dy^2=1 ==> [{dx'=0,dy'=0}]dx^2+dy^2=1".asSequent,
      diffInvariant("dx=old(dx) & dy=old(dy)".asFormula)(1),
    )
    result
      .subgoals
      .loneElement shouldBe "dx_0^2+dy_0^2=1, dx_0=dx, dy_0=dy ==> [{dx'=0,dy'=0&true&dx=dx_0&dy=dy_0}]dx^2+dy^2=1"
      .asSequent
  }

  it should "cut in multiple formulas" in withQE { _ =>
    val result =
      proveBy("v>=0, x>0 ==> [{x'=v,v'=2}]x>=0".asSequent, diffInvariant("v>=0".asFormula :: "x>0".asFormula :: Nil)(1))
  ...
  it should "let us directly prove variable x+y^2*3-z = x+y^2*3-z by abbreviation" in withQE { _ =>
    proveBy(
      "x+y^2*3-z=x+y^2*3-z".asFormula,
      let(FuncOf(Function("s_", None, Unit, Real), Nothing), "x+y^2*3-z".asTerm, by(Ax.equalReflexive)),
    ) shouldBe Symbol("proved")
  }
  ...
  it should "let us prove variable [x':=5;](x+y)'>=0" in withQE { _ =>
    ...
    val result = TactixLibrary.proveBy(
      "[x':=5;](x+y>=0)'".asFormula,
  ...
  it should "prove const [{x'=5}](x+c())'>=0" in withQE { _ =>
    proveBy(
      "[{x'=5}](x+c())'>=0".asFormula,
      derive(1, 1 :: 0 :: Nil) & DE(1) & G(1) & Dassignb(1) & QE,
    ) shouldBe Symbol("proved")
  }
  ...
  it should "let us prove variable [{x'=5}](x+y)'>=0" in withQE { _ =>
    ...
    TactixLibrary.proveBy(
      "[{x'=5}](x+y)'>=0".asFormula,
      let(
        FuncOf(Function("c", None, Unit, Real), Nothing),
        Variable("y"),
        derive(1, 1 :: 0 :: Nil) & DE(1) & G(1) & Dassignb(1) & QE,
      ),
    ) shouldBe Symbol("proved")
  ...
  it should "prove const x+c()>=0 -> [{x'=5}]x+c()>=0" in withQE { _ =>
    proveBy("x+c()>=0 -> [{x'=5}]x+c()>=0".asFormula, implyR(1) & dI(Symbol("full"))(1)) shouldBe Symbol("proved")
  }

  it should "prove const x+c()>=0 -> [{x'=5}]x+c()>=0 manual" in withQE { _ =>
    proveBy(
      "x+c()>=0 -> [{x'=5}]x+c()>=0".asFormula,
      implyR(1) & dI(Symbol("none"))(1) < (close, derive(1, 1 :: Nil) & DE(1) & G(1) & Dassignb(1) & QE),
    ) shouldBe Symbol("proved")
  }

  it should "let us prove variable x+y>=0 -> [{x'=5}]x+y>=0 manual" in withQE { _ =>
    proveBy(
      "x+y>=0 -> [{x'=5}]x+y>=0".asFormula,
      implyR(1) &
        let(
          FuncOf(Function("c", None, Unit, Real), Nothing),
          Variable("y"),
          dI(Symbol("none"))(1) < (close, derive(1, 1 :: Nil) & DE(1) & G(1) & Dassignb(1) & QE),
        ),
    ) shouldBe Symbol("proved")
  }
  ...
  it should "let us prove variable x+y>=0 -> [{x'=5}]x+y>=0" in withQE { _ =>
    proveBy(
      "x+y>=0 -> [{x'=5}]x+y>=0".asFormula,
      implyR(1) &
        let(FuncOf(Function("c", None, Unit, Real), Nothing), Variable("y"), dI(Symbol("full"))(1)),
    ) shouldBe Symbol("proved")
  }

  it should "let us prove variable x+y+z>=0 -> [{x'=5,y'=2}]x+y+z>=0" in withQE { _ =>
    proveBy(
      "x+y+z>=0 -> [{x'=5,y'=2}]x+y+z>=0".asFormula,
      implyR(1) &
        let(FuncOf(Function("c", None, Unit, Real), Nothing), Variable("z"), dI(Symbol("full"))(1)),
    ) shouldBe Symbol("proved")
  }

  it should "let us prove variable x+z>=0&y+z>=0 -> [{x'=5,y'=2}](x+z>=0&y+z>=0)" in withQE { _ =>
    proveBy(
      "x+z>=0&y+z>=0 -> [{x'=5,y'=2}](x+z>=0&y+z>=0)".asFormula,
      implyR(1) &
        let(FuncOf(Function("c", None, Unit, Real), Nothing), Variable("z"), dI(Symbol("full"))(1)),
    ) shouldBe Symbol("proved")
  }
  ...
  it should "let us prove variable x>=0 & v>=0 -> [{x'=v}]x>=0" in withQE { _ =>
    proveBy(
      "x>=0 & v>=0 -> [{x'=v}]x>=0".asFormula,
      implyR(1) &
        let(FuncOf(Function("v", None, Unit, Real), Nothing), Variable("v"), dI(Symbol("full"))(1)),
    ) shouldBe Symbol("proved")
  }

  it should "let us prove variable a>=0 & x>=0 & v>=0 -> [{x'=v,v'=a}]v>=0" in withQE { _ =>
    proveBy(
      "a>=0 & x>=0 & v>=0 -> [{x'=v,v'=a}]v>=0".asFormula,
      implyR(1) & let(FuncOf(Function("a", None, Unit, Real), Nothing), Variable("a"), dI(Symbol("full"))(1)),
    ) shouldBe Symbol("proved")
  }

  it should "perhaps prove variable a>=0 & x>=0 & v>=0 -> [{x'=v,v'=a}]v>=0 directly if diffInd were powerful enough" in withQE {
    _ =>
      proveBy("a>=0 & x>=0 & v>=0 -> [{x'=v,v'=a}]v>=0".asFormula, implyR(1) & dI(Symbol("full"))(1)) shouldBe Symbol(
        "proved"
      )
  }
  ...
  it should "let us prove variable a>=0 & x>=0 & v>=0 -> [{x'=v,v'=a}]v>=0 despite silly names" in withQE { _ =>
    proveBy(
      "a>=0 & x>=0 & v>=0 -> [{x'=v,v'=a}]v>=0".asFormula,
      implyR(1) & let(FuncOf(Function("gobananas", None, Unit, Real), Nothing), Variable("a"), dI(Symbol("full"))(1)),
    ) shouldBe Symbol("proved")
  }

  it should "FEATURE_REQUEST: expand definitions only temporarily in dI but not in result" taggedAs TodoTest in withMathematica {
    _ =>
      val entry = ArchiveParser
        .parse(
          """ArchiveEntry "DI"
      ...
      proveByS(entry.sequent, implyR(1) & diffInvariant("unitCircle(x, y)".asFormula)(1), entry.defs)
        .subgoals
        .loneElement shouldBe "unitCircle(x,y) ==> [{x'=y, y'=-x & unitCircle(x,y)}]unitCircle(x,y)".asSequent
  }

  private val dconstifyTests = Table(
    ("Name", "Sequent", "Expected"),
    ("replace a with a() in v'=a", "==> [{v'=a}]v=v0()+a*t()", "==> [{v'=a()}]v=v0()+a()*t()"),
    ("not self-replace a() with a() in v'=a()", "==> [{v'=a()}]v=v0()+a()*t()", "==> [{v'=a()}]v=v0()+a()*t()"),
    ("not replace a with a() when a is not free in p", "==> [{v'=a}]v>0", "==> [{v'=a}]v>0"),
    (
  ...
  "Differential introduce constants" should "replace a with a()" in withTactics {
    ...
    forEvery(dconstifyTests) { (name, input, expectedResult) =>
      withClue(name) {
        the[BelleAbort] thrownBy proveBy(
          input.asSequent,
          DifferentialTactics.Dconstify(checkSequentTactic(expectedResult.asSequent))(1),
        ) should have message "Sequent as expected, now aborting"
      }
    }
  }
  ...
  it should "not constify bound postcondition" in withTactics {
    proveBy(
      "x>=0, y>=1, z=2 ==> [{x'=5&y>=1}][{x'=z} ++ y:=z+1;](x>=0 & y>=1)".asSequent,
      DifferentialTactics.Dconstify(skip)(1),
  ...
  it should "add y'=1 to [x'=2]x>0 from parsed tactic" in withTactics {
    proveBy("[{x'=2}]x>0".asFormula, BelleParser("""dG("{y'=0*y+1}", 1)"""))
      .subgoals
      .loneElement shouldBe "==> \\exists y [{x'=2,y'=1}]x>0".asSequent
    proveBy("[{x'=2}]x>0".asFormula, BelleParser("""dG("y'=1", 1)"""))
      .subgoals
      .loneElement shouldBe "==> \\exists y [{x'=2,y'=1}]x>0".asSequent
    proveBy("[{x'=2}]x>0".asFormula, BelleParser("""dG("{y'=1}", 1)"""))
      .subgoals
      .loneElement shouldBe "==> \\exists y [{x'=2,y'=1}]x>0".asSequent
    proveBy("[{x'=2}]x>0".asFormula, BelleParser("""dG("{y'=1&true}", 1)"""))
      .subgoals
      .loneElement shouldBe "==> \\exists y [{x'=2,y'=1}]x>0".asSequent
  }
  ...
  it should "do basic unification" in withTactics {
    ...
    proveBy("[{x'=2}]x>0".asFormula, dG("{y'=2*y+3}".asDifferentialProgram, None)(1) & existsR("0".asTerm)(1))
      .subgoals
      .loneElement shouldBe "y=0 ==> [{x'=2,y'=2*y+3}]x>0".asSequent
    proveBy("[{x'=2}]x>0".asFormula, dG("{y'=2*y-3}".asDifferentialProgram, None)(1) & existsR("0".asTerm)(1))
      .subgoals
      .loneElement shouldBe "y=0 ==> [{x'=2,y'=2*y+(-3)}]x>0".asSequent
    proveBy("[{x'=2}]x>0".asFormula, dG("{y'=-2*y+3}".asDifferentialProgram, None)(1) & existsR("0".asTerm)(1))
    ...
    proveBy("[{x'=2}]x>0".asFormula, dG("{y'=-2*y-z}".asDifferentialProgram, None)(1) & existsR("0".asTerm)(1))
      .subgoals
      .loneElement shouldBe "y=0 ==> [{x'=2,y'=(-2)*y+-z}]x>0".asSequent

    // ay,-ay
    proveBy("[{x'=2}]x>0".asFormula, dG("{y'=2*y}".asDifferentialProgram, None)(1) & existsR("0".asTerm)(1))
      .subgoals
      .loneElement shouldBe "y=0 ==> [{x'=2,y'=2*y+0}]x>0".asSequent
    proveBy("[{x'=2}]x>0".asFormula, dG("{y'=-2*y}".asDifferentialProgram, None)(1) & existsR("0".asTerm)(1))
      .subgoals
      .loneElement shouldBe "y=0 ==> [{x'=2,y'=(-2)*y+0}]x>0".asSequent
    ...
    proveBy("[{x'=2}]x>0".asFormula, dG("{t'=1}".asDifferentialProgram, None)(1) & existsR("0".asTerm)(1))
      .subgoals
      .loneElement shouldBe "t=0 ==> [{x'=2,t'=1}]x>0".asSequent
    proveBy("[{x'=2}]x>0".asFormula, dG("{t'=-1}".asDifferentialProgram, None)(1) & existsR("0".asTerm)(1))
      .subgoals
      .loneElement shouldBe "t=0 ==> [{x'=2,t'=-1}]x>0".asSequent
    ...
    proveBy("[{x'=2}]x>0".asFormula, dG("{t'=t+3}".asDifferentialProgram, None)(1) & existsR("0".asTerm)(1))
      .subgoals
      .loneElement shouldBe "t=0 ==> [{x'=2,t'=1*t+3}]x>0".asSequent
    proveBy("[{x'=2}]x>0".asFormula, dG("{t'=t-3}".asDifferentialProgram, None)(1) & existsR("0".asTerm)(1))
      .subgoals
      .loneElement shouldBe "t=0 ==> [{x'=2,t'=1*t+(-3)}]x>0".asSequent
    proveBy("[{x'=2}]x>0".asFormula, dG("{t'=-t+3}".asDifferentialProgram, None)(1) & existsR("0".asTerm)(1))
    ...
    proveBy("[{x'=2}]x>0".asFormula, dG("{t'=t/2+3}".asDifferentialProgram, None)(1) & existsR("0".asTerm)(1))
      .subgoals
      .loneElement shouldBe "t=0 ==> [{x'=2,t'=1/2*t+3}]x>0".asSequent
    proveBy("[{x'=2}]x>0".asFormula, dG("{t'=t/2-3}".asDifferentialProgram, None)(1) & existsR("0".asTerm)(1))
      .subgoals
      .loneElement shouldBe "t=0 ==> [{x'=2,t'=1/2*t+(-3)}]x>0".asSequent
    proveBy("[{x'=2}]x>0".asFormula, dG("{t'=-t/2+3}".asDifferentialProgram, None)(1) & existsR("0".asTerm)(1))
    ...
    proveBy("[{x'=2}]x>0".asFormula, dG("{t'=t/2}".asDifferentialProgram, None)(1) & existsR("0".asTerm)(1))
      .subgoals
      .loneElement shouldBe "t=0 ==> [{x'=2,t'=1/2*t+0}]x>0".asSequent
    proveBy("[{x'=2}]x>0".asFormula, dG("{t'=-t/2}".asDifferentialProgram, None)(1) & existsR("0".asTerm)(1))
      .subgoals
      .loneElement shouldBe "t=0 ==> [{x'=2,t'=(-1/2)*t+0}]x>0".asSequent
    ...
    proveBy("[{x'=2}]x>0".asFormula, dG("{t'=0*t}".asDifferentialProgram, None)(1) & existsR("0".asTerm)(1))
      .subgoals
      .loneElement shouldBe "t=0 ==> [{x'=2,t'=0}]x>0".asSequent
  ...
  it should "auto-cut avoid singularities" in withMathematica { _ =>
    proveBy(
      "x>1 ==> [{x'=x+1}]x>1".asSequent,
      dG("{y'=(-1/2*(x+1)/(x-1))*y}".asDifferentialProgram, Some("(x-1)*y^2=1".asFormula))(1),
    ).subgoals.loneElement shouldBe "x>1 ==> \\exists y [{x'=x+1,y'=(-1/2*(x+1)/(x-1))*y+0&true&x-1!=0}](x-1)*y^2=1"
      .asSequent
  }
  ...
  it should "not allow non-linear ghosts (1)" in withTactics {
    a[BelleThrowable] should be thrownBy proveBy(
      "[{x'=2}]x>0".asFormula,
      dG("{y'=y*y+1}".asDifferentialProgram, None)(1),
    )
  }
  ...
  it should "not allow non-linear ghosts (2)" in withTactics {
    a[BelleThrowable] should be thrownBy proveBy(
      "[{x'=2}]x>0".asFormula,
      dG("{y'=1*y+y}".asDifferentialProgram, None)(1),
    )
  }
  ...
  it should "not allow ghosts that are already present in the ODE" in withTactics {
    a[BelleThrowable] should be thrownBy proveBy(
      "[{x'=2}]x>0".asFormula,
      dG("{x'=0*x+1}".asDifferentialProgram, None)(1),
    )
  }
  ...
  it should "give useful error messages on shape mismatch" in withTactics {
    the[BelleThrowable] thrownBy proveBy(
      "[{x'=2}]x>0".asFormula,
      dG("{t'=x*t*x^2}".asDifferentialProgram, None)(1) & existsR("0".asTerm)(1),
    ) should
  ...
  it should "give a useful error message on invalid transformation of postcondition" in withMathematica { _ =>
    the[BelleUserCorrectableException] thrownBy proveBy(
      "==> [{x'=v}]x>0".asSequent,
      dG("z'=v".asDifferentialProgram, Some("z>=0".asFormula))(1),
    ) should have message
      """Formula
        |  z>=0
  ...
  it should "give a useful error message when facts cannot be preserved for postcondition transformation" in withMathematica {
    _ =>
      the[BelleUserCorrectableException] thrownBy proveBy(
  ...
  it should "expand definitions to avoid singularities" in withMathematica { _ =>
    ...
    proveBy(
      ...
      """9>0, 1>0, T()>0, m() < -(9/1)^(1/2), t=0, v>-(9/1)^(1/2), x>=0, v < 0
        |==>  \exists y [{x'=v,v'=-9+1*v^2,t'=1,y'=(-1/2*1*(v-(9/1)^(1/2)))*y+0 & (t<=T() & x>=0 & v < 0) & 1!=0}]y^2*(v+(9/1)^(1/2))=1"""
        .stripMargin
        .asSequent
  }
  ...
  it should "work in a complicated context" in withQE { _ =>
    ...
    val result = proveBy(s, tactic)
    result.subgoals.loneElement shouldBe "x>0 ==> a=2 -> [b:=3;]<?c=5;{c'=2}>\\exists y [{x'=2,y'=1}](y>0 & x*y>0)"
      .asSequent
  }
  ...
  it should "add arbitrary shape" in withQE { _ =>
    ...
    proveBy(s, dG("{y'=0.5*y}".asDifferentialProgram, fml)(1))
      .subgoals
      .loneElement shouldBe "x<0 ==> \\exists y [{x'=-x,y'=0.5*y+0}]x*y^2=-1".asSequent
  ...
  it should "do fancy unification for proving x>0->[{x'=-x}]x>0" in withMathematica { _ =>
    val result = proveBy(
      "x>0->[{x'=-x}]x>0".asFormula,
      implyR(1) &
        dG("{y'=(1/2)*y+0}".asDifferentialProgram, Some("x*y^2=1".asFormula))(1) & existsR("1/x^(1/2)".asTerm)(
          1
        ) & dI()(1) & QE,
    )
    result shouldBe Symbol("proved")
  ...
  it should "do fancy unification for proving x>0->[{x'=x}]x>0" in withMathematica { _ =>
    val result = proveBy(
      "x>0->[{x'=x}]x>0".asFormula,
      implyR(1) &
        dG("{y'=(-1/2)*y+0}".asDifferentialProgram, Some("x*y^2=1".asFormula))(1) & dI()(1, 0 :: Nil) & existsR(
          "1/x^(1/2)".asTerm
        )(1) & QE,
    )
    result shouldBe Symbol("proved")
  ...
  it should "prove x>0&a<0&b>=0->[{x'=a*x+b}]x>0 by ghosts" in withMathematica { _ =>
    val result = proveBy(
      "x>0&a<0&b>=0->[{x'=a*x+b}]x>0".asFormula,
      implyR(1) &
  ...
  it should "work with maybe bound" in withQE { _ =>
    ...
    result
      .subgoals
      .loneElement shouldBe "==> \\forall t_ (t_>=0 -> \\forall x (x=2*t_+x_1 -> [{x:=x+3;}* ++ y:=x;]y>0))".asSequent
  }
  ...
  it should "find solution for x'=v with evolution domain constraint" in withQE { _ =>
    ...
    result
      .subgoals
      .loneElement shouldBe "x>0 & v>=0 ==> \\forall t_ (t_>=0 -> \\forall s_ (0<=s_&s_<=t_ -> v*s_+x>=0) -> v*t_+x>0)"
      .asSequent
  }
  ...
  it should "solve straight 2D driving" in withQE { _ =>
    ...
    result
      .subgoals
      .loneElement shouldBe "==> \\forall t_ (t_>=0 -> (dx*(a*(t_^2/2)+v*t_)+x)^2+(dy*(a*(t_^2/2)+v*t_)+y)^2<=r^2)"
      .asSequent
  }
  ...
  it should "solve more complicated constants" in withQE { _ =>
    ...
    result
      .subgoals
      .loneElement shouldBe "==> \\forall t_ (t_>=0 -> (dx^2*((a+c)*(t_^2/2)+v*t_+5*t_)+x)^2+(c*((3-dy)*((a+c)*(t_^2/2)+v*t_))+y)^2<=r^2)"
      .asSequent
  }
  ...
  it should "work when ODE is not sole formula in succedent" in withQE { _ =>
    ...
    result.subgoals.loneElement shouldBe "x>0 & v>=0 & a>0 ==> y=1, \\forall t_ (t_>=0 -> a*(t_^2/2)+v*t_+x>0), z=3"
      .asSequent
  }
  ...
  it should "solve simple nested ODEs" in withQE { _ =>
    val result = proveBy("x>0 ==> [{x'=2}][{x'=3}]x>0".asSequent, solve(1))
    result.subgoals.loneElement shouldBe "x_1>0 ==> \\forall t_ (t_>=0 -> \\forall x (x=2*t_+x_1 -> [{x'=3}]x>0))"
      .asSequent
  }
  ...
  it should "solve outer nested ODEs even when innermost cannot be solved" in withQE { _ =>
    ...
    result
      .subgoals
      .loneElement shouldBe "x_1>0 ==> \\forall t__0 (t__0>=0->\\forall x_2 (x_2=2*t__0+x_1->\\forall t_ (t_>=0->\\forall x (x=3*t_+x_2->[{x'=x}]x>0))))"
      .asSequent
  }

  it should "not try to preserve t_>=0 in evolution domain constraint when solving nested ODEs" in withQE { _ =>
    val result =
      proveBy("x>0 ==> [{x'=2}][{x'=3}][{x'=x}]x>0".asSequent, solve(1) & (allR(1) & implyR(1)) * 2 & solve(1))
    result
      .subgoals
      .loneElement shouldBe "x_1>0, t_>=0, x_2=2*t_+x_1 ==> \\forall t_ (t_>=0->\\forall x (x=3*t_+x_2->[{x'=x}]x>0))"
      .asSequent
  }

  it should "solve complicated nested ODEs" in withQE { _ =>
    val result = proveBy(
      "v=0 & x<s & 0<T, t=0, a_0=(s-x)/T^2 ==> [{x'=v,v'=a_0,t'=1&v>=0&t<=T}](t>0->\\forall a (a = (v^2/(2 *(s - x)))->[{x'=v,v'=-a,t'=1 & v>=0}](x + v^2/(2*a) <= s & (x + v^2/(2*a)) >= s)))"
        .asSequent,
      solve(1),
    )
    result
      .subgoals
      .loneElement shouldBe "v_1=0 & x_1<s & 0<T, t_1=0, a_0=(s-x_1)/T^2 ==> \\forall t_ (t_>=0->\\forall s_ (0<=s_&s_<=t_->a_0*s_+v_1>=0&s_+t_1<=T)->\\forall t (t=t_+t_1->\\forall v (v=a_0*t_+v_1->\\forall x (x=a_0*(t_^2/2)+v_1*t_+x_1->t>0->\\forall a (a=v^2/(2*(s-x))->[{x'=v,v'=-a,t'=1&v>=0}](x+v^2/(2*a)<=s&x+v^2/(2*a)>=s))))))"
      .asSequent
  }
  ...
  it should "retain initial evolution domain for the sake of contradictions" in withQE { _ =>
    ...
    result
      .subgoals
      .loneElement shouldBe "x<=0 ==> \\forall t_ (t_>=0 -> \\forall s_ (0<=s_ & s_<=t_ -> s_+x>0) -> t_+x>0)".asSequent
  }
  ...
  it should "preserve contradictions in constants" in withQE { _ =>
    ...
    result
      .subgoals
      .loneElement shouldBe "y>0 ==> \\forall t_ (t_>=0 -> \\forall s_ (0<=s_ & s_<=t_ -> y <= 0) -> t_+x>0)".asSequent
  }
  ...
  it should "solve triple integrator with division" in withMathematica { _ =>
    ...
    result
      .subgoals
      .loneElement shouldBe "x>=0&v>=0&a>=0&s>=0&g()>0 ==> \\forall t_ (t_>=0->(s*(t_^3/6)+a*(t_^2/2))/g()+v*t_+x>=0)"
      .asSequent
  }
  ...
  it should "solve double integrator with sum of constants" in withQE { _ =>
    ...
    result
      .subgoals
      .loneElement shouldBe "y<b, x<=0, Y()>=0, Z()<Y() ==> \\forall t_ (t_>=0 -> (Z()-Y())*(t_^2/2)+x*t_+y<b)"
      .asSequent
  }

  "endODEHeuristic" should "instantiate with duration in positive polarity in succedent" in withQE { _ =>
    proveBy("x>=0 ==> [{x'=1 & x<=5}]x>=0".asSequent, solve(1) & DifferentialTactics.endODEHeuristic)
      .subgoals
      .loneElement shouldBe "x>=0 ==> \\forall t_ (t_>=0->t_+x<=5->t_+x>=0)".asSequent
  }

  it should "not try to instantiate in negative polarity in succedent" in withQE { _ =>
    proveBy(" ==> ![{x'=1 & x<=5}]x>=0".asSequent, solve(1, 0 :: Nil) & DifferentialTactics.endODEHeuristic)
      .subgoals
      .loneElement shouldBe "==> !\\forall t_ (t_>=0->\\forall s_ (0<=s_&s_<=t_->s_+x<=5)->t_+x<=5&t_+x>=0)".asSequent
  }

  it should "instantiate in negative polarity in antecedent" in withQE { _ =>
    proveBy("![{x'=1 & x<=5}]x>=0, x>=0 ==> ".asSequent, solve(-1, 0 :: Nil) & DifferentialTactics.endODEHeuristic)
      .subgoals
      .loneElement shouldBe "!\\forall t_ (t_>=0->t_+x<=5->t_+x>=0), x>=0 ==> ".asSequent
  }

  it should "not try to instantiate in positive polarity in antecedent" in withQE { _ =>
    proveBy("[{x'=1 & x<=5}]x>=0 ==> ".asSequent, solve(-1) & DifferentialTactics.endODEHeuristic)
      .subgoals
      .loneElement shouldBe "\\forall t_ (t_>=0->\\forall s_ (0<=s_&s_<=t_->s_+x<=5)->t_+x<=5&t_+x>=0) ==> ".asSequent
  }
  ...
  it should "keep positions stable" in withTactics {
    val result =
      proveBy("==> a=1, [{x'=3&x>=0}]x>=0, b=2".asSequent, DifferentialTactics.diffUnpackEvolutionDomainInitially(2))
    result.subgoals.loneElement shouldBe "x>=0 ==> a=1, [{x'=3&x>=0}]x>=0, b=2".asSequent
  }

  "Differential Invariants" should "FEATURE_REQUEST: prove random differential invariant equations" taggedAs TodoTest in withMathematica {
    tool =>
      ...
      for (i <- 1 to randomTrials) {
        ...
        withSafeClue("Random invariant " + invString + "\n" + randClue) {
          ...
          val y = vars(1)
          val parts = {
            try { Some((tool.deriveBy(Neg(inv), y), tool.deriveBy(inv, x))) }
            catch {
          ...
          parts match {
            ...
            case Some((diffy: Term, diffx: Term)) =>
              val sys = ODESystem(
                DifferentialProduct(AtomicODE(DifferentialSymbol(x), diffy), AtomicODE(DifferentialSymbol(y), diffx)),
                True,
              )
              ...
              val opit =
                if (rand.rand.nextBoolean()) (a: Term, b: Term) => cmp(a, b)
                else {
              ...
              withSafeClue("Random differential invariant " + conjecture + "\n" + randClue) {
                print(conjecture)
                val result = proveBy(conjecture, implyR(1) & dI()(1))
                result shouldBe Symbol("proved")
              }
          }
        }
  ...
  it should "FEATURE_REQUEST: prove crazy ^0 case" taggedAs TodoTest in withQE { _ =>
    // @note test is supposed to fail until feature is implemented
    proveBy(
      "x+(y-y-(0-(0+0/1)+(41+x)^0))>=68->[{x'=0,y'=1&true}]x+(y-y-(0-(0+0/1)+(41+x)^0))>=68".asFormula,
      implyR(1) & dI()(1),
    ) shouldBe Symbol("proved")
  }
  it should "FEATURE_REQUEST: prove crazy case" taggedAs TodoTest in withQE { _ =>
    // @note test is supposed to fail until feature is implemented
    proveBy(
      "(z+y+x)*(41/(67/x+((0+0)/y)^1))!=94->[{x'=-41/67*x,y'=41/67*x+41/67*(x+y+z)&true}](z+y+x)*(41/(67/x+((0+0)/y)^1))!=94"
        .asFormula,
      implyR(1) & dI()(1),
    ) shouldBe Symbol("proved")
  }
  ...
  it should "open diff ind x>b() |- [{x'=2}]x>b()" in withQE { _ =>
    proveBy(
      Sequent(IndexedSeq("x>b()".asFormula), IndexedSeq("[{x'=2}]x>b()".asFormula)),
      openDiffInd(1),
    ) shouldBe Symbol("proved")
  }

  it should "open diff ind x>b |- [{x'=2}]x>b" in withQE { _ =>
    proveBy(Sequent(IndexedSeq("x>b".asFormula), IndexedSeq("[{x'=2}]x>b".asFormula)), openDiffInd(1)) shouldBe Symbol(
      "proved"
    )
  }
  ...
  it should "prove a strict barrier certificate 1" in withMathematica { qeTool =>
    val seq =
      "(87*x^2)/200 - (7*x*y)/180 >= -(209*y^2)/1080 + 10 ==> [{x'=(5*x)/4 - (5*y)/6, y'=(9*x)/4 + (5*y)/2}] (87*x^2)/200 - (7*x*y)/180>= -(209*y^2)/1080 + 10 "
        .asSequent
    TactixLibrary.proveBy(seq, DifferentialTactics.dgBarrier(1)) shouldBe Symbol("proved")
  }
  ...
  it should "prove a strict barrier certificate 2" in withMathematica { qeTool =>
    val seq =
      "(23*x^2)/11 + (34*x*y)/11 + (271*y^2)/66 - 5 <= 0 ==> [{x'=(x/2) + (7*y)/3 , y'=-x - y}] (23*x^2)/11 + (34*x*y)/11 + (271*y^2)/66 - 5<=0"
        .asSequent
    TactixLibrary.proveBy(seq, DifferentialTactics.dgBarrier(1)) shouldBe Symbol("proved")
  }
  ...
  "DConstV" should "extend domain constraint with const assumptions" in withMathematica { _ =>
    val seq =
      "f()>0 , v>0, a>0, b>0, <{x'=c+f()}> x>0, c<0 ==> z=1, a>0, [{v'=a+b,x'=y+f() & x>=v | x>=5}]v>0, x=5, y=1"
        .asSequent
    ...
    pr.subgoals.loneElement shouldBe
      "f()>0, v>0, a>0, b>0, <{x'=c+f()&f()>0&c < 0&true}>x>0, c < 0 ==> z=1, a>0, [{v'=a+b,x'=y+f()&f()>0&a>0&b>0&(x>=v|x>=5)}]v>0, x=5, y=1"
        .asSequent
  }

  it should "extend with assumptions hidden in conjuncts" in withMathematica { _ =>
    val seq =
      "f()>0 & v>0 & x=0 & a>0, <{x'=c+f()}> x>0, x=0&c<0&(v<0 | x>0) ==> z=1 | a>0, !x=5, [{v'=a+b,x'=y+f() & x>=v | x>=5}]v>0, x=5 -> y=1"
        .asSequent
    ...
    pr.subgoals.loneElement shouldBe
      "<{x'=c+f()&true}>x>0, f()>0, x=0, v>0, c < 0, v < 0|x>0, x=0, a>0 ==> z=1|a>0, !x=5, [{v'=a+b,x'=y+f()&f()>0&a>0&(x>=v|x>=5)}]v>0, x=5->y=1"
        .asSequent
  }
  ...
  "dCClosure" should "strengthen the postcondition to its interior and cut in the closure" in withMathematica { _ =>
    val result =
      proveBy("t = 0, x = 1 ==> [{t'=1, x'=x & t <= 1}](x>0&x<=3)".asSequent, DifferentialTactics.dCClosure(true)(1))
    val result2 =
      proveBy("t = 0, x = 1 ==> [{t'=1, x'=x & t <= 1}](x>0&x<=3)".asSequent, DifferentialTactics.dCClosure(false)(1))
  ...
  "Derive" should "correctly derive" in withMathematica { tool =>
    ...
    def powZeroFree[T <: Expression](e: T): T = ExpressionTraversal
      .traverse(
    ...
    forEvery(tasks)({ case (inv, dx, dy, dz) =>
      val ode = DifferentialProduct(
        ...
        DifferentialProduct(AtomicODE(DifferentialSymbol(vy), dy), AtomicODE(DifferentialSymbol(vz), dz)),
      )
      // Uses dI
