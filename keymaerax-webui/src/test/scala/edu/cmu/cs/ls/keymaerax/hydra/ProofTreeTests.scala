...
/** Tests the proof tree data structure. Created by smitsch on 3/31/17. */
class ProofTreeTests extends TacticTestBase {
  ...
  "Tactic execution" should "create a tree with one open goal from implyR" in withDatabase { db =>
    ...
    tree.root.children.head.localProvable.subgoals.head shouldBe
      Sequent(IndexedSeq("x>0".asFormula), IndexedSeq("x>0".asFormula))
  ...
  it should "create a proved tree from QE" in withDatabase { db =>
    withMathematica { _ =>
      ...
      tree
        .openGoals
        .head
        .runTactic("guest", ExhaustiveSequentialInterpreter(_, throwWithDebugInfo = false), QE, "QE", wait = true)
      ...
      tree.root.localProvable.subgoals.head shouldBe Sequent(IndexedSeq(), IndexedSeq("x>0 -> x>0".asFormula))
      tree.root.children should have size 1
      tree.root.children.head.localProvable.conclusion shouldBe
        Sequent(IndexedSeq(), IndexedSeq("x>0 -> x>0".asFormula))
  ...
  it should "create a proved tree from implyR ; QE" in withDatabase { db =>
    withMathematica { _ =>
      val modelContent = """ArchiveEntry "Test" ProgramVariables Real x; End. Problem x>0 -> x>0 End. End."""
      val proofId = db.createProof(modelContent)
  ...
  it should "continue on the correct subgoal after a cut" in withDatabase { db =>
    ...
    tree.nodes.map(_.makerShortName) should contain theSameElementsInOrderAs
      None :: Some("cut") :: Some("cut") :: Some("implyR") :: Nil
    tree.openGoals should have size 2
    val g1 = tree.openGoals.head
    ...
    tree.tactic shouldBe
      cut("y=37".asFormula) &
      Idioms.<(BelleLabels.cutUse -> (implyR(Symbol("R"), "x>0->x>0".asFormula) & todo), BelleLabels.cutShow -> todo)
  }

  it should "use provables with definitions" in withDatabase { db =>
    withMathematica { _ =>
      val modelContent =
        """ArchiveEntry "Test" Definitions Real y=1; End. ProgramVariables Real x; End. Problem x^2/y>=0 End. End."""
      val proofId = db.createProof(modelContent)
  ...
  "Tactic suggestion" should "return single-pos tactics" in withTactics {
    withDatabase { db =>
      val modelContent = """ArchiveEntry "Test" ProgramVariables Real x; End. Problem x>0 -> x>0 End. End."""
      val proofId = db.createProof(modelContent)
      val tree = DbProofTree(db.db, proofId.toString)
  ...
  it should "return single-pos tactics with input suggestions" in withTactics {
    withDatabase { db =>
      val modelContent =
        """ArchiveEntry "Test" ProgramVariables Real x; End. Problem [{x:=x+1;}*@invariant(x>7)]x>5 End. End."""
      val proofId = db.createProof(modelContent)
      val tree = DbProofTree(db.db, proofId.toString)
      tree.openGoals should have size 1
      val tactics = tree.openGoals.head.applicableTacticsAt(SuccPosition(1))
      ...
      tree.openGoals.head.tacticInputSuggestions(SuccPosition(1)).loneElement shouldBe
        (FormulaArg("J") -> "x>7".asFormula)
    }
  }

  it should "return two-pos tactics" in withTactics {
    withDatabase { db =>
      val modelContent = """ArchiveEntry "Test" ProgramVariables Real x; End. Problem x>0->x>0 End. End."""
      val proofId = db.createProof(modelContent)
      var tree = DbProofTree(db.db, proofId.toString)
      ...
      tree = DbProofTree(db.db, proofId.toString)
      tree.openGoals.head.applicableTacticsAt(AntePosition(1), Some(SuccPosition(1))).loneElement._1.codeName shouldBe
        "closeId"
    }
  }
  ...
  it should "create the same tree as ripple loading from proof with steps" in withTactics {
    withDatabase { db =>
      val modelContent = """ArchiveEntry "Test" ProgramVariables Real x; End. Problem x>0->x>0 End. End."""
      val proofId = db.createProof(modelContent)
      ...
      for (i <- 0 until numStepsPerProof) {
        ...
        ft.openGoals should have size st.openGoals.size
        ft.openGoals.zip(st.openGoals).foreach({ case (ftn, stn) => ftn.goal shouldBe stn.goal })
        ft.openGoals.head.applicableTacticsAt(SuccPosition(1)) shouldBe
          st.openGoals.head.applicableTacticsAt(SuccPosition(1))
  ...
  it should "create the same tree as ripple loading from proof with steps that do not have provables" in withTactics {
    withDatabase { db =>
      val modelContent = """ArchiveEntry "Test" ProgramVariables Real x; End. Problem x>0->x>0 End. End."""
      val proofId = db.createProof(modelContent)
  ...
  private def checkTree(
      db: DBAbstraction,
      proofId: Int,
      tactic: BelleExpr,
      expected: Sequent,
      expectOpenGoals: List[Sequent] = Nil,
  ): ProvableSig = {
    var tree = DbProofTree(db, proofId.toString)
    tree
      .openGoals
      .loneElement
      .runTactic("guest", LazySequentialInterpreter(_, throwWithDebugInfo = false), tactic, "proof", wait = true)
    ...
    } else { tree.openGoals.flatMap(_.goal) should contain theSameElementsInOrderAs expectOpenGoals }
    p.conclusion shouldBe expected
    p
  }

  "Delayed substitution" should "create a provable with expanded definitions from expand/expandAllDefs" in withTactics {
    withDatabase { db =>
      withMathematica { _ =>
        val modelContent = {
          """
  ...
  it should "create a provable with expanded definitions from US tactic" in withDatabase { db =>
    withMathematica { _ =>
      val modelContent = {
        """
          |ArchiveEntry "Delayed Substitution"
  ...
  it should "create a provable with expanded definitions with the lazy interpreter" in withDatabase { db =>
    withMathematica { _ =>
      val modelContent = {
        """
          |ArchiveEntry "Delayed Substitution"
  ...
  it should "work with lemmas that expand all definitions" in withDatabase { db =>
    withMathematica { _ =>
      val modelContent = {
        """Definitions
          |  Real sq(Real x) = x*x;
      ...
      LemmaDBFactory
        ...
        .add(Lemma(
          ...
          Lemma.requiredEvidence(
            provedLemma,
            ToolEvidence(
              List("tool" -> "KeYmaera X", "model" -> lemma.fileContent, "tactic" -> lemma.tactics.loneElement._2)
            ) :: Nil,
          ),
          Some("user/tests/Lemma 1"),
        ))
  ...
  it should "work with lemmas that partially expand" in withDatabase { db =>
    withMathematica { _ =>
      val modelContent = {
        """Definitions
          |  Real sq(Real x) = x*x;
  ...
  it should "work when expanding to different extent" in withDatabase { db =>
    withMathematica { _ =>
      val modelContent = {
        """Definitions
          |  Real sq(Real x) = x*x;
  ...
  it should "FEATURE_REQUEST: work with delayed let substitution" taggedAs TodoTest in withDatabase { db =>
    withMathematica { _ =>
      ...
      checkTree(
        ...
        proofId,
        implyR(1),
        "==> y+z>=0 -> [x:=y+z;]x>=0".asSequent,
        "y+z>=0 ==> [x:=y+z;]x>=0".asSequent :: Nil,
      )
      checkTree(
        db.db,
        proofId,
        Let("y()".asTerm, "y".asTerm, Let("z()".asTerm, "z".asTerm, assignb(1))),
        "==> y+z>=0 -> [x:=y+z;]x>=0".asSequent,
        "y()+z()>=0 ==> y()+z()>=0".asSequent :: Nil,
      )
  ...
  it should "FEATURE_REQUEST: work when delayed let substitution is not the first step in a tactic" taggedAs TodoTest in
    withDatabase { db =>
      withMathematica { _ =>
        val proofId = db.createProof(
          """ArchiveEntry "Simple"
        ...
        checkTree(
          db.db,
          proofId,
          implyR(1) & Let("y()".asTerm, "y".asTerm, assignb(1)),
          "==> y>=0 -> [x:=y;]x>=0".asSequent,
          "y()>=0 ==> y()>=0".asSequent :: Nil,
        )
        // now finish the proof
  ...
  "Performance" should "not degrade when doing the usual interaction (without tactic extraction) in a loop" in
    withTactics {
      withDatabase { db =>
        val modelContent = """ArchiveEntry "Test" ProgramVariables Real x; End. Problem x>0 -> x>0 End. End."""
        val proofId = db.createProof(modelContent)
        ...
        for (i <- 0 until numStepsPerProof) {
          ...
          if (i % 2 == 1) goals.head.goal shouldBe
            Some(Sequent(IndexedSeq("x>0".asFormula), IndexedSeq("x>0".asFormula)))
          ...
          if (i % 2 == 1) { tactics should have size 1; tactics(0)._1.codeName shouldBe "chaseAt" }
            ...
            tactics should have size 2; tactics(0)._1.codeName shouldBe "implyR";
            tactics(1)._1.codeName shouldBe "chaseAt"
          }
  ...
  it should "not degrade too much when doing the usual interaction with tactic extraction in a loop" in withTactics {
    withDatabase { db =>
      val modelContent = """ArchiveEntry "Test" ProgramVariables Real x; End. Problem x>0 -> x>0 End. End."""
      val proofId = db.createProof(modelContent)
  ...
  it should "not degrade over multiple proofs" in withTactics {
    withDatabase { db =>
      val modelContent = """ArchiveEntry "Test" ProgramVariables Real x; End. Problem x>0 -> x>0 End. End."""
      ...
      for (proof <- 0 until numProofs) {
        ...
        for (i <- 1 to numStepsPerProof) {
          ...
          val goals = tree.openGoals
          goals should have size 1
          if (i % 2 == 0) goals.head.goal shouldBe
            Some(Sequent(IndexedSeq("x>0".asFormula), IndexedSeq("x>0".asFormula)))
          else goals.head.goal shouldBe Some(Sequent(IndexedSeq(), IndexedSeq("x>0->x>0".asFormula)))
