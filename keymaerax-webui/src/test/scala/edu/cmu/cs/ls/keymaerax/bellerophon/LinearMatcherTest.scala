...
class LinearMatcherTest extends SystemTestBase {
  ...
  private lazy val semanticRenaming = UnificationMatch("quark(||)".asPlainFormula, "quarks=6".asPlainFormula).isInstanceOf[URenAboveUSubst]


  "New unification match" should "unify renaming and instance 3*f(x)>2 and 5*x>2" in {
    shouldMatch("3*f(x)>2".asPlainFormula,
      "3*(5*x)>2".asPlainFormula, RenUSubst(
        (FuncOf(Function("f", None, Real, Real), dot), Times(Number(5), dot)) :: Nil
      ))
  }

  it should "unify renaming and instance p(x) and x>5" in {
    shouldMatch("p(x)".asPlainFormula,
      "x>5".asPlainFormula, RenUSubst(
        (PredOf(Function("p", None, Real, Bool), dot), Greater(dot, Number(5))) :: Nil
      ))
  }
  ...
  it should "unify [x:=f();]p(x) with [x:=7+x;]x^2>=5" in {
    shouldMatch("[x:=f();]p(x)".asPlainFormula, "[x:=7+x;]x^2>=5".asPlainFormula, RenUSubst(
        ("f()".asPlainTerm, "7+x".asPlainTerm) ::
          (PredOf(Function("p", None, Real, Bool), dot), GreaterEqual(Power(dot, "2".asPlainTerm), "5".asPlainTerm)) :: Nil))
  }

  it should "fail unify nonlinear [x:=f();]p(x) <-> p(f()) with [x:=7+x;]x^2>=5 <-> (7+x)^2>=5" in {
    a[ProverException] shouldBe thrownBy(matcher("[x:=f();]p(x) <-> p(f())".asPlainFormula, "[x:=7+x;]x^2>=5 <-> (7+x)^2>=5".asPlainFormula))
  }

  it should "unify [x:=f();]p(x) with [y:=7+z;]y^2>=5" in {
    shouldMatch("[x:=f();]p(x)".asPlainFormula, "[y:=7+z;]y^2>=5".asPlainFormula, RenUSubst(
  ...
  it should "fail to unify nonlinear [y:=f();]p(y) <-> p(f()) with [y:=7+z;]y^2>=5 <-> (7+z)^2>=5" in {
    a[ProverException] shouldBe thrownBy(matcher("[y:=f();]p(y) <-> p(f())".asPlainFormula, "[y:=7+z;]y^2>=5 <-> (7+z)^2>=5".asPlainFormula))
  }

  it should "fail to unify nonlinear [x_:=f();]p(x_) <-> p(f()) with [y:=7+z;]y^2>=5 <-> (7+z)^2>=5" in {
    a[ProverException] shouldBe thrownBy(matcher("[x_:=f();]p(x_) <-> p(f())".asPlainFormula, "[y:=7+z;]y^2>=5 <-> (7+z)^2>=5".asPlainFormula))
  }

  it should "fail to unify nonlinear [x:=f();]p(x) <-> p(f()) with [y:=7+z;]y^2>=5 <-> (7+z)^2>=5" in {
    a[ProverException] shouldBe thrownBy(matcher("[x:=f();]p(x) <-> p(f())".asPlainFormula, "[y:=7+z;]y^2>=5 <-> (7+z)^2>=5".asPlainFormula))
  }

  it should "unify [x_:=y;]p(x_) with [y_0:=y;]y_0>2" in {
    shouldMatch("[x_:=y;]p(x_)".asPlainFormula, "[y_0:=y;]y_0>2".asPlainFormula, RenUSubst(
  ...
  it should "unify [x_:=f();]p(x_) with [y_0:=y;]y_0>2" in {
    shouldMatch("[x_:=f();]p(x_)".asPlainFormula, "[y_0:=y;]y_0>2".asPlainFormula, RenUSubst(
      (Variable("x_"), Variable("y",Some(0))) ::
...
//  }
  ...
  it should "maybe unify [x_:=y;]y_0>0<->y_0>0 with [a:=z;]y_0>0<->y_0>0" in {
    shouldMatch("[x_:=y;]y_0>0<->y_0>0".asPlainFormula, "[a:=z;]y_0>0<->y_0>0".asPlainFormula, RenUSubst(
      (Variable("x_"), Variable("a")) :: (Variable("y"), Variable("z")) :: Nil))
  }

  it should "maybe unify [x_:=y;]x_>0<->y>0 with [a:=z;]a>0<->z>0" in {
    shouldMatch("[x_:=y;]x_>0<->y>0".asPlainFormula, "[a:=z;]a>0<->z>0".asPlainFormula, RenUSubst(
      (Variable("x_"), Variable("a")) :: (Variable("y"), Variable("z")) :: Nil))
  }

  it should "maybe unify [x_:=y;]y>0<->y>0 with [a:=z;]z>0<->z>0" in {
    shouldMatch("[x_:=y;]y>0<->y>0".asPlainFormula, "[a:=z;]z>0<->z>0".asPlainFormula, RenUSubst(
      (Variable("x_"), Variable("a")) :: (Variable("y"), Variable("z")) :: Nil))
  }

  //@todo really? needs cyclic decluttering to say the least
  it should "unify [x_:=y;]y_0>0<->y_0>0 with [y_0:=z;]y_0>0<->z>0" ignore {
    shouldMatch("[x_:=y;]y_0>0<->y_0>0".asPlainFormula, "[y_0:=z;]y_0>0<->z>0".asPlainFormula, RenUSubst(
      (Variable("x_"), Variable("y",Some(0))) :: Nil))
  }
  ...
  it should "unify q_(x_) & x_=f(x_) -> p_(x_) with complicated formula" ignore {
    shouldMatch("q_(x_) & x_=f(x_) -> p_(x_)".asPlainFormula,
      ...
      RenUSubst(
        ...
        ("p_(._0)".asPlainFormula, "v>=0&.+v^2/(2*B)<=S".asPlainFormula) ::
  ...
  "Dassignb unification" should "unify [u':=f();]p(u') with [u':=b();]u'>=0" in {
    shouldMatch("[u':=f();]p(u')".asPlainFormula, "[u':=b();]u'>=0".asPlainFormula, RenUSubst(
  ...
  it should "unify [x':=f();]p(x') with [u':=b();]u'>=0" in {
    shouldMatch("[x':=f();]p(x')".asPlainFormula, "[u':=b();]u'>=0".asPlainFormula, RenUSubst(
  ...
  "More unification" should "maybe unify p() -> [a;]p() with y>0 -> [x:=2;]y>0 " in {
    shouldMatch("p() -> [a;]p()".asPlainFormula, "y>0 -> [x:=2;]y>0".asPlainFormula, RenUSubst(
  ...
  it should "unify [a;]p() -> p() with [x:=2;]y>0 -> y>0" in {
    ...
    shouldMatch("[a;]p() -> p()".asPlainFormula, "[x:=2;]y>0 -> y>0".asPlainFormula, RenUSubst(
  ...
  it should "unify renaming and instance [y:=y;]p(||) and [y_0:=y_0;](y_0>77&true)" in {
    shouldMatch("[y:=y;]p(||)".asPlainFormula,
      "[y_0:=y_0;](y_0>77&true)".asPlainFormula, RenUSubst(
  ...
  it should "maybe unify renaming and instance [y:=y;]p(||)<->p(||) and [y_0:=y_0;](true)<->(true)" in {
    shouldMatch("[y:=y;]p(||)<->p(||)".asPlainFormula,
      "[y_0:=y_0;](true)<->(true)".asPlainFormula, RenUSubst(
        (Variable("y"), Variable("y",Some(0))) ::
  ...
  it should "unify renaming x=0 and y_0=0" in {
    shouldMatch("x=0".asPlainFormula,
      "y_0=0".asPlainFormula, RenUSubst(
  ...
  it should "maybe unify renaming x>1&x=2<->x<3 and y_0>1&y_0=2<->y_0<3" in {
    shouldMatch("x>1&x=2<->x<3".asPlainFormula,
      "y_0>1&y_0=2<->y_0<3".asPlainFormula, RenUSubst(
      (Variable("x"), Variable("y",Some(0))) :: Nil
    ))
  }
  ...
  it should "maybe unify renaming and instance [y:=y;]y>5<->y>5 and [y_0:=y_0;]y_0>5<->y_0>5" in {
    shouldMatch("[y:=y;]y>5<->y>5".asPlainFormula,
      "[y_0:=y_0;]y_0>5<->y_0>5".asPlainFormula, RenUSubst(
  ...
  it should "maybe unify renaming and instance p(||)<->[y:=y;]p(||) and (y_0=1)<->[y_0:=y_0;](y_0=1)" ignore {
    shouldMatch("p(||)<->[y:=y;]p(||)".asPlainFormula,
      "(y_0=1)<->[y_0:=y_0;](y_0=1)".asPlainFormula, RenUSubst(
      (Variable("y"), Variable("y",Some(0))) ::
  ...
  it should "maybe unify renaming and instance [y:=y;]p(||)<->p(||) and [y_0:=y_0;](y_0=0)<->(y_0=0)" in {
    shouldMatch("[y:=y;]p(||)<->p(||)".asPlainFormula,
      "[y_0:=y_0;](y_0=0)<->(y_0=0)".asPlainFormula, RenUSubst(
      (Variable("y"), Variable("y",Some(0))) ::
  ...
  it should "maybe unify renaming and instance p(||)<->[y:=y;]p(||) and (true)<->[y_0:=y_0;](true)" in {
    shouldMatch("p(||)<->[y:=y;]p(||)".asPlainFormula,
      "(true)<->[y_0:=y_0;](true)".asPlainFormula, RenUSubst(
      (Variable("y"), Variable("y",Some(0))) ::
        (UnitPredicational("p", AnyArg), "(true)".asPlainFormula) ::
        Nil
    ))
  ...
  it should "maybe unify renaming and instance p(||)<->[y:=y;]p(||) and (y_0>77&true)<->[y_0:=y_0;](y_0>77&true)" ignore {
    shouldMatch("p(||)<->[y:=y;]p(||)".asPlainFormula,
      "(y_0>77&true)<->[y_0:=y_0;](y_0>77&true)".asPlainFormula, RenUSubst(
      (Variable("y"), Variable("y",Some(0))) ::
        (UnitPredicational("p", AnyArg), (if (semanticRenaming) "(y_0>77&true)" else "y>77&true").asPlainFormula) ::
        Nil
    ))
  ...
  it should "maybe unify renaming and instance [y:=y;]p(||)<->p(||) and [y_0:=y_0;](y_0>77&true)<->(y_0>77&true)" in {
    shouldMatch("[y:=y;]p(||)<->p(||)".asPlainFormula,
      "[y_0:=y_0;](y_0>77&true)<->(y_0>77&true)".asPlainFormula, RenUSubst(
      (Variable("y"), Variable("y",Some(0))) ::
        (UnitPredicational("p", AnyArg), (if (semanticRenaming) "(y_0>77&true)" else "y>77&true").asPlainFormula) ::
        Nil
    ))
  ...
  it should "maybe unify renaming and long instance" in {
    shouldMatch("[x_:=x_;]p(||)<->p(||)".asPlainFormula,
      "[x_0:=x_0;](((x_0>0&true)&true)&true->(2>=0|false)|false)<->((x_0>0&true)&true)&true->(2>=0|false)|false".asPlainFormula, RenUSubst(
  ...
  it should "match abstract loop against loopy single ODE" in {
    shouldMatch("[{a;}*]p(||)".asPlainFormula,
      "[{{x'=v}}*](v>=0&true)".asPlainFormula, RenUSubst(
  ...
  it should "match abstract loop against loopy ODE system " in {
    shouldMatch("[{a;}*]p(||)".asPlainFormula,
      "[{{x'=v,v'=A}}*](v>=0&true)".asPlainFormula, RenUSubst(
  ...
  it should "match abstract loop against loopy ODE system with domain" in {
    shouldMatch("[{a;}*]p(||)".asPlainFormula,
      "[{{x'=v,v'=A&v<=5}}*](v>=0&true)".asPlainFormula, RenUSubst(
  ...
  it should "match abstract loop against loopy initialized ODE" in {
    shouldMatch("[{a;}*]p(||)".asPlainFormula,
      "[{v:=5;{x'=v,v'=A}}*](v>=0&true)".asPlainFormula, RenUSubst(
  ...
  "Reunifier ideally" should "fail to unify nonlinear p(f()) <-> [x:=f();]p(x) with (7+x)^2>=5 <-> [x:=7+x;]x^2>=5" taggedAs OptimisticTest ignore {
    a[ProverException] shouldBe thrownBy(matcher("p(f()) <-> [x:=f();]p(x)".asPlainFormula, "(7+x)^2>=5 <-> [x:=7+x;]x^2>=5".asPlainFormula))
  }
  ...
  it should "projection unify renaming and instance p(x,y) and x*y>5" taggedAs(IgnoreInBuildTest, TodoTest) ignore {
    shouldMatch("p(x,y)".asPlainFormula,
      "x*y>5".asPlainFormula, RenUSubst(
        ("p(.(.,.))".asPlainFormula,
          "π(.(.,.),1,0)*π(.(.,.),1,1)>5".asPlainFormula) :: Nil
      ))
  }
  ...
  it should "projection unify renaming and instance p(x,y,z) and x*y>z" taggedAs(IgnoreInBuildTest, TodoTest) ignore {
    shouldMatch("p(x,y,z)".asPlainFormula,
      "x*y>z".asPlainFormula, RenUSubst(
        ("p(.(.,.,.))".asPlainFormula,
          "π(.(.,.,.),1,0)*π(.(.,.,.),2,2)>π(.(.,.,.),2,3)".asPlainFormula) :: Nil
      ))
  }
}
