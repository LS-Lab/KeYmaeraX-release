...
class HilbertTests extends TacticTestBase {
  ...
  "Hilbert calculus" should "prove x>=5 -> [{x'=2&x<=9}]x<=9" in withTactics {
    proveBy(
      Sequent(IndexedSeq(), IndexedSeq("x>=5 -> [{x'=2&x<=9}]x<=9".asFormula)),
      implyR(1) &
        DW(1) &
  ...
  it should "prove x>=5 -> [{x'=2&x<=9}]x<=10" in withMathematica { _ =>
    proveBy(
      Sequent(IndexedSeq(), IndexedSeq("x>=5 -> [{x'=2&x<=9}]x<=10".asFormula)),
      implyR(1) &
        DW(1) &
        abstractionb(1) & QE,
    ) shouldBe Symbol("proved")
  ...
  it should "prove x>=5 -> [{x'=2}](x>=5)'" in withMathematica { _ =>
    proveBy(
      Sequent(IndexedSeq(), IndexedSeq("x>=5 -> [{x'=2}](x>=5)'".asFormula)),
      implyR(1) &
        DE(1) &
  ...
  it should "prove (y)'=y forward" in withMathematica { _ =>
    ...
    proveBy(
      ...
      Dvar(1, 0 :: Nil) & byUS(Ax.equalReflexive),
    ) shouldBe Symbol("proved")
    proveBy(
      Sequent(IndexedSeq(), IndexedSeq(Equal(Differential(x), DifferentialSymbol(x)))),
      Dvar(1, 0 :: Nil) & byUS(Ax.equalReflexive),
  ...
  it should "derive (y)'=y'" in withMathematica { _ =>
    proveBy(
      Sequent(IndexedSeq(), IndexedSeq("(y)'=y'".asFormula)),
  ...
  it should "derive (x+y)'=x'+y'" in withMathematica { _ =>
    proveBy(
      Sequent(IndexedSeq(), IndexedSeq("(x+y)'=x'+y'".asFormula)),
      derive(1, 0 :: Nil) & byUS(Ax.equalReflexive),
    ) shouldBe Symbol("proved")
  }
  ...
  it should "derive (x*y)'=x'*y+x*y'" in withMathematica { _ =>
    proveBy(
      Sequent(IndexedSeq(), IndexedSeq("(x*y)'=x'*y+x*y'".asFormula)),
      derive(1, 0 :: Nil) & byUS(Ax.equalReflexive),
    ) shouldBe Symbol("proved")
  }
  ...
  it should "derive (x^2)' >= 7 without crashing" in withMathematica { _ =>
    proveBy(Sequent(IndexedSeq(), IndexedSeq("(x^2)' >= 7".asFormula)), HilbertCalculus.stepAt(1, 0 :: Nil))
      .subgoals shouldBe List(Sequent(IndexedSeq(), IndexedSeq("(2 * (x^(2-1))) * (x)' >= 7".asFormula)))
  }

  it should "FEATURE_REQUEST: derive (5*3+2*9)'=0*3+5*0+(0*9+2*0) unless optimized" taggedAs TodoTest in withMathematica {
    _ =>
      proveBy(
        Sequent(IndexedSeq(), IndexedSeq("(5*3+2*9)'=0*3+5*0+(0*9+2*0)".asFormula)),
        derive(1, 0 :: Nil) & byUS(Ax.equalReflexive),
      ) shouldBe Symbol("proved")
  }

  it should "FEATURE_REQUEST: derive (5*3+2*9)'=5*0+2*0 if optimized (left linear preferred but not const optimized)" taggedAs TodoTest in withMathematica {
    _ =>
      proveBy(
        Sequent(IndexedSeq(), IndexedSeq("(5*3+2*9)'=5*0+2*0".asFormula)),
        derive(1, 0 :: Nil) & byUS(Ax.equalReflexive),
      ) shouldBe Symbol("proved")
  }
  ...
  it should "derive (5*3+2*9)'=0 if optimized (const optimized)" in withMathematica { _ =>
    proveBy(
      Sequent(IndexedSeq(), IndexedSeq("(5*3+2*9)'=0".asFormula)),
      derive(1, 0 :: Nil) & byUS(Ax.equalReflexive),
    ) shouldBe Symbol("proved")
  }
  ...
  it should "derive (5*x+2*y)'=5*x'+2*y'" in withMathematica { _ =>
    proveBy(
      Sequent(IndexedSeq(), IndexedSeq("(5*x+2*y)'=5*x'+2*y'".asFormula)),
      derive(1, 0 :: Nil) & byUS(Ax.equalReflexive),
    ) shouldBe Symbol("proved")
  }
  ...
  it should "derive (5*x+2*y>=6)' <-> 5*x'+2*y'>=0" in withMathematica { _ =>
    proveBy(
      Sequent(IndexedSeq(), IndexedSeq("(5*x+2*y>=6)' <-> 5*x'+2*y'>=0".asFormula)),
  ...
  it should "derive (7*x<2*y & 22*x=4*y+8)' <-> (7*x'<=2*y' & 22*x'=4*y'+0)" in withMathematica { _ =>
    proveBy(
      Sequent(IndexedSeq(), IndexedSeq("(7*x<2*y & 22*x=4*y+8)' <-> (7*x'<=2*y' & 22*x'=4*y'+0)".asFormula)),
      derive(1, 0 :: Nil) & byUS(Ax.equivReflexive),
    ) shouldBe Symbol("proved")
  }

  it should "derive (x*x<2*y & 5*x+2*y>=6+z & 22*x=4*y+8)' <-> (x'*x+x*x'<=2*y' & 5*x'+2*y'>=0+z' & 22*x'=4*y'+0)" in withMathematica {
    _ =>
      proveBy(
        Sequent(
          IndexedSeq(),
          IndexedSeq(
            "(x*x<2*y & 5*x+2*y>=6+z & 22*x=4*y+8)' <-> (x'*x+x*x'<=2*y' & 5*x'+2*y'>=0+z' & 22*x'=4*y'+0)".asFormula
          ),
        ),
        derive(1, 0 :: Nil) & byUS(Ax.equivReflexive),
      ) shouldBe Symbol("proved")
  ...
  it should "derive [{x'=7,y'=-9,z'=2}](x*x<2*y & 5*x+2*y>=6+z & 22*x=4*y+8)' <-> [{x'=7,y'=-9,z'=2}](x'*x+x*x'<=2*y' & 5*x'+2*y'>=0+z' & 22*x'=4*y'+0)" in withMathematica {
    _ =>
      proveBy(
        Sequent(
          IndexedSeq(),
          IndexedSeq(
            "[{x'=7,y'=-9,z'=2}](x*x<2*y & 5*x+2*y>=6+z & 22*x=4*y+8)' <-> [{x'=7,y'=-9,z'=2}](x'*x+x*x'<=2*y' & 5*x'+2*y'>=0+z' & 22*x'=4*y'+0)"
              .asFormula
          ),
        ),
  ...
  it should "reduce [{x'=7}](5*x>=6)' to [{x'=7}]5*x'>=0" in withMathematica { _ =>
    proveBy(Sequent(IndexedSeq(), IndexedSeq("[{x'=7}](5*x>=6)'".asFormula)), derive(1, 1 :: Nil))
  ...
  it should "FEATURE_REQUEST: prove x>=5 -> [{x'=2&x<=9}](5<=x&x<=10)" taggedAs TodoTest in withMathematica { _ =>
    proveBy(
      Sequent(IndexedSeq(), IndexedSeq("x>=5 -> [{x'=2&x<=9}](5<=x&x<=10)".asFormula)),
      implyR(1) &
  ...
  it should "FEATURE_REQUEST: auto-prove x>=5 -> [{x'=2&x<=9}](5<=x&x<=10) with DC" taggedAs TodoTest in withMathematica {
    _ =>
      proveBy(
        Sequent(IndexedSeq(), IndexedSeq("x>=5 -> [{x'=2&x<=9}](5<=x&x<=10)".asFormula)),
        implyR(1) &
  ...
  it should "step fallback assignb/d only when index is defined" in withMathematica { _ =>
    ...
    proveBy("x>=0 ==> [x:=x+1;][{x'=-x}]x>0".asSequent, step(1))
      .subgoals
      .loneElement shouldBe "x_0>=0, x=x_0+1 ==> [{x'=-x}]x>0".asSequent
    proveBy("x>=0 ==> <x:=x+1;>[{x'=-x}]x>0".asSequent, step(1))
      .subgoals
      .loneElement shouldBe "x_0>=0, x=x_0+1 ==> [{x'=-x}]x>0".asSequent
    // @note prop triggers same "no axioms applicable" error as above, but for different reasons, so shouldn't fallback to assignb
    proveBy("x>=0 ==> [x:=x+1;][{x'=-x}]x>0".asSequent, prop)
      .subgoals
      .loneElement shouldBe "x>=0 ==> [x:=x+1;][{x'=-x}]x>0".asSequent
  }
  ...
  it should "reduce x>5 |- [x:=x+1;][x:=2*x;]x>1 to x>5 |- [x:=x+1;x:=2*x;]x>1 by useAt backwards" in withTactics {
    proveBy(
      ...
      useAt(Ax.composeb, PosInExpr(1 :: Nil))(SuccPos(0)),
  ...
  it should "reduce [x:=x+1;x:=2*x;]x>1 |- x>5 to [x:=x+1;][x:=2*x;]x>1 |- x>5 by useAt" in withTactics {
    proveBy(
      Sequent(IndexedSeq("[x:=x+1;x:=2*x;]x>1".asFormula), IndexedSeq("x>5".asFormula)),
      useAt(Ax.composeb)(AntePos(0)),
    ).subgoals should contain only Sequent(IndexedSeq("[x:=x+1;][x:=2*x;]x>1".asFormula), IndexedSeq("x>5".asFormula))
  }

  it should "reduce [x:=x+1;][x:=2*x;]x>1 |- x>5 to [x:=x+1;x:=2*x;]x>1 |- x>5 by useAt backwards" in withTactics {
    proveBy(
      Sequent(IndexedSeq("[x:=x+1;][x:=2*x;]x>1".asFormula), IndexedSeq("x>5".asFormula)),
      useAt(Ax.composeb, PosInExpr(1 :: Nil))(AntePos(0)),
    ).subgoals should contain only Sequent(IndexedSeq("[x:=x+1;x:=2*x;]x>1".asFormula), IndexedSeq("x>5".asFormula))
  }

  it should "reduce x>5 |- [c;d;]x>1 to x>5 |- [c;][d;]x>1 by useAt" in withTactics {
    proveBy(Sequent(IndexedSeq("x>5".asFormula), IndexedSeq("[c;d;]x>1".asFormula)), useAt(Ax.composeb)(SuccPos(0)))
      .subgoals should contain only Sequent(IndexedSeq("x>5".asFormula), IndexedSeq("[c;][d;]x>1".asFormula))
  }

  it should "reduce x>5 |- [c;][d;]x>1 to x>5 |- [c;d;]x>1 by useAt backwards" in withTactics {
    proveBy(
      Sequent(IndexedSeq("x>5".asFormula), IndexedSeq("[c;][d;]x>1".asFormula)),
      useAt(Ax.composeb, PosInExpr(1 :: Nil))(SuccPos(0)),
    ).subgoals should contain only Sequent(IndexedSeq("x>5".asFormula), IndexedSeq("[c;d;]x>1".asFormula))
  }

  it should "reduce [c;d;]x>1 |- x>5 to [c;][d;]x>1 |- x>5 by useAt" in withTactics {
    proveBy(Sequent(IndexedSeq("[c;d;]x>1".asFormula), IndexedSeq("x>5".asFormula)), useAt(Ax.composeb)(AntePos(0)))
      .subgoals should contain only Sequent(IndexedSeq("[c;][d;]x>1".asFormula), IndexedSeq("x>5".asFormula))
  }

  it should "reduce [c;][d;]x>1 |- x>5 to [c;d;]x>1 |- x>5 by useAt backwards" in withTactics {
    proveBy(
      Sequent(IndexedSeq("[c;][d;]x>1".asFormula), IndexedSeq("x>5".asFormula)),
      useAt(Ax.composeb, PosInExpr(1 :: Nil))(AntePos(0)),
    ).subgoals should contain only Sequent(IndexedSeq("[c;d;]x>1".asFormula), IndexedSeq("x>5".asFormula))
  }

  it should "reduce [ctrl;ode;](x>0&y<1) by composeb" in withTactics {
    proveBy("[ctrl{|^@|};ode{|^@|};](x>0&y<1)".asFormula, useAt(Ax.boxAnd)(1)).subgoals should contain only Sequent(
      IndexedSeq(),
      IndexedSeq("([ctrl{|^@|};ode{|^@|};]x>0)&([ctrl{|^@|};ode{|^@|};]y<1)".asFormula),
    )
    proveBy(
      "[ctrl{|^@|};ode{|^@|};](x>0&y<1)".asFormula,
      useAt(Ax.composeb)(1) &
        useAt(Ax.boxAnd)(1, PosInExpr(1 :: Nil)),
    ).subgoals should contain only Sequent(
      IndexedSeq(),
      IndexedSeq("[ctrl{|^@|};](([ode{|^@|};]x>0)&([ode{|^@|};]y<1))".asFormula),
    )
    proveBy(
      "[ctrl{|^@|};ode{|^@|};](x>0&y<1)".asFormula,
      useAt(Ax.composeb)(1) &
  ...
  it should "use Barcan" in withTactics {
    proveBy("[x:=2;]\\forall y y<=x".asFormula, useAt(Ax.barcan)(1))
      .subgoals
      .loneElement shouldBe "==> \\forall y [x:=2;]y<=x".asSequent
    the[InapplicableUnificationKeyFailure] thrownBy proveBy(
      "[x:=2;]\\forall x x<=3".asFormula,
      useAt(Ax.barcan)(1),
    ) should
  ...
  it should "use box true" in withTactics {
    ...
    the[SubderivationSubstitutionException] thrownBy proveBy(
      "x=3 ==> [x:=0;]true".asSequent,
      useAt(Ax.boxTrueAxiom)(1),
    ) should have message
    ...
    proveBy("x=3 ==> [x:=0;]true".asSequent, useAt(Ax.boxTrueTrue)(1)).subgoals.loneElement shouldBe "x=3 ==> true"
      .asSequent
    proveBy("x=3 ==> [x:=0;]true".asSequent, HilbertCalculus.boxTrue(1)) shouldBe Symbol("proved")
    proveBy("==> [y:=1;][x:=0;]true".asSequent, HilbertCalculus.boxTrue(1, 1 :: Nil))
      .subgoals
      .loneElement shouldBe "==> [y:=1;]true".asSequent
    proveBy("[x:=0;]true ==> ".asSequent, HilbertCalculus.boxTrue(-1)).subgoals.loneElement shouldBe "true ==>"
      .asSequent
  }

  "Chase" should "prove [?p();?(p()->q());]p() by chase" in withTactics {
    proveBy(Sequent(IndexedSeq(), IndexedSeq("[?p();?(p()->q());]p()".asFormula)), chase(1) & prop) shouldBe Symbol(
      "proved"
    )
  }
  ...
  it should "prove [?p();?(p()->q()); ++ ?r();?q();]q() by chase" in withTactics {
    proveBy(
      Sequent(IndexedSeq(), IndexedSeq("[?p();?(p()->q()); ++ ?r();?q();]q()".asFormula)),
      chase(1) & prop,
    ) shouldBe Symbol("proved")
  }
  ...
  it should "prove [?p();?(p()->q()); ++ ?!p();](p()->q()) by chase" in withTactics {
    ...
    proveBy(
      Sequent(IndexedSeq(), IndexedSeq("[?p();?(p()->q()); ++ ?!p();](p()->q())".asFormula)),
      chase(1, Nil) & prop,
    ) shouldBe Symbol("proved")
  }
  ...
  it should "prove [?p();?(p()->q()); ++ ?r();?q(); ++ ?!p()&!r();](p()|r()->q()) by chase" in withTactics {
    proveBy(
      Sequent(IndexedSeq(), IndexedSeq("[?p();?(p()->q()); ++ ?r();?q(); ++ ?!p()&!r();](p()|r()->q())".asFormula)),
      chase(1, Nil) & prop,
    ) shouldBe Symbol("proved")
  }
  ...
  it should "prove [?x>0;x:=x+1; ++ ?x=0;x:=1;]x>0 by chase" in withMathematica { _ =>
    proveBy(
      Sequent(IndexedSeq(), IndexedSeq("[?x>0;x:=x+1; ++ ?x=0;x:=1;]x>0".asFormula)),
      chase(1, Nil) & QE,
    ) shouldBe Symbol("proved")
  }
  ...
  private def shouldCMon(ctx: Context[Formula], basic: ProvableSig = basicImpl): Unit = {
    ...
    val done = CMon(ctx)(basic)
      ...
      IndexedSeq(ctx(basic.conclusion.ante.head)),
      IndexedSeq(ctx(basic.conclusion.succ.head)),
    )
  }
  ...
  it should "prove y<2 & x>5 |- y<2 & x>2 from x>5 |- x>2" in withMathematica { _ =>
    shouldCMon(Context("y<2 & ⎵".asFormula))
  }
  it should "prove x>5 & y<2 |- x>2 & y<2 from x>5 |- x>2" in withMathematica { _ =>
    shouldCMon(Context("⎵ & y<2".asFormula))
  }
  it should "prove x<99 -> x>5 |- x<99 -> x>2 from x>5 |- x>2" in withMathematica { _ =>
    shouldCMon(Context("x<99 -> ⎵".asFormula))
  }
  it should "prove x<99 | x>5 |- x<99 | x>2 from x>5 |- x>2" in withMathematica { _ =>
    shouldCMon(Context("x<99 | ⎵".asFormula))
  }
  it should "prove in monotone context x<99 | _ & y<2" in withMathematica { _ =>
    shouldCMon(Context("x<99 | ⎵ & y<2".asFormula))
  }
  it should "prove in monotone context (x<99 | _) & y<2" in withMathematica { _ =>
    shouldCMon(Context("(x<99 | ⎵) & y<2".asFormula))
  }
  it should "prove in monotone context x<7 -> (x<99 | _) & y<2" in withMathematica { _ =>
  ...
  it should "prove in monotone context x<y -> x<7 -> (x<99 | x<10 -> (_ & z=2 | x=5 & y=3)) & y<2" in withMathematica {
    _ => shouldCMon(Context("x<y -> x<7 -> (x<99 | x<10 -> (⎵ & z=2 | x=5 & y=3)) & y<2".asFormula))
  }
  it should "prove in monotone context \\forall y _" in withMathematica { _ =>
    shouldCMon(Context("\\forall y ⎵".asFormula))
  }
  it should "prove in monotone context \\forall x _" in withMathematica { _ =>
    shouldCMon(Context("\\forall x ⎵".asFormula))
  }
  it should "prove in monotone context \\exists y _" in withMathematica { _ =>
    shouldCMon(Context("\\exists y ⎵".asFormula))
  }
  it should "prove in monotone context \\exists x _" in withMathematica { _ =>
    shouldCMon(Context("\\exists x ⎵".asFormula))
  }
  it should "prove in monotone context !!\\exists x _" in withMathematica { _ =>
    shouldCMon(Context("!!\\exists x ⎵".asFormula))
  }
  it should "prove in monotone context ![a:=2;]!\\exists x _" in withMathematica { _ =>
    shouldCMon(Context("![a:=2;]!\\exists x ⎵".asFormula))
  }
  it should "prove in monotone context \\forall y (_ | x<y)" in withMathematica { _ =>
    shouldCMon(Context("\\forall y (⎵ | x<y)".asFormula))
  }
  it should "prove in monotone context \\forall x (_ | x<y)" in withMathematica { _ =>
    shouldCMon(Context("\\forall x (⎵ | x<y)".asFormula))
  }
  it should "prove in monotone context \\exists y (_ | x<y)" in withMathematica { _ =>
    shouldCMon(Context("\\exists y (⎵ | x<y)".asFormula))
  }
  it should "prove in monotone context \\exists x (_ | x<y)" in withMathematica { _ =>
    shouldCMon(Context("\\exists x (⎵ | x<y)".asFormula))
  }
  it should "prove in antimonotone context _ -> y<2" in withMathematica { _ =>
    shouldCMonA(Context("⎵ -> y<2".asFormula))
  }
  it should "prove in antimonotone context _ -> y<2 & x<10" in withMathematica { _ =>
    shouldCMonA(Context("⎵ -> y<2 & x<10".asFormula))
  }
  it should "prove in antimonotone context (_ -> y<2) & x<10" in withMathematica { _ =>
    shouldCMonA(Context("(⎵ -> y<2) & x<10".asFormula))
  }
  it should "prove in antimonotone context (_ -> y<2) & x<10 | x=7" in withMathematica { _ =>
    shouldCMonA(Context("(⎵ -> y<2) & x<10 | x=7".asFormula))
  }
  it should "prove in antimonotone context (<x:=5;>_ -> y<2) & x<10 | x=7" in withMathematica { _ =>
    shouldCMonA(Context("(<x:=5;>⎵ -> y<2) & x<10 | x=7".asFormula))
  }
  it should "prove in antimonotone context (a=3|<x:=5;>_ -> y<2) & x<10 | x=7" in withMathematica { _ =>
    shouldCMonA(Context("(a=3|<x:=5;>⎵ -> y<2) & x<10 | x=7".asFormula))
  }
  it should "prove in antimonotone context (<x:=5;>_&a=3 -> y<2) & x<10 | x=7" in withMathematica { _ =>
    shouldCMonA(Context("(<x:=5;>⎵&a=3 -> y<2) & x<10 | x=7".asFormula))
  }
  it should "prove in antiantimonotone context ((_ -> y<2) -> z=0) & x<10 | x=7" in withMathematica { _ =>
  ...
  private def shouldReduceTo(
      input: Formula,
      pos: Int,
      inExpr: PosInExpr,
      result: Formula,
      fact: ProvableSig = basicEq,
  ): Unit = proveBy(input, CEat(fact)(pos, inExpr.pos)).subgoals should contain only
  ...
  it should "reduce x<5 & \\forall x x^2<4 | x>=2 to x<5 & \\forall x (-2<x&x<2) | x>=2" in withMathematica { _ =>
    shouldReduceTo(
      ...
      1,
      PosInExpr(0 :: 1 :: 0 :: Nil),
      "x<5 & \\forall x (-2<x&x<2) | x>=2".asFormula,
      basicEquiv,
    )
  }
  ...
  it should "reduce x<5 & x^2<4 -> [{x' = 5*x & x^2<4}](x^2<4 & x>=1) to x<5 & (-2<x&x<2) -> [{x' = 5*x & -2<x&x<2}]((-2<x&x<2) & x>=1)" in withMathematica {
    _ =>
      ...
      shouldReduceTo(
        ...
        1,
        PosInExpr(),
        "x<5 & (-2<x&x<2) -> [{x' = 5*x & -2<x&x<2}]((-2<x&x<2) & x>=1)".asFormula,
        basicEquiv,
        C,
      )
  }
  ...
  it should "use DX to forward <{x'=2}>x=y -> bla() to (true&x=y) -> bla()" in withTactics {
    useFor(Ax.dDX)(SuccPosition(1, 0 :: Nil))(ProvableSig.startPlainProof(
      Sequent(IndexedSeq(), IndexedSeq("<{x'=2}>x=y -> bla()".asFormula))
  ...
  it should "use <*> approx to forward <x:=x+1;>x=y to <{x:=x+1;}*>x=y" in withTactics {
    useFor(Ax.loopApproxd, PosInExpr(0 :: Nil))(SuccPosition(1, Nil))(ProvableSig.startPlainProof(
      Sequent(IndexedSeq(), IndexedSeq("<x:=x+1;>x=y".asFormula))
  ...
  it should "use <*> approx to forward <{x:=x+1;}*>x=y -> bla() to <x:=x+1;>x=y -> bla()" in withTactics {
    useFor(Ax.loopApproxd)(SuccPosition(1, 0 :: Nil))(ProvableSig.startPlainProof(
      Sequent(IndexedSeq(), IndexedSeq("<{x:=x+1;}*>x=y -> bla()".asFormula))
  ...
  it should "use <*> approx to forward bla() <-> <{x:=x+1;}*>x=y to <x:=x+1;>x=y -> bla()" in withTactics {
    useFor(Ax.loopApproxd)(SuccPosition(1, 1 :: Nil))(ProvableSig.startPlainProof(
      Sequent(IndexedSeq(), IndexedSeq("bla() <-> <{x:=x+1;}*>x=y".asFormula))
    )).conclusion shouldBe Sequent(IndexedSeq(), IndexedSeq("<x:=x+1;>x=y -> bla()".asFormula))
  }

  it should "use DX to forward <x:=1;>(true&x=y) to <x:=1;><{x'=2}>x=y" in withTactics {
    useFor(Ax.dDX, PosInExpr(0 :: Nil))(SuccPosition(1, 1 :: Nil))(ProvableSig.startPlainProof(
      Sequent(IndexedSeq(), IndexedSeq("<x:=1;>(true&x=y)".asFormula))
  ...
  it should "use DX to forward <x:=1;><{x'=2}>x=y -> bla() to <x:=1;>(true&x=y) -> bla()" in withTactics {
    useFor(Ax.dDX)(SuccPosition(1, 0 :: 1 :: Nil))(ProvableSig.startPlainProof(
      Sequent(IndexedSeq(), IndexedSeq("<x:=1;><{x'=2}>x=y -> bla()".asFormula))
  ...
  it should "use <*> approx to forward <x:=1;>x=1 to <{x:=1;}*>x=1" in withTactics {
    useFor(Ax.loopApproxd, PosInExpr(0 :: Nil))(SuccPosition(1, Nil))(ProvableSig.startPlainProof(
      Sequent(IndexedSeq(), IndexedSeq("<x:=1;>x=1".asFormula))
  ...
  it should "use <*> approx to forward <x:=1;><x:=x+1;>x=y to <x:=1;><{x:=x+1;}*>x=y" in withTactics {
    useFor(Ax.loopApproxd, PosInExpr(0 :: Nil))(SuccPosition(1, 1 :: Nil))(ProvableSig.startPlainProof(
      Sequent(IndexedSeq(), IndexedSeq("<x:=1;><x:=x+1;>x=y".asFormula))
  ...
  it should "use <*> approx to forward <x:=1;><{x:=x+1;}*>x=y -> bla() to <x:=1;><x:=x+1;>x=y -> bla()" in withTactics {
    useFor(Ax.loopApproxd, PosInExpr(1 :: Nil))(SuccPosition(1, 0 :: 1 :: Nil))(ProvableSig.startPlainProof(
      Sequent(IndexedSeq(), IndexedSeq("<x:=1;><{x:=x+1;}*>x=y -> bla()".asFormula))
  ...
  it should "use <*> approx to forward bla() -> <x:=1;><x:=x+1;>x=y to bla() -> <x:=1;><{x:=x+1;}*>x=y" in withTactics {
    useFor(Ax.loopApproxd, PosInExpr(0 :: Nil))(SuccPosition(1, 1 :: 1 :: Nil))(ProvableSig.startPlainProof(
      Sequent(IndexedSeq(), IndexedSeq("bla() -> <x:=1;><x:=x+1;>x=y".asFormula))
  ...
  it should "use <*> approx to forward bla() -> (<x:=1;><{x:=x+1;}*>x=y -> foo()) to bla() -> (<x:=1;><x:=x+1;>x=y -> foo())" in withTactics {
    useFor(Ax.loopApproxd, PosInExpr(1 :: Nil))(SuccPosition(1, 1 :: 0 :: 1 :: Nil))(ProvableSig.startPlainProof(
      Sequent(IndexedSeq(), IndexedSeq("bla() -> (<x:=1;><{x:=x+1;}*>x=y -> foo())".asFormula))
  ...
  it should "use <*> approx to forward (<x:=1;><x:=x+1;>x=y -> bla()) -> foo() to (<x:=1;><{x:=x+1;}*>x=y -> bla()) -> foo()" in withTactics {
    useFor(Ax.loopApproxd, PosInExpr(0 :: Nil))(SuccPosition(1, 0 :: 0 :: 1 :: Nil))(ProvableSig.startPlainProof(
      Sequent(IndexedSeq(), IndexedSeq("(<x:=1;><x:=x+1;>x=y -> bla()) -> foo()".asFormula))
    )).conclusion shouldBe Sequent(IndexedSeq(), IndexedSeq("(<x:=1;><{x:=x+1;}*>x=y -> bla()) -> foo()".asFormula))
  }

  it should "use <*> approx to forward <x:=1;><{x:=x+1;}*>x=y <-> bla() to <x:=1;><x:=x+1;>x=y -> bla()" in withTactics {
    useFor(Ax.loopApproxd)(SuccPosition(1, 0 :: 1 :: Nil))(ProvableSig.startPlainProof(
      Sequent(IndexedSeq(), IndexedSeq("<x:=1;><{x:=x+1;}*>x=y <-> bla()".asFormula))
    )).conclusion shouldBe Sequent(IndexedSeq(), IndexedSeq("<x:=1;><x:=x+1;>x=y -> bla()".asFormula))
  }

  it should "use ^' derive power to forward (x^2)'=0 to 2*x^(2-1)*(x)'=0" in withMathematica { _ =>
    useFor(Ax.Dpower)(SuccPosition(1, 0 :: Nil))(ProvableSig.startPlainProof(
      Sequent(IndexedSeq(), IndexedSeq("(x^2)'=0".asFormula))
