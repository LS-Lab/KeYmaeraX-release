...
class FreeVariablesTests extends FlatSpec with Matchers with BeforeAndAfterEach with BeforeAndAfterAll {
  ...
  "Free variables of -x" should "be {x}" in { StaticSemantics("-x".asPlainTerm) should be(SetLattice(Set(V("x")))) }
  ...
  "Free variables of x+1" should "be {x}" in { StaticSemantics("x+1".asPlainTerm) should be(SetLattice(Set(V("x")))) }
  ...
  "Free variables of x'*y+x*z'" should "be {x,y}" in {
    ...
    StaticSemantics("x'*y+x*z'".asPlainTerm) should be(SetLattice(
      Set(V("x"), V("y"), DifferentialSymbol(Variable("x", None, Real)), DifferentialSymbol(Variable("z", None, Real)))
    ))
    StaticSemantics(Plus(Times(Differential(V("x")), V("y")), Times(V("x"), Differential(V("z"))))) shouldBe
      SetLattice(Set(
        ...
        V("y"),
        V("z"),
        DifferentialSymbol(Variable("x", None, Real)),
  ...
  "Free variables of true" should "be {}" in { StaticSemantics("true".asPlainFormula).fv should be(SetLattice.bottom) }
  ...
  "Free variables of Exists x. x=t & y=x | x=z" should "be {t,x,y,z}" in {
    StaticSemantics("\\exists x x=t & y=x | x=z ".asPlainFormula).fv should
      be(SetLattice(Set(V("t"), V("x"), V("y"), V("z"))))
  }
  ...
  "Free variables of Forall x. x=t & y=x | x=z" should "be {t,x,y,z}" in {
    StaticSemantics("\\forall x x=t & y=x | x=z ".asPlainFormula).fv should
      be(SetLattice(Set(V("t"), V("x"), V("y"), V("z"))))
  }
  ...
  "Free variables of {x:=1 ++ x:=x+1 ++ z:=x};{x:=1 ++ x:=x+1 ++ z:=x};" should "be {x}" in {
    StaticSemantics("{x:=1; ++ x:=x+1; ++ z:=x;}{x:=1; ++ x:=x+1; ++ z:=x;}".asPlainProgram).fv should
      be(SetLattice(Set(V("x"))))
  }
  ...
  it should "work on sequents" in {
    TacticHelper.freshIndexInSequent(
      "x",
      Sequent(scala.collection.immutable.IndexedSeq.empty, scala.collection.immutable.IndexedSeq.empty),
  ...
  it should "return names on sequents" in {
    TacticHelper.freshNamedSymbol(
      "x".asVariable,
      Sequent(scala.collection.immutable.IndexedSeq.empty, scala.collection.immutable.IndexedSeq.empty),
    ) shouldBe "x".asVariable
    ...
    TacticHelper.freshNamedSymbol("x".asPlainFunction, "==> x>x_0(y), z>x".asPlainSequent) shouldBe
      "x_1".asPlainFunction
    TacticHelper.freshNamedSymbol("x".asVariable, "x_0>4 ==> x>5, x<7".asPlainSequent) shouldBe "x_1".asVariable
    TacticHelper.freshNamedSymbol("x_55".asVariable, "x_4=7, x_5=3 ==> x>x_2, x_8<5".asPlainSequent) shouldBe
      "x_9".asVariable
    TacticHelper.freshNamedSymbol(".".asPlainNamedSymbol, "==> ._2 > 7".asPlainSequent) shouldBe
      "._3".asPlainNamedSymbol
    ...
    the[IllegalArgumentException] thrownBy
      TacticHelper.freshNamedSymbol(UnitFunctional("x", AnyArg, Real), "==> x(||) > 0".asPlainSequent) should
      have message
      "Cannot obtain fresh symbol, since class edu.cmu.cs.ls.keymaerax.core.UnitFunctional does not have index"
    the[IllegalArgumentException] thrownBy
      TacticHelper.freshNamedSymbol(UnitPredicational("x", AnyArg), "==> x(||)".asPlainSequent) should have message
      "Cannot obtain fresh symbol, since class edu.cmu.cs.ls.keymaerax.core.UnitPredicational does not have index"
    the[IllegalArgumentException] thrownBy
      TacticHelper.freshNamedSymbol(ProgramConst("x", AnyArg), "==> [x;]y>0".asPlainSequent) should have message
      "Cannot obtain fresh symbol, since class edu.cmu.cs.ls.keymaerax.core.ProgramConst does not have index"
  }
}
