...
object ProofEquality extends Equality[Statement] {
  override def areEqual(a: Statement, b: Any): Boolean = {
    ...
    (a, b) match {
      ...
      case (Block(ss), Block(ssR)) if ss.length == ssR.length => ss.zip(ssR).forall({ case (l, r) => areEqual(l, r) })
      case (Switch(scrutinee, pats), Switch(scrutineeR, patsR))
          if scrutinee == scrutineeR && pats.length == patsR.length =>
      ...
      case (For(metX, met0, metIncr, conv, guard, body, ge), For(metXR, met0R, metIncrR, convR, guardR, bodyR, geR)) =>
        metX == metXR && met0 == met0R && metIncr == metIncrR && conv == convR && guard == guardR &&
        areEqual(body, bodyR) && ge == geR
