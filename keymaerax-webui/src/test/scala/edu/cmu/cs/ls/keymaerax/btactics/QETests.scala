...
class QETests extends TacticTestBase {
  ...
  "Full QE" should "prove x>0 & y>0 |- x*y>0" in withMathematica { qeTool =>
    proveBy(
      Sequent(IndexedSeq("x>0".asFormula, "y>0".asFormula), IndexedSeq("x*y>0".asFormula)),
      ToolTactics.fullQE(qeTool),
  ...
  it should "abbreviate/not abbreviate interpreted symbols known to Mathematica according to the configuration" in
    withMathematica { _ =>
  ...
  it should "abort on test timeout" in {
    the[TestFailedDueToTimeoutException] thrownBy withMathematica(
      { _ =>
        val s =
          "A()>=0, B()>0, V()>=0, ep()>0, -B()<=a, a<=A(), r!=0, w_0*r=v_0, abs(x_0-xo_0)>v_0^2/(2*B())+V()*v_0/B()+(A()/B()+1)*(A()/2*t^2+t*(v_0+V())), t_0=0, v_0>=0, dx_0^2+dy_0^2=1, r_0!=0, -t*V()<=xo-xo_0, xo-xo_0<=t*V(), -t*(v-a/2*t)<=x-x_0, x-x_0<=t*(v-a/2*t), v=v_0+a*t, dx^2+dy^2=1, t>=0, t<=ep(), v>=0 ==> v=0, abs(x-xo)>v^2/(2*B())+V()*(v/B())"
            .asSequent
        proveBy(s, QE) shouldBe Symbol("proved")
      },
      2,
    ) should have message "The code passed to failAfter did not complete within 2 seconds."
  }

  it should "not translate quantified predicates verbatim" in withMathematica { _ =>
    the[TacticInapplicableFailure] thrownBy proveBy("\\forall x p(x) ==> p(y)".asSequent, QE) should have message
      "The sequent mentions uninterpreted functions or predicates; attempted to prove without but failed. Please apply further manual steps to expand definitions and/or instantiate arguments."
  }

  it should "not translate quantified functions verbatim" in withMathematica { _ =>
    the[TacticInapplicableFailure] thrownBy proveBy("\\forall x f(x)=0 ==> f(y)=0".asSequent, QE) should have message
      "The sequent mentions uninterpreted functions or predicates; attempted to prove without but failed. Please apply further manual steps to expand definitions and/or instantiate arguments."
  }

  it should "be able to run QE's internal steps" in withMathematica { _ =>
    proveBy(
      "x>z, y=0, z>y ==> x>=0".asSequent,
      BelleParser("applyEqualities; toSingleFormula; universalClosure(\"x::z::nil\", 1); rcf"),
    ) shouldBe Symbol("proved")
    proveBy(
      "x>z, y=0, z>y ==> x>=0".asSequent,
      BelleParser("applyEqualities; toSingleFormula; universalClosure(\"nil\", 1); rcf"),
    ) shouldBe Symbol("proved")
  }
  ...
  it should "prove the STTT example 10 acceleration subgoal" in withMathematica { tool =>
    val s =
      "A()>0, B()>=b(), b()>0, lw()>0, v_0>=0, abs(y_0-ly())+v_0^2/(2*b())+(A()/b()+1)*(A()/2*c^2+c*v_0) < lw(), -B()<=a, a<=A(), r!=0, w_0*r=v_0, v=v_0+a*c, -c*(v-a/2*c)<=y-y_0, y-y_0<=c*(v-a/2*c), c>=0, v>=0 ==> abs(y-ly())+v^2/(2*b()) < lw()"
        .asSequent
  ...
  it should "prove the Robix static safety acceleration subgoal" in
    withTemporaryConfig(Map(Configuration.Keys.QE_ALLOW_INTERPRETED_FNS -> "true")) {
      withMathematica { tool =>
        val s =
          "A>=0, B>0, ep>0, v_0>=0, -B<=a, a<=A, abs(x_0-xo)>v_0^2/(2*B)+(A/B+1)*(A/2*ep^2+ep*v_0), v=v_0+a*t, -t*(v-a/2*t)<=x-x_0, x-x_0<=t*(v-a/2*t), t>=0, t<=ep, v>=0 ==> abs(x-xo)>v^2/(2*B)"
            .asSequent
        withTemporaryConfig(Map(Configuration.Keys.MATHEMATICA_PARALLEL_QE -> "true")) {
          proveBy(
            s,
            ArithmeticSpeculativeSimplification.autoMonotonicityTransform & ArithmeticSimplification.smartHide &
              ToolTactics.fullQE(tool),
          ) shouldBe Symbol("proved")
        }
      }
  ...
  it should "prove the Robix static safety acceleration subgoal when expanding abs" taggedAs SlowTest in
    withTemporaryConfig(Map(Configuration.Keys.QE_ALLOW_INTERPRETED_FNS -> "false")) {
      withMathematica { tool =>
        val s =
          "A>=0, B>0, ep>0, v_0>=0, -B<=a, a<=A, abs(x_0-xo)>v_0^2/(2*B)+(A/B+1)*(A/2*ep^2+ep*v_0), v=v_0+a*t, -t*(v-a/2*t)<=x-x_0, x-x_0<=t*(v-a/2*t), t>=0, t<=ep, v>=0 ==> abs(x-xo)>v^2/(2*B)"
  ...
  it should "FEATURE_REQUEST: prove the Robix passive safety acceleration subgoal" taggedAs TodoTest ignore
    withMathematica { tool =>
      val s =
        "A>=0, B>0, V()>=0, ep>0, v_0>=0, -B<=a, a<=A, abs(x_0-xo_0)>v_0^2/(2*B)+V()*v_0/B+(A/B+1)*(A/2*ep^2+ep*(v_0+V())), abs(x_0-xo_0)>v_0^2/(2*B)+V()*(v_0/B), -t*V()<=xo-xo_0, xo-xo_0<=t*V(), v=v_0+a*t, -t*(v-a/2*t)<=x-x_0, x-x_0<=t*(v-a/2*t), t>=0, t<=ep, v>=0 ==> abs(x-xo)>v^2/(2*B)+V()*(v/B)"
          .asSequent
      withTemporaryConfig(Map(Configuration.Keys.MATHEMATICA_PARALLEL_QE -> "true")) {
        proveBy(
          s,
          ArithmeticSpeculativeSimplification.autoMonotonicityTransform & ArithmeticSimplification.smartHide &
            DebuggingTactics.print("Hidden") & ToolTactics.fullQE(tool),
        ) shouldBe Symbol("proved")
      }
    }
  ...
  it should "give a legible error message on failure when temporary hiding symbols are present" in withQE { tool =>
    the[TacticInapplicableFailure] thrownBy
      proveBy("x=0 ==> x>=0".asSequent, Using(List("x>=0".asFormula), ToolTactics.fullQE(tool))) should have message
  ...
  it should "abbreviate uninterpreted functions with arity>0 since not supported by Reduce/Resolve" in withQE { tool =>
    proveBy(
      "v>=0, t>=0, t<=ep, ep>=0, x>=v*ep+f(v) ==> x>=v*t+f(v)".asSequent,
      ToolTactics.fullQE(Declaration(Map.empty), List.empty)(tool),
    ) shouldBe Symbol("proved")
  }

  it should "abbreviate interpreted functions when not converting" in withTemporaryConfig(
    Map(Configuration.Keys.MATHEMATICA_QE_METHOD -> "Reduce", Configuration.Keys.QE_ALLOW_INTERPRETED_FNS -> "false")
  ) {
    withMathematica { tool =>
  ...
  it should
    "try to abbreviate interpreted functions even when converting to Mathematica is enabled and only proceed expanded on counterexample" in
    withTemporaryConfig(
      Map(Configuration.Keys.MATHEMATICA_QE_METHOD -> "Reduce", Configuration.Keys.QE_ALLOW_INTERPRETED_FNS -> "true")
    ) {
      withMathematica { tool =>
        // @note not solvable by Reduce without abbreviating
        val s =
  ...
  it should "fail on tool mismatch" in withMathematica { _ =>
    the[BelleThrowable] thrownBy
      proveBy("0=0".asFormula, TactixLibrary.QE(Declaration(Map.empty), Nil, Some("Z3"))) should have message
      "QE requires Z3, but got None"
  }

  it should "switch between tools" in withTactics {
    withDatabase { db =>
      val provider = MultiToolProvider(
        new Z3ToolProvider :: MathematicaToolProvider(ToolConfiguration.config(ToolName.Mathematica)) :: Nil
      )
      ...
      interpreter(
        BelleParser("implyR(1); andR(1); <(QE(\"Z3\"), QE(\"Mathematica\"))"),
  ...
  it should "use the default tool" in withTactics {
    withDatabase { db =>
      ...
      interpreter(
        BelleParser("implyR(1); andR(1); <(QE, QE)"),
        BelleProvable.plain(ProvableSig.startPlainProof(ArchiveParser.parseAsFormula(modelContent))),
      )
      db.extractTactic(proofId) shouldBe BelleParser(
  ...
  it should "use the right tool" in withQE { tool: Tool =>
    withTactics {
      withDatabase { db =>
        ...
        interpreter(
          QEX(Some(tool.name), Some(Number(7))),
          BelleProvable.plain(ProvableSig.startPlainProof(ArchiveParser.parseAsFormula(modelContent))),
        )
  ...
  it should "complain about the wrong tool" in withZ3 { _ =>
    the[BelleThrowable] thrownBy proveBy("x>1 -> x>0".asFormula, QEX(Some("Mathematica"), Some(Number(7)))) should
      have message "QE requires Mathematica, but got None"
  }
  ...
  "CEX in QE" should "not fail QE when FindInstance fails" in withMathematica { tool =>
    withTemporaryConfig(Map(Configuration.Keys.MATHEMATICA_QE_METHOD -> "Resolve")) {
      val fml =
        """(\forall w2 \exists w3 \forall w4 \exists w5 \forall w6 \exists w7 \forall w8 \exists w9 \forall w10
  ...
  "Partial QE" should "not fail on |-" in withMathematica { qeTool =>
    proveBy(Sequent(IndexedSeq(), IndexedSeq()), ToolTactics.partialQE(qeTool)).subgoals.loneElement shouldBe
      "==> false".asSequent
  }
  ...
  it should "not forget assumptions on x^2>=0 |- y>1" in withMathematica { qeTool =>
    proveBy(Sequent(IndexedSeq("x^2>=0".asFormula), IndexedSeq("y>1".asFormula)), ToolTactics.partialQE(qeTool))
      .subgoals
      .loneElement shouldBe "x^2>=0 ==> y>1".asSequent
  }
