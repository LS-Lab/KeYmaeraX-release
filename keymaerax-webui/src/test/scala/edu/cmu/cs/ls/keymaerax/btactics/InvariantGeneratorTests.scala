...
/** Tests invariant generators. [[edu.cmu.cs.ls.keymaerax.btactics.InvariantGenerator]]. */
@UsualTest
class InvariantGeneratorTests extends TacticTestBase with PrivateMethodTester {
  ...
  "Loop invariants" should "be generated from pre and postconditions" in withTactics {
    InvariantGenerator
      .loopInvariantGenerator("x>=1 ==> [{x:=x+1;}*][x:=x+1;]x>=2".asSequent, SuccPos(0), Declaration(Map.empty))
  ...
  it should "be generated from unexpanded pre and postconditions" in withTactics {
    ...
    InvariantGenerator
      .loopInvariantGenerator("init(x) ==> [{x:=x+1;}*][x:=x+1;]post(x)".asSequent, SuccPos(0), Declaration(Map.empty))
      .toList should contain theSameElementsInOrderAs
  ...
  it should "be generated from unexpanded pre and postconditions (with CEX tool for filtering)" in withMathematica {
    _ =>
      ...
      InvariantGenerator
        .loopInvariantGenerator("init(x) ==> [{x:=x+1;}*][x:=x+1;]post(x)".asSequent, SuccPos(0), defs)
        .toList should contain theSameElementsInOrderAs
  ...
  it should "not include equivalent postcondition conjuncts if it has a counterexample tool" in withMathematica { _ =>
    ...
    InvariantGenerator
      .loopInvariantGenerator(
        "x>=2 & x>=3 ==> [{x:=x+1;}*](x>=1 & x>=2 & y>=3)".asSequent,
        SuccPos(0),
  ...
  it should "not fail on missing counterexample tool" in withTactics {
    InvariantGenerator
      .loopInvariantGenerator("x>=2 & x>=3 ==> [{x:=x+1;}*](x>=1 & x>=2)".asSequent, SuccPos(0), Declaration(Map.empty))
      .toList should contain theSameElementsInOrderAs
      (("x>=1".asFormula, None) :: ("x>=2".asFormula, None) :: ("x>=3".asFormula, None) ::
  ...
  it should "find an invariant for the runaround robot" in withMathematica { _ =>
    ...
    InvariantGenerator.loopInvariantGenerator(s, SuccPos(0), Declaration(Map.empty)).toList should
  ...
  "Differential counterexample generator" should "analyze a simple example" in withMathematica { t =>
    t.refuteODE(
      ODESystem("{x'=v}".asDifferentialProgram),
      List("x>=0".asFormula, "v>=0".asFormula),
      "x>=0".asFormula,
    ) shouldBe None
  }

  it should "find a counterexample in a simple example" in withMathematica { t =>
    t.refuteODE(ODESystem("{x'=v}".asDifferentialProgram), List("x>=0".asFormula), "x>=0".asFormula) shouldBe
      Some(Map("v".asNamedSymbol -> Number(-1), "x".asNamedSymbol -> Number(0)))
  }
  ...
  "Differential invariant generator" should "use Pegasus lazily" in withTactics {
    ...
    val gen = InvariantGenerator
      .differentialInvariantGenerator("x>0 ==> [{x'=x^2&true}]x>=0".asSequent, SuccPos(0), Declaration(Map.empty))
  ...
  it should "not fail if Mathematica is unavailable" in withTactics {
    val gen = InvariantGenerator
      .pegasusInvariants("x>0 ==> [{x'=x^2&true}]x>=0".asSequent, SuccPos(0), Declaration(Map.empty))
  ...
  "Pegasus" should "FEATURE_REQUEST: generate invariants in correct order" taggedAs TodoTest in withMathematica { _ =>
    ...
    InvariantGenerator.pegasusInvariants(s, SuccPos(0), Declaration(Map.empty)).toList should
  ...
  "Configurable generator" should "return annotated conditional invariants" in withQE { tool =>
    ...
    TactixLibrary.invGenerator("==> [{y'=2*y&true}]y>0".asSequent, SuccPosition(1), Declaration(Map.empty)) should
      contain theSameElementsInOrderAs expectedInvs("y>=old(y)")
    TactixLibrary.invGenerator("==> [{y'=(-2)*y&true}]y>0".asSequent, SuccPosition(1), Declaration(Map.empty)) should
      contain theSameElementsInOrderAs expectedInvs("y<=old(y)")
  }

  it should "FEATURE_REQUEST: return annotated conditional invariants with negated numbers" taggedAs TodoTest in
    withQE { tool =>
      // parse formula with invariant annotations to populate invariant generator
      "y>0 ==> [{x:=2; ++ x:=-2;}{{y'=x*y}@invariant((y'=2*y -> y>=old(y)), (y'=-2*y -> y<=old(y)))}]y>0".asSequent
      def expectedInvs(inv: String) =
  ...
  "Pegasus" should "return trivial invariant postcondition result if sanity timeout > 0" in withMathematica { _ =>
    ...
    withTemporaryConfig(Map(Configuration.Keys.Pegasus.SANITY_TIMEOUT -> "0")) {
      InvariantGenerator.pegasusInvariants(seq, SuccPosition(1), Declaration(Map.empty)).toList should
...
@ExtremeTest
class NonlinearExamplesTests
    extends Suites(new NonlinearExamplesTester(
      "Nonlinear_",
...
@ExtremeTest
class NonlinearExamplesTester(
    val benchmarkName: String,
    val url: String,
    val timeout: Int,
    val genCheck: Boolean,
    val keepUnverifiedCandidates: Boolean,
  ...
  it should "output plots" ignore withMathematica { _ =>
    entries.foreach(e => {
      ...
      println(
        edu.cmu.cs.ls.keymaerax.tools.qe.MathematicaOpSpec.string(e.name).toString + "\n" +
  ...
  it should "classification of problems" in withMathematica { tool =>
    setTimeouts(tool) {
      ...
      val categories: List[(String, List[String])] =
        ("Boundedness" -> ("Initial Set" :: "Unsafe Set" :: "Evolution Constraint" :: Nil)) ::
      ...
      classifications.foreach({ case (name, (dimension, clazz, details)) =>
        val detailsString =
          categories.map({ case (key, subKeys) => subKeys.map(details(key)(_)).mkString(",") }).mkString(",")
  ...
  it should "generate invariants with default DiffSat strategy" in withMathematicaMatlab { tool =>
    setTimeouts(tool) {
      withTemporaryConfig(Map(
        ...
        Configuration.Keys.Pegasus.LinearFirstIntegrals.TIMEOUT ->
          "10", /* half of FirstIntegrals (work on disjoint classes) */
        Configuration.Keys.Pegasus.LinearGenericMethod.TIMEOUT ->
          "10", /* half of FirstIntegrals (work on disjoint classes) */
        ...
        entries.foreach(e => {
          ...
          try {
            val result =
              robustRunInvGen(e.name, e.model, matlab = true, stripProofHints = false, keepUnverifiedCandidates)
  ...
  it should "generate invariants with DiffSat strategy and restricted maximum degrees" ignore withMathematicaMatlab {
    tool =>
      setTimeouts(tool) {
        withTemporaryConfig(Map(
          Configuration.Keys.Pegasus.SANITY_TIMEOUT -> "0",
  ...
  it should "generate invariants with default DiffSat strategy, not using depedencies" in withMathematicaMatlab {
    tool =>
      setTimeouts(tool) {
  ...
  it should "generate invariants with default DiffSat strategy, no cut minimize" in withMathematicaMatlab { tool =>
    setTimeouts(tool) {
      // No Auto-Reduction
      withTemporaryConfig(Map(
        Configuration.Keys.Pegasus.SANITY_TIMEOUT -> "0",
  ...
  it should "generate invariants with DiffSat strategy without heuristics" in withMathematicaMatlab { tool =>
    setTimeouts(tool) {
      // No Heuristic Search
      withTemporaryConfig(Map(
        Configuration.Keys.Pegasus.SANITY_TIMEOUT -> "0",
        ...
        Configuration.Keys.Pegasus.LinearFirstIntegrals.TIMEOUT ->
          "15", /* half of FirstIntegrals (work on disjoint classes) */
        Configuration.Keys.Pegasus.LinearGenericMethod.TIMEOUT ->
          "15", /* half of FirstIntegrals (work on disjoint classes) */
  ...
  it should "generate invariants with default DiffSat strategy, and prove without proof hints" in
    withMathematicaMatlab { tool =>
      setTimeouts(tool) {
        // No Proof Hints
        withTemporaryConfig(Map(
          Configuration.Keys.Pegasus.SANITY_TIMEOUT -> "0",
          ...
          entries.foreach(e => {
            ...
            try {
              val result =
                robustRunInvGen(e.name, e.model, matlab = true, stripProofHints = true, keepUnverifiedCandidates)
              writer.write(result.toCsv(infoPrinter) + "\r\n")
            } finally { writer.close() }
          })
        }
  ...
  it should "generate invariants with default DiffSat strategy and strict method timeouts" in withMathematicaMatlab {
    tool =>
      setTimeouts(tool) {
  ...
  it should "generate invariants with Barrier only" in withMathematicaMatlab { tool =>
    setTimeouts(tool) {
      withTemporaryConfig(Map(
        Configuration.Keys.Pegasus.SANITY_TIMEOUT -> "0",
        Configuration.Keys.Pegasus.PreservedStateHeuristic.TIMEOUT -> "0", /* disable */
  ...
  it should "generate invariants with Darboux only (default max. degree)" in withMathematica { tool =>
    setTimeouts(tool) {
      withTemporaryConfig(Map(
        Configuration.Keys.Pegasus.SANITY_TIMEOUT -> "0",
        Configuration.Keys.Pegasus.PreservedStateHeuristic.TIMEOUT -> "0", /* disable */
        ...
        entries.foreach(e => {
          ...
          try {
            val result =
              robustRunInvGen(e.name, e.model, matlab = false, stripProofHints = false, keepUnverifiedCandidates)
            writer.write(result.toCsv(infoPrinter) + "\r\n")
          } finally { writer.close() }
        })
      }
  ...
  it should "generate invariants with invariant heuristics (summands) only" in withMathematica { tool =>
    setTimeouts(tool) {
      withTemporaryConfig(Map(
        Configuration.Keys.Pegasus.SANITY_TIMEOUT -> "0",
        Configuration.Keys.Pegasus.PreservedStateHeuristic.TIMEOUT -> "0", /* disable */
  ...
  it should "generate invariants with first integrals only" in withMathematica { tool =>
    setTimeouts(tool) {
      withTemporaryConfig(Map(
        Configuration.Keys.Pegasus.SANITY_TIMEOUT -> "0",
        Configuration.Keys.Pegasus.PreservedStateHeuristic.TIMEOUT -> "0", /* disable */
        ...
        Configuration.Keys.Pegasus.LinearFirstIntegrals.TIMEOUT ->
          "60", /* half of FirstIntegrals, work on disjoint classes */
        Configuration.Keys.Pegasus.LinearGenericMethod.TIMEOUT ->
          "60", /* half of FirstIntegrals, work on disjoint classes */
        Configuration.Keys.Pegasus.Darboux.TIMEOUT -> "0", /* disable */
        Configuration.Keys.Pegasus.Darboux.STAGGERED -> "false",
        ...
        entries.foreach(e => {
          ...
          try {
            val result =
              robustRunInvGen(e.name, e.model, matlab = false, stripProofHints = false, keepUnverifiedCandidates)
            writer.write(result.toCsv(infoPrinter) + "\r\n")
          } catch { case ex: Throwable => writer.write(s"${e.name} failed ${ex.getMessage}\r\n") }
          finally { writer.close() }
        })
      }
    }
  ...
  private def robustRunInvGen(
      name: String,
      modelContent: String,
      matlab: Boolean,
      stripProofHints: Boolean,
      keepUnverifiedCandidates: Boolean,
  ): BenchmarkResult = {
    ...
    if (genCheck) {
      ...
      try {
        pegasusGen(name, expandedModel, keepUnverifiedCandidates, defs) match {
          ...
          case None =>
            println(
              "Skipping " + name + " for unknown shape, expected A -> [{x'=f(x)}]p(x), but got " + model.prettyString
            )
