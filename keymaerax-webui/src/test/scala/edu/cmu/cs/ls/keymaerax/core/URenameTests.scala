...
import edu.cmu.cs.ls.keymaerax.btactics.{DerivedAxioms, RandomFormula, TacticTestBase, TactixLibrary}
...
//    val proof = proof1
    ...
    val proof = TactixLibrary.proveBy("(x+y)'=x'+y'".asFormula, derive(1, 0::Nil) & byUS(DerivedAxioms.equalReflexive))
