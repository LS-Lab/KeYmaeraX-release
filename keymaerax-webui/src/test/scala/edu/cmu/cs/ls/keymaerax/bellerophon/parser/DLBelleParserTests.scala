...
class DLBelleParserTests
  ...
  it should "parse integer position locators" in {
    ...
    parse("""hideR(2~="x>=0")""") should
      have(Symbol("locator")(Fixed(SuccPos(1), Some("x>=0".asFormula), exact = false)))
    ...
    parse("""trueAnd(2~="[x:=2;]#(true&x=2)#")""") should have(
      Symbol("locator")(Fixed(SuccPosition.base0(1, PosInExpr(1 :: Nil)), Some("true&x=2".asFormula), exact = false))
    )
    parse("""trueAnd(2.1=="[x:=2;]#(true&x=2)#")""") should have(
      Symbol("locator")(Fixed(SuccPosition.base0(1, PosInExpr(1 :: Nil)), Some("true&x=2".asFormula), exact = true))
    )
  ...
  it should "elaborate to interpreted functions" in {
    val tanh = Function(
      ...
      Real,
      Real,
      Some("<{tanh:=._0;t:=._1;}{{tanh'=-(1-tanh^2),t'=-1}++{tanh'=1-tanh^2,t'=1}}>(tanh=0&t=0)".asFormula),
    )
  ...
  it should "parse hash locators" in {
    ...
    t.locator shouldBe
      Find.FindR(0, Some("[x:=1;](true&x=1)".asFormula), PosInExpr(1 :: Nil), exact = true, defs = BuiltinSymbols.all)
  }
