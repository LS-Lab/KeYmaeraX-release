...
import edu.cmu.cs.ls.keymaerax.btactics.ModelPlex.{INDEXED_POST_VAR, createMonitorSpecificationConjecture}
...
class PartialObservableModelplexTests extends TacticTestBase {
  ...
  private def report(result: Formula, qeFreeResult: Formula, proof: ProvableSig, name: String, start: Long = 0, approxEnd: Long = 0, synthEnd: Long = 0, partialQEEnd: Long = 0): Unit = {
    println("================================")
  ...
  private def deriveMonitor(entry: ParsedArchiveEntry,
                            ...
                            checkWitnessResult: ProvableSig=>Any = (_: ProvableSig) => _): Formula = {
    ...
    val (approx, _) =
      ...
      else (entry.model.exhaustiveSubst(USubst(entry.defs.substs.filter(_.what.isInstanceOf[SystemConst]))).asInstanceOf[Formula], skip)

    val stateVars = StaticSemantics.boundVars(approx).toSet[Variable].filter(_.isInstanceOf[BaseVariable]).map(_.asInstanceOf[BaseVariable])
    ...
    val ModelPlexConjecture(init, modelplexInput, assumptions) = createMonitorSpecificationConjecture(approx,
      stateVars.toList, unobservable, postVarFactory)
    ...
    val synthResult = proveBy(foResult,
      ModelPlex.optimizationOneWithSearch(Some(tool), assumptions, unobservable.keys.toList, Some(ModelPlex.mxSimplify), postVarFactory)(1)
    )
    ...
    val inputs = CodeGenerator.getInputs(prg) -- unobservable.keySet.filter(_.isInstanceOf[BaseVariable]).map(_.asInstanceOf[BaseVariable])
  ...
  it should "derive an approximated model monitor with unobservable parameter" in withMathematica { tool =>
    ...
    deriveMonitor(curvedBot, Some(curvedBot.tactics.head._3), unobservable, tool,
      checkFOResult = (pr: ProvableSig) => pr.subgoals.loneElement shouldBe "==> \\exists Da (\\forall acc (0 < acc&acc<=Da->(xr+w/((-1)*acc)-xo)^2+(yr-v/((-1)*acc)-yo)^2!=(v^2+w^2)/acc^2)&\\exists acc ((0 < acc&acc<=Da)&\\forall v_0 (v_0=v->\\forall w_0 (w_0=w->\\forall xr_0 (xr_0=xr->\\forall yr_0 (yr_0=yr->(xr+w/((-1)*((-1)*acc))-xo)^2+(yr-v/((-1)*((-1)*acc))-yo)^2!=(v^2+w^2)/((-1)*acc)^2&\\exists v \\exists w \\exists xr \\exists yr ((xr+w/((-1)*((-1)*acc))-xo)^2+(yr-v/((-1)*((-1)*acc))-yo)^2!=(v^2+w^2)/((-1)*acc)^2&wpost=w&yrpost_0=yr_0&yrpost=yr&vpost_0=v_0&xrpost_0=xr_0&apost=(-1)&vpost=v&wpost_0=w_0&xrpost=xr&accpost=(-1)*acc))))))|\\forall acc (0 < acc&acc<=Da->(xr+w/acc-xo)^2+(yr-v/acc-yo)^2!=(v^2+w^2)/acc^2)&\\exists acc ((0 < acc&acc<=Da)&\\forall v_0 (v_0=v->\\forall w_0 (w_0=w->\\forall xr_0 (xr_0=xr->\\forall yr_0 (yr_0=yr->(xr+w/(1*(1*acc))-xo)^2+(yr-v/(1*(1*acc))-yo)^2!=(v^2+w^2)/(1*acc)^2&\\exists v \\exists w \\exists xr \\exists yr ((xr+w/(1*(1*acc))-xo)^2+(yr-v/(1*(1*acc))-yo)^2!=(v^2+w^2)/(1*acc)^2&wpost=w&yrpost_0=yr_0&yrpost=yr&vpost_0=v_0&xrpost_0=xr_0&apost=1&vpost=v&wpost_0=w_0&xrpost=xr&accpost=1*acc)))))))".asSequent)
  }
  ...
  it should "derive a model monitor from water tank with flow disturbance for unknown disturbance parameter" in withMathematica { tool =>
    ...
    val unobservable = ListMap("fd".asVariable -> None, Function("D", None, Unit, Real, interpreted=false) -> None)
    val chaseStartPos = List.fill(unobservable.count(_._1.isInstanceOf[Function]))(0) ++ (if (unobservable.values.exists(_.isDefined)) 1::Nil else Nil)
  ...
  it should "derive a model monitor from water tank with load measurement uncertainty" in withMathematica { tool =>
    ...
    val unobservable = ListMap[Variable,Option[Formula]](
      ...
      "c_0".asVariable -> None, "l_0".asVariable -> None
    )
    deriveMonitor(entry, Some(entry.tactics.head._3), unobservable, tool)
  }
  ...
  it should "prove planar flight motion" in withMathematica { _ =>
    ...
    checkArchiveEntries(entries.find(_.name == "ModelPlex/Partial Observability/Planar Flight Motion Safety with Intruder Velocity Uncertainty").toList)
    checkArchiveEntries(entries.find(_.name == "ModelPlex/Partial Observability/Planar Flight Motion Safety with Ownship Angular Velocity Disturbance and Intruder Velocity Uncertainty").toList)

    checkArchiveEntries(entries.find(_.name == "ModelPlex/Partial Observability/Planar Flight Motion Safety 2 Simple").toList)
    checkArchiveEntries(entries.find(_.name == "ModelPlex/Partial Observability/Planar Flight Motion Safety 2 Simple with Ownship Actuator Disturbance").toList)
    checkArchiveEntries(entries.find(_.name == "ModelPlex/Partial Observability/Planar Flight Motion Safety 2 Simple with Position Uncertainty").toList)
    checkArchiveEntries(entries.find(_.name == "ModelPlex/Partial Observability/Planar Flight Motion Safety 2 Simple with Intruder Linear Velocity Uncertainty").toList)

    checkArchiveEntries(entries.find(_.name == "ModelPlex/Partial Observability/Planar Flight Motion Safety 3 Simple").toList)
    checkArchiveEntries(entries.find(_.name == "ModelPlex/Partial Observability/Planar Flight Motion Safety 3 Simple with Ownship Actuator Disturbance").toList)
    checkArchiveEntries(entries.find(_.name == "ModelPlex/Partial Observability/Planar Flight Motion Safety 3 Simple with Intruder Linear Velocity Uncertainty").toList)
  }
  ...
  it should "derive model monitor from approximated planar flight motion simple 2 with ownship actuator disturbance" in withMathematica { tool =>
    ...
    deriveMonitor(entry, Some(entry.tactics.head._3), unobservableVars, tool) shouldBe "(v2+(-dy)*v2*x+dx*v2*y=0&1+wD() < 0|v2!=0&1+(-dy)*x+dx*y=0&1+wD() < 0&v2+v1*v2+(-dx)*v1*v2+(-dy)*v2*x+dx*v2*y+v2*wD()+(-dy)*v2*x*wD()+dx*v2*y*wD() < 0&v2+v1*v2+(-dx)*v1*v2+(-dy)*v2*x+dx*v2*y+(-v2)*wD()+dy*v2*x*wD()+(-dx)*v2*y*wD() < 0|v2+(-dy)*v2*x+dx*v2*y=0&wD() < 0|(-1)+dx!=0&v1!=0&v2!=0&v1*v2+(-dx)*v1*v2 < 0&1+(-dy)*x+dx*y=0&wD() < 0|v2!=0&1+(-dy)*x+dx*y=0&(-1)+wD() < 0&v2+v1*v2+(-dx)*v1*v2+(-dy)*v2*x+dx*v2*y+v2*wD()+(-dy)*v2*x*wD()+dx*v2*y*wD() < 0&v2+v1*v2+(-dx)*v1*v2+(-dy)*v2*x+dx*v2*y+(-v2)*wD()+dy*v2*x*wD()+(-dx)*v2*y*wD() < 0|(-1)+dx!=0&v1!=0&v2!=0&v1*v2+(-dx)*v1*v2 < 0&1+(-dy)*x+dx*y=0&v2+v1*v2+(-dx)*v1*v2+(-dy)*v2*x+dx*v2*y+v2*wD()+(-dy)*v2*x*wD()+dx*v2*y*wD() < 0&v2+v1*v2+(-dx)*v1*v2+(-dy)*v2*x+dx*v2*y+(-v2)*wD()+dy*v2*x*wD()+(-dx)*v2*y*wD() < 0|v2+(-dy)*v2*x+dx*v2*y < 0&wD() < 0&-v2+(-v1)*v2+dx*v1*v2+dy*v2*x+(-dx)*v2*y+(-v2)*wD()+dy*v2*x*wD()+(-dx)*v2*y*wD() < 0|v2!=0&v2+(-dy)*v2*x+dx*v2*y < 0&wD() < 0&v2+v1*v2+(-dx)*v1*v2+(-dy)*v2*x+dx*v2*y+(-v2)*wD()+dy*v2*x*wD()+(-dx)*v2*y*wD() < 0|v2!=0&v2+(-dy)*v2*x+dx*v2*y < 0&v2+v1*v2+(-dx)*v1*v2+(-dy)*v2*x+dx*v2*y+v2*wD()+(-dy)*v2*x*wD()+dx*v2*y*wD() < 0&v2+v1*v2+(-dx)*v1*v2+(-dy)*v2*x+dx*v2*y+(-v2)*wD()+dy*v2*x*wD()+(-dx)*v2*y*wD() < 0|v2!=0&-v2+dy*v2*x+(-dx)*v2*y < 0&wD() < 0&-v2+(-v1)*v2+dx*v1*v2+dy*v2*x+(-dx)*v2*y+v2*wD()+(-dy)*v2*x*wD()+dx*v2*y*wD() < 0|v2!=0&-v2+dy*v2*x+(-dx)*v2*y < 0&wD() < 0&v2+v1*v2+(-dx)*v1*v2+(-dy)*v2*x+dx*v2*y+v2*wD()+(-dy)*v2*x*wD()+dx*v2*y*wD() < 0|v2!=0&-v2+dy*v2*x+(-dx)*v2*y < 0&v2+v1*v2+(-dx)*v1*v2+(-dy)*v2*x+dx*v2*y+v2*wD()+(-dy)*v2*x*wD()+dx*v2*y*wD() < 0&v2+v1*v2+(-dx)*v1*v2+(-dy)*v2*x+dx*v2*y+(-v2)*wD()+dy*v2*x*wD()+(-dx)*v2*y*wD() < 0)&(dx+-dxpost=0&(-1)+dx^2+dy^2=0&(-1)+dxpost^2+dypost^2=0&(-1)+w1ppost=0&w2post=0&-wD()<=0&(-dy)*x+dypost*xpost+dx*y+(-dxpost)*ypost=0|(-1)+dx^2+dy^2=0&(-1)+dxpost^2+dypost^2=0&(-1)+w1ppost=0&w2post=0&(-dy)*x+dypost*xpost+dx*y+(-dxpost)*ypost < 0&-dx+dxpost+(-dy)*x+(-dy)*wD()*x+dypost*xpost+dypost*wD()*xpost+dx*y+dx*wD()*y+(-dxpost)*ypost+(-dxpost)*wD()*ypost<=0&dx+-dxpost+dy*x+(-dy)*wD()*x+(-dypost)*xpost+dypost*wD()*xpost+(-dx)*y+dx*wD()*y+dxpost*ypost+(-dxpost)*wD()*ypost<=0|(-1)+dx^2+dy^2=0&(-1)+dxpost^2+dypost^2=0&(-1)+w1ppost=0&w2post=0&dy*x+(-dypost)*xpost+(-dx)*y+dxpost*ypost < 0&dx+-dxpost+dy*x+dy*wD()*x+(-dypost)*xpost+(-dypost)*wD()*xpost+(-dx)*y+(-dx)*wD()*y+dxpost*ypost+dxpost*wD()*ypost<=0&-dx+dxpost+(-dy)*x+dy*wD()*x+dypost*xpost+(-dypost)*wD()*xpost+dx*y+(-dx)*wD()*y+(-dxpost)*ypost+dxpost*wD()*ypost<=0)".asFormula
  }
  ...
  it should "FEATURE_REQUEST: derive model monitor from approximated planar flight motion with ownship velocity disturbance and intruder velocity uncertainty" taggedAs TodoTest ignore withMathematica { tool =>
    ...
    //@todo partial QE may not terminate
    deriveMonitor(entry, Some(entry.tactics.head._3), unobservableVars, tool,
      checkWitnessResult = _.subgoals.loneElement shouldBe " ==> \\exists v2 \\exists w2 (((0 < v2m&v2-vU()<=v2m&v2m<=v2+vU())&0 < w2m&w2-wU()<=w2m&w2m<=w2+wU())&w1ppost>0&\\forall w1pd \\forall v2p \\forall w2p (w1ppost<=w1pd&w1pd<=w1ppost+wD()&v2m-vU()<=v2p&v2p<=v2m+vU()&w2m-wU()<=w2p&w2p<=w2m+wU()->-w1pd*w2p*(x^2+y^2)+2*v2p*w1pd*dy*x+2*(v1*w2p-v2p*w1pd*dx)*y+2*v1*v2p*dx>2*v1*v2p+2*(v2p*w1pd+v1*w2p)+w1pd*w2p)&\\exists w1 ((w1ppost<=w1&w1<=w1ppost+wD())&dx^2+dy^2=1&(dxpost^2+dypost^2=1&I2(v1,v2,w1,w2,dxpost,dypost,xpost,ypost)=I2(v1,v2,w1,w2,dx,dy,x,y))&\\exists w2 ((0 < w2&0 < w2mpost&w2-wU()<=w2mpost&w2mpost<=w2+wU())&0 < v2mpost&v2-vU()<=v2mpost&v2mpost<=v2+vU())))".asSequent)
  }
  ...
  it should "derive a model monitor from ETCS essentials with train acceleration disturbance" in withMathematica { tool =>
    ...
    monitor shouldBe "m()-z<=SB(v)&(-aD()<=0&A()+b() < 0&accpost+b()=0&-ep()<=0&-ep()+tpost<=0&-tpost<=0&-v<=0&(-aD())*tpost+b()*tpost+-v+vpost<=0&-vpost<=0&(-aD())*tpost^2+b()*tpost^2+(-2)*tpost*v+(-2)*z+2*zpost<=0|-aD()<=0&accpost+b()=0&A()+2*aD()+b() < 0&-ep()<=0&-ep()+tpost<=0&-tpost<=0&-v<=0&(-aD())*tpost+b()*tpost+-v+vpost<=0&-vpost<=0&(-aD())*tpost^2+b()*tpost^2+(-2)*tpost*v+(-2)*z+2*zpost<=0|-aD()<=0&A()+b() < 0&accpost+b()=0&-ep()<=0&-ep()+tpost<=0&-tpost<=0&-v<=0&(-aD())*tpost+b()*tpost+-v+vpost<=0&-vpost<=0&(-A())*tpost^2+(-aD())*tpost^2+(-2)*tpost*v+(-2)*z+2*zpost<=0&(-aD())*tpost^2+b()*tpost^2+(-2)*tpost*v+(-2)*z+2*zpost<=0|-aD()<=0&accpost+b()=0&A()+2*aD()+b() < 0&-ep()<=0&-ep()+tpost<=0&-tpost<=0&-v<=0&(-aD())*tpost+b()*tpost+-v+vpost<=0&-vpost<=0&(-A())*tpost^2+(-aD())*tpost^2+(-2)*tpost*v+(-2)*z+2*zpost<=0&(-aD())*tpost^2+b()*tpost^2+(-2)*tpost*v+(-2)*z+2*zpost<=0|-aD()<=0&A()+b() < 0&accpost+b()=0&-ep()<=0&-ep()+tpost<=0&-tpost<=0&-v<=0&(-A())*tpost+(-aD())*tpost+-v+vpost<=0&(-aD())*tpost+b()*tpost+-v+vpost<=0&-vpost<=0&(-aD())*tpost^2+b()*tpost^2+(-2)*tpost*v+(-2)*z+2*zpost<=0|-aD()<=0&accpost+b()=0&A()+2*aD()+b() < 0&-ep()<=0&-ep()+tpost<=0&-tpost<=0&-v<=0&(-A())*tpost+(-aD())*tpost+-v+vpost<=0&(-aD())*tpost+b()*tpost+-v+vpost<=0&-vpost<=0&(-aD())*tpost^2+b()*tpost^2+(-2)*tpost*v+(-2)*z+2*zpost<=0|-aD()<=0&accpost+b()=0&-ep()<=0&-ep()+tpost<=0&-tpost<=0&-v<=0&(-A())*tpost+(-aD())*tpost+-v+vpost<=0&(-aD())*tpost+b()*tpost+-v+vpost<=0&-vpost<=0&(-A())*tpost^2+(-aD())*tpost^2+(-2)*tpost*v+(-2)*z+2*zpost<=0&(-aD())*tpost^2+b()*tpost^2+(-2)*tpost*v+(-2)*z+2*zpost<=0)|m()-z>=SB(v)&(A()+-accpost=0&-aD()<=0&-A()+-b() < 0&-ep()<=0&-ep()+tpost<=0&-tpost<=0&-v<=0&(-A())*tpost+(-aD())*tpost+-v+vpost<=0&-vpost<=0&(-A())*tpost^2+(-aD())*tpost^2+(-2)*tpost*v+(-2)*z+2*zpost<=0|A()+-accpost=0&-aD()<=0&-A()+2*aD()+-b() < 0&-ep()<=0&-ep()+tpost<=0&-tpost<=0&-v<=0&(-A())*tpost+(-aD())*tpost+-v+vpost<=0&-vpost<=0&(-A())*tpost^2+(-aD())*tpost^2+(-2)*tpost*v+(-2)*z+2*zpost<=0|A()+-accpost=0&-aD()<=0&-A()+-b() < 0&-ep()<=0&-ep()+tpost<=0&-tpost<=0&-v<=0&(-A())*tpost+(-aD())*tpost+-v+vpost<=0&-vpost<=0&(-A())*tpost^2+(-aD())*tpost^2+(-2)*tpost*v+(-2)*z+2*zpost<=0&(-aD())*tpost^2+b()*tpost^2+(-2)*tpost*v+(-2)*z+2*zpost<=0|A()+-accpost=0&-aD()<=0&-A()+2*aD()+-b() < 0&-ep()<=0&-ep()+tpost<=0&-tpost<=0&-v<=0&(-A())*tpost+(-aD())*tpost+-v+vpost<=0&-vpost<=0&(-A())*tpost^2+(-aD())*tpost^2+(-2)*tpost*v+(-2)*z+2*zpost<=0&(-aD())*tpost^2+b()*tpost^2+(-2)*tpost*v+(-2)*z+2*zpost<=0|A()+-accpost=0&-aD()<=0&-A()+-b() < 0&-ep()<=0&-ep()+tpost<=0&-tpost<=0&-v<=0&(-A())*tpost+(-aD())*tpost+-v+vpost<=0&(-aD())*tpost+b()*tpost+-v+vpost<=0&-vpost<=0&(-A())*tpost^2+(-aD())*tpost^2+(-2)*tpost*v+(-2)*z+2*zpost<=0|A()+-accpost=0&-aD()<=0&-A()+2*aD()+-b() < 0&-ep()<=0&-ep()+tpost<=0&-tpost<=0&-v<=0&(-A())*tpost+(-aD())*tpost+-v+vpost<=0&(-aD())*tpost+b()*tpost+-v+vpost<=0&-vpost<=0&(-A())*tpost^2+(-aD())*tpost^2+(-2)*tpost*v+(-2)*z+2*zpost<=0|A()+-accpost=0&-aD()<=0&-ep()<=0&-ep()+tpost<=0&-tpost<=0&-v<=0&(-A())*tpost+(-aD())*tpost+-v+vpost<=0&(-aD())*tpost+b()*tpost+-v+vpost<=0&-vpost<=0&(-A())*tpost^2+(-aD())*tpost^2+(-2)*tpost*v+(-2)*z+2*zpost<=0&(-aD())*tpost^2+b()*tpost^2+(-2)*tpost*v+(-2)*z+2*zpost<=0)".asFormula
  }
  ...
  it should "derive a model monitor from approximated VSL with car actuator disturbance" in withMathematica { tool =>
    ...
    val monitor = deriveMonitor(entry, Some(entry.tactics.head._3), unobservable, tool)
    monitor shouldBe "-aD()<=0&acc1post+B()=0&-ep()<=0&-ep()+tpost<=0&-v1<=0&-v1post<=0&vsl+-vslpost=0&xsl+-xslpost=0|vslpost>=0&xslpost>=accCompensation(x1,v1,vslpost)&-aD()<=0&acc1post+B()=0&-ep()<=0&-ep()+tpost<=0&-v1<=0&-v1post<=0|xsl>=accCompensation(x1,v1,vsl)&-B()<=acc1post&acc1post<=A()&(-aD()<=0&-ep()<=0&-ep()+tpost<=0&-v1<=0&-v1post<=0&vsl+-vslpost=0&xsl+-xslpost=0|vslpost>=0&xslpost>=accCompensation(x1,v1,vslpost)&-aD()<=0&-ep()<=0&-ep()+tpost<=0&-v1<=0&-v1post<=0)|x1>=xsl&-B()<=acc1post&acc1post<=A()&acc1post<=(v1-vsl)/ep()-aD()&(-aD()<=0&-ep()<=0&-ep()+tpost<=0&-v1<=0&-v1post<=0&vsl+-vslpost=0&xsl+-xslpost=0|vslpost>=0&xslpost>=accCompensation(x1,v1,vslpost)&-aD()<=0&-ep()<=0&-ep()+tpost<=0&-v1<=0&-v1post<=0)".asFormula
  }
  ...
  it should "derive a model monitor from approximated VSL with car position uncertainty" in withMathematica { tool =>
    ...
    val unobservable = ListMap(
      ...
      "v1_0".asVariable -> None, "t_0".asVariable -> None, "x1_0".asVariable -> None)
    val monitor = deriveMonitor(entry, Some(entry.tactics.head._3), unobservable, tool)
    monitor shouldBe "a1post+B()=0&-ep()<=0&-ep()+tpost<=0&-v1<=0&-v1post<=0&vsl+-vslpost=0&xsl+-xslpost=0&-xU()<=0|a1post+B()=0&-ep()<=0&-ep()+tpost<=0&-v1<=0&-v1post<=0&-vslpost<=0&-xU()<=0&-xslpost+accCompensation(x1m,v1,vslpost)<=0|-A()+a1post<=0&-a1post+-B()<=0&-ep()<=0&-ep()+tpost<=0&-v1<=0&-v1post<=0&vsl+-vslpost=0&xsl+-xslpost=0&-xU()<=0&-xsl+accCompensation(x1m,v1,vsl)<=0|-A()+a1post<=0&-a1post+-B()<=0&-ep()<=0&-ep()+tpost<=0&-v1<=0&-v1post<=0&-vslpost<=0&-xU()<=0&-xslpost+accCompensation(x1m,v1,vslpost)<=0&-xsl+accCompensation(x1m,v1,vsl)<=0|-A()+a1post<=0&-a1post+-B()<=0&-ep() < 0&-ep()+tpost<=0&-v1<=0&-v1post<=0&a1post*ep()+-v1+vsl<=0&vsl+-vslpost=0&xsl+-xslpost=0&-x1m+xsl+xU()<=0&-xU()<=0|-A()+a1post<=0&-a1post+-B()<=0&-ep() < 0&-ep()+tpost<=0&-v1<=0&-v1post<=0&a1post*ep()+-v1+vsl<=0&vsl+-vslpost=0&xsl+-xslpost=0&-xU()<=0&-x1m+xsl+-xU()<=0|-A()+a1post<=0&-a1post+-B()<=0&-ep() < 0&-ep()+tpost<=0&-v1<=0&-v1post<=0&a1post*ep()+-v1+vsl<=0&-vslpost<=0&-x1m+xsl+xU()<=0&-xU()<=0&-xslpost+accCompensation(x1m,v1,vslpost)<=0|-A()+a1post<=0&-a1post+-B()<=0&-ep() < 0&-ep()+tpost<=0&-v1<=0&-v1post<=0&a1post*ep()+-v1+vsl<=0&-vslpost<=0&-xU()<=0&-x1m+xsl+-xU()<=0&-xslpost+accCompensation(x1m,v1,vslpost)<=0".asFormula
  }
