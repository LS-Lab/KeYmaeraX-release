...
object TotalizerTest {
  def apply(name: String, proof: String, sandbox: String, fallback: Option[String] = None): TotalizerTestCase = {
    val pf =
      try { KaisarProgramParser.parseSingle(proof) }
      catch { case th: Throwable => throw new Exception("Parse error in test model: " + name, th) }
    val sb =
      try { KaisarProgramParser.parseSingle(sandbox) }
      catch { case th: Throwable => throw new Exception("Parse error in test sandbox: " + name, th) }
    val fb = fallback.map(fall => {
      try { KaisarProgramParser.parseSingle(fall) }
      catch { case th: Throwable => throw new Exception("Parse error in test fallback: " + name, th) }
    })
