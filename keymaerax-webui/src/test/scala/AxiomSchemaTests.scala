...
class AxiomSchemaTests extends TacticTestBase {
  ...
  "Vectorial DG" should "return 1D VDG" in {
    ...
    vdg._1.conclusion shouldBe
      "==> [{y__1'=g1(||),c{|y__1|}&q(|y__1|)}]y__1*y__1<=f_(|y__1|)->[{y__1'=g1(||),c{|y__1|}&q(|y__1|)}]p(|y__1|)->[{c{|y__1|}&q(|y__1|)}]p(|y__1|)"
        .asSequent
    vdg._2.conclusion shouldBe
      "==> [{c{|y__1|}&q(|y__1|)}]p(|y__1|)->[{y__1'=g1(||),c{|y__1|}&q(|y__1|)}]p(|y__1|)".asSequent
  }
  ...
  it should "return a 2D VDG" in {
    ...
    val f1 = Imply(
      Box(ghostode, LessEqual(Plus(Times(y1, y1), Times(y2, y2)), ffunc)),
