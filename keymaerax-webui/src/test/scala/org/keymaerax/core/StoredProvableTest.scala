...
import org.keymaerax.tagobjects.{SlowTest, SummaryTest, TodoTest, UsualTest}
    ...
    for (_ <- 1 to 100)
      try {
        ...
        case possible: ClassCastException
            if possible.getMessage.contains("cannot be cast to org.keymaerax.core.Variable") => /* continue */
        case possible: ClassCastException
            if possible.getMessage.contains("cannot be cast to org.keymaerax.core.DifferentialSymbol") => /* continue */
      }
    True
  }
}
