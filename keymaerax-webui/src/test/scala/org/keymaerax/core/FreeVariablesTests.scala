...
class FreeVariablesTests extends AnyFlatSpec with Matchers with BeforeAndAfterEach with BeforeAndAfterAll {
  ...
  it should "return names on sequents" in {
    ...
    the[IllegalArgumentException] thrownBy
      ...
      have message "Cannot obtain fresh symbol, since class org.keymaerax.core.UnitFunctional does not have index"
    the[IllegalArgumentException] thrownBy
      TacticHelper.freshNamedSymbol(UnitPredicational("x", AnyArg), "==> x(||)".asPlainSequent) should have message
      "Cannot obtain fresh symbol, since class org.keymaerax.core.UnitPredicational does not have index"
    the[IllegalArgumentException] thrownBy
      TacticHelper.freshNamedSymbol(ProgramConst("x", AnyArg), "==> [x;]y>0".asPlainSequent) should have message
      "Cannot obtain fresh symbol, since class org.keymaerax.core.ProgramConst does not have index"
  }
}
