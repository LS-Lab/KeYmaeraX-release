...
import org.keymaerax.tagobjects.{AdvocatusTest, CoverageTest, OptimisticTest}
...
class USubstTests extends TacticTestBase {
  ...
  it should "substitute simple formula [a]p(x) <-> [a](p(x)&true)" in {
    ...
    val s = USubst(Seq(
      SubstitutionPair(PredOf(p, DotTerm()), GreaterEqual(DotTerm(), Number(2))),
  ...
  it should "clash when using [:=] for a substitution with a free occurrence of a bound variable" in {
    ...
    val s = USubst(Seq(
      SubstitutionPair(PredOf(p1, DotTerm()), NotEqual(DotTerm(), "x_".asTerm)),
  ...
  it should "clash when using [:=] for a substitution with a free occurrence of a bound variable for constants" in {
    ...
    val prem = ProvableSig.axioms("[:=] assign")
    val s = USubst(
      Seq(SubstitutionPair(PredOf(p1, DotTerm()), Equal(DotTerm(), "x_".asTerm)), SubstitutionPair(fn, "0".asTerm))
    )
    a[SubstitutionClashException] should be thrownBy prem(s)
    a[SubstitutionClashException] should be thrownBy s(prem.conclusion)
  }
  ...
  it should "handle nontrivial binding structures" in {
    ...
    val prem = ProvableSig.axioms("[:=] assign")
    val conc =
      "[x_:=x_^2;][{y:=y+1;++{z:=x_+z;}*}; z:=x_+y*z;]y>x_ <-> [{y:=y+1;++{z:=x_^2+z;}*}; z:=x_^2+y*z;]y>x_^2".asFormula
    ...
    val s = USubst(Seq(
      ...
      SubstitutionPair(
        ...
        Box(
          Compose(
            Choice(Assign(y, Plus(y, Number(1))), Loop(Assign(z, Plus(DotTerm(), z)))),
...
//    )
  ...
  it should "refuse to accept ill-kinded substitutions outright" in {
    a[CoreException] should be thrownBy SubstitutionPair(
      FuncOf(Function("a", None, Unit, Real), Nothing),
      Greater(Variable("x"), Number(5)),
    )
    a[CoreException] should be thrownBy SubstitutionPair(
      FuncOf(Function("a", None, Real, Real), DotTerm()),
      Greater(Variable("x"), Number(5)),
    )
    a[CoreException] should be thrownBy SubstitutionPair(
      FuncOf(Function("a", None, Unit, Real), Nothing),
  ...
  it should "refuse to accept ill-shaped substitutions outright" in {
    ...
    a[CoreException] should be thrownBy SubstitutionPair(
      Plus(FuncOf(Function("a", None, Unit, Real), Nothing), FuncOf(Function("b", None, Unit, Real), Nothing)),
      Number(9),
    )
    a[CoreException] should be thrownBy SubstitutionPair(
      And(Greater(Number(7), Number(2)), Less(Number(2), Number(1))),
      False,
    )
    ...
    a[CoreException] should be thrownBy SubstitutionPair(
      Assign(Variable("x"), Number(7)),
      Assign(Variable("y"), Number(7)),
    )
    ...
    a[CoreException] should be thrownBy SubstitutionPair(
      ...
      AssignAny(Variable("x")),
    )
    a[CoreException] should be thrownBy SubstitutionPair(
      ODESystem(AtomicODE(DifferentialSymbol(Variable("x")), Number(7)), True),
      AssignAny(Variable("x")),
    )
  }
  ...
  it should "refuse ++ union that lead to duplicate substitutions" in {
    val list1 = (
      USubst(SubstitutionPair(FuncOf(Function("a", None, Real, Real), DotTerm()), Number(5)) :: Nil),
    ...
    val list2 = (
      USubst(
        SubstitutionPair(PredOf(Function("p", None, Unit, Bool), Nothing), Greater(Variable("x"), Number(5))) :: Nil
      ),
    ...
    val list3 = (
      USubst(SubstitutionPair(ProgramConst("c"), Assign(Variable("y"), Number(7))) :: Nil),
  ...
  "Uniform substitution of SpaceDependents" should "put old vars into DG constant" in {
    ...
    val s = USubst(
      SubstitutionPair(
        ...
        AtomicODE(DifferentialSymbol(Variable("z")), Power(Variable("z"), Number(5))),
      ) ::
      ...
      val s = USubst(
        SubstitutionPair(
          ...
          SubstitutionPair(
            ...
            Times(Power(Variable("z"), Number(2)), Power(y, Number(2))),
          ) :: Nil
      )
      pr(s)
    }
    ...
    val s = USubst(
      SubstitutionPair(
        ...
        SubstitutionPair(
          ...
          Forall(Seq(y), LessEqual(Variable("z"), Power(y, Number(2)))),
        ) ::
        SubstitutionPair(UnitFunctional("a", Except(y :: Nil), Real), Power(Variable("z"), Number(3))) ::
        SubstitutionPair(UnitFunctional("b", Except(y :: Nil), Real), Power(Variable("z"), Number(2))) :: Nil
    )
  ...
  it should "possibly accept new vars if only bound in DG's postcondition assigned" in {
    ...
    val s = USubst(
      SubstitutionPair(
        ...
        SubstitutionPair(
          UnitPredicational("p", Except(y :: Nil)),
          Box(Assign(y, Variable("z")), LessEqual(Variable("z"), Power(y, Number(2)))),
        ) ::
        SubstitutionPair(UnitFunctional("a", Except(y :: Nil), Real), Power(Variable("z"), Number(3))) ::
        SubstitutionPair(UnitFunctional("b", Except(y :: Nil), Real), Power(Variable("z"), Number(2))) :: Nil
    )
      ...
      val s = USubst(
        SubstitutionPair(
          ...
          SubstitutionPair(
            UnitPredicational("p", Except(y :: Nil)),
            Box(
              ODESystem(AtomicODE(DifferentialSymbol(y), Variable("z"))),
              LessEqual(Variable("z"), Power(y, Number(2))),
            ),
          ) ::
          SubstitutionPair(UnitFunctional("a", Except(y :: Nil), Real), Power(Variable("z"), Number(3))) ::
          SubstitutionPair(UnitFunctional("b", Except(y :: Nil), Real), Power(Variable("z"), Number(2))) :: Nil
  ...
  it should "possibly accept new vars if only bound in DGd postcondition as quantifiers" in {
    ...
    val s = USubst(
      SubstitutionPair(
        ...
        SubstitutionPair(
          UnitPredicational("p", Except(y :: Nil)),
          Forall(Seq(y), GreaterEqual(Power(y, Number(2)), Number(0))),
        ) ::
  ...
  it should "possibly accept new vars if only bound in DGd postcondition as quantifiers, renamed" in {
    ...
    inst shouldBe Symbol("proved")
    inst.conclusion shouldBe Sequent(IndexedSeq(), IndexedSeq(expected))
    val inst2 = inst(Sequent(IndexedSeq(), IndexedSeq(expected2)), UniformRenaming(y, Variable("x")))
    inst2 shouldBe Symbol("proved")
    inst2.conclusion shouldBe Sequent(IndexedSeq(), IndexedSeq(expected2))
    val inst3 = (ProvableSig.startPlainProof(expected2)(UniformRenaming(y, Variable("x")), 0)(inst, 0))
  ...
  it should "instantiate nontrivial binding structures in [] congruence" in {
    ...
    val s = USubst(
      SubstitutionPair(ap_, prog) ::
        ...
        SubstitutionPair(
          PredicationalOf(ctx_, DotFormula),
          Box("{y:=y+1;++{z:=x+z;}*}; z:=x+y*z;".asProgram, DotFormula),
        ) :: Nil
    )
  ...
  it should "instantiate crazy program in <> congruence" ignore {
    ...
    val prem = Equiv(prem1, prem2)
    val prog =
      "{{{{?true;++{?\\forall z6 ([{z5'=21&[{?true;{?true;++?true;{?true;}*}}*]-40 < 0}]<{{{{z7'=0&true}{?true;++{z5'=0,z3'=0&\\forall z5 true}{?true;?true;}?true;?true;}}{{{z3'=0*0&true}++{?false;}*}++z1:=*;}{{{?true;++?true;}*}*++z1:=*;}}?true;?true;}*{{{z5:=z5;++?true;}++z7:=*;z6:=z6;}{{z6'=6&z5 < 0}}*++?true;}*>PP{qq()})';}*}z3:=z3;}{{?true;z3:=ff((gg()-ff(((0/(z2*z4)-(1-48))/(-28-66))')+(z5+z2')/(0*(gg()+(z4'-gg())-z7)/z1*z2'))/((z5'-z4)*gg())^5);++z2:=z2;?true;}z4:=*;++{{?true;}*++?true;}z5:=*;}}{{?true;{{{{z7'=0&true}}*++{{z1:=z3';++{{{z3:=z6;z2:=z2;}*?qq();}{z6:=*;}*}*}++{?true;z1:=*;}*++z4:=z4;?<{?<?true;>[?true;]true;{z4:=0;{?true;}*}{?true;?true;}{?true;}*}{{z1:=0;}*}*?true;++{{?true;++{{?true;}*}*}++?<z5:=0;><?true;>true;}{{{?true;++?true;}{z5'=0&true}}?true;}*>[{{z3'=0+0+(0+0)&[?true;?true;]\\forall z2 true}++?true;++?true;?true;?true;}{{?true;}*{?true;?true;}?true;}z2:=*;]<?true;><z7:=*;++z5:=z5;++?true;++?true;>z4'+gg()!=z1+(0-0);}{z4:=-50/z1^1;}*?true;}*}*}*++?true;}}z2:=(ff(-13-z5))';"
        .asProgram
    val conc = Equiv(Diamond(prog, prem1), Diamond(prog, prem2))
  ...
  "Congruence rules" should "instantiate CT from y+z=z+y" in {
    ...
    pr.conclusion shouldBe Sequent(IndexedSeq(), IndexedSeq(Equal(Minus(term1, Number(5)), Minus(term2, Number(5)))))
    pr.subgoals should be(List(Sequent(IndexedSeq(), IndexedSeq(fml))))
  }
  ...
  it should "instantiate CQ from y+z=z+y in context y>1&.<=5" in {
    ...
    pr.conclusion shouldBe
      Sequent(
        IndexedSeq(),
        IndexedSeq(Equiv(
          And(Greater(y, Number(1)), LessEqual(term1, Number(5))),
  ...
  it should "instantiate CQ from y+z=z+y in context \\forall x .<=5" in {
    ...
    pr.conclusion shouldBe
      Sequent(
        IndexedSeq(),
        IndexedSeq(Equiv(Forall(Seq(y), LessEqual(term1, Number(5))), Forall(Seq(y), LessEqual(term2, Number(5))))),
      )
    pr.subgoals should be(List(Sequent(IndexedSeq(), IndexedSeq(fml))))
  }
  ...
  it should "instantiate CQ from y+z=z+y in context [x:=x-1]" in {
    ...
    pr.conclusion shouldBe
      Sequent(
        IndexedSeq(),
        IndexedSeq(Equiv(Box(prog, GreaterEqual(term1, Number(0))), Box(prog, GreaterEqual(term2, Number(0))))),
      )
    pr.subgoals should be(List(Sequent(IndexedSeq(), IndexedSeq(fml))))
  }
  ...
  it should "?instantiate CQ from z^2*y=-(-z)^2*-y+0 in context \\forall y" taggedAs OptimisticTest ignore {
    ...
    pr.conclusion shouldBe
      Sequent(
        IndexedSeq(),
        IndexedSeq(
          Equiv(Forall(Seq(y), GreaterEqual(term1, Number(0))), Forall(Seq(y), GreaterEqual(term2, Number(0))))
        ),
      )
    pr.subgoals should be(List(Sequent(IndexedSeq(), IndexedSeq(fml))))
  }
  ...
  it should "?instantiate CQ from z^2*y=-(-z)^2*-y+0 in complex contexts" taggedAs OptimisticTest ignore {
    ...
    pr.conclusion shouldBe
      Sequent(
        IndexedSeq(),
        IndexedSeq(
          Equiv(Forall(Seq(u), Box(prog, GreaterEqual(term1, u))), Forall(Seq(u), Box(prog, GreaterEqual(term2, u))))
        ),
      )
    pr.subgoals should be(List(Sequent(IndexedSeq(), IndexedSeq(fml))))
  }
  ...
  it should "have no effect on other predicationals" taggedAs CoverageTest in {
    ...
    USubst(SubstitutionPair(PredicationalOf(Function("q", None, Bool, Bool), DotFormula), True) :: Nil)(
      fml
    ) shouldBe fml
  }
  ...
  def contextapp(context: Formula, arg: Formula): Formula = {
    val mycontext =
      Function(
        ...
        Bool,
        Bool,
      ) // @TODO eisegesis  should be Function("dottingC_", None, Real->Bool, Bool) //@TODO introduce function types or the Predicational datatype
