...
class KeYmaeraXLexerTests extends AnyFlatSpec with Matchers {
  ...
  it should "parse forall" in {
    ...
    KeYmaeraXLexer(input).head shouldBe org.keymaerax.parser.Token(FORALL, Region(1, 1, 1, 7))
  }
  ...
  it should "parse forall 2 times" in {
    ...
    KeYmaeraXLexer(input).head shouldBe org.keymaerax.parser.Token(FORALL, Region(1, 1, 1, 7))
    KeYmaeraXLexer(input).tail.head shouldBe org.keymaerax.parser.Token(FORALL, Region(1, 9, 1, 15))
  }

  // @todo Nathan
  it should "parse an identifier" in {
    val input = "input"
    KeYmaeraXLexer(input).head shouldBe org.keymaerax.parser.Token(IDENT("input"), Region(1, 1, 1, 5))
  }

  it should "parse an identifier -- checking length is ok" in {
    val input = "thisisalongvariablename"
    KeYmaeraXLexer(input).head shouldBe
      org.keymaerax.parser.Token(IDENT("thisisalongvariablename"), Region(1, 1, 1, input.length))
  }

  it should "parse an integer" in {
    val n = "99"
    KeYmaeraXLexer(n).head shouldBe org.keymaerax.parser.Token(NUMBER(n), Region(1, 1, 1, n.length))
  }
