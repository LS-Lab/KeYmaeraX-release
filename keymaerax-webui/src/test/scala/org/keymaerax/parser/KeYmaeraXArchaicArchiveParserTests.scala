...
import org.keymaerax.btactics.{DebuggingTactics, TacticTestBase, TactixLibrary}
import org.keymaerax.core.{Bool, FuncOf, GreaterEqual, Number, Real, SubstitutionPair, Trafo, Tuple, Unit, Variable}
...
class KeYmaeraXArchaicArchiveParserTests extends TacticTestBase {
  ...
  it should "allow line breaks and escaped quotation marks in double-quoted strings" in {
    val input = """ArchiveEntry "Entry \"The Fan-
                  |                          tas-
                  |                          tic\" 1"
  ...
  it should "FEATURE_REQUEST: parse definitions with dot arguments" taggedAs TodoTest in {
    ...
    entry.kind shouldBe "theorem"
    entry.defs should beDecl(Declaration(Map(
      Name("f", None) -> Signature(
        Some(Real),
        Real,
        Some((Name("\\cdot", Some(0)), Real) :: Nil),
        Right(None),
        UnknownLocation,
      ),
      Name("g", None) -> Signature(
        Some(Tuple(Real, Real)),
        Real,
        Some((Name("\\cdot", Some(0)), Real) :: (Name("\\cdot", Some(1)), Real) :: Nil),
        Right(None),
        UnknownLocation,
      ),
      Name("h", None) -> Signature(
        Some(Real),
        Real,
        Some((Name("\\cdot", Some(0)), Real) :: Nil),
        Right(Some(".+2".asTerm)),
  ...
  it should "not report builtin interpreted symbols as undeclared" ignore {
    ...
    entry.defs should beDecl(Declaration(Map(
      Name("p", None) -> Signature(
        ...
        Bool,
        Some(List.empty),
        Right(Some("min(4,5)>=3".asFormula)),
        UnknownLocation,
      )
    )))
  }
  ...
  it should "elaborate only free differential symbols to differentials" in {
    ...
    entry.defs should beDecl(Declaration(Map(
      Name("p", None) -> Signature(
        ...
        Bool,
        Some(List(Name("x", None) -> Real)),
        Right(Some("\\forall x' x+x' <= x+(x')^2".asFormula)),
        UnknownLocation,
      )
    )))
  }
  ...
  it should "FEATURE_REQUEST: expand program definitions to decide whether symbol is undefined (2)" taggedAs TodoTest in {
    ...
    the[ParseException] thrownBy parse(
      input
    ) should have message """4:5 Definition p uses undefined symbol(s) z. Please add arguments or define as functions/predicates/programs"""
  }
  ...
  it should "parse a predicate definition with nested programs" in {
    ...
    entry.defs should beDecl(Declaration(Map(
      Name("p", None) -> Signature(
        ...
        Bool,
        Some((Name("x", None), Real) :: Nil),
        Right(Some("[x:=x+1;]x>=1".asFormula)),
        UnknownLocation,
      )
    )))
  ...
  it should "parse when definitions use constant function symbols" in {
    val input = """ArchiveEntry "Definitions with Constants"
                  |Definitions
                  |  Real A;
    ...
    entry.defs should beDecl(Declaration(Map(
      ...
      Name("Dist", None) -> Signature(
        Some(Tuple(Real, Real)),
        Real,
        Some((Name("v", None), Real) :: (Name("t", None), Real) :: Nil),
        Right(Some("v*t + A()*t^2/2".asTerm)),
        UnknownLocation,
      ),
    )))
  ...
  it should "parse when function argument has same name as constant in annotations" in {
    ...
    entry.defs should beDecl(Declaration(Map(
      ...
      Name("plus", None) -> Signature(
        Some(Tuple(Real, Real)),
        Real,
        Some((Name("x", None), Real) :: (Name("y", None), Real) :: Nil),
        Right(Some("sq(x)+y".asTerm)),
        UnknownLocation,
      ),
    )))
  ...
  it should "parse a plain problem format" in {
    val input = """ProgramVariables. R x. R y. End.
                  |Problem. x>y -> x>=y End.
      """.stripMargin
    val entry = parse(input).loneElement
  ...
  it should "parse a tactic with sub-position locators" in withZ3 { _ =>
    ...
    entry.tactics shouldBe (
      "Proof 1",
      "simplify('R==\"x>y -> #x*0#>=y\"); QE",
      simplify(Find.FindR(0, Some("x>y -> x*0>=y".asFormula), PosInExpr(1 :: 0 :: Nil), exact = true, entry.defs)) & QE,
    ) :: Nil
    entry.info shouldBe empty
  }
  ...
  it should "parse a multi-line pending tactic with arguments and branch labels" in withTactics {
    ...
    entry.tactics shouldBe (
      ...
      """implyR(1) ; pending("dC(\"x>=old(x)\", 1) <(
        ...
        |                          \"Show\": dI(1); done
        |                        )")""".stripMargin,
      implyR(1) & DebuggingTactics.pending(
        """dC(\"x>=old(x)\", 1) <(
          |                          \"Use\": todo,
          |                          \"Show\": dI(1); done
          |                        )""".stripMargin
  ...
  it should "add to all entries but not auto-expand if tactic uses US to expand" in {
    ...
    entry2.tactics shouldBe (
      "Proof Entry 2",
      """US("gt(._0,._1) ~> ._0>._1") ; useLemma("Entry 1")""",
  ...
  it should "add to entries selectively according to definitions used" in {
    ...
    val input = """
                  ...
                  |  Problem [y:=y;]y=y End.
                  |End.""".stripMargin
    inside(parser.parse(input)) { case e1 :: e2 :: Nil =>
      e1.fileContent shouldBe
        """SharedDefinitions
          ...
          |  Problem [asgn;]y=y End.
          |End.""".stripMargin
      e1.defs should beDecl(Declaration(Map(
        ...
        Name("incy", None) -> Signature(
          Some(Unit),
          Trafo,
          None,
          Right(Some("{contincy{|^@|}; ++ discincy{|^@|};}".asProgram)),
          UnknownLocation,
        ),
        Name("sq", None) -> Signature(
          Some(Real),
          Real,
          Some(List((Name("x", None), Real))),
          Right(Some("x^2+((x)')^2".asTerm)),
          UnknownLocation,
        ),
        Name("nonneg", None) -> Signature(
          Some(Real),
          Bool,
          Some(List((Name("x", None), Real))),
          Right(Some("x>=0".asFormula)),
      ...
      e2.fileContent shouldBe
        """SharedDefinitions
          ...
          |  Problem [y:=y;]y=y End.
          |End.""".stripMargin
      e2.defs should beDecl(Declaration(Map(
        Name("discincy", None) -> Signature(Some(Unit), Trafo, None, Right(Some("y:=y+1;".asProgram)), UnknownLocation),
        Name("contincy", None) -> Signature(Some(Unit), Trafo, None, Right(Some("{y'=1}".asProgram)), UnknownLocation),
        Name("incy", None) -> Signature(
          Some(Unit),
  ...
  it should "add to exercises selectively according to definitions used" in {
    val input = """
                  ...
                  |End.""".stripMargin
    inside(parse(input)) { case e1 :: e2 :: e3 :: Nil =>
      e1.fileContent shouldBe
        """SharedDefinitions
          |  HP inc ::= { x:=x+1; };
          |  HP asgn ::= { __________; };
      ...
      e3.fileContent shouldBe
        """SharedDefinitions
          ...
          |  Problem [inc;]x>=old(x) End.
          |End.""".stripMargin
      e3.defs should beDecl(Declaration(Map(
        Name("inc", None) -> Signature(Some(Unit), Trafo, None, Right(Some("x:=x+1;".asProgram)), UnknownLocation),
        Name("x", None) -> Signature.variable(),
      )))
  ...
  it should "not complain about undeclared builtin interpreted functions" ignore {
    parse(
      """ArchiveEntry "Builtin interpreted"
        ...
        |End.
        |""".stripMargin
    ).loneElement.model shouldBe GreaterEqual(FuncOf(InterpretedSymbols.expF, Variable("x")), Number(0))
  }
    ...
    the[ParseException] thrownBy parse(
      """ArchiveEntry "Entry 1".
        | Problem. true End""".stripMargin
                            ...
                            |Expected: .
                            |Hint: ParseState( :+ ArchiveEntry :+ DOUBLE_QUOTES_STRING :+ MetaInfo(Map()) :+ Definitions(List(),List())  <|>  PROBLEM_BLOCK$, PERIOD$, TRUE$, END_BLOCK$, EOF$)"""
      .stripMargin
    ...
    the[ParseException] thrownBy parse(
      """ArchiveEntry "Entry 1".
        ...
        |Tactic "My tactic" closeTrue End. """.stripMargin
                            ...
                            |Expected: .
                            |Hint: ParseState( :+ ArchiveEntry :+ DOUBLE_QUOTES_STRING :+ MetaInfo(Map()) :+ Definitions(List(),List())  <|>  PROBLEM_BLOCK$, PERIOD$, TRUE$, END_BLOCK$, TACTIC_BLOCK$, DOUBLE_QUOTES_STRING, ID("closeTrue"), END_BLOCK$, PERIOD$, EOF$)"""
      .stripMargin
  }
  ...
  it should "report a mismatch between model and annotation an entry without definitions" in {
    the[ParseException] thrownBy parse(
      "ArchiveEntry \"Test\" Problem x>y -> [{x:=y;}*@invariant(x()>=y)]x>=y End. End."
    ) should have message
