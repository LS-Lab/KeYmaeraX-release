...
object CustomAssertions {
   ...
   */
  def withSafeClue[T](clue: Any)(fun: => T): T = { try { fun } catch { case e: Any => withClue(clue) { fail(e) } } }
  ...
  def theDeductionOf[E <: Throwable: ClassTag](f: => ProvableSig): Either[ProvableSig, E] =
    try { Left(f) } catch { case e if classTag[E].runtimeClass.isInstance(e) => Right(e.asInstanceOf[E]) }

  /**
   * Checks that a provable, if present, has a sole subgoal equal to its conclusion, so is equivalent to a single
   * Provable.startProof.
   */
  val throwOrNoop: Matcher[Either[ProvableSig, Throwable]] =
    throwOrNoop[Throwable]((p: ProvableSig) => p.subgoals.size == 1 && p.subgoals.head == p.conclusion)

  /** Checks that a provable, if present, matches the specified noop condition. */
  def throwOrNoop[E <: AnyRef](
      noopCond: (ProvableSig => Boolean)
  )(implicit manifest: Manifest[E]): Matcher[Either[ProvableSig, E]] = Matcher { (pr: Either[ProvableSig, E]) =>
    MatchResult(
      pr match {
