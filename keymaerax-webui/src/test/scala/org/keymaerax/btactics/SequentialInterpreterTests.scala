...
import org.keymaerax.infrastruct.{PosInExpr, Position, RenUSubst}
 ...
 * switch out theInterpreter when other interpreters are implemented.
...
class SequentialInterpreterTests extends TacticTestBase {
  ...
  "Locators" should "apply searchy with sub-positions" in withTactics {
    inside(theInterpreter(
      composeb(Find.FindRPlain("[x:=2;][y:=*;?y>=x;]y>=2".asFormula, PosInExpr(1 :: Nil))),
      BelleProvable.plain(ProvableSig.startPlainProof("==> [x:=2;][y:=*;?y>=x;]y>=2".asSequent)),
    )) { case BelleProvable(p, _) => p.subgoals.loneElement shouldBe "==> [x:=2;][y:=*;][?y>=x;]y>=2".asSequent }
  }
  ...
  "AndR" should "prove |- 1=1 ^ 2=2" in withMathematica { _ =>
    inside(theInterpreter(andR(1), BelleProvable.plain(ProvableSig.startPlainProof("1=1 & 2=2".asFormula)))) {
      case BelleProvable(p, _) => p.subgoals should contain theSameElementsInOrderAs "==> 1=1".asSequent :: "==> 2=2"
          .asSequent :: Nil
    }
  }
  ...
  "Sequential Combinator" should "prove |- 1=2 -> 1=2" in withMathematica { _ =>
    inside(
      theInterpreter(implyR(1) & close, BelleProvable.plain(ProvableSig.startPlainProof("1=2 -> 1=2".asFormula)))
  ...
  "Either combinator" should "prove |- 1=2 -> 1=2 by AndR | (ImplyR & Close)" in withMathematica { _ =>
    inside(theInterpreter(
      andR(1) | (implyR(1) & close),
      BelleProvable.plain(ProvableSig.startPlainProof("1=2 -> 1=2".asFormula)),
    )) { case BelleProvable(p, _) => p shouldBe Symbol("proved") }
  }

  it should "prove |- 1=2 -> 1=2 by (ImplyR & Close) | AndR" in withMathematica { _ =>
    inside(theInterpreter(
      (implyR(1) & close) | andR(1),
      BelleProvable.plain(ProvableSig.startPlainProof("1=2 -> 1=2".asFormula)),
    )) { case BelleProvable(p, _) => p shouldBe Symbol("proved") }
  }

  it should "failover to right whenever a non-closing and non-partial tactic is provided on the left" in withMathematica {
    _ =>
  ...
  it should "run right if left makes no progress" in withMathematica { _ =>
    proveBy("x>=0 ==> \\forall y (y=x -> y>=0)".asSequent, prop | allR(1))
      .subgoals
      .loneElement shouldBe "x>=0 ==> y=x -> y>=0".asSequent
    proveBy(
      "x>=0 -> \\forall y (y=x -> y>=0)".asFormula,
      SaturateTactic(prop | allR(Symbol("R")) | exhaustiveEqL2R(Symbol("Llast"))),
  ...
  it should "support 'Rlike unification matching" in withMathematica { _ =>
    val result = proveBy(
      "(a=0&b=1 -> c=2) | (d=3 -> e=4) | (f=5&g=6 -> h=7)".asFormula,
  ...
  it should "report when no unification found with 'Rlike unification matching" in withMathematica { _ =>
    the[BelleProofSearchControl] thrownBy proveBy(
      "==> a=0, b=1, c=2->d=3".asSequent,
      onAll(implyR(Symbol("Rlike"), "p_()&q_()->r_()".asFormula)),
    ) should have message
  ...
  it should "support 'Llike unification matching" in withMathematica { _ =>
    val result = proveBy(
      "(a=0&b=1 -> c=2) & (d=3 -> e=4) & (f=5&g=6 -> h=7) -> i=8".asFormula,
      implyR(Symbol("R")) & SaturateTactic(andL(Symbol("L"))) & SaturateTactic(
        onAll(implyL(Symbol("Llike"), "p_()&q_()->r_()".asFormula))
      ),
    )
  ...
  it should "prove x=2&y=3&z=4 |- z=4" in withMathematica { _ =>
    proveBy(
      "x=2&y=3&z=4 ==> z=4".asSequent,
      SaturateTactic(andL(Symbol("_"))) &
  ...
  it should "saturate until no longer applicable" in withMathematica { _ =>
    proveBy(
      "x=2&y=3&(z=4|z=5) ==> x=2".asSequent,
      SaturateTactic(andL(Symbol("Llast"))) &
        assertE("x=2".asFormula, "x=2 not at -1")(-1) & assertE("y=3".asFormula, "y=3 not at -2")(-2) &
  ...
  it should "try right branch when used in combination with either combinator" in withMathematica { _ =>
    proveBy(
      "x=2&y=3&(z=4|z=5) ==> x=2".asSequent,
      SaturateTactic(SaturateTactic(andL(Symbol("Llast"))) | close),
    ) shouldBe Symbol("proved")
  }
  ...
  it should "trace in the database" in withDatabase { db =>
    withMathematica { _ =>
      val fml = "[x:=3;](x>0 & x>1 & x>2)".asFormula
      val modelContent =
        s"""ArchiveEntry "Test" ProgramVariables Real x; End.\n Problem ${fml.prettyString} End. End."""
      db.createProof(modelContent, "saturateTest")
      db.proveBy(
        modelContent,
        SaturateTactic(onAll(boxAnd(Symbol("R")) & andR(Symbol("R")))) & master(),
      ) shouldBe Symbol("proved")
    }
  }
  ...
  it should "not endless repeat on new labels" in withMathematica { _ =>
    failAfter(1 second) {
      proveBy("x>=0 ==> x>=0".asSequent, SaturateTactic(label("A label"))).subgoals.loneElement shouldBe "x>=0 ==> x>=0"
        .asSequent
    }
  }

  "Repeat combinator" should "repeat the specified number of times" in withMathematica { _ =>
    proveBy("x=2&y=3&z=4 ==> x=2".asSequent, andL(Symbol("L")) * 0).subgoals.loneElement shouldBe "x=2&y=3&z=4 ==> x=2"
      .asSequent
    proveBy("x=2&y=3&z=4 ==> x=2".asSequent, andL(Symbol("L")) * 1).subgoals.loneElement shouldBe "x=2, y=3&z=4 ==> x=2"
      .asSequent
    proveBy("x=2&y=3&z=4 ==> x=2".asSequent, andL(Symbol("L")) * 2)
      .subgoals
      .loneElement shouldBe "x=2, y=3, z=4 ==> x=2".asSequent
    val ex =
      the[IllFormedTacticApplicationException] thrownBy proveBy("x=2&y=3&z=4 ==> x=2".asSequent, andL(Symbol("L")) * 3)
  ...
  "+ combinator" should "saturate with at least 1 repetition" in withMathematica { _ =>
    val result =
      proveBy("x=2&y=3&(z=4|z=5) ==> x=2".asSequent, andL(Symbol("Llast")) & SaturateTactic(andL(Symbol("Llast"))))
  ...
  it should "fail when not at least 1 repetition is possible" in withMathematica { _ =>
    a[BelleThrowable] should be thrownBy proveBy(
      "z=4|z=5 ==> x=2".asSequent,
      andL(Symbol("Llast")) & SaturateTactic(andL(Symbol("Llast"))),
    )
  }

  it should "saturate with at least 1 repetition and try right branch in combination with either combinator" in withMathematica {
    _ =>
      proveBy(
        "x=2&y=3&(z=4|z=5) ==> x=2".asSequent,
        SaturateTactic((andL(Symbol("Llast")) & SaturateTactic(andL(Symbol("Llast")))) | close),
      ) shouldBe Symbol("proved")
  }
  ...
  "Label simplification" should "retain necessary leading labels" in withTactics {
    proveByS(
      ...
      { (labels: Option[List[BelleLabel]]) =>
        labels match {
          case Some(l) =>
            l should contain theSameElementsAs BelleLabel.fromString("0<=x//3<=y::0<=x//y<=4::x<=2//3<=y::x<=2//y<=4")
  ...
  "Branch Combinator" should "prove |- (1=1->1=1) & (2=2->2=2)" in withMathematica { _ =>
    val tactic = andR(SuccPos(0)) < (implyR(SuccPos(0)) & close, implyR(SuccPos(0)) & close)
    inside(theInterpreter.apply(
      tactic,
      BelleProvable.plain(ProvableSig.startPlainProof("(1=1->1=1) & (2=2->2=2)".asFormula)),
    )) { case BelleProvable(p, _) => p shouldBe Symbol("proved") }
  }
  ...
  it should "prove |- (1=1->1=1) & (2=2->2=2) with new < combinator" in withMathematica { _ =>
    ...
    val tactic = andR(SuccPos(0)) & Idioms.<(implyR(SuccPos(0)) & close, implyR(SuccPos(0)) & close)
    inside(theInterpreter.apply(
      tactic,
      BelleProvable.plain(ProvableSig.startPlainProof("(1=1->1=1) & (2=2->2=2)".asFormula)),
    )) { case BelleProvable(p, _) => p shouldBe Symbol("proved") }
  ...
  it should "handle cases were subgoals are added." in withMathematica { _ =>
    val tactic = andR(SuccPos(0)) < (andR(SuccPos(0)), implyR(SuccPos(0)) & close)
  ...
  it should "fail whenever there's a non-partial tactic that doesn't close its goal." in withMathematica { _ =>
    val tactic =
      andR(SuccPos(0)) < (andR(SuccPos(0)) & DebuggingTactics.done, implyR(SuccPos(0)) & close & DebuggingTactics.done)
  ...
  it should "handle cases were subgoals are added -- switch order" in withMathematica { _ =>
    val tactic = andR(SuccPos(0)) < (implyR(SuccPos(0)) & close, andR(SuccPos(0)))
  ...
  it should "work with loop labels" in withMathematica { _ =>
    ...
    inside(theInterpreter.apply(tactic, v)) { case BelleProvable(p, Some(l)) =>
      ...
      l should contain theSameElementsInOrderAs (BelleLabels.initCase :: BelleLabels.useCase :: BelleLabels
        .indStep :: Nil)
    }
  }
  ...
  it should "not screw up empty labels" in withMathematica { _ =>
    proveBy(
      "((P_() <-> F_()) & (F_() -> (Q_() <-> G_()))) ->(P_() & Q_() <-> F_() & G_())".asFormula,
      prop,
    ) shouldBe Symbol("proved")
  }
  ...
  it should "finish working branches before failing" in withMathematica { _ =>
    ...
    a[BelleThrowable] should be thrownBy proveBy(
      ...
      implyR(1) & andR(1) < (
        ...
        andR(1) < (id & logDone(2), testb(1) & prop & logDone(3)),
      ),
    )
  ...
  it should "finish working branches before failing with combined error message" in withMathematica { _ =>
    ...
    the[BelleThrowable] thrownBy proveBy(
      ...
      implyR(1) & andR(1) < (
        id & /* not reached */ logDone(1),
        andR(1) < (id & /* not reached */ logDone(2), testb(1) & prop & logDone(3)),
      ),
    ) should have message
    ...
    proveBy(
      ...
      implyR(1) &
        HybridProgramCalculus.loop("x>=0".asFormula)(1) < (
          QE,
          QE,
          choiceb(1) & andR(1) < (assignb(1) & label("1"), assignb(1)),
        ),
      (labels: Option[List[BelleLabel]]) => {
  ...
  "Case combinator" should "match tactics with labels" in withMathematica { _ =>
    ...
    for (t <- ts) {
      inside(theInterpreter.apply(t, v)) { case BelleProvable(p, _) =>
        p.subgoals should contain theSameElementsAs List("x>2 ==> x>1".asSequent, "x>1 ==> x+1>1".asSequent)
      }
    }
  }
    ...
    val ts = List(BelleLabels.initCase -> nil, BelleLabels.useCase -> id, BelleLabels.indStep -> assignb(1))
      ...
      .map(t =>
        implyR(1) & andR(1) /* creates labels that become parents of loop labels */ < (
          HybridProgramCalculus.loop("x>1".asFormula)(1) & CaseTactic(t),
          id,
        )
      )
  ...
  it should "match tactics with label suffixes (2)" in withMathematica { _ =>
    ...
    for (t <- ts) {
      inside(theInterpreter.apply(t, v)) { case BelleProvable(p, labels) =>
  ...
  it should "choose the first applicable unification when there are many options -- flipped order" in withMathematica {
    _ =>
      val pattern1 = SequentType("==> p() -> p()".asSequent)
      val pattern2 = SequentType("==> p() -> q()".asSequent)
      val e = USubstPatternTactic(Seq(
  ...
  "ChooseSome" should "record in the database" ignore withDatabase { db =>
    withMathematica { _ =>
...
//  }
  ...
  it should "print nice errors and provide a stack trace" ignore withMathematica { _ =>
    ...
    def split = andR(1) < (repTwo, skip)

    val thrown = intercept[Throwable] {
      theInterpreter.apply(
        Idioms.nil
          & Idioms.nil
          & split
          & Idioms.nil
    ...
    s.filter(_.getFileName == "SequentialInterpreterTests.scala").slice(0, 11).map(_.getLineNumber) shouldBe Array(
      280, 279, 278, 271, 271, 269, 266, 266, 263, 262, 276,
    )
  }
  ...
  it should "stop saturation when proof is closed" in withMathematica { _ =>
    ...
    ) match { case BelleProvable(pr, _) => pr shouldBe Symbol("proved") }
  ...
  it should "replay when expanded on branches" in withMathematica { _ =>
    ...
    proveBy(
      Sequent(IndexedSeq.empty, IndexedSeq(entry.model.asInstanceOf[Formula])),
      expandAllDefs(entry.defs.substs) &
        DebuggingTactics.assert(_ == "==> x>0 -> [y:=x; ++ ?y>0;]y>0".asSequent, "Unexpected expand result") &
        implyR(1) & choiceb(1) & andR(1) < (assignb(1) & id, testb(1) & implyR(1) & id),
      entry.defs,
    ) shouldBe Symbol("proved")
  ...
  it should "support introducing variables for function symbols of closed provables" in withMathematica { _ =>
    ...
    proveBy(
      ...
      implyR(1) & cut("r>0".asFormula) < (
        dC("x^2+y^2=r".asFormula)(1) < (cut("r>1".asFormula), dIRule(1) < (QE, unfoldProgramNormalize & QE)),
        ODE(1),
      ),
    ...
    proveBy(
      ...
      let(
        "r()".asTerm,
        "r".asTerm,
        by(proveBy("x^2+y^2=r() -> [{x'=r()*y,y'=-r()*x}]x^2+y^2=r()".asFormula, implyR(1) & ODE(1))),
      ),
    ) shouldBe Symbol("proved")
  }
  ...
  it should "detect expanded definitions when stitching together provables" in withMathematica { _ =>
    ...
    proveByS("p(x) ==> [{x'=g(y)}]p(x)".asSequent, dC("x<=old(x)".asFormula)(1) < (skip, dIClose(1)), defs)
      .subgoals
      .loneElement shouldBe "p(x_0), x_0=x ==> [{x'=-y^2 & true&x<=x_0}]p(x)".asSequent

    proveByS("p(x) ==> [{x'=g(y)}]p(x)".asSequent, diffInvariant("x<=old(x)".asFormula)(1), defs)
      .subgoals
      .loneElement shouldBe "p(x_0), x_0=x ==> [{x'=-y^2 & true&x<=x_0}]p(x)".asSequent
  }
  ...
  it should "detect expanded definitions when stitching together provables after QE" in withMathematica { _ =>
    ...
    proveByS("==> x^2<=g(x) & x^2/g(x)>=0".asSequent, andR(1) < (skip, QE), defs)
      .subgoals
  ...
  it should "extend parent provable" in withMathematica { _ =>
    ...
    proveByS(
      "min(v,0)>=0, x_0>=0, x=x_0 ==> [{x'=v}]g(x)>=g(x_0)".asSequent,
      EqualityTactics.minmax(-1) & dIRule(1) < (QE, TactixLibrary.unfoldProgramNormalize & QE),
      defs,
    ) shouldBe Symbol("proved")
  }
  ...
  it should "extend parent provable on branch (1)" in withMathematica { _ =>
    ...
    proveByS(
      "min(v,0)>=0, x_0>=0, x=x_0 ==> g(x)>=0 & [{x'=v}]g(x)>=g(x_0)".asSequent,
      andR(1) < (skip, EqualityTactics.minmax(-1) & dIRule(1) < (QE, TactixLibrary.unfoldProgramNormalize & QE)),
      defs,
  ...
  it should "extend parent provable on branch after all definitions were expanded" in withMathematica { _ =>
    ...
    proveByS(
      ...
      andR(1) < (
        skip,
        expandAllDefs(defs.substs) & EqualityTactics.minmax(-1) & dIRule(1) < (
          QE,
          TactixLibrary.unfoldProgramNormalize & QE,
        ),
      ),
      defs,
    ).subgoals.loneElement shouldBe "min(v,0)>=0, x_0>=0, x=x_0 ==> 2*x>=0".asSequent
  ...
  it should "FEATURE_REQUEST: apply constified conclusion to non-constified open goal" taggedAs TodoTest in withMathematica {
    _ =>
      ...
      proveBy("p(x) ==> [{x'=g(y)}]p(x)".asSequent, dC("x<=old(x)".asFormula)(1) < (skip, dIClose(1)))
        .subgoals
        .loneElement shouldBe "p(x_0), x_0=x ==> [{x'=-y^2 & true&x<=x_0}]p(x)".asSequent
  }
  ...
  "Using" should "keep mentioned and abbreviate unmentioned formulas" in withMathematica { _ =>
    ...
    proveBy(
      "x>y, y>=0, x>0 ==> y>=0, x>=0".asSequent,
      Using(
        "x>0".asFormula :: "x>=0".asFormula :: Nil,
        check("p__0(x,y), p__1(y), x>0 ==> q__0(y), x>=0".asSequent),
      ), // restricted inside
  ...
  it should "abbreviate only temporarily" in withMathematica { _ =>
    proveBy(
      "x>0 | x>y, y=0 | y>0 ==> x>0".asSequent,
      Using("x>0 | x>y".asFormula :: Nil, SaturateTactic(orL(Symbol("L")))),
    ).subgoals shouldBe List("x>0, y=0 | y>0 ==> x>0".asSequent, "x>y, y=0 | y>0 ==> x>0".asSequent)
  }
  ...
  it should "work with QE" in withQE { _ =>
    proveBy(
      "x>0, y=0 | y>0 ==> x^2>0".asSequent,
      Using("x>0".asFormula :: "x^2>0".asFormula :: Nil, QE),
    ) shouldBe Symbol("proved")
  }

  it should "work with loops" in withTactics {
    proveBy(
      "x>0, y=0 | y>0, z=0 | z=1 ==> [{x:=x+z;}*]x>0".asSequent,
      Using("z=0 | z=1".asFormula :: Nil, SaturateTactic(orL(Symbol("L")))),
  ...
  it should "work with ODEs" in withTactics {
    proveBy(
      "x>0, y=0 | y>0, z=0 | z=1 ==> [{x'=z*x}]x>0".asSequent,
      Using("z=0 | z=1".asFormula :: Nil, SaturateTactic(orL(Symbol("L")))),
    ).subgoals shouldBe List(
  ...
  it should "keep track of original values" in withTactics {
    ...
    proveBy(
      "x=2, [{x'=x}]x>=0, c()=3 ==> [x:=c();]x=2".asSequent,
      Using("c()=3".asFormula :: "[x:=c();]x=2".asFormula :: Nil, DLBySubst.assignEquality(1)),
    ).subgoals.loneElement shouldBe "x_0=2, c()=3, x_1=c(), x=x_0, [{x'=x}]x>=0 ==> x_1=2".asSequent
    proveBy(
      "x=1 | x=2, c()=3 | c()=4 ==> [x:=c();]x<=2, [{x'=x}]x>=0".asSequent,
      Using(
        "c()=3 | c()=4".asFormula :: "[x:=c();]x<=2".asFormula :: Nil,
  ...
  it should "be allowed but optional to mention formulas of position tactics" in withQE { _ =>
    ...
    proveBy("x=2, c()=3 ==> [x:=c();]x=2".asSequent, Using("c()=3".asFormula :: Nil, DLBySubst.assignEquality(1)))
      .subgoals
      .loneElement shouldBe "x_0=2, c()=3, x=c() ==> x=2".asSequent
    proveBy(
      "x=2, c()=3 ==> [{x'=c()}]x>=2".asSequent,
  ...
  it should "be allowed but optional to mention formulas of searchy position locators" in withTactics {
    proveBy(
      "x=2, c()=3 ==> [x:=4;]x=4, [x:=c();]x=2".asSequent,
      Using(
        "c()=3".asFormula :: "[x:=c();]x=2".asFormula :: Nil,
        DLBySubst.assignEquality(Symbol("R"), "[x:=c();]x=2".asFormula),
      ),
    ...
    proveBy(
      ...
      Using(
        "c()=3".asFormula :: "[x:=c();]x=2".asFormula :: Nil,
        DLBySubst.assignEquality(Symbol("Rlike"), "[x:=c();]x=c()".asFormula),
      ),
    ).
        // @todo undesired renaming
      subgoals
      .loneElement shouldBe "x_0=2, c()=3, x=4 ==> [x_0:=c();]x_0=2, x=4".asSequent
  }

  it should "be allowed but optional to mention formulas of two-position tactics" in withTactics {
    proveBy("x=2, c()=3 ==> [x:=4;]x=4, [x:=c();]x=2".asSequent, Using(Nil, SequentCalculus.exchangeL(-1, -2)))
      .subgoals
      .loneElement shouldBe "c()=3, x=2 ==> [x:=4;]x=4, [x:=c();]x=2".asSequent
    proveBy("x=2, c()=3 ==> [x:=4;]x=4, [x:=c();]x=2".asSequent, Using(Nil, SequentCalculus.exchangeR(1, 2)))
      .subgoals
      .loneElement shouldBe "x=2, c()=3 ==> [x:=c();]x=2, [x:=4;]x=4".asSequent
  }
  ...
  it should "create useful labels" in withTactics {
    val s =
      """init(rv)&(w=wUp()|w=wLo()), C(w,dhf,dhd,r,rv,h)
    ...
    val t =
      "prop using \"(w*dhf>=0->(case1(w,dhd,r,rv)->bounds1(w,dhd,r,rv,h))&(case2(w,dhd,r,rv)->bounds2(w,dhd,h))&(case3(w,dhf,dhd,r,rv)->bounds3(w,dhd,r,rv,h))&(case4(w,dhf,dhd,r,rv)->bounds4(w,dhf,dhd,r,rv,h)))&(w*dhf < 0->(case5(w,dhf,dhd,r,rv)->bounds5(w,dhd,r,rv,h))&(case6(w,dhf,dhd,r,rv)->bounds6(w,dhf,dhd,r,rv,h)))\""
        .asTactic
    proveByS(
      s,
      t,
      { (labels: Option[List[BelleLabel]]) =>
        labels match {
          case Some(l) => l should contain theSameElementsAs
              ...
              BelleLabel.fromString(
                "w*dhf>=0->(case1(w,dhd,r,rv)->bounds1(w,dhd,r,rv,h))&(case2(w,dhd,r,rv)->bounds2(w,dhd,h))&(case3(w,dhf,dhd,r,rv)->bounds3(w,dhd,r,rv,h))&(case4(w,dhf,dhd,r,rv)->bounds4(w,dhf,dhd,r,rv,h))//(case2(w,dhd,r,rv)->bounds2(w,dhd,h))&(case3(w,dhf,dhd,r,rv)->bounds3(w,dhd,r,rv,h))&(case4(w,dhf,dhd,r,rv)->bounds4(w,dhf,dhd,r,rv,h))//case2(w,dhd,r,rv)->bounds2(w,dhd,h)"
              ) ++
              BelleLabel.fromString(
                "w*dhf>=0->(case1(w,dhd,r,rv)->bounds1(w,dhd,r,rv,h))&(case2(w,dhd,r,rv)->bounds2(w,dhd,h))&(case3(w,dhf,dhd,r,rv)->bounds3(w,dhd,r,rv,h))&(case4(w,dhf,dhd,r,rv)->bounds4(w,dhf,dhd,r,rv,h))//(case2(w,dhd,r,rv)->bounds2(w,dhd,h))&(case3(w,dhf,dhd,r,rv)->bounds3(w,dhd,r,rv,h))&(case4(w,dhf,dhd,r,rv)->bounds4(w,dhf,dhd,r,rv,h))//(case3(w,dhf,dhd,r,rv)->bounds3(w,dhd,r,rv,h))&(case4(w,dhf,dhd,r,rv)->bounds4(w,dhf,dhd,r,rv,h))//case3(w,dhf,dhd,r,rv)->bounds3(w,dhd,r,rv,h)"
              ) ++
              BelleLabel.fromString(
                "w*dhf>=0->(case1(w,dhd,r,rv)->bounds1(w,dhd,r,rv,h))&(case2(w,dhd,r,rv)->bounds2(w,dhd,h))&(case3(w,dhf,dhd,r,rv)->bounds3(w,dhd,r,rv,h))&(case4(w,dhf,dhd,r,rv)->bounds4(w,dhf,dhd,r,rv,h))//(case2(w,dhd,r,rv)->bounds2(w,dhd,h))&(case3(w,dhf,dhd,r,rv)->bounds3(w,dhd,r,rv,h))&(case4(w,dhf,dhd,r,rv)->bounds4(w,dhf,dhd,r,rv,h))//(case3(w,dhf,dhd,r,rv)->bounds3(w,dhd,r,rv,h))&(case4(w,dhf,dhd,r,rv)->bounds4(w,dhf,dhd,r,rv,h))//case4(w,dhf,dhd,r,rv)->bounds4(w,dhf,dhd,r,rv,h)"
              ) ++
              BelleLabel.fromString(
                "w*dhf < 0->(case5(w,dhf,dhd,r,rv)->bounds5(w,dhd,r,rv,h))&(case6(w,dhf,dhd,r,rv)->bounds6(w,dhf,dhd,r,rv,h))//case5(w,dhf,dhd,r,rv)->bounds5(w,dhd,r,rv,h)"
              ) ++
              BelleLabel.fromString(
  ...
  it should "match correctly in index search" in withTactics {
    ...
    proveBy(s, Using(List("x>=0".asFormula), SimplifierV3.simplify(1)))
      .subgoals
      .loneElement shouldBe "x>=0, p(x,y), q(x,y,z) ==> p(x,y) & q(x)".asSequent
  }

  "Sequential interpreter" should "execute dI proof steps instead of wasting time traversing tactics" in withMathematica {
    _ =>
