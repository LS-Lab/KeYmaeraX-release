...
class SimplifierV2Tests extends TacticTestBase {
  ...
  it should "simplify under quantifiers and modalities" in withQE { _ =>
    val fml =
      ("(\\forall x (P() & Q() & x = 0 & P() & Q())) & (\\exists y (P() & Q() & Q() & y = 5)) | " +
    ...
    result.subgoals.head.succ should contain only
      "\\forall x (P()&Q()&x=0)&\\exists y (P()&Q()&y=5)|<{x_'=v&q(x_)}>P()".asFormula
  }
  ...
  it should "simplify program auxiliaries in loop" in withQE { _ =>
    // ETCS essentials:
    val fml =
      ("[{SB:=v^2/(2*b)+(A/b+1)*(A/2*ep^2+ep*v);{?m-z<=SB;a:=-b;++?m-z>=SB;a:=A;}" +
        "t:=0;{z'=v,v'=a,t'=1&v>=0&t<=ep}}*]z<=m").asFormula
    val (res, pr) = rewriteLoopAux(fml, List(Variable("SB")))
    pr shouldBe Symbol("proved")
    res shouldBe
      "[{{?m-z<=v^2/(2*b)+(A/b+1)*(A/2*ep^2+ep*v);a:=-b;++?m-z>=v^2/(2*b)+(A/b+1)*(A/2*ep^2+ep*v);a:=A;}t:=0;{z'=v,v'=a,t'=1&v>=0&t<=ep}}*]z<=m"
        .asFormula
  }
  ...
  it should "simplify program auxiliaries with precondition" in withQE { _ =>
    // ETCS essentials:
    val fml =
      ("v^2<=2*b*(m-z)&b>0&A>=0->[{SB:=v^2/(2*b)+(A/b+1)*(A/2*ep^2+ep*v);{?m-z<=SB;a:=-b;++?m-z>=SB;a:=A;}" +
        "t:=0;{z'=v,v'=a,t'=1&v>=0&t<=ep}}*]z<=m").asFormula
    val (res, pr) = rewriteLoopAux(fml, List(Variable("SB")))
    pr shouldBe Symbol("proved")
    res shouldBe
      "v^2<=2*b*(m-z)&b>0&A>=0->[{{?m-z<=v^2/(2*b)+(A/b+1)*(A/2*ep^2+ep*v);a:=-b;++?m-z>=v^2/(2*b)+(A/b+1)*(A/2*ep^2+ep*v);a:=A;}t:=0;{z'=v,v'=a,t'=1&v>=0&t<=ep}}*]z<=m"
        .asFormula
  }

  it should "simplify program auxiliaries with multiple preconditions" in withQE { _ =>
    val fml =
      ("a<=b & b<=c -> v^2<=2*b*(m-z)&b>0&A>=0->[{SB:=v^2/(2*b)+(A/b+1)*(A/2*ep^2+ep*v);{?m-z<=SB;a:=-b;++?m-z>=SB;a:=A;}" +
        "t:=0;{z'=v,v'=a,t'=1&v>=0&t<=ep}}*]z<=m").asFormula
    val (res, pr) = rewriteLoopAux(fml, List(Variable("SB")))
    pr shouldBe Symbol("proved")
    res shouldBe
      "a<=b&b<=c->v^2<=2*b*(m-z)&b>0&A>=0->[{{?m-z<=v^2/(2*b)+(A/b+1)*(A/2*ep^2+ep*v);a:=-b;++?m-z>=v^2/(2*b)+(A/b+1)*(A/2*ep^2+ep*v);a:=A;}t:=0;{z'=v,v'=a,t'=1&v>=0&t<=ep}}*]z<=m"
        .asFormula
  }
  ...
  it should "leave open bad rewrites" in withQE { _ =>
    ...
    res shouldBe
      "v^2<=2*b*(m-z)&b>0&A>=0->[{{?m-z<=v^2/(2*b)+(A/b+1)*(A/2*ep^2+ep*v);++?m-z>=v^2/(2*b)+(A/b+1)*(A/2*ep^2+ep*v);}t:=0;{z'=v,v'=a,t'=1&v>=0&t<=ep}}*]z<=m"
        .asFormula
  }
  ...
  it should "work with cases tactic" in withQE { _ =>
    ...
    val result =
      proveBy(fml, Idioms.cases((Case("x>0".asFormula), Idioms.ident), (Case("0>=x".asFormula), Idioms.ident)))
