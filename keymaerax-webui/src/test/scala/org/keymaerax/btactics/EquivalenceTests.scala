...
class EquivalenceTests extends TacticTestBase {
  "A<->B & A |- B" should "prove by rewriting A to B using equivlaence and then closeId." in withTactics {
    ...
    val equivPos = -1
    val t = SequentCalculus.implyR(1) & SequentCalculus.andL(-1) /*end setup*/ &
      SequentCalculus.equivL(equivPos) <
      (
        SequentCalculus.andL(equivPos) & SequentCalculus.id,
        SequentCalculus.andL(equivPos) & SequentCalculus.notL(Symbol("Llast")).*(2) & SequentCalculus.id,
      )
  ...
  "Instantiated Equiv Rewriting" should "prove by a built-in tactic" in withTactics {
    ...
    val t = SequentCalculus.implyR(1) & SequentCalculus.andL(-1) &
  ...
  it should "prove by a built-in tactic succ" in withTactics {
    ...
    val t = SequentCalculus.implyR(1) & PropositionalTactics.equivRewriting(SeqPos(-1), SeqPos(1))
