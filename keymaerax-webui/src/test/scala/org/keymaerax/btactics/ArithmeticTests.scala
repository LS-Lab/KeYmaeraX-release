...
import org.keymaerax.bellerophon.BelleThrowable
...
import org.keymaerax.tools.ext.{CounterExampleTool, Goal, QETacticTool}
import org.keymaerax.tools.qe.KeYmaeraToMathematica
import org.keymaerax.tools.{ConversionException, Tool}
...
class ArithmeticTests extends TacticTestBase {
  ...
  it should "not support differential symbols in conversion" in {
    the[ConversionException] thrownBy KeYmaeraToMathematica("5=5 | x' = 1".asFormula) should have message
      "Name conversion of differential symbols not allowed: x'"
  }
  ...
  it should "not prove differential symbols by some hidden assumption in Z3" in withZ3 { _ =>
    the[BelleThrowable] thrownBy proveBy("x>=y -> x' >= y'".asFormula, TactixLibrary.QE) should have message
  ...
  it should "work with functions" in withQE { _ =>
    ...
    proveBy("ep>0 & t>=ep & abs(x_0-xo_0)*ep>v -> abs(x_0-xo_0)*t>v".asFormula, TactixLibrary.QE) shouldBe
      Symbol("proved")
  }
  ...
  "counterExample" should "not choke on differential symbols" in withMathematica { tool =>
    ...
    inside(tool.findCounterExample("v'>=0".asFormula)) { case Some(m) => m.loneElement._1 shouldBe "v'".asVariable }
  }

  it should "not choke on function symbols" in withMathematica { tool =>
    inside(tool.findCounterExample("v>=A()".asFormula)) { case Some(m) =>
      m.size shouldBe 2
      ...
      (m("v".asVariable), m("A()".asFunction)) match {
        ...
        case (v, a) =>
          fail("Unexpected counterexample result, expected terms but got: " + v.prettyString + ", " + a.prettyString)
      }
    }
  }

  it should "avoid name clashes between variables and parameterless functions" in withMathematica { tool =>
    inside(tool.findCounterExample("a>=a()".asFormula)) { case Some(m) =>
      m.size shouldBe 2
    ...
    inside(tool.findCounterExample("a=1&a()=2 -> a=a()".asFormula)) { case Some(m) =>
      m.size shouldBe 2
      m.keySet should contain theSameElementsAs List("a".asVariable, "a()".asFunction)
      (m("a".asVariable), m("a()".asFunction)) match {
  ...
  it should "support interpreted function symbols" in withMathematica { tool =>
    inside(tool.findCounterExample("abs(x) < 0".asFormula)) { case Some(m) =>
      m.size shouldBe 1
