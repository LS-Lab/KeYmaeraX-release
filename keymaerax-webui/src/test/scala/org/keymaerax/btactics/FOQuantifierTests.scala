...
import org.keymaerax.bellerophon.{BelleThrowable, UnexpandedDefinitionsFailure}
import org.keymaerax.btactics.FOQuantifierTactics._
...
class FOQuantifierTests extends TacticTestBase {
  ...
  it should "instantiate the first variable if none is specified" in withTactics {
    val result =
      proveBy(Sequent(IndexedSeq("\\forall x x>0".asFormula), IndexedSeq()), allInstantiate(None, Some("z".asTerm))(-1))
    result.subgoals.loneElement shouldBe "z>0 ==> ".asSequent
  }

  it should "instantiate the first variable with itself if none is specified" in withTactics {
    val result = proveBy(Sequent(IndexedSeq("\\forall x x>0".asFormula), IndexedSeq()), allInstantiate(None, None)(-1))
    result.subgoals.loneElement shouldBe "x>0 ==> ".asSequent
  }
  ...
  it should "diffWeaken simple" in withTactics {
    val result = proveBy("[{x'=5&x<7}]x<7".asFormula, DifferentialEquationCalculus.dW(1) & prop)
    result shouldBe Symbol("proved")
  }

  it should "diffWeaken ouch" in withMathematica { _ =>
    val result = proveBy(
      "[{x'=1}][{x'=2&x>0}]x>0".asFormula,
      DifferentialEquationCalculus.dW(1) & DifferentialEquationCalculus.dW(1) & prop,
    )
    result shouldBe Symbol("proved")
  }
  ...
  it should "diffWeaken before loopy" in withMathematica { _ =>
    val result = proveBy(
      "[{x'=1&x>0}][{x:=2;}*]x>0".asFormula,
      DifferentialEquationCalculus.dW(1) & HybridProgramCalculus.loop("x>0".asFormula)(1) & master(),
    )
    result shouldBe Symbol("proved")
  }

  it should "diffWeaken before semibound" in withMathematica { _ =>
    val result = proveBy("[{x'=1&x>0}][{x:=2;++y:=2;}]x>0".asFormula, DifferentialEquationCalculus.dW(1) & master())
    result shouldBe Symbol("proved")
  }
  ...
  it should "instantiate by position" in withTactics {
    val result = proveBy(
      Sequent(IndexedSeq("\\forall x x>0".asFormula, "y>0".asFormula), IndexedSeq()),
  ...
  it should "instantiate ODE" in withTactics {
    val result = proveBy(
      Sequent(IndexedSeq("\\forall t_ [{x'=2,t_'=1&true}]x>b".asFormula), IndexedSeq()),
  ...
  it should "instantiate in the presence of self assignments" in withTactics {
    proveBy("\\forall x [x:=x;][{x'=1}]x>=2 ==>".asSequent, allL("y".asVariable)(-1)).subgoals.loneElement shouldBe
      "[y:=y;][{y'=1}]y>=2 ==>".asSequent
  }

  it should "instantiate in the presence of differentials" in withTactics {
    proveBy("\\forall x (f(x))'=g(x) ==>".asSequent, allL("y".asVariable)(-1)).subgoals.loneElement shouldBe
      "(f(x))'=g(y) ==>".asSequent
  }

  it should "instantiate differential symbols" in withTactics {
    proveBy("\\forall x' x'>=0 ==> y'>=0".asSequent, allL("y'".asTerm)(-1)).subgoals.loneElement shouldBe
      "y'>=0 ==> y'>=0".asSequent
  }
  ...
  it should "instantiate differential symbol assignments" in withTactics {
    proveBy("\\forall y' [y':=y'+1;]y'>0 ==>".asSequent, allInstantiate(Some("y'".asVariable), Some("z+1".asTerm))(-1))
      .subgoals
      .loneElement shouldBe "y'=z+1, [y':=y'+1;]y'>0 ==> ".asSequent
  }

  it should "FEATURE_REQUEST: instantiate differential symbol assignments when renaming is required" taggedAs
    TodoTest in withTactics {
      proveBy(
        "y'=4, \\forall y' [y':=y'+1;]y'>0 ==>".asSequent,
        allInstantiate(Some("y'".asVariable), Some("z+1".asTerm))(-2),
  ...
  "all instantiate inverse" should "introduce a universal quantifier for a variable" in withTactics {
    proveBy("==> x=4".asSequent, allInstantiateInverse("x".asTerm -> "y".asVariable)(1)).subgoals.loneElement shouldBe
      "==> \\forall y y=4".asSequent
  }

  it should "weaken any term to a universal quantifier" in withTactics {
    proveBy("==> x=4".asSequent, allInstantiateInverse("4".asTerm -> "y".asVariable)(1)).subgoals.loneElement shouldBe
      "==> \\forall y x=y".asSequent
  }
  ...
  it should "instantiate variables bound in an ODE" in withTactics {
    proveBy(
      ...
      existsInstantiate(None, None)(1),
    ).subgoals.loneElement shouldBe "==> [{x'=2,y'=0*y+1&true}]x>0".asSequent

    proveBy(
      Sequent(IndexedSeq(), IndexedSeq("\\exists y [{x'=2,y'=0*y+1&true}]x>0".asFormula)),
      existsInstantiate(None, Some("z".asTerm))(1),
    ).subgoals.loneElement shouldBe "==> [{x'=2,z'=0*z+1&true}]x>0".asSequent
  ...
  it should "instantiate ODE" in withTactics {
    val result =
      proveBy("\\exists t_ (t_=0&![{x'=2,t_'=1&true}]x>b)".asFormula, existsInstantiate(None, Some("0".asTerm))(1))
  ...
  it should "instantiate ODE correctly when there are multiple succedents" in withTactics {
    val result = proveBy("x = y ==> z != 1, \\exists z [{z'=1}]x=z , y!=1".asSequent, existsR(Number(1))(2))
  ...
  it should "instantiate in the presence of space exceptions" in withTactics {
    proveBy("z=1 ==> \\exists y [y:=f(|y|)^2;]y>=0".asSequent, existsR("z+1".asTerm)(1)).subgoals.loneElement shouldBe
      "z=1, y=z+1  ==> [y:=f(|y|)^2;]y>=0".asSequent
  }
  ...
  it should "instantiate in the presence of space exceptions in ODEs" in withTactics {
    proveBy("z=1 ==> \\exists y [{y'=f(|y|)^2, z' = g(|y|), a'=h(||)}]y>=0".asSequent, existsR("z+1".asTerm)(1))
      .subgoals
      .loneElement shouldBe "z=1, y=z+1  ==>  [{y'=f(|y|)^2,z'=g(|y|),a'=h(||)&true}]y>=0".asSequent
  }

  it should "instantiate in the presence of program constants" in withTactics {
    proveBy("==> \\exists y [?y>=0;prg;]y>=0".asSequent, existsR("z".asTerm)(1)).subgoals.loneElement shouldBe
      "y=z ==> [?y>=0;prg;]y>=0".asSequent
  }

  it should "instantiate in the presence of program constants in loops" in withTactics {
    proveBy("==> \\exists y [{?y>=0;prg;}*]y>=0".asSequent, existsR("c()".asTerm)(1)).subgoals.loneElement shouldBe
      "y=c() ==> [{?y>=0;prg;}*]y>=0".asSequent
  }

  it should "FEATURE_REQUEST: instantiate in the presence of space exceptions with self reference" taggedAs TodoTest in
    withTactics {
      val result =
        proveBy("z=1, ==> \\exists y [{y'=f(|y|)^2, z' = g(|y|), a'=h(||)}]y>=0".asSequent, existsR("z+y".asTerm)(1))
  ...
  "restricted exists generalize" should "only generalize the specified occurrences of t" in withTactics {
    val result = proveBy(
      Sequent(IndexedSeq("a+b=a+b".asFormula), IndexedSeq()),
  ...
  it should "generalize with other formulas around" in withTactics {
    proveBy("x=1, a+b=a+b, y=2 ==> c=3".asSequent, existsGeneralize(Variable("z"), PosInExpr(0 :: Nil) :: Nil)(-2))
      .subgoals
  ...
  it should "generalize when quantified variable already exists" in withTactics {
    proveBy("z=1, a+b=a+b, y=2 ==> z>=0".asSequent, existsGeneralize(Variable("z"), PosInExpr(0 :: Nil) :: Nil)(-2))
      .subgoals
  ...
  it should "reject positions that refer to different terms" in withTactics {
    a[BelleThrowable] should be thrownBy proveBy(
      Sequent(IndexedSeq("a+b=a+c".asFormula), IndexedSeq()),
      existsGeneralize(Variable("z"), PosInExpr(0 :: Nil) :: PosInExpr(1 :: Nil) :: Nil)(-1),
    )
  }
  ...
  it should "introduce a new existential quantifier in context" in withTactics {
    val result = proveBy(
      "a=2 -> [x:=3;]\\forall x x^2 >= -f()^2 ==>".asSequent,
  ...
  it should "not auto-generate names that overlap with bound variables and cause clashes" in withTactics {
    proveBy("B()<=2 -> \\forall B B()<=2 ==>".asSequent, existsGen(None, "B()".asTerm)(-1))
      .subgoals
      .loneElement shouldBe "\\exists B_0 (B_0<=2 -> \\forall B B_0<=2) ==>".asSequent
  }

  "all generalize" should "introduce a new universal quantifier" in withTactics {
    val result = proveBy(
      "\\forall x x^2 >= -f()^2".asFormula,
  ...
  it should "introduce a new universal quantifier in context" in withTactics {
    val result = proveBy(
      "a=2 -> [x:=3;]\\forall x x^2 >= -f()^2".asFormula,
  ...
  it should "introduce a new universal quantifier in context before an ODE" in withTactics {
    val result =
      proveBy("a=2 -> [x:=3;][{x'=5}]x>0".asFormula, universalGen(Some(Variable("x")), Variable("x"))(1, 1 :: 1 :: Nil))
  ...
  it should "not auto-generate names that overlap with bound variables and cause clashes" in withTactics {
    proveBy("B()<=2 -> \\exists B B()<=2".asFormula, universalGen(None, "B()".asTerm)(1)).subgoals.loneElement shouldBe
      "==> \\forall B_0 (B_0<=2 -> \\exists B B_0<=2)".asSequent
  }
  ...
  it should "ignore bound variables in closure" in withTactics {
    ...
    result.subgoals.loneElement shouldBe
      "==> \\forall z \\forall f (\\forall x \\forall y (x>5 & f<2 & y+3>z))".asSequent
  }
  ...
  it should "use the provided order of symbols" in withTactics {
    val result =
      proveBy("a>0 & x>5 & y<2".asFormula, universalClosure(Variable("x") :: Variable("a") :: Variable("y") :: Nil)(1))
...
//  }
  ...
  it should "not rename variables bound somewhere else" in withTactics {
    val result = proveBy(
      Sequent(
        IndexedSeq("\\forall x x>0".asFormula, "\\exists x x=1".asFormula),
        IndexedSeq("\\forall x x>0".asFormula, "<x:=2;>x=2".asFormula),
      ),
      allSkolemize(1),
    )
  ...
  it should "not rename variables bound somewhere else if not top-level" in withTactics {
    val result = proveBy(
      Sequent(IndexedSeq("x>0 & \\forall x x>0".asFormula), IndexedSeq("\\forall x x>0".asFormula)),
      allSkolemize(1),
    )
  ...
  it should "not rename variables bound somewhere else if not top-level 2" in withTactics {
    val result = proveBy(
      Sequent(IndexedSeq("x>0 & (\\forall x x>0 | \\forall x x<0)".asFormula), IndexedSeq("\\forall x x>0".asFormula)),
      allSkolemize(1),
    )
  ...
  it should "not rename variables bound somewhere else if not top-level 3" in withTactics {
    val result = proveBy(
      Sequent(
        IndexedSeq("x>0 & (\\forall y \\forall x x>0 | \\forall y \\forall x x<0)".asFormula),
        IndexedSeq("\\forall x x>0".asFormula),
      ),
      allSkolemize(1),
    )
    result.subgoals.loneElement shouldBe
      "x_0>0 & (\\forall y \\forall x x>0 | \\forall y \\forall x x<0) ==> x>0".asSequent
  }

  it should "not rename variables bound somewhere else if not top-level 4" in withTactics {
    val result = proveBy(
      Sequent(
        IndexedSeq("x>0 & (\\forall x \\forall x x>0 | \\forall x \\forall x x<0)".asFormula),
        IndexedSeq("\\forall x x>0".asFormula),
      ),
      allSkolemize(1),
    )
    result.subgoals.loneElement shouldBe
      "x_0>0 & (\\forall x \\forall x x>0 | \\forall x \\forall x x<0) ==> x>0".asSequent
  }
  ...
  it should "give advice on program constant formula" in withTactics {
    the[UnexpandedDefinitionsFailure] thrownBy
      proveBy("x=0 ==> \\forall x (x=2 -> [ode;]x>=0)".asSequent, allSkolemize(1)) should have message
      "Skolemization not possible because bound variables x are not fresh in the sequent; also unable to rename, because formula x=2->[ode;]x>=0 contains unexpanded symbols ode;. Please hide all assumptions mentioning x, or expand definitions first."
  }
  ...
  it should "work in the presence of differential symbols and differentials" in withTactics {
    ...
    proveBy("x=0 ==> \\forall x (f(x))'=0".asSequent, allR(1)).subgoals.loneElement shouldBe
      "x_0=0 ==> (f(x))'=0".asSequent
  }
  ...
  it should "skolemize with boundrenaming when variable to skolemize is there already" in withTactics {
    val result =
      proveBy(Sequent(IndexedSeq("x>0".asFormula, "\\exists x x>0".asFormula), IndexedSeq()), existsSkolemize(-2))
    result.subgoals.loneElement shouldBe "x_0>0, x>0 ==> ".asSequent
  }

  it should "FEATURE_REQUEST: keep positions stable" taggedAs TodoTest in withTactics {
    proveBy("\\exists x (x=y&x>=0), y=2 ==>".asSequent, existsSkolemize(-1)).subgoals.loneElement shouldBe
      "x=y&x>=0, y=2 ==>".asSequent
  }

  "quantifier rules" should "not prove false \\forall x \\exists y p(x,y) -> \\exists y \\forall x p(x,y)" in
    withTactics {
      val result = proveBy(
        "\\forall x \\exists y p(x,y) -> \\exists y \\forall x p(x,y)".asFormula,
  ...
  it should "not prove false \\forall x \\exists y p(x,y) -> \\exists y \\forall x p(x,y) with insts" in withTactics {
    val result = proveBy(
      "\\forall x \\exists y p(x,y) -> \\exists y \\forall x p(x,y)".asFormula,
      implyR(1) & existsR(Variable("y"), Variable("y"))(1) & allL(Variable("x"), Variable("x"))(-1) & allR(1) &
        existsL(-1) & (close | skip),
    )
    result should not be Symbol("proved")
    result.isProved shouldBe false
  }
  ...
  it should "fail to instantiate non-existent quantified variables" in withTactics {
    val result = proveBy(
      " (\\forall x q(x)) -> \\exists y q(y) ".asFormula,
      implyR(1) & (allL(Variable("z"), Variable("y"))(-1) | nil) & (existsR(Variable("z"), Variable("x"))(1) | nil) &
        (close | skip),
    )
