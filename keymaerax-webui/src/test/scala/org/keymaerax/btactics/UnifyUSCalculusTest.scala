...
import org.keymaerax.btactics.EqualityTactics.eqL2R
import org.keymaerax.btactics.SequentCalculus._
import org.keymaerax.btactics.TactixLibrary.{prop, QE}
...
class UnifyUSCalculusTest extends TacticTestBase {
  ...
  it should "prove ([a;][b;]p(x)) -> [a;b;]p(x)" in withTactics {
    proveBy("([a;][b;]p(x)) -> [a;b;]p(x)".asFormula, useAt(Ax.composeb)(1, 1 :: Nil) & implyR(1) & id) shouldBe
      Symbol("proved")
  }
  ...
  it should "prove [a;b;]p(x) -> ([a;][b;]p(x))" in withTactics {
    proveBy("[a;b;]p(x) -> ([a;][b;]p(x))".asFormula, useAt(Ax.composeb)(1, 0 :: Nil) & implyR(1) & id) shouldBe
      Symbol("proved")
  }
  ...
  it should "prove (p()->q()) -> [?p();]q()" in withTactics {
    proveBy("(p()->q()) -> [?p();]q()".asFormula, useAt(Ax.testb)(1, 1 :: Nil) & implyR(1) & id) shouldBe
      Symbol("proved")
  }
  ...
  it should "prove [?p();]q() -> (p()->q())" in withTactics {
    proveBy("[?p();]q() -> (p()->q())".asFormula, useAt(Ax.testb)(1, 0 :: Nil) & implyR(1) & id) shouldBe
      Symbol("proved")
  }
  ...
  it should "prove (1>=1) -> 1+0>=1" in withTactics {
    proveBy("(1>=1) -> 1+0>=1".asFormula, useAt(Ax.plusZero)(1, 1 :: 0 :: Nil) & implyR(1) & id) shouldBe
      Symbol("proved")
  }
  ...
  it should "prove x+1>0 -> [x:=x+1;]x>0" in withTactics {
    proveBy("x+1>0 -> [x:=x+1;]x>0".asFormula, useAt(Ax.assignbAxiom)(1, 1 :: Nil) & implyR(1) & id) shouldBe
      Symbol("proved")
  }
  ...
  it should "use an unproved conditional equivalence subproof" in withTactics {
    ...
    proveBy(mainProof, useAt(subProof, PosInExpr(List(1, 0)))(1, PosInExpr(List(0, 0)))).subgoals should
      contain theSameElementsInOrderAs List("==> P -> (Q<->R)".asSequent, "P ==> !R <-> !R".asSequent)
  }
  ...
  "Chase" should "prove [?p();?(p()->q());]p() by chase" in withTactics {
    proveBy(Sequent(IndexedSeq(), IndexedSeq("[?p();?(p()->q());]p()".asFormula)), chase(1) & prop) shouldBe
      Symbol("proved")
  }
  ...
  it should "chase inside quantifiers" in {
    proveBy("==> \\exists x [x:=x+1;]\\exists y [y:=y-1;]x>=y".asSequent, deepChase(1)).subgoals.loneElement shouldBe
      "==> \\exists x \\exists y x+1>=y-1".asSequent
    proveBy("\\forall x \\forall y [x:=x+y;]x>=0 ==>".asSequent, deepChase(-1)).subgoals.loneElement shouldBe
      "\\forall x \\forall y x+y>=0 ==>".asSequent
  }

  it should "chase more inside quantifiers" in {
    proveBy("\\forall m \\forall z (b()>0->[a:=A();][t:=0;{z'=v,v'=a,t'=1}]z>=0) ==>".asSequent, deepChase(-1))
      .subgoals
  ...
  private def shouldCMon(ctx: Context[Formula], basic: ProvableSig = basicImpl): Unit = {
    ...
    val done = CMon(ctx)(basic)
      ...
      Sequent(IndexedSeq(ctx(basic.conclusion.ante.head)), IndexedSeq(ctx(basic.conclusion.succ.head)))
  }
  ...
  "US" should "uniformly substitute in a single goal" in {
    proveBy("J(x) ==> J(x+1)".asSequent, USX("J(.) ~> .>=0".asSubstitutionPair :: Nil)).subgoals.loneElement shouldBe
      "x>=0 ==> x+1>=0".asSequent
  }

  it should "uniformly substitute in all goals" in {
    proveBy("J(x) ==> J(x+1) & J(x+2)".asSequent, andR(1) & USX("J(.) ~> .>=0".asSubstitutionPair :: Nil))
      .subgoals shouldBe List("x>=0 ==> x+1>=0".asSequent, "x>=0 ==> x+2>=0".asSequent)
  }
