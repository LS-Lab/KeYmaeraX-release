...
import org.keymaerax.tagobjects.SlowTest
...
class ODELivenessTests extends TacticTestBase {
  ...
  it should "get instantiated vdg" in withQE { _ =>
    ...
    vdginst._1.conclusion shouldBe
      "==> [{v'=v^2+z,z'=v*y+z,c{|v,z|}&q(|v,z|)}]v*v+z*z<=f_(|v,z|)->[{v'=v^2+z,z'=v*y+z,c{|v,z|}&q(|v,z|)}]p(|v,z|)->[{c{|v,z|}&q(|v,z|)}]p(|v,z|)"
        .asSequent

    vdginst._2 shouldBe Symbol("proved")
    vdginst._2.conclusion shouldBe
      "==> [{c{|v,z|}&q(|v,z|)}]p(|v,z|)->[{v'=v^2+z,z'=v*y+z,c{|v,z|}&q(|v,z|)}]p(|v,z|)".asSequent
  }
  ...
  it should "get instantiated ddg" in withQE { _ =>
    ...
    ddginst.conclusion shouldBe
      "==> [{v'=v^2+z,z'=v*y+z,c{|y_,z_,v,z|}&q(|y_,z_,v,z|)}]2*(v*(v^2+z)+z*(v*y+z))<=a_(|y_,z_,v,z|)*(v*v+z*z)+b_(|y_,z_,v,z|)->[{v'=v^2+z,z'=v*y+z,c{|y_,z_,v,z|}&q(|y_,z_,v,z|)}]p(|y_,z_,v,z|)->[{c{|y_,z_,v,z|}&q(|y_,z_,v,z|)}]p(|y_,z_,v,z|)"
        .asSequent
  }
  ...
  it should "vDG on left and right of sequent" in withQE { _ =>
    ...
    val pr = proveBy(
      ...
      vDG("z'=z^100*y,y'=z*y^2".asDifferentialProgram)(1) & vDG("a'=a^2+b^3+x+y+100".asDifferentialProgram)(-1) &
        // only affine ones can be added to diamonds on left and boxes on right
        vDG("z'=y*a*z+a+b+w,w'=w+z".asDifferentialProgram)(2) & vDG("b'=a*b+b*a+1,c'=b+c".asDifferentialProgram)(-2),
    )

    pr.subgoals.length shouldBe 1
    pr.subgoals(0) shouldBe
      "[{a'=a^2+b^3+x+y+100,z'=100&true}]z=1, <{b'=a*b+b*a+1,c'=b+c,a'=x+y+z&true}>a=1 ==> <{z'=z^100*y,y'=z*y^2,x'=1&true}>1+1=3, [{z'=y*a*z+a+b+w,w'=w+z,y'=2&true}]1+1=0"
        .asSequent
  }
  ...
  it should "dDG and bDG nonlinear ghosts on right of sequent" in withQE { _ =>
    ...
    val pr = proveBy(
      ...
      dDG("z'=z^2,w'=w*z".asDifferentialProgram, "x^2".asTerm, "y^2".asTerm)(2) <
        (skip, bDG("u'=u^3".asDifferentialProgram, "x".asTerm)(2)),
    )

    pr.subgoals.length shouldBe 3
    pr.subgoals(0) shouldBe " ==>  <{x'=1}>1+1=3, [{z'=z^2,w'=w*z,y'=2}]2*(z*z^2+w*(w*z))<=x^2*(z*z+w*w)+y^2".asSequent
    pr.subgoals(1) shouldBe " ==>  <{x'=1}>1+1=3, [{u'=u^3,z'=z^2,w'=w*z,y'=2}]u*u<=x".asSequent
    pr.subgoals(2) shouldBe " ==>  <{x'=1}>1+1=3, [{u'=u^3,z'=z^2,w'=w*z,y'=2}]1+1=0".asSequent
  }
  ...
  it should "derive global existence axiom for nested linear system" in withQE { _ =>
    ...
    res.get.conclusion.succ(0) shouldBe
      "<{gextimevar_'=1,y'=f()*x*x+z,z'=v^2+g()*y+z,x'=v,v'=a&true}>gextimevar_>p()".asFormula
  }
  ...
  it should "derive global existence for large and badly structured system" in withQE { _ =>
    val ode = "ee'=ff, g'=aa^2*g+gg,gg'=g+b^2*g,f'=a,dd'=ee,a'=b,aa'=bb,cc'=dd,e_'=f,bb'=cc,d'=e_,c'=d,b'=c,ff'=aa"
    ...
    res.get.isProved shouldBe true
    res.get.conclusion.succ(0) shouldBe
      "<{gextimevar_'=1,ee'=ff,g'=aa^2*g+gg,gg'=g+b^2*g,f'=a,dd'=ee,a'=b,aa'=bb,cc'=dd,e_'=f,bb'=cc,d'=e_,c'=d,b'=c,ff'=aa&true}>gextimevar_>p()"
        .asFormula
  }
  ...
  "compatibility" should "automatically match by compatibility" in withQE { _ =>
    ...
    val seq =
      "[{x'=1 & x < 6}]x>1,[{x'=1 & x < 4}]x>4, [{v'=2,x'=1,a'=b}]v<=5, [{v'=2,x'=1}]x+z<=5 ==> a > 0, [{x'=1,v'=2,a'=a^2+x+v^2 & x < 5}]1+1=2"
        .asSequent
    ...
    pr.subgoals.length shouldBe 1
    pr.subgoals(0) shouldBe
      "[{x'=1&x < 6}]x>1, [{x'=1&x < 4}]x>4, [{v'=2,x'=1,a'=b&true}]v<=5, [{v'=2,x'=1&true}]x+z<=5  ==>  a > 0, [{x'=1,v'=2,a'=a^2+x+v^2&(x < 5&x>1)&x+z<=5}]1+1=2"
        .asSequent
  }
  ...
  it should "introduce compatible cuts" in withQE { _ =>
    ...
    val pr =
      proveBy(seq, compatCut("1=1".asFormula)(1) < (compatCut("1=1".asFormula)(-2), compatCut("1=1".asFormula)(2)))
  ...
  it should "use hints in sequence" in withQE { _ =>
    val seq =
      "[{a'=b^2,b'=a^2}]a^2+b^2 <= 5, [{x'=y^2,y'=-x^2}] 2*(y*(-x^2)+x*y^2) <= c() ==> <{x' = y^2, y' = -x^2, a'=b^2, b'=a^2, t'=1}> t > 100"
        .asSequent
    ...
    pr.subgoals.length shouldBe 1
    pr.subgoals(0) shouldBe
      "[{a'=b^2,b'=a^2}]a^2+b^2 <= 5, [{x'=y^2,y'=-x^2}] 2*(y*(-x^2)+x*y^2) <= c() ==> <{t'=1&true}>t>100".asSequent
  }

  it should "automatically delete irrelevant ODEs and stabilize" in withQE { _ =>
    val seq =
      "d >0 , 1+1=2 ==> 1+1=3, <{a'=b+c+e_*5, x'=x+1, v'=2, e_' = a+e_, b'=c+f(),c'=d+e_() & c <= 5}> x+v<= 5, 2+2=1"
        .asSequent
  ...
  it should "continue using assms (format 1)" in withQE { _ =>
    val seq =
      "[{d'=d^2+f,f'=f,e_'=5&e_<=5}] d^2 <= e_*f ==> <{a'=b,b'=c,c'=d,d'=d^2+f,f'=f,e_'=5 & e_ <= 5}> e_<= 5".asSequent
  ...
  it should "continue using assms (format 2)" in withQE { _ =>
    val seq =
      "[{d'=d^2+f,f'=f,e_'=5&e_<=5}] 2*(d*(d^2+f)) <= 1*(d^2) ==> <{a'=b,b'=c,c'=d,d'=d^2+f,f'=f,e_'=5 & e_ <= 5}> e_<= 5"
        .asSequent
  ...
  "kdomd" should "refine ODE postcondition (with auto DC of assumptions)" in withQE { _ =>
    val seq =
      "[{x'=x,v'=v}] v <= 100 , a > 0, [{x'=x,v'=v&x+v^2<=6}] x <= 1 , b < 0, [{x'=x,v'=v&x=1}] 1+1=2 ==> <{x'=x, v'=v}> x+v^2 > 5"
        .asSequent
    ...
    pr.subgoals(0) shouldBe
      "[{x'=x,v'=v&true}]v<=100, a>0, [{x'=x,v'=v&x+v^2<=6}]x<=1, b < 0, [{x'=x,v'=v&x=1}]1+1=2 ==> <{x'=x,v'=v&true}>x>5"
        .asSequent
    pr.subgoals(1) shouldBe
      "[{x'=x,v'=v&true}]v<=100, a>0, [{x'=x,v'=v&x+v^2<=6}]x<=1, b < 0, [{x'=x,v'=v&x=1}]1+1=2 ==> [{x'=x,v'=v&((true&!x+v^2>5)&v<=100)&x<=1}](!x>5)"
        .asSequent
  }
  ...
  "dV" should "work on simple examples (1)" in withQE { _ =>
    val pr = proveBy("a>0 ==> <{x'=a,y'=z}>x>=b()".asSequent, dV("a".asTerm)(1))

    val pr2 = proveBy("c=1 ==> a<=0 , <{y'=z,x'=a+c}>x>=b()".asSequent, dV("a".asTerm)(2))
  ...
  it should "work on simple examples (1) symbolically" in withQE { _ =>
    val pr = proveBy(
      "a>0 ==> <{x'=a,y'=z}>x>=b()".asSequent,
      cutR("\\exists e_ (e_ > 0 & \\forall x \\forall y (a >= e_))".asFormula)(1) <
        (QE, implyR(1) & existsL(Symbol("Llast")) & dV("e_".asTerm)(1)),
    )

    val pr2 = proveBy(
      "c=1 ==> a<=0 , <{y'=z,x'=a+c}>x>=b()".asSequent,
      cutR("\\exists e_ (e_ > 0 & \\forall y \\forall x (a+c >= e_))".asFormula)(2) <
        (QE, implyR(2) & existsL(Symbol("Llast")) & dV("e_".asTerm)(2)),
    )
  ...
  it should "work on simple examples (1) automatically" in withQE { _ =>
    val pr = proveBy("a>0 ==> <{x'=a,y'=z}>x>=b()".asSequent, dVAuto()(1))

    val pr2 = proveBy("c=1 ==> a<=0 , <{y'=z,x'=a+c}>x>=b()".asSequent, dVAuto()(2))
  ...
  it should "diff var flat flight progress [function]" in withQE { _ =>
    val pr = proveBy(
      "b>0 -> \\exists d (d^2<=b^2 & <{x'=d}>x>=p())".asFormula,
      implyR(1) & existsR("b".asTerm)(1) & andR(1) < (QE, dVAuto()(1)),
    )
    pr shouldBe Symbol("proved")
  }
  ...
  it should "diff var flat flight progress [variable]" in withQE { _ =>
    val pr = proveBy(
      "b>0 -> \\forall p \\exists d (d^2<=b^2 & <{x'=d}>x>=p)".asFormula,
      implyR(1) & allR(1) & existsR("b".asTerm)(1) & andR(1) < (QE, dVAuto()(1)),
    )
    pr shouldBe Symbol("proved")
  }
  ...
  it should "support higher derivatives" in withMathematica { _ =>
    ...
    val pr = proveBy(
      "j > 0 ==> <{d'=c, x'=v, v'=a, a'=j, c'=-d}> x > 100".asSequent,
      // Should be old(x), etc.
  ...
  it should "support semialgebraic dV (disjunctive)" in withMathematica { _ =>
    val pr = proveBy(
      "v!=0 -> <{x'=v}> (x>100 | x < 100)".asFormula,
      implyR(1) &
        // encode abs(v)
        cut("\\exists absv (absv = abs(v))".asFormula) < (existsL(-2), hideR(1) & QE) & semialgdV("absv".asTerm)(1),
    )
  ...
  it should "support semialgebraic dV auto (disjunctive)" in withMathematica { _ =>
    val pr = proveBy("v!=0 -> v=0 | <{x'=v}> (x>100 | x < 100)".asFormula, implyR(1) & orR(1) & semialgdVAuto()(2))

    pr shouldBe Symbol("proved")
  }

  it should "support semialgebraic dV auto 2" taggedAs SlowTest in withMathematica { _ =>
    val pr = proveBy("<{u'=-u^3}>(-1<=u & u <=1 )".asFormula, semialgdVAuto()(1))
  ...
  "cor" should "refine a closed domain" in withQE { _ =>
    ...
    val pr = proveBy(
      ...
      cut("[{x'=x,y'=y&true&!(x=1|y=1|x=-1|y=-1)}](x<1&y<1&x>-1&y>-1)".asFormula) < (skip, hideR(1) & ODE(1)) &
  ...
  "univariate" should "automatically odeReduce univariate" in withMathematica { _ =>
    ...
    val pr = proveBy(fml, implyR(1) & odeReduce(true, Nil)(1) & cohideR(1) & byUS(Ax.TExgt))

    // @todo: works with Mathematica but something is wrong with ODE and z3??
    val fail =
      "b()>0&k>0&initx_>0  ==>  [{v'=-k*v^3-v^2-b()*v+1,y'=y,t'=1&true&v*v<=initx_*initx_+root_*root_}]v*v<=initx_*initx_+root_*root_"
        .asSequent
...
//    pr shouldBe 'proved
  ...
  it should "work with interleaved cases" in withQE { _ =>
    val fml =
      "a > 0 & b() < 0 & x<= a& x>= b()& y^2 = 0 -> <{k'=z+x+k,  y'=y*(y-a)*(y-b()),z'=x+z, x'=x*(x-a)*(x-b()), t'=1}> t > 1000"
        .asFormula
  ...
  it should "try manual univariate boundedness proof" in withQE { _ =>
    val fml =
      "a*r^2+b*r+c = 0 & (v-r=0 | v-r < 0 & a*v0^2+b*v0+c > 0 | a*v0^2+b*v0+c < 0 & v-r > 0 ) & v=v0 -> [{v' = a*v^2+b*v+c}] v^2 <= v0^2+r^2"
        .asFormula
    val pr = proveBy(
      fml,
      implyR(1) &
        cut("\\exists d \\exists e_ \\forall v a*v^2+b*v+c=(v-r)*(d*v+e_)".asFormula) <
        (
            ...
            (
              ODEInvariance.diffDivConquer("v-r".asTerm, Some("d*v+e_".asTerm))(1) <
                (
                  ...
                  cut("a*v0^2+b*v0+c < 0".asFormula) <
                    (dC("v<=v0".asFormula)(1) < (dW(1) & QE, ODE(1)), cohideOnlyR(2) & QE),
                  cut("a*v0^2+b*v0+c > 0".asFormula) <
                    (dC("v>=v0".asFormula)(1) < (dW(1) & QE, ODE(1)), cohideOnlyR(2) & QE),
                ),
              ODE(1),
  ...
  it should "prove Example 1 (manual)" in withMathematica { _ =>
    val tac = implyR(1) & allR(1) & dDDG("0".asTerm, "0".asTerm)(1) < (ODE(1), cohideR(1) & byUS(Ax.TExgt))
  ...
  it should "prove Example 2 (manual)" in withMathematica { _ =>
    ...
    val tac = allR(1) &
      dDDG("2*u".asTerm, "0".asTerm)(1) < (ODE(1), dDDG("2".asTerm, "0".asTerm)(1) < (ODE(1), byUS(Ax.TExgt)))
  ...
  it should "prove Example 3 (manual)" in withMathematica { _ =>
    val tac = implyR(1) & allR(1) & dBDG("1/4".asTerm, 2)(1) < (ODE(1), cohideR(1) & byUS(Ax.TExgt))

    val pr = proveBy(
      "u^2 + v^2 <= 1/4 -> \\forall tau <{u'=-v-u*(1/4-u^2-v^2), v'=u-v*(1/4-u^2-v^2), t'=1}> t > tau".asFormula,
      tac,
    )
  ...
  it should "prove Example 3 (automatic)" in withMathematica { _ =>
    val tac = implyR(1) & allR(1) & gEx("u*u+v*v<=1/4".asFormula :: Nil)(1)
  ...
  it should "prove Example 4 (manual)" in withMathematica { _ =>
    ...
    val tac = implyR(1) &
      ...
      cut("\\exists t t=0".asFormula) <
      (existsL(-2), cohideR(2) & QE) & vDG("t'=1".asDifferentialProgram)(1) &
      cut("\\exists p u^2+v^2=p".asFormula) <
      (existsL(-3), cohideR(2) & QE) &
      kDomainDiamond("t > 2*p".asFormula)(1) <
      (skip, dC("p-1/2*t >= u^2+v^2-1/4".asFormula)(1) < (ODE(1), ODE(1))) &
      // Global existence:
      useAt(Ax.commaCommuteD)(1) &
      dDDG("0".asTerm, "0".asTerm, 2)(1) <
      (ODE(1), cohideR(1) & byUS(Ax.TExgt))
  ...
  it should "prove Examples 5,6,7 (manual) " in withMathematica { _ =>
    ...
    val tac = implyR(1) &
      ...
      (skip, useAt(Ax.diamond, PosInExpr(1 :: Nil))(1) & propClose) &
      // Manual dV
      cut("\\exists t t=0".asFormula) <
      (existsL(-3), cohideR(2) & QE) & vDG("t'=1".asDifferentialProgram)(1) &
      cut("\\exists p u^2+v^2=p".asFormula) <
      (existsL(-4), cohideR(2) & QE) &
      kDomainDiamond("t > 2/3*p".asFormula)(1) <
      (
        skip,
        dC("p-3/2*t >= 2- (u^2+v^2)".asFormula)(1) <
          (hideL(-2) & ODE(1), hideL(-2) & dC("u^2+v^2>=1".asFormula)(1) < (ODE(1), ODE(1))),
      ) & useAt(Ax.commaCommuteD)(1) &
      dBDG("2".asTerm, 2)(1) <
      (
    ...
    val fml2 =
      "u^2+v^2 = 1 -> <{u'=-v-u*(1/4-u^2-v^2), v'=u-v*(1/4-u^2-v^2) & 1 <= u^2+v^2 & u^2+v^2 <= 2}> (u^2+v^2 >= 2)"
        .asFormula

    val tac2 = implyR(1) & dDR("u^2+v^2 <= 2".asFormula)(1) < (skip, ODE(1)) &
      closedRef("true".asFormula)(1) <
      (
  ...
  it should "prove Examples 5,6,7 (automatic) " in withMathematica { _ =>
    ...
    val tac2 = implyR(1) & dDR("u^2+v^2 <= 2".asFormula)(1) < (skip, ODE(1)) &
      closedRef("true".asFormula)(1) < (implyRi & byUS(pr), QE, ODE(1))
  ...
  it should "prove Example 8 (manual)" in withMathematica { _ =>
    val tac = kDomainDiamond("u^2 <= 1".asFormula)(1) <
      ...
      // This is a manual implementation of dV
      cut("\\exists t t=0".asFormula) <
      (existsL(-1), cohideR(2) & QE) & vDG("t'=1".asDifferentialProgram)(1) &
      cut("\\exists p u^2=p".asFormula) <
      (existsL(-2), cohideR(2) & QE) &
      kDomainDiamond("t > p".asFormula)(1) <
      (skip, dC("p-t >= u^2-1".asFormula)(1) < (ODE(1), ODE(1))) &
      // Global existence:
      useAt(Ax.commaCommuteD)(1) &
  ...
  it should "prove Sogokon & Jackson FM'15 Example 11" in withMathematica { _ =>
    ...
    val tac1 = implyR(1) &
      ...
      cut(Box(ODESystem(ode, True), "(x1^2+x2^2-1)^2<=1/30".asFormula)) < (skip, cohideOnlyR(2) & ODE(1)) &
      ...
      kDomainDiamond(Not(S1))(1) < (skip, dC("x1>=-1/4".asFormula)(1) < (ODE(1), ODE(1))) &
    ...
    val tac2 = implyR(1) &
      ...
      // Use a staging set that cannot be left without reaching the target
      kDomainDiamond(Not(S2))(1) < (skip, dC("x1<=1/4".asFormula)(1) < (ODE(1), ODE(1))) &
      // Save the staging set as context
      saveBox(1) &
      // Use a progress function
    ...
    val tac = implyR(1) &
      ...
      (
        saveBox(1) &
          ...
          cut("\\exists oldv oldv = x2-x1".asFormula) < (existsL(-5), cohideR(2) & QE) &
          compatCut("x2-x1>=oldv".asFormula)(1) <
          (gEx("2*(x2*(x2-x1)^2)<= oldv^2*(x2*x2)+oldv^2".asFormula :: Nil)(1), cohideOnlyR(2) & hideL(-2) & ODE(1)),
        ODE(1),
      )
  ...
  it should "prove RAL goal reachability" in withMathematica { _ =>
    val seq =
      "v>0, vl<=v, v<=vh, a=0, eps>0, k*eps^2-2*eps < k*(x^2+y^2)-2*x, k*(x^2+y^2)-2*x < k*eps^2-2*eps, y>0 ==> <{x'=-v*k*y, y'=v*(k*x-1), v'=0 & v>=0}>( x^2+y^2<=eps^2 & (vl<=v & v<=vh) )"
        .asSequent

    val tac = cut("\\exists oldv (oldv = v)".asFormula) <
      (existsL(Symbol("Llast")), hideR(1) & QE) &
      // known invariants: v stays constant, and robot always on annulus
      cut(
        "[{x'=-v*k*y, y'=v*(k*x-1), v'=0}]( v=oldv & k*eps^2-2*eps < k*(x^2+y^2)-2*x & k*(x^2+y^2)-2*x < k*eps^2-2*eps) "
          .asFormula
      ) <
      (skip, hideR(1) & ODE(1)) &
      ...
      (existsL(Symbol("Llast")), hideL(Symbol("Llast")) & hideR(1) & QE) &
  ...
  it should "prove RAL velocity bounds" in withMathematica { _ =>
    // abridged proof
    val seq =
      "v>=0, 0<vl, vl<vh, A>0, B>0 ==> \\exists a ( (a <= A & -B <= a) & <{x'=-v*k*y, y'=v*(k*x-1), v'=a & v>=0}>( vl<=v & v<=vh ))"
        .asSequent

    val tac = // Trichotomy
      cut("v < vl|vl<=v&v<=vh|vh < v".asFormula) < (skip, hideR(1) & QE) &
        orL(Symbol("Llast")) <
        (
          // v < vl, pick a = A
          existsR("A".asTerm)(1) &
