...
class ODEStabilityTests extends TacticTestBase {
  ...
  "stability" should "prove stability for pendulum" in withMathematica { _ =>
    ...
    val qe = proveBy(
      "a()>0&b()>0, eps>0 ==> \\exists bnd (bnd>0&\\forall theta \\forall w ((theta*theta+w*w < 1*1)&!theta*theta+w*w < eps*eps->-(a()*(2*theta*w)*2/4+(2*(b()*theta+w)*(b()*w+(-a()*theta-b()*w))+2*w*(-a()*theta-b()*w))*4/16)>=bnd))"
        .asSequent,
      QE,
    )

    val pr1 = proveBy(
      Imply("a() > 0 & b() > 0".asFormula, stable),
      unfoldProgramNormalize &
        // On ||x||=eps, there is a global lower bound on k
        cutR(
          "\\exists k (k > 0 & \\forall theta \\forall w (theta*theta+w*w = eps*eps -> a()*(theta^2)/2 + ((b()*theta+w)^2+w^2)/4 >= k))"
            .asFormula
        )(1) <
        (
          QE,
          unfoldProgramNormalize &
            ...
            cutR(
              "\\exists del (del > 0 & del < eps & \\forall theta \\forall w (theta*theta+w*w < del*del -> a()*(theta^2)/2 + ((b()*theta+w)^2+w^2)/4 < k))"
                .asFormula
            )(1) <
            (
              ...
              unfoldProgramNormalize & existsR("del".asTerm)(1) &
                ...
                (
                  prop,
                  unfoldProgramNormalize & allL(-8) & allL(-8) & // theta, w
                    implyL(-8) <
                    (
                      hideR(1) & prop,
                      ...
                      dC("theta*theta+w*w=eps*eps->a()*(theta^2)/2 + ((b()*theta+w)^2+w^2)/4>=k".asFormula)(1) <
                        (
                          hideL(-5) &
                            ...
                            dC("a()*(theta^2)/2 + ((b()*theta+w)^2+w^2)/4 < k".asFormula)(1) < (ODE(1), ODE(1)),
    ...
    val pr2 = proveBy(
      Imply(
        And(stable, "\\forall eps (eps>0-><{theta'=w,w'=-a()*theta-b()*w&true}>theta*theta+w*w < eps*eps)".asFormula),
        "\\forall eps (eps>0-><{theta'=w,w'=-a()*theta-b()*w&true}>[{theta'=w,w'=-a()*theta-b()*w&true}]theta*theta+w*w < eps*eps)"
          .asFormula,
      ),
              ...
              ODELiveness.kDomainDiamond("theta*theta+w*w < del*del".asFormula)(1) <
                (
                  ...
                  andL(Symbol("Llast")) &
                    ...
                    // TODO: make tactic that adds universals directly into domain (without the universals)
                    dC(
                      "(theta*theta+w*w < del*del->[{theta'=w,w'=-a()*theta-b()*w&true}]theta*theta+w*w < eps*eps)"
                        .asFormula
    ...
    val pr3 = proveBy(
      Imply(
        And(
          stable,
          "\\exists del (del>0&\\forall theta \\forall w (theta*theta+w*w < del*del->\\forall eps (eps>0-><{theta'=w,w'=-a()*theta-b()*w&true}>theta*theta+w*w < eps*eps)))"
            .asFormula,
        ),
        attractive,
      ),
      implyR(1) & andL(-1) & existsL(-2) & andL(-2) & existsR(1) &
        ...
        (
          ...
          allR(1) & allR(1) & implyR(1) &
            ...
            cutR(And(
              stable,
              "\\forall eps (eps>0-><{theta'=w,w'=-a()*theta-b()*w&true}>theta*theta+w*w < eps*eps)".asFormula,
            ))(1) < (andR(1) < (prop, allL(-3) & allL(-3) & implyL(-3) < (prop, prop)), cohideR(1) & byUS(pr2)),
        ),
    )

    val pr4 = proveBy(
      Imply("a() > 0 & b() > 0".asFormula, Imply(stable, attractive)),
      implyR(1) & implyR(1) &
        cutR(And(
          stable,
          "\\exists del (del>0&\\forall theta \\forall w (theta*theta+w*w < del*del->\\forall eps (eps>0-><{theta'=w,w'=-a()*theta-b()*w&true}>theta*theta+w*w < eps*eps)))"
                      ...
                      allR(1) & allR(1) & andL(-2) & implyR(1) & allL(-3) & allL(-3) &
                        ...
                        (
                          ...
                          allR(1) & implyR(1) & ODELiveness.saveBox(1) &
                            cutR(
                              "\\exists bnd \\forall theta \\forall w (theta*theta+w*w < 1 * 1 -> a()*(theta^2)/2 + ((b()*theta+w)^2+w^2)/4 >= bnd)"
                                .asFormula
                            )(1) <
                            (
                              cohideR(1) & QE,
                              implyR(1) & existsL(Symbol("Llast")) &
                                ODELiveness.kDomainDiamond(
                                  "a()*(theta^2)/2 + ((b()*theta+w)^2+w^2)/4 < bnd ".asFormula
                                )(1) <
                                (
                                  hideL(-7) & hideL(-4) & hideL(-2) & ODELiveness.dV(None)(1) &
                                    // Nasty
                                    cutR(
                                      "\\exists bnd (bnd>0&\\forall theta \\forall w ((theta*theta+w*w < 1*1)&!theta*theta+w*w < eps*eps->-(a()*(2*theta*w)*2/4+(2*(b()*theta+w)*(b()*w+(-a()*theta-b()*w))+2*w*(-a()*theta-b()*w))*4/16)>=bnd))"
                                        .asFormula
                                    )(1) <
                                    (
                                      byUS(qe),
    ...
    val pr5 = proveBy(
      Imply("a() > 0 & b() > 0".asFormula, And(stable, attractive)),
      implyR(1) &
  ...
  it should "prove global exponential stability for pendulum" in withMathematica { _ =>
    ...
    val pr1 = proveBy(
      Imply("a() = 1 & b() =1".asFormula, estable),
      implyR(1) & andL(-1) & exhaustiveEqL2R(-1) & exhaustiveEqL2R(-2) &
        existsR("2".asTerm)(1) <
        (andR(1) <
          (
            QE,
            existsR("1/4".asTerm)(1) &
              andR(1) <
              (
                QE,
                unfoldProgramNormalize &
                  dC("(theta^2)/2 + ((theta+w)^2+w^2)/4 <= 1/4 * aux".asFormula)(1) < (ODE(1), ODE(1)),
              ),
          )),
    )
  ...
  it should "prove stability for 1st and 3rd axis of rigid body" in withMathematica { _ =>
    val ode =
      "x1'=(I2()-I3())/I1() *x2*x3, x2'=(I3()-I1())/I2() *x3*x1, x3'=(I1()-I2())/I3()*x1*x2".asDifferentialProgram

    // Stability in x1 axis
    val stable1 =
      "\\forall eps (eps>0->\\exists del (del>0&\\forall x1 \\forall x2 \\forall x3 (x2*x2+x3*x3 < del*del->[{x1'=(I2()-I3())/I1()*x2*x3,x2'=(I3()-I1())/I2()*x3*x1,x3'=(I1()-I2())/I3()*x1*x2&true}]x2*x2+x3*x3 < eps*eps)))"
        .asFormula
    // Stability in x3 axis
    val stable3 =
      "\\forall eps (eps>0->\\exists del (del>0&\\forall x1 \\forall x2 \\forall x3 (x1*x1+x2*x2 < del*del->[{x1'=(I2()-I3())/I1()*x2*x3,x2'=(I3()-I1())/I2()*x3*x1,x3'=(I1()-I2())/I3()*x1*x2&true}]x1*x1+x2*x2 < eps*eps)))"
        .asFormula

    // Lyapunov function 1/2*((I1()-I2())/I3() *x2^2 - (I3()-I1())/I2()*x3^2)
    // Picking tau = eps
    val pr1 = proveBy(
      Imply("I1() > I2() & I2() > I3() & I3() > 0".asFormula, stable1),
      unfoldProgramNormalize &
        cutR(
          (
            "\\exists k (" +
              "\\exists del (del > 0 & del < eps & \\forall x2 \\forall x3 (x2*x2+x3*x3 < del*del -> 1/2*((I1()-I2())/I3() *x2^2 - (I3()-I1())/I2()*x3^2)  < k)) &" +
              "\\forall x2 \\forall x3 (x2*x2+x3*x3 = eps*eps -> 1/2*((I1()-I2())/I3() *x2^2 - (I3()-I1())/I2()*x3^2) >= k))"
          ).asFormula
        )(1) <
        (
          QE,
          unfoldProgramNormalize & existsR("del".asTerm)(1) &
            ...
            (
              prop,
              unfoldProgramNormalize & allL(-8) & allL(-8) &
                implyL(-8) <
                (
                  hideR(1) & prop,
                  // Move the forall quantified antecedent into domain constraint
                  // TODO: make tactic that adds universals directly into domain (without the universals)
                  dC("x2*x2+x3*x3 = eps*eps -> 1/2*((I1()-I2())/I3() *x2^2 - (I3()-I1())/I2()*x3^2) >= k".asFormula)(
                    1
                  ) <
                    (
                      hideL(-5) &
                        // This part is slightly simpler without having to close over the sub-domain
                        dC("1/2*((I1()-I2())/I3() *x2^2 - (I3()-I1())/I2()*x3^2)  < k".asFormula)(1) < (ODE(1), ODE(1)),
                      dWPlus(1) & allL(-5) & allL(-5) & prop,
                    ),
                ),
            ),
    ...
    // Picking tau = eps
    val pr3 = proveBy(
      Imply("I1() > I2() & I2() > I3() & I3() > 0".asFormula, stable3),
      unfoldProgramNormalize &
        cutR(
          (
            "\\exists k (" +
              "\\exists del (del > 0 & del < eps & \\forall x1 \\forall x2 (x1*x1+x2*x2 < del*del -> 1/2*(-(I3()-I1())/I2()*x1^2 + (I2()-I3())/I1() *x2^2)  < k)) &" +
              "\\forall x1 \\forall x2 (x1*x1+x2*x2 = eps*eps -> 1/2*(-(I3()-I1())/I2()*x1^2 + (I2()-I3())/I1() *x2^2) >= k))"
          ).asFormula
        )(1) <
        (
          QE,
            ...
            (
              ...
              unfoldProgramNormalize & allL(-8) & allL(-8) &
                ...
                (
                  ...
                  // Move the forall quantified antecedent into domain constraint
                  // TODO: make tactic that adds universals directly into domain (without the universals)
                  dC("x1*x1+x2*x2 = eps*eps -> 1/2*(-(I3()-I1())/I2()*x1^2 + (I2()-I3())/I1() *x2^2) >= k".asFormula)(
                    1
                  ) <
                    (
                      hideL(-5) &
  ...
  it should "prove global asymptotic stability for 1st and 3rd axis of rigid body with friction" in withMathematica {
    _ =>
      val ode =
        "x1'=(I2()-I3())/I1() *x2*x3 - a1()*x1, x2'=(I3()-I1())/I2() *x3*x1 - a2()*x2, x3'=(I1()-I2())/I3()*x1*x2 - a3()*x3"
          .asDifferentialProgram

      // Main parameter assumptions
      val Gamma = "I1() > I2() & I2() > I3() & I3() > 0 & a1() > 0 & a2() > 0 & a3() > 0".asFormula
      val stable1 =
        "\\forall eps (eps>0->\\exists del (del>0&\\forall x1 \\forall x2 \\forall x3 (x2*x2+x3*x3 < del*del->[{x1'=(I2()-I3())/I1()*x2*x3-a1()*x1,x2'=(I3()-I1())/I2()*x3*x1-a2()*x2,x3'=(I1()-I2())/I3()*x1*x2-a3()*x3&true}]x2*x2+x3*x3 < eps*eps)))"
          .asFormula
      // Globally attractive
      val gattractive1 =
        "\\forall eps (eps>0-><{x1'=(I2()-I3())/I1()*x2*x3-a1()*x1,x2'=(I3()-I1())/I2()*x3*x1-a2()*x2,x3'=(I1()-I2())/I3()*x1*x2-a3()*x3&true}>[{x1'=(I2()-I3())/I1()*x2*x3-a1()*x1,x2'=(I3()-I1())/I2()*x3*x1-a2()*x2,x3'=(I1()-I2())/I3()*x1*x2-a3()*x3&true}]x2*x2+x3*x3 < eps*eps)"
          .asFormula
      // Globally attractive (with stability)
      val gattractives1 =
        "\\forall eps (eps>0-><{x1'=(I2()-I3())/I1()*x2*x3-a1()*x1,x2'=(I3()-I1())/I2()*x3*x1-a2()*x2,x3'=(I1()-I2())/I3()*x1*x2-a3()*x3&true}>x2*x2+x3*x3 < eps*eps)"
          .asFormula

      val energy =
        "\\forall eps (eps>0-><{x1'=(I2()-I3())/I1()*x2*x3-a1()*x1,x2'=(I3()-I1())/I2()*x3*x1-a2()*x2,x3'=(I1()-I2())/I3()*x1*x2-a3()*x3&true}>I1()*x1^2+I2()*x2^2+I3()*x3^2 < eps*eps)"
          .asFormula
          ...
          (
            ...
            cutR("\\exists r (I1() * x1^2 + I2() * x2^2 + I3() * x3^2 = r)".asFormula)(1) <
              (
                QE,
                implyR(1) & existsL(Symbol("Llast")) &
                  ODELiveness.compatCut("x2*x2+x1*x1+x3*x3<=r/I3()".asFormula)(1) <
                  (skip, hideR(1) & dC("I1() * x1^2 + I2() * x2^2 + I3() * x3^2 <= r".asFormula)(1) < (ODE(1), ODE(1))),
              ) & ODELiveness.odeReduce(true, "x2*x2+x1*x1+x3*x3<=r/I3()".asFormula :: Nil)(1) & cohideR(1) &
              byUS(Ax.TExgt),
          ),
      )
        ...
        unfoldProgramNormalize &
          ...
          (
            ...
            unfoldProgramNormalize & existsR("del".asTerm)(1) &
              ...
              (
                prop,
                unfoldProgramNormalize & allL(-11) & allL(-11) &
                  implyL(-11) <
                  (
                    hideR(1) & prop,
                    // Move the forall quantified antecedent into domain constraint
                ...
                ODELiveness.kDomainDiamond("x2*x2+x3*x3 < del*del".asFormula)(1) <
                  (
                    ...
                    andL(Symbol("Llast")) &
                      ...
                      // TODO: make tactic that adds universals directly into domain (without the universals)
                      dC(
                        "(x2*x2+x3*x3 < del*del->[{x1'=(I2()-I3())/I1()*x2*x3-a1()*x1,x2'=(I3()-I1())/I2()*x3*x1-a2()*x2,x3'=(I1()-I2())/I3()*x1*x2-a3()*x3&true}]x2*x2+x3*x3 < eps*eps)"
                          .asFormula
                      )(1) < (dW(1) & prop, dWPlus(1) & allL(-3) & allL(-3) & allL(-3) & close),
                  ),
              ),
          ),
      )
      ...
      val pr13 = proveBy(
        Imply(Gamma, gattractives1),
        implyR(1) &
          cutR(energy)(1) <
          (
            implyRi & byUS(pr),
            implyR(1) & allR(1) & implyR(1) & allL("eps*(I3()^(1/2))".asTerm)(-2) &
              implyL(-2) <
              (
                hideR(1) & QE,
                ODELiveness.kDomainDiamond(
                  "I1()*x1^2+I2()*x2^2+I3()*x3^2 < eps*(I3())^(1/2)*(eps*(I3())^(1/2))".asFormula
                )(1) < (close, dW(1) & QE),
              ),
          ),
      )
      ...
      // Propositional manipulation
      val pr14 = proveBy(
        Imply(Gamma, And(stable1, gattractive1)),
        implyR(1) &
          cutR(And(stable1, Imply(stable1, gattractive1)))(1) <
          (
            andR(1) <
              (
                implyRi & byUS(pr11),
                implyR(1) &
                  cutR(And(stable1, gattractives1))(1) <
                  (andR(1) < (prop, implyRi & cohideR(1) & byUS(pr13)), cohideR(1) & byUS(pr12)),
              ),
            prop,
          ),
      )
      ...
      val stable2 =
        "\\forall eps (eps>0->\\exists del (del>0&\\forall x1 \\forall x2 \\forall x3 (x1*x1+x2*x2 < del*del->[{x1'=(I2()-I3())/I1()*x2*x3-a1()*x1,x2'=(I3()-I1())/I2()*x3*x1-a2()*x2,x3'=(I1()-I2())/I3()*x1*x2-a3()*x3&true}]x1*x1+x2*x2 < eps*eps)))"
          .asFormula
      // Globally attractive
      val gattractive2 =
        "\\forall eps (eps>0-><{x1'=(I2()-I3())/I1()*x2*x3-a1()*x1,x2'=(I3()-I1())/I2()*x3*x1-a2()*x2,x3'=(I1()-I2())/I3()*x1*x2-a3()*x3&true}>[{x1'=(I2()-I3())/I1()*x2*x3-a1()*x1,x2'=(I3()-I1())/I2()*x3*x1-a2()*x2,x3'=(I1()-I2())/I3()*x1*x2-a3()*x3&true}]x1*x1+x2*x2 < eps*eps)"
          .asFormula
      // Globally attractive (with stability)
      val gattractives2 =
        "\\forall eps (eps>0-><{x1'=(I2()-I3())/I1()*x2*x3-a1()*x1,x2'=(I3()-I1())/I2()*x3*x1-a2()*x2,x3'=(I1()-I2())/I3()*x1*x2-a3()*x3&true}>x1*x1+x2*x2 < eps*eps)"
          .asFormula
                ...
                ODELiveness.kDomainDiamond("x1*x1+x2*x2 < del*del".asFormula)(1) <
                  (
                    ...
                    andL(Symbol("Llast")) &
                      ...
                      // TODO: make tactic that adds universals directly into domain (without the universals)
                      dC(
                        "(x1*x1+x2*x2 < del*del->[{x1'=(I2()-I3())/I1()*x2*x3-a1()*x1,x2'=(I3()-I1())/I2()*x3*x1-a2()*x2,x3'=(I1()-I2())/I3()*x1*x2-a3()*x3&true}]x1*x1+x2*x2 < eps*eps)"
                          .asFormula
                      )(1) < (dW(1) & prop, dWPlus(1) & allL(-3) & allL(-3) & allL(-3) & close),
                  ),
              ),
      ...
      // Propositional manipulation
      val pr24 = proveBy(
        Imply(Gamma, And(stable2, gattractive2)),
        implyR(1) &
          cutR(And(stable2, Imply(stable2, gattractive2)))(1) <
          (
            andR(1) <
              (
                implyRi & byUS(pr21),
                implyR(1) &
                  cutR(And(stable2, gattractives2))(1) <
                  (andR(1) < (prop, implyRi & cohideR(1) & byUS(pr23)), cohideR(1) & byUS(pr22)),
              ),
            prop,
          ),
      )
  ...
  it should "prove epsilon stability for Moore-Greitzer Jet Engine" in withMathematica { _ =>
    ...
    val stable =
      "\\forall eps (eps>0->\\exists del (del>0&\\forall x1 \\forall x2 (x1*x1+x2*x2 < del*del->[{x1'=-x2-3/2*x1^2-1/2*x1^3,x2'=3*x1-x2&true}]x1*x1+x2*x2 < (tau+eps)*(tau+eps))))"
        .asFormula
    ...
    val pr1 = proveBy(
      Imply("tau = 10^-10".asFormula, stable),
      unfoldProgramNormalize &
        ...
        (
          ...
          unfoldProgramNormalize &
            ...
            cutR(
              s"\\exists del (del > 0 & del < eps+tau & \\forall x1 \\forall x2 (x1*x1+x2*x2 < del*del -> $lyapstr < 2.95 * 10^-19))"
                .asFormula
            )(1) <
            (
              hideL(Symbol("Llast")) & QE,
              unfoldProgramNormalize & existsR("del".asTerm)(1) &
                ...
                (
                  prop,
                  unfoldProgramNormalize & allL(-6) & allL(-6) & // x1, x2
                    implyL(-6) <
                    (
                      hideR(1) & prop,
                      // Move the forall quantified antecedent into domain constraint
                      // TODO: make tactic that adds universals directly into domain (without the universals)
                      dC(
                        s"(x1*x1+x2*x2=(eps+tau)*(eps+tau)->$lyapstr >= 2.95 * 10^-19) & $lyapstr < 2.95 * 10^-19"
                          .asFormula
                      )(1) <
                        (
                          hideL(-3) & ODE(1),
...
//    println(qe)
    ...
    val pr1 = proveBy(
      ...
      unfoldProgramNormalize &
        cutR(
          "\\exists tau (tau > 0 & tau < eps & \\forall theta \\forall w (theta*theta+w*w <= tau*tau -> 1/12*((-6)*b*w^2+a*theta^2*(b*((-6)+theta^2)+2*theta*w)) <= 0))"
            .asFormula
        )(1) <
        (
          QE,
          unfoldProgramNormalize &
            ...
            cutR(
              "\\exists k (k > 0 & \\forall theta \\forall w (theta*theta+w*w = tau*tau -> a*(theta^2)/2 + ((b*theta+w)^2+w^2)/4 >= k))"
                .asFormula
            )(1) <
            (
              hideL(Symbol("Llast")) & QE,
              unfoldProgramNormalize &
                // There is del s.t. ||x||<del -> v < k
                cutR(
                  "\\exists del (del > 0 & del < tau & \\forall theta \\forall w (theta*theta+w*w < del*del -> a*(theta^2)/2 + ((b*theta+w)^2+w^2)/4 < k))"
                    .asFormula
                )(1) <
                (
                  ...
                  unfoldProgramNormalize & existsR("del".asTerm)(1) &
                    ...
                    (
                      prop,
                      unfoldProgramNormalize & allL(-11) & allL(-11) & // theta, w
                        implyL(-11) <
                        (
                          hideR(1) & prop,
    ...
    val pr2 = proveBy(
      Imply(
        And(
          stable,
          "\\forall eps (eps>0-><{theta'=w,w'= -a*(theta - theta^3/6)-b*w&true}>theta*theta+w*w < eps*eps)".asFormula,
        ),
        "\\forall eps (eps>0-><{theta'=w,w'= -a*(theta - theta^3/6)-b*w&true}>[{theta'=w,w'= -a*(theta - theta^3/6)-b*w&true}]theta*theta+w*w < eps*eps)"
          .asFormula,
      ),
      implyR(1) & andL(-1) & allR(1) & implyR(1) & allL(-1) &
        implyL(-1) <
              ...
              ODELiveness.kDomainDiamond("theta*theta+w*w < del*del".asFormula)(1) <
                (
                  ...
                  andL(Symbol("Llast")) &
                    ...
                    // TODO: make tactic that adds universals directly into domain (without the universals)
                    dC(
                      "(theta*theta+w*w < del*del->[{theta'=w,w'= -a*(theta - theta^3/6)-b*w&true}]theta*theta+w*w < eps*eps)"
                        .asFormula
                    )(1) < (dW(1) & prop, dWPlus(1) & allL(-3) & allL(-3) & close),
                ),
            ),
    ...
    val pr3 = proveBy(
      Imply(
        And(
          stable,
          "\\exists del (del>0&\\forall theta \\forall w (theta*theta+w*w < del*del->\\forall eps (eps>0-><{theta'=w,w'= -a*(theta - theta^3/6)-b*w&true}>theta*theta+w*w < eps*eps)))"
            .asFormula,
        ),
        attractive,
      ),
      implyR(1) & andL(-1) & existsL(-2) & andL(-2) & existsR(1) &
        ...
        (
          ...
          allR(1) & allR(1) & implyR(1) &
            ...
            cutR(And(
              stable,
              "\\forall eps (eps>0-><{theta'=w,w'= -a*(theta - theta^3/6)-b*w&true}>theta*theta+w*w < eps*eps)"
                .asFormula,
            ))(1) < (andR(1) < (prop, allL(-3) & allL(-3) & implyL(-3) < (prop, prop)), cohideR(1) & byUS(pr2)),
        ),
    )
