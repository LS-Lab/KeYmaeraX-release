...
class DependencyAnalysisTests extends TacticTestBase {
  ...
  "DependencyAnalysis" should "correctly find SCCs" in withMathematica { qeTool =>
    ...
    scc(adjls) should contain only
      (Set("Tl".asVariable), Set("t".asVariable, "Tw".asVariable, "v".asVariable), Set("l".asVariable))
  }
  ...
  "DependencyAnalysis" should "analyse dependencies for some examples" in withMathematica { qeTool =>
    ...
    sccs2 should contain only
      (Set("l".asVariable), Set("t".asVariable), Set("v".asVariable), Set("Tl".asVariable, "Tw".asVariable))
  }
  ...
  private def timeheufixedPOQE(problems: List[(Program, Sequent)], po: Ordering[Variable]): List[Double] = {
    val timeLs = ListBuffer[(Double)]()
    for ((p, seq) <- problems) {
      val t = timeCall(_ => { proveBy(seq, heuQEPO(po)) })
      timeLs += t
    }
    timeLs.toList
  }
  ...
  // Timing tests
  "DependencyAnalysis" should "record time to re-prove the ODE logs" taggedAs IgnoreInBuildTest in withMathematica {
    qeTool =>
  ...
  "DependencyAnalysis" should "record time to re-prove lab 2" taggedAs IgnoreInBuildTest in withMathematica { qeTool =>
    ...
    val pof = inducedOrd(transClose(
      analyseModalVars(p2, varSetToBaseVarSet(StaticSemantics.vars(p2).toSet), false).view.mapValues(v => v._1).toMap
    ))
    val pot = inducedOrd(transClose(
      analyseModalVars(p2, varSetToBaseVarSet(StaticSemantics.vars(p2).toSet), true).view.mapValues(v => v._1).toMap
    ))
  ...
  "DependencyAnalysis" should "record time to re-prove lab 3" taggedAs IgnoreInBuildTest in withMathematica { qeTool =>
    ...
    val pof = inducedOrd(transClose(
      analyseModalVars(p3, varSetToBaseVarSet(StaticSemantics.vars(p3).toSet), false).view.mapValues(v => v._1).toMap
    ))
    val pot = inducedOrd(transClose(
      analyseModalVars(p3, varSetToBaseVarSet(StaticSemantics.vars(p3).toSet), true).view.mapValues(v => v._1).toMap
    ))
