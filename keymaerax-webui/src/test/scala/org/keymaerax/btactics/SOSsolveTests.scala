...
import org.keymaerax.btactics.SOSSolve.{
  ExponentOutOfScopeFailure,
  NonUniversalOutOfScopeFailure,
  RatFormError,
  SOSSolveAborted,
  SOSSolveNoSOS,
...
import org.keymaerax.tools.ext.SOSsolveTool.Witness
...
class SOSsolveTests extends TacticTestBase with PrivateMethodTester {
  private lazy val prob1 =
    ("!(-200 + 10 * t10_0 + v2_0 = 0 &" + "t10_0 >= 0 &" + "st2_0 = 0 &" + "-2000 * st2_0 - 4000 * s2_0 -" +
      "  200 * st2_0 * t2_0 - 5 * t2_0^2 +" + "  10 * st2_0 * t2_0^2 + z2_0 = 0 &" + "t2_0 >= 0 &" + "s2_0 >= 0 &" +
      "v2_0 >= 0 &" + "z2_0 >= 0 &" + "-200 * st2_0 - 10 * t2_0 +" + "  20 * st2_0 * t2_0 + v2_0 = 0 &" +
      "t10_0 + t2_0 < 0)").asFormula
  private lazy val polys1 =
    ("st2_0," + "-200 + 10 * t10_0 + v2_0," + "-200 * st2_0 - 10 * t2_0 + 20 * st2_0 * t2_0 + v2_0," +
      "-2000 * st2_0 - 4000 * s2_0 - 200 * st2_0 * t2_0 - 5 * t2_0^2 + 10 * st2_0 * t2_0^2 + z2_0," +
      "-GEQ11^2 + s2_0," + "-GEQ12^2 + t10_0," + "-GEQ13^2 + t2_0," + "-GEQ14^2 + v2_0," + "-GEQ15^2 +  z2_0," +
      "-1 + GT16^2 * (-t10_0 - t2_0)").split(',').map(_.asTerm).toList
  private lazy val vars1 = ("v2_0, t10_0, st2_0, z2_0," + "t2_0, s2_0, GEQ11, GEQ12, GEQ13, GEQ14, GEQ15, GT16")
    .split(',')
    .map(_.asTerm)
    .toList
  ...
  "sosSolveTool" should "return the certificate" in withMathematica { _ =>
    ...
    sosSolveTool.sosSolve(polys1, vars1, 1, None) shouldBe Witness(
      "1+20*GT16^2".asTerm,
      ...
      List(
        ...
        (4, "t2_0", "GEQ13^2", "1"),
        (2, "s2_0", "GEQ11^2", "1"),
      ).map { case (i, a, b, c) => (i, a.asTerm, b.asTerm, c.asTerm) },
    )
  }
  ...
  "SOSSolve" should "prove using the certificate" in withMathematica { _ =>
    ...
    proveBy(
      prob1,
      prop & SOSSolve.prepareArith & SOSSolve.witnessSOS(1, SOSSolve.lexicographicVariableOrdering),
  ...
  it should "illustrate automatic and interactive treatment of division" in withMathematica { _ =>
    ...
    val fml =
      "((A>=0&ep>=0&1>=0&1>0&sbsc>0&ms>0&g>0&-1<=0&0<=1&v_1<=vdes&0<=Tw&Tw<=A&v_1^2-d^2<=2*(1*sbsc/ms)*(m-z)&d>=0&t>=0&v>=0&t<=ep&v_1>=0&0<=ep->(1*Tw/1-(ms*0+ms*g*0)-((v*(1-1)+1)*sbsc+0))/ms=0+(Tw/ms-1*sbsc/ms)*1|m-z<=(v_1^2-d^2)/(2*1*sbsc/ms)+(A/ms/(1*sbsc/ms)+1)*(A/ms/2*ep^2+ep*v_1)|em=1))"
        .asFormula
    import SOSSolve._
    ...
    proveBy(
      fml,
      prop & cut("2*ms^2*sbsc > 0".asFormula) & Idioms.<(
        sos(RatForm(false)),
        cut("ms>0&sbsc>0".asFormula) &
          Idioms.<(cohideOnlyL(Symbol("Llast")) & cohideOnlyR(Symbol("Rlast")) & sos(), prop & done),
      ),
    ) shouldBe Symbol("proved")
  }
...
class SOSsolveQELoggerTests extends TacticTestBase with PrivateMethodTester {
  ...
  "SOSSolve" should "try to prove stuff generated by QELogger" in withMathematica { _ =>
    ...
    val variableOrdering = Configuration(Configuration.Keys.SOSSOLVE_VARIABLE_ORDERING) match {
      case "lexicographic" => SOSSolve.lexicographicVariableOrdering
      case "preferAuxiliary" => SOSSolve.preferAuxiliaryVariableOrdering
      case "deferAuxiliary" => SOSSolve.deferAuxiliaryVariableOrdering
    ...
    class Counter(status: Status) {
      ...
      def log(n: String, concl: Sequent, subgoal: Sequent): Unit = { QELogger.logSequent(concl, subgoal, n, fileName) }
      def inc(): Status = {
    ...
    def processEntry(degree: Int, variableOrdering: Ordering[Variable], timeout: Int)(
        entry: (String, Sequent, Sequent)
    ): Unit = entry match {
      case (n, seq0, seq) =>
...
//            val ratFormStrategy = SOSSolve.NoRatForm
              ...
              val preprocessed = preprocTimer.time { proveBy(seq, OnAll(SOSSolve.preprocess(ratFormStrategy))) }
              println("preprocessed.")
              val res =
                if (preprocessOnly) Seq()
                else for ((subgoal, subgoalN) <- preprocessed.subgoals.zipWithIndex) yield {
                  println(
                    "Subgoal " + subgoalN + ": " + subgoal.ante.mkString(", ") +
                      subgoal.succ.mkString(" ==> ", ", ", "")
                  )
                  proveBy(
                    subgoal,
                    SOSSolve.sos(
              ...
              case ExponentOutOfScopeFailure(_) => outofScopeUniversal.inc(n, seq0, seq)
              case ex: Throwable =>
        ...
        if (status == Success) {
          ...
          println(
            "       prove/search  : " +
              ((preprocTimer.getTimeMs + ratFormTimer.getTimeMs + witnessTimer.getTimeMs) / sosTimer.getTimeMs)
          )
        ...
        try {
          ...
          val timings = Seq(status.name) ++
            timers.map(timer => if (status == Success || timer == qeTimer) timer.getTimeMs else 0.0)
          val fw = new FileWriter(f, true)
          fw.append(timings.mkString(" ") + "\n")
          fw.close()
        } catch { case ex: Exception => throw ex }

        val expected = aborted.count + noSos.count + divisorElimination.count + divisorWelldefinedness.count +
          unknown.count
...
class ConvertQELogToIsabelle extends TacticTestBase with PrivateMethodTester {
  ...
  def variableToIsabelle(v: Variable, showType: Boolean): String = v match {
    case BaseVariable(name, index, Real) =>
      val vString = name.replace("_", "\\<^sub>\\<p>") /* for /p/rivate */ +
        (index match {
          case None => ""
  ...
  def toIsabelle(t: Term, typeReal: Boolean): String = t match {
    ...
    case Divide(a, b) =>
      ...
      else throw ToIsabelleTermException("keymaerax_nonnatural_exponent")
    case Power(a, b) => printOp("^", toIsabelle(a, typeReal), toIsabelle(b, false))
    case Neg(a) => printOp(toIsabelle(_: Term, typeReal), "-", a)
    case Number(n) =>
      if (typeReal || (n.isValidInt && n.toIntExact >= 0)) paren(n.toString + (if (typeReal) "::real" else "::nat"))
      else throw ToIsabelleTermException("keymaerax_nonnatural_number")
    case v: BaseVariable =>
      if (typeReal) variableToIsabelle(v, false) else throw ToIsabelleTermException("keymaerax_variable_in_exponent")
    case v: DifferentialSymbol => toIsabelle(Differential(v.x), typeReal): String
    case Differential(a) =>
      printOp(toIsabelle(_: Term, typeReal), "(keymaerax_differential::real\\<Rightarrow>real)", a)
    case FuncOf(InterpretedSymbols.minF, Pair(a, b)) => "(min::real\\<Rightarrow>real\\<Rightarrow>real)" +
        paren(toIsabelle(a, typeReal)) + paren(toIsabelle(b, typeReal))
    case FuncOf(InterpretedSymbols.maxF, Pair(a, b)) => "(max::real\\<Rightarrow>real\\<Rightarrow>real)" +
        paren(toIsabelle(a, typeReal)) + paren(toIsabelle(b, typeReal))
  ...
  def toIsabelle(fml: Formula): String = fml match {
    ...
    case Forall(vs, a) =>
      paren("\\<forall> " + vs.map(variableToIsabelle(_, true)).mkString(" ") + " . " + paren(toIsabelle(a)))
    case Exists(vs, a) =>
      paren("\\<exists> " + vs.map(variableToIsabelle(_, true)).mkString(" ") + " . " + paren(toIsabelle(a)))
  ...
  def toIsabelle(filterFalse: Option[Int], entry: (String, Sequent, Sequent)): String = entry match {
    case (s: String, _: Sequent, seq: Sequent) if (seq.ante.length == 0 && seq.succ.length == 1) =>
      val converted =
        try { s + "#" + toIsabelle(seq.succ(0)) + "\n" }
        catch { case ToIsabelleTermException(msg) => s + "#" + msg + "\n" }
      if (filterFalse.isDefined) {
        val qe = proveBy(seq, ?(QEX(None, filterFalse.map(Number(_)))))
        if (qe.subgoals.exists(_.succ.contains(False))) s + "#" + "keymaerax_qe_reduces_to_false\n" else converted
      } else converted
