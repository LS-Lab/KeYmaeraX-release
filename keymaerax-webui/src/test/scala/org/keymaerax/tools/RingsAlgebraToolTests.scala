...
import org.keymaerax.tools.ext.{RingsAlgebraTool, RingsLibrary}
...
class RingsAlgebraToolTests extends TacticTestBase {
  ...
  "RingsAlgebraTool" should "compute groebner basis" in withMathematica { _ =>
    ...
    tool.groebnerBasis(List(x, y, Variable("z"))) shouldBe List("z", "1+9*y+6*a*b+9*a^2*b^2", "5+y+x+y^2+x*y+x*y^2")
      .map(_.asTerm)
  }
  ...
  it should "compute the Lie derivative" in withMathematica { _ =>
    ...
    val odemap =
      Map("x".asVariable -> "-y".asTerm, "y".asVariable -> "-x^2".asTerm, "z".asVariable -> "14*x*y*z".asTerm)
  ...
  it should "compute the Picard iteration" in {
    ...
    ode.PicardIteration(x0R, 4).map(R.fromRing).toList shouldBe
      "1/2*r1()+2*r0()+(-1)/100*t*r1()^2+(-1)/5*t*r0()*r1()+(-t)*r0()^2+(-3)/50*t^2*r1()^2+(-9)/10*t^2*r0()*r1()+(-3)*t^2*r0()^2"
        .asTerm ::
      "1/10*r1()+r0()+3/5*t*r1()+3*t*r0()+3/10*t^2*r1()+3/2*t^2*r0()+(-1)/200*t^2*r1()^2+(-1)/10*t^2*r0()*r1()+1/10*t^3*r1()+(-1)/2*t^2*r0()^2+1/2*t^3*r0()"
        .asTerm :: Nil
  }
  ...
  it should "normalize with Rings" in withMathematica { _ =>
    ...
    res.subgoals.loneElement.succ shouldBe
      seq.succ.updated(1, "0<=468+a()+(-x)*r1()+(-t)*r1()+(-i^4)*x+(-i^4)*t".asFormula)
  }
