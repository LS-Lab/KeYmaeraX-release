...
import org.keymaerax.bellerophon.BelleLabel
import org.keymaerax.btactics.{BelleLabels, TacticTestBase, TactixLibrary}
import org.keymaerax.core._
import org.keymaerax.info.{Os, OsType}
import org.keymaerax.parser.StringConverter._
import org.keymaerax.tagobjects.{IgnoreInBuildTest, SlowTest, TodoTest}
import org.keymaerax.tools.ext.{ExtMathematicaOpSpec, JLinkMathematicaLink, Mathematica, MathematicaLink, ToolExecutor}
import org.keymaerax.tools.install.ToolConfiguration
import org.keymaerax.tools.qe.{
...
class JLinkMathematicaLinkTests extends TacticTestBase with PrivateMethodTester {
  ...
  if (new java.io.File("/Applications/Mathematica9.app").exists) {
    "Mathematica 9" should "not fail activation test on MacOS" taggedAs IgnoreInBuildTest in {
      ...
      mathematica.init(ToolConfiguration(mathKernel = Some("/Applications/Mathematica9.app/Contents/MacOS/MathKernel")))
  ...
  "Mathematica 10" should "not fail activation test on MacOS" taggedAs IgnoreInBuildTest in {
    val mathematica = new Mathematica(new JLinkMathematicaLink("Mathematica"), "Mathematica")
    mathematica.init(ToolConfiguration(mathKernel = Some("/Applications/Mathematica.app/Contents/MacOS/MathKernel")))
    mathematica shouldBe Symbol("initialized")
    mathematica.shutdown()
  }
