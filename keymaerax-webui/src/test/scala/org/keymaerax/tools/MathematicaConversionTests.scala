...
import org.keymaerax.parser.{InterpretedSymbols, Parser}
...
import org.keymaerax.tools.qe.ExprFactory._
import org.keymaerax.tools.qe.MathematicaConversion.{KExpr, MExpr}
import org.keymaerax.tools.qe.{K2MConverter, KeYmaeraToMathematica, MathematicaOpSpec, MathematicaToKeYmaera}
...
class MathematicaConversionTests extends AnyFlatSpec with Matchers with BeforeAndAfterEach with BeforeAndAfterAll {
  ...
  override def beforeAll(): Unit = {
    ...
    PrettyPrinter.setPrinter(org.keymaerax.parser.KeYmaeraXPrettyPrinter)
  ...
  it should "convert numbers" in { ml.runUnchecked("2+2")._2 should be(Number(4)) }
  ...
  it should "convert arithmetic expressions correctly" in {
    ...
    ml.runUnchecked("ForAll[{kyx`x}, kyx`x/4 == 4]")._2 shouldBe
      Forall(Seq(x), Equal(Times(Divide(num(1), num(4)), x), num(4)))
  }

  it should "convert rules correctly with the nonQEConverter" in {
    val localMl = new BaseKeYmaeraMathematicaBridge[KExpr](
      link,
      new UncheckedBaseK2MConverter(),
      new UncheckedBaseM2KConverter(),
    ) {}
  ...
  it should "convert names correctly" in {
    ...
    ml.runUnchecked("Apply[kyx`x, {kyx`y}]")._2 should
      be(FuncOf(Function("x", None, Real, Real), Variable("y", None, Real)))
    ml.runUnchecked("Apply[kyx`x, {kyx`y0}]")._2 should
      be(FuncOf(Function("x", None, Real, Real), Variable("y0", None, Real)))
  }
  ...
  it should "associate minus and negation correctly" in {
    KeYmaeraToMathematica("5--2".asTerm) shouldBe
      MathematicaOpSpec.minus(new MExpr(BigInt(5).bigInteger), MathematicaOpSpec.neg(new MExpr(BigInt(2).bigInteger)))
  }
  ...
  it should "convert multi-argument Apply to nested lists" in {
    val in = FuncOf(
      ...
      Pair(Variable("x"), Pair(Variable("y"), Variable("z"))),
    )
    val expected = makeExpr(
      new MExpr(Expr.SYMBOL, "kyx`f"),
      Array[MExpr](
        new MExpr(Expr.SYMBOL, "kyx`x"),
  ...
  it should "convert special functions only when forced to" in {
    ...
    the[CoreException] thrownBy KeYmaeraToMathematica("max(x,y)".asTerm) should have message
      "Core requirement failed: Interpreted functions not allowed in soundness-critical conversion to Mathematica"
    Configuration.set(Configuration.Keys.QE_ALLOW_INTERPRETED_FNS, "true", saveToFile = false)
    KeYmaeraToMathematica("abs(x)".asTerm) shouldBe
      makeExpr(new MExpr(Expr.SYMBOL, "Abs"), Array[MExpr](new MExpr(Expr.SYMBOL, "kyx`x")))
    KeYmaeraToMathematica("min(x,y)".asTerm) shouldBe makeExpr(
      new MExpr(Expr.SYMBOL, "Min"),
      Array[MExpr](new MExpr(Expr.SYMBOL, "kyx`x"), new MExpr(Expr.SYMBOL, "kyx`y")),
    )
    KeYmaeraToMathematica("max(x,y)".asTerm) shouldBe makeExpr(
      new MExpr(Expr.SYMBOL, "Max"),
      Array[MExpr](new MExpr(Expr.SYMBOL, "kyx`x"), new MExpr(Expr.SYMBOL, "kyx`y")),
    )
  }
  ...
  it should "distinguish uninterpreted names from interpreted ones by namespace" in {
    ...
    KeYmaeraToMathematica(Variable("Abs")) shouldBe new MExpr(Expr.SYMBOL, "kyx`Abs")
    a[CoreException] should be thrownBy KeYmaeraToMathematica(
      FuncOf(Function(InterpretedSymbols.absF.name, None, Unit, Real, InterpretedSymbols.absF.interp), Nothing)
    )
    KeYmaeraToMathematica("Abs(x)".asTerm) shouldBe
      makeExpr(new MExpr(Expr.SYMBOL, "kyx`Abs"), Array[MExpr](new MExpr(Expr.SYMBOL, "kyx`x")))
    KeYmaeraToMathematica("abs(x)".asTerm) shouldBe
      makeExpr(new MExpr(Expr.SYMBOL, "Abs"), Array[MExpr](new MExpr(Expr.SYMBOL, "kyx`x")))
    a[CoreException] should be thrownBy KeYmaeraToMathematica(FuncOf(
      Function(InterpretedSymbols.absF.name, None, Tuple(Real, Real), Real, InterpretedSymbols.absF.interp),
      Pair(Variable("x"), Variable("y")),
    ))
    KeYmaeraToMathematica(Variable("min")) shouldBe new MExpr(Expr.SYMBOL, "kyx`min")
    KeYmaeraToMathematica(Variable("Min")) shouldBe new MExpr(Expr.SYMBOL, "kyx`Min")
    a[CoreException] should be thrownBy KeYmaeraToMathematica(
      FuncOf(Function(InterpretedSymbols.minF.name, None, Real, Real, InterpretedSymbols.minF.interp), Variable("x"))
    )
    KeYmaeraToMathematica("Min(x,y)".asTerm) shouldBe makeExpr(
      new MExpr(Expr.SYMBOL, "kyx`Min"),
      Array[MExpr](new MExpr(Expr.SYMBOL, "kyx`x"), new MExpr(Expr.SYMBOL, "kyx`y")),
    )
    KeYmaeraToMathematica("min(x,y)".asTerm) shouldBe makeExpr(
      new MExpr(Expr.SYMBOL, "Min"),
    ...
    a[CoreException] should be thrownBy KeYmaeraToMathematica(FuncOf(
      Function(
        ...
        None,
        Tuple(Real, Tuple(Real, Real)),
        Real,
        InterpretedSymbols.minF.interp,
      ),
      Pair(Variable("x"), Pair(Variable("y"), Variable("z"))),
    ))
    ...
    KeYmaeraToMathematica(Variable("Max")) shouldBe new MExpr(Expr.SYMBOL, "kyx`Max")
    a[CoreException] should be thrownBy KeYmaeraToMathematica(
      FuncOf(Function(InterpretedSymbols.maxF.name, None, Real, Real, InterpretedSymbols.maxF.interp), Variable("x"))
    )
    KeYmaeraToMathematica("Max(x,y)".asTerm) shouldBe makeExpr(
      new MExpr(Expr.SYMBOL, "kyx`Max"),
      Array[MExpr](new MExpr(Expr.SYMBOL, "kyx`x"), new MExpr(Expr.SYMBOL, "kyx`y")),
    )
    KeYmaeraToMathematica("max(x,y)".asTerm) shouldBe makeExpr(
      new MExpr(Expr.SYMBOL, "Max"),
    ...
    a[CoreException] should be thrownBy KeYmaeraToMathematica(FuncOf(
      Function(
        ...
        Tuple(Real, Tuple(Real, Real)),
        Real,
        InterpretedSymbols.maxF.interp,
      ),
      Pair(Variable("x"), Pair(Variable("y"), Variable("z"))),
    ))
  }
  ...
  it should "convert special constants from Mathematica that are representable in KeYmaeraX" in {
    ...
    KeYmaeraToMathematica("exp(1)".asTerm) shouldBe
    ...
    KeYmaeraToMathematica("sin(x)".asTerm) shouldBe
      makeExpr(new MExpr(Expr.SYMBOL, "Sin"), Array[MExpr](new MExpr(Expr.SYMBOL, "kyx`x")))
    KeYmaeraToMathematica(FuncOf(InterpretedSymbols.PI, Nothing)) shouldBe (new MExpr(Expr.SYMBOL, "Pi"))
    ml.run(KeYmaeraToMathematica(FuncOf(InterpretedSymbols.sinF, FuncOf(InterpretedSymbols.PI, Nothing))))._2 shouldBe
      Number(0)
    ml.run(KeYmaeraToMathematica(
        FuncOf(InterpretedSymbols.cosF, Times(Number(2), FuncOf(InterpretedSymbols.PI, Nothing)))
      ))
      ._2 shouldBe Number(1)
  }

  it should "convert decimal to rational" in {
    KeYmaeraToMathematica("0.1".asTerm) shouldBe makeExpr(
      new MExpr(Expr.SYMBOL, "Rational"),
      Array(new MExpr(BigInt(1).bigInteger), new MExpr(BigInt(10).bigInteger)),
    )
    MathematicaToKeYmaera(KeYmaeraToMathematica("0.1".asTerm)) shouldBe "1/10".asTerm
    KeYmaeraToMathematica("1.5".asTerm) shouldBe makeExpr(
      new MExpr(Expr.SYMBOL, "Rational"),
      Array(new MExpr(BigInt(15).bigInteger), new MExpr(BigInt(10).bigInteger)),
    )
    MathematicaToKeYmaera(KeYmaeraToMathematica("1.5".asTerm)) shouldBe "15/10".asTerm
    KeYmaeraToMathematica("3.033".asTerm) shouldBe makeExpr(
      new MExpr(Expr.SYMBOL, "Rational"),
      Array(new MExpr(BigInt(3033).bigInteger), new MExpr(BigInt(1000).bigInteger)),
    )
  ...
  it should "convert non-long numbers to big ints" in {
    ...
    KeYmaeraToMathematica(Number(Number(Long.MaxValue).value + 1)) shouldBe
      new MExpr((BigInt(Long.MaxValue) + 1).bigInteger)
    KeYmaeraToMathematica(Number(Number(Long.MinValue).value - 1)) shouldBe
      new MExpr((BigInt(Long.MinValue) - 1).bigInteger)
    KeYmaeraToMathematica(Number(Double.MaxValue)) shouldBe
      new MExpr(scala.BigDecimal(Double.MaxValue).toBigInt.bigInteger)
  }
}
