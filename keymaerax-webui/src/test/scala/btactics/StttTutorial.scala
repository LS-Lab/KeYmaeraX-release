...
class StttTutorial extends TacticTestBase {
  ...
  "Example 1" should "be provable" in withQE { _ => withDatabase { db =>
    ...
    val tactic = implyR(Symbol("_")) & andL(Symbol("_")) & dC("v>=0".asFormula)(1) <(
      /* use */ dW(1) & prop,
      /* show */ dI()(1)
      )
  ...
  it should "be provable with diffInvariant" in withQE { _ => withDatabase { db =>
    val modelContent = ArchiveParser.getEntry("STTT16/Tutorial Example 1", io.Source.fromInputStream(
      getClass.getResourceAsStream("/examples/tutorials/sttt/sttt.kyx")).mkString).get.fileContent
    val tactic = implyR(Symbol("_")) & andL(Symbol("_")) & diffInvariant("v>=0".asFormula)(Symbol("R")) & dW(Symbol("R")) & prop
    db.proveBy(modelContent, tactic) shouldBe Symbol("proved")
  }}
  ...
  it should "be provable with multi-arg invariant" in withQE { _ => withDatabase { _ =>
    val modelContent = io.Source.fromInputStream(getClass.getResourceAsStream("/examples/tutorials/sttt/example1a.kyx")).mkString
    val tactic = implyR(Symbol("_")) & SaturateTactic(andL(Symbol("_"))) & diffInvariant("v>=0".asFormula :: "x>=old(x)".asFormula :: Nil)(1) &
      dW(1) & SaturateTactic(alphaRule) & exhaustiveEqL2R(Symbol("L"), "x0=x_0".asFormula) & prop
...
//    db.proveBy(modelContent, tactic) shouldBe 'proved
  ...
  "Example 2" should "be provable with master and custom loop invariant" in withQE { _ => withDatabase { db =>
    ...
    db.proveBy(modelContent, master(new ConfigurableGenerator(Map((loop, ("v>=0".asFormula, None)::Nil))))) shouldBe Symbol("proved")
  }}
  ...
  it should "be provable with chase etc" in withQE { _ => withDatabase { db =>
    ...
    val tactic = implyR(Symbol("R")) & SaturateTactic(andL(Symbol("L"))) &
      loop("v >= 0 & x+v^2/(2*B()) <= S()".asFormula)(Symbol("R")) <(
        ...
        printIndexed("Step") & chase(Symbol("R")) & printIndexed("After chase") & normalize & printIndexed("Normalized") &
          OnAll(solveEnd(Symbol("R"))) & printIndexed("After diffSolve") & OnAll(QE)
        )
  ...
  "Example 9b" should "be provable" in withQE { _ => withDatabase { db =>
    ...
    val tactic = implyR(Symbol("R")) & SaturateTactic(andL(Symbol("L"))) &
      loop("v >= 0 & xm <= x & xr = (xm + S())/2 & 5/4*(x-xr)^2 + (x-xr)*v/2 + v^2/4 < ((S() - xm)/2)^2".asFormula)(Symbol("R")) <(
        print("Base case") & SaturateTactic(andR(Symbol("R")) <(id, skip)) & id,
        print("Use case") & QE,
        print("Step") & SaturateTactic(andL(Symbol("L"))) & chase(Symbol("R")) & andR(Symbol("R")) <(
          allR(Symbol("R")) & SaturateTactic(implyR(Symbol("R"))) & ode & QE,
          implyR(Symbol("R")) & ode & QE
        )
      )
  ...
  "Example 10" should "FEATURE_REQUEST: be provable" in withQE { _ => withDatabase { db =>
    ...
    def hideQE = SaturateTactic(hideL(Symbol("Llike"), "dx_0^2+dy_0^2=1".asFormula)) & hideL(Symbol("L"), "c<=ep()".asFormula) & hideL(Symbol("L"), "r!=0".asFormula)
    ...
    val tactic = implyR(Symbol("R")) & SaturateTactic(andL(Symbol("L"))) &
      loop("v >= 0 & dx^2+dy^2 = 1 & r != 0 & abs(y-ly()) + v^2/(2*b()) < lw()".asFormula)(Symbol("R")) <(
        ...
        print("Step") & chase(Symbol("R")) & normalize & printIndexed("Normalized") <(
  ...
  it should "be provable with multi-arg diff. invariant" in withQE { _ => withDatabase { _ =>
    ...
    def ode(a: String) = diffInvariant("c>=0".asFormula :: "dx^2+dy^2=1".asFormula :: s"v=old(v)+$a*c".asFormula ::
      s"-c*(v-$a/2*c) <= y - old(y) & y - old(y) <= c*(v-$a/2*c)".asFormula :: Nil)(Symbol("R")) & dW(Symbol("R"))
