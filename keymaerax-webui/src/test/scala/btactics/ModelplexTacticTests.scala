...
//    println("Number of branches (open/all): " + proof.subgoals.size + "/" + numBranches(proof))
  ...
  it should "chase controller monitor condition to tests" in withMathematica { tool =>
    ...
    val monitorCode = (new CGenerator(new CMonitorGenerator('resist, Declaration(Map.empty))))(testProg2.subgoals.head.succ.head, stateVars.toSet, inputs, "Monitor")
  ...
  it should "generate a quantitative monitor" in withMathematica { tool =>
    ...
    val ctrlMonitorCode = (new CGenerator(new CMonitorGenerator('resist, entry.defs)))(ctrlMonitorProg, ctrlMonitorStateVars.toSet, ctrlInputs, "Monitor")
  ...
  it should "generate monitors from plant overapproximation" ignore withMathematica { tool =>
    ...
    val plantMonitorCode = (new CGenerator(new CMonitorGenerator('resist, entry.defs)))(plantMonitorProg, plantMonitorStateVars.toSet, plantInputs, "Monitor")
    //@todo update expected code
  ...
  "RSSRail" should "generate a controller monitor" in withMathematica { tool =>
    ...
    val ctrlMonitorProg = proveBy(reassociatedCtrlMonitorFml, ModelPlex.chaseToTests(combineTests=false)(1)*2).subgoals.head.succ.head
    val ctrlInputs = CodeGenerator.getInputs(ctrlMonitorProg)
    val ctrlMonitorCode = (new CGenerator(new CMonitorGenerator('resist, entry.defs)))(ctrlMonitorProg, stateVars.toSet, ctrlInputs, "Monitor")
