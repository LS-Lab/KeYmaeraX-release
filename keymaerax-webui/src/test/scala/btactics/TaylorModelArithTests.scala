...
class TaylorModelArithTests extends TacticTestBase {
  ...
  val context3 =
    ("-1 <= x0(), x0() <= 1, -1 <= y0(), y0() <= 1, -1 <= z0(), z0() <= 1," +
      "x = x0() + y0() + rx, -0.01 <= rx, rx <= 0.02," + "y = 0.5*x0() - y0() + ry, 0 <= ry, ry <= 0.1")
      .split(',')
      .map(_.asFormula)
      .toIndexedSeq
  ...
  it should "multiply approximately" in withMathematica { qeTool =>
    (tm1234 *! tm1234).prettyPrv.conclusion.succ(0) shouldBe
      "\\exists err_ (12.34*y*(12.34*y)=152.2756*y0()^2+-(152.27560*x0()*y0())+38.068900*x0()^2+err_&(-45.684)<=err_&err_<=47.207)"
        .asFormula
    (tm1234 * tm1234).prettyPrv.conclusion.succ(0) shouldBe
      "\\exists err_ (12.34*y*(12.34*y)=152.2*y0()^2+-(152.3*x0()*y0())+38.06*x0()^2+err_&(-45.709)<=err_&err_<=47.316)"
        .asFormula
  }
  ...
  it should "square approximately" in withMathematica { qeTool =>
    tm1234.squareExact.prettyPrv.conclusion.succ(0) shouldBe
      "\\exists err_ ((12.34*y)^2=152.2756*y0()^2+-(152.27560*x0()*y0())+38.068900*x0()^2+err_&(-45.683)<=err_&err_<=47.206)"
        .asFormula
    tm1234.square.prettyPrv.conclusion.succ(0) shouldBe
      "\\exists err_ ((12.34*y)^2=152.2*y0()^2+-(152.3*x0()*y0())+38.06*x0()^2+err_&(-45.708)<=err_&err_<=47.315)"
        .asFormula
  }
  ...
  it should "^(2*n)" in withMathematica { qeTool =>
    (tm1 ^ 4).prettyPrv.conclusion.succ(0) shouldBe
      "\\exists err_ (x^4=y0()^4+4*x0()*y0()^3+6*x0()^2*y0()^2+4*x0()^3*y0()+x0()^4+err_&(-0.64)<=err_&err_<=0.64967)"
        .asFormula
  }
  ...
  it should "exponentiate approximately" in withMathematica { qeTool =>
    (tm1234 ^ 3).prettyPrv.conclusion.succ(0) shouldBe
      "\\exists err_ ((12.34*y)^3=-(1879*y0()^3)+2818*x0()*y0()^2+-(1410*x0()^2*y0())+234.8*x0()^3+err_&(-1122.3)<=err_&err_<=1359.0)"
        .asFormula
  }
  ...
  it should "approx" in withMathematica { qeTool =>
    ...
    tmA.prettyPrv.conclusion.succ(0) shouldBe
      "\\exists err_ ((1/3*x+y)^2=0.4444*y0()^2+-(1.112*x0()*y0())+0.6944*x0()^2+err_&(-0.32093)<=err_&err_<=0.33240)"
        .asFormula
  }
  ...
  it should "prettyPrv" in withMathematica { _ =>
    println(
      TaylorModelArith
        .TM("e0".asTerm, PolynomialArithV2.ofTerm("e0".asTerm), Number(0), Number(0), IndexedSeq(), QE)
        .prettyPrv
    )
  }
  ...
  it should "evaluate" in withMathematica { _ =>
    val context =
      ("x = 1/3 * x0()^2 + 1/5 * x0() * y0() + 5/8 * y0()^2, -1 <= x0(), x0() <= 1, -1 <= y0(), y0() <= 1," +
        "x0() <= 1/100*y0() + 0.0001," + "x0() >= 1/100*y0()," + "y0() = 12.34")
        .split(',')
        .map(_.asFormula)
        .toIndexedSeq
    val x = TaylorModelArith
      .TM(
        "x".asTerm,
        PolynomialArithV2.ofTerm("1/3 * x0()^2 + 1/5 * x0() * y0() + 5/8 * y0()^2".asTerm),
        Number(0),
        Number(0),
        context,
        QE,
    ...
    x.evaluate(Seq(x0)).prettyPrv.conclusion.succ(0) shouldBe
      "\\exists err_ (x=0.6270*y0()^2+err_&0.0050751<=err_&err_<=0.0053640)".asFormula
    x.evaluate(Seq(y0)).prettyPrv.conclusion.succ(0) shouldBe
      "\\exists err_ (x=95.12+2.468*x0()+0.3333*x0()^2+err_&0.052250<=err_&err_<=0.052284)".asFormula
    x.evaluate(Seq(x0, y0)).prettyPrv.conclusion.succ(0) shouldBe
      "\\exists err_ (x=95.12+0.02468*y0()+0.00003333*y0()^2+err_&0.052250<=err_&err_<=0.052540)"
        .asFormula // @note insertion is simultaneous and not recursive
    x.evaluate(Seq(x0)).evaluate(Seq(y0)).prettyPrv.conclusion.succ(0) shouldBe
      "\\exists err_ (x=95.42+err_&0.061876<=err_&err_<=0.062166)".asFormula
  }
  ...
  it should "trimContext" in {
    TaylorModelArith
      .trimContext(
        "0 <= x(), x() <= 1, x <= y, -1 <= y(), y() <= 1".split(',').map(_.asFormula).toIndexedSeq,
        Seq("x()".asTerm, "y()".asTerm),
      )
      .loneElement shouldBe "x<=y".asFormula
  }
    ...
    withTemporaryConfig(Map(Configuration.Keys.QE_ALLOW_INTERPRETED_FNS -> "true")) {
      ...
      res.subgoals(0).ante shouldBe
        (contextE ++ IndexedSeq(
          ...
          "\\exists err_ (y = 2.4 + -(3.704 * s) + 0.05 * r1 + err_ & (-1467) * 10 ^ (-5) <= err_ & err_ <= 1258 * 10 ^ (-5))",
        ).map(_.asFormula))
      res.subgoals(0).succ.loneElement shouldBe "Safe(0 + s,y,x)".asFormula

      res.subgoals(1).ante shouldBe
        (contextE ++ IndexedSeq(
          "t = 0.01",
          "\\exists err_ (x = 1.424 + 0.15 * r0 + err_ & (-1020) * 10 ^ (-6) <= err_ & err_ <= 6286 * 10 ^ (-7))",
          "\\exists err_ (y = 2.36296 + 0.05 * r1 + err_ & (-1467) * 10 ^ (-5) <= err_ & err_ <= 1258 * 10 ^ (-5))",
        ).map(_.asFormula))
      res.subgoals(1).succ.loneElement shouldBe "[{x'=y, y'=(1 - x^2) * y - x, t'=1}]Safe(t,y,x)".asFormula
    }
  }
  ...
  "identityPrecondition" should "precondition" in withMathematica { _ =>
    ...
    val y = TM(
      "y".asTerm,
      ofTerm(
        "2.362222245 + 0.049486204 * r1 + -(0.011533035 * r0) + -(0.0002100 * r0 * r1) + -(0.0005400 * r0 ^ 2)".asTerm
      ),
      "(-3991) * 10^(-8)".asTerm,
      "1096 * 10^(-7)".asTerm,
      context,
    ...
    res.subgoals(0).ante shouldBe
      ("(-1)<=e0, e0<=1, (-1)<=e1, e1<=1, t=0.01," +
        ...
        "(-14964)*10^(-5)<=r0, r0<=15126*10^(-5)," + "(-61612)*10^(-6)<=r1, r1<=61683*10^(-6)")
        .split(',')
        .map(_.asFormula)
        .toIndexedSeq
    ...
    xs.map(_.prettyPrv.conclusion.succ(0)) shouldBe List(
      "\\exists err_ (x=1.423+r0+err_&0<=err_&err_<=0)".asFormula,
      "\\exists err_ (y=2.362+r1+err_&0<=err_&err_<=0)".asFormula,
    )
    rs.map(_.prettyPrv.conclusion.succ(0)) shouldBe List(
      "\\exists err_ (r0=0.0003980*e1+0.1349*e0+err_&(-0.014340)<=err_&err_<=0.015960)".asFormula,
      "\\exists err_ (r1=0.03958*e1+-(0.01039*e0)+-(0.0001512*e0*e1)+-(0.0004374*e0^2)+err_&(-0.011052)<=err_&err_<=0.011561)"
        .asFormula,
    )
