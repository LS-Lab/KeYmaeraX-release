...
class InvariantArithTests extends TacticTestBase {
  ...
  "ACASX Arith" should "prove ACAS X fml (division rewritten by hand)" in withMathematica { qeTool =>
    ...
    val tactic = implyR(1) & DebuggingTactics.print("Before normalizing") & prop &
      ...
      OnAll(DebuggingTactics.print("before") & prepareArith & DebuggingTactics.print("after")) &
      printGoal

    val res = proveBy(i, tactic)

    val g = res.subgoals(17)
    print(g)
  }
...
//        prind._1 match
//        {
//          case None => proveBy(res.subgoals(prind._2),QE)
//          case Some(w) =>
//            proveBy(res.subgoals(prind._2), nil)//w._1.foldLeft(nil)( _ & elim(_)) & witnessTac(w._2))
//        }
//      )
//
//    print(foo)
