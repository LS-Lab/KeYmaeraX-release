...
class LoopInvTests extends TacticTestBase {
  ...
  it should "prove x=0&v=0-> [{{v:=-1; ++ v:=1;};{x'=v&v>=0}}*]v>=0 by invariant v>=0" in withMathematica { _ =>
    ...
    proveBy(fml, implyR(1) &  loop("v>=0".asFormula)(1) <(QE, QE, chase(1) & unfoldProgramNormalize & OnAll(ODE(1)))) shouldBe Symbol("proved")
  }
  ...
  it should "prove x=0&v=0-> [{{v:=-1; ++ ?x<9;v:=1;};t:=0;{x'=v,t'=1&t<=1}}*]x<=10 by invariant x<=10" ignore withMathematica { _ =>
    ...
    proveBy(fml, implyR(1) & loop("x<=10".asFormula)(1) <(QE,QE, chase(1) & unfoldProgramNormalize & OnAll(solve(1) & QE))) shouldBe Symbol("proved")
    proveBy(fml, implyR(1) & loop("x<=10".asFormula)(1) <(QE,QE, chase(1) & unfoldProgramNormalize & OnAll(odeInvariant(1)))) shouldBe Symbol("proved")
  }
  ...
  it should "prove x=0&v=0&t=0-> [{{v:=-1; ++ ?x<9;v:=1;};t:=0;{x'=v,t'=1&t<=1}}*]x<=10 by invariant x<=10-t" ignore withMathematica { _ =>
    val fml = "x=0&v=0&t=0-> [{{v:=-1; ++ ?x<9;v:=1;};t:=0;{x'=v,t'=1&t<=1}}*]x<=10".asFormula
    proveBy(fml, implyR(1) & loop("x<=10-t&t>=0".asFormula)(1) <(QE,QE, chase(1) & unfoldProgramNormalize & OnAll(solve(1) & QE))) shouldBe Symbol("proved")
    //x<=10-t&t>=0 not an ODE invariant
    proveBy(fml, implyR(1) & loop("x<=10-t&t>=0".asFormula)(1) <(QE,QE, chase(1) & unfoldProgramNormalize & OnAll(odeInvariant(1)))) shouldBe Symbol("proved")
  }
  ...
  it should "prove the invariants of first a branch informative (scripted)" in withMathematica { _ =>
    ...
    proveBy(fml, implyR(1) & loop("v*v<=10-x".asFormula)(1) <(QE, QE, auto(TactixLibrary.invGenerator, None))) shouldBe Symbol("proved")
  }
  ...
  it should "FEATURE_REQUEST: find an invariant when first a branch informative" taggedAs TodoTest in withMathematica { _ =>
  ...
  it should "FEATURE_REQUEST: find an invariant when first a branch informative nosolve" taggedAs TodoTest in withMathematica { _ =>
    //todo: Pegasus doesn't generate the invariant for a:=1 case
    val fml = "x=0&v=0&a=0 -> [{{?10-x>=2+(4+v)*v; a:=1; ++ v:=0;a:=0; ++ a:=-1;};t:=0;{x'=v,v'=a,t'=1,z'=z&t<=1&v>=0}}*]x<=10".asFormula
    proveBy(fml, implyR(1) & loopPostMaster(InvariantGenerator.pegasusInvariants)(1)) shouldBe Symbol("proved")
  }
  ...
  it should "FEATURE_REQUEST: find an invariant for a simple time-triggered example" taggedAs TodoTest in withMathematica { _ =>
    val fml =
 ...
 it should "find an invariant for x>=5 & y>=0 -> [{x:=x+y;y:=y+1;}*]x>=0" in withMathematica { _ =>
  ...
  it should "FEATURE_REQUEST: prove x>=5 & y>=0 -> [{x:=x+y;y:=y+1;{x'=x^2+y,y'=x+1}}*]x>=0 by invariant x>=0&y>=0" taggedAs TodoTest in withMathematica{ _ =>
    ...
    proveBy(fml, implyR(1) & loop("x>=0&y>=0".asFormula)(1) <(QE, QE, composeb(1) & assignb(1) & composeb(1) & assignb(1) &
      odeInvariant(1))) shouldBe Symbol("proved")
  }
