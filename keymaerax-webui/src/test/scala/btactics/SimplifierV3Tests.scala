...
class SimplifierV3Tests extends TacticTestBase {
  ...
  "SimplifierV3" should "simplify repeated propositions under context" in withMathematica { _ =>
    ...
    proveBy(Sequent(ctxt, IndexedSeq(fml, fml, fml)), tactic(2)).subgoals should contain only Sequent(
      ctxt,
      IndexedSeq(
        "R()->P()&Q()->P()&(R()&P())&Q()&R()&P()&Z()&Y()".asFormula,
        "R()->P()&Q()->Z()".asFormula,
        "R()->P()&Q()->P()&(R()&P())&Q()&R()&P()&Z()&Y()".asFormula,
      ),
    )
  ...
  it should "do full sequent simplification" in withMathematica { _ =>
    ...
    pr.subgoals.loneElement shouldBe
      "x>=-k, ar>0, x*y=z+y, dhd-(a*t_+dho)=0 ==> P_()|Q_()|Q(), P_()|Q_()|Q(), true".asSequent
...
//      )
  ...
  it should "search for close heuristics" in withMathematica { _ =>
    val fml =
      " 0 > x -> x <= 0 & y = 0 & z<x -> x != y+z | x >= 5 -> 5 < x | (x !=5 -> 5<x ) & a = 0 & y = z+a+b & a+z+b = y"
        .asFormula
    val result = proveBy(fml, SimplifierV3.simpTac()(1))
  ...
  it should "allow controlled custom rewrites" in withMathematica { _ =>
    ...
    val result = proveBy(
      ...
      SimplifierV3.simpTac(List(custom1, custom2))(1) & SimplifierV3.simpTac(List(custom1, custom2))(1),
    )
  ...
  it should "simplify terms under quantifiers" in withMathematica { _ =>
    ...
    result.subgoals.loneElement shouldBe
      "x_0=0, v_0=5 ==> \\forall t \\forall s \\forall y (t>=0&0<=s&s<=t&y>0->x=v_0*t->x>=0)".asSequent
  }
  ...
  it should "handle modalities (poorly) " in withMathematica { _ =>
    ...
    val custom1 = proveBy("F_() = 0 -> (F_() = 0)".asFormula, TactixLibrary.QE)
    val fml =
      "<{x_'=v&q(x_)}>(z = 0 -> x_' * y + z >= x' + k) & [{x_'=v&q(x_)}](z = 0 -> x_' * y + z >= x' + k)".asFormula
    val ctxt = IndexedSeq("k=0".asFormula)
    val result = proveBy(Sequent(ctxt, IndexedSeq(fml)), SimplifierV3.simpTac(List(custom1))(1))
    result.subgoals.loneElement shouldBe
      "k=0 ==> <{x_'=v&q(x_)}>(z=0->x_'*y>=x'+k) & [{x_'=v&q(x_)}](z=0->x_'*y>=x'+k)".asSequent
  }
  ...
  it should "simplify terms" in withMathematica { _ =>
    ...
    result.subgoals.loneElement shouldBe
      "x_0=0, v_0=5 ==> \\forall t \\forall s (t>=0&0<=s&s<=t->x=v_0*t->x>=5)".asSequent
  }
  ...
  it should "simplify in multi-arg formula and term positions with arbitrary nesting" in withMathematica { _ =>
    val fml =
      "P( f(x+0,y,(0*z+0,a+0),b-0,c), k,(f(x+0,y,0*z+0,(a+0,b-0,c)),f(x+0,(y,0*z+0),a+0,(b-0,c))), (a,f(x+0,(y,0*z+0,a+0,b-0),c)))"
        .asFormula
    val ctxt = IndexedSeq()
    val tactic = simpTac()
    val result = proveBy(Sequent(ctxt, IndexedSeq(fml)), tactic(1))
    result.subgoals.loneElement shouldBe
      "==> P((f((x,(y,((0,a),(b,c))))),(k,((f((x,(y,(0,(a,(b,c)))))),f((x,((y,0),(a,(b,c)))))),(a,f((x,((y,(0,(a,b))),c))))))))"
        .asSequent
  ...
  it should "support equality rewriting" in withMathematica { _ =>
    ...
    val fml =
      "\\forall t (t = 0 -> (\\forall s (1 = s -> \\forall r (r = 5+s -> \\forall q (t+r = q -> r*s+t+a+b+t*r+q<=5+q+r+t+s+r+a+b)))))"
        .asFormula
    val ctxt = IndexedSeq()
    ...
    result.subgoals.loneElement shouldBe
      "==> \\forall t (t=0->\\forall s (1=s->\\forall r (r=6->\\forall q (6=q->6+a+b+6<=24+a+b))))".asSequent
  }
  ...
  it should "work in programs" in withMathematica { _ =>
    proveBy("==> [x:=1+0^3*x;]x>=0".asSequent, simpTac()(1, PosInExpr(0 :: 1 :: Nil))).subgoals.loneElement shouldBe
      "==> [x:=1;]x>=0".asSequent
  }

  it should "FEATURE_REQUEST: work in ODEs" taggedAs TodoTest in withMathematica { _ =>
    proveBy("x>=0 ==> [{x'=1+0*y}]x>=0".asSequent, simpTac()(1, PosInExpr(0 :: 0 :: 1 :: Nil)))
      .subgoals
      .loneElement shouldBe "x>=0 ==> [{x'=1}]x>=0".asSequent
    // @todo x is bound in ODE and occurs free in 0*x and therefore clashes/CQ aborts
    proveBy("x>=0 ==> [{x'=1+0*x}]x>=0".asSequent, simpTac()(1, PosInExpr(0 :: 0 :: 1 :: Nil)))
      .subgoals
      .loneElement shouldBe "x>=0 ==> [{x'=1}]x>=0".asSequent
  }
  ...
  "simplifyAllCtx" should "consider succedent when searching for contradictions" in withMathematica { _ =>
    proveBy("(x=1 | x=2) | x=3 ==> x=1 | x=2 | x=3".asSequent, simplifyAllCtx(-1)).subgoals.loneElement shouldBe
      "false, !x=3, !x=2, !x=1 ==>".asSequent
  }

  it should "simplify succedent" in withMathematica { _ =>
    proveBy("(x=1 & y=2) & z=3 ==> x=1 & y=2 & z=3".asSequent, simplifyAllCtx(1)).subgoals.loneElement shouldBe
      "z=3, x=1, y=2 ==> true".asSequent
  }

  it should "FEATURE_REQUEST: consider remaining succedent when simplifying a formula in the succedent" taggedAs
    TodoTest in withMathematica { _ =>
      proveBy("x=1 ==> x=1 & y=2, y!=2".asSequent, simplifyAllCtx(1)) shouldBe Symbol("proved")
    }

  it should "FEATURE_REQUEST: find contradictions hidden in double negations" taggedAs TodoTest in withMathematica {
    _ =>
