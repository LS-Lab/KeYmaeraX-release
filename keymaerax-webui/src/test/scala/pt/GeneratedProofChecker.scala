...
object countable {
  ...
  implicit def
    `Quickcheck_Exhaustive.countable_option`[A : countable]:
      countable[Option[A]]
    = new countable[Option[A]] {
  }
  implicit def
...
object enum {
  ...
  implicit def `Enum.enum_option`[A : enum]: enum[Option[A]] = new
    enum[Option[A]] {
    val `Enum.enum` = enum_optiona[A]
    val `Enum.enum_all` = (a: Option[A] => Boolean) => enum_all_option[A](a)
    val `Enum.enum_ex` = (a: Option[A] => Boolean) => enum_ex_option[A](a)
  }
...
def enum_all_option[A : enum](p: Option[A] => Boolean): Boolean =
  p(None) && enum_all[A](((x: A) => p(Some[A](x))))

def enum_ex_option[A : enum](p: Option[A] => Boolean): Boolean =
  p(None) || enum_ex[A](((x: A) => p(Some[A](x))))

def enum_optiona[A : enum]: List[Option[A]] =
  None :: Lista.map[A, Option[A]](((a: A) => Some[A](a)), enum[A])
...
object finite {
  ...
  implicit def `Optiona.finite_option`[A : finite]: finite[Option[A]] = new
    finite[Option[A]] {
  }
  implicit def
...
final case class NotR[A, B, C]() extends rrule[A, B, C]
...
final case class
  DIGeqSchema[A, C,
               B](a: Syntax.ODE[A, C], b: Syntax.trm[A, C], c: Syntax.trm[A, C])
  extends ruleApp[A, B, C]
...
final case class AassignAny() extends axiom
...
final case class
  subst_exta[A, C, B,
              D](a: A => Option[Syntax.trm[Sum_Type.sum[A, C], C]],
                  ...
                  f: C => Option[C] => Option[Syntax.ODE[A, C]], g: D)
...
def FVS[A : Enum.enum : HOL.equal, B : Enum.enum : HOL.equal,
         C : Enum.enum : HOL.equal](sigma: subst_ext[A, B, C, Unit]):
      Set.set[Sum_Type.sum[C, C]]
  =
  Complete_Lattices.Sup_set[Sum_Type.sum[C,
  C]](Set.image[Sum_Type.sum[A, Sum_Type.sum[B, C]],
                 Set.set[Sum_Type.sum[C, C]]](((a:
          Sum_Type.sum[A, Sum_Type.sum[B, C]])
         =>
        SFV[A, B, C](sigma, a)),
       Set.top_set[Sum_Type.sum[A, Sum_Type.sum[B, C]]]))
...
def OsubstFO[A, B,
  ...
  =
  (x0, sigma) match {
  case (Syntax.OVar(c, sp), sigma) => Syntax.OVar[C, A](c, sp)
...
def PsubstFO[A, B, C,
  ...
  case (Syntax.AssignAny(x), sigma) => Syntax.AssignAny[D, A, C](x)
...
def PPsubst[A, B, C,
  ...
  case (Syntax.AssignAny(x), sigma) => Syntax.AssignAny[D, A, B](x)
  case (Syntax.DiffAssign(x, theta), sigma) =>
...
def Tsubst[A, B, C](x0: Syntax.trm[A, B], sigma: subst_ext[A, C, B, Unit]):
  ...
  case (Syntax.Functional(f), sigma) =>
    ((SFunls[A, C, B, Unit](sigma)).apply(f) match {
       case None => Syntax.Functional[A, B](f)
       case Some(fa) => fa
     })
  case (Syntax.Plus(theta_1, theta_2), sigma) =>
...
def SODEs[A, B, C, D](x0: subst_ext[A, B, C, D]):
      C => Option[C] => Option[Syntax.ODE[A, C]]
  =
  x0 match {
  case subst_exta(sFunctions, sFunls, sPredicates, sContexts, sPrograms, sODEs,
                   more)
...
def Osubst[A, B, C](x0: Syntax.ODE[A, B], sigma: subst_ext[A, C, B, Unit]):
  ...
  =
  (x0, sigma) match {
  case (Syntax.OVar(c, sp), sigma) =>
    ((SODEs[A, C, B, Unit](sigma)).apply(c).apply(sp) match {
       case None => Syntax.OVar[B, A](c, sp)
       case Some(ca) => ca
     })
  case (Syntax.OSing(x, theta), sigma) =>
...
def Psubst[A, B, C](x0: Syntax.hp[A, B, C], sigma: subst_ext[A, B, C, Unit]):
  ...
  case (Syntax.AssignAny(x), sigma) => Syntax.AssignAny[C, A, B](x)
  case (Syntax.DiffAssign(x, theta), sigma) =>
...
def ddl_OUrename(x: myvars, y: myvars, xa2: Syntax.ODE[myvars, myvars]):
  ...
  =
  (x, y, xa2) match {
  case (x, y, Syntax.OVar(c, sp)) => sys.error("undefined")
...
def ddl_PUrename(x: myvars, y: myvars, xa2: Syntax.hp[myvars, myvars, myvars]):
  ...
  case (x, y, Syntax.AssignAny(z)) =>
    Syntax.AssignAny[myvars, myvars, myvars](ddl_swap(x, y, z))
...
myvars](Syntax.Pvar[myvars, myvars, myvars](vc), vb))
  case (x, y, Syntax.Not(Syntax.Diamond(Syntax.AssignAny(vc), vb))) =>
    Syntax.Not[myvars, myvars,
                myvars](Syntax.Diamond[myvars, myvars,
myvars](Syntax.AssignAny[myvars, myvars, myvars](vc), vb))
  ...
  Predicate.sup_pred[Unit](Predicate.bind[Syntax.ODE[myvars, myvars],
   Unit](Predicate.single[Syntax.ODE[myvars, myvars]](xa),
          ((a: Syntax.ODE[myvars, myvars]) =>
            (a match {
               case Syntax.OVar(_, _) => Predicate.bot_pred[Unit]
...
((aa: Unit) => {
               ...
               case Syntax.AssignAny(_) => Predicate.bot_pred[Unit]
 ...
 case Syntax.AssignAny(_) => Predicate.single[Unit](())
 case Syntax.DiffAssign(_, _) => Predicate.bot_pred[Unit]
 case Syntax.Test(_) => Predicate.bot_pred[Unit]
 case Syntax.EvolveODE(_, _) => Predicate.bot_pred[Unit]
 case Syntax.Choice(_, _) => Predicate.bot_pred[Unit]
                      ...
                      ((a: Syntax.hp[myvars, myvars, myvars]) =>
                        (a match {
                           ...
                           case Syntax.DiffAssign(_, theta) =>
                             Predicate.bind[Unit,
     Unit](ddl_TRadmit_i(theta), ((aa: Unit) => {
          val (): Unit = aa;
          Predicate.single[Unit](())
        }))
                           case Syntax.Test(_) => Predicate.bot_pred[Unit]
                                      ...
                                      ((aa: Unit) =>
{
 ...
 Predicate.single[Unit](())
                         ...
                         case Syntax.Loop(_) => Predicate.bot_pred[Unit]
                                      ...
                                      Predicate.sup_pred[Unit](Predicate.bind[Syntax.hp[myvars,
         myvars, myvars],
                                       Unit](Predicate.single[Syntax.hp[myvars,
                                 myvars, myvars]](xa),
      ((a: Syntax.hp[myvars, myvars, myvars]) =>
        (a match {
           case Syntax.Pvar(_) => Predicate.bot_pred[Unit]
...
Predicate.single[Unit](())
           ...
           case Syntax.Loop(_) => Predicate.bot_pred[Unit]
                        ...
                        Predicate.bind[Syntax.hp[myvars, myvars, myvars],
Unit](Predicate.single[Syntax.hp[myvars, myvars, myvars]](xa),
       ((a: Syntax.hp[myvars, myvars, myvars]) =>
         (a match {
            case Syntax.Pvar(_) => Predicate.bot_pred[Unit]
...
def fradmit_i(xa: Syntax.formula[myvars, myvars, myvars]): Predicate.pred[Unit]
  =
  ...
  myvars]](Sum_Type.Inl[myvars, myvars](x),
            Static_Semantics.FVSeq[myvars, myvars, myvars]((a, s)))) &&
                (Syntax.fsafe[myvars, myvars,
                               myvars]((Lista.foldr[Syntax.formula[myvars,
                            myvars, myvars],
             Syntax.formula[myvars, myvars,
                             myvars]](((aa:
  Syntax.formula[myvars, myvars, myvars])
 =>
(b: Syntax.formula[myvars, myvars, myvars]) =>
Syntax.And[myvars, myvars, myvars](aa, b)),
                                       a)).apply(Syntax.TT[myvars, myvars,
                    myvars])) &&
                  Syntax.fsafe[myvars, myvars,
                                myvars]((Lista.foldr[Syntax.formula[myvars,
                             myvars, myvars],
  ...
  =>
 (b: Syntax.formula[myvars, myvars, myvars]) =>
 Syntax.Or[myvars, myvars, myvars](aa, b)),
Proof_Checker.closeI[Syntax.formula[myvars, myvars,
                                     myvars]](s,
       j))).apply(Syntax.FF[myvars, myvars, myvars]))))
           Some[List[(List[Syntax.formula[myvars, myvars, myvars]],
                       List[Syntax.formula[myvars, myvars,
    myvars]])]](List((a, Proof_Checker.replaceI[Syntax.formula[myvars, myvars,
                        myvars]](s, j, p))))
...
def ddl_merge_rules(x0: (List[(List[Syntax.formula[myvars, myvars, myvars]],
  ...
  case ((p1, c1), (Nil, c2), i) =>
   ...
   List[Syntax.formula[myvars, myvars, myvars]])].apply(p1)) &&
           Product_Type.equal_proda[List[Syntax.formula[myvars, myvars,
                 myvars]],
                                     List[Syntax.formula[myvars, myvars,
                  myvars]]](Lista.nth[(List[Syntax.formula[myvars, myvars,
                    myvars]],
List[Syntax.formula[myvars, myvars, myvars]])](p1, i),
                             c2))
      Some[(List[(List[Syntax.formula[myvars, myvars, myvars]],
                   List[Syntax.formula[myvars, myvars, myvars]])],
             (List[Syntax.formula[myvars, myvars, myvars]],
...
def ddl_rule_result(x0: (List[(List[Syntax.formula[myvars, myvars, myvars]],
  ...
  case ((sg, c), (i, Proof_Checker.Cut(phi))) =>
    (if (Syntax.fsafe[myvars, myvars, myvars](phi))
      {
        ...
        val (aa, s):
          ...
          = a;
        ddl_merge_rules((sg, c),
                         (List((aa ++ List(phi), s), (aa, s ++ List(phi))),
                           (aa, s)),
                         i)
      }
      else None)
...
j),
 ...
 k))))
  ...
  case ((sg, c), (i, Proof_Checker.URename(x, ya))) =>
    (if (ddl_FRadmit(Denotational_Semantics.seq2fml[myvars, myvars,
             myvars](Lista.nth[(List[Syntax.formula[myvars, myvars, myvars]],
                                 List[Syntax.formula[myvars, myvars,
              myvars]])](sg, i))) &&
           (ddl_FRadmit(Denotational_Semantics.seq2fml[myvars, myvars,
                myvars](ddl_SUrename(x, ya,
                                      Lista.nth[(List[Syntax.formula[myvars,
                              myvars, myvars]],
          List[Syntax.formula[myvars, myvars, myvars]])](sg, i)))) &&
             Syntax.fsafe[myvars, myvars,
                           myvars](Denotational_Semantics.seq2fml[myvars,
                           myvars,
                           myvars](ddl_SUrename(x, ya,
         Lista.nth[(List[Syntax.formula[myvars, myvars, myvars]],
                     List[Syntax.formula[myvars, myvars, myvars]])](sg, i))))))
      ddl_merge_rules((sg, c),
                       (List(ddl_SUrename(x, ya,
   Lista.nth[(List[Syntax.formula[myvars, myvars, myvars]],
               List[Syntax.formula[myvars, myvars, myvars]])](sg, i))),
                         Lista.nth[(List[Syntax.formula[myvars, myvars,
                 myvars]],
    ...
    (if (Nat.less_eq_nat(Lista.size_list[Syntax.formula[myvars, myvars,
      ...
      fail()
             ...
             List[Syntax.formula[myvars, myvars,
                ...
                List[Syntax.formula[myvars, myvars, myvars]])](sg, i)),
                                    j)),
                                    List(Lista.nth[Syntax.formula[myvars,
                           myvars,
                           myvars]](Product_Type.snd[List[Syntax.formula[myvars,
                                  myvars, myvars]],
              List[Syntax.formula[myvars, myvars,
  ...
  List[Syntax.formula[myvars, myvars, myvars]])](sg, i)),
                            i))
  case ((sg, c), (i, Proof_Checker.DIGeqSchema(ode, theta_1, theta_2))) =>
    {
      val proved:
            (List[Syntax.formula[myvars, myvars, myvars]],
              List[Syntax.formula[myvars, myvars, myvars]])
  ...
  myvars](Syntax.Geq[myvars, myvars, myvars](theta_1, theta_2),
    ...
    myvars](ode, Syntax.DPredl[myvars, myvars, myvars](x)),
      ...
      val wanted:
        ...
        = Lista.nth[(List[Syntax.formula[myvars, myvars, myvars]],
                      List[Syntax.formula[myvars, myvars, myvars]])](sg, i);
      (if (Product_Type.equal_proda[List[Syntax.formula[myvars, myvars,
                 myvars]],
                                     List[Syntax.formula[myvars, myvars,
                  myvars]]](proved, wanted) &&
             (Syntax.osafe[myvars, myvars](ode) &&
               (Syntax.dfree[myvars, myvars](theta_1) &&
                 (Syntax.dfree[myvars, myvars](theta_2) &&
                   (Set.less_eq_set[Sum_Type.sum[myvars,
          myvars]](Static_Semantics.FVT[myvars, myvars](theta_1),
                    Set.image[myvars,
                               Sum_Type.sum[myvars,
     myvars]](((a: myvars) => Sum_Type.Inl[myvars, myvars](a)),
               Syntax.ODE_dom[myvars, myvars](ode))) &&
                     Set.less_eq_set[Sum_Type.sum[myvars,
           myvars]](Static_Semantics.FVT[myvars, myvars](theta_2),
                     Set.image[myvars,
                                Sum_Type.sum[myvars,
      myvars]](((a: myvars) => Sum_Type.Inl[myvars, myvars](a)),
                Syntax.ODE_dom[myvars, myvars](ode))))))))
...
def osafe_i[A, B : Enum.enum : HOL.equal](xa: Syntax.ODE[A, B]):
  ...
  Predicate.sup_pred[Unit](Predicate.bind[Syntax.ODE[A, B],
   Unit](Predicate.single[Syntax.ODE[A, B]](xa),
          ((a: Syntax.ODE[A, B]) =>
            (a match {
               case Syntax.OVar(_, _) => Predicate.single[Unit](())
               case Syntax.OSing(_, _) => Predicate.bot_pred[Unit]
               case Syntax.OProd(_, _) => Predicate.bot_pred[Unit]
             }))),
           ...
           Unit](Predicate.single[Syntax.hp[A, B, C]](xa),
                  ((a: Syntax.hp[A, B, C]) =>
                    (a match {
                       ...
                       case Syntax.Loop(aa) =>
                         Predicate.bind[Unit,
 Unit](hpsafe_i[A, B, C](aa), ((ab: Unit) => {
       val (): Unit = ab;
       Predicate.single[Unit](())
     }))
                     }))),
   ...
   Unit](Predicate.single[(A => Syntax.formula[B, C, D],
          ...
          ((a: (A => Syntax.formula[B, C, D],
            ...
            (a match {
               ...
               case (_, Syntax.AssignAny(_)) => Predicate.bot_pred[Unit]
...
Syntax.hp[B, Sum_Type.sum[C, A], D]),
      ...
      ((a: (A => Syntax.formula[B, C, D], Syntax.hp[B, Sum_Type.sum[C, A], D]))
        ...
        (a match {
           case (_, Syntax.Pvar(_)) => Predicate.bot_pred[Unit]
           case (_, Syntax.Assign(_, _)) => Predicate.bot_pred[Unit]
           case (_, Syntax.AssignAny(_)) => Predicate.single[Unit](())
           case (_, Syntax.DiffAssign(_, _)) => Predicate.bot_pred[Unit]
           case (_, Syntax.Test(_)) => Predicate.bot_pred[Unit]
           case (_, Syntax.EvolveODE(_, _)) => Predicate.bot_pred[Unit]
           case (_, Syntax.Choice(_, _)) => Predicate.bot_pred[Unit]
  ...
  Predicate.sup_pred[Unit](Predicate.bind[(A => Syntax.trm[B, C],
   ...
   Unit](Predicate.single[(A => Syntax.trm[B, C],
          ...
          ((a: (A => Syntax.trm[B, C],
             ...
             =>
            (a match {
               case (sigma, (Syntax.OVar(c, sp), u)) =>
                 Predicate.bind[Unit,
                                 Unit](Predicate.if_pred(USubst.OUadmitFO[A, B,
                                   C](sigma,
                                       Syntax.OVar[C,
            Sum_Type.sum[B, A]](c, sp),
                                       u)),
((aa: Unit) => {
                 val (): Unit = aa;
                 Predicate.single[Unit](())
...
(A => Syntax.trm[B, C],
  (Syntax.ODE[Sum_Type.sum[B, A], C], Set.set[Sum_Type.sum[C, C]])))
                                       =>
                                      (a match {
 case (_, (Syntax.OVar(_, _), _)) => Predicate.bot_pred[Unit]
...
def oadmit_i[A : Enum.enum : HOL.equal, B,
              C : Enum.enum : HOL.equal](xa: USubst.subst_ext[A, B, C, Unit],
  ...
  Predicate.sup_pred[Unit](Predicate.bind[(USubst.subst_ext[A, B, C, Unit],
   ...
   Unit](Predicate.single[(USubst.subst_ext[A, B, C, Unit],
          ...
          ((a: (USubst.subst_ext[A, B, C, Unit],
             ...
             =>
            (a match {
               case (_, (Syntax.OVar(_, None), _)) => Predicate.single[Unit](())
               case (_, (Syntax.OVar(_, Some(_)), _)) =>
                 Predicate.bot_pred[Unit]
               case (_, (Syntax.OSing(_, _), _)) => Predicate.bot_pred[Unit]
               case (_, (Syntax.OProd(_, _), _)) => Predicate.bot_pred[Unit]
             }))),
...
(USubst.subst_ext[A, B, C, Unit],
  (Syntax.ODE[A, C], Set.set[Sum_Type.sum[C, C]])))
                                       =>
                                      (a match {
 case (_, (Syntax.OVar(_, None), _)) => Predicate.bot_pred[Unit]
 case (sigma, (Syntax.OVar(c, Some(x)), _)) =>
   Predicate.bind[Unit,
                   Unit](Predicate.if_pred(((USubst.SODEs[A, B, C,
                   Unit](sigma)).apply(c).apply(Some[C](x))
      match {
      case None => false
      case Some(ode) =>
        ! (Set.member[Sum_Type.sum[C, C]](Sum_Type.Inl[C, C](x),
   Static_Semantics.BVO[A, C](ode)))
    })),
                          ((aa: Unit) => {
   val (): Unit = aa;
   Predicate.single[Unit](())
...
(Syntax.ODE[A, C], Set.set[Sum_Type.sum[C, C]]))]((xa, (xb, xc))),
                      ((a: (USubst.subst_ext[A, B, C, Unit],
                         ...
                         =>
                        (a match {
                           case (_, (Syntax.OVar(_, _), _)) =>
                             Predicate.bot_pred[Unit]
                           case (sigma, (Syntax.OSing(_, theta), u)) =>
                             Predicate.bind[Unit,
     Unit](Predicate.if_pred(USubst.TUadmit[A, B, C](sigma, theta, u)),
            ((aa: Unit) =>
              {
                val (): Unit = aa;
                Predicate.bind[Unit,
 ...
 (Syntax.ODE[A, C], Set.set[Sum_Type.sum[C, C]]))]((xa, (xb, xc))),
                       ((a: (USubst.subst_ext[A, B, C, Unit],
                         ...
                         (a match {
                            case (_, (Syntax.OVar(_, _), _)) =>
                              Predicate.bot_pred[Unit]
                            case (_, (Syntax.OSing(_, _), _)) =>
                              Predicate.bot_pred[Unit]
                            case (sigma, (Syntax.OProd(oDE1, oDE2), u)) =>
                              Predicate.bind[Unit,
      Unit](oadmit_i[A, B, C](sigma, oDE1, u),
             ((aa: Unit) =>
               {
                 val (): Unit = aa;
                 Predicate.bind[Unit,
                                 Unit](oadmit_i[A, B, C](sigma, oDE2, u),
((ab: Unit) =>
  {
    val (): Unit = ab;
    Predicate.bind[Unit,
                    Unit](eq_i_i[Set.set[C]](Set.inf_set[C](Syntax.ODE_dom[A,
                     ...
                     Syntax.ODE_dom[A, C](USubst.Osubst[A, C, B](oDE2, sigma))),
      Set.bot_set[C]),
                           ((ac: Unit) => {
    val (): Unit = ac;
    Predicate.single[Unit](())
...
Syntax.hp[A, B, C]),
 ...
 B, C, Unit],
  ...
  Predicate.single[Unit](())
          ...
          Predicate.bind[(USubst.subst_ext[A, B, C, Unit], Syntax.hp[A, B, C]),
                          Unit](Predicate.single[(USubst.subst_ext[A, B, C,
                            Unit],
           Syntax.hp[A, B, C])]((xa, xb)),
                                 ((a: (USubst.subst_ext[A, B, C, Unit],
...
def ddl_Rsafe(r: (List[(List[Syntax.formula[myvars, myvars, myvars]],
                         List[Syntax.formula[myvars, myvars, myvars]])],
                   (List[Syntax.formula[myvars, myvars, myvars]],
                     List[Syntax.formula[myvars, myvars, myvars]]))):
      Boolean
  ...
  Lista.foldl[Boolean,
               (List[Syntax.formula[myvars, myvars, myvars]],
                 List[Syntax.formula[myvars, myvars,
                                      myvars]])](((acc: Boolean) =>
           (x: (List[Syntax.formula[myvars, myvars, myvars]],
                 List[Syntax.formula[myvars, myvars, myvars]]))
             =>
           acc && Syntax.Ssafe[myvars, myvars, myvars](x)),
          true,
          Product_Type.fst[List[(List[Syntax.formula[myvars, myvars, myvars]],
               ...
               myvars]])],
                            (List[Syntax.formula[myvars, myvars, myvars]],
                              List[Syntax.formula[myvars, myvars,
           myvars]])](r)) &&
    Syntax.Ssafe[myvars, myvars,
                  myvars](Product_Type.snd[List[(List[Syntax.formula[myvars,
                              myvars, myvars]],
          List[Syntax.formula[myvars, myvars, myvars]])],
    (List[Syntax.formula[myvars, myvars, myvars]],
      List[Syntax.formula[myvars, myvars, myvars]])](r))
 ...
 Unit](sigma)).apply(i)
  ...
  Unit](sigma)).apply(f)
       ...
       Unit](sigma)).apply(f)
          ...
          (HOL.All[myvars](((f: myvars) =>
                             HOL.All[Option[myvars]](((sp: Option[myvars]) =>
               ((USubst.SODEs[myvars, myvars, myvars,
                               Unit](sigma)).apply(f).apply(sp)
                  match {
                  case None => true
                  case Some(a) => Syntax.osafe[myvars, myvars](a)
                }))))) &&
            (HOL.All[myvars](((f: myvars) =>
                               HOL.All[myvars](((x: myvars) =>
         ((USubst.SODEs[myvars, myvars, myvars,
                         Unit](sigma)).apply(f).apply(Some[myvars](x))
            match {
            case None => true
            case Some(fa) =>
              ! (Set.member[Sum_Type.sum[myvars,
  myvars]](Sum_Type.Inl[myvars, myvars](x),
            Static_Semantics.BVO[myvars, myvars](fa)))
          }))))) &&
...
def ddl_DCaxiom: Syntax.formula[myvars, myvars, myvars] =
  Syntax.Implies[myvars, myvars,
                  myvars](Syntax.Box[myvars, myvars,
                                      myvars](Syntax.EvolveODE[myvars, myvars,
                        myvars](Syntax.OVar[myvars, myvars](x, None),
                                 Syntax.Predicational[myvars, myvars,
               myvars](i2())),
...
def ddl_assignAnyAxiom: Syntax.formula[myvars, myvars, myvars] =
  Syntax.Equiv[myvars, myvars,
                myvars](Syntax.Box[myvars, myvars,
                                    myvars](Syntax.AssignAny[myvars, myvars,
                      myvars](x),
     Syntax.Predicational[myvars, myvars, myvars](i1())),
                         Syntax.Forall[myvars, myvars,
...
def ddl_get_axiom(x0: Proof_Checker.axiom):
  ...
  case Proof_Checker.AassignAny() => ddl_assignAnyAxiom
}
          ...
          myvars, myvars]],
                        ...
                        (List[Syntax.formula[myvars, myvars, myvars]],
                          List[Syntax.formula[myvars, myvars, myvars]])](res)),
                               i))
           None else ddl_rule_result(res, (i, ra)))
     })
  case Proof_Checker.AxiomaticRule(ar) =>
    Some[(List[(List[Syntax.formula[myvars, myvars, myvars]],
                 List[Syntax.formula[myvars, myvars, myvars]])],
           (List[Syntax.formula[myvars, myvars, myvars]],
       ...
       case Some(res) =>
         (if (ddl_ssafe(sub) &&
                (ddl_Radmit(sub, res) &&
                  (ddl_Rsafe(res) &&
                    (Set.equal_seta[Sum_Type.sum[myvars,
          myvars]](USubst.FVS[myvars, myvars, myvars](sub),
                    Set.bot_set[Sum_Type.sum[myvars, myvars]]) ||
                      (Lista.nulla[(List[Syntax.formula[myvars, myvars,
                 myvars]],
                                     List[Syntax.formula[myvars, myvars,
                  myvars]])](Product_Type.fst[List[(List[Syntax.formula[myvars,
                                 myvars, myvars]],
             List[Syntax.formula[myvars, myvars, myvars]])],
       (List[Syntax.formula[myvars, myvars, myvars]],
         List[Syntax.formula[myvars, myvars, myvars]])](res)) ||
                        Product_Type.equal_proda[List[(List[Syntax.formula[myvars,
                                    myvars, myvars]],
                List[Syntax.formula[myvars, myvars, myvars]])],
          (List[Syntax.formula[myvars, myvars, myvars]],
            List[Syntax.formula[myvars, myvars,
                                 myvars]])](res, ddl_CQaxrule))))))
           Some[(List[(List[Syntax.formula[myvars, myvars, myvars]],
                        List[Syntax.formula[myvars, myvars, myvars]])],
                  (List[Syntax.formula[myvars, myvars, myvars]],
                    List[Syntax.formula[myvars, myvars,
 myvars]]))](ddl_Rsubst(res, sub))
           else None)
     })
  case Proof_Checker.Ax(a) =>
    Some[(List[(List[Syntax.formula[myvars, myvars, myvars]],
           ...
           (List[Syntax.formula[myvars, myvars, myvars]],
             List[Syntax.formula[myvars, myvars,
                                  myvars]]))]((Nil,
        (Nil, List(ddl_get_axiom(a)))))
  case Proof_Checker.FNC(pt, seq, ra) =>
    (ddl_pt_result(pt) match {
       case None => None
       case Some(res) => ddl_fnc(res, seq, ra)
     })
          ...
          List[Syntax.formula[myvars, myvars,
            ...
            myvars, myvars]],
                          ...
                          (List[Syntax.formula[myvars, myvars, myvars]],
                            List[Syntax.formula[myvars, myvars,
         myvars]])](res2)))))
           None else (ddl_pt_result(pt1) match {
                        case None => None
                        case Some(res1) => ddl_pro(res1, res2)
                      }))
     })
  case Proof_Checker.Start(f) =>
    Some[(List[(List[Syntax.formula[myvars, myvars, myvars]],
                 List[Syntax.formula[myvars, myvars, myvars]])],
           (List[Syntax.formula[myvars, myvars, myvars]],
             List[Syntax.formula[myvars, myvars, myvars]]))](ddl_start_proof(f))
...
object Denotational_Semantics {

def seq2fml[A, B,
             C](x0: (List[Syntax.formula[A, B, C]],
                      List[Syntax.formula[A, B, C]])):
      Syntax.formula[A, B, C]
  =
  x0 match {
  case (ante, succ) =>
    Syntax.Implies[A, B,
                    C]((Lista.foldr[Syntax.formula[A, B, C],
                                     Syntax.formula[A, B,
             C]](((a: Syntax.formula[A, B, C]) =>
                   (b: Syntax.formula[A, B, C]) => Syntax.And[A, B, C](a, b)),
                  ante)).apply(Syntax.TT[A, B, C]),
                        (Lista.foldr[Syntax.formula[A, B, C],
                                      Syntax.formula[A, B,
              C]](((a: Syntax.formula[A, B, C]) =>
                    (b: Syntax.formula[A, B, C]) => Syntax.Or[A, B, C](a, b)),
                   succ)).apply(Syntax.FF[A, B, C]))
}
...
def BVO[A, B : HOL.equal](x0: Syntax.ODE[A, B]): Set.set[Sum_Type.sum[B, B]] =
  x0 match {
  case Syntax.OVar(c, Some(x)) =>
    Set.uminus_set[Sum_Type.sum[B, B]](Set.insert[Sum_Type.sum[B,
                        B]](Sum_Type.Inl[B, B](x),
                             Set.insert[Sum_Type.sum[B,
              B]](Sum_Type.Inr[B, B](x), Set.bot_set[Sum_Type.sum[B, B]])))
  case Syntax.OVar(c, None) => Set.top_set[Sum_Type.sum[B, B]]
...
def MBV[A, B, C : Enum.enum : HOL.equal](x0: Syntax.hp[A, B, C]):
  ...
  case Syntax.AssignAny(v) => BVP[A, B, C](Syntax.AssignAny[C, A, B](v))
...
def FVO[A, B : Enum.enum : HOL.equal](x0: Syntax.ODE[A, B]): Set.set[B] = x0
  match {
  case Syntax.OVar(c, Some(x)) => Set.top_set[B]
  case Syntax.OVar(c, None) => Set.top_set[B]
  case Syntax.OSing(x, theta) =>
...
def FVP[A, B, C : Enum.enum : HOL.equal](x0: Syntax.hp[A, B, C]):
  ...
  case Syntax.AssignAny(x) => Set.bot_set[Sum_Type.sum[C, C]]
  case Syntax.DiffAssign(x, theta) => FVT[A, C](theta)
  case Syntax.Test(phi) => FVF[A, B, C](phi)
  case Syntax.EvolveODE(ode, phi) =>
    Set.sup_set[Sum_Type.sum[C, C]](Set.image[C,
       Sum_Type.sum[C, C]](((a: C) => Sum_Type.Inl[C, C](a)), FVO[A, C](ode)),
                                     FVF[A, B, C](phi))
  case Syntax.Choice(alpha, beta) =>
...
(List[Syntax.formula[A, B, C]], List[Syntax.formula[A, B, C]])):
  ...
  x0 match {
  case (a, s) =>
    Set.sup_set[Sum_Type.sum[C, C]]((Lista.foldr[Syntax.formula[A, B, C],
          Set.set[Sum_Type.sum[C, C]]](((x: Syntax.formula[A, B, C]) =>
 (acc: Set.set[Sum_Type.sum[C, C]]) =>
 Set.sup_set[Sum_Type.sum[C, C]](acc, FVF[A, B, C](x))),
a)).apply(Set.bot_set[Sum_Type.sum[C, C]]),
                                     (Lista.foldr[Syntax.formula[A, B, C],
           Set.set[Sum_Type.sum[C, C]]](((x: Syntax.formula[A, B, C]) =>
  (acc: Set.set[Sum_Type.sum[C, C]]) =>
  Set.sup_set[Sum_Type.sum[C, C]](acc, FVF[A, B, C](x))),
 s)).apply(Set.bot_set[Sum_Type.sum[C, C]]))
}
...
final case class OVar[B, A](a: B, b: Option[B]) extends ODE[A, B]
...
def equal_ODE[A : HOL.equal,
  ...
  case (OVar(x11, x12), OProd(x31, x32)) => false
  case (OProd(x31, x32), OVar(x11, x12)) => false
  case (OVar(x11, x12), OSing(x21, x22)) => false
  case (OSing(x21, x22), OVar(x11, x12)) => false
  ...
  case (OVar(x11, x12), OVar(y11, y12)) =>
    HOL.eq[B](x11, y11) && Optiona.equal_optiona[B](x12, y12)
}
...
final case class AssignAny[C, A, B](a: C) extends hp[A, B, C]
...
def equal_hp[A : HOL.equal, B : HOL.equal,
  ...
  =
  (x0, x1) match {
  case (Sequence(x81, x82), Loop(x9)) => false
  case (Loop(x9), Sequence(x81, x82)) => false
  case (Choice(x71, x72), Loop(x9)) => false
  case (Loop(x9), Choice(x71, x72)) => false
  case (Choice(x71, x72), Sequence(x81, x82)) => false
  case (Sequence(x81, x82), Choice(x71, x72)) => false
  case (EvolveODE(x61, x62), Loop(x9)) => false
  case (Loop(x9), EvolveODE(x61, x62)) => false
  case (EvolveODE(x61, x62), Sequence(x81, x82)) => false
  case (Sequence(x81, x82), EvolveODE(x61, x62)) => false
  case (EvolveODE(x61, x62), Choice(x71, x72)) => false
  case (Choice(x71, x72), EvolveODE(x61, x62)) => false
  case (Test(x5), Loop(x9)) => false
  case (Loop(x9), Test(x5)) => false
  case (Test(x5), Sequence(x81, x82)) => false
  case (Sequence(x81, x82), Test(x5)) => false
  case (Test(x5), Choice(x71, x72)) => false
  case (Choice(x71, x72), Test(x5)) => false
  case (Test(x5), EvolveODE(x61, x62)) => false
  case (EvolveODE(x61, x62), Test(x5)) => false
  case (DiffAssign(x41, x42), Loop(x9)) => false
  case (Loop(x9), DiffAssign(x41, x42)) => false
  case (DiffAssign(x41, x42), Sequence(x81, x82)) => false
  case (Sequence(x81, x82), DiffAssign(x41, x42)) => false
  case (DiffAssign(x41, x42), Choice(x71, x72)) => false
  case (Choice(x71, x72), DiffAssign(x41, x42)) => false
  case (DiffAssign(x41, x42), EvolveODE(x61, x62)) => false
  case (EvolveODE(x61, x62), DiffAssign(x41, x42)) => false
  case (DiffAssign(x41, x42), Test(x5)) => false
  case (Test(x5), DiffAssign(x41, x42)) => false
  case (AssignAny(x3), Loop(x9)) => false
  case (Loop(x9), AssignAny(x3)) => false
  case (AssignAny(x3), Sequence(x81, x82)) => false
  case (Sequence(x81, x82), AssignAny(x3)) => false
  case (AssignAny(x3), Choice(x71, x72)) => false
  case (Choice(x71, x72), AssignAny(x3)) => false
  case (AssignAny(x3), EvolveODE(x61, x62)) => false
  case (EvolveODE(x61, x62), AssignAny(x3)) => false
  case (AssignAny(x3), Test(x5)) => false
  case (Test(x5), AssignAny(x3)) => false
  case (AssignAny(x3), DiffAssign(x41, x42)) => false
  case (DiffAssign(x41, x42), AssignAny(x3)) => false
  case (Assign(x21, x22), Loop(x9)) => false
  case (Loop(x9), Assign(x21, x22)) => false
  case (Assign(x21, x22), Sequence(x81, x82)) => false
  case (Sequence(x81, x82), Assign(x21, x22)) => false
  case (Assign(x21, x22), Choice(x71, x72)) => false
  case (Choice(x71, x72), Assign(x21, x22)) => false
  case (Assign(x21, x22), EvolveODE(x61, x62)) => false
  case (EvolveODE(x61, x62), Assign(x21, x22)) => false
  case (Assign(x21, x22), Test(x5)) => false
  case (Test(x5), Assign(x21, x22)) => false
  case (Assign(x21, x22), DiffAssign(x41, x42)) => false
  case (DiffAssign(x41, x42), Assign(x21, x22)) => false
  case (Assign(x21, x22), AssignAny(x3)) => false
  case (AssignAny(x3), Assign(x21, x22)) => false
  case (Pvar(x1), Loop(x9)) => false
  case (Loop(x9), Pvar(x1)) => false
  case (Pvar(x1), Sequence(x81, x82)) => false
  case (Sequence(x81, x82), Pvar(x1)) => false
  case (Pvar(x1), Choice(x71, x72)) => false
  case (Choice(x71, x72), Pvar(x1)) => false
  case (Pvar(x1), EvolveODE(x61, x62)) => false
  case (EvolveODE(x61, x62), Pvar(x1)) => false
  case (Pvar(x1), Test(x5)) => false
  case (Test(x5), Pvar(x1)) => false
  case (Pvar(x1), DiffAssign(x41, x42)) => false
  case (DiffAssign(x41, x42), Pvar(x1)) => false
  case (Pvar(x1), AssignAny(x3)) => false
  case (AssignAny(x3), Pvar(x1)) => false
  case (Pvar(x1), Assign(x21, x22)) => false
  case (Assign(x21, x22), Pvar(x1)) => false
  case (Loop(x9), Loop(y9)) => equal_hp[A, B, C](x9, y9)
  case (Sequence(x81, x82), Sequence(y81, y82)) =>
    equal_hp[A, B, C](x81, y81) && equal_hp[A, B, C](x82, y82)
  case (Choice(x71, x72), Choice(y71, y72)) =>
    equal_hp[A, B, C](x71, y71) && equal_hp[A, B, C](x72, y72)
  case (EvolveODE(x61, x62), EvolveODE(y61, y62)) =>
    equal_ODE[A, C](x61, y61) && equal_formulaa[A, B, C](x62, y62)
  case (Test(x5), Test(y5)) => equal_formulaa[A, B, C](x5, y5)
  case (DiffAssign(x41, x42), DiffAssign(y41, y42)) =>
    HOL.eq[C](x41, y41) && equal_trma[A, C](x42, y42)
  case (AssignAny(x3), AssignAny(y3)) => HOL.eq[C](x3, y3)
...
def Ssafe[A, B,
           C : Enum.enum : HOL.equal](s:
(List[formula[A, B, C]], List[formula[A, B, C]])):
      Boolean
  =
  Lista.foldl[Boolean,
               formula[A, B,
                        C]](((acc: Boolean) => (x: formula[A, B, C]) =>
                              acc && fsafe[A, B, C](x)),
                             true,
                             Product_Type.fst[List[formula[A, B, C]],
       List[formula[A, B, C]]](s)) &&
    Lista.foldl[Boolean,
                 formula[A, B,
                          C]](((acc: Boolean) => (x: formula[A, B, C]) =>
                                acc && fsafe[A, B, C](x)),
                               true,
                               Product_Type.snd[List[formula[A, B, C]],
         List[formula[A, B, C]]](s))
...
def oprod[A, B](x0: ODE[A, B], oDE2: ODE[A, B]): ODE[A, B] = (x0, oDE2) match {
  ...
  case (OVar(c, d), oDE2) => OProd[A, B](OVar[B, A](c, d), oDE2)
...
def DPredl[A, B, C](fid: A): formula[B, C, A] =
  Prop[A, B, C](fid, ((a: A) => Var[A, B](a)))
...
object Parser {
  ...
  @inline
  val space:Parse[Option[myvars]] = {(str,i) =>
    //println("The i'th char is:" + str(i))
    if(str(i) == '(') { // some case
      //println("Checking someness-ness: " + str + " " + i)
      val ((_id, elem), i2) = p1(eatSym("NB"), mv)(str,i)
      (Some(elem),i2)
    } else {
      //println("Checking non-ness: " + str + " " + i)
      val (ident, i2) = eatSym("All")(str,i)
      (None,i2)
    }
  }
  ...
  @inline
  def p7[T1,T2,T3,T4,T5,T6,T7,T8](f1:Parse[T1],f2:Parse[T2],f3:Parse[T3],f4:Parse[T4],f5:Parse[T5],f6:Parse[T6],f7:Parse[T7],f8:Parse[T8]):Parse[(T1,T2,T3,T4,T5,T6,T7,T8)] ={(str,i) =>
    val i1 = eatChar(str,i,'(')
    val (res1,i2) = f1(str,i1)
    val i3 = eatChar(str,i2,' ')
    val (res2,i4) = f2(str,i3)
    ...
    val (res7,i14) = f7(str,i13)
    val i15 = eatChar(str,i14,' ')
    val (res8,i16) = f8(str,i15)
    val i17 = eatChar(str,i16,')')
    ((res1,res2,res3,res4,res5,res6,res7,res8),i17)
  }
    ...
    val (res:ODE[F,myvars], beforeParen:Int) =
      con match {
        case "OVar" =>
          val (c,i4) = mv(str,i3)
          val i5 = eatChar(str,i4,' ')
          val (sp,i6) = space(str,i5)
          (OVar[myvars,F](c,sp),i6)
        case "OSing" =>
          val (x,i4) = mv(str,i3)
          val i5 = eatChar(str,i4,' ')
...
*/
  val subst:Parse[USubst.subst_ext[myvars,myvars,myvars,Unit]] = {(str,i) =>
    val ((_id, fun,funl,pred,con,hpp,oode,sp), i1:Int) = p7(eatSym("subst_exta"),
    ...
    val ooode:(myvars => Option[myvars] => Option[ODE[myvars,myvars]]) =
      sp match {
        case None     => {case id:myvars => ({case (None) => oode(id) case Some(_:myvars) => None})}
        case Some(id) => {case id:myvars => ({case (None) => None case Some(i:myvars) => if(i == id) {oode(id)} else {None}})}
      }
    (USubst.subst_exta[myvars,myvars,myvars,Unit](fun,funl,pred,con,hpp,ooode,()),i1)
  }
...
object GeneratedProofChecker {
 ...
 import Int._
  ...
  def main(input : Array[String]) = {
    val path    = if(input.isEmpty) {"/usr0/home/bbohrer/research/verified-pipeline/code/velocitycar_dist.pt"   /*"/usr0/home/bbohrer/KeYmaeraX/velocityCar.pt"*/}  else input(0) //e.g. "/usr0/home/bbohrer/KeYmaeraX/velocityCar.pt"
