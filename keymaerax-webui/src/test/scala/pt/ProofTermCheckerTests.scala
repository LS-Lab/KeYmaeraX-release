 ...
 * for Differential Dynamic Logic: Toward Independently Checkable Proof Certificates for Dynamic Logics", CPP 16
 * @todo
 *   update description of proof terms.
 * @author
 *   Nathan Fulton
 * @author
 *   Brandon Bohrer
...
class ProofTermCheckerTests extends TacticTestBase {
  ...
  it should "work for prop tautologies with manual proof" in
    withTemporaryConfig(Map(Configuration.Keys.PROOF_TERM -> "true")) {
      withMathematica { _ =>
        val f = "A() -> A()".asFormula
        val provable = TermProvable.startPlainProof(f)
  ...
  it should "work for prop tautologies with propClose" in
    withTemporaryConfig(Map(Configuration.Keys.PROOF_TERM -> "true")) {
      withMathematica { _ =>
        val f = "A() -> A()".asFormula
        val provable = TermProvable.startPlainProof(f)
  ...
  it should "FEATURE_REQUEST: work for monoCars" taggedAs TodoTest in
    withTemporaryConfig(Map(Configuration.Keys.PROOF_TERM -> "true")) {
      withMathematica { _ =>
        val fml =
  ...
  it should "FEATURE_REQUEST: work for double velCar" taggedAs TodoTest in
    withTemporaryConfig(Map(Configuration.Keys.PROOF_TERM -> "true")) {
      withMathematica { _ =>
  ...
  it should "FEATURE_REQUEST: work for ETCS dI-ified proof" taggedAs TodoTest in withMathematica(_ => {
    val fml =
      "    v^2<=2*b()*(m-x) & v>=0  & A()>=0 & b()>0-> [{{?(2*b()*(m-x) >= v^2+(A()+b())*(A()*ep()^2+2*ep()*v)); a:=A(); ++ a:=-b(); } t := 0;{x'=v, v'=a, t'=1 & v>=0 & t<=ep()}}*@invariant(v^2<=2*b()*(m-x))] x <= m"
        .asFormula
    val tac = BelleParser(
      "implyR(1) ; loop({`v^2<=2*b()*(m-x)`}, 1) ; <(closeId, QE,composeb(1) ; choiceb(1) ;andR(1) ; <(composeb(1) ; testb(1) ; implyR(1) ; assignb(1) ; composeb(1) ; assignb(1) ; dC({`2*b()*(m-x)>=v^2+(A()+b())*(A()*(ep()-t)^2+2*(ep()-t)*v)`}, 1) ; <(dW(1) ; QE,dI(1)), assignb(1) ; composeb(1) ; assignb(1) ; dI(1)))"
    )
    val provable = ProvableSig.startPlainProof(fml)
    val tacticResult = proveBy(provable, tac)
    tacticResult match {
  ...
  "Isabelle converter" should "convert simple FOLR certificate" in
    withTemporaryConfig(Map(Configuration.Keys.PROOF_TERM -> "true")) {
      withMathematica { _ =>
  ...
  it should "convert prop tautologies" in withTemporaryConfig(Map(Configuration.Keys.PROOF_TERM -> "true")) {
    withMathematica { _ =>
      val f = "A() -> A()".asFormula
      val provable = TermProvable.startPlainProof(f)
  ...
  it should "FEATURE_REQUEST: convert leaderfollower" taggedAs TodoTest in withMathematica(_ => {
    ...
    val ParsedArchiveEntry(name, kind, modelText, _, _, formula, parsedTactics, _, _) =
      ArchiveParser.parse(archive).head
  ...
  it should "FEATURE_REQUEST: convert VelocityCarDist" taggedAs TodoTest in withMathematica(_ => {
    val f =
      "(d>=0 & (V()>=0 & ep()>=0)) -> [{{{{?d>=V()*ep(); v:=*; ?0<=v&v<=V();} ++ v:=0;}t := 0;}{d'=-v, t'=1 & t<=ep()}}*@invariant(d>=0)] ( d>=0)"
        .asFormula
    val t = implyR(1) &
        ...
        composeb(1) & composeb(1) & choiceb(1) &
          ...
          (
            composeb(1) & testb(1) & implyR(1) & composeb(1) & randomb(1) & allR(1) & testb(1) & implyR(1) &
              assignb(1) & dC("d>=V()*(ep()-t)".asFormula)(1) < (dW(1) & QE, dI()(1)),
  ...
  it should "parse axioms" in { val (AdConst(), 9) = axiom("(AdConst)", 0) }

  it should "parse axRules" in { val (CE(), 4) = axrule("(CE)", 0) }
  ...
  it should "parse reals" in {
    val (Ratreal(Frct((int_of_integer(den), int_of_integer(num)))), _) =
      real("(Ratreal (Frct ((int_of_integer 2) (int_of_integer 1))))", 0)
    den.intValue shouldBe 2
    num.intValue shouldBe 1
  }

  it should "parse terms" in {
    val (Const(Ratreal(Frct((int_of_integer(den1), int_of_integer(num1))))), _) =
      trm(mv)("(Const (Ratreal (Frct ((int_of_integer 2) (int_of_integer 1)))))", 0)
  ...
  it should "parse odes" in {
    ...
    val (OProd(OSing(i4(), Function(i4(), emp)), OSing(i5(), Var(i4()))), _) =
      ode(mv)("(OProd (OSing i4 (Function i4 e)) (OSing i5 (Var i4)))", 0)
  ...
  it should "parse formulas" in {
    ...
    val (
      Diamond(
        EvolveODE(OProd(OSing(i5(), Var(i4())), OSing(i4(), Function(i4(), _))), Geq(_, _)),
        Not(Not(Diamond(DiffAssign(i4(), Function(i4(), _)), Not(Geq(DiffVar(i4()), _))))),
      ),
      _,
    ) = formula(mv, mv)(
      "(Diamond (EvolveODE (OProd (OSing i5 (Var i4)) (OSing i4 (Function i4 e))) (Geq z z)) (Not (Not (Diamond (DiffAssign i4 (Function i4 e)) (Not (Geq (DiffVar i4) z))))))",
      0,
    )
    val (Prop(i1(), args), _) = formula(mv, mv)("(Prop i1 (s (Function i1 e)))", 0)
    args(i1()) match {
      case (Function(i1(), emp)) => enum_myvarsa
          .foreach(i => (if (true) { emp(i) shouldBe Const(Ratreal(Frct((int_of_integer(0), int_of_integer(1))))) }))
    }
  ...
  it should "parse sequents" in {
    val ((a, s), _) = sequent(
      "(() ((Not (And (Not (And (Prop i1 (s (Function i1 e))) (Not (Diamond (DiffAssign i1 (Function i1 e)) (Not (Prop i1 (s (DiffVar i1)))))))) (Not (And (Not (Prop i1 (s (Function i1 e)))) (Not (Not (Diamond (DiffAssign i1 (Function i1 e)) (Not (Prop i1 (s (DiffVar i1)))))))))))))",
      0,
    )
    a.length shouldBe 0
    s.length shouldBe 1
  ...
  it should "parse substs" in {
    val (pt.lib.USubst.subst_exta(fun, funl, pred, con, prog, ode, _), _) = subst(
      "(subst_exta ((Some (Var i4)) None None None None None None None None None None) ns ((Some (Not (Diamond (DiffAssign i4 (Function (Inl i4) est)) (Not (Geq (DiffVar i4) zst))))) None None None None None None None None None None) ns ns ns)",
      0,
    )
    fun(i1()) shouldBe Some(Var(i4()))
    fun(i2()) shouldBe None
    (pred(i1()) match {
      case Some(Not(Diamond(DiffAssign(i4(), Function(Inl(i4()), _)), Not(Geq(DiffVar(i4), _))))) => true
      case _ => false
    }) shouldBe true
  }
