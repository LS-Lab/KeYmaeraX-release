      ...
      (indUseCaseLbl, debugT("Use case") & ls(ImplyRightT) & (la(AndLeftT)*) & ls(AndRightT) &&(
        la(instantiateT(Variable("t"), Number(0))) &
          ...
          hideT(SuccPosition(0)) & debugT("Use case 1") &
            cutT(Some("\\exists maxz0 maxz(w*(dhf-dhd)) = maxz0".asFormula)) & onBranch(
            (cutShowLbl, hideT(SuccPosition(0)) & debugT("Cut proof 0") & cohideT(SuccPosition(0)) &
              ls(instantiateExistentialQuanT(Variable("maxz0"), "maxz(w*(dhf-dhd))".asTerm)) &
              debugT("Cut proof") & AxiomCloseT) /* Open Goal */,
            (cutUseLbl, la(skolemizeT) & debugT("Cut use case") &
              cutT(Some("0<=0&0 < maxz0/a&0=rv*0&0=w*a/2*0^2+dhd*0|0>=maxz0/a&0=rv*0&0=dhf*0-w*maxz0^2/(2*a)".asFormula)) &
              onBranch(
                (cutShowLbl, hideT(SuccPosition(0)) & hideT(AntePosition(9)) & debugT("Cut 2 proof") & arith), /* Open Goal */
                (cutUseLbl, debugT("Cut 2 use case") & arith) /* Closes, surprisingly (it contains some maxz) */
              ))
          ),
    ...
    val tactic = ls(ImplyRightT) & la(AndLeftT) & ls(wipeContextInductionT(Some(invariant))) & onBranch(
      ...
      (indUseCaseLbl, debugT("Use case") & ls(ImplyRightT) & (la(AndLeftT)*) & ls(AndRightT) &&(
        la(instantiateT(Variable("t"), Number(0))) &
          ...
          la(instantiateT(Variable("ho"), Number(0))) & la(ImplyLeftT) && (
          hideT(SuccPosition(0)) & debugT("Use case 1") &
            cutT(Some("\\exists max0 max(0, w*(dhf-dhd)) = maxz0".asFormula)) & onBranch(
