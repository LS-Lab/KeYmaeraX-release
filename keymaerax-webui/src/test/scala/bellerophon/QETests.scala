...
class QETests extends TacticTestBase {
  ...
  "Full QE" should "prove x>0 & y>0 |- x*y>0" in withMathematica { implicit qeTool =>
    proveBy(Sequent(IndexedSeq("x>0".asFormula, "y>0".asFormula), IndexedSeq("x*y>0".asFormula)), ToolTactics.fullQE) shouldBe 'proved
  }

  it should "prove x^2<0 |-" in withMathematica { implicit qeTool =>
    proveBy(Sequent(IndexedSeq("x^2<0".asFormula), IndexedSeq()), ToolTactics.fullQE) shouldBe 'proved
  }

  it should "fail on |-" in withMathematica { implicit qeTool =>
    a [BelleError] should be thrownBy proveBy(Sequent(IndexedSeq(), IndexedSeq()), ToolTactics.fullQE)
  }

  "Partial QE" should "not fail on |-" in withMathematica { implicit qeTool =>
    val result = proveBy(Sequent(IndexedSeq(), IndexedSeq()), ToolTactics.partialQE)
  ...
  it should "turn x^2>=0 |- y>1 into |- y>1" in withMathematica { implicit qeTool =>
    val result = proveBy(Sequent(IndexedSeq("x^2>=0".asFormula), IndexedSeq("y>1".asFormula)), ToolTactics.partialQE)
    result.subgoals should have size 1
    result.subgoals.head.ante shouldBe empty
