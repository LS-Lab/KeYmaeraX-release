...
class SMTQETests extends FlatSpec with Matchers with BeforeAndAfterEach {
  ...
  it should "prove complex 22" in {
    smt.qe("x > 0 -> !x^2-2*x+1=0".asFormula) should be("false".asFormula)
  }

  it should "prove complex 2" in {
    smt.qe("(c<1&c>=0&H>=0&g()>0&v^2<=2*g()*(H-h)&h>=0&k4_t_1=0&h_2()=h&v_2()=v&h_3=0&k4_t_4()=0&v_3=-1*k4_t_5*g()+v&0>=0&0=1/2*(-1*k4_t_5^2*g()+2*h+2*k4_t_5*v)&k4_t_5>=0&v_5=-c*(-1*k4_t_5*g()+v)->(-c*(-1*k4_t_5*g()+v))^2<=2*g()*(H-0))".asFormula) should be("true".asFormula)
  }
