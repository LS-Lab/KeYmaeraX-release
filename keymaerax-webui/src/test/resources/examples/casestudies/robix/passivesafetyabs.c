...
long double boundaryDist(state pre, state curr, parameters params) {
...
bool monitorSatisfied(state pre, state curr, parameters params) {
  return boundaryDist(pre,curr,params) <= 0.0;
}

/* Run controller `ctrl` monitored, return `fallback` if `ctrl` violates monitor */
state monitoredCtrl(state curr, parameters params, state input,
                    state (*ctrl)(state,parameters,state), state (*fallback)(state,parameters,state)) {
  state pre = curr;
