...
object ExpressionTraversal {
  ...
  def fail(x: Expr) = throw new UnknownOperatorException("Unimplemented case in Expr traversal", x)
  def failFTPG[T, A : FTPG](x: A) = throw new UnknownOperatorException("Unimplemented case in Expr traversal", x)
