...
final case class Provable private (val conclusion: Sequent, val subgoals: scala.collection.immutable.IndexedSeq[Sequent]) {
  /**
   * Checks whether the conclusion of this Provable has been proved by this derivation.
   */
   ...
   * Will return a Provable with the same conclusion but an updated set of premisses.
  ...
  final def apply(rule : Rule, subgoal : Int) : Provable = {
    ...
    "All previous premisses still around except the one that the proof rule has been applied to") ensuring (
  ...
  final def apply(subderivation : Provable, subgoal : Int) : Provable = {
    ...
    r => subgoals.drop(subgoal).toSet.subsetOf(r.subgoals.toSet),
    "All previous premisses still around except the one replaced by a derivation") ensuring (
    r => subderivation.subgoals.toSet.subsetOf(r.subgoals.toSet), "All premisses in joined derivation are new subgoals")
