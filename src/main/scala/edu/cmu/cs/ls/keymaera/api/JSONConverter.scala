    ...
    val fn = new ExpressionTraversalFunction {
      ...
      override def postP(p: PosInExpr, e: Program): Either[Option[StopTraversal], Program] = {
        ...
        val o = e match {
          ...
          case e:EmptyContEvolveProgram => JsObject("name" -> JsString("EmptyContEvolveProgram") :: ("children" -> JsArray(jsonStack.pop())) :: Nil ++: cf)
          case ContEvolveProduct(_,_) => JsObject("name" -> JsString("ContEvolveProgram") :: ("children" -> JsArray(jsonStack.pop())) :: Nil ++: cf)
