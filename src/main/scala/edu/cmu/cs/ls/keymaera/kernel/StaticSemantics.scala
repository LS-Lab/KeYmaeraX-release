...
object StaticSemantics {
  ...
  private def fmlVars(f: Formula): VCF = f match {
    ...
    case DotFormula => VCF(fv = SetLattice.topExceptCDotFormula, bv = SetLattice.bottom)
    case PredOf(p, arg) => VCF(fv = freeVars(arg), bv = SetLattice.bottom)
    case PredicationalOf(p, arg) => VCF(fv = SetLattice.topExceptCDotFormula, bv = SetLattice.bottom)
