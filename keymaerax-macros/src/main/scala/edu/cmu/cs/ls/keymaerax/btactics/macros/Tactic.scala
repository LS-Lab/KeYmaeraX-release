...
class TacticImpl(val c: blackbox.Context) {
  ...
  def apply(annottees: c.Expr[Any]*): c.Expr[Any] = {
    ...
    def paramify(tn: TermName, params: Seq[Tree]): (String, String, DisplayInfo, String, List[ArgInfo], Boolean) = {
      ...
      val (idx, _wereNamed, paramMap) = params.foldLeft((0, false, defaultMap))({ case (acc, x) =>
      ...
      val (
        ...
        premisesString,
        conclusionString,
        contextPremisesString,
        contextConclusionString,
        revealInternal,
        inputGenerator,
      ) = (
      ...
      val simpleDisplay = paramMap("names") match {
        ...
        case di => c.abort(
            c.enclosingPosition,
            "@Tactic expected names: String or names: (String, String) or names: SimpleDisplayInfo, got: " + di,
          )
      }
      ...
      val displayInfo =
        (inputs, premisesString, conclusionString, contextPremisesString, contextConclusionString) match {
          ...
          case (ins, "", concl, _, _) if concl != "" && ins.nonEmpty =>
            InputAxiomDisplayInfo(simpleDisplay, concl, inputs)
          ...
          case _ => c.abort(
              c.enclosingPosition,
              "Unsupported argument combination for @Tactic: If premises or inputs are given, conclusion must be given",
            )
        }
    ...
    def isTactic(tRet: Tree): Boolean = {
      tRet match {
        case tq"DependentTactic" | tq"DependentPositionTactic" | tq"DependentTwoPositionTactic" |
            tq"InputPositionTactic" | tq"BuiltInLeftTactic" | tq"BuiltInRightTactic" | tq"BuiltInTactic" |
            tq"BuiltInPositionTactic" | tq"CoreLeftTactic" | tq"CoreRightTactic" | tq"BuiltInTwoPositionTactic" |
            tq"InputTwoPositionTactic" | tq"InputTactic" | tq"StringInputTactic" |
            tq"DependentPositionWithAppliedInputTactic" | tq"AppliedBuiltInTwoPositionTactic" | tq"BelleExpr" => true
        case _ => false
      }
    }
    ...
    def getInput(param: Tree): ArgInfo = {
      param match {
        case v: ValDef => v.tpt match {
            ...
            case t => c.abort(
                c.enclosingPosition,
                "Expected supported input type in tactic definition, got unsupported type: " + t,
              )
          }
      }
    }
    def getInputs(params: Seq[Tree]): (Option[ArgInfo], List[ArgInfo]) = {
      ...
      val gen = infos.find((ai: ArgInfo) =>
        ai match {
          case _: GeneratorArg => true
          case _ => false
        }
      )
      if (
    ...
    def inferType(defaultType: Tree, args: List[ArgInfo], pos: PosArgs, anonSort: Option[AnonSort]): Tree = {
      ...
      defaultType match {
        case tq"" => (args, pos) match {
            ...
            case (Nil, _: TwoPos | _: AnteSuccPos) => (tq"edu.cmu.cs.ls.keymaerax.bellerophon.BuiltInTwoPositionTactic")
            ...
            case (Nil, AntePos(Some(sname), pname)) =>
              if (anonSort.contains(CoreAnonSort)) (tq"edu.cmu.cs.ls.keymaerax.bellerophon.CoreLeftTactic")
              else (tq"edu.cmu.cs.ls.keymaerax.bellerophon.BuiltInLeftTactic")
            case (Nil, SuccPos(Some(sname), pname)) =>
              if (anonSort.contains(CoreAnonSort)) (tq"edu.cmu.cs.ls.keymaerax.bellerophon.CoreRightTactic")
            ...
            case t =>
              c.abort(c.enclosingPosition, s"Unsupported argument combination in @Tactic: $args; $pos; $defaultType")
          }
        case _ =>
          ...
          if (argHasInput && !retHasInput) {
            c.abort(
              c.enclosingPosition,
              "@Tactic detected tactic with inputs. Annotated return type must be an input tactic type (e.g. InputTactic)",
            )
          }
          defaultType
      }
    ...
    def arguePos(
      ...
      (hasInputs, pos) match {
        case (false, NoPos(None)) =>
          ...
          } else { q"""new edu.cmu.cs.ls.keymaerax.btactics.TacticFactory.TacticForNameFactory ($funStr).by($acc)""" }
    ...
    def argue(
        ...
        pos: PosArgs,
        args: List[ArgInfo],
        generatorOpt: Option[ArgInfo],
        tRet: Tree,
        anonSort: Option[AnonSort],
    ): (Tree, Seq[ValDef], Seq[Tree], (Tree, Tree)) = {
      ...
      val (curried, typeTree) = args.foldRight[(Tree, Tree)](base)({ case (arg, (acc, accTy)) =>
        ...
        val vd = aiToVal(arg)
        if (vd.rhs.nonEmpty) c.abort(c.enclosingPosition, "Nonempty val")
      ...
      if (displayLevel == "internal") None
        ...
        args.find((ai: ArgInfo) => {
          display match {
            ...
            case RuleDisplayInfo(_, conc, prem, _) =>
              !(sdContains(conc, ai.name) || prem.exists(sd => sdContains(sd, ai.name)))
          }
        })
      }
    }
    def assemble(
      ...
      val (curriedTermTree, argSeq, argTySeq, (base, baseType)) =
        argue(codeName, rhs, positions, defArgs, generatorOpt, tRet, anonSort)
      ...
      val info = inferredRetType match {
        case "DependentTactic" | "BuiltInTactic" | "BelleExpr" => (
          q"""new edu.cmu.cs.ls.keymaerax.btactics.macros.PlainTacticInfo(codeName = $codeString, longDisplayName = $longDisplayName, display = ${convDI(
              display
            )(c)}, displayLevel = ${convSymbol(displayLevel)(
              c
            )}, needsGenerator = $needsGenerator, revealInternalSteps = $revealInternalSteps)(theExpr = $expr)"""
        )
        case "InputTactic" | "StringInputTactic" => (
          q"""new edu.cmu.cs.ls.keymaerax.btactics.macros.InputTacticInfo(codeName = $codeString, longDisplayName = $longDisplayName, display = ${convDI(
              display
            )(c)}, inputs = ${convAIs(displayInputs)(c)}, displayLevel = ${convSymbol(displayLevel)(
              c
            )},  needsGenerator = $needsGenerator, revealInternalSteps = $revealInternalSteps)(theExpr = $expr)"""
        )
        case "DependentPositionTactic" | "BuiltInLeftTactic" | "BuiltInRightTactic" | "BuiltInPositionTactic" |
            "CoreLeftTactic" | "CoreRightTactic" => (
          q"""new edu.cmu.cs.ls.keymaerax.btactics.macros.PositionTacticInfo(codeName = $codeString, longDisplayName = $longDisplayName, display = ${convDI(
              display
            )(c)}, displayLevel = ${convSymbol(displayLevel)(
              c
            )}, needsGenerator = $needsGenerator, revealInternalSteps = $revealInternalSteps)(theExpr = $expr)"""
        ...
        case "InputPositionTactic" | "DependentPositionWithAppliedInputTactic" => (
          q"""new edu.cmu.cs.ls.keymaerax.btactics.macros.InputPositionTacticInfo(codeName = $codeString, longDisplayName = $longDisplayName, display = ${convDI(
              display
            )(c)}, inputs = ${convAIs(displayInputs)(c)}, displayLevel = ${convSymbol(displayLevel)(
              c
            )}, needsGenerator = $needsGenerator, revealInternalSteps = $revealInternalSteps)(theExpr = $expr)"""
        ...
        case "DependentTwoPositionTactic" | "InputTwoPositionTactic" | "BuiltInTwoPositionTactic" =>
          q"""new edu.cmu.cs.ls.keymaerax.btactics.macros.TwoPositionTacticInfo(codeName = $codeString, longDisplayName = $longDisplayName, display = ${convDI(
              display
            )(c)}, displayLevel = ${convSymbol(displayLevel)(c)}, needsGenerator = $needsGenerator)(theExpr = $expr)"""
        case "AppliedBuiltInTwoPositionTactic" =>
          q"""new edu.cmu.cs.ls.keymaerax.btactics.macros.InputTwoPositionTacticInfo(codeName = $codeString, longDisplayName = $longDisplayName, display = ${convDI(
              display
            )(c)}, displayLevel = ${convSymbol(displayLevel)(c)}, needsGenerator = $needsGenerator)(theExpr = $expr)"""
        case ty => c.abort(c.enclosingPosition, s"Unsupported return type in @Tactic: $ty")
      }
      missingInput(displayLevel, displayInputs, display).map(ai =>
        c.abort(
          c.enclosingPosition,
          s"Tactic $declName must mention every input in DisplayInfo, but argument $ai is not mentioned in DisplayInfo $display",
        )
      )

      /** Save a classfile annotation which helps runtime reflection identify the annotated declarations */
      val ann =
        q"new edu.cmu.cs.ls.keymaerax.btactics.macros.InternalAnnotation()" // codeName = $codeName, inputs = ${convAIs(displayInputs)(c)}
      val mds = mods match {
        case Modifiers(flags, privateWithin, annotations) => Modifiers(flags, privateWithin, ann :: annotations)
      }
      ...
      if (isDef) {
        ...
        if (defArgs.isEmpty) c.Expr(q"""$mds def $declName: $baseType = $application""")
        else c.Expr(q"""$mds def $declName (..$argSeq): $baseType = $application""")
      } else {
        val application = q"""edu.cmu.cs.ls.keymaerax.btactics.macros.DerivationInfo.registerL($base, $info)"""
    ...
    def anonSort(s: String): Option[AnonSort] = {
      s match {
        case "anon" | "anonL" | "anonR" | "anonLR" | "inputanon" | "inputanonL" | "inputanonP" | "inputanonR" |
            "inputanonS" => Some(SimpleAnonSort)
    ...
    def pickRhs(f: Ident, rhs: Tree, params: Option[Seq[Tree]] = None): Tree = {
      if (anonSort(f.toString).isDefined) { rhs }
      else {
        params match {
    ...
    val (isDef, mods, codeName, inArgs, tRet, fOpt, params, rhs) = annottees.map(_.tree).toList match {
      ...
      case q"$mds def ${codeName: TermName}: $tRet = $rhs" :: Nil => (true, mds, codeName, Nil, tRet, None, Nil, rhs)
      ...
      case q"$mds val ${codeName: TermName}: $tRet = $rhs" :: Nil => (false, mds, codeName, Nil, tRet, None, Nil, rhs)
      case _ :: _ :: _ =>
        c.abort(c.enclosingPosition, "Expected one tactic definition, got multiple: " + annottees.map(show(_)))
      case _ => c.abort(c.enclosingPosition, "Expected tactic definition, got: " + annottees.map(show(_)))
    }
    tRet match {
      case tq"" => c.abort(
          c.enclosingPosition,
          "@Tactic expects a return type annotation on tactics. Please annotate the return type, see MiscTactics.scala for supported types",
        )
      case _ => if (!isTactic(tRet)) c.abort(
          c.enclosingPosition,
          "Invalid annottee: Expected val(or def) <tactic>: <Tactic> = <anon> ((args) => rhs)..., got: " + tRet + " " +
            tRet.getClass + ", see MiscTactics.scala for supported types",
        )
    }
    val isCoreAnon = fOpt match {
      case Some(f) => anonSort(f.toString)
      case None => None
    }
    ...
    if (debugTactics.contains(codeName.decodedName.toString)) { println(s"DEBUG (${codeName}): \n$res\n") }
    res
  }
}
