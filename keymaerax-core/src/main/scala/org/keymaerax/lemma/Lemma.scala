...
import org.keymaerax.parser.{Declaration, KeYmaeraXExtendedLemmaParser}
import org.keymaerax.pt._
import org.keymaerax.tools.ToolEvidence
import org.keymaerax.{lemma, Configuration}
...
object Lemma {
   ...
   * quickly as possible. If converting the lemma back into a string already gives exactly the string we started with,
  ...
  private def fromStringInternal(lemma: String): Lemma = {
    ...
    val fact =
      ...
      } else { ElidingProvable(provable, Declaration(Map.empty)) }
...
/**
 * Lemmas are named Provables, supported by some evidence of how they came about. The soundness-critical part in a lemma
 * is its provable fact, which can only be obtained from the prover core.
 *
 * @example
 *   {{{
 *   // prove a lemma
...
final case class Lemma(fact: ProvableSig, evidence: immutable.List[Evidence], name: Option[String] = None) {
  ...
  assert(
    hasStamp,
    "Lemma should have kyxversion (unless compatibility mode, which is " + Lemma.LEMMA_COMPAT_MODE + ")\nbut got\n" +
  ...
  override def toString: String = {
    ...
    (
      r => {
        ...
        reparsed.fact.underlyingProvable == fact.underlyingProvable && reparsed.evidence == evidence &&
        reparsed.name == name
      },
  ...
  /**
   * Produces a string representation of this lemma that is speculated to reparse as this lemma and will be checked to
   * do so in [[toString]].
   */
