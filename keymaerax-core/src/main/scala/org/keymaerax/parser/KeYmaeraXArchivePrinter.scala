 ...
 */
class KeYmaeraXArchivePrinter(prettierPrinter: Expression => FormatProvider, withComments: Boolean = false)
    extends (ParsedArchiveEntry => String) {
  ...
  def apply(entry: ParsedArchiveEntry): String = {
    ...
    val printedTactics = entry
      .tactics
      .map({ case (tname, t, _) => s"""$TACTIC_BEGIN "$tname"\n$t\n$END_BLOCK""" })
      .mkString("\n\n")

    val defsBlock = printDefsBlock(entry.defs, symbols)

    val abbrvNames = entry
      .defs
      .isubsts
      .map({ case SubstitutionPair(FuncOf(Function(n, i, _, _, _), _), _) => Name(n, i) })

    val interpretationSanitized = abbrvNames.foldLeft(entry.model.prettyString)({ case (m, n) =>
      m.replaceAll("(" + n.prettyString + ")" + "<<.*?>>", "$1")
    })
    val printed = print(
      head,
      entry.name,
      defsBlock,
      varsBlock,
      prettierPrinter(entry.model).print(interpretationSanitized),
      printedTactics,
    )
    ...
    val finalPrint =
      if (withComments) {
        assert(
          ArchiveParser(printed).map(_.model) == ArchiveParser(entry.fileContent).map(_.model),
    ...
    s"/* Exported from KeYmaera X v$Version */\n\n$finalPrint"
  }
...
object KeYmaeraXArchivePrinter {
  ...
  private val printer = new KeYmaeraXPrecedencePrinter {
    override protected def pp(q: PosInExpr, term: Term): String = term match {
      case FuncOf(f, c) =>
        val fnName = /* TODO print import instead of builtin if (InterpretedSymbols.builtin.contains(f)) f.name + f.index.map("_" + _).getOrElse("") else*/
          f.asString
  ...
  def printName(name: String, idx: Option[Int]): String = name +
    (idx match {
      case Some(i) => "_" + i
      case None => ""
    })
  ...
  def printSort(sort: Sort): String = sort match {
    ...
    case _ => throw new IllegalArgumentException(
        "Sort " + sort + " not supported as sort of declaration; please use one of [Real, Bool, HP]."
      )
  }

  /**
   * Prints the sort of term `arg` as a function domain with named arguments (argument names from variables in term
   * `arg`). Term must be one of [Nothing, BaseVariable, (nested) Pair of BaseVariable].
   */
  def printDomain(arg: Term): String = arg match {
    ...
    case _ => throw new IllegalArgumentException(
        "Sort " + arg.prettyString +
          " not supported as domain of declaration; please use of of [Nothing, BaseVariable, Pair]"
      )
  }
  ...
  /**
   * Creates a term that fits `domain`, using `name` to name arguments from names, returns created term and the
   * remaining names.
   */
  private def createArg(domain: Sort, names: List[Name]): (Term, List[Name]) = domain match {
    ...
    case Tuple(l, r) =>
      ...
      (Pair(ls, rs), rnames)
    case _ => throw new IllegalArgumentException(
        "Sort " + domain + " not supported as domain of declaration; please use one of [Unit, Real, Tuple]"
      )
  }
  ...
  def printDefsBlock(decl: Declaration, symbols: Set[NamedSymbol]): String = {
    val defs =
      decl
        .decls
        .filter(
          _._2.domain.isDefined
        ) // .filter(dn => !InterpretedSymbols.byName.exists({ case ((n, i), _) => Name(n, i) == dn._1 }))

    val printedDecls = symbols
      .filter(s => !defs.keySet.contains(Name(s.name, s.index)))
      .map({
        case Function(name, idx, domain, sort, None) =>
          if (!decl.decls.contains(Name(name, idx)))
    ...
    if (printedDecls.nonEmpty || printedDefs.nonEmpty) "Definitions\n" + printedDecls +
      (if (printedDecls.nonEmpty && printedDefs.nonEmpty) "\n" else "") + printedDefs.mkString("\n") + "\n" +
      END_BLOCK + "\n"
    else ""
  }
  ...
  private def nameDots(e: Expression, names: List[Name]): Expression = {
    ...
    assert(
      dots.isEmpty || dots.size == names.size,
      "Argument names mismatch, expected name for each dot: expression " + e.prettyString + " contains " + dots.size +
        " dots " + dots.map(_.prettyString).mkString(",") + ", but got " + names.size + " names " +
        names.map(_.prettyString).mkString(","),
    )
  ...
  def apply(entry: ParsedArchiveEntry): String = {
    ...
    try {
      ...
      if (withComments) {
        ...
        """(Theorem|Lemma|ArchiveEntry|Exercise)[^"]*"[^"]*"""".r.findFirstIn(entry.problemContent) match {
          case Some(_) => s"""${entry.problemContent.stripSuffix(END_BLOCK).trim()}
                             #$printedTactics
                             #$END_BLOCK""".stripMargin('#')
          case None if entry.problemContent.contains(PROBLEM_BLOCK.img) =>
            s"""$head "${entry.name}"
    ...
    } finally { PrettyPrinter.setPrinter(pp) }
  }
}
