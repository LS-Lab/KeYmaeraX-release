...
import org.keymaerax.bellerophon.{BelleExpr, DefTactic}
import org.keymaerax.bellerophon.parser.{BelleLexer, BelleParser}
import org.keymaerax.bellerophon.parser.BelleParser.{BelleToken, DefScope}
...
object KeYmaeraXArchiveParser extends KeYmaeraXArchiveParserBase {
  ...
  override protected def convert(t: Tactic, defs: Declaration): (String, String, BelleExpr) =
    try {
      ...
      case ex: ParseException if ex.msg.startsWith("Lexer") =>
        ...
        val msg =
          (if (omitStart < 0 || restStart < 0) ex.msg else ex.msg.substring(0, omitStart) + ex.msg.substring(restStart))
            .replace(s"${ex.loc.line}:${ex.loc.column}", s"${shiftedLoc.line}:${shiftedLoc.column}")
  ...
  private def shiftLoc(loc: Location, offset: Location): Location = {
    ...
    else { loc.addLines(offset.line - 1) }
  }
