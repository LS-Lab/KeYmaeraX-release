...
trait OpSpec extends Ordered[OpSpec] {
  ...
  /**
   * Compare this operator specification to another one such that ``this<other`` says that ``this`` binds stronger than
   * ``other``.
   */
...
case class UnitOpSpec(op: Terminal, prec: Int, const: String => Expression) extends OpSpec {
  final def assoc: OpNotation = AtomicFormat
}

object UnitOpSpec {
  def apply(op: Terminal, prec: Int, const: Expression) = new UnitOpSpec(op, prec, s => { assert(s == op.img); const })
}

/** Unary operator notation specification with a constructor and expected argument kind. */
case class UnaryOpSpec[T <: Expression](
    op: Terminal,
    prec: Int,
    assoc: UnaryNotation,
    kind: Kind,
...
object UnaryOpSpec {
  def apply[T <: Expression](op: Terminal, prec: Int, assoc: UnaryNotation, kind: Kind, const: T => T) =
    new UnaryOpSpec[T](op, prec, assoc, kind, (s, e) => { assert(s == op.img); const(e) })

  // mixed converters

  def lUnaryOpSpecT[T <: Expression](op: Terminal, prec: Int, assoc: UnaryNotation, kind: Kind, const: (Term) => T) =
    new UnaryOpSpec(op, prec, assoc, kind, (s, e: T) => { assert(s == op.img); const(e.asInstanceOf[Term]) })

  def lUnaryOpSpecF[T <: Expression](op: Terminal, prec: Int, assoc: UnaryNotation, kind: Kind, const: (Formula) => T) =
...
case class BinaryOpSpec[T <: Expression](
    op: Terminal,
    prec: Int,
    assoc: BinaryNotation,
    kind: (Kind, Kind),
    const: (String, T, T) => T,
) extends OpSpec

object BinaryOpSpec {
  def apply[T <: Expression](op: Terminal, prec: Int, assoc: BinaryNotation, kind: (Kind, Kind), const: (T, T) => T) =
    new BinaryOpSpec[T](op, prec, assoc, kind, (s, e1, e2) => { assert(s == op.img); const(e1, e2) })

  // mixed converters

  def lBinaryOpSpec[T <: Expression](
      op: Terminal,
      prec: Int,
      assoc: BinaryNotation,
      kind: (Kind, Kind),
    ...
    assoc,
    kind,
    (s, e1: T, e2: T) => { assert(s == op.img); const(e1.asInstanceOf[Term], e2.asInstanceOf[Term]) },
  )
}
...
case class TernaryOpSpec[T <: Expression](
    ...
    op3: Terminal,
    prec: Int,
    assoc: TernaryNotation,
    kind: (Kind, Kind, Kind),
    const: (String, T, T, T) => T,
) extends OpSpec
 ...
 *   - NUMBER lexer does not contain - sign to enable `x-5` to be parsed. Parser will make up for this, respecting
...
object OpSpec {
  ...
  private[parser] def func(name: String, index: Option[Int] = None, domain: Sort, sort: Sort): Function =
    Function(name, index, domain, sort)
  ...
  val sFuncOf = UnaryOpSpec[Term](
    ...
    PrefixFormat,
    unterm,
    (name: String, e: Term) => FuncOf(func(name, None, e.sort, Real), e),
  )
  val sDifferentialSymbol =
    UnaryOpSpec[Term](PRIME, 0, PostfixFormat, unterm, (v: Term) => DifferentialSymbol(v.asInstanceOf[Variable]))
  val sDifferential = UnaryOpSpec[Term](PRIME, 5, PostfixFormat, unterm, Differential.apply)
  val sPower = BinaryOpSpec[Term](POWER, 20, RightAssociative /*!*/, binterm, Power.apply)
  val sTimes = BinaryOpSpec[Term](STAR, 40, LeftAssociative, binterm, Times.apply)
  val sDivide = BinaryOpSpec[Term](SLASH, 40, LeftAssociative /*!*/, binterm, Divide.apply)
  val sNeg = UnaryOpSpec[Term](MINUS, 59 /*!*/, PrefixFormat, unterm, Neg.apply) // -x^2 == -(x^2) != (-x)^2
  val sPlus = BinaryOpSpec[Term](PLUS, 60, LeftAssociative, binterm, Plus.apply)
  val sMinus = BinaryOpSpec[Term](MINUS, 60, LeftAssociative /*!*/, binterm, Minus.apply)
  val sPair = BinaryOpSpec[Term](COMMA, 444, RightAssociative, binterm, Pair.apply)
  ...
  val sDifferentialFormula = UnaryOpSpec[Formula](PRIME, 80, PostfixFormat, unfml, DifferentialFormula.apply)
  val sEqual = lBinaryOpSpec(EQ, 90, AtomicBinaryFormat, bintermfml, Equal.apply)
  assert(sEqual > sMinus, "formulas bind weaker than their constituent terms")
  val sNotEqual = lBinaryOpSpec(NOTEQ, 90, AtomicBinaryFormat, bintermfml, NotEqual.apply)
  val sGreaterEqual = lBinaryOpSpec(GREATEREQ, 90, AtomicBinaryFormat, bintermfml, GreaterEqual.apply)
  val sGreater = lBinaryOpSpec(RDIA, 90, AtomicBinaryFormat, bintermfml, Greater.apply)
  val sLessEqual = lBinaryOpSpec(LESSEQ, 90, AtomicBinaryFormat, bintermfml, LessEqual.apply)
  val sLess = lBinaryOpSpec(LDIA, 90, AtomicBinaryFormat, bintermfml, Less.apply)
  val sForall = BinaryOpSpec[Expression](
    FORALL,
    95,
    MixedBinary,
    quantfml,
    (x: Expression, f: Expression) => Forall(Seq(x.asInstanceOf[Variable]), f.asInstanceOf[Formula]),
  )
  val sExists = BinaryOpSpec[Expression](
    ...
    MixedBinary,
    quantfml,
    (x: Expression, f: Expression) => Exists(Seq(x.asInstanceOf[Variable]), f.asInstanceOf[Formula]),
  )
  val sBox = BinaryOpSpec[Expression](
    ...
    MixedBinary,
    modalfml,
    (_: String, a: Expression, f: Expression) => Box(a.asInstanceOf[Program], f.asInstanceOf[Formula]),
  )
  val sDiamond = BinaryOpSpec[Expression](
    ...
    MixedBinary,
    modalfml,
    (_: String, a: Expression, f: Expression) => Diamond(a.asInstanceOf[Program], f.asInstanceOf[Formula]),
  )
  val sNot = UnaryOpSpec[Formula](NOT, 99, PrefixFormat, unfml, Not.apply)
  val sAnd = BinaryOpSpec[Formula](AMP, 110, RightAssociative, binfml, And.apply)
  val sOr = BinaryOpSpec[Formula](OR, 120, RightAssociative, binfml, Or.apply)
  val sImply = BinaryOpSpec[Formula](IMPLY, 150, RightAssociative /*!*/, binfml, Imply.apply)
  val sRevImply =
    BinaryOpSpec[Formula](
      REVIMPLY,
      150,
      LeftAssociative,
      binfml,
      (l: Formula, r: Formula) => Imply(r, l),
    ) /* swaps arguments */
  val sEquiv = BinaryOpSpec[Formula](EQUIV, 160, NonAssociative /*!*/, binfml, Equiv.apply)
  ...
  val sAssign = lBinaryOpSpec[Program](
    ...
    AtomicBinaryFormat,
    bintermprog,
    (x: Term, e: Term) => Assign(x.asInstanceOf[Variable], e),
  )
  ...
  val sAssignAny =
    lUnaryOpSpecT[Program](ASSIGNANY, 200, PostfixFormat, untermprog, (x: Term) => AssignAny(x.asInstanceOf[Variable]))
  ...
  val sIfThenElse = TernaryOpSpec[Program](
    ...
    260,
    TernaryPrefixFormat,
    (FormulaKind, ProgramKind, ProgramKind),
    (_: String, e1: Expression, e2: Expression, e3: Expression) => {
      val p = e1.asInstanceOf[Formula]
      e3 match {
        case Test(True) => Choice(Compose(Test(p), e2.asInstanceOf[Program]), Test(Not(p)))
        case prg => Choice(Compose(Test(p), e2.asInstanceOf[Program]), Compose(Test(Not(p)), prg.asInstanceOf[Program]))
      }
    },
  )
  ...
  val sAtomicODE = BinaryOpSpec[Program](
    ...
    AtomicBinaryFormat,
    bintermprog,
    (_: String, xp: Expression, e: Expression) => AtomicODE(xp.asInstanceOf[DifferentialSymbol], e.asInstanceOf[Term]),
  )
  val sDifferentialProduct = BinaryOpSpec(COMMA, 95, RightAssociative, bindiffprog, DifferentialProduct.apply)
  val sODESystem = BinaryOpSpec[Expression](
    AMP,
    150,
    NonAssociative,
    diffprogfmlprog,
    (_: String, ode: Expression, h: Expression) =>
      ODESystem(ode.asInstanceOf[DifferentialProgram], h.asInstanceOf[Formula]),
  )
  val sLoop = UnaryOpSpec[Program](STAR, 220, PostfixFormat, unprog, Loop.apply)
  val sDLoop = UnaryOpSpec[Program](DSTAR, 220, PostfixFormat, unprog, (p: Program) => Dual(Loop(Dual(p))))
  val sDual = UnaryOpSpec[Program](DUAL, 220, PostfixFormat, unprog, Dual.apply)
  val sCompose =
    BinaryOpSpec[Program](SEMI, 230, RightAssociative, binprog, Compose.apply) // @todo compatibility mode for parser
  val sChoice = BinaryOpSpec[Program](CHOICE, 250, RightAssociative, binprog, Choice.apply)
