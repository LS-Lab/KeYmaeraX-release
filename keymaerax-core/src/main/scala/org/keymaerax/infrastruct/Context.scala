...
import org.keymaerax.core.StaticSemantics.signature
...
/**
 * Context management, position splitting, and extraction tools. Useful for splitting an expression at a position into
 * the subexpression at that position and the remaining context around it. Or for replacing a subexpression by another
 * subexpression at all cost. Or for splitting a position into a formula position and a term position.
 * @example
...
object Context {
  ...
  /**
   * `true` gives slower guarded contexts that fail inadmissible term instantiation. `false` gives theoretically fast
   * unguarded replacement contexts that are slow in practice.
   */
  ...
  private def directAt[T <: Expression](t: T, pos: PosInExpr): (Context[T], Expression) =
    (new ReplacementContext[T](t, pos), sub(t, pos)) ensures
      (
        ...
        r =>
          !CROSSCHECK || {
            val rat = context(t, pos); r._2 == rat._2 && r._1(r._2) == t && Context(rat._1)(rat._2) == t
          },
        "cross-contract consistency",
      )

  /**
   * Split `C{e}=t(pos)` expression t at position pos into the expression e at that position and the context C within
  ...
  /**
   * Split `C{e}=t(pos)` term t at position pos into the expression e at that position and the context C within which
   * that expression occurs. Thus `C{e}` will equal the original `t` and `e` occurs at position pos in `t` (provided
   * that back-substitution is admissible, otherwise a direct replacement in `C` at `pos` to `e` will equal `t`).
   */
    ...
    if (!GUARDED) { directAt(t, pos) }
    else
      {
        ...
        (
          r => backsubstitution(r, t, pos),
          "Reassembling the expression at that position into the context returns the original formula: " + t + " at " +
            pos + " gives " + Context(split(t, pos)._1)(split(t, pos)._2) + " in context " + split(t, pos),
        )

  /**
   * Split `C{e}=f(pos)` formula f at position pos into the expression e at that position and the context C within which
   * that expression occurs. Thus `C{e}` will equal the original `f` and `e` occurs at position pos in `f` (provided
   * that back-substitution is admissible, otherwise a direct replacement in `C` at `pos` to `e` will equal `f`).
   */
    ...
    else
      {
        ...
        (
          ...
          "Reassembling the expression at that position into the context returns the original formula: " + f + " at " +
            pos + " gives " + Context(split(f, pos)._1)(split(f, pos)._2) + " in context " + split(f, pos),
        )

  /**
   * Split `C{e}=a(pos)` program `a` at position pos into the expression e at that position and the context C within
   * which that expression occurs. Thus `C{e}` will equal the original `a` and `e` occurs at position pos in `a`.
   * (provided that back-substitution is admissible, otherwise a direct replacement in `C` at `pos` to `e` will equal
   * `t`).
   */
    ...
    else
      {
        ...
        (
          ...
          "Reassembling the expression at that position into the context returns the original formula: " + a + " at " +
            pos + " gives " + Context(split(a, pos)._1)(split(a, pos)._2) + " in context " + split(a, pos),
        )
    ...
    if (pos == HereP) (DotTerm(term.sort), term)
    else
      {
        term match {
          ...
          case f: BinaryCompositeTerm if pos.head == 0 =>
            val sp = context(f.left, pos.child); (f.reapply(sp._1, f.right), sp._2)
          case f: BinaryCompositeTerm if pos.head == 1 =>
    ...
    if (pos == HereP) (DotFormula, formula)
    else
      {
        formula match {
          ...
          case PredicationalOf(c, t) if pos.head == 0 =>
            val sp = context(t, pos.child); (PredicationalOf(c, sp._1), sp._2)
          // pseudo-homomorphic cases
          case f: ComparisonFormula if pos.head == 0 =>
            val sp = context(f.left, pos.child); (f.reapply(sp._1, f.right), sp._2)
          ...
          case f: BinaryCompositeFormula if pos.head == 0 =>
            val sp = context(f.left, pos.child); (f.reapply(sp._1, f.right), sp._2)
          case f: BinaryCompositeFormula if pos.head == 1 =>
            val sp = context(f.right, pos.child); (f.reapply(f.left, sp._1), sp._2)
    ...
    if (pos == HereP) (DotProgram, program)
    else
      {
        program match {
          ...
          // homomorphic cases
          case f: UnaryCompositeProgram if pos.head == 0 =>
            val sp = context(f.child, pos.child); (f.reapply(sp._1), sp._2)
          case f: BinaryCompositeProgram if pos.head == 0 =>
            val sp = context(f.left, pos.child); (f.reapply(sp._1, f.right), sp._2)
          case f: BinaryCompositeProgram if pos.head == 1 =>
            val sp = context(f.right, pos.child); (f.reapply(f.left, sp._1), sp._2)
          case _ => throw new IllegalArgumentException(
  ...
  private def contextODE(program: DifferentialProgram, pos: PosInExpr): (DifferentialProgram, Expression) =
    if (pos == HereP) (DotDiffProgram, program)
    else
      {
        program match {
          ...
          case DifferentialProduct(l, r) if pos.head == 0 =>
            val sp = contextODE(l, pos.child); (DifferentialProduct(sp._1, r), sp._2)
          case DifferentialProduct(l, r) if pos.head == 1 =>
            val sp = contextODE(r, pos.child); (DifferentialProduct(l, sp._1), sp._2)
          case _ => throw new IllegalArgumentException(
  ...
  /** @see [[org.keymaerax.btactics.StaticSemanticsTools.boundAt()]] for same positions */
  private def part(term: Term, pos: PosInExpr): Term =
    if (pos == HereP) term
    else {
      term match {
        ...
        case _ =>
          throw new IllegalArgumentException("part position " + pos + " of term " + term + " may not be defined")
      }
    }

  private def part(formula: Formula, pos: PosInExpr): Expression =
    if (pos == HereP) formula
    else {
      formula match {
        // base cases
        ...
        case _ =>
          throw new IllegalArgumentException("part position " + pos + " of formula " + formula + " may not be defined")
      }
    }

  private def part(program: Program, pos: PosInExpr): Expression =
    if (pos == HereP) program
    else {
      program match {
        ...
        case _ =>
          throw new IllegalArgumentException("part position " + pos + " of program " + program + " may not be defined")
      }
    }

  private def partODE(program: DifferentialProgram, pos: PosInExpr): Expression =
    if (pos == HereP) program
    else {
      program match {
...
//    case _ => default
//  }
  private def asOrElse(e: Expression, default: => Term): Term = e match {
    case t: Term => t
    case _ => default
  }
  private def asOrElse(e: Expression, default: => Formula): Formula = e match {
    case t: Formula => t
    case _ => default
  }
  private def asOrElse(e: Expression, default: => Program): Program = e match {
    case t: Program => t
    case _ => default
  }
  private def asOrElseD(e: Expression, default: => DifferentialProgram): DifferentialProgram = e match {
    case t: DifferentialProgram => t
    case _ => default
  }

  /** Replace within term at position pos by repl @see [[StaticSemanticsTools.boundAt()]] for same positions */
  def replaceAt(term: Term, pos: PosInExpr, repl: Expression): Term =
    if (pos == HereP) asOrElse(
      repl,
      throw new IllegalArgumentException(
        "replaceAt position " + pos + " of term " + term + " by " + repl + " may not be defined"
      ),
    )
    else term match {
  ...
  def replaceAt(formula: Formula, pos: PosInExpr, repl: Expression): Formula =
    if (pos == HereP) asOrElse(
      repl,
      throw new IllegalArgumentException(
        "replaceAt position " + pos + " of formula " + formula + " by " + repl + " may not be defined"
      ),
    )
    else formula match {
      // base cases
  ...
  def replaceAt(program: Program, pos: PosInExpr, repl: Expression): Program =
    if (pos == HereP) asOrElse(
      repl,
      throw new IllegalArgumentException(
        "replaceAt position " + pos + " of program " + program + " by " + repl + " may not be defined"
      ),
    )
    else program match {
  ...
  private def replaceAtODE(program: DifferentialProgram, pos: PosInExpr, repl: Expression): DifferentialProgram =
    if (pos == HereP) asOrElseD(
      repl,
      throw new IllegalArgumentException(
        "replaceAtODE position " + pos + " of program " + program + " by " + repl + " may not be defined"
      ),
    )
    else program match {
...
//    case f:BinaryCompositeFormula if pos.head==1 => val sp = split(f.right, pos.child); (f.type.apply(f.left, sp._1), sp._2)
  ...
  private def split(program: Program, pos: PosInExpr): (Program, Expression) =
    if (pos == HereP) (DotProgram, program)
    else
      {
        program match {
  ...
  private def splitODE(program: DifferentialProgram, pos: PosInExpr): (DifferentialProgram, Expression) =
    if (pos == HereP) (DotDiffProgram, program)
    else
      {
        program match {
          ...
          case DifferentialProduct(l, r) if pos.head == 0 =>
            val sp = splitODE(l, pos.child); (DifferentialProduct(sp._1, r), sp._2)
          case DifferentialProduct(l, r) if pos.head == 1 =>
            val sp = splitODE(r, pos.child); (DifferentialProduct(l, sp._1), sp._2)
          case _ => throw new IllegalArgumentException(
...
private case class GuardedContext[+T <: Expression](ctx: T) extends Context[T] {
  ...
  private def instantiate(withA: Program): T = {
    ...
    if (
      withA.isInstanceOf[DifferentialProgram] && !withA.isInstanceOf[ODESystem]
    ) // 2nd test redundant: ODESystem does not extend DifferentialProgram
      instantiate(withA.asInstanceOf[DifferentialProgram])
    else USubst(SubstitutionPair(DotProgram, withA) :: Nil)(ctx).asInstanceOf[T]
  }
  ...
  private def instantiate(withA: DifferentialProgram): T = {
    assert(
      !isFormulaContext || isTermContext,
      "can only instantiate differential programs within a differential program context " + this,
    )
