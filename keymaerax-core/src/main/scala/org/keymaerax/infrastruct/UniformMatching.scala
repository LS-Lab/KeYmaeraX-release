 ...
 *
 * Their `unify(shape, input)` function matches second argument `input` against the pattern `shape` of the first
 * argument, but not vice versa. Uniform matching leaves `input` alone and only substitutes into `shape`. Fast
...
abstract class UniformMatching extends BaseMatcher {
  ...
  /**
   * `unifies2(s1,s2, t1,t2)` unifies the two expressions of shape (s2,s2) against the two inputs (t1,t2) by sweeping
   * uniform matching.
   * {{{
   *   s1 = t1 | u1     s2 = t2 | u2
  ...
  protected def unifies2(s1: Term, s2: Term, t1: Term, t2: Term): List[SubstRepl] = join(unify(s1, t1), unify(s2, t2))
  ...
  ): List[SubstRepl] = join(unifyODE(s1, t1), unifyODE(s2, t2))
  ...
  protected def unify(e1: Program, e2: Program): List[SubstRepl] = e1 match {
    case _: ProgramConst => unifier(e1, e2)
    case _: SystemConst =>
  ...
  protected def unifyODE(e1: DifferentialProgram, e2: DifferentialProgram): List[SubstRepl] = e1 match {
    case _: DifferentialProgramConst => unifier(e1, e2)
    case DotDiffProgram => unifier(e1, e2)
   ...
   *   1. `unify((•_1, •_2), (a, b))` yields `ua = •_1 ~> a, •_2 ~> b`
   *   1. the inverse is `ua^-1 = a ~> •_1, b ~> •_2`, therefore `ua^-1(e2) = •_1^2*•_2`, resulting in `f(•_1, •_2) ~>
   *      (•_1^2*•_2)`
   *
   * The inverse substitution is applied top-down, i.e., larger abstractions get precedence when components of `t`
   * overlap: `t = (x, y, x + y)` and `e2 = x + y` yields `f(•_1, •_2, •_3) ~> •_3`; not `f(•_1, •_2, •_3) ~> •_1 + •_2`
   */
  ...
  protected override def unify(s1: Sequent, s2: Sequent): List[SubstRepl] =
    if (!(s1.ante.length == s2.ante.length && s1.succ.length == s2.succ.length)) ununifiable(s1, s2)
      ...
      val antesubst = s1
        .ante
        .indices
        .foldLeft(List[SubstRepl]())((subst, i) => joinFolder(subst, s1.ante(i), s2.ante(i)))
