...
/**
 * Like a [[org.keymaerax.core.SubstitutionPair]] replacing `what~>repl` but not checked for substitutability.
 *
 * @param what
 * @param repl
 * @author
...
/**
 * Standalone Renaming Uniform Substitution operation, simultaneously combining [[URename]] and [[USubst]] to uniformly
 * substitute while simultaneously uniformly renaming multiple variables.
 *
 * This implementation uses one-pass uniform substitution implementation a la [[USubstOne]]. Liberal list of
...
final case class USubstRenOne(private[infrastruct] val subsDefsInput: immutable.Seq[(Expression, Expression)])
    extends (Expression => Expression) {
  insist(
    subsDefsInput.forall(sp => sp._1.kind == sp._2.kind),
    "Substitution renaming only to expressions of the same kind: " + this,
  )
  insist(
    subsDefsInput.forall(sp => sp._1.sort == sp._2.sort),
    "Substitution renaming only to expressions of the same sort: " + this,
  )
  insist(
    { val repls = subsDefsInput.map(sp => sp._1).toList; repls.length == repls.distinct.length },
    "No contradictory or duplicate substitutions/renamings: " + this,
  )

  /** substitution part */
  private val subsDefs: immutable.Map[Expression, URenSubstitutionPair] = subsDefsInput
    .filter(sp => !sp._1.isInstanceOf[Variable])
    .map(sp => (sp._1, URenSubstitutionPair(sp._1, sp._2)))
    .toMap

  /** renaming part, augmented with transpositions */
  private val rens: immutable.Map[Variable, Variable] =
    augmentTranspositions(RenUSubst.renamingPartOnly(subsDefsInput).toMap)
  ...
  private val matchHeads: immutable.Map[Function, URenSubstitutionPair] = subsDefs
    .filter(sp => sp._1.isInstanceOf[ApplicationOf])
    .map(sp => (sp._2.what.asInstanceOf[ApplicationOf].func, sp._2))

  // @todo check for substitutable expressions like in USubst

  override def toString: String = "USubstRen{" +
    subsDefsInput.map(sp => sp._1.prettyString + "~>" + sp._2.prettyString).mkString(", ") + "}"

  /**
   * This USubstRen implemented strictly from core operations. Implemented by performing successive uniform renamings
   * after renaming-aware uniform substitution.
   */
  ...
  def apply(p: DifferentialProgram): DifferentialProgram =
    try usubstODE(boundVars(p), p).asInstanceOf[DifferentialProgram]
...
//        (u++boundVars(r), r)
  ...
  private def requireAdmissible(
    ...
    if (!clashes.isEmpty) throw new SubstitutionClashException(
      toString,
      taboo.prettyString,
      e.prettyString,
  ...
  private def substBoundVars(program: Program): SetLattice[Variable] = {
    program match {
      ...
      // @todo optimizable: store boundVars(ProgramConst/SystemConst/DifferentialProgramConst) in substitution pair
      case a: ProgramConst =>
        if (subsDefs.contains(a)) StaticSemantics.boundVars(subsDefs(a).repl.asInstanceOf[Program])
        else StaticSemantics.spaceVars(renSpace(a.space))
      case a: SystemConst =>
        if (subsDefs.contains(a)) StaticSemantics.boundVars(subsDefs(a).repl.asInstanceOf[Program])
        else StaticSemantics.spaceVars(renSpace(a.space))
      case c: DifferentialProgramConst =>
        if (subsDefs.contains(c)) StaticSemantics.boundVars(subsDefs(c).repl.asInstanceOf[DifferentialProgram])
        else StaticSemantics.spaceVars(renSpace(c.space))
