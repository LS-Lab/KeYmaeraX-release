...
object PositionLocator {
  ...
  def withMarkers(e: Expression, pos: PosInExpr): String = {
    ...
    if (pos == HereP) exprPP(e)
      ...
      val (ph, left, right) = placeholder(
        e.sub(pos)
          .getOrElse(
            throw new IllegalArgumentException(
              "Sub-position " + pos.prettyString + " does not exist in " + e.prettyString
            )
          )
      )
...
/**
 * Locates the formula at the specified fixed position. Can optionally specify the expected formula or expected shape of
 * formula at that position as contract.
 */
...
object Fixed {
  def apply(seqPos: Int, inExpr: List[Int], shape: Option[Expression], exact: Boolean): Fixed =
    new Fixed(Position(seqPos, inExpr), shape, exact)
  def apply(seqPos: Int, inExpr: List[Int], shape: Option[Expression]): Fixed =
    new Fixed(Position(seqPos, inExpr), shape)
 ...
 * `start` (remaining in antecedent/succedent as `start` says).
 */
case class Find(goal: Int, shape: Option[Expression], start: Position, exact: Boolean, defs: Declaration)
    extends PositionLocator {
  ...
  final def findPosition(s: Sequent, pos: Position): Option[Position] = {
    require(
      ...
      "Find must point to a valid position in the sequent, but " + start.prettyString + " is undefined in " +
        s.prettyString,
    )
    shape match {
      case Some(f: Formula) =>
        if (!exact) {
          if (UnificationMatch.unifiable(f, s(pos.top)).isDefined) { Some(pos) }
          else {
            val nextPos = pos.advanceIndex(1)
            if (nextPos.isIndexDefined(s)) findPosition(s, nextPos) else None
          }
        } else if (s(pos.top) == f || defs.exhaustiveSubst(s(pos.top)) == substShape.get) { Some(pos) }
        else {
          val nextPos = pos.advanceIndex(1)
          if (nextPos.isIndexDefined(s)) findPosition(s, nextPos) else None
        }
...
case class LastAnte(goal: Int, inExpr: PosInExpr = PosInExpr.HereP) extends PositionLocator {
  ...
  override def toPosition(s: Sequent): Option[Position] = {
    ...
    if (pos.isIndexDefined(s)) Some(pos) else None
  }
}
