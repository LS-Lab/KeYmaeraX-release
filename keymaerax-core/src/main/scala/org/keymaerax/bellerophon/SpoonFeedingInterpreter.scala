...
case class DbBranchPointer(parent: Int, branch: Int, predStep: Int, openBranchesAfterExec: List[Int] = Nil)
  ...
  override def branch(count: Int): List[ExecutionContext] =
    ...
    else if (openBranchesAfterExec.isEmpty) (0 until count)
      .map(DbBranchPointer(predStep, _, predStep, openBranchesAfterExec))
      .toList
    else {
      assert(
        openBranchesAfterExec.size == count,
        "Remaining open branches " + openBranchesAfterExec.size + " and " + count + " subgoal tactics do not match",
      )
      // create branch indexes for elements with same parent but keep order stable (groupBy destroys order)
      val grouped = openBranchesAfterExec
  ...
  override def glue(ctx: ExecutionContext, createdSubgoals: Int): ExecutionContext =
    ...
    if (this != ctx) ctx match {
      case DbAtomPointer(id) =>
        ...
        if (newGoals.nonEmpty)
          DbBranchPointer(parent, branch, id, newGoals.head :: openBranchesAfterExec ++ newGoals.tail)
...
case class SpoonFeedingInterpreter(
  ...
  override def apply(expr: BelleExpr, v: BelleValue): BelleValue = {
    if (runningInner == null) {
      ...
      logger.debug(
        "Handing auxiliary proof of an already running tactic (like initiated by UnifyUSCalculus or Simplifier) to fresh inner interpreter"
      )
      inner(Nil)(expr, v)
    }
  }
      ...
      try {
        val (result, resultCtx) = tactic match {
          case SeqTactic(s) =>
            ...
            nonNilSteps
              .zipWithIndex
              .foldLeft((goal, ctx))({ case ((g, c), (t, i)) =>
                try { runTactic(t, g, level, c, strict, convertPending, executePending) }
                catch {
          ...
          case EitherTactic(s) =>
            for ((t, i) <- s.zipWithIndex.dropRight(1)) {
              try {
                ...
                case e: BelleThrowable =>
                  if (convertPending) runTactic(
                    DebuggingTactics.pending(BellePrettyPrinter(tactic)),
                    goal,
                    level,
                    ctx,
                    strict,
          ...
          case RepeatTactic(child, times) if times >= 1 =>
            ...
            try {
              result = runTactic(
                if (times == 1) child else SeqTactic(Seq(child, RepeatTactic(child, times - 1))),
                result._1,
                level,
                result._2,
                strict,
          ...
          case CaseTactic(children) => goal match {
              case BelleProvable(p, Some(labels)) =>
                ...
                runTactic(
                  BranchTactic(labels.map(getBranchTactic)),
                  goal,
                  level,
                  ctx,
                  strict,
              ...
              case _ =>
                throw new IllFormedTacticApplicationException("Case tactic applied on a proof state without labels")
            }
          case BranchTactic(children) if children.isEmpty =>
            throw new IllFormedTacticApplicationException("Branching with empty children")
          case BranchTactic(children) if children.nonEmpty =>
            goal match {
              case BelleProvable(p, labels) =>
                ...
                val (provables, resultCtx) = branchTactics
                  .zipWithIndex
                  .foldRight((List[BelleValue](), branchCtxs.last))({ case (((ct, cp), i), (accProvables, accCtx)) =>
                    val localCtx = branchCtxs(i).glue(accCtx, 0)
                    // must execute at least some tactic on every branch, even if no-op
                    val branchResult = runTactic(
                      ct,
                      cp,
                      level,
                      localCtx,
                      strict = if (ct.isInstanceOf[NoOpTactic]) true else strict,
                      convertPending,
                      executePending,
                    ) match {
                      case r @ (rp: BelleDelayedSubstProvable, rctx) =>
                        if (rp.p.isProved) {
                          ...
                          val (constification, remaining) = rp
                            .subst
                            .subsDefsInput
                            .partition({
                              case SubstitutionPair(FuncOf(Function(fn, fi, Unit, Real, _), Nothing), v: Variable) =>
                                fn == v.name && fi == v.index
                              case _ => false
                            })
                          if (constification.nonEmpty) {
                            ...
                            val result = rp
                              .parent
                              .map({ case (p, i) => exhaustiveSubst(p, USubst(remaining))(subst, i) })
                              .getOrElse(subst)
                    ...
                    val branchOpenGoals = branchResult._1 match {
                      case bdp: BelleDelayedSubstProvable => bdp.parent match {
                          case Some((pp, i)) =>
                            ...
                            cp match {
                              case BelleProvable(cbp, _) => assertSubMatchesModuloConstification(cbp, pp, 0, bdp.subst)
                            }
                            bdp.p.subgoals.size
                          case None =>
                            cp match {
                              case BelleProvable(cbp, _) =>
                                assertSubMatchesModuloConstification(cbp, bdp.p, 0, bdp.subst)
                            }
                            bdp.p.subgoals.size
                        }
                      case BelleProvable(bp, _) =>
                        cp match {
                          case BelleProvable(cbp, _) => assertSubMatchesModuloConstification(cbp, bp, 0, USubst(Nil))
                        }
                        bp.subgoals.size
                ...
                val (resultProvable, mergedLabels, _) = provables
                  .reverse
                  .zipWithIndex
                  .foldRight[(ProvableSig, List[BelleLabel], List[SubstitutionPair])]((p, origLabels, List.empty))({
                    case ((p: BelleDelayedSubstProvable, i), (provable, labels, substs)) =>
                      val combinedSubsts = combineSubsts(List(p.subst, USubst(substs)))
                      (
                        applySubDerivation(provable, i, p.p, combinedSubsts)._2,
                        updateLabels(labels, i, p),
                        combinedSubsts.subsDefsInput.toList,
                      )
                ...
                val substs = provables.flatMap({
                  case p: BelleDelayedSubstProvable => Some(p.subst)
                  case _ => None
                })
                val rp =
                  if (substs.isEmpty) goal match {
                    case dp: BelleDelayedSubstProvable =>
                      new BelleDelayedSubstProvable(resultProvable, resultLabels, dp.subst, dp.parent)
                    case _: BelleProvable => BelleProvable(resultProvable, resultLabels)
                  }
                  else new BelleDelayedSubstProvable(
                    resultProvable,
                    resultLabels,
                    combineSubsts(substs),
                    goal match {
                      case dp: BelleDelayedSubstProvable => dp.parent
                      case _ => None
                    },
                  )
          ...
          case t @ USubstPatternTactic(children) =>
            val provable = goal match {
              ...
              case _ => throw new IllFormedTacticApplicationException(
                  "Cannot attempt US unification with a non-Provable value."
                ).inContext(tactic, "")
            }
          ...
          case OnAll(e) =>
            ...
            try {
              if (provable.subgoals.size <= 1) runTactic(e, goal, level, ctx, strict, convertPending, executePending)
                ...
                BranchTactic(Seq.tabulate(provable.subgoals.length)(_ => e)),
                goal,
                level,
                ctx,
                strict,
            ...
            } catch { case e: BelleThrowable => throw e.inContext(OnAll(e.context), "") }
          ...
          case Let(abbr, value, innerTactic) =>
            ...
            val (in: ProvableSig, us: USubst, innerMost) =
              try { flattenLets(innerTactic, subst(abbr, value) :: Nil, value -> abbr :: Nil) }
              catch {
                case e: Throwable => throw new IllFormedTacticApplicationException(
                    "Unable to start inner proof in let: " + e.getMessage,
                    e,
                  )
              }
            ...
            if (descend > 0) {
              ...
              val innerFeeder = SpoonFeedingInterpreter(
                innerId,
                -1,
                idProvider,
                defs,
              ...
              val result = innerFeeder.runTactic(
                innerMost,
                BelleProvable(in, None),
                level,
                DbAtomPointer(-1),
                strict,
                convertPending,
              ...
              runningInner(tactic, BelleProvable(provable.sub(0), lbl)) match {
                case p: BelleDelayedSubstProvable =>
                  val r = applySubDerivation(provable, 0, p.p, p.subst) match {
                    case (true, mergedProvable) =>
                      (new BelleDelayedSubstProvable(mergedProvable, lbl, p.subst, p.parent), ctx.store(tactic))
                  }
                  runningInner = null
                  r
          ...
          // look into tactics
          case d: DependentTactic if level > 0 || d.isInternal =>
            try {
              val v = goal
          ...
          case TryCatch(t, catchClazz, doCatch, doFinally) =>
            ...
            Try(runTactic(t, goal, level, ctx, strict, convertPending, executePending)) match {
              ...
              case Failure(ex) => matchingCause(ex) match {
                  ...
                  case Some(cause) => Try(runTactic(
                      doCatch(catchClazz.cast(cause)),
                      goal,
                      level,
                      ctx,
                      strict,
                ...
                if (ctx.onBranch >= 0) tactic match {
                  ...
                  case _ =>
                    if (provable.subgoals.size > 1) {
                      ...
                      result._1 match {
                        case dp: BelleDelayedSubstProvable =>
                          if (BelleProvable(dp.p, dp.label) != goal) throw new IllFormedTacticApplicationException(
                            "Tactic " + tactic.prettyString + " not suitable for " + provable.subgoals.size +
                              " subgoals"
                          )
                          else result
                        case r =>
                          if (r != goal) throw new IllFormedTacticApplicationException(
                            "Tactic " + tactic.prettyString + " not suitable for " + provable.subgoals.size +
                              " subgoals"
                          )
                          else result
                      ...
                      assert(
                        recordInternal || !BelleExpr.isInternal(tactic.prettyString),
                        "Unable to record internal tactic",
                      )
                      runningInner =
                        inner(listenerFactory(rootProofId)(tactic.prettyString, ctx.parentId, ctx.onBranch))
                        ...
                        case p: BelleDelayedSubstProvable =>
                          ...
                          val parent = p.parent match {
                            ...
                            case Some((pparent, pidx)) =>
                              ...
                              else if (pparent.conclusion == provable.subgoals(pidx))
                                Some(provable(pparent, pidx), pidx)
                              // @todo pparent.conclusion constified subgoal need to first finish dIRule subproof to unconstify
                              else throw new NotImplementedError(
                                "Delayed substitution parent provables: missing implementation to merge " +
                                  pparent.prettyString + " with " + provable.prettyString
                              )
                          }
                          val result = (
                            new BelleDelayedSubstProvable(mergedProvable, resultLabels, p.subst, parent),
                            ctx.store(tactic),
                          )
                          runningInner = null
                          result
                  ...
                  val newCtx = ctx match {
                    case DbBranchPointer(_, _, predStep, soleBranch :: Nil) =>
                      DbBranchPointer(soleBranch, 0, predStep) // DbAtomPointer(soleBranch)
                  ...
                  runTactic(tactic, goal, level, newCtx, strict, convertPending, executePending) match {
                    case (bp @ BelleProvable(p, _), resultCtx) => ctx match {
                        ...
                        case dbp @ DbBranchPointer(_, _, predStep, openBranchesAfterExec) =>
                          ...
                          if (p.subgoals.size != provable.subgoals.size) (
                            bp,
                            dbp.copy(
                              predStep = predStep + numSteps,
                              openBranchesAfterExec = openBranchesAfterExec.dropRight(1),
                            ),
                          )
                  ...
                  (goal, ctx) match {
                    case (BelleProvable(goalP, _), DbBranchPointer(_, _, _, openBranches)) =>
                      assert(
                        goalP.subgoals.size == openBranches.size,
                        "Open subgoals and expected open subgoals do not match",
                      )
                      ...
                      runTactic(tactic, goal, level, newCtx, strict, convertPending, executePending) match {
                        case (bp @ BelleProvable(resultP, _), resultCtx) => ctx match {
                            ...
                            case dbp @ DbBranchPointer(_, _, predStep, openBranchesAfterExec)
                                if openBranchesAfterExec.size == openBranches.size =>
                              val numSteps = (newCtx, resultCtx) match {
                                case (DbAtomPointer(i), DbAtomPointer(j)) => j - i
                                case (DbAtomPointer(i), _: DbBranchPointer) => ???
                              }
                              if (resultP.subgoals.size != provable.subgoals.size) (
                                bp,
                                dbp.copy(
                                  predStep = predStep + numSteps,
                                  openBranchesAfterExec = openBranchesAfterExec.dropRight(1),
        ...
        val preservedSubstResult = goal match {
          case p: BelleDelayedSubstProvable => result match {
              case fp: BelleDelayedSubstProvable =>
                assert(
                  p.parent.isEmpty || fp.parent.isEmpty ||
                    exhaustiveSubst(p.parent.get._1, p.subst ++ fp.subst) ==
                    exhaustiveSubst(fp.parent.get._1, p.subst ++ fp.subst),
        ...
        case e: BelleThrowable =>
          if (convertPending) tactic match {
            ...
            case BranchTactic(children) => runTactic(
                BranchTactic(children.map(c => DebuggingTactics.pending(BellePrettyPrinter(c)))),
                goal,
                level,
                ctx,
                strict,
  ...
  /**
   * Applies `sub` to subgoal `i` of `goal`. Expands definitions `defs` and substitutions found by unification prior to
   * merging provables, if necessary. Returns the merged provable and the applied substitutions.
   */
  private def applySub(goal: ProvableSig, sub: ProvableSig, i: Int): (ProvableSig, USubst) = {
    ...
    val symbols = allSymbols --
      StaticSemantics.symbols(goal.subgoals(i)).intersect(StaticSemantics.symbols(sub.conclusion))
    val defSubsts = USubst(
      goal
        .defs
        .substs
        .filter({ case SubstitutionPair(what, _) => symbols.intersect(StaticSemantics.symbols(what)).nonEmpty })
    )
    ...
    if (substGoal.subgoals(i) == substSub.conclusion) (substGoal(substSub, i), defSubsts)
      ...
      if (substGoal != goal || substSub != sub)
        applySub(substGoal, substSub, i) match { case (p, s) => (p, defSubsts ++ s) }
      else {
