...
object UIIndex {
   ...
   * that this expression occurs at the indicated position pos in the given sequent. Disregard tactics that require
   * input.
   */
  def theStepAt(
      expr: Expression,
  ...
  ): Option[DerivationInfo] = expr match {
   ...
   * optionally considering that this expression occurs at the indicated position pos in the given sequent.
   */
  def allStepsAt(
      expr: Expression,
      pos: Option[Position],
      ...
      expr match {
        ...
        case Box(a, post) =>
          ...
          a match {
            ...
            case ODESystem(ode, _) if !post.isInstanceOf[Modal] && containsPrime(post) =>
              ode match {
                case _: AtomicODE => "DE differential effect" :: "dW" :: "dC" :: (maybeSplit :+ "GV" :+ "MR")
                case _: DifferentialProduct => "DE differential effect (system)" :: "dW" :: "dC" ::
                    (maybeSplit :+ "GV" :+ "MR")
                case _ => maybeSplit :+ "GV" :+ "MR"
              }
            case ODESystem(_, _) =>
              if (pos.forall(_.isSucc)) {
                if (pos.forall(_.isTopLevel))
                  ("ODE" :: "ODEinv" :: "solve" :: "dC" :: "dIRule" :: "dW" :: "dG" :: Nil) ++
                    (maybeSplit :+ "GV" :+ "MR")
                else ("solve" :: "dC" :: "dIRule" :: "dW" :: "dG" :: Nil) ++ (maybeSplit :+ "GV" :+ "MR")
              } else ("solve" :: "dC" :: Nil) ++ (maybeSplit :+ "GV" :+ "MR")
            case ProgramConst(name, _) if substs.exists({
                  case SubstitutionPair(ProgramConst(wn, _), _) => wn == name
                  case _ => false
                }) => s"""expand("$name")""" :: rules
            case SystemConst(name, _) if substs.exists({
                  case SubstitutionPair(SystemConst(wn, _), _) => wn == name
                  case _ => false
                }) => s"""expand("$name")""" :: rules
            case _ => rules
          }
        ...
        case Diamond(a, post) =>
          ...
          a match {
            ...
            case _: Loop =>
              if (isTop) { if (isSucc) "con" +: maybeSplit :+ "<*> iterate" else "fp" +: maybeSplit :+ "<*> iterate" }
              else "<*> iterate" +: maybeSplit
            case ODESystem(_, _) =>
              if (pos.forall(_.isSucc)) {
                if (pos.forall(_.isTopLevel))
                  ("dV" :: "solve" :: "kDomainDiamond" :: "dDR" :: "compatCut" :: "gEx" :: Nil)
  ...
  def allTwoPosSteps(pos1: Position, pos2: Position, sequent: Sequent): List[DerivationInfo] = {
    ...
    ((pos1, pos2, expr1, expr2) match {
      case (p1: AntePosition, p2: SuccPosition, Some(e1), Some(e2)) if p1.isTopLevel && p2.isTopLevel && e1 == e2 =>
        "closeId" :: Nil
      case (p1: AntePosition, p2: SuccPosition, Some(e1), Some(e2))
          if p1.isTopLevel && !p2.isTopLevel && e1 == e2 => /*@todo "knownR" ::*/ Nil
      ...
      case (_: AntePosition, _, Some(_: Equiv), Some(_: Formula)) => "instantiatedEquivRewriting" ::
          Nil // @note for applying function definitions.
