...
object BelleParser extends TacticParser with Logging {
  ...
  case class DefScope[K, V](
      defs: scala.collection.mutable.Map[K, V] = scala.collection.mutable.Map.empty[K, V],
  ...
  /**
   * Parses `t` backwards-compatible with definitions `defs` expanded in tactic arguments depending on whether `t`
   * expands symbols or uses substitutions explicitly or not.
   */
  def parseBackwardsCompatible(t: String, defs: Declaration): BelleExpr =
    if (
      !Configuration.getBoolean(Configuration.Keys.TACTIC_AUTO_EXPAND_DEFS_COMPATIBILITY).getOrElse(false) ||
      tacticExpandsDefsExplicitly(t) || tacticSubstsDefsExplicitly(t)
    ) BelleParser.parseWithInvGen(t, None, defs)
  ...
  def parseWithTacticDefs(s: String, defs: Map[String, BelleExpr]): BelleExpr = {
    firstUnacceptableCharacter(s) match {
      ...
      case None => parseTokenStream(
          BelleLexer(s),
          DefScope[String, DefTactic](
            scala.collection.mutable.Map[String, DefTactic](defs.map({ case (n, t) => (n, DefTactic(n, t)) }).toList*)
          ),
          None,
  ...
  /**
   * Non-unicode characters that are allowed in KeYmaera X input files. Should correspond to the unicode that's printed
   * in the web UI.
   */
  ...
  private val TACTIC_AUTO_EXPAND_DEFS_COMPATIBILITY = Configuration
    .getBoolean(Configuration.Keys.TACTIC_AUTO_EXPAND_DEFS_COMPATIBILITY)
    .getOrElse(false)
    ...
    if (disallowedChars.nonEmpty) {
      val nonAsciiCharacter: Char = {
        if (disallowedChars.nonEmpty) disallowedChars.head
          ...
          "Expected at least one match but matchData.hasNext returned false when matches.nonEmpty was true!"
        )
      }
  ...
  def parseTokenStream(
    ...
    result.stack match {
      case Bottom :+ BelleAccept(e) =>
        if (expandAll && defs.substs.nonEmpty) TactixLibrary.expandAllDefs(defs.substs) & e else e
  ...
  private def parseStep(
    ...
    stack match {
      ...
      case r :+ ParsedBelleExpr(left, leftLoc, None) :+ BelleToken(SEQ_COMBINATOR | DEPRECATED_SEQ_COMBINATOR, _) :+
          BelleToken(BRANCH_COMBINATOR, branchCombinatorLoc) => ParserState(
      ...
      case r :+ ParsedBelleExpr(left, leftLoc, None) :+ BelleToken(SEQ_COMBINATOR | DEPRECATED_SEQ_COMBINATOR, _) :+
          ...
          case Some(BelleToken(SEQ_COMBINATOR | DEPRECATED_SEQ_COMBINATOR, _)) =>
            ParserState(st.stack :+ st.input.head, st.input.tail)
      ...
      case r :+ ParsedBelleExpr(left, leftLoc, l) :+ BelleToken(BRANCH_COMBINATOR, combinatorLoc) :+
          ParsedBelleExprList(branchTactics) =>
        assert(branchTactics.nonEmpty)
        val parsedExpr = {
          if (branchTactics.forall(_.label.isDefined)) left.switch(branchTactics.map(t => t.label.get -> t.expr)*)
          else if (branchTactics.forall(_.label.isEmpty)) left.<(branchTactics.map(_.expr)*)
          else {
            ...
            throw ParseException(
              msg,
              branchTactics.find(_.label.isEmpty).map(_.loc.begin).getOrElse(UnknownLocation),
              "<missing>",
              "\"A label\":",
            )
          }
      ...
      case r :+ ParsedBelleExpr(left, leftLoc, l) :+ BelleToken(BRANCH_COMBINATOR, combinatorLoc) :+
        ...
        val parsedExpr = left.<(Seq.empty[BelleExpr]*)
      ...
      case _ :+ ParsedBelleExpr(_, _, _) :+ BelleToken(BRANCH_COMBINATOR, loc) :+ BelleToken(OPEN_PAREN, _) :+
          ParsedBelleExpr(_, _, _) :+ BelleToken(CLOSE_PAREN, _) =>
      ...
      case _ :+ BelleToken(_: EXPRESSION, _) :+ BelleToken(COLON, _) =>
        ParserState(stack :+ st.input.head, st.input.tail)
      ...
      case r :+ BelleToken(label: EXPRESSION, l1) :+ BelleToken(COLON, _) :+ ParsedBelleExpr(expr, l2, None) =>
        st.input.headOption match {
          ...
          case _ =>
            ParserState(r :+ ParsedBelleExpr(expr, l1.spanTo(l2), Some(label.undelimitedExprString.asLabel)), st.input)
        }
      ...
      case _ :+ ParsedBelleExpr(_, _, _) :+ BelleToken(COMMA, commaLoc) => st.input.headOption match {
          case Some(inputHead) =>
            if (hasOpenParen(st)) ParserState(st.stack :+ inputHead, st.input.tail)
              ...
              "Comma-separated lists of expressions need to be surrounded by parentheses.",
              commaLoc,
            )
      ...
      case r :+ BelleToken(LET, loc) => st.input match {
          case BelleToken(OPEN_PAREN, _) :: BelleToken(expr: EXPRESSION, _) :: BelleToken(CLOSE_PAREN, _) ::
              BelleToken(IN, _) :: tail =>
      ...
      case r :+ BelleToken(TACTIC, loc) => st.input match {
          case BelleToken(IDENT(name), _) :: BelleToken(AS, _) :: tail =>
            if (tacticDefs.defs.contains(name))
              throw BelleParseException(s"Tactic definition: unique name '$name' expected in scope", st)
            val (innerExpr, innerLoc, remainder) = parseInnerExpr(tail, tacticDefs, g, defs, expandAll)
      ...
      case r :+ ParsedBelleExpr(expr, eLoc, l) :+ BelleToken(USING, _) => st.input match {
          case BelleToken(EXPRESSION(s, delims), esLoc) :: tail =>
            val fmls = s
              .stripPrefix(delims._1)
              .stripSuffix(delims._2)
              .split(Regex.quote(DOUBLE_COLON))
              .map(_.trim)
              .toList match {
              ...
              case l => throw ParseException(
                  ...
                  esLoc,
                  s,
                  l.mkString(DOUBLE_QUOTE, DOUBLE_COLON, DOUBLE_COLON + LIST_END + DOUBLE_QUOTE),
                )
            }
      ...
      case r :+ BelleToken(IDENT(name), identLoc) =>
        try {
          ...
          case e: ClassCastException => throw ParseException(
              s"Could not convert tactic $name because the arguments passed to it were of incorrect type",
              e,
            )
        }
      // endregion
      ...
      case r :+ BelleToken(EOF, _) =>
        if (st.input.isEmpty) ParserState(r, st.input)
          ...
          "Internal parser error: did not expect to find EOF while input stream is unempty.",
          UnknownLocation,
        )
      ...
      case Bottom =>
        if (st.input.isEmpty)
          throw ParseException(
            "Empty inputs are not parsable.",
            UnknownLocation,
          ) // ParserState(stack :+ FinalItem(), Nil) //Disallow empty inputs.
          ...
          "Bellerophon programs should start with identifiers, open parens, or optional/doall/partial.",
          st.input.head.location,
        )
      ...
      case r :+ ParsedBelleExpr(e, _, _) => st.input.headOption match {
          ...
          case None =>
            ...
            else throw BelleParseException(
              s"Cannot continue parsing because detected an infinite loop with stack ${st.topString}",
              st,
            )
        }
  ...
  private def constructTactic(
    ...
    if (tacticDef.isDefined) {
      ...
      try {
        ...
        if (
          TACTIC_AUTO_EXPAND_DEFS_COMPATIBILITY && expandAll &&
          defs.decls.exists(d => d._2.interpretation.isRight && d._2.interpretation.toOption.get.isDefined)
        ) {
          t.prettyString match {
            case "QE" | "smartQE" | "master" => TactixLibrary.expandAllDefs(defs.substs) & t
            case text =>
              if (text.startsWith("ODE") || text.startsWith("diffInvariant") || text.startsWith("dI")) TactixLibrary
                .expandAllDefs(defs.substs) & t
              else t
          }
        } else t
      } catch {
        case e: ParseException => throw ParseException(e.getMessage + s" Encountered while parsing $name", location, e)
      }
    }
  }
  ...
  private def parseArgumentList(
      ...
      defs: Declaration,
      expandAll: Boolean,
  ): (List[TacticArg], TokenStream) = input match {
    case BelleToken(OPEN_PAREN, _) +: rest =>
      ...
      def arguments(al: List[BelleToken]): List[TacticArg] = al match {
        ...
        case BelleToken(ABSOLUTE_POSITION(posString), l1) :: BelleToken(matchKind, _) ::
            BelleToken(expr: EXPRESSION, l2) :: tail =>
          val what: Formula = expr.expression match {
            case f: Formula => expand(f)
            case FuncOf(Function(name, idx, domain, _, _), child) =>
              PredOf(Function(name, idx, domain, Bool), expand(child))
            case e =>
              throw ParseException("Expected formula as exact position locator match, but got " + e.prettyString, l2)
          }
          Right(parsePositionLocator(posString, l1.spanTo(l2), Some(what), exact = matchKind == EXACT_MATCH, defs)) +:
            arguments(tail)
        case BelleToken(p: BASE_POSITION, l1) :: BelleToken(matchKind, _) :: BelleToken(expr: EXPRESSION, l2) :: tail =>
          Right(parsePositionLocator(
            p.positionString,
            l1.spanTo(l2),
            Some(expand(expr.expression)),
            exact = matchKind == EXACT_MATCH,
            defs,
          )) +: arguments(tail)
        case BelleToken(p: BASE_POSITION, l1) :: BelleToken(matchKind, _) :: BelleToken(expr: EXPRESSION_SUB, l2) ::
            tail => Right(parsePositionLocator(
            p.positionString,
            l1.spanTo(l2),
            Some(expand(expr.expression._1)),
            expr.expression._2,
            exact = matchKind == EXACT_MATCH,
            defs,
          )) +: arguments(tail)
  ...
  private def parseArgumentToken(
      ...
      defs: Declaration,
  )(tok: BelleToken, loc: Location): TacticArg = tok.terminal match {
    case terminal: TACTIC_ARGUMENT => terminal match {
        case ABSOLUTE_POSITION(posString) =>
          Right(parsePositionLocator(posString, tok.location, None, exact = true, defs))
        case LAST_ANTECEDENT(posString) =>
          Right(parsePositionLocator(posString, tok.location, None, exact = true, defs))
        case LAST_SUCCEDENT(posString) => Right(parsePositionLocator(posString, tok.location, None, exact = true, defs))
        case SEARCH_ANTECEDENT(posString) =>
          Right(parsePositionLocator(posString, tok.location, None, exact = true, defs))
        case SEARCH_SUCCEDENT(posString) =>
          Right(parsePositionLocator(posString, tok.location, None, exact = true, defs))
        case SEARCH_EVERYWHERE(posString) =>
          Right(parsePositionLocator(posString, tok.location, None, exact = true, defs))
        ...
        case _ => throw ParseException(
            s"Expected a tactic argument (Belle Expression or position locator) but found ${terminal.img}",
            tok.location,
          )
      }
    case _ => throw ParseException(
        "Encountered non-tactic-argument terminal when trying to parse a tactic argument",
        tok.location,
      )
  }
  ...
  private def parseArg(ai: ArgInfo, undelim: String, loc: Location): TacticArg = ai match {
    ...
    case _: SubstitutionArg =>
      try { Left(undelim.asSubstitutionPair) }
      catch {
        case exn: ParseException => throw ParseException(
            s"Could not parse $undelim as a substitution pair when a substitution pair was expected. Error: $exn",
            loc,
            exn,
          )
      }
    case _: FormulaArg =>
      try { Left(undelim.asFormula) }
      catch {
        case exn: ParseException => throw ParseException(
            s"Could not parse $undelim as a formula when a formula was expected. Error: $exn",
            loc,
            exn,
          )
      }
    case _: TermArg =>
      try { Left(undelim.asTerm) }
      catch {
        case exn: ParseException =>
          throw ParseException(s"Could not parse $undelim as a term when a term was expected. Error: $exn", loc, exn)
      }
    case _: VariableArg =>
      try { Left(undelim.asVariable) }
      catch {
        case exn: ParseException => throw ParseException(
            s"Could not parse $undelim as a variable when a variable was expected. Error: $exn",
            loc,
            exn,
          )
      }
    case _: NumberArg =>
      try {
        ...
        case exn: ParseException => throw ParseException(
            s"Could not parse $undelim as an expression when an expression was expected. Error: $exn",
            loc,
            exn,
          )
      }
    case _: ExpressionArg =>
      try { Left(undelim.asExpr) }
      catch {
        case exn: ParseException => throw ParseException(
            s"Could not parse $undelim as an expression when an expression was expected. Error: $exn",
            loc,
    ...
    case ListArg(ai) =>
      ...
      if (listElems.last == "nil") Left(listElems.dropRight(1).map(parseArg(ai, _, loc).left.toOption.get).toList)
        ...
        parseArg(ai, listElems.head, loc) // allow single-element lists without ::nil notation
      else throw ParseException(
        s"Could not parse $undelim as a list when a list of ${ai.sort} was expected; lists must end in :: nil",
        loc,
      )
  }
  ...
  private def parseInt(s: String, location: Location, nonZero: Boolean = true) =
    try {
      ...
      if (nonZero && pos == 0)
        throw ParseException("0 is not a valid absolute (sub)position -- must be (-inf, -1] \\cup [1, inf)", location)
      else pos
    } catch {
      case _: NumberFormatException =>
        throw ParseException("Could not parse absolute position a (sequence of) integer(s)", location)
    }
