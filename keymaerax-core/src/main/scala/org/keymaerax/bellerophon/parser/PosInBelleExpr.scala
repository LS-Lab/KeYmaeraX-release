...
case class PosInBelleExpr(idxs: Seq[Int]) {
  ...
  def apply(e: BelleExpr) = e match {
    ...
    case e: SeqTactic =>
      if (hd == 0) e.seq.head else if (hd == 1) SeqTactic(e.seq.tail) else throw new PositionException(hd, e)
    case EitherTactic(l :: r :: Nil) => if (hd == 0) l else if (hd == 1) r else throw new PositionException(hd, e)
    case e: EitherTactic =>
      if (hd == 0) e.alts.head else if (hd == 1) EitherTactic(e.alts.tail) else throw new PositionException(hd, e)
    ...
    case e: USubstPatternTactic =>
      if (hd < e.options.length * 2) { if (hd % 2 == 0) e.options(hd)._1 else e.options(((hd - 1) / 2))._2 }
