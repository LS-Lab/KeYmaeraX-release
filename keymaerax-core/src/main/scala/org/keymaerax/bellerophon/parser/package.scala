 ...
 *   - [[org.keymaerax.bellerophon.parser.BelleParser tactic parser]] for BelleExpr
 *
 * All Bellerophon tactic expressions are of type [[org.keymaerax.bellerophon.BelleExpr]], which provides the following
 * tactic combinators
 *
 *   - `s ; t` [[org.keymaerax.bellerophon.SeqTactic sequential composition]] executes `t` on the output of `s`, failing
 *     if either fail.
