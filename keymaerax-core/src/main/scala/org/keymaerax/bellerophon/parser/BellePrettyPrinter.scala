...
object BellePrettyPrinter extends (BelleExpr => String) {
  ...
  private def pp(e: BelleExpr, indent: Int): String = {
    ...
    Try(TacticInfo.apply(e.prettyString)).toOption match {
      ...
      case _ => e match {
          ...
          case SeqTactic(l +: (r: SeqTactic) +: Nil) =>
            sanitizeBinary(wrapLeft(e, l, indent), op(e).terminal.img, wrap(r, indent))
          case SeqTactic(l +: r +: Nil) =>
            sanitizeBinary(wrapLeft(e, l, indent), op(e).terminal.img, wrapRight(e, r, indent))
          case SeqTactic(s +: t) =>
            sanitizeBinary(wrapLeft(e, s, indent), op(e).terminal.img, wrapRight(e, SeqTactic(t), indent))
          case EitherTactic(l +: (r: EitherTactic) +: Nil) =>
            sanitizeBinary(wrapLeft(e, l, indent), op(e).terminal.img, wrap(r, indent))
          case EitherTactic(l +: r +: Nil) =>
            sanitizeBinary(wrapLeft(e, l, indent), op(e).terminal.img, wrapRight(e, r, indent))
          case EitherTactic(s +: t) =>
            sanitizeBinary(wrapLeft(e, s, indent), op(e).terminal.img, wrapRight(e, EitherTactic(t), indent))
          case BranchTactic(ts) => op(e).terminal.img + OPEN_PAREN.img + newline(indent + 1) +
              ts.map(pp(_, indent + 1)).mkString(COMMA.img + newline(indent + 1)) + newline(indent) + CLOSE_PAREN.img
          case CaseTactic(ts) => op(e).terminal.img + OPEN_PAREN.img + newline(indent + 1) +
              ts.map({ case (label, t) =>
                  DOUBLE_QUOTE + label.prettyString + DOUBLE_QUOTE + COLON.img + SPACE + pp(t, indent + 2)
                })
                .mkString(COMMA.img + newline(indent + 1)) + newline(indent) + CLOSE_PAREN.img
          case SaturateTactic(t) => sanitizeUnary(wrapLeft(e, t, indent), op(e).terminal.img)
          case it: StringInputTactic =>
            if (it.inputs.nonEmpty) {
              val eargs = BelleExpr
                .persistable(it.inputs)
                .flatMap(input => argPrinter(Left(input)))
                .mkString(COMMA.img + SPACE)
          ...
          case Let(abbr, value, inner) => op(e).terminal.img + SPACE + OPEN_PAREN.img +
              argPrinter(Left((abbr, value) match {
                case (a: Term, v: Term) => Equal(a, v)
                case (a: Formula, v: Formula) => org.keymaerax.core.Equiv(a, v)
          ...
          case t @ DependentPositionTactic(name) =>
            if (!t.isInternal) name // name of a DependentPositionTactic is the codeName
              ...
              "Anonymous tactic cannot be re-parsed: please replace anonymous tactic with its inner steps."
            )
          case adp: AppliedDependentPositionTactic => adp.pt match {
              case e: DependentPositionWithAppliedInputTactic =>
                ...
                e.name + OPEN_PAREN.img + eargs + sep + argPrinter(Right(adp.locator)).getOrElse(EMPTY_ARGS) +
                  CLOSE_PAREN.img
              case e: DependentPositionTactic =>
                val argStr = argPrinter(Right(adp.locator))
                  .map(s => OPEN_PAREN.img + s + CLOSE_PAREN.img)
                  .getOrElse(EMPTY_ARGS)
          ...
          case it: InputTactic =>
            ...
            val argString =
              if (eargs.isEmpty) EMPTY_ARGS
              else OPEN_PAREN.img + (if (eargs == "\"nil\"") EMPTY_ARGS else eargs) + CLOSE_PAREN.img
            it.name + argString
          case t: AppliedBuiltinTwoPositionTactic => t.positionTactic.name + OPEN_PAREN.img + t.posOne.prettyString +
              COMMA.img + SPACE + t.posTwo.prettyString + CLOSE_PAREN.img
          ...
          case LabelBranch(BelleLabelTx(BelleSubLabel(BelleRollbackTxLabel, label), None, _)) =>
            LabelBranch(BelleTopLevelLabel(label)).prettyString
          ...
          case Using(es, t) =>
            (t match {
              case _: SeqTactic | _: EitherTactic | _: RepeatTactic | _: CaseTactic | _: ParallelTactic |
                  _: SaturateTactic | _: Using | _: BranchTactic => OPEN_PAREN.img + pp(t, indent) + CLOSE_PAREN.img
              case _ => pp(t, indent)
            }) + SPACE + USING.img + SPACE + DOUBLE_QUOTE +
              es.map(KeYmaeraXOmitInterpretationPrettyPrinter).mkString(SPACE + DOUBLE_COLON + SPACE) +
              (if (es.isEmpty) LIST_END else if (es.size == 1) "" else SPACE + DOUBLE_COLON + SPACE + LIST_END) +
              DOUBLE_QUOTE
  ...
  private def argListPrinter(args: List[BelleParser.TacticArg]) = {
    ...
    if (args.isEmpty) EMPTY_ARGS
    else OPEN_PAREN.img + BelleExpr.persistable(args).flatMap(argPrinter).reduce(_ + COMMA + SPACE + _) +
      CLOSE_PAREN.img
  }
  ...
  private def argPrinter(arg: BelleParser.TacticArg): Option[String] = {
    arg match {
      ...
      case Left(x :: xs) => Some(
          (x :: xs)
            .flatMap(c => argPrinter(Left(c)))
            .map(_.stripPrefix(DOUBLE_QUOTE).stripSuffix(DOUBLE_QUOTE))
  ...
  private def wrapLeft(parent: BelleExpr, current: BelleExpr, indent: Int): String =
    if (op(parent) < op(current) || (op(parent) == op(current) && !op(current).leftAssoc)) wrap(current, indent)
    else pp(current, indent)
