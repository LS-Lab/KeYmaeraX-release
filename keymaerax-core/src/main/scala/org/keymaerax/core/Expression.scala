...
sealed trait ProgramComparison extends AtomicFormula with BinaryComposite {
...
object DifferentialProduct {
  ...
  private def reassociate(left: DifferentialProgram, right: DifferentialProgram): DifferentialProduct = left match {
    ...
    case DotDiffProgram => new DifferentialProduct(DotDiffProgram, right)
