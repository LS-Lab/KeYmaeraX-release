...
class ProverException(msg: String, cause: Throwable = null) extends RuntimeException(msg, cause) {
  ...
  /**
   * Returns the logical context, i.e. stack of proof rules or nested logical context information describing in which
   * context this exception occurred.
   */
...
/**
 * Reasoning exceptions directly from the KeYmaera X Prover Kernel. The most important distinction of prover kernel
 * exceptions are [[CriticalCoreException]] for unsound logical mistakes versus [[NoncriticalCoreException]] for
 * plausible but inappropriate reasoning attempts.
 */
...
/**
 * Critical reasoning exceptions directly from the KeYmaera X Prover Core that indicate a proof step was attempted that
 * would be unsound, and was consequently denied.
 */
class CriticalCoreException(msg: String) extends CoreException(msg)

/**
 * Exceptions that arise from trying to substitute a subderivation in for a subgoal that does not equal the conclusion
 * of the subderivation.
 * @see
 *   [[org.keymaerax.core.Provable.apply(rule:edu\.cmu\.cs\.ls\.keymaerax\.core\.Provable,subgoal:edu\.cmu\.cs\.ls\.keymaerax\.core\.Provable\.Subgoal):edu\.cmu\.cs\.ls\.keymaerax\.core\.Provable*]]
 */
case class SubderivationSubstitutionException(
    subderivation: String /*Provable*/,
    conclusion: String,
    subgoal: String /*Sequent*/,
    subgoalid: Int,
    provable: String, /*Provable*/
) extends CriticalCoreException(
 ...
 */
case class SubstitutionClashException(
    subst: String /*USubst*/,
    U: String /*SetLattice[NamedSymbol]*/,
    e: String /*Expression*/,
    context: String /*Expression*/,
    clashes: String /*SetLattice[NamedSymbol]*/,
    info: String = "",
) extends CriticalCoreException(
...
case class SkolemClashException(
    msg: String,
    clashedNames: SetLattice[Variable],
    vars: String /*Seq[Variable]*/,
    s: String, /*Sequent*/
...
/**
 * Noncritical reasoning exceptions directly from the KeYmaera X Prover Core that indicate a proof step was perfectly
 * plausible to try, just did not fit the expected pattern, and consequently turned out impossible.
 */
class NoncriticalCoreException(msg: String) extends CoreException(msg)

/**
 * Rule is not applicable as indicated. For example, InapplicableRuleException can be raised when trying to apply
 * [[AndRight]] at the correct position 2 in bounds on the right-hand side where it turns out there is an Or formula not
...
case class UnknownOperatorException(msg: String, e: Expression)
    extends ProverException(msg + ": " + e.prettyString + " of " + e.getClass + " " + e) {}

/**
 * Nil case for exceptions, which is almost useless except when an exception type is needed to indicate that there
 * really was none.
 */
