...
sealed trait SetLattice[A] {
  ...
  def subsetOf(other: SetLattice[A]): Boolean = (this, other) match {
    ...
    case (FiniteSet(ts), os: CoFiniteSet[A]) =>
      ts.intersect(os.excluded).isEmpty /* this is a subset of that top if that doesn't exclude any of this */
    case (ts: CoFiniteSet[A], FiniteSet(_)) => false /* infinite sets are not a subset of any finite set */
    case (ts: CoFiniteSet[A], os: CoFiniteSet[A]) =>
      os.excluded
        .subsetOf(ts.excluded) /* this coset is a subset of that coset if that excluded at most this's excluded */
  }

  /** `this++other`: Set union. */
  def ++(other: SetLattice[A]): SetLattice[A] = {
    (this, other) match {
      case (_, FiniteSet(os)) => this ++ os
      case (FiniteSet(ts), os: CoFiniteSet[A]) => os ++ ts
      case (ts: CoFiniteSet[A], os: CoFiniteSet[A]) => CoFiniteSet(
          ts.excluded.intersect(os.excluded),
  ...
  def --(other: SetLattice[A]): SetLattice[A] = {
    (this, other) match {
      ...
      case (FiniteSet(ts), os: CoFiniteSet[A]) =>
        FiniteSet(
          ts.intersect(os.excluded)
        ) /* ts -- (all except os) == ts/\os, subtract everything but excluded, so retain excluded */
      case (ts: CoFiniteSet[A], os: CoFiniteSet[A]) =>
        FiniteSet(
  ...
  def intersect(other: SetLattice[A]): SetLattice[A] = {
    (this, other) match {
      ...
      case (FiniteSet(ts), os: CoFiniteSet[A]) =>
        FiniteSet(
          ts -- os.excluded
        ) /* ts /\ (all except os) == ts--os, only exclusions disappear upon intersection with infinite sets */
      case (ts: CoFiniteSet[A], os: CoFiniteSet[A]) => CoFiniteSet(
          ts.excluded ++ os.excluded,
...
object SetLattice {
  ...
  def extendToDifferentialSymbols(sl: SetLattice[Variable]): SetLattice[Variable] = sl match {
    ...
    case CoFiniteSet(excluded, symbols)
        if excluded.forall(x => !x.isInstanceOf[BaseVariable] || excluded.contains(DifferentialSymbol(x))) => sl
...
/**
 * Represents a co-set i.e. an infinite set that is the complement of a finite set. It includes all elements, except
 * explicitly excluded ones. Additionally, keeps track of which symbols specifically are named to be included (i.e., a
...
private case class CoFiniteSet[A](excluded: immutable.Set[A], literally: immutable.Set[A]) extends SetLattice[A] {
  ...
  def --(other: Iterable[A]): CoFiniteSet[A] = CoFiniteSet(excluded ++ other, literally -- other)
  def intersect(other: immutable.Set[A]): SetLattice[A] =
    FiniteSet(other -- excluded) /* (all except ts) /\ os == os--ts */

  def toSetLattice[B >: A]: SetLattice[B] = CoFiniteSet(excluded.toSet, literally.toSet)
  def toSet[B >: A]: Set[B] =
    throw new IllegalStateException("CoSets are infinite so have no finite Set representation")
