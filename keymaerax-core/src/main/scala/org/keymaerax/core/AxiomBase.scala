...
//          Sequent(immutable.IndexedSeq(pany), immutable.IndexedSeq(Box(Loop(a), pany))))),
  ...
  private def assertCheckAxiomFile(axs: Map[String, Formula]): Boolean = {
    ...
    insist(
      axs("refinement assign") == ProgramEquivalence(Assign(x, f0), Compose(AssignAny(x), Test(Equal(x, f0)))),
      "refinement assign",
    )
    insist(axs("refinement stutter") == ProgramEquivalence(Assign(x, x), Test(True)), "refinement stutter")
    insist(
    ...
    insist(
      axs("refinement DW") ==
        ProgramEquivalence(ODESystem(ode, pany), Compose(Test(pany), Compose(ODESystem(ode, pany), Test(pany)))),
      "refinement DW",
    )
    insist(
    ...
    insist(
      axs("nondet commute fresh") == ProgramEquivalence(
