...
import org.keymaerax.tools.qe.KMComparator._
import org.keymaerax.tools.qe.MKComparator._
import org.keymaerax.tools.qe.MathematicaConversion.MExpr
...
object MathematicaConversion {
  ...
  type KExpr = org.keymaerax.core.Expression
  ...
  def apply(e: MExpr): T = convert(e) ensures
    (
      ...
      "Roundtrip conversion is identity." + "\nMathematica expression:   " + e.toString + "\t@[" +
        e.args.map(_.head()).mkString(", ") + "]" + "\nConverted to KeYmaera X:  " + convert(e) + "\t@" +
        convert(e).getClass.getSimpleName + "\nRoundtrip to Mathematica: " + k2m.convert(convert(e)).toString + "\t@[" +
        k2m.convert(convert(e)).args.map(_.head()).mkString(", ") + "]",
    )
  ...
  def apply(e: T): MExpr = convert(e) ensures
    (
      ...
      "Roundtrip conversion is identity." + "\nKeYmaera X expression    " + e + "\t@" + e.getClass.getSimpleName +
        "\nConverted to Mathematica " + convert(e).toString + "\nRoundtrip to KeYmaera X  " + m2k.convert(convert(e)) +
        "\t@" + m2k.convert(convert(e)).getClass.getSimpleName,
    )
...
/**
 * Compares Mathematica expressions for equality (handles conversion differences, since Mathematica silently converts
 * some expressions, e.g. division 2/5 into rational 2/5).
 */
class KMComparator(val l: MExpr) {
  ...
  def ===(r: MExpr): Boolean =
    ...
    (l.head() == r.head() && l.args().length == r.args().length &&
      l.args().zip(r.args()).forall({ case (la, ra) => la === ra })) ||
  ...
  private def inequalityEquals(l: MExpr, r: MExpr): Boolean = {
    def checkInequality(l: Array[MExpr], r: MExpr): Boolean = hasHead(r, l(1)) && r.args.length == 2 &&
      r.args().head === l(0) && r.args().last === l(2)
    @tailrec
    def checkInequalities(l: Array[MExpr], r: MExpr): Boolean = {
      ...
      // And[c[a,b], ...] == {a c b ... }
      else MathematicaOpSpec.and.applies(r) && checkInequality(l, r.args().head) &&
      checkInequalities(l.tail.tail, r.args().last)
    }
  ...
  private def rationalEquals(l: MExpr, r: MExpr): Boolean = {
    ...
    (r.realQ() && BigDecimal(l.args().head.asLong) / BigDecimal(l.args().last.asLong) == BigDecimal(r.asDouble()))
  }
  ...
  private def naryEquals(l: MExpr, r: MExpr, expectedHead: MExpr): Boolean = {
    ...
    def flattenBinaryL(e: MExpr): List[MExpr] = {
      if (e.head() == expectedHead) flattenBinaryL(e.args().head) :+ e.args()(1) else List(e)
    }
    // Op[a, Op[b,c]] === Op[a,b,c]
    def flattenBinaryR(e: MExpr): List[MExpr] = {
      if (e.head() == expectedHead) e.args().head +: flattenBinaryR(e.args()(1)) else List(e)
    }
    ...
    if (lf.size == r.args().length) lf.zip(r.args()).forall({ case (r, l) => r === l })
      ...
      if (rf.size == r.args().length) rf.zip(r.args()).forall({ case (r, l) => r === l }) else false
    }
  }
}
...
class MKComparator[T](val l: T) {
  def ===(r: T): Boolean = (l, r) match {
    ...
    case (lf: BinaryCompositeFormula, rf: BinaryCompositeFormula) if lf.getClass == rf.getClass =>
      lf.left === rf.left && lf.right === rf.right
    case (lf: ComparisonFormula, rf: ComparisonFormula) if lf.getClass == rf.getClass =>
      lf.left === rf.left && lf.right === rf.right
    case (lt: UnaryCompositeTerm, rt: UnaryCompositeTerm) if lt.getClass == rt.getClass => lt.child === rt.child
    case (lt: BinaryCompositeTerm, rt: BinaryCompositeTerm) if lt.getClass == rt.getClass =>
      lt.left === rt.left && lt.right === rt.right
