...
abstract class BaseMathematicaCommandRunner extends MathematicaCommandRunner {
  ...
  protected def timeConstrained(cmd: MExpr): MExpr =
    if (timeout < 0) cmd else MathematicaOpSpec.timeConstrained(cmd, MathematicaOpSpec.long(timeout))
...
case class JLinkMathematicaCommandRunner(ml: KernelLink) extends BaseMathematicaCommandRunner with Logging {
  ...
  /** @inheritdoc */
  override def doRun[T](cmd: MExpr, m2k: M2KConverter[T]): (String, T) =
    try {
      ensureKernel()
  ...
  @inline
  private def ensureKernel(): Unit = { if (ml == null) { throw ToolCommunicationException("No MathKernel set!") } }

  /** Applies an expression converter, throws [[ConversionException]] if something goes wrong */
  private def apply[T](converter: MExpr => T, expr: MExpr, ctx: Expr = null): T =
    try { converter(expr) }
    catch {
  ...
  private def await(ctx: Expr = null): Expr = {
    ...
    try { ml.waitForAnswer() }
    catch {
      case ex: MathLinkException => throw MathematicaMathlinkException(s"Error executing Mathematica command: $ctx", ex)
    }
  ...
  private def getAnswer[T](cmdIdx: Long, converter: MExpr => T, ctx: Expr): (String, T) = disposeAfter(
    await(ctx),
    res => {
      if (isTimedOut(res)) { throw MathematicaComputationTimedOutException(ctx.toString) }

      if (isAborted(res)) { throw MathematicaComputationAbortedException(ctx.toString) }
      ...
      if (isException(res)) {
        ...
        if (txtMsg.contains("nsmet")) {
          throw MathematicaInapplicableMethodException(
            s"Input $ctx is not solvable with the available methods, cause: $txtMsg"
          )
        }
      ...
      if (isFailed(res)) { throw ToolExecutionException(s"Command failed: $ctx") }
      ...
      if (MathematicaOpSpec.list.applies(res) && res.args.length == 2 && res.args.head.asInt() == cmdIdx) {

        val theResult = res.args.last
        if (isTimedOut(theResult)) { throw MathematicaComputationTimedOutException(ctx.toString) }
        ...
        throw MathematicaUnknownCauseCriticalException(
          s"Unexpected result: either result length != 2 or JLink returned a stale answer\ncommand: $ctx\nreturned result: $res"
        )
      }
    },
  )
