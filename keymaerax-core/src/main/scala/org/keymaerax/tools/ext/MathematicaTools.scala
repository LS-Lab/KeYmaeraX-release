...
import org.keymaerax.infrastruct.{ExpressionTraversal, FormulaTools, PosInExpr}
import org.keymaerax.tools._
import org.keymaerax.tools.ext.ExtMathematicaOpSpec._
import org.keymaerax.tools.ext.SimulationTool.{SimRun, SimState, Simulation}
import org.keymaerax.tools.install.PegasusInstaller
...
class UncheckedBaseM2KConverter extends MathematicaToKeYmaera {
  ...
  override def convert(e: MExpr): KExpr = {
    ...
    if (isTimedOut(e)) { throw MathematicaComputationTimedOutException(e.toString) }

    if (isFailed(e)) { throw MathematicaComputationFailedException(e.toString) }

    if (MathematicaOpSpec.rule.applies(e)) convertRule(e)
    else if (e.listQ() && e.args().forall(r => r.listQ() && r.args().forall(MathematicaOpSpec.rule.applies)))
      convertRuleList(e)
    // Derivatives are of the form Derivative[1][x]
    else if (
      e.head.args().length == 1 && e.head().args.head.integerQ() && e.head().args.head.asInt() == 1 &&
      e.head.head.symbolQ() && ExtMathematicaOpSpec.derivative.applies(e.head)
    ) convertDerivative(e)
    else if (MathematicaOpSpec.lfalse.applies(e)) False
    else if (MathematicaOpSpec.ltrue.applies(e)) True
    else if (
      e.symbolQ() &&
      MathematicaNameConversion.uncheckedUnmaskName(e.asString())._1.endsWith(UncheckedBaseConverter.CONST_FN_SUFFIX)
    ) MathematicaNameConversion.toKeYmaera(e) match {
      case BaseVariable(name, index, sort) => FuncOf(
          Function(name.substring(0, name.length - UncheckedBaseConverter.CONST_FN_SUFFIX.length), index, Unit, sort),
          Nothing,
        )
    }
  ...
  private def convertRule(e: MExpr): Formula = convert(e.args()(0)) match {
    ...
    case _: Program =>
      throw new IllegalArgumentException("There is no conversion from Mathematica to hybrid programs " + e)
  }
    ...
    override def convert(e: KExpr): MExpr = {
      ...
      insist(
        StaticSemantics
          .symbols(e)
          .forall({
            case fn @ Function(_, _, _, _, Some(_)) => interpretedSymbols.exists(_._2 == fn)
            case _ => true
          }),
      ...
      e match {
        ...
        case _: Program =>
          throw new IllegalArgumentException("There is no conversion from hybrid programs to Mathematica " + e)
        case _: Function => throw new IllegalArgumentException(
            "There is no conversion from unapplied function symbols to Mathematica " + e
          )
      }
    }
...
object CEXM2KConverter extends M2KConverter[Either[KExpr, NamedSymbol]] {
  private val baseConverter = new UncheckedBaseM2KConverter {
    ...
    override def convert(e: MExpr): KExpr = {
      if (ExtMathematicaOpSpec.complex.applies(e)) {
        ...
        FuncOf(
          ...
          Pair(convert(r).asInstanceOf[Term], convert(i).asInstanceOf[Term]),
        )
      } else super.convert(e)
    }
    ...
    override protected def convertAtomicTerm(e: MExpr): KExpr = {
      MathematicaNameConversion.toKeYmaera(e) match {
        ...
        case BaseVariable(name, None, _) if name == UncheckedBaseConverter.CONST_PRED_SUFFIX =>
          super.convertAtomicTerm(e)
        ...
        case BaseVariable(name, index, _) if name.endsWith(UncheckedBaseConverter.CONST_FN_SUFFIX + "_") =>
          FuncOf(
            Function(
              name.stripSuffix("_").stripSuffix(UncheckedBaseConverter.CONST_FN_SUFFIX) + "_",
              index,
              Unit,
              Real,
            ),
        ...
        case BaseVariable(name, index, _) if name.endsWith(UncheckedBaseConverter.CONST_PRED_SUFFIX + "_") =>
          PredOf(
            Function(
              name.stripSuffix("_").stripSuffix(UncheckedBaseConverter.CONST_PRED_SUFFIX) + "_",
              index,
              Unit,
              Bool,
            ),
  ...
  override private[tools] def convert(e: MExpr): Either[KExpr, NamedSymbol] =
    try { Left(baseConverter.convert(e)) }
    catch { case _: ConversionException => Left(Equal(Variable("unknown"), Variable("unknown"))) }
...
object PlotConverter extends UncheckedBaseK2MConverter {
  override def apply(e: KExpr): MExpr = e match {
    case Imply(pre, Box(ODESystem(ode, domain), post)) =>
      ...
      val postPlotStyle =
        rule(symbol("PlotStyle"), list(symbol("Red"), UnaryMathOpSpec(symbol("Opacity"))(double(0.5))))
      val prePlotStyle =
        rule(symbol("PlotStyle"), list(symbol("Green"), UnaryMathOpSpec(symbol("Opacity"))(double(0.5))))
      val domainPlotStyle =
  ...
  override def convert(e: KExpr): MExpr = {
    ...
    e match {
      ...
      case _: Program =>
        throw new IllegalArgumentException("There is no conversion from hybrid programs to Mathematica " + e)
      case _: Function =>
        throw new IllegalArgumentException("There is no conversion from unapplied function symbols to Mathematica " + e)
    }
  }
...
object PegasusM2KConverter extends UncheckedBaseM2KConverter with Logging {
  ...
  private def convertDiffSatResult(e: MExpr): Expression = {
    ...
    def resultType: BinaryMathOpSpec = new BinaryMathOpSpec(symbol("Rule")) {
      override def applies(e: MExpr): Boolean = super.applies(e) && e.args()(0) == symbol("ResultType") &&
        e.args()(1) == symbol("DiffSat")
    }
    ...
    def cuts: BinaryMathOpSpec = new BinaryMathOpSpec(symbol("Rule")) {
      override def applies(e: MExpr): Boolean = super.applies(e) && e.args()(0) == symbol("Cuts") &&
        list.applies(e.args()(1))
    }
    ...
    def result: NaryMathOpSpec = new NaryMathOpSpec(symbol("Rule")) {
      override def applies(e: MExpr): Boolean = super.applies(e) && e.args()(0) == symbol("Result") &&
        e.args()(1).listQ && e.args()(1).args.length == 3 && invariant.applies(e.args()(1).args()(0)) &&
        cuts.applies(e.args()(1).args()(1)) && proved.applies(e.args()(1).args()(2))
    }
    ...
    /**
     * Converts a result of the shape {{{Result -> { Invariant -> <...>, Cuts -> <...>, Proved -> <...> }}}} into
    ...
    def convertResult(e: MExpr): Formula = {
      ...
      /**
       * Converts {{{Cuts -> { { <formula>, Hint-><variable> }, ... }}}} into
      ...
      require(
        result.applies(e),
        "Expected a result list of 3 rules: { Invariant -> <...>, Cuts -> <...>, Proved -> <...>",
      )
      Equiv(
        pred("Result"),
        And(
          Equal(LIST_LENGTH, Number(3)),
          And(
            convertInvariant(e.args()(1).args()(0)),
  ...
  private def convertErrorResult(e: MExpr): Expression = {
    ...
    def convertResult(e: MExpr): Expression = {
      ...
      def result: NaryMathOpSpec = new NaryMathOpSpec(symbol("Rule")) {
        override def applies(e: MExpr): Boolean = super.applies(e) && e.args()(0) == symbol("Result") &&
          e.args()(1).listQ && e.args()(1).args.length == 2 && errorString.applies(e.args()(1).args()(0)) &&
          internalError.applies(e.args()(1).args()(1))
      }
  ...
  def extractResult(result: Expression): (immutable.Seq[(Formula, String)], Formula) = {
    def extractCuts(cuts: Formula): immutable.Seq[(Formula, String)] = cuts match {
      case And(Equal(LIST_LENGTH, Number(numCuts)), cutFmls) => FormulaTools
          .leftConjuncts(cutFmls, numCuts.toIntExact)
          .map({
    ...
    result match {
      case And(Equal(BaseVariable("ResultType", None, Real), BaseVariable("DiffSat", None, Real)), result) =>
        result match {
          case Equiv(
                PredOf(Function("Result", None, _, _, _), Nothing),
                And(Equal(LIST_LENGTH, Number(n)), resultElements),
              ) if n.toIntExact == 3 =>
            resultElements match {
              case And(
                    Equiv(PredOf(Function("Invariant", None, _, _, _), Nothing), invariant),
                    And(
                      Equiv(PredOf(Function("Cuts", None, _, _, _), Nothing), cuts),
              ...
              case _ => throw ConversionException(
                  "Expected (Invariant <-> ...) & (Cuts <-> ...) & (Proved <-> ...), but got " +
                    resultElements.prettyString
                )
            }
          case _ =>
            throw ConversionException("Expected Result <-> length(list)=3 & ..., but got " + result.prettyString)
        }
      case And(Equal(BaseVariable("ResultType", None, Real), BaseVariable("Trivial", None, Real)), result) =>
        result match {
  ...
  def decodeTermList(listTrm: Expression): List[Term] = {
    listTrm match {
      case Pair(l, r) => l :: decodeTermList(r)
      case Nothing => List()
      case _ => throw ConversionException(
          "Expected a term list to decode but got a ill-formed expression: " + listTrm.prettyString
        )
    }
  }
...
class MathematicaODESolverTool(override val link: MathematicaLink)
  ...
  private def defunctionalize(f: Formula, arg: Term, fnNames: String*) = ExpressionTraversal.traverse(
    new ExpressionTraversalFunction {
      override def postT(p: PosInExpr, e: Term): Either[Option[StopTraversal], Term] = e match {
        case FuncOf(Function(name, idx, _, range, None), fnArg)
            if arg == fnArg && (fnNames.isEmpty || fnNames.contains(name)) => Right(Variable(name, idx, range))
        case _ => Left(None)
      }
    },
    f,
...
class MathematicaPDESolverTool(override val link: MathematicaLink)
  ...
  def pdeSolve(diffSys: DifferentialProgram): Iterator[Term] = {
    ...
    val characteristics: List[MExpr] = DifferentialHelper
      .atomicOdes(diffSys)
    ...
    result match {
      ...
      case r: Or =>
        FormulaTools.disjuncts(r).flatMap(disj => FormulaTools.conjuncts(disj).map { case Equal(_, b) => b }).iterator
      case r => throw ToolExecutionException("Mathematica did not solve the PDE for : " + diffSys + "\n" + r)
    }
  }
}

class MathematicaLyapunovSolverTool(override val link: MathematicaLink)
    extends BaseKeYmaeraMathematicaBridge[KExpr](link, PegasusK2MConverter, PegasusM2KConverter)
    with LyapunovSolverTool {
...
private class DiffUncheckedM2KConverter extends UncheckedBaseM2KConverter {
  ...
  private def toKeYmaera(e: MExpr): NamedSymbol = {
    ...
    } else { MathematicaNameConversion.toKeYmaera(e) }
  }
...
class MathematicaSimplificationTool(override val link: MathematicaLink)
  ...
  override def simplify(expr: Expression, assumptions: List[Formula]): Expression = expr match {
    ...
    case _ =>
      ...
      result match {
        case r: Expression => r
        case _ => throw ToolExecutionException(
            "Mathematica did not successfuly simplify: " + expr + " under assumptions " + assumptions
          )
      }
  }
}
...
class MathematicaSimulationTool(override val link: MathematicaLink)
  ...
  def simulate(initial: Formula, stateRelation: Formula, steps: Int = 10, n: Int = 1): Simulation = {
    ...
    val init = ExtMathematicaOpSpec.setDelayed(
      MathematicaOpSpec.symbol("kyx`init"),
      ExtMathematicaOpSpec.function(ExtMathematicaOpSpec.map(
    ...
    val (stepPreVars, stepPostVars) = StaticSemantics
      .symbols(stateRelation)
      .filter(_.isInstanceOf[Variable])
      .partition(_.name.startsWith("pre"))
    val pre2post = stepPostVars
      .filter(_.name != "t_")
      .map(v => Variable("pre" + v.name, v.index, v.sort) -> v)
      .toMap[NamedSymbol, NamedSymbol]

    val stepModuleInit = stepPreVars
      .toList
      .sorted
      .map(s =>
        ExtMathematicaOpSpec.set(
          ...
          ExtMathematicaOpSpec.replaceAll(
            basek2m(
              pre2post.getOrElse(s, throw new IllegalArgumentException("No post variable for " + s.prettyString))
            ),
            ExtMathematicaOpSpec.placeholder,
          ),
        )
    ...
    val exec = ExtMathematicaOpSpec.map(
      ExtMathematicaOpSpec.function(ExtMathematicaOpSpec.n(ExtMathematicaOpSpec.nestList(
        MathematicaOpSpec.symbol("kyx`step"),
        ExtMathematicaOpSpec.placeholder,
        MathematicaOpSpec.int(steps),
      ))),
      MathematicaOpSpec.mapply(MathematicaOpSpec.symbol("kyx`init"), MathematicaOpSpec.list(MathematicaOpSpec.int(n))),
    )
