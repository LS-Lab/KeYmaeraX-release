...
import org.keymaerax.tools.ext.ExtMathematicaOpSpec.{mwhile, part}
import org.keymaerax.tools.ext.SOSsolveTool.Result
...
class Mathematica(private[tools] val link: MathematicaLink, override val name: String)
  ...
  override def qe(formula: Formula): Lemma = {
    ...
    try { mQE.run(ProvableSig.proveArithmeticLemma(_, formula)) }
      ...
      case ex @ (_: MathematicaComputationAbortedException | _: MathematicaComputationTimedOutException)
        ...
        if (qeMaxTimeout == 0) throw ex else doMaxQE(formula)
      }
      case ex @ (_: MathematicaComputationAbortedException | _: MathematicaComputationTimedOutException)
          if qeCexTimeout != 0 => {
        mCEX.timeout = qeCexTimeout
        val cex =
          try { mCEX.findCounterExample(stripUniversalClosure(formula)) }
          catch {
        ...
        cex match {
          ...
          case Some(cex) =>
            ...
            throw BelleCEX(
              ...
              cex,
              Sequent(IndexedSeq(), IndexedSeq(formula)),
            )
        }
      }
    }
      ...
      case OneOf(oneOfGoals) => module(
          ...
          if (continueOnFalse) {
            compoundExpr(
              mwhile(
                and(
                  equal(part(symbol("res"), int(2)), bool(false)),
                  greater(ExtMathematicaOpSpec.length(symbol("res")), int(0)),
                ),
            ...
            override def convert(e: MExpr): KExpr = {
              if (e.listQ()) {
                val (i, fml) = e.args.toList match {
                  ...
                  case e =>
                    throw ConversionException("Expected QE result list of length 2 {i,fml}, but got " + e.mkString(","))
                }
  ...
  private def splitFormula(fml: Formula): Goal = fml match {
    case Forall(x, p) =>
      val universalClosure = (f: Formula) =>
        x.intersect(StaticSemantics.freeVars(f).toSet.toList) match {
          case Nil => f
          case x => Forall(x, f)
        }
    ...
    case _: Imply =>
      ...
      if (propSubgoals.size > 1) {
        if (expandedSubgoals.size > 1) OneOf(List(
          Atom(fml),
          AllOf(propSubgoals.map(p => Atom(p.toFormula))),
          AllOf(expandedSubgoals.map(p => Atom(p.toFormula))),
        ))
  ...
  override def odeSolve(
      ...
      diffArg: Variable,
      iv: Predef.Map[Variable, Variable],
  ): Option[Formula] = mODE.odeSolve(diffSys, diffArg, iv)
  ...
  override def invgen(
      ...
      postCond: Formula,
  ): Seq[Either[Seq[(Formula, String)], Seq[(Formula, String)]]] = mPegasus.invgen(ode, assumptions, postCond)
  override def lzzCheck(ode: ODESystem, inv: Formula): Boolean = mPegasus.lzzCheck(ode, inv)
  override def refuteODE(
      ode: ODESystem,
      assumptions: Seq[Formula],
      postCond: Formula,
  ): Option[Map[NamedSymbol, Expression]] = mPegasus.refuteODE(ode, assumptions, postCond)
  override def genODECond(
      ode: ODESystem,
      assumptions: Seq[Formula],
      postCond: Formula,
  ): (List[Formula], List[Formula]) = mPegasus.genODECond(ode, assumptions, postCond)
  override def sosSolve(polynomials: List[Term], variables: List[Term], degree: Int, timeout: Option[Int]): Result =
    mSOSsolve.sosSolve(polynomials, variables, degree, timeout)
