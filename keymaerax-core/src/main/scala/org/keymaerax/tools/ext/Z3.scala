...
import org.keymaerax.tools.qe.{DefaultSMTConverter, Z3QETool, Z3Solver}
import org.keymaerax.tools.{ConversionException, Tool, ToolExecutionException, ToolOperationManagement}
...
final class Z3 extends Tool with QETacticTool with SimplificationTool with ToolOperationManagement {
  ...
  override def qe(g: Goal, continueOnFalse: Boolean): (Goal, Formula) = g match {
    case Atom(fml) =>
      val Sequent(IndexedSeq(), IndexedSeq(Equiv(_, result))) = qe(fml).fact.conclusion: @unchecked
      g -> result
