...
import org.keymaerax.parser.KeYmaeraXLexer
...
case class ToolEvidence(info: immutable.List[(String, String)]) extends Evidence {
  ...
  /**
   * Equality modulo CRLF == LF in the evidence values (Lexer normalizes, so parsed evidence disagrees with in-memory
   * evidence)
   */
      ...
      info
        ...
        .forall {
          case ((k1, v1), (k2, v2)) => k1 == k2 &&
            ...
            ||
              KeYmaeraXLexer
                .normalizeNewlines(v1.stripSuffix(" ").stripPrefix(" ")) == KeYmaeraXLexer.normalizeNewlines(v2) ||
              KeYmaeraXLexer
                .normalizeNewlines(v1) == KeYmaeraXLexer.normalizeNewlines(v2.stripSuffix(" ").stripPrefix(" ")))
          case _ => false
        }
    case _ => false
  }
