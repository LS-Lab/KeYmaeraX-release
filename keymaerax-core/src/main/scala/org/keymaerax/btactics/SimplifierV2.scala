...
object SimplifierV2 {
  ...
  def mksubst(s: Subst): Subst = {
    ...
    if (!s.renaming.isEmpty) { throw new ProverException("Substitution contains variable renaming: " + s) }
    return s
  }
  ...
  def flattenPlus(t: Term): (Term, BigDecimal) = {
    t match {
      case Plus(l, r) =>
        ...
        (Plus(lt, rt), (lc + rc))
      case n: Number => (Number(0), n.value)
      case _ => (t, 0)
    }
  }
  ...
  def flattenTimes(t: Term): (Term, BigDecimal) = {
    t match {
      case Times(l, r) =>
        ...
        (Times(lt, rt), (lc * rc))
      case n: Number => (Number(1), n.value)
      case _ => (t, 1)
    }
  }
  ...
  def reassoc(t: Term): ProvableSig = {
    ...
    t match {
      case Plus(_, _) =>
        ...
        if (c == 0) init else proveBy(Equal(t, Plus(tt, Number(c))), QE & done)
      case Times(_, _) =>
        val (tt, c) = flattenTimes(t)
        if (c == 1) init else proveBy(Equal(t, Times(tt, Number(c))), QE & done)
      case _ => init
    }
  }
  ...
  def groundTermEval(t: Term): Option[(Term, ProvableSig)] = {
    ...
    res match {
      ...
      case Some(v) =>
        ...
        if (pr.isProved) Some(Number(v), pr) else None
    }
  }
  ...
  def termSimp(t: Term): (Term, ProvableSig) = {
    ...
    val recpf = t match {
      case bop: BinaryCompositeTerm =>
        ...
        proveBy(
          Equal(t, nt),
          CEat(lpr)(SuccPosition(1, 1 :: 0 :: Nil)) & CEat(rpr)(SuccPosition(1, 1 :: 1 :: Nil)) &
      ...
      case uop: UnaryCompositeTerm =>
        ...
        proveBy(Equal(t, nt), CEat(upr)(SuccPosition(1, 1 :: 0 :: Nil)) & byUS(Ax.equalReflexive))
      case FuncOf(fn, c) if c != Nothing =>
        ...
        val tactic = simp
          .zipWithIndex
          .map({ case ((_, eqPr), i) => useAt(eqPr)(1, pref ++ PosInExpr.parseInt(i).pos) })
    ...
    val _ = groundTermEval(rt) match {
      case Some((t, eq)) => return (t, useFor(eq, PosInExpr(0 :: Nil))(SuccPosition(1, 1 :: Nil))(recpf))
      case _ => ()
    }

    // Arithmetic propositions (non-ground rewrites like 0+x = x, x+0 = x , etc.)
    val apf = qeHeuristics(recpf) match {
      case None => recpf
      case Some(pr) => pr
    }
  ...
  private lazy val plusLemma =
    remember("(A_() = B_()) & (X_() = Y_()) -> (A_()+X_() = B_()+Y_())".asFormula, QE & done, namespace).fact
  private lazy val minusLemma =
    remember("(A_() = B_()) & (X_() = Y_()) -> (A_()-X_() = B_()-Y_())".asFormula, QE & done, namespace).fact
  private lazy val timesLemma =
    remember("(A_() = B_()) & (X_() = Y_()) -> (A_()*X_() = B_()*Y_())".asFormula, QE & done, namespace).fact
  private lazy val divideLemma = remember(
    "(A_() = B_()) & (X_() = Y_()) -> (A_()/X_() = B_()/Y_())".asFormula,
    implyR(1) & andL(-1) & exhaustiveEqL2R(-1) & exhaustiveEqL2R(-2) & cohideR(1) & byUS(Ax.equalReflexive),
    namespace,
  ).fact
  private lazy val powerLemma = remember(
    "(A_() = B_()) & (X_() = Y_()) -> (A_()^X_() = B_()^Y_())".asFormula,
    implyR(1) & andL(-1) & exhaustiveEqL2R(-1) & exhaustiveEqL2R(-2) & cohideR(1) & byUS(Ax.equalReflexive),
    namespace,
  ).fact
  ...
  /**
   * Takes a term `t`, with an equality context `ctx` and returns `ctx |- t = t'` using all equalities of the shape `t''
   * \= n:Number`. This is probably hopelessly slow...
   */
  def equalityRewrites(t: Term, ctx: IndexedSeq[Formula]): ProvableSig = {

    val rest = ctx.foldLeft(t)((t: Term, f: Formula) =>
      f match {
        case Equal(l, n: Number) => t.replaceFree(l, n)
        case _ => t
      }
    )
    proveBy(
      Sequent(ctx, IndexedSeq(Equal(t, rest))),
      ctx
        .zipWithIndex
        .foldRight[BelleExpr](ident)((f: (Formula, Int), tac: BelleExpr) =>
          f match {
            case (Equal(l, n: Number), i) => eqL2R(-(i + 1))(1) & tac
            case _ => tac
          }
  ...
  def termSimpWithRewrite(t: Term, ctx: IndexedSeq[Formula]): (Term, ProvableSig) = {
    ...
    (
      ttf,
      proveBy(
        Sequent(ctx, IndexedSeq(Equal(t, ttf))),
        CEat(tpr)(SuccPosition(1, 1 :: Nil)) & cut(Equal(t, tt)) < (close, hideR(SuccPos(0)) & by(teq)),
      ),
    )
  }
  ...
  private lazy val andLemma = remember(
    "((P_() <-> F_()) & (F_() -> (Q_() <-> G_()))) ->(P_() & Q_() <-> F_() & G_())".asFormula,
    prop & done,
    namespace,
  ).fact

  private lazy val implyLemma = remember(
    "((P_() <-> F_()) & (F_() -> (Q_() <-> G_()))) ->(P_() -> Q_() <-> F_() -> G_())".asFormula,
    prop & done,
    namespace,
  ).fact

  private lazy val orLemma = remember(
    "((P_() <-> F_()) & (!(F_()) -> (Q_() <-> G_()))) ->(P_() | Q_() <-> F_() | G_())".asFormula,
    prop & done,
    namespace,
  ).fact

  private lazy val equivLemma = remember(
    "((P_() <-> F_()) & (Q_() <-> G_())) ->((P_() <-> Q_()) <-> (F_() <-> G_()))".asFormula,
    prop & done,
    namespace,
  ).fact

  private lazy val notLemma = remember("(P_() <-> F_()) ->(!P_() <-> !F_())".asFormula, prop & done, namespace).fact
  ...
  def addContext(f: Formula, ctx: IndexedSeq[Formula]): (IndexedSeq[Formula], BelleExpr) = {
    ...
    if (ctx.contains(f) | f.equals(True) | f.equals(Not(False))) { return (ctx, hideL(Symbol("Llast"))) }
    f match {
      ...
      case And(l, r) =>
        ...
        (
          ctxR,
          andL(Symbol("Llast")) & implyRi()(AntePos(ctx.length + 1), SuccPos(0)) & tacL & implyR(SuccPos(0)) & tacR,
        )
        ...
        if (nu == f) { (ctx :+ f, ident) }
          ...
          (
            ...
            useAt(Ax.andReflexive, PosInExpr(1 :: Nil))(AntePos(ctx.length)) & andL(Symbol("Llast")) &
              implyRi()(AntePos(ctx.length), SuccPos(0)) & useAt(cpr, PosInExpr(0 :: Nil))(SuccPosition(1, 0 :: Nil)) &
              implyR(Symbol("_")) & tacU,
          )
        }
      case Equal(n: Number, r) =>
        ...
        (
          ctx :+ Equal(r, n),
          implyRi()(AntePos(ctx.length), SuccPos(0)) & useAt(eqSym, PosInExpr(0 :: Nil))(SuccPosition(1, 0 :: Nil)) &
            implyR(Symbol("_")),
        )
      case _ => (ctx :+ f, ident)
    }
  ...
  def search(ctx: IndexedSeq[Formula], f: Formula, g: Formula, h: Formula, lem: ProvableSig): Option[ProvableSig] = {
    ...
    if (ind >= 0) {
      return Some(
        proveBy(Sequent(ctx, IndexedSeq(Equiv(g, h))), cohide2(AntePos(ind), SuccPos(0)) & implyRi & byUS(lem))
      )
    } else None
  }
  ...
  def closeHeuristics(ctx: IndexedSeq[Formula], f: Formula, flip: Boolean = true): Option[ProvableSig] = {
    ...
    val init: Option[ProvableSig] = f match {
      case Less(l, r) => search(ctx, GreaterEqual(l, r), f, False, ltGeFalse) orElse
          search(ctx, Greater(l, r), f, False, ltGtFalse) orElse search(ctx, Equal(l, r), f, False, ltEqFalse) orElse
          (if (flip) {
             closeHeuristics(ctx, Greater(r, l), false) match {
               case None => None
               case Some(pr) => { Some(useFor(Ax.flipGreater)(SuccPosition(1, 0 :: Nil))(pr)) }
             }
           } else None)
      case LessEqual(l, r) => search(ctx, Less(l, r), f, True, leLtTrue) orElse
          search(ctx, Equal(l, r), f, True, leEqTrue) orElse search(ctx, Greater(l, r), f, False, leGtFalse) orElse
          (if (flip) {
             closeHeuristics(ctx, GreaterEqual(r, l), false) match {
               case None => None
               case Some(pr) => { Some(useFor(Ax.flipGreaterEqual)(SuccPosition(1, 0 :: Nil))(pr)) }
             }
           } else None)
      case Greater(l, r) => search(ctx, LessEqual(l, r), f, False, gtLeFalse) orElse
          search(ctx, Less(l, r), f, False, gtLtFalse) orElse search(ctx, Equal(l, r), f, False, gtEqFalse) orElse
          (if (flip) {
             closeHeuristics(ctx, Less(r, l), false) match {
               case None => None
               case Some(pr) => { Some(useFor(Ax.flipLess)(SuccPosition(1, 0 :: Nil))(pr)) }
             }
           } else None)
      case GreaterEqual(l, r) => search(ctx, Greater(l, r), f, True, geGtTrue) orElse
          search(ctx, Equal(l, r), f, True, geEqTrue) orElse search(ctx, Less(l, r), f, False, geLtFalse) orElse
          (if (flip) {
             closeHeuristics(ctx, LessEqual(r, l), false) match {
               case None => None
               case Some(pr) => { Some(useFor(Ax.flipLessEqual)(SuccPosition(1, 0 :: Nil))(pr)) }
             }
           } else None)
      case NotEqual(l, r) => search(ctx, Less(l, r), f, True, neLtTrue) orElse
          search(ctx, Greater(l, r), f, True, neGtTrue) orElse search(ctx, Equal(l, r), f, False, neEqFalse) orElse
          (if (flip) {
             closeHeuristics(ctx, NotEqual(r, l), false) match {
               case None => None
               case Some(pr) => { Some(useFor(neqSym, PosInExpr(0 :: Nil))(SuccPosition(1, 0 :: Nil))(pr)) }
             }
           } else None)
      case Equal(l, r) => search(ctx, Less(l, r), f, False, eqLtFalse) orElse
          ...
          (if (flip) {
             closeHeuristics(ctx, Equal(r, l), false) match {
               case None => None
               case Some(pr) => { Some(useFor(eqSym, PosInExpr(0 :: Nil))(SuccPosition(1, 0 :: Nil))(pr)) }
             }
           } else None)
      case _ => None
    }
    ...
    init match {
      ...
      case None =>
        ...
        search(ctx, f, f, True, trueReflex) match {
          case Some(v) => Some(v)
          case None => search(ctx, Not(f), f, False, falseReflex)
        }
    }
    ...
    val (recf: Formula, recpr: ProvableSig) = f match {
      case And(l, r) =>
        ...
        (
          nf,
          proveBy(
            Sequent(ctx, IndexedSeq(Equiv(f, nf))),
            cut(Equiv(l, lf)) <
              (
                cut(Imply(lf, Equiv(r, rf))) <
                  (
                    useAt(andLemma, PosInExpr(1 :: Nil))(SuccPos(0)) & andR(1) < (id, id),
          ...
          proveBy(
            ...
            cut(Equiv(l, lf)) <
              (
                cut(Imply(lf, Equiv(r, rf))) <
                  (
                    useAt(implyLemma, PosInExpr(1 :: Nil))(SuccPos(0)) & andR(1) < (id, id),
                    hideL(Symbol("Llast")) & hideR(SuccPos(0)) & implyR(1) & tac & by(rpr),
                  ),
                hideR(SuccPos(0)) & by(lpr),
              ),
            ...
            cut(Equiv(l, lf)) <
              (
                cut(Imply(Not(lf), Equiv(r, rf))) <
                  (
                    useAt(orLemma, PosInExpr(1 :: Nil))(SuccPos(0)) & andR(1) < (id, id),
                    hideL(Symbol("Llast")) & hideR(SuccPos(0)) & implyR(1) & tac & by(rpr),
                  ),
                hideR(SuccPos(0)) & by(lpr),
              ),
          ...
          proveBy(
            ...
            cut(Equiv(l, lf)) <
              (
                cut(Equiv(r, rf)) <
                  (
                    useAt(equivLemma, PosInExpr(1 :: Nil))(SuccPos(0)) & andR(1) < (id, id),
      ...
      case Not(u) =>
        ...
        (
          ...
          proveBy(
            Sequent(ctx, IndexedSeq(Equiv(f, nf))),
            cut(Equiv(u, uf)) < (useAt(notLemma, PosInExpr(1 :: Nil))(SuccPos(0)) & id, hideR(SuccPos(0)) & by(upr)),
          ),
        )
      case cf: ComparisonFormula =>
        ...
        (
          nf,
          proveBy(
            Sequent(ctx, IndexedSeq(Equiv(cf, nf))),
            cut(Equal(l, lt)) <
              (
                cut(Equal(r, rt)) <
                  (
                    useAt(lem, PosInExpr(1 :: Nil))(SuccPos(0)) & andR(1) < (id, id),
                    hideL(Symbol("Llast")) & hideR(SuccPos(0)) & by(rpr),
                  ),
                hideR(SuccPos(0)) & by(lpr),
              ),
      ...
      case q: Quantified =>
        ...
        (
          nf,
          proveBy(
            Sequent(ctx, IndexedSeq(Equiv(q, nf))),
            droppedCtx.map(f => hideL(Symbol("L"), f)).reduceOption[BelleExpr](_ & _).getOrElse(skip) & equivR(1) &
              onAll(instantiate & implyRi()(AntePos(remainingCtx.length), SuccPos(0)) & equivifyR(1)) <
              (skip, commuteEquivR(1)) & onAll(by(upr)),
          ),
        )
      case m: Modal =>
    ...
    val (_, proppr: ProvableSig) = propHeuristics(recf) match {
      ...
      case Some((ff, pr)) =>
        ...
        (
          ff,
          proveBy(
            Sequent(ctx, IndexedSeq(Equiv(f, ff))),
            cut(Equiv(f, recf)) <
              (
                cut(Equiv(recf, ff)) <
                  (
                    implyRi()(AntePos(ctx.length + 1), SuccPos(0)) & implyRi()(AntePos(ctx.length), SuccPos(0)) &
                      cohideR(SuccPos(0)) & byUS(equivTrans),
    ...
    closepr match {
      ...
      case Some(pr) =>
        ...
        (
          closef,
          proveBy(
            Sequent(ctx, IndexedSeq(Equiv(f, closef))),
            cut(Equiv(f, normf)) <
              (
  ...
  def rsimpTac(ipos: IndexedSeq[Integer]): DependentPositionTactic = new DependentPositionTactic("restricted simp") {
    override def factory(pos: Position): DependentTactic = new SingleGoalDependentTactic(name) {
      override def computeExpr(sequent: Sequent): BelleExpr = {
        ...
        ) // We can only do this at the top level because CE can't take antecedent contexts

        val positions =
          ipos.distinct.filter(i => i < sequent.ante.length) // Tactic doesn't work if the positions aren't distinct
        ...
        cutAt(ff)(pos) <
          (
            ident,
            cohideOnlyR(pos) & equivifyR(1) & commuteEquivR(1) & positions.foldLeft[BelleExpr](ident)(
              (tac: BelleExpr, i: Integer) => implyRi(keep = true)(AntePos(i), SuccPos(0)) & tac
            ) & cohideR(1) & implyR(1) * positions.length & by(pr),
          )
      }
    }
  }
        ...
        cutAt(ff)(pos) <
          (
            ...
            cohideOnlyR(cutPos) &
              cut(ctxAnd) <
              (
                implyRi()(AntePos(sequent.ante.size), SuccPos(0)) & useAt(swapImply)(1) & cohideR(1) & equivifyR(1) &
                  commute & by(pr),
                hideR(1) & (andR(1) < (close, doIf(!_.isProved)(?(close)))) * (sequent.ante.size - 1) &
                  doIf(!_.isProved)(?(close)),
              ),
          )
      }
    }
  ...
  def isNop(p: Program): Boolean = {
    p match {
      case Assign(x, y) => {
        y match {
          case v: Variable => v.name.equals(x.name)
          case _ => false
        }
      }
      case _ => false
  ...
  def stripNoOp(p: Program): Program = {
    p match {
      case Compose(p, pp) =>
        ...
        if (isNop(sp)) spp else if (isNop(spp)) sp else Compose(sp, spp)
      case Choice(l, r) => Choice(stripNoOp(l), stripNoOp(r))
      case _ => p
    }
  }
    ...
    f match {
      case (Imply(pre, rhs)) =>
        ...
        val seq = proveBy(
          ...
          implyR(1) & modusPonens(AntePos(1), AntePos(0)) & hideL(-2) & ?(by(pr)),
        )
        return (tar, seq)
        ...
        val seq = proveBy(
          ...
          loop(f)(1) <
            (
              ...
              useAt(Ax.iterateb)(-1) & andL(-1) & chase(3, 3, (e: Expression) => hideBox(e))(SuccPosition(1, Nil)) &
                chase(3, 3, (e: Expression) => hideBox(e))(AntePosition(2, Nil)) & (?(close)),
            ),
        )
        (tar, seq)
