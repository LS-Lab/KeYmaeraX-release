...
object SwitchedSystems {
  ...
  private def stripTimer(odes: List[ODESystem], topt: Option[Variable]): List[ODESystem] = {
    topt match {
      case None => odes
      case Some(t) => odes.map(ode => ODESystem(stripTimer(ode.ode, t), ode.constraint))
    }
  }

  private val defprefix = "_DEFAULT_"
  private def defaultName(n: Number): String = { defprefix + n.toString }

  private def mkMode(name: String): Term = {
    if (name.startsWith(defprefix)) { Number(name.drop(defprefix.length).toInt) }
  ...
  def stateDependentFromProgram(p: Program, topt: Option[Variable]): StateDependent = {
    ...
    p match {
      ...
      case l: Loop => stateDependentFromLoop(l)
      case _ => throw new IllegalArgumentException("Unable to parse program as state-dependent switched system " + p)
    }
  }
  ...
  case class Controlled(
    ...
    transitions.map(_.map(f => require(names.contains(f._1), "Unable to jump to undefined mode: " + f._1)))
  ...
  def controlledFromProgram(p: Program, topt: Option[Variable]): Controlled = {
    p match {
      ...
      case _ => throw new IllegalArgumentException("Unable to parse program for controlled switching " + p)
    }
  }

  case class Guarded(modes: List[(String, ODESystem, List[(String, Formula)])], u: Variable) extends SwitchedSystem {

    private val underlyingControlled =
      Controlled(None, modes.map(t => (t._1, t._2, t._3.map(s => (s._1, Test(s._2))))), u)
            ...
            .map(s =>
              (
                ...
                s._2 match {
                  case Test(g) => g
                  case _ =>
                    throw new IllegalArgumentException("Unable to parse program for guarded switching at: " + s._2)
                },
              )
            ),
        )
  ...
  case class Timed(
      modes: List[(String, DifferentialProgram, Option[Term], List[(String, Option[Term])])],
      u: Variable,
      timer: Variable,
  ) extends SwitchedSystem {
    ...
    private val underlyingControlled = Controlled(
      ...
      modes.map(t =>
        (
          t._1,
          ODESystem(
            DifferentialProduct(AtomicODE(DifferentialSymbol(timer), Number(1)), t._2),
            t._3 match {
              case None => True
              case Some(tt) => LessEqual(timer, tt)
            },
          ),
          t._4
            .map(s =>
              (
                ...
                Compose(
                  Test(s._2 match {
                    case None => True
                    case Some(ss) => GreaterEqual(timer, ss)
                  }),
                  Assign(timer, Number(0)),
                ),
              )
            ),
        )
    ...
    val modes = c
      ...
      .map(t =>
        (
          ...
          t._2.constraint match {
            case True => None
            case LessEqual(t: Variable, bound) if t == timer => Some(bound)
          },
          t._3
            .map(s =>
              (
                ...
                s._2 match {
                  ...
                  case Compose(Test(GreaterEqual(t1: Variable, bound)), Assign(t2, n: Number))
                      if t1 == t2 && t2 == timer && n.value == 0 => Some(bound)
                  case _ =>
                    throw new IllegalArgumentException("Unable to parse program for timed switching at: " + s._2)
                },
              )
            ),
        )
  ...
  private def switchedFromProgram(p: Program, topt: Option[Variable]): SwitchedSystem = {
    try { stateDependentFromProgram(p, topt) }
    catch {
      case e: IllegalArgumentException =>
        try { guardedFromProgram(p, topt) }
        catch {
          case e: IllegalArgumentException =>
            try { timedFromProgram(p, topt) }
            catch {
              case e: IllegalArgumentException =>
                try { controlledFromProgram(p, topt) }
                catch {
                  case e: IllegalArgumentException => throw new IllegalArgumentException(
                      "Unable to parse program as switched system: " + p + " Last error: " + e.getMessage
                    )
                }
            }
        }
  ...
  def stabilitySpec(ss: SwitchedSystem): Formula = {
    ...
    val fmlbody = Imply(init, Box(ss.asProgram, Less(normsq, Power(eps, Number(2)))))
    ...
    Forall(
      eps :: Nil,
      Imply(
        Greater(eps, Number(0)), // \forall eps > 0
        Exists(
          del :: Nil,
          And(
            Greater(del, Number(0)), // \exists del > 0
            qbody,
          ),
        ),
      ),
  ...
  def attractivitySpec(ss: SwitchedSystem): Formula = {
    ...
    val fmlbody = Imply(
      init,
      Box(
        Compose(Assign(tVar, Number(0)), ss.asClockedProgram(tVar)),
    ...
    Forall(
      ...
      Imply(
        ...
        Forall(
          del :: Nil,
          Imply(
            Greater(del, Number(0)), // \forall del > 0
            Exists(
              TVar :: Nil,
              And(
                GreaterEqual(TVar, Number(0)), // \exists T >= 0
                qbody,
              ),
            ),
          ),
  ...
  def proveStabilityCLF(V: scala.Option[Term]): DependentPositionWithAppliedInputTactic = "stabilityCLF".byWithInputs(
    List(V),
    { (pos: Position, seq: Sequent) =>
      if (!(pos.isTopLevel && pos.isSucc)) throw new IllFormedTacticApplicationException(
        "proveStabilityCLF: position " + pos + " must point to a top-level succedent position"
      )
      ...
      val prog = seq.sub(pos) match {
        ...
        case _ => throw new IllFormedTacticApplicationException(
            "proveStabilityCLF: position " + pos + " must point to a valid top-level succedent position"
          )
      }
      ...
      V match {
        ...
        case None => ToolProvider.lyapunovTool() match {
            case Some(lt) => lt.genCLF(ss.odes) match {
                case Some(v) => proveStabilityCLF(v, ss, seq.ante.length, pos)
                case None => throw new MissingLyapunovFunction(
                    "V not provided, tried to generate automatically but failed; please provide V manually"
                  )
              }
            case None => throw new MissingLyapunovFunction(
                "No Lyapunov solver available; please configure Mathematica+Matlab, or provide V manually"
              )
          }
      }
    },
  ...
  @Derivation
  val proveStabilityCLFInfo: InputPositionTacticInfo = InputPositionTacticInfo.create(
    name = "stabilityCLF",
    ...
    displayConclusion =
      "Γ |- ∀ℇ>0 ∃∆>0 ∀x<sup>2</sup><∆<sup>2</sup> [{ x'=f_p(x) & Q }*]x<sup>2</sup><ℇ<sup>2</sup>, Δ",
    constructor = TacticConstructor1.create(OptionArg(TermArg("V")))((V: scala.Option[Term]) => proveStabilityCLF(V)),
  )
  ...
  private def proveStabilityCLF(lyap: Term, ss: SwitchedSystem, apos: Integer, pos: Position): BelleExpr = {
    require(
      StaticSemantics.freeVars(lyap).toSet.subsetOf(ss.cvars.toSet),
      "Common Lyapunov function should only mention " + ss.cvars + " free",
    )
    val eps = Variable("eps")
    val del = Variable("del")
  ...
  def proveAttractivityCLF(V: scala.Option[Term]): DependentPositionWithAppliedInputTactic = "attractivityCLF"
    .byWithInputs(
      List(V),
      { (pos: Position, seq: Sequent) =>
        if (!(pos.isTopLevel && pos.isSucc)) throw new IllFormedTacticApplicationException(
          "proveAttractivityCLF: position " + pos + " must point to a top-level succedent position"
        )

        val progPre = seq.sub(pos) match {
          case Some(f: Formula) => stripTillBox(f)
          case _ => throw new IllFormedTacticApplicationException(
              "proveAttractivityCLF: position " + pos + " must point to a valid top-level succedent position"
            )
        }

        val (t, prog) = progPre match {
          case Compose(Assign(t, _), p) => (t, p)
          case _ => throw new IllFormedTacticApplicationException(
              "proveAttractivityCLF: position " + pos + " does not match attractivity specification"
            )
        }
  ...
  @Derivation
  val proveAttractivityCLFInfo: InputPositionTacticInfo = InputPositionTacticInfo.create(
    ...
    constructor = TacticConstructor1.create(OptionArg(TermArg("V")))((V: scala.Option[Term]) => proveAttractivityCLF(V)),
  )
  ...
  private def proveAttractivityCLF(lyap: Term, ss: SwitchedSystem, apos: Integer, pos: Position): BelleExpr = {
    ...
    val epsu = Exists(
      ...
      And(
        Greater(u, Number(0)),
        cvars.foldRight(
          Imply(And(if (fullInv) Less(lyap, w) else True, GreaterEqual(normsq, epssq)), GreaterEqual(lyap, u)): Formula
        )((v, f) => Forall(v :: Nil, f)),
      ),
    )
    ...
    val derPair: List[(Formula, Formula)] = ss
      .odes
      .zip(kis)
      .map(odei => {
        val ki = odei._2
        ...
        (
          Exists(
            ...
            And(
              ...
              cvars.foldRight(
                Imply(
                  And(odei._1.constraint, And(GreaterEqual(lyap, u), if (fullInv) Less(lyap, w) else True)),
                  bod,
                ): Formula
              )((v, f) => Forall(v :: Nil, f)),
            ),
    ...
    // Succedent has form:
    // assums, [ODE](lyap< W & ...)
    def odetac(bd: Formula): BelleExpr = dC(Less(lyap, w))(pos) < (
                          ...
                          (allR(pos) * cvars.length) & implyR(pos) & // strip forall x (d^2 < del^2 -> )
                            (allL(-(apos + 4)) * cvars.length) & implyL(-(apos + 4)) < ( // discharge first cut on del
                              ...
                              ArithmeticSimplification.hideFactsAbout(del :: Nil) &
                                hideL(-(apos + 1)) & hideL(-(apos + 1)) & hideL(
                                  -(apos + 2)
  ...
  def proveStabilityStateMLF(Vp: List[Term]): DependentPositionWithAppliedInputTactic = "stabilityStateMLF"
    .byWithInputs(
      List(Vp),
      { (pos: Position, seq: Sequent) =>
        if (!(pos.isTopLevel && pos.isSucc)) throw new IllFormedTacticApplicationException(
          "proveStabilityStateMLF: position " + pos + " must point to a top-level succedent position"
        )

        val prog = seq.sub(pos) match {
          case Some(f) => stripTillBox(f.asInstanceOf[Formula])
          case _ => throw new IllFormedTacticApplicationException(
              "proveStabilityStateMLF: position " + pos + " must point to a valid top-level succedent position"
            )
        }
        ...
        val vp = Vp match {
          case Nil => ToolProvider.lyapunovTool() match {
              case Some(t) => {
                ...
                if (res.nonEmpty) res
                else throw new MissingLyapunovFunction(
                  "Vp not provided, tried to generate automatically but failed; please provide Vp manually"
                )
              }
              case None => throw new MissingLyapunovFunction(
                  "No Lyapunov solver available; please configure Mathematica+Matlab, or provide Vp manually"
                )
            }
          case _ => Vp
        }
        ...
        ss match {
          ...
          case _ => throw new IllFormedTacticApplicationException(
              "proveStabilityStateMLF: not implemented for switching mechanism: " + ss
            )
        }
      },
    )
  ...
  @Derivation
  val proveStabilityStateMLFInfo: InputPositionTacticInfo = InputPositionTacticInfo.create(
    ...
    constructor = TacticConstructor1.create(ListArg(TermArg("Vp")))((Vp: List[Term]) => proveStabilityStateMLF(Vp)),
  )
  ...
  private def proveStabilityStateMLF(lyaps: List[Term], ss: SwitchedSystem, apos: Integer, pos: Position): BelleExpr = {
    ...
    require(
      lyaps.map(StaticSemantics.freeVars(_).toSet).flatten.toSet.subsetOf(ss.cvars.toSet),
      "Multiple Lyapunov functions should only mention " + ss.cvars + " free",
    )
    require(
    ...
    val wbod: List[Formula] = lyaps
      ...
      .map(lyapi => {
        ...
        Exists(
          ...
          And(
            Greater(wi, Number(0)),
            cvars.foldRight(Imply(Equal(normsq, epssq), GreaterEqual(lyapi._1, lyapi._2)): Formula)((v, f) =>
              Forall(v :: Nil, f)
            ),
          ),
        )
    ...
    val delw: List[Formula] = lyaps
      ...
      .map(lyapi => {
        ...
        Exists(
          deli :: Nil,
          And(
            And(Greater(deli, Number(0)), Less(deli, eps)),
            cvars.foldRight(Imply(Less(normsq, Power(deli, Number(2))), Less(lyapi._1, w)): Formula)((v, f) =>
              Forall(v :: Nil, f)
            ),
          ),
        )
    ...
    def odetac(lyap: Term): BelleExpr = dC(Less(lyap, w))(Symbol("Rlast")) < (
                        ...
                        (allR(pos) * cvars.length) & implyR(pos) & // strip forall x (d^2 < del^2 -> )
                          cutR(And(lyapw, Less(normsq, epssq)))(pos) < (
                            ArithmeticSimplification.hideFactsAbout(wis) &
                              andR(pos) < (
                                implyRiLast & implyRiLast & andL(Symbol("Llast")) & implyR(Symbol("Rlast")) & implyR(
                                  Symbol("Rlast")
                                ) &
  ...
  def proveStabilityTimeMLF(Vp: List[Term], Lp: List[Formula]): DependentPositionWithAppliedInputTactic =
    "stabilityTimeMLF".byWithInputs(
      ...
      { (pos: Position, seq: Sequent) =>
        if (!(pos.isTopLevel && pos.isSucc)) throw new IllFormedTacticApplicationException(
          "proveStabilityTimeMLF: position " + pos + " must point to a top-level succedent position"
        )

        val prog = seq.sub(pos) match {
          case Some(f) => stripTillBox(f.asInstanceOf[Formula])
          case _ => throw new IllFormedTacticApplicationException(
              "proveStabilityTimeMLF: position " + pos + " must point to a valid top-level succedent position"
            )
        }
        ...
        ss match {
          ...
          case _ => throw new IllFormedTacticApplicationException(
              "proveStabilityTimeMLF: not implemented for switching mechanism: " + ss
            )
        }
      },
    )
  ...
  @Derivation
  val proveStabilityTimeMLFInfo: InputPositionTacticInfo = InputPositionTacticInfo.create(
  ...
  private def expExpand(t: Term, n: Int): Term = {
    (1 to n).reverse.foldRight(Number(1): Term)((i, f) => Plus(Divide(Power(t, Number(i)), Number(factorial(i))), f))
  }

  private def proveStabilityTimeMLF(
      lyaps: List[Term],
      lambdas: List[Formula],
      ss: Timed,
      apos: Integer,
      pos: Position,
  ): BelleExpr = {
    require(
      lyaps.length == lambdas.length,
      "No. of Lyapunov functions and lambdas must match but got " + lyaps.length + " and " + lambdas.length,
    )
    require(
      lyaps.map(StaticSemantics.freeVars(_).toSet).flatten.toSet.subsetOf(ss.cvars.toSet),
      "Multiple Lyapunov functions should only mention " + ss.cvars + " free",
                  ...
                  Times(
                    ...
                    mD.getOrElse(
                      throw new IllegalArgumentException(
                        "Lyapunov functions with lambda <=0 must have maximum dwell time"
                      )
                    ),
                  ),
                ),
    ...
    val delw: List[Formula] = lyapsE
      ...
      .map((lyapE, deli) => {
        Exists(
          ...
          And(
            And(Greater(deli, Number(0)), Less(deli, eps)),
            cvars.foldRight(Imply(Less(normsq, Power(deli, Number(2))), Less(lyapE, w)): Formula)((v, f) =>
              Forall(v :: Nil, f)
            ),
          ),
        )
    ...
    def odetac(less: Formula): BelleExpr = DebuggingTactics.debug("ODE tac: " + less, debugTactic) &
      ...
      val invBody = invLess
        ...
        .map((less, lambda, mD) =>
          lambda match {
            ...
            case LessEqual(l, n: Number) if n.value == 0 => And(less, LessEqual(ss.timer, mD.get))
            case _ => ???
          }
        )
          ...
          cutR(wMin)(pos) < (
            ...
            (andL(-(apos + 2)) & hideL(-(apos + lyaps.length + 1))) * lyaps.length &
              ...
              cutR(delwFml)(pos) < (
                andL(Symbol("Llast")) & ArithmeticSimplification.hideFactsAbout(wis) &
                  lyaps
                    .map(lyap => DebuggingTactics.debug("lyap: " + lyap, debugTactic) & QE & done)
                    .reduceRight((p1, p2) => andR(pos) < (p1, p2)),
                implyR(pos) &
                  (andL(Symbol("Llast")) * (lyaps.length - 1)) &
                  (existsL(-(apos + 3 + lyaps.length))) * lyaps.length &
                  andL(-(apos + 3 + lyaps.length)) * lyaps.length &
  ...
  def proveAttractivityStateMLF(Vp: List[Term]): DependentPositionWithAppliedInputTactic = "attractivityStateMLF"
    .byWithInputs(
      List(Vp),
      { (pos: Position, seq: Sequent) =>
        if (!(pos.isTopLevel && pos.isSucc)) throw new IllFormedTacticApplicationException(
          "proveAttractivityStateMLF: position " + pos + " must point to a top-level succedent position"
        )

        val progPre = seq.sub(pos) match {
          case Some(f: Formula) => stripTillBox(f)
          case _ => throw new IllFormedTacticApplicationException(
              "proveAttractivityStateMLF: position " + pos + " must point to a valid top-level succedent position"
            )
        }
        ...
        val (t, prog) = progPre match {
          ...
          case _ => throw new IllFormedTacticApplicationException(
              "proveAttractivityMLF: position " + pos + " does not match attractivity specification"
            )
        }
        ...
        ss match {
          ...
          case _ => throw new IllFormedTacticApplicationException(
              "proveAttractivityStateMLF: not implemented for switching mechanism: " + ss
            )
        }
      },
    )
  ...
  @Derivation
  val proveAttractivityStateMLFInfo: InputPositionTacticInfo = InputPositionTacticInfo.create(
    ...
    displayConclusion =
      "Γ |- ∀ℇ>0 ∀∆>0 ∃T≥0 ∀x<sup>2</sup><∆<sup>2</sup> [t:=0; {t'=1,x'=f_p(x) & Q}*](t≥T → x<sup>2</sup><ℇ<sup>2</sup>), Δ",
    constructor = TacticConstructor1.create(ListArg(TermArg("Vp")))((Vp: List[Term]) => proveAttractivityStateMLF(Vp)),
  )
  ...
  private def proveAttractivityStateMLF(
      ...
      .map(lyapi => {
        ...
        Exists(
          ...
          And(
            ...
            cvars.foldRight(
              Imply(
                And(if (fullInv) Less(lyapi._1, w) else True, GreaterEqual(normsq, epssq)),
                GreaterEqual(lyapi._1, ui),
              ): Formula
            )((v, f) => Forall(v :: Nil, f)),
          ),
        )
        ...
        (
          Exists(
            ...
            And(
              ...
              cvars.foldRight(
                Imply(
                  And(ode.constraint, And(GreaterEqual(lyap, u), if (fullInv) Less(lyap, w) else True)),
                  bod,
                ): Formula
              )((v, f) => Forall(v :: Nil, f)),
            ),
        ...
        boxAnd(Symbol("Rlast")) & andR(Symbol("Rlast")) < (
          ...
          useAt(Ax.DCC, PosInExpr(1 :: Nil))(Symbol("Rlast")) & andR(Symbol("Rlast")) < (
            implyRiLast & hideL(Symbol("Llast")) &
              implyRiLast * 3 & cohideOnlyR(Symbol("Rlast")) & cohideOnlyL(Symbol("Llast")) & implyR(
                Symbol("Rlast")
              ) * 4 &
              dC(bd)(1) < (
    ...
    val conttac = ss match {
      case StateDependent(odes) => {
        val invPre = (ss.odes zip lyaps).map(odel =>
          And(
            odel._1.constraint,
      ...
      case Guarded(modes, mode) => {
        ...
        val invPre = (modes zip lyaps).map(namel =>
          And(
            Equal(mode, mkMode(namel._1._1)),
                              ...
                              existsRmon(Tchoice)(pos) < (
                                ...
                                andL(Symbol("Llast")) & andR(pos) < (
                                  ...
                                  (allR(pos) * cvars.length) & implyR(pos) & // strip forall x (d^2 < del^2 -> )
                                    ...
                                    ArithmeticSimplification.hideFactsAbout(del :: wis) &
                                    hideL(-(apos + 1)) & hideL(-(apos + lyaps.length + 1)) & hideL(
  ...
  def proveAttractivityTimeMLF(Vp: List[Term], Lp: List[Formula], rate: Term): DependentPositionWithAppliedInputTactic =
    "attractivityTimeMLF".byWithInputs(
      List(Vp, Lp, rate),
      { (pos: Position, seq: Sequent) =>
        if (!(pos.isTopLevel && pos.isSucc)) throw new IllFormedTacticApplicationException(
          "proveAttractivityTimeMLF: position " + pos + " must point to a top-level succedent position"
        )

        val progPre = seq.sub(pos) match {
          case Some(f: Formula) => stripTillBox(f)
          case _ => throw new IllFormedTacticApplicationException(
              "proveAttractivityTimeMLF: position " + pos + " must point to a valid top-level succedent position"
            )
        }
        ...
        val (t, prog) = progPre match {
          case Compose(Assign(t, _), p) => (t, p)
          case _ => throw new IllFormedTacticApplicationException(
              "proveAttractivityTimeMLF position " + pos + " does not match attractivity specification"
            )
        }
        ...
        ss match {
          ...
          case _ => throw new IllFormedTacticApplicationException(
              "proveAttractivityTimeMLF: not implemented for switching mechanism: " + ss
            )
        }
      },
    )
  ...
  @Derivation
  val proveAttractivityTimeMLFInfo: InputPositionTacticInfo = InputPositionTacticInfo.create(
    ...
    displayConclusion =
      "Γ |- ∀ℇ>0 ∀∆>0 ∃T≥0 ∀x<sup>2</sup><∆<sup>2</sup> [t:=0; {t'=1,x'=f_p(x) & Q}*](t≥T → x<sup>2</sup><ℇ<sup>2</sup>), Δ",
    constructor = TacticConstructor3.create(ListArg(TermArg("Vp")), ListArg(FormulaArg("Lp")), TermArg("rate"))(
      (Vp: List[Term], Lp: List[Formula], rate: Term) => proveAttractivityTimeMLF(Vp, Lp, rate)
    ),
  )

  private def proveAttractivityTimeMLF(
      lyaps: List[Term],
      lambdas: List[Formula],
      rate: Term,
      ss: Timed,
    ...
    def odetac(less: Formula): BelleExpr = DebuggingTactics.debug("odetac for: " + less, debugTactic) &
      dC(GreaterEqual(ss.timer, Number(0)))(Symbol("Rlast")) < ( // t>=0 (trivial from t'=1)
      ...
      val invLHS: List[Term] = lambdas
        ...
        .map((lambda, lyapE) =>
          Times(
            lyapE,
            Times(
              expExpand(Times(tVar, rate), expDepth),
              expExpand(Times(Minus(lambda.asInstanceOf[ComparisonFormula].left, rate), ss.timer), expDepth),
            ),
          )
        )
              ...
              cutR(ubodFml)(pos) < (
                ...
                implyR(pos) & (andL(Symbol("Llast")) * (lyaps.length - 1)) &
                  ...
                  cutR(uMin)(pos) < (
                    ...
                    implyR(pos) & existsL(Symbol("Llast")) & andL(Symbol("Llast")) * lyaps.length &
                      existsRmon(Tchoice)(pos) < (
                        hideL(Symbol("Llast")) * lyaps.length & cohideOnlyL(
                          Symbol("Llast")
                        ) & QE, // use u_>0 to show there exists a good choice for T_
                        andL(Symbol("Llast")) & andR(pos) < (
                          id,
                          (allR(pos) * cvars.length) & implyR(pos) & // strip forall x (d^2 < del^2 -> )
  ...
  def guardedTransitionMap(ss: SwitchedSystem): List[(Int, Int, Formula)] = {
    ss match {
      case StateDependent(odes) => odes
          .zipWithIndex
          .map(ode1 =>
            odes
              .zipWithIndex
              .filter(p => p._2 != ode1._2)
              .map(ode2 => (ode1._2 + 1, ode2._2 + 1, And(ode1._1.constraint, ode2._1.constraint): Formula))
          )
          .flatten
      case Guarded(modes, u) => modes
          .zipWithIndex
          .map(modei => modei._1._3.map(trans => (modei._2 + 1, modes.indexWhere(p => p._1 == trans._1) + 1, trans._2)))
          .flatten
  ...
  def odeActive(sys: ODESystem, dom: Formula): Option[(String, Map[NamedSymbol, Expression])] = {
    ...
    val (normalized, _) =
      try { SimplifierV3.semiAlgNormalize(sys.constraint) }
      catch {
        case ex: IllegalArgumentException => throw new IllegalArgumentException(
            "Unable to normalize domain to semi-algebraic format: " + sys.constraint,
            ex,
          )
      }
    ...
    if (prfwd.nonEmpty) return Some("Unable to enter ODE " + sys, prfwd.get)

    val checkbwd = Imply(And(dom, bwdfml), sys.constraint)
    val prbwd = findCounterExample(checkbwd)

    if (prbwd.nonEmpty) return Some("Unable to leave ODE " + sys, prbwd.get)
  ...
  def stateSwitchActive(odes: List[ODESystem], dom: Formula = True): Option[(String, Map[NamedSymbol, Expression])] = {
    ...
    if (pr.nonEmpty) return Some("System cannot evolve forwards", pr.get)
  ...
  def timeSwitch(
      ...
      transitions: List[List[(Int, Term)]],
      s: Variable = "s_".asVariable,
      u: Variable = "u_".asVariable,
      topt: Option[BaseVariable] = None,
  ): Program = {

    if (odes.isEmpty) throw new IllegalArgumentException("Time-dependent switching requires at least 1 ODE")

    if (odes.length != transitions.length)
      throw new IllegalArgumentException("ODEs and transitions map must have same length")

    val todespre = odes.map((odet) =>
      odet._2 match {
        case None =>
          ODESystem(DifferentialProduct(AtomicODE(DifferentialSymbol(s), Number(1)), odet._1.ode), odet._1.constraint)
        case Some(tt) => ODESystem(
            DifferentialProduct(AtomicODE(DifferentialSymbol(s), Number(1)), odet._1.ode),
    ...
    val ctrl = transitions
      ...
      .map(it => {
        val (trans, ind) = it
        val transprog = trans.map(ij => {
          if (ij._1 < 0 || ij._1 >= odes.length)
            throw new IllegalArgumentException("Unexpected transition to unknown ODE index: " + ij._1)
          if (ij._1 == ind) throw new IllegalArgumentException("Self transition not allowed for ODE index: " + ij._1)
  ...
  def slowSwitch(
    ...
    val transitions = (0 to odes.length - 1)
      .map(i => (0 to odes.length - 1).filterNot(j => j == i).map(j => (j, dwell)).toList)
      .toList
  ...
  def stableOrigin(
      prog: Program,
      varsopt: Option[List[(Variable, Term)]] = None,
      restr: Option[Formula] = None,
  ): Formula = {

    val bv = StaticSemantics
      .boundVars(prog)
      .intersect(StaticSemantics.freeVars(prog))
      .toSet
      .filterNot(StaticSemantics.isDifferential(_))
    ...
    val eps = Variable("eps")
    val del = Variable("del")
    if (av.contains(eps) || av.contains(del)) throw new IllegalArgumentException(
      "Hybrid program must not mention variables eps or del for stability specification"
    )
    ...
    val body = Imply(init, Box(prog, Less(normsq, Power(eps, Number(2)))))
  ...
  /**
   * Prove stability specification at the given top-level position using the given Lyapunov function V. V must satisfy
  ...
  def proveStable(
      lyap: Term,
      prog: Program,
      varsopt: Option[List[Variable]] = None,
      restr: Option[Formula] = None,
  ): DependentPositionTactic = anon((pos: Position, seq: Sequent) => {
    if (!(pos.isTopLevel && pos.isSucc)) throw new IllFormedTacticApplicationException(
      "proveStable: position " + pos + " must point to a top-level succedent position"
    )
