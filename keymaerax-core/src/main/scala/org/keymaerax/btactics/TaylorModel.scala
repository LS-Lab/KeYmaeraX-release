...
object TaylorModelTactics extends Logging {
  ...
  def debugTac(message: => String): StringInputTactic = new StringInputTactic("debugTac", message :: Nil) {
    override def result(provable: ProvableSig): ProvableSig = {
      debug(_ => "===== " + message + " ====\n" + ppSubgoals(provable) + "\n===== " + message + " (end)\n")
      provable
    }
  ...
  val existsLstable = new BuiltInLeftTactic("existsLstable") {
    override def computeResult(provable: ProvableSig, pos: AntePosition): ProvableSig = {
      ...
      ).apply(ExchangeLeftRule(antepos, AntePos(Llast)), 0).apply(HideLeft(AntePos(Llast)), 0)
    }
  }
  ...
  def rewriteAnte(hide: Boolean) = anon { (pos: Position, seq: Sequent) =>
    seq
      ...
      .zipWithIndex
      .filter {
        case (Equal(_, _), i) => true
        case _ => false
      }
      .foldLeft[BelleExpr](skip) { case (t, (f, i)) => eqL2R(-i - 1)(pos) & (if (hide) hideL(-i - 1) else skip) & t }
  }
  ...
  private def rewriteFormula(prv: ProvableSig) = anon { (pos: Position, seq: Sequent) =>
    prv.conclusion.succ.toList match {
      case Equiv(lhs, rhs) :: Nil => seq.sub(pos) match {
          case Some(fml: Formula) =>
            val poss = FormulaTools.posOf(
              fml,
              a =>
                a match {
                  case (subfml: Formula) => {
      ...
      case _ => throw new InputFormatFailure(
          "rewriteFormula expects an Equiv as single conclusion of provable but got:\\n" + prv
        )
    }
  }
  ...
  case class TMNames(
    ...
    def monomials(len: Int, order: Int): List[List[Int]] = {
      def monomialsAcc(len: Int, xs: List[Int], n: Int, acc: List[List[Int]]): List[List[Int]] = {
        if (len == 0) xs :: acc else (0 to n).foldLeft(acc) { case (a, i) => monomialsAcc(len - 1, i :: xs, n - i, a) }
      }
    ...
    def all_vars(n: Int, order: Int): List[Term] = {
      ...
      (make(precondC) :: make(right) :: make(lower) :: make(upper) :: make(lowerC) :: make(upperC) :: make(interval) ::
        make(remainder) :: preconds.map(make)).flatten ++ allPostCoeffs
    }
  }
  ...
  private[btactics] def coarsenTimesBounds(t: Term) = anon { (seq: Sequent) =>
      ...
      seq.sub(pos) match {
        case Some(LessEqual(l, Plus(Times(t, g), c))) =>
          ...
          seq
            .ante
            .find {
              case (LessEqual(Number(n), s)) if n == 0 && s == t => true
              case _ => false
            } match {
              ...
              seq.ante.collectFirst { case fml @ LessEqual(s, _) if s == t => fml } match {
                ...
                case Some(ub_fml @ LessEqual(_, ub)) =>
                  ...
                  cutL(LessEqual(l, Plus(FuncOf(InterpretedSymbols.maxF, Pair(Number(0), Times(ub, g))), c)))(pos) &
                    Idioms.<(
                      skip,
                      cohideOnlyR(Symbol("Rlast")) & cutR(And(nonneg, ub_fml))(1) & Idioms.<(
                        andR(1) & Idioms.<(id, id),
                        cohideR(1) &
                          useAt(Ax.leTimesMonoLemma, PosInExpr(Nil))(
                            1
                          ) & // TODO: in high-order (>=4), nonlinear ODE, a QE at this point is very slow (8s), but QE on the subgoal in a separate test is fast(<1s)
                          done,
                      ),
                    )
              }
    ...
    val timesLeMono = anon { (pos: Position, seq: Sequent) =>
      pos.checkAnte
      seq.sub(pos) match {
        case Some(LessEqual(Plus(Times(t, g), c), u)) => seq
            .ante
            .find {
              case (LessEqual(Number(n), s)) if n == 0 && s == t => true
              case _ => false
            ...
            case None => throw new IllegalArgumentException("could not find matching nonnegativity assumption")
            case Some(nonneg) => seq.ante.collectFirst { case fml @ LessEqual(s, _) if s == t => fml } match {
                case None => throw new IllegalArgumentException("could not find matching upper bound assumption")
                case Some(ub_fml @ LessEqual(_, ub)) =>
    ...
    seq
      ...
      .map {
        case (LessEqual(Plus(Times(s, _), _), _), pos) if s == t => timesLeMono(pos)
        case (LessEqual(_, Plus(Times(s, _), _)), pos) if s == t => leTimesMono(pos)
        case _ => skip
      }
  ...
  private[btactics] def unfoldMinMax = chaseCustom({
    case Greater(FuncOf(InterpretedSymbols.minF, _), _) =>
      (Ax.minGtNorm.provable, PosInExpr(0 :: Nil), List(PosInExpr(0 :: Nil), PosInExpr(1 :: Nil))) :: Nil
    case GreaterEqual(FuncOf(InterpretedSymbols.minF, _), _) =>
      (Ax.minGeNorm.provable, PosInExpr(0 :: Nil), List(PosInExpr(0 :: Nil), PosInExpr(1 :: Nil))) :: Nil
    case LessEqual(FuncOf(InterpretedSymbols.minF, _), _) =>
      (Ax.minLeNorm.provable, PosInExpr(0 :: Nil), List(PosInExpr(0 :: Nil), PosInExpr(1 :: Nil))) :: Nil
    case LessEqual(_, FuncOf(InterpretedSymbols.maxF, _)) =>
      (Ax.leMaxNorm.provable, PosInExpr(0 :: Nil), List(PosInExpr(0 :: Nil), PosInExpr(1 :: Nil))) :: Nil
    case _ => Nil
  })
  ...
  private def solveTrivialInequalities: DependentPositionTactic = anon { (pos: Position, seq: Sequent) =>
    ...
    val ssp = seq.sub(pos)
    seq.sub(pos) match {
      case Some(LessEqual(a, b)) => useAt(Ax.trivialInequality, PosInExpr(1 :: Nil))(pos) & andR(pos) &
          Idioms.<(QE & done, QE & done) // TODO: this is purely algebraic, optimize for that?
      case Some(And(f, g)) => andR(pos) & Idioms.<(solveTrivialInequalities(1), solveTrivialInequalities(1))
      case _ => throw new IllegalArgumentException("solveTrivialInequalities cannot be applied here: " + ssp)
    }
  }
  ...
  private[btactics] def refineTrivialInequalities: DependentPositionTactic = anon { (pos: Position, seq: Sequent) =>
    ...
    val ssp = seq.sub(pos)
    seq.sub(pos) match {
      case Some(Imply(And(_, _), And(_, _))) => useAt(Ax.refineConjunction, PosInExpr(1 :: Nil))(pos) & andR(pos) &
          Idioms.<(refineTrivialInequalities(1), refineTrivialInequalities(1))
      case Some(Imply(Less(_, _), Greater(_, _))) => useAt(Ax.trivialRefineLtGt, PosInExpr(1 :: Nil))(pos) &
          Idioms.<(QE & done) // TODO: this is purely algebraic, optimize for that?
      case Some(Imply(GreaterEqual(_, _), LessEqual(_, _))) => useAt(Ax.trivialRefineGeLe, PosInExpr(1 :: Nil))(pos) &
          Idioms.<(QE & done) // TODO: this is purely algebraic, optimize for that?
      case _ => throw new IllegalArgumentException("solveTrivialInequalities cannot be applied here: " + ssp)
    }
  }
  ...
  case class TaylorModel(
      ode: DifferentialProgram,
      order: Int,
      names: TMNames = TMNames("a", "r", "i", "Rem", "h", "tm"),
  ) {
    ...
    protected val params = StaticSemantics
      .symbols(ode)
      .flatMap({
        case fn @ Function(_, _, Unit, Real, _) => Some(fn)
        case _ => None
      })
    ...
    private val ringsODE = ringsLib.ODE(
      localTime,
      state,
      names.right_vars(dim),
    ...
    private val exactCoefficients: List[Map[List[Int], Term]] = picard_iterationR
      .map(p => ringsLib.distributive(p, localTime :: right_vars))
      .toList

    /**
     * Just a template polynomial whose coefficients are supposed to be close to [[picard_iterationR]] only uses those
     * coefficients that are actually used
     */
    def tmMonomial(coeff: Term, exponents: List[Int]) = {
      val monomial = (localTime :: right_vars)
        .lazyZip(exponents)
        .filter { case (_, e) => e > 0 }
    ...
    private val horner_order = (0 until dim).flatMap(i => List(tdL(i), tdU(i))).toList ++
      (localTime :: bounded_vars.map(Power(_, Number(2))) ++ bounded_vars)
    private val horner_orderR = horner_order.map(ringsLib.toRing)
    toc("horner_order")

    // numeric condition for initial approximation:
    private val initialNumbericCondition = (0 until dim)
      .map { i =>
        /**
         * time-constant coefficients, i.e., at initial time. this is the same as [[initial_condition]], but perhaps a
         * bit more convenient to use here, because we directly get the exponents.
         */
    ...
    // Horner(f (p + r) - p')
    private val hornerFprPp = fpr.lazyZip(pp).map { case (a, b) => ringsLib.toHorner(a - b, horner_orderR) }
    ...
    private val post = {
      state
        .lazyZip(picard_iteration_approx)
        .zipWithIndex
        .map { case ((x, p), i) => tmEnclosure(x, i, p, postIvlL(i), postIvlU(i)) }
        .toList
    }

    private def postIvlL(i: Int) = { Plus(Times(localTime, tdL(i)), cL(i)) }

    private def postIvlU(i: Int) = { Plus(Times(localTime, tdU(i)), cU(i)) }
    ...
    lazy val postEq = proveBy(
      Equiv(boxTMEnclosure.child, postUnfolded),
      equivR(1) & Idioms.<(
        existsLstable(-1) & andLstable(-1) & andLstable(-1) & useAt(Ax.eqAddIff, PosInExpr(0 :: Nil))(-1) &
          eqL2R(-1)(-2) & eqL2R(-1)(-3) & andR(1) & Idioms.<(close(-3, 1), skip) & hideL(-3) & hideL(-1) &
          andLstable(Symbol("Llast")) *
          (dim - 1) &
          (1 to dim)
            .map { i =>
              val p = -i
              val l = -(dim + i)
              existsLstable(p) & andLstable(p) & useAt(Ax.eqAddIff, PosInExpr(0 :: Nil))(p) & eqL2R(p)(l): BelleExpr
            }
            .reduceLeft(_ & _) & hideL(-1) * dim &
          (andR(1) & Idioms.<(id, skip)) *
          (dim - 1) & id,
        existsR(Minus(time, time0))(1) & andLstable(-1) & andR(1) &
          Idioms.<(andR(1) & Idioms.<(cohideR(1) & byUS(Ax.plusDiffRefl), id), hideL(-1)) &
          andLstable(Symbol("Llast")) *
          (dim - 1) &
          (andR(1) & Idioms.<(useAt(Ax.unfoldExistsLemma)(1) & id, skip)) *
          (dim - 1) & useAt(Ax.unfoldExistsLemma)(1) & id,
      ),
    )
    ...
    private lazy val domain_rewrite = proveBy(
      Equiv(
        And(LessEqual(time0, time), LessEqual(time, Plus(time0, timestep))),
        And(LessEqual(Number(0), Minus(time, time0)), LessEqual(Minus(time, time0), timestep)),
      ),
      QE & done,
    )

    val numericAssumption = And(
      And(
        // t=0, x = A*r + A_c
        initial_condition,
        // r : [rL, rU]
    ...
    lazy val lemma: ProvableSig = proveBy(
      ...
      debugTac("start") & implyR(1) & debugTac("pre push") & useAt(postEq, PosInExpr(0 :: Nil))(1, 1 :: Nil) &
        ...
        Idioms.<(
          // Initial Condition
          tocTac("dIClosed") & debugTac("Initial Condition") & andL(-1) &
            // obtain initial numberic condition
            andL(-2) & hideL(-3) & debugTac("obtained initial numberic condition") &
            ringsLib.normalizeLessEquals(QE)(-2) & SaturateTactic(andL(Symbol("L"))) & rewriteAnte(true)(1) &
            ringsLib.normalizeLessEquals(QE)(1) & debugTac("Initial Condition numeric trivialities") & prop &
            tocTac("Initial Condition done?") & debugTac("Initial Condition done?") & done,
          // Differential Invariant
          tocTac("Differential Invariant") & debugTac("Differential Invariant") & foldAndLessEqExists(time :: Nil)(1) &
            tocTac("foldAndLessEqExists") & implyR(1) & SaturateTactic(andL(Symbol("L")) | instLeq(Symbol("L"))) &
            tocTac("Saturate instLeq") & rewriteAnte(true)(1) & tocTac("rewriteAnte") & unfoldMinMax(1) &
            tocTac("unfoldMinMax") & debugTac("unfoldMinMax") & coarsenTimesBounds(Minus(time, time0)) &
            tocTac("coarsenTimesBounds") & debugTac("coarsenTimesBounds") &
            FOQuantifierTactics.allLs(Minus(time, time0) :: remainders)(Symbol("L")) & implyL(Symbol("L")) &
            Idioms.<(
              cohideOnlyR(2) & prop & done,
              cohideOnlyL(Find.FindLMatch("P_() & Q_()".asFormula)) & implyRi()(AntePosition(1), SuccPosition(1)) &
                debugTac("refine it!") & tocTac("to refine") & refineTrivialInequalities(1) & tocTac("refined it") &
                done,
            ),
        ),
    )
    ...
    lazy val timestepLemma = {
      proveBy(
        ...
        implyR(1) &
          ...
          dC(LessEqual(time0, time))(1) &
          Idioms.<(
            useAt(Ax.trueAnd)(1, 0 :: 1 :: Nil),
            dI(Symbol("diffInd"))(1) & Idioms.<(
              useAt(Ax.lessEqual, PosInExpr(0 :: Nil))(1) & orR(1) & hideR(1) &
                useAt(Ax.equalSym, PosInExpr(1 :: 0 :: Nil))(1) & prop & done,
          ...
          (vars.length - vars.indexOf(time)) &
          useAt(Ax.commaCommute)(1, 1 :: 1 :: 1 :: 1 :: Nil) *
          (vars.length - vars.indexOf(time)) & andR(1) &
          Idioms.<(
            ...
            boxAnd(1) & andR(1) & Idioms.<(
              ...
              andL(-1) & hideL(-2) & andL(-1) & andL(-1) & hideL(-3) &
                cutR(boxTMEnclosure)(1) /* TODO: does a dC here yield better structure and less overhead? */ & Idioms.<(
                  ...
                  andL(-2) & hideL(-3) & SaturateTactic(andL(-2) & hideL(-3)) /* extract t=t0() */ & eqL2R(-2)(-1) &
                    hideL(-2) & implyR(1) & dC(boxTMEnclosure.child)(1) & Idioms.<(
                      // make implication via dW
                      hideL(-2) & dWPlus(1) & andL(-2) &
                        // obtain existential witnesses
                        existsL(-3) /* local time */ &
                        // instantiate them
                        allL(localTime)(-1) & FOQuantifierTactics.allLs(state)(-1) & andLstable(-3) & andLstable(-3) &
                        eqL2R(-3)(1) & implyL(-1) &
                        Idioms.<(hideR(1) & andR(1) & Idioms.<(hideL(-3) & QE & done, id), id),
                      id,
                    ),
                ),
              // safety for next step
              andL(-1) & andL(-1) & hideL(-3) & andL(-2) & hideL(-3) & dC(boxTMEnclosure.child)(1) & Idioms.<(
                hideL(-2) & dWPlus(1) & implyR(1) & andL(-2) & existsL(-4) /* time */ & allL(time)(-1) &
                  FOQuantifierTactics.allLs(state)(-1) & DifferentialTactics.diffRefine(True)(1) & Idioms.<(
                    implyL(-1) & Idioms.<(
                      hideR(1) & cutR(Equal(localTime, timestep))(1) &
                        Idioms.<(andL(-3) & hideL(-4) & hideL(-2) & QE & done, implyR(1) & eqL2R(-4)(-3) & prop & done),
                      id,
                    ),
    ...
    protected def getBoundees(fml: Formula): List[Term] = fml match {
      case And(And(Less(_, t1), Less(t2, _)), ivls) if t1 == t2 => t1 :: getBoundees(ivls)
      case And(Less(_, t1), Less(t2, _)) if t1 == t2 => t1 :: Nil
      case _ => throw new TacticInapplicableFailure("getBoundees called on non-And")
    }

    private def instantiationForBound(
        prec: Int,
        bounds: IntervalArithmeticV2.DecimalBounds,
        fml: Formula,
    ): ((Term, BigDecimal), (Term, BigDecimal)) = fml match {
    ...
    private def instantiationForBounds(
        prec: Int,
        bounds: IntervalArithmeticV2.DecimalBounds,
        fml: Formula,
    ): List[((Term, BigDecimal), (Term, BigDecimal))] = fml match {
      ...
      case And(LessEqual(_, t1), LessEqual(t2, _)) if t1 == t2 => instantiationForBound(prec, bounds, fml) :: Nil
      case _ => throw new TacticInapplicableFailure("instantiationForBounds called on non-And")
    }

    private def numericPicard(
        prec: Integer,
        boundees: IndexedSeq[Term],
        timebound: Term,
        right_bounds: List[Formula],
      ...
      val bounds = IntervalArithmeticV2.collect_bounds(
        prec,
        LessEqual(Number(0), localTime) :: LessEqual(localTime, timebound) :: right_bounds ++ lowers ++ uppers,
      )
      boundees.map(IntervalArithmeticV2.eval_ivl(prec)(bounds))
    }

    def numericPicardIteration(
        prec: Integer,
        boundees: IndexedSeq[Term],
        timebound: Term,
        right_bounds: List[Formula],
      ...
      for (k <- 0 until 40) {
        ...
        subset = remainders0.lazyZip(remainders1).forall { case ((l0, u0), (l, u)) => l0 < l && u < u0 }
        val remainders2 =
          if (!subset) {
    ...
    def instantiateLemma(
      ...
      val subst0 = USubst(
        Seq(SubstitutionPair(time0, t0), SubstitutionPair(names.timestep, timebound)) ++ (0 until dim).flatMap(i =>
          Seq(
            SubstitutionPair(names.precondC(i), precondC(i)),
      ...
      val approxCoeffs = exactCoefficients.map(
        _.view
          .mapValues(t =>
            midpoint(IntervalArithmeticV2.eval_ivl(prec)(IntervalArithmeticV2.DecimalBounds())(subst0(t)))
          )
      )
      val approxCoeffSubst = USubst(
        approxCoeffs
          .zipWithIndex
          .flatMap { case (coeffs, i) =>
      ...
      val initApproxSubst = USubst(initApproxInstantiations.flatMap { case ((t, x), (u, y)) =>
        SubstitutionPair(t, Number(x)) :: SubstitutionPair(u, Number(y)) :: Nil
      })
      val constant_errors = initApproxInstantiations.map { case ((a, b), (c, d)) => (b, d) }
      val subst_remainders = numericPicardIteration(
        prec -
          1, // TODO: sometimes the same precision yields unprovable (for IA) goals of the form number<number... perhaps something rounds differently?,
        boundees.map(approxCoeffSubst(_)).toIndexedSeq,
        timebound,
        concrete_right_bound,
        (0 until dim).map(remainder_estimation(_)).toIndexedSeq,
        constant_errors.toIndexedSeq,
      ) match {
        case None => throw new RuntimeException("Picard Iteration did not converge")
        case Some(remainders) => USubst(
            remainders
              .zipWithIndex
              .map { case ((l, _), i) =>
                SubstitutionPair(names.lower(i), IntervalArithmeticV2.mathematicaFriendly(l))
              } ++
              remainders
                .zipWithIndex
                .map { case ((_, u), i) =>
                  SubstitutionPair(names.upper(i), IntervalArithmeticV2.mathematicaFriendly(u))
                }
          )
      }
        ...
        case Some(Box(ODESystem(ode1, And(LessEqual(t0, time1), LessEqual(time2, Plus(t00, timebound)))), goal))
          ...
          debugTac("Starting to do something") & DC(cut_fml)(pos) & Idioms.<(
            ...
            useAt(lemma2, PosInExpr(1 :: Nil))(1) & debugTac("Used Lemma") & andR(1) & Idioms.<(
              id,
              andR(1) & Idioms.<(
                debugTac("Initial Numberic condition") & SaturateTactic(andL(Symbol("L"))) &
                  IntervalArithmeticV2.intervalArithmeticBool(prec, qeTool)(1) & done,
                SaturateTactic(allR(1)) & SaturateTactic(implyR(1)) & SaturateTactic(andL(Symbol("L"))) &
                  debugTac("Numberic condition") & IntervalArithmeticV2.intervalArithmeticBool(prec, qeTool)(1) & done,
              ),
            ),
          )
        }
