 ...
 *   STTT, Special issue for selected papers from FASE'17, 2018.
 * @author
 *   Stefan Mitsch
 */
object ComponentSystem {
    ...
    anon((pos: Position, seq: Sequent) => {
      seq.sub(pos) match {
        case Some(Box(Assign(x, s), Box(Assign(y, t), p))) =>
          val rensubst = (_: Option[Subst]) =>
            RenUSubst(Seq(
              "x_".asVariable -> x,
              "y_".asVariable -> y,
              "s_()".asTerm -> s,
              "t_()".asTerm -> t,
              PredOf(Function("p_", None, Tuple(Real, Real), Bool), "(._1,._2)".asTerm) ->
                p.replaceFree(x, "._1".asTerm).replaceFree(y, "._2".asTerm),
            ))
        ...
        case Some(Box(Assign(x, s), Box(AssignAny(y), p))) =>
          val rensubst = (_: Option[Subst]) =>
            RenUSubst(Seq(
              ...
              "t_()".asTerm -> s,
              PredOf(Function("p_", None, Tuple(Real, Real), Bool), "(._1,._2)".asTerm) ->
                p.replaceFree(x, "._2".asTerm).replaceFree(y, "._1".asTerm),
            ))
        ...
        case Some(Box(a, Box(b, p))) =>
          val prgVars = StaticSemantics
            .freeVars(p)
            .intersect(StaticSemantics.boundVars(a) ++ StaticSemantics.boundVars(b))
            .toSet[Variable]
            .toList
            .sorted[NamedSymbol]
          ...
          val swapped = proveBy(
            Equiv(Box(b, Box(a, q)), Box(a, Box(b, q))),
            SeqTactic(
              chase(1, 0 :: Nil),
              chase(1, 1 :: Nil),
              equivR(1),
          ...
          val rensubst = (_: Option[Subst]) =>
            RenUSubst(
              dots.map({ case (v, d) => d -> v }) :+
                (dots.foldLeft[Formula](q)({ case (qq, (v, d)) => qq.replaceAll(v, d) }) ->
                  dots.foldLeft(p)({ case (pp, (v, d)) => pp.replaceFree(v, d) }))
            )
          useAt(swapped, PosInExpr(1 :: Nil), rensubst)(pos)
        case Some(e) => throw new TacticInapplicableFailure(
            "programIndepedence only applicable to box properties, but got " + e.prettyString
          )
  ...
  lazy val dropControl: DependentPositionTactic = anon((pos: Position, seq: Sequent) => {
    ...
    val lemma2 = seq.sub(pos) match {
      case Some(have @ Box(a, Box(b, p)))
          if StaticSemantics.freeVars(p).intersect(StaticSemantics.boundVars(b)).isEmpty &&
      ...
      case Some(have @ Box(b, Box(a, p)))
          if StaticSemantics.freeVars(p).intersect(StaticSemantics.boundVars(b)).isEmpty &&
            StaticSemantics.freeVars(a).intersect(StaticSemantics.boundVars(b)).isEmpty =>
        proveBy(Imply(Box(a, p), have), implyR(1) & abstractionb(1) & id & done)
      case Some(have @ Box(a, p))
          if p.isFOL && StaticSemantics.freeVars(p).intersect(StaticSemantics.boundVars(a)).isEmpty &&
      ...
      case Some(e) =>
        throw new TacticInapplicableFailure("dropControl only applicable to box properties, but got " + e.prettyString)
      case None => throw new IllFormedTacticApplicationException(
          "Position " + pos + " does not point to a valid position in sequent " + seq.prettyString
        )
    }
  ...
  def dropPlant(keep: Set[Variable]): DependentPositionTactic = anon((pos: Position, seq: Sequent) => {
    ...
    val lemma3 = seq.sub(pos) match {
      case Some(have @ Box(ODESystem(ode, hq), p)) =>
        ...
        require(
          xs.flatMap(x => StaticSemantics.freeVars(x.e).toSet).intersect(ys.map(_.xp.x)).isEmpty,
          "Right-handside of x'=theta must not overlap primed ys",
        )
        require(
          ys.flatMap(y => StaticSemantics.freeVars(y.e).toSet).intersect(xs.map(_.xp.x)).isEmpty,
          "Right-handside of y'=eta must not overlap primed ys",
        )
        ...
        xs.reduceOption(DifferentialProduct.apply) match {
          case Some(xsys) if StaticSemantics.boundVars(xsys).toSet.exists(_.name != "t") =>
            ys.reduceOption(DifferentialProduct.apply) match {
              case Some(ysys) =>
                val universalXClosure =
                  ys.map(_.xp.x).foldLeft[Formula](Box(ODESystem(ode, h), p))((f, y) => Forall(y :: Nil, f))
                val partitionedOdeLemma = proveBy(
                  Imply(Box(ODESystem(DifferentialProduct(ysys, xsys), h), p), Box(ODESystem(ode, h), p)),
                  SeqTactic(
                    implyR(1),
                ...
                proveBy(
                  Imply(Box(ODESystem(xsys, h), p), have),
                  SeqTactic(
                    implyR(1),
                    DifferentialTactics.diffRefine(h)(1),
                    BranchTactic(
                      SeqTactic(
                        ...
                        BranchTactic(
                          ...
                          SeqTactic(
                            hideR(1),
                            useAt(partitionedOdeLemma, PosInExpr(1 :: Nil))(1, List.fill(ys.size)(0)),
                            ys.indices
                              .reverse
          ...
          case None => ys.reduceOption(DifferentialProduct.apply) match {
              case Some(ysys) =>
                val universalXClosure =
                  ys.map(_.xp.x).foldLeft[Formula](Box(ODESystem(ode, h), p))((f, y) => Forall(y :: Nil, f))
                val partitionedOdeLemma = proveBy(
                  Imply(Box(ODESystem(ysys, h), p), Box(ODESystem(ode, h), p)),
                  SeqTactic(
                    implyR(1),
                    // sort ys into reverse order because we apply DGi outside in (innermost universal quantifier first)
                    AxiomaticODESolver.selectionSort(h, p, ode, ys.map(_.xp.x) ++ xs.map(_.xp.x), SuccPosition(1)),
                      ...
                      SeqTactic(
                        ...
                        BranchTactic(
                          ...
                          SeqTactic(
                            ...
                            allR(1) * ys.size,
                            dW(1),
                            useAt(Ax.testb, PosInExpr(1 :: Nil))(1),
                            id,
                            done,
                          ),
                        ),
      ...
      case Some(e) =>
        throw new TacticInapplicableFailure("Lemma 3 only applicable to box ODEs, but got " + e.prettyString)
      case None => throw new IllFormedTacticApplicationException(
          "Position " + pos + " does not point to a valid position in sequent " + seq.prettyString
        )
    }
    ...
    seq.sub(pos) match {
      case Some(have @ Box(a, p)) =>
        ...
        val approximate = abv
          ...
          .reverse
          .map(i => PosInExpr(List.fill(i)(0)))
          .map(useAt(Ax.randomb, PosInExpr(1 :: Nil))(1, _))
          .reduceOption[BelleExpr](_ & _)
          .getOrElse(skip)

        val decompose = Range(0, abv.size - 1)
          .map(i => PosInExpr(List.fill(i)(1)))
          .map(useAt(Ax.composeb)(-1, _))
          .reduceOption[BelleExpr](_ & _)
          .getOrElse(skip)
      ...
      case Some(e) =>
        throw new TacticInapplicableFailure("higherdV only applicable to box properties, but got " + e.prettyString)
      case None => throw new IllFormedTacticApplicationException(
          "Position " + pos + " does not point to a valid position in sequent " + seq.prettyString
        )
    }
  ...
  private lazy val introduceTestLemma = AnonymousLemmas.remember(
    ...
    SeqTactic(
      implyR(1),
      equivR(1),
      BranchTactic(testb(-2, 1 :: Nil), testb(1, 1 :: Nil)),
      onAll(implyRi()(-2, 1) & useAt(Ax.K, PosInExpr(1 :: Nil))(1) & monb & prop & done),
    ),
    namespace,
  )
  ...
  def introduceTest(f: Formula): DependentPositionTactic = anon((pos: Position, seq: Sequent) => {
    seq.sub(pos) match {
      case Some(Box(a, p)) =>
        val subst = (_: Option[Subst]) =>
          RenUSubst(SystemConst("a") -> a :: "p(||)".asFormula -> p :: "q(||)".asFormula -> f :: Nil)
        useAt(introduceTestLemma, PosInExpr(1 :: 1 :: Nil), subst)(pos)
      case Some(e) => throw new TacticInapplicableFailure(
          "introduceTest only applicable to box properties, but got " + e.prettyString
        )
      case None => throw new IllFormedTacticApplicationException(
          "Position " + pos + " does not point to a valid position in sequent " + seq.prettyString
        )
  ...
  def weakenTest(f: Formula): DependentPositionTactic = anon((pos: Position, seq: Sequent) => {
    seq.sub(pos) match {
      case Some(Box(Test(r), p)) =>
        val subst =
          (_: Option[Subst]) => RenUSubst(Seq("r_()".asFormula -> r, "p_()".asFormula -> p, "q_()".asFormula -> f))
        useAt(weakenTestLemma, PosInExpr(1 :: 1 :: Nil), subst)(pos)
      case Some(e) =>
        throw new TacticInapplicableFailure("weakenTest only applicable to box tests, but got " + e.prettyString)
      case None => throw new IllFormedTacticApplicationException(
          "Position " + pos + " does not point to a valid position in sequent " + seq.prettyString
        )
    }
  ...
  /** STTT Fig. 12 */
  private def useCompatibility(compatibility: Lemma, plant1Vars: Set[Variable]) = anon((pos: Position, seq: Sequent) =>
    seq.sub(pos) match {
      case Some(
            Box(delta, Box(ctrl, Box(Assign(told, t), Box(plant, Box(in1star, Box(cons, Imply(pi2out, pi1in)))))))
          ) => SeqTactic(
          DebuggingTactics.print("Using compatibility"),
          useAt(compatibility, PosInExpr(1 :: Nil))(pos ++ PosInExpr(List.fill(5)(1))),
          DebuggingTactics.print("Applied compatibility lemma"),
          ...
          dropControl(pos),
          DebuggingTactics.print("Dropped all statements except port memory"),
          chase(1),
          prop,
          DebuggingTactics.done("Fig. 12 done"),
        )
      case Some(e) =>
        throw new TacticInapplicableFailure("Fig. 12 only applicable to box properties, but got " + e.prettyString)
      case None => throw new IllFormedTacticApplicationException(
          "Position " + pos + " does not point to a valid position in sequent " + seq.prettyString
        )
    }
  ...
  private def proveSystemCompStep4(
      inputAssumptions: Map[Set[Variable], Formula],
      outputGuarantees: Map[Set[Variable], Formula],
    ...
    seq.sub(pos) match {
      ...
      case Some(Box(cons, _)) if remainingCons > 0 =>
        val headConVars: Set[Variable] =
          leftAssignments(cons, remainingCons).flatMap(p => StaticSemantics.boundVars(p).toSet).toSet
        SeqTactic(
          (if (remainingCons > 1) composeb(pos) else skip),
          // Step 2.7 (introduce output guarantee of component 2)
          introduceTest(outputGuarantees(headConVars))(pos.topLevel ++ pos.inExpr.parent.parent),
          BranchTactic(skip, prop & DebuggingTactics.done("Step 2.7")) & DebuggingTactics.print("Introduced test"),
          // Step 2.8 (move output guarantee past connection)
          programIndependence()(pos.topLevel ++ pos.inExpr.parent),
          programIndependence()(pos),
          DebuggingTactics.print("Moved test"),
          // Step 2.9 (weaken output guarantee to input assumption)
          weakenTest(inputAssumptions(headConVars))(pos ++ PosInExpr(1 :: Nil)),
          BranchTactic(skip, useCompatibility(compatibility, plant1Vars)(pos.topLevel)),
          DebuggingTactics.print("Weakened test"),
          // Step 2.10
          overapproximateProgram(pos),
          DebuggingTactics.print("Overapproximated program"),
          // Step 2.11
          useAt(Ax.composeb, PosInExpr(1 :: Nil))(pos),
          useAt(Ax.composeb, PosInExpr(1 :: Nil))(pos.topLevel ++ pos.inExpr.parent),
          // recurse until done
          proveSystemCompStep4(inputAssumptions, outputGuarantees, plant1Vars, compatibility, remainingCons - 1)(pos),
        )
      case Some(e) => throw new TacticInapplicableFailure(
          "proveSystemCompStep4 only applicable to box properties, but got " + e.prettyString
        )
      case None => throw new IllFormedTacticApplicationException(
          "Position " + pos + " does not point to a valid position in sequent " + seq.prettyString
        )
  ...
  /** STTT Fig. 13 */
  private def justifyFout(
      fout: Formula,
      keepPlantVars: Set[Variable],
      comGuaranteeLiveness: Lemma,
      c2use: Lemma,
      c2step: Lemma,
  ) = anon((pos: Position, seq: Sequent) =>
    seq.sub(pos) match {
      ...
      case Some(e) =>
        throw new TacticInapplicableFailure("justifyFout only applicable to implications, but got " + e.prettyString)
      case None => throw new IllFormedTacticApplicationException(
          "Position " + pos + " does not point to a valid position in sequent " + seq.prettyString
        )
    }
  ...
  private def proveSystemCompStep(
      c1step: Lemma,
      c2use: Lemma,
      c2step: Lemma,
      compatibility: Lemma,
      ...
      case Some(
            Box(
              ...
              Box(
                Compose(ctrl1, ctrl2),
                Box(
                  rememberStart @ Assign(told, t),
                  Box(ODESystem(DifferentialProduct(time, plant3), q3), Box(Compose(in3, cp3), inv1)),
                ),
              ),
            )
          ) =>
        ...
        val in1cons = ports(in1).filter({ case (port, pin) =>
          !StaticSemantics.boundVars(port).intersect(connectedVars).isEmpty
        })
        val in1Assumptions =
          in1cons.map({ case (port, pin) => StaticSemantics.boundVars(port).toSet -> pin.cond }).toMap
        val guarantees = in1cons
          .map({ case (port, _) =>
            StaticSemantics.boundVars(port).toSet ->
              piouts
                .find(piout =>
                  !conStaticSem(StaticSemantics.boundVars(port)).intersect(StaticSemantics.freeVars(piout)).isEmpty
                )
                .getOrElse(True)
          })
          .toMap
        ...
        val c2ComGuaranteeLiveness = if (ports(in2).isEmpty) emptyConLiveness else comGuaranteeLiveness
          ...
          BranchTactic(
            ...
            SeqTactic(
              ...
              (5 to 7)
                .map(i => PosInExpr(List.fill(i)(1)))
                .reverse
                .map(programIndependence(false)(1, _))
                .reduce[BelleExpr](_ & _),
              DebuggingTactics.print("Step 2.3 done"),
              // Step 2.4 (drop cp2)
              dropControl(pos ++ PosInExpr(List.fill(7)(1))),
              DebuggingTactics.print("Step 2.4 done"),
              // Step 2.5 (drop in2*)
              dropControl(pos ++ PosInExpr(List.fill(5)(1))),
              DebuggingTactics.print("Step 2.5 done"),
              // Steps 2.6-2.11, note that reserved time `t` is present in contract models but not part of the plant per STTT
              proveSystemCompStep4(
                in1Assumptions,
                guarantees,
                plant1Vars.filter(_.name != "t"),
                compatibility,
                in1cons.size,
              )(pos ++ PosInExpr(List.fill(5)(1))),
              if (ports(in2).nonEmpty) abstractionb(pos ++ PosInExpr(List.fill(5)(1))) else skip,
              DebuggingTactics.print("Steps 2.6-2.11 done"),
              dropPlant(plant1Vars)(pos ++ PosInExpr(1 :: 1 :: 1 :: Nil)),
              DebuggingTactics.print("Step 2.12 done"),
              composeb(pos ++ PosInExpr(1 :: Nil)),
              dropControl(pos ++ PosInExpr(1 :: 1 :: Nil)), // drop ctrl2
              composeb(pos),
              dropControl(pos ++ PosInExpr(1 :: Nil)),
              DebuggingTactics.print("Step 2.13 done"), // drop delta2
              hideL(Symbol("Llast")),
              // use step lemma
              anon((pp: Position, ss: Sequent) =>
                ss.sub(pp) match {
                  case Some(have @ Box(_, p)) => useAt(
                      proveBy(Equiv(Box(in1, p), have), chase(1, 0 :: Nil) & chase(1, 1 :: Nil) & QE),
                      PosInExpr(1 :: Nil),
                    )(pp)
                }
              ...
              BranchTactic(
                SeqTactic(
                  ...
                  useAt(Ax.composeb, PosInExpr(1 :: Nil))(1, 1 :: Nil),
                  useAt(Ax.composeb, PosInExpr(1 :: Nil))(1),
                  DebuggingTactics.print("Close by C1 step lemma"),
                  useLemma(c1step, Some(prop)),
                  DebuggingTactics.done("Close by C1 induction step"),
                ),
                SeqTactic(
                  DebuggingTactics.print("Proving C1 diff. refine"),
                  cohideR(1),
                  CMon(1, 1 :: 1 :: 1 :: 1 :: Nil),
                  composeb(1, 0 :: 1 :: Nil),
                  useAt(Ax.DR, PosInExpr(1 :: Nil))(1),
                  dW(1),
                  prop,
                  DebuggingTactics.done("Proving C1 diff. refine"),
                ),
              ),
              DebuggingTactics.print("Step done"),
              DebuggingTactics.done("Step"),
            ),
            // show
            justifyFout(fout, plant2Vars, c2ComGuaranteeLiveness, c2use, c2step)(Symbol("Rlast")),
          ),
          DebuggingTactics.print("Done disassembling system into components"),
          DebuggingTactics.done("Disassembling system into components"),
        )
      case Some(e) => throw new TacticInapplicableFailure(
          "proveSystemCompStep only applicable to box properties, but got " + e.prettyString
        )
      case None => throw new IllFormedTacticApplicationException(
          "Position " + pos + " does not point to a valid position in sequent " + seq.prettyString
        )
    ...
    seq.sub(pos) match {
      case Some(
              ...
              Compose(
                ...
                Compose(
                  ...
                  Compose(
                    rememberStart @ Assign(told, t),
                    Compose(
                      plant3 @ ODESystem(DifferentialProduct(_, _), q3),
                      Compose(in3, Compose(cp1, Compose(cp2, con))),
                    ),
                  ),
                ),
              ),
              inv3 @ And(inv1, And(inv2, zeta)),
            )
          ) =>
        ...
        SeqTactic(
          ...
          BranchTactic(
            ...
            SeqTactic(
              ...
              proveSystemCompStep(
                ...
                c2step,
                compatibility,
                comGuaranteeLiveness,
                Imply(inv2, Box(delta3, Box(ctrl3, Box(rememberStart, Box(plant3, pi2Out))))),
              )(pos),
              DebuggingTactics.print("Done proving inv1"),
              DebuggingTactics.done("Proving inv1"),
            ),
            SeqTactic(
              boxAnd(1),
              andR(1),
              BranchTactic(
                ...
                SeqTactic(
                  ...
                  proveSystemCompStep(
                    ...
                    compatibility,
                    comGuaranteeLiveness,
                    Imply(inv1, Box(delta3, Box(ctrl3, Box(rememberStart, Box(plant3, pi1Out))))),
                  )(pos),
                  DebuggingTactics.print("Done proving inv2"),
                  DebuggingTactics.done("Proving inv2"),
                ),
                // zeta
                SeqTactic(
                  ...
                  (cr1.head to cr2.last)
                    .map(i => PosInExpr(List.fill(i)(1)))
                    .reverse
                    .map(abstractionb(1, _))
                    .reduce[BelleExpr](_ & _),
                  SaturateTactic(allR(1)),
                  cohideR(1),
                  by(comGuaranteeSafety),
          ...
          DebuggingTactics.print("Done proving component loops and communication guarantees"),
          DebuggingTactics.done("Proving component loops and communication guarantees"),
        )
      case Some(e) => throw new TacticInapplicableFailure(
          "proveSystemStep only applicable to box properties, but got " + e.prettyString
        )
      case None => throw new IllFormedTacticApplicationException(
          "Position " + pos + " does not point to a valid position in sequent " + seq.prettyString
        )
  ...
  private def proveSystem(
    ...
    seq.sub(pos) match {
      case Some(
            Imply(And(timeInit, And(globalFacts, And(init1, And(init2, zeta)))), Box(Loop(sys), And(post1, post2)))
          ) =>
        ...
        SeqTactic(
          ...
          BranchTactic(
            SeqTactic(
              ...
              BranchTactic(
                SeqTactic(
                  DebuggingTactics.print("Close by C1 base case lemma"),
                  useLemma(c1base, Some(prop)),
                  DebuggingTactics.done("C1Base"),
                ),
                SeqTactic(
                  andR(1),
                  BranchTactic(
                    SeqTactic(
                      DebuggingTactics.print("Close by C2 base case lemma"),
                      useLemma(c2base, Some(prop)),
                      DebuggingTactics.done("C2Base"),
                    ),
            ...
            SeqTactic(
              ...
              BranchTactic(
                SeqTactic(
                  DebuggingTactics.print("Close by C1 use case lemma"),
                  useLemma(c1use, Some(prop)),
                  DebuggingTactics.done("C1Use"),
                ),
                SeqTactic(
                  DebuggingTactics.print("Close by C2 use case lemma"),
                  useLemma(c2use, Some(prop)),
                  DebuggingTactics.done("C2Use"),
                ),
              ),
            ...
            SeqTactic(
              ...
              DebuggingTactics.done("Component system step", Some("user/" + systemName + " Step")),
            ),
          ),
        )
      case Some(e) =>
        throw new TacticInapplicableFailure("proveSystem only applicable to box properties, but got " + e.prettyString)
      case None => throw new IllFormedTacticApplicationException(
          "Position " + pos + " does not point to a valid position in sequent " + seq.prettyString
        )
    }
  ...
  private def checkComponentLemmas(cbase: Lemma, cuse: Lemma, cstep: Lemma): Unit = {
    ...
    if ((FormulaTools.conjuncts(invbase).toSet -- FormulaTools.conjuncts(invuse)).nonEmpty)
      throw new InputFormatFailure(
        "Component invariants in base case and use case do not match: please provide lemmas A1->I, I -> G1, I->[c]I\n" +
          invbase.prettyString + " <> " + invuse.prettyString
      )
    if ((FormulaTools.conjuncts(invstepb).toSet -- FormulaTools.conjuncts(invbase)).nonEmpty)
      throw new InputFormatFailure(
        "Component invariants in base case and step do not match: please provide lemmas A1->I, I -> G1, I->[c]I\n" +
          invbase.prettyString + " <> " + invstepa.prettyString
      )
    if ((FormulaTools.conjuncts(invstepb).toSet -- FormulaTools.conjuncts(invstepa)).nonEmpty)
      throw new InputFormatFailure(
        "Component invariants in step do not match: please provide lemmas A1->I, I -> G1, I->[c]I\n" +
          invbase.prettyString + " <> " + invstepa.prettyString
      )
    ...
    ) throw new InputFormatFailure("Component use case makes additional non-global assumptions")
    if (
      StaticSemantics
        .freeVars(
        ...
        .toSet
        .exists(_.isInstanceOf[Variable])
    ) throw new InputFormatFailure("Component step makes additional non-global assumptions")
  }
  ...
  private def checkPrograms(
    ...
    if (mem._1 != mem._2)
      throw new InputFormatFailure("System and component port memories must agree, but " + mem._1 + " <> " + mem._2)
    if (ctrl._1 != ctrl._2)
      throw new InputFormatFailure("System and component controllers must agree, but " + ctrl._1 + " <> " + ctrl._2)
    if (t._1 != t._2)
      throw new InputFormatFailure("System and component time memory must agree, but " + t._1 + " <> " + t._2)
    if (!DifferentialHelper.atomicOdes(plant._2).toSet.subsetOf(DifferentialHelper.atomicOdes(plant._1).toSet))
      throw new InputFormatFailure(
        "System plant must contain component plant, but " + plant._1.prettyString + " does not contain all of " + plant
          ._2
      )
    if (!ports(in._1).toSet.subsetOf(ports(in._2).toSet)) throw new InputFormatFailure(
      "System input ports must be subset of component input ports, but " +
    ...
    if (cp._1 != cp._2) throw new InputFormatFailure(
      "System and component internal connections must agree, but " + cp._1 + " <> " + cp._2
    )
