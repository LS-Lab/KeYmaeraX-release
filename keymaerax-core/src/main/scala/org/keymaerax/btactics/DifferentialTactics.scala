...
import org.keymaerax.core.{NamedSymbol, *}
...
import org.keymaerax.parser.{Declaration, InterpretedSymbols, Name, TacticReservedSymbols}
...
private object DifferentialTactics extends Logging {
        ...
        case Some(Box(ODESystem(DifferentialProduct(AtomicODE(d @ DifferentialSymbol(x), t), c), h), p)) =>
          ...
          cutLR(g)(pos) < (
            ...
            /* show */ cohide(Symbol("Rlast")) & equivifyR(1) & commuteEquivR(1) &
              UnifyUSCalculus.US(
                subst,
                UnifyUSCalculus.uniformRenameF(aX, x)(AxiomInfo("DE differential effect (system)").provable),
              ),
          )
        ...
        if (pos.isTopLevel) {
          val t = expand.getOrElse(skip) & DI(pos) &
            ...
            implyR(pos) & andR(Symbol("Rlast")) & Idioms.<(
              if (auto == Symbol("full")) ToolTactics.hideNonFOL & (abbrvPrimes(
                Symbol("Rlast")
              ) & QE & done | DebuggingTactics.done("Differential invariant must hold in the beginning"))
                  ...
                  (
                    if (auto == Symbol("full") || auto == Symbol("cex")) {
                      TryCatch(
                        ...
                        (if (auto == Symbol("full")) abbrvPrimes(Symbol("Rlast")) & QE & done | DebuggingTactics.done(
                           "Differential invariant must be preserved"
                         )
  ...
  private[btactics] def diffCut(formulas: List[Formula]): DependentPositionWithAppliedInputTactic = inputanon {
  ...
  def diffRefine(R: Formula): DependentPositionWithAppliedInputTactic =
    "dR".byWithInputs(List(R), (pos: Position, sequent: Sequent) => diffRefineInternal(R, hide = true)(pos, sequent))

  @Derivation
  val diffRefineInfo: InputPositionTacticInfo = InputPositionTacticInfo.create(
    name = "dR",
    ...
    constructor = TacticConstructor1.create(FormulaArg("R"))(diffRefine),
  )
    ...
    (pos: Position, sequent: Sequent) =>
      ...
      if (R.size == 1) {
        DifferentialEquationCalculus.dC(R.head)(pos) < (
          skip,
          DifferentialEquationCalculus.dIX(SuccPosition.base0(sequent.succ.size - 1, pos.inExpr)) & OnAll(
            QE & done
          ) & done,
        )
      } else {
   ...
   */
  // todo: rename the tactic directly
  val inverseDiffCut: DependentPositionTactic = "dCi".by { (pos: Position, s: Sequent) =>
  ...
  @Derivation
  val inverseDiffCutInfo: PositionTacticInfo = PositionTacticInfo.create(
    name = "dCi",
    displayNameLong = Some("Inverse Differential Cut"),
    displayLevel = DisplayLevel.Browse,
    displayPremises = "Γ |- [x'=f(x) & Q]P ;; Γ |- R, Δ",
    displayConclusion = "Γ |- [x'=f(x) & Q∧R]P, Δ",
    constructor = TacticConstructor0.create()(() => inverseDiffCut),
  )
   ...
   * }}}
   */
  val inverseDiffGhost: DependentPositionTactic = "dGi".by { (pos: Position, s: Sequent) =>
    val polarity = (if (pos.isSucc) 1 else -1) * FormulaTools.polarityAt(s(pos.top), pos.inExpr)
    s.sub(pos) match {
  ...
  @Derivation
  val inverseDiffGhostInfo: PositionTacticInfo = PositionTacticInfo.create(
    name = "dGi",
    displayName = Some("Inverse Differential Ghost"),
    displayLevel = DisplayLevel.Browse,
    displayPremises = "Γ |- [x'=f(x) & Q]P, Δ",
    displayConclusion = "Γ |- ∃y [x'=f(x),E & Q]P, Δ",
    constructor = TacticConstructor0.create()(() => inverseDiffGhost),
  )
  ...
  private[btactics] lazy val Dvariable: BuiltInPositionTactic = "Dvariable".by { (pr: ProvableSig, pos: Position) =>
  ...
  @Derivation
  private[btactics] val DvariableInfo: PositionTacticInfo = PositionTacticInfo.create(
    name = "Dvariable",
    displayName = Some("x'"),
    displayConclusion = "__(x)'__ = x",
    constructor = TacticConstructor0.create()(() => Dvariable),
  )
   ...
   * }}}
   */
  lazy val diffUnpackEvolutionDomainInitially: DependentPositionTactic = "diffUnpackEvolDomain".by {
    (pos: Position, sequent: Sequent) =>
      sequent.sub(pos) match {
        case Some(Box(ODESystem(_, q), _)) =>
          ...
          cut(q) < (skip, useAt(Ax.DWQinitial, PosInExpr(1 :: Nil))(pos) & implyR(pos) & close)
        case Some(e) => throw new TacticInapplicableFailure(
  ...
  @Derivation
  val diffUnpackEvolutionDomainInitiallyInfo: PositionTacticInfo = PositionTacticInfo.create(
    name = "diffUnpackEvolDomain",
    displayName = Some("Unpack evolution domain"),
    displayLevel = DisplayLevel.Browse,
    displayPremises = "Γ, Q |- [x'=f(x) & Q]P, Δ",
    displayConclusion = "Γ |- [x'=f(x) & Q]P, Δ",
    constructor = TacticConstructor0.create()(() => diffUnpackEvolutionDomainInitially),
  )

  /** [[DifferentialEquationCalculus.dW]]. diffWeaken by diffCut(consts) <(diffWeakenG, V&close) */
  // todo: rename the tactic directly
  private[btactics] lazy val diffWeaken: DependentPositionTactic = "dW".by { (pos: Position, sequent: Sequent) =>
  ...
  @Derivation
  private[btactics] val diffWeakenInfo: PositionTacticInfo = PositionTacticInfo.create(
    name = "dW",
    ...
    constructor = TacticConstructor0.create()(() => diffWeaken),
  )
   ...
   */
  // todo: rename the tactic directly
  private[btactics] lazy val diffWeakenPlus: DependentPositionTactic = "dWplus".by {
    (pos: Position, sequent: Sequent) =>
      if (pos.isAnte) { throw new TacticInapplicableFailure("dW+ only in succedent") }
      else if (!pos.isTopLevel) { DW(pos) & abstractionb(pos) }
      else sequent.sub(pos) match {
        case Some(box @ Box(a: ODESystem, p)) =>
          ...
          val cutAndDW = anon((seq: Sequent) => {
            ...
            val dw = diffWeakenG(Symbol("R"), odeAfterCut) & implyR(1) & andL(Symbol("Llast")) * cuts.size & notL(
              Symbol("Llast")
            ) * succCuts.size &
            ...
            if (cuts.isEmpty) dw
              ...
              skip,
              V(Symbol("Rlast")) &
                (if (anteCuts.nonEmpty)
                   (andR(Symbol("Rlast")) < (closeIdWith(Symbol("Rlast")) & done, skip)) * (anteCuts.size - 1) &
  ...
  @Derivation
  private[btactics] val diffWeakenPlusInfo: PositionTacticInfo = PositionTacticInfo.create(
    name = "dWplus",
    displayName = Some("dW+"),
    displayNameAscii = Some("dWplus"),
    displayNameLong = Some("Differential Weaken"),
    displayLevel = DisplayLevel.Browse,
    ...
    displayContextConclusion = "Γ |- C( [x'=f(x) & Q]P ), Δ",
    revealInternalSteps = true,
    constructor = TacticConstructor0.create()(() => diffWeakenPlus),
  )
  ...
  /**
   * Invariance check
   * @return Returns True if it determines that the only possibilty is for postcondition to be invariant at the position
   *   it is called (either a loop invariant or ODE invariant) This can be used to prevent (unnecessary) invariant
   *   generation for loops or ODEs from happening Return False in all other cases (including when the sequent or
   *   position are not of the expected shape)
   */
   ...
   * @return Leaves False as the only subgoal if it finds a counterexample to the ODE question at the position it is
   *   called Succeeds in all other cases (including when the sequent or position are not of the expected shape)
   */
  ...
  @Derivation
  val cexODEInfo: PositionTacticInfo = PositionTacticInfo.create(
    name = "cexODE",
    displayLevel = DisplayLevel.Internal,
    constructor = TacticConstructor0.create()(() => cexODE),
  )
    ...
    anon((pos: Position, seq: Sequent, defs: Declaration) => {
      val invariantCandidates =
        try { InvariantGenerator.differentialInvariantGenerator.generate(seq, pos, defs) }
          ...
          case err: Exception =>
            logger.warn(
              s"Failed to produce a proof for this ODE. Underlying cause: ChooseSome: error listing options $err"
            )
            LazyList[Invariant]()
        }
      ...
      val insistOnProof =
        pos
          .isTopLevel // @todo come up wtih better heuristic for determining when to insist on a proof being completed. Solvability is a pretty decent heuristic.
  ...
  lazy val mathematicaODE: DependentPositionTactic = anon((pos: Position, seq: Sequent, defs: Declaration) => {
    ...
    ): DependentPositionTactic = fastODE(try {
      generator.generate(seq, pos, defs).iterator
    } catch {
      case ex: Exception =>
   ...
   * @author Nathan Fulton
   */
  val splitWeakInequality: DependentPositionTactic = "splitWeakInequality".by { (pos: Position, seq: Sequent) =>
  ...
  @Derivation
  val splitWeakInequalityInfo: PositionTacticInfo = PositionTacticInfo.create(
    ...
    constructor = TacticConstructor0.create()(() => splitWeakInequality),
  )
  ...
  def dgDbx(qco: Term): DependentPositionWithAppliedInputTactic = inputanon((pos: Position, seq: Sequent) => {
    ...
    val (p, pop) = property match {
      case bop: ComparisonFormula if bop.right.isInstanceOf[Number] && bop.right.asInstanceOf[Number].value == 0 =>
        (bop.left, bop)
      case e => throw new TacticInapplicableFailure(s"Not sure what to do with shape ${e
            .prettyString}, dgDbx requires 0 on RHS")
    }
        ...
        diffCut(pcy)(pos) < (
          ...
          diffInd(Symbol("diffInd"))(pos) < (
            ...
            cohideOnlyL(Symbol("Llast")) & andL(-1) &
              ...
              implyRi()(AntePos(0), SuccPos(0)) & inspectAndCut < (QE, byUS(barrierCond2)),
          ),
        ),
  ...
  val dgBarrier: DependentPositionTactic = "barrier".by { (pos: Position, _: Sequent) =>
    Dconstify(dgBarrierAux(pos))(pos)
  }

  @Derivation
  val dgBarrierInfo: PositionTacticInfo = PositionTacticInfo.create(
    ...
    constructor = TacticConstructor0.create()(() => dgBarrier),
  )
   ...
   * }}}
   *
   * @param tryHard configures how hard the tactic tries to prove invariance in particular use tryHard = true when speed
   *   is secondary & certain that P is invariant use tryHard = false when speed is of interest e.g., within automated
   *   invariant search
   */
  ...
  private def interiorImplication: DependentTactic = anon { (seq: Sequent) =>
    ...
    (seq.ante(0), seq.succ(0)) match {
      case (_: And, _: And) =>
        andL(-1) & andR(1) & Idioms.<(hideL(-2) & interiorImplication, hideL(-1) & interiorImplication)
  ...
  val dCClosure: DependentPositionTactic = "dCClosure".by { (pos: Position) => dCClosure(true)(pos) }

  @Derivation
  val dCClosureInfo: PositionTacticInfo = PositionTacticInfo.create(
    name = "dCClosure",
    ...
    displayConclusion = "Γ |- [x'=f(x)&Q]P, Δ",
    revealInternalSteps = true,
    constructor = TacticConstructor0.create()(() => dCClosure),
  )

  val dIClosed: DependentPositionTactic = "dIClosed".by { (pos: Position, seq: Sequent) =>
    seq.sub(pos) match {
      case Some(Box((sys @ (_: ODESystem)), _)) => ODESpecific(sys.ode).dIClosed(pos)
      case _ => throw new TacticRequirementError("Did not match form for dI closed.")
    }
  }

  @Derivation
  val dIClosedInfo: PositionTacticInfo = PositionTacticInfo.create(
    name = "dIClosed",
    ...
    revealInternalSteps = true,
    constructor = TacticConstructor0.create()(() => dIClosed),
  )

  // TODO how to write description in a general way for nth order polynomial.
  val taylorB: DependentPositionTactic = "taylorB".by { (pos: Position, seq: Sequent) =>
    pos.checkTop
    seq.sub(pos) match {
  ...
  @Derivation
  val taylorBInfo: PositionTacticInfo = PositionTacticInfo.create(
    name = "taylorB",
    ...
    revealInternalSteps = true,
    constructor = TacticConstructor0.create()(() => taylorB),
  )

  // TODO how to write description in a general way for nth order polynomial.
  val taylorStep: DependentPositionTactic = "taylorStep".by { (pos: Position, seq: Sequent) =>
    pos.checkTop
    seq.sub(pos) match {
      case Some(Box(ODESystem(ode, _), post)) => {
        // Take Lie derivative.
        ...
        // General case: cut next order derivative. For use branch apply dI. For show, recurse.
          ...
          diffCut(postDerivative)(pos) < (dIRule(pos) < (QE, SaturateTactic(Dassignb(pos)) & QE), fullSimplify)
        )(pos)
      }
      case Some(e) =>
  ...
  @Derivation
  val taylorStepInfo: PositionTacticInfo = PositionTacticInfo.create(
    name = "taylorStep",
    ...
    displayConclusion = "Γ |- x=x_0 -> [x'=f(x), t'=1 & Q]q(x)>=q(x_0)+q'(x_0).t",
    revealInternalSteps = true,
    constructor = TacticConstructor0.create()(() => taylorStep),
  )
  ...
  case class ODESpecific(ode: DifferentialProgram, variant: String => String = _ + "_") {
    ...
    val dIopenClosedProvable: ProvableSig = proveBy(
      ...
      implyR(1) & andL(-1) & andL(Symbol("Llast")) & andL(Symbol("Llast")) &
        ...
        HybridProgramCalculus.generalize(nonneg(
          p(vars)
        ))(1) & Idioms.<(skip, andL(-1) & FOQuantifierTactics.allLs(vars)(Symbol("Llast")) & prop & done) &
                      ...
                      dR(And(r(vars), nonneg(q(vars))), hide = false)(1) & Idioms.<(
                        ...
                        andL(Symbol("L")) &
                          HybridProgramCalculus.generalize(P(vars))(1) & Idioms.<(
                            skip,
                            andL(-1) & FOQuantifierTactics.allLs(vars)(Symbol("Llast")) & prop & done,
                          ) &
    ...
    val dIClosed: DependentPositionTactic = anon { (pos: Position, seq: Sequent) =>
      ...
      seq.sub(pos) match {
        case Some(Box(ODESystem(ode, _), post)) =>
          import TaylorModelTactics.Timing.*
          toc("== dIClosed")
