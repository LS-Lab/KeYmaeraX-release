...
object DebuggingTactics extends TacticProvider {
  ...
  /**
   * debug is a no-op tactic that prints a message and the current provable, if doPrint (defaults to the system property
   * DEBUG) is true.
   */
  ...
  /**
   * printIndexed is a no-op tactic that prints a message and the current provable (verbose sequent with formula
  ...
  def assertAt(
      ...
      assertion: Expression => Boolean,
      ex: (=> String) => Throwable = new TacticAssertionError(_),
  ): BuiltInPositionTactic = new BuiltInPositionTactic(ANON) with NoOpTactic {
    override def computeResult(provable: ProvableSig, pos: Position): ProvableSig = {
  ...
  def assertAt(msg: => String, assertion: Expression => Boolean, ex: (=> String) => Throwable): BuiltInPositionTactic =
    assertAt((_: Expression) => msg, assertion, ex)

  /** assert is a no-op tactic that raises an error if the provable is not of the expected size. */
  def assert(anteSize: Int, succSize: Int, msg: => String, ex: (=> String) => Throwable): BuiltInTactic =
    new BuiltInTactic(ANON) with NoOpTactic {
      override def result(provable: ProvableSig): ProvableSig = {
        if (
          provable.subgoals.size != 1 || provable.subgoals.head.ante.size != anteSize ||
          provable.subgoals.head.succ.size != succSize
        ) {
          throw ex(
            msg + "\nExpected 1 subgoal with: " + anteSize + " antecedent and " + succSize +
              " succedent formulas,\n\t but got " + provable.subgoals.size + " subgoals (head subgoal with: " +
              provable.subgoals.head.ante.size + "antecedent and " + provable.subgoals.head.succ.size +
              " succedent formulas)"
          )
        }
        provable
  ...
  def assert(anteSize: Int, succSize: Int, msg: => String = ""): BuiltInTactic =
    assert(anteSize, succSize, msg, new TacticAssertionError(_))

  // @todo rename to something else otherwise scala assert no longer works!
  /**
   * assert is a no-op tactic that raises an error if the provable has not the expected formula at the specified
   * position.
   */
  def assert(
      fml: Formula,
              ...
              if (provable.subgoals.size != 1 || provable.subgoals.head.at(pos)._2 != fml) {
                throw ex(
                  message + "\nExpected 1 subgoal with " + fml + " at position " + pos + ",\n\t but got " +
                    provable.subgoals.size + " subgoals (head subgoal with " + provable.subgoals.head.sub(pos) +
                    " at position " + pos + ")"
                )
              }
              provable
  ...
  /**
   * assert is a no-op tactic that raises a tactic assertion error if the provable has not the expected formula at the
   * specified position.
   */
  def assert(fml: Formula, message: => String): DependentPositionWithAppliedInputTactic =
    assert(fml, message, new TacticAssertionError(_))

  /** assert is a no-op tactic that raises an error if the provable does not satisfy a condition on the sole subgoal. */
  def assert(cond: Sequent => Boolean, message: => String, ex: (=> String) => Throwable): BuiltInTactic =
    new BuiltInTactic(ANON) with NoOpTactic {
      override def result(provable: ProvableSig): ProvableSig = {
  ...
  /**
   * assert is a no-op tactic that raises a tactic assertion error if the provable does not satisfy a condition on the
   * sole subgoal.
   */
  def assert(cond: Sequent => Boolean, message: => String): BuiltInTactic =
    assert(cond, message, new TacticAssertionError(_))

  /** assertOnAll is a no-op tactic that raises an error the provable does not satisfy a condition on all subgoals. */
  def assertOnAll(
      cond: Sequent => Boolean,
      message: => String,
      ex: (=> String) => Throwable = new TacticAssertionError(_),
  ): BuiltInTactic = new BuiltInTactic(ANON) with NoOpTactic {
    override def result(provable: ProvableSig): ProvableSig = {
  ...
  def assertProvableSize(provableSize: Int, ex: (=> String) => Throwable = new TacticAssertionError(_)): BuiltInTactic =
    new BuiltInTactic(ANON) with NoOpTactic {
      override def result(provable: ProvableSig): ProvableSig = {
  ...
  /**
   * assert is a no-op tactic that raises a tactic assertion error if the provable does not satisfy a condition at
   * position pos.
   */
  def assert(cond: (Sequent, Position) => Boolean, message: => String): BuiltInPositionTactic =
    assert(cond, message, new TacticAssertionError(_))

  /**
   * assertE is a no-op tactic that raises an error if the provable has not the expected expression at the specified
   * position.
   */
  def assertE(
      expected: => Expression,
              ...
              if (provable.subgoals.size != 1 || provable.subgoals.head.at(pos)._2 != expected) {
                throw ex(
                  message + "\nExpected 1 subgoal with " + expected + " at position " + pos + ",\n\t but got " +
                    provable.subgoals.size + " subgoals (head subgoal with " + provable.subgoals.head.at(pos) +
                    " at position " + pos + ")"
                )
              }
              provable
  ...
  /**
   * assertE is a no-op tactic that raises a tactic assertion error if the provable has not the expected expression at
  ...
  private def doneImpl(msg: String = "", storeLemma: Option[String] = None): StringInputTactic = new StringInputTactic(
    ...
    override def result(provable: ProvableSig): ProvableSig = {
      if (provable.isProved) {
        ...
        if (storeLemma.isDefined) LemmaDBFactory
          .lemmaDB
          .add(Lemma(provable, Lemma.requiredEvidence(provable), storeLemma))
        provable
...
object Idioms {
  ...
  def saturate(t: ProvableSig => ProvableSig): ProvableSig => ProvableSig = (pr: ProvableSig) => {
    val r = pr(opt(t))
    if (r != pr) r(saturate(t)) else r
  }

  /** Executes tactic `t` `n` times. */
  def times(t: ProvableSig => ProvableSig, n: Int): ProvableSig => ProvableSig =
    (pr: ProvableSig) => { if (n > 0) times(t, n - 1)(pr(t)) else pr }
  ...
  def cases(
    ...
    val simplifyAllButCase = (fml: Formula) =>
      ANON by { (seq: Sequent) =>
        (0 until seq.ante.length - 1)
          .map(i => simplify(fml)(AntePosition.base0(i)))
          .reduceOption[BelleExpr](_ & _)
          .getOrElse(ident) &
          seq
            .succ
            .indices
            .map(i => simplify(fml)(SuccPosition.base0(i)))
            .reduceOption[BelleExpr](_ & _)
            .getOrElse(ident)
      }
   ...
   * `lemmaName`. Must be used on a provable with exactly 1 subgoal (e.g., as created in a Case). Lemma lookup can be
   * disabled with `doLemmaLookup` (lemma lookup enabled by default).
   */
  def rememberAs(lemmaName: String, t: BelleExpr)(implicit
      lemmaDB: LemmaDB,
      lookupRememberedLemmas: Boolean = true,
  ): BelleExpr = new BuiltInTactic("RememberAs") {
    override def result(provable: ProvableSig): ProvableSig = {
      if (provable.subgoals.size > 1)
  ...
  def repeatWhile(condition: Expression => Boolean)(t: BelleExpr): DependentPositionTactic = anon { (pos: Position) =>
    SaturateTactic(
      DebuggingTactics.assertAt((_: Expression) => "Stopping loop", condition, new TacticInapplicableFailure(_))(pos) &
        t
    )
  }
  ...
  def doIfFw(condition: ProvableSig => Boolean)(t: => ProvableSig => ProvableSig): BuiltInTactic =
    doIfElseFw(condition)(t, nil.result)

  /** Executes t if condition is true and f otherwise. */
  def doIfElse(condition: ProvableSig => Boolean)(t: => BelleExpr, f: => BelleExpr): DependentTactic =
    new DependentTactic(ANON) {
      override def computeExpr(provable: ProvableSig): BelleExpr = { if (condition(provable)) t else f }
    }
  def doIfElseFw(
      condition: ProvableSig => Boolean
  )(t: => ProvableSig => ProvableSig, f: => ProvableSig => ProvableSig): BuiltInTactic = new BuiltInTactic(ANON) {
    override def result(provable: ProvableSig): ProvableSig = { if (condition(provable)) t(provable) else f(provable) }
  }

  /** must(t) runs tactic `t` but only if `t` actually changed the goal. */
  def must(t: BelleExpr, msg: Option[String] = None): BelleExpr = anons { (before: ProvableSig) =>
    t & anonnoop { (after: ProvableSig) =>
      {
        if (before == after)
          throw new BelleNoProgress(msg.getOrElse("Tactic " + t + " did not result in mandatory change"))
        after
      }
    }
  }
  ...
  def mapSubpositions[T](pos: Position, sequent: Sequent, trafo: (Expression, Position) => Option[T]): List[T] = {
    ...
    sequent.sub(pos) match {
      case Some(f: Formula) => ExpressionTraversal.traverse(
      ...
      case Some(t: Term) => ExpressionTraversal.traverse(
          new ExpressionTraversalFunction() {
            override def preT(p: PosInExpr, t: Term): Either[Option[StopTraversal], Term] = mapTerm(p, t)
          },
          t,
      ...
      val subPos: Position = {
        ...
        if (ante >= 0) AntePosition.base0(ante, in)
          ...
          if (succ >= 0) SuccPosition.base0(succ, in)
            ...
            "Cannot find formula " + f.prettyString + " in sequent " + seq.prettyString
          )
        }
      }
      t(subPos)
...
object TacticFactory {
  ...
  implicit class TacticForNameFactory(val name: String) extends Logging {
    ...
    def forward(t: BuiltInLeftTactic): BuiltInLeftTactic =
      by((pr: ProvableSig, pos: AntePosition) => t.computeResult(pr, pos))
    def forward(t: BuiltInRightTactic): BuiltInRightTactic =
      by((pr: ProvableSig, pos: SuccPosition) => t.computeResult(pr, pos))
    def forward(t: CoreLeftTactic): CoreLeftTactic =
      coreby((pr: ProvableSig, pos: AntePosition) => t.computeResult(pr, pos))
    def forward(t: CoreRightTactic): CoreRightTactic =
      coreby((pr: ProvableSig, pos: SuccPosition) => t.computeResult(pr, pos))
    ...
    def forward(t: DependentPositionWithAppliedInputTactic): DependentPositionWithAppliedInputTactic =
      byWithInputs(t.inputs, (pos: Position, _: Sequent) => t(pos))
    def forward(t: BuiltInTwoPositionTactic): BuiltInTwoPositionTactic =
      by((ps: ProvableSig, posOne: Position, posTwo: Position) => t.computeResult(ps, posOne, posTwo))
    // @TODO: implement
    ...
    def byWithInputs(inputs: Seq[Any], t: (Position, Sequent) => BelleExpr): DependentPositionWithAppliedInputTactic =
    ...
    def byWithInputs(
        inputs: Seq[Any],
        t: (Position, Sequent, Declaration) => BelleExpr,
    ): DependentPositionWithAppliedInputTactic = new DependentPositionWithAppliedInputTactic(name, inputs) {
      override def factory(pos: Position): DependentTactic = new SingleGoalDependentTactic(this.name) {
        override def computeExpr(sequent: Sequent, defs: Declaration): BelleExpr = {
          require(pos.isIndexDefined(sequent), "Cannot apply at undefined position " + pos + " in sequent " + sequent)
          t(pos, sequent, defs)
    ...
    /** A position tactic with multiple inputs. */
    def byWithInputs(inputs: Seq[Any], t: Position => BelleExpr): DependentPositionWithAppliedInputTactic =
      new DependentPositionWithAppliedInputTactic(name, inputs) {
        override def factory(pos: Position): DependentTactic = new SingleGoalDependentTactic(this.name) {
          override def computeExpr(sequent: Sequent): BelleExpr = {
            require(pos.isIndexDefined(sequent), "Cannot apply at undefined position " + pos + " in sequent " + sequent)
    ...
    def byWithInputs(input: Seq[Any], t: Sequent => BelleExpr): InputTactic = byWithInputs(
      input,
      new SingleGoalDependentTactic(name) {
        override def computeExpr(sequent: Sequent): BelleExpr = t(sequent)
      },
    )
    def byWithInputsS(input: Seq[String], t: ProvableSig => ProvableSig): StringInputTactic =
      new StringInputTactic(name, input) {
        override def result(ps: ProvableSig): ProvableSig = t(ps)
      }
    def byWithInputsP(input: Seq[Any], t: ProvableSig => ProvableSig): InputTactic = byWithInputs(
      input,
    ...
    /**
     * Creates a CoreRightTactic from a function turning provables and succedent positions into new provables. Unlike
     * [[by]], the coreby will augment MatchErrors from the kernel into readable error messages.
     * @example
     *   {{{
    ...
    /**
     * Creates a CoreRightTactic with applied inputs from a function turning provables and succedent positions into new
     * provables. Unlike [[by]], the coreby will augment MatchErrors from the kernel into readable error messages.
     * @example
     *   {{{
     *   "andR" coreby((pr,pos)=> pr(AndRight(pos.top),0))
     *   }}}
    ...
    ): DependentPositionWithAppliedInputTactic = byWithInputs(
      inputs,
      (pos: Position, _: Sequent) => {
        new BuiltInTactic(name) {
          override def result(provable: ProvableSig): ProvableSig = { t(provable, pos.checkSucc) }
        }
      },
    )

    /**
     * Creates a CoreLeftTactic with applied inputs from a function turning provables and antecedent positions into new
     * provables. Unlike [[by]], the coreby will augment MatchErrors from the kernel into readable error messages.
     * @example
     *   {{{
    ...
    def corebyWithInputsL(
        inputs: Seq[Any],
        t: (ProvableSig, AntePosition) => ProvableSig,
    ): DependentPositionWithAppliedInputTactic = byWithInputs(
      inputs,
      (pos: Position, _: Sequent) => {
        new BuiltInTactic(name) {
          override def result(provable: ProvableSig): ProvableSig = { t(provable, pos.checkAnte) }
        }
      },
    )

    /**
     * Creates a Tactic with applied inputs from a function turning provables and antecedent positions into new
     * provables.
     */
    def byWithInputsP(
        inputs: Seq[Any],
        t: (ProvableSig, Position) => ProvableSig,
    ): DependentPositionWithAppliedInputTactic = byWithInputs(
      inputs,
      (pos: Position, _: Sequent) => {
        new BuiltInTactic(name) {
          override def result(provable: ProvableSig): ProvableSig = { t(provable, pos) }
        }
      },
    )
    ...
    def by(t: ProvableSig => ProvableSig): BuiltInTactic = new BuiltInTactic(name) {
      @inline
      override def result(provable: ProvableSig): ProvableSig = { t(provable) }
    }

    /** Creates a BuiltInTactic from a function turning provables and antecedent positions into new provables. */
    def by(t: (ProvableSig, Position) => ProvableSig): BuiltInPositionTactic = new BuiltInPositionTactic(name) {
      @inline
      override def computeResult(provable: ProvableSig, pos: Position): ProvableSig = {
        requireOneSubgoal(provable, this.name)
        t(provable, pos)
    ...
    /**
     * Creates a BuiltInLeftTactic from a function turning provables and antecedent positions into new provables. Unlike
     * [[by]], the coreby will augment MatchErrors from the kernel into readable error messages.
     * @example
     *   {{{
     *   "andL" coreby((pr,pos)=> pr(AndLeft(pos.top),0))
     ...
     *   }}}
     */
    def by(t: (ProvableSig, Position, Position) => ProvableSig): BuiltInTwoPositionTactic =
      new BuiltInTwoPositionTactic(name) {
        @inline
  ...
  def corelabelledby[S <: Expression, T <: Expression](
      ...
      unapply: S => Option[(T, T)],
      pos: Position,
      seq: Sequent,
      labels: (T, T) => (String, String) =
        (s: T, t: T) => (KeYmaeraXOmitInterpretationPrettyPrinter(s), KeYmaeraXOmitInterpretationPrettyPrinter(t)),
  ): BelleExpr =
    if (pos.isTopLevel) {
      try {
        seq.sub(pos.checkTop).flatMap(f => unapply(f.asInstanceOf[S])) match {
          case Some((l, r)) =>
            ...
            val (lLabel, rLabel) =
              if (lText == rText)
                (BelleSubLabel(BelleTopLevelLabel("L"), lText), BelleSubLabel(BelleTopLevelLabel("R"), rText))
            ...
            }) < (LabelBranch(lLabel), LabelBranch(rLabel))
        }
      } catch {
        case ex: ClassCastException => throw new TacticInapplicableFailure(
            "Tactic " + name + " applied at " + pos + " on a non-matching expression in " + seq.prettyString,
            ex,
          )
