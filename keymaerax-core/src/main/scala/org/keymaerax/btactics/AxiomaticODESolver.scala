...
object AxiomaticODESolver {
  ...
  private lazy val postCondSimplifier = SimplifierV3.simpTac(
    Nil,
  ...
  private def pushNegation: DependentPositionTactic = anon((pos: Position, s: Sequent) => {
    s.sub(pos) match {
      ...
      case Some(Not(Forall(x, p))) if StaticSemantics.boundVars(p).intersect(x.toSet).isEmpty =>
        useAt(Ax.notAll)(pos) & pushNegation(pos ++ PosInExpr(0 :: Nil))
      case Some(Not(Forall(x, p))) if !StaticSemantics.boundVars(p).intersect(x.toSet).isEmpty =>
        SeqTactic(
      ...
      case Some(Not(Exists(x, p))) if StaticSemantics.boundVars(p).intersect(x.toSet).isEmpty =>
        useAt(Ax.notExists)(pos) & pushNegation(pos ++ PosInExpr(0 :: Nil))
      case Some(Not(Exists(x, p))) if !StaticSemantics.boundVars(p).intersect(x.toSet).isEmpty =>
        SeqTactic(
          x.map(DLBySubst.stutter(_)(pos ++ PosInExpr(0 :: 0 :: Nil))).reduce[BelleExpr](_ & _), // stutter
      ...
      case Some(Not(Imply(_, _))) => SeqTactic(
          useAt(Ax.notImply)(pos),
          pushNegation(pos ++ PosInExpr(0 :: Nil)),
          pushNegation(pos ++ PosInExpr(1 :: Nil)),
        )
      case Some(Not(And(_, _))) => SeqTactic(
          useAt(Ax.notAnd)(pos),
          pushNegation(pos ++ PosInExpr(0 :: Nil)),
          pushNegation(pos ++ PosInExpr(1 :: Nil)),
          ...
          anon((p: Position, s: Sequent) =>
            s.sub(p) match {
              case Some(Or(Not(Equal(_, _)), q)) if !q.isInstanceOf[Not] =>
                useAt(Ax.implyExpand, PosInExpr(1 :: Nil))(p)
              case _ => skip
            }
          )(pos),
        )
      case Some(Not(Or(_, _))) => SeqTactic(
          useAt(Ax.notOr)(pos),
          pushNegation(pos ++ PosInExpr(0 :: Nil)),
          pushNegation(pos ++ PosInExpr(1 :: Nil)),
        )
  ...
  private def boxAxiomaticSolve(instEnd: Boolean = false): DependentPositionTactic =
    anon((pos: Position, s: Sequent) => {
      val (ode, q, post) = s.sub(pos) match {
        case Some(Box(ODESystem(o, qq), pp)) => (o, qq, pp)
        case Some(f) => throw new TacticInapplicableFailure(
            "Position " + pos + " does not point to a differential equation, but to " + f.prettyString
          )
        case None => throw new IllFormedTacticApplicationException(
            "Position " + pos + " does not point to a differential equation"
          )
      }
      ...
      val renameDuration =
        if (
          StaticSemantics.symbols(s(pos.top)).contains(DURATION) &&
          StaticSemanticsTools.boundAt(s(pos.top), pos.inExpr).contains(DURATION)
        ) {
          @tailrec
      ...
      val consts = assumptions
        ...
        .filter(
          StaticSemantics
            .symbols(_)
            .intersect(StaticSemantics.symbols(ode) ++ StaticSemantics.symbols(q) ++ StaticSemantics.symbols(post))
            .nonEmpty
        )
        .map(
          SimplifierV3
            .simpWithDischarge(IndexedSeq[Formula](), _, SimplifierV3.defaultFaxs, SimplifierV3.defaultTaxs)
            ._1
        )
      ...
      lazy val allExtract1 = remember(
        "\\forall x_ (p_(x_) -> q_()) -> (\\forall x_ p_(x_) -> q_())".asFormula,
        implyR(1) * 2 & allL(-1) & allL(-2) & prop & done,
        namespace,
      )
      lazy val allExtract2 = remember(
        "\\forall x_ (p_(x_) -> r_(x_)&q_()) -> \\forall x_ (p_(x_)->r_(x_)) ".asFormula,
        implyR(1) & allR(1) & allL(-1) & prop & done,
        namespace,
      )
      lazy val allExtract3 = remember(
        "p_() & \\forall x_ (r_(x_) -> q_(x_)) -> \\forall x_ (r_(x_) -> q_(x_) & p_())".asFormula,
        implyR(1) & andL(-1) & allR(1) & allL(-2) & prop & done,
        namespace,
      )
          ...
          case Some(_) =>
            ...
            s.sub(pos) match {
              case Some(Box(ODESystem(_, qq), _)) if polarity > 0 =>
                if (qq == True) SeqTactic(
                  useAt(allExtract1)(pos ++ PosInExpr(0 :: 1 :: 0 :: Nil)),
                  ...
                  andR(pos),
                  BranchTactic(prop & done, skip),
                )
                else skip // useAt(allExtract2)(pos ++ PosInExpr(0::1::0::Nil))
              case Some(Box(ODESystem(_, qq), _)) if polarity < 0 => skip // @todo
              case Some(e) =>
                throw new TacticInapplicableFailure("simpConsts only applicable to box ODEs, but got " + e.prettyString)
              case None => throw new IllFormedTacticApplicationException(
      ...
      SeqTactic(
        ...
        cutInSoln(osize)(odePosAfterInitialVals),
        DebuggingTactics.debug("Cut in a sol'n", ODE_DEBUGGER),
        ...
        (inverseDiffCut(osize)(odePosAfterInitialVals) &
          DebuggingTactics.debug("did an inverse diff cut", ODE_DEBUGGER)) * (osize + (if (cutConsts) 1 else 0)),
  ...
  private def dfsLoop(
      ...
      curr: Variable,
      acc: List[Variable],
  ): Option[(Set[Variable], List[Variable])] = {
    if (visited.contains(curr)) { None }
    else if (acc.contains(curr)) { Some(visited + curr, acc) }
    else {
  ...
  def alist(ode: DifferentialProgram): Option[List[AtomicODE]] = {
    ode match {
      ...
      case DifferentialProduct(ode1, ode2) => (alist(ode1), alist(ode2)) match {
          case ((_, None)) => None
          case ((None, _)) => None
  ...
  private def dfsOuterLoop(
      odes: List[AtomicODE],
      visited: Set[Variable],
      acc: List[Variable],
  ): Option[(Set[Variable], List[Variable])] = {
    if (visited.size == odes.size) { Some(visited, acc) }
    else {
      ...
      alist(ode) match {
        case Some(atomics) => dfsOuterLoop(atomics, Set(), Nil).map(_._2) match {
            ...
            case Some(vs) =>
              ...
              if (vs.exists(_.name == "kyxtime"))
                Some(vs.filter(_.name != "kyxtime") ++ vs.filter(_.name == "kyxtime").sortBy(_.index))
              else Some(vs)
          }
        case None => None
      }
    } catch { case _: Cycle => None }
  }

  def sortSubst(
      dom: Formula,
      post: Formula,
      context: DifferentialProgram,
  ...
  def sortAxiomInst(
      ...
      ode1: DifferentialProgram,
      ode2: DifferentialProgram,
  ): Provable = { Provable.axioms(", sort")(sortSubst(dom, post, context, ode1, ode2)) }
  ...
  def splitODEAt(ode: DifferentialProgram, v: Variable): (List[AtomicODE], List[AtomicODE], List[AtomicODE]) = {
    ode match {
      case DifferentialProduct(AtomicODE(DifferentialSymbol(v2), e), tail) => {
        if (v == v2) { (Nil, List(AtomicODE(DifferentialSymbol(v2), e)), alist(tail).get) }
        else {
      ...
      case AtomicODE(DifferentialSymbol(v2), e) => {
        if (v == v2) { (Nil, List(AtomicODE(DifferentialSymbol(v2), e)), Nil) }
        else { (Nil, Nil, List(AtomicODE(DifferentialSymbol(v2), e))) }
      }
    }
  }
    ...
    .anon((pos: Position, s: Sequent) => {
      val (ode: DifferentialProgram, dom: Formula, post: Formula) = s.sub(pos) match {
        ...
        case Some(sub) => throw new TacticInapplicableFailure(
            "Expected [] or <> modality at position " + pos + ", but got " + sub.prettyString
          )
        case None => throw new IllFormedTacticApplicationException(
            "Position " + pos + " does not point to a valid position in sequent " + s.prettyString
          )
      ...
      val bv = StaticSemantics
        ...
        .map({ case DifferentialSymbol(v) => v })
  ...
  val addTimeVar: DependentPositionTactic = TacticFactory.anon((pos: Position, s: Sequent) => {
    s.sub(pos ++ PosInExpr(0 :: Nil)) match {
      ...
      case _ => throw new TacticInapplicableFailure(
          s"setupTimeVar should only be called on differential programs without an existing time variable but found ${s
              .apply(pos)} of type ${s.apply(pos).getClass}."
        )
    }
  ...
  def cutInSoln(odeSize: Int, diffArg: Term = Variable("kyxtime")): DependentPositionTactic =
    anon((pos: Position, s: Sequent) => {
      val system: ODESystem = s.sub(pos) match {
        ...
        case Some(e) =>
          throw new TacticInapplicableFailure("solDC only applicable to box ODEs, but got " + e.prettyString)
        case None => throw new IllFormedTacticApplicationException(
            "Position " + pos + " does not point to a valid position in sequent " + s.prettyString
          )
      }
      ...
      def extract(f: Formula, p: PosInExpr, n: Int): List[(Variable, Term)] =
        if (n == 0) Nil
        else extract(f, p.parent, n - 1) :+
          (f.sub(p) match {
            case Some(Box(Assign(v, t: Variable), _)) => t -> v
            case Some(e) => throw new TacticInapplicableFailure(
                "solDC.extract only applicable to box assignments, but got " + e.prettyString
              )
            case None => throw new IllFormedTacticApplicationException(
                "Position " + pos + " does not point to a valid position in formula " + f.prettyString
              )
          })
      ...
      val solutions =
        try { Integrator(initialConditions, Minus(TIMEVAR, Variable(TIMEVAR.name, Some(initIdx))), system) }
        catch {
          case ex: IllegalArgumentException =>
            throw new TacticInapplicableFailure("Unable to obtain symbolic solution with builtin integrator", ex)
        }

      val sortedDifferentials = sortAtomicOdes(atomicOdes(system), diffArg).filter(_.xp.x != TIMEVAR).map(_.xp.x)
      val sortedSolutions = solutions.sortWith({ case (Equal(a, _), Equal(b, _)) =>
        sortedDifferentials.indexOf(a) < sortedDifferentials.indexOf(b)
      })
      ...
      val fact = s.sub(withInitialsPos) match {
        ...
        case Some(fml: Formula) if polarity == 0 =>
          throw new TacticInapplicableFailure(
            "cutAndProveFml only applicable in positive or negative polarity contexts"
          )
        case Some(e) =>
          throw new TacticInapplicableFailure("cutAndProveFml only applicable to box ODEs, but got " + e.prettyString)
        case None => throw new IllFormedTacticApplicationException(
            "Position " + pos + " does not point to a valid position in sequent " + s.prettyString
          )
      }
        ...
        ss.sub(pp) match {
          case Some(And(p, Equal(x, f))) => RenUSubst(
              ("x_".asVariable, x) :: ("p_(.)".asFormula, p.replaceFree(x, DotTerm())) ::
                ("f(.)".asTerm, f.replaceFree(x, DotTerm())) :: Nil
            )
          case Some(e) => throw new TacticInapplicableFailure(
              "domSimplifyStep only applicable to conjunctions, but got " + e.prettyString
            )
          case None => throw new IllFormedTacticApplicationException(
              "Position " + pp + " does not point to a valid position in sequent " + ss.prettyString
            )
  ...
  private def inverseDiffCut(odeSize: Int): DependentPositionTactic = anon((pos: Position, s: Sequent) => {
    ...
    val fact = s.sub(withInitialsPos) match {
      ...
      case Some(fml: Formula) if polarity == 0 =>
        throw new TacticInapplicableFailure("dCi2 only applicable in negative or positive polarity contexts")
      case Some(e) => throw new TacticInapplicableFailure("dCi2 only applicable to formulas, but got " + e.prettyString)
      case None => throw new IllFormedTacticApplicationException(
          "Position " + withInitialsPos + " does not point to a valid position in sequent " + s.prettyString
        )
    }
