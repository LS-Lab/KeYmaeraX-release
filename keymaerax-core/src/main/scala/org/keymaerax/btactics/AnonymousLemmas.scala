...
object AnonymousLemmas {
  ...
  def store(p: ProvableSig, namespace: String): Lemma = {
    ...
    val id = lemmaDB.add(Lemma(p, Lemma.requiredEvidence(p, evidence), Some(generateName(p.conclusion, namespace))))
  ...
  def find(fml: Formula, namespace: String): Option[Lemma] =
    find(Sequent(immutable.IndexedSeq(), immutable.IndexedSeq(fml)), namespace)
  ...
  private def getAnonymousLemma(id: lemmaDB.LemmaID, s: Sequent): Option[Lemma] = {
    lemmaDB.get(id) match {
      case None => None
      case Some(lemma) if lemma.fact.conclusion == s =>
        Some(
          Lemma(lemma.fact, lemma.evidence, None)
        ) // @note make anonymous, otherwise we look up non-existent ProvableInfo
      case Some(lemma) if lemma.fact.conclusion != s =>
        throw new IllegalStateException("Anonymous lemma " + lemma + " does not conclude expected " + s.prettyString)
    }
  }
}
