...
object ArithmeticSimplification extends TacticProvider {
  ...
  /**
   * Simplifies arithmetic by removing formulas that are **probably** irrelevant to the current sub-goal. Does not
...
// todo: unsure
  ...
  lazy val smartSuccHide = new DependentTactic("smartSuccHide") {
    override def computeExpr(p: ProvableSig) = {
      assert(
        p.subgoals.length == 1,
        s"${this.name} is only relevant to Provables with one subgoal; found ${p.subgoals.length} subgoals",
      )
  ...
  def hideFactsAbout(irrelevant: String*): BelleExpr = anon((sequent: Sequent) => {
    ...
    val hideAnte = sequent
      .ante
      .zipWithIndex
      .filter(p => StaticSemantics.symbols(p._1).intersect(irrelevantSet).nonEmpty)
      ...
      equality match {
        case Equal(what, to) =>
          ...
          cutLR(seq(pos.top).replaceFree(what, to))(pos) <
            (
              skip,
              if (pos.isAnte) implyR(Symbol("Rlast")) &
                seq.succ.indices.map(i => hideR(i + 1)).reverse.reduceOption[BelleExpr](_ & _).getOrElse(skip) & QE &
                done
              else implyR(pos) &
                seq
                  .succ
...
//  }
  ...
  def irrelevantIndices(fmls: IndexedSeq[Formula], baseline: IndexedSeq[Formula]): Seq[Int] = {
    val theFilter: (Seq[(Formula, Int)], Set[NamedSymbol]) => Seq[(Formula, Int)] =
      transitiveRelevance //    relevantFormulas(s.ante.zipWithIndex, symbols(s.succ))
  ...
  private def irrelevantSuccPositions(s: Sequent): Seq[SuccPosition] = {
    ...
    s.succ
      .zipWithIndex
      .filter { case (fml, _) => (StaticSemantics.symbols(fml) -- anteSymbols).nonEmpty }
      .map { case (_, i) => SuccPosition(i + 1) }
  }

  /**
   * Returns all formulas that transitively mention relevantSymbols. For example, if
   * {{{
   * fmls = (
   *   (x>=y, 0)
   *   (y>=z, 5)
  ...
  private def transitiveRelevance(fmls: Seq[(Formula, Int)], relevantSymbols: Set[NamedSymbol]): Seq[(Formula, Int)] = {
    ...
    if (newlyRelevantSymbols.isEmpty) relevantFmls
      ...
      transitiveRelevance(
        fmls,
        relevantSymbols ++ newlyRelevantSymbols,
      ) // sic: recurse on [[indexedFmls]], not [[relevantFmls]], because some of the previously irrelevant formulas might now be relevant.
  }
  ...
  private def retainStrongest(
      fmls: Seq[(Formula, Int)],
      assumptions: Formula,
      cex: CounterExampleTool,
  ): Seq[(Formula, Int)] = {
    ...
    val irrelevant = pairs
      .filter({ case ((p, _), (q, _)) => cex.findCounterExample(Imply(And(assumptions, p), q)).isEmpty })
      .map(_._2)
