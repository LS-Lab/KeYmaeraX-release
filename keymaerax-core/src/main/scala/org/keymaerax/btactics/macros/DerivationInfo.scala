...
sealed trait DerivationInfo {
  ...
  val codeName: String
  require(
    "^[a-zA-Z0-9_]*$".r.matches(codeName) | codeName.startsWith("expand"),
    "codeName must consist only of a-z, A-Z, 0-9, _",
  )
...
sealed trait TacticInfo extends DerivationInfo {
  val constructor: TacticConstructor
  override final def inputs: List[ArgInfo] = constructor.args.toList
