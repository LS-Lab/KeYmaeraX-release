...
object EvidencePrinter {
  ...
  def stampHead(options: OptionMap): String = {
    val version = VersionNumber.CURRENT
    val commandLineOptions = nocomment(options.getOrElse(Symbol("commandLine"), "<unavailable>").asInstanceOf[String])
