...
/**
  ...
  *  - [[Context Context]] Convenience representation of formulas used as contexts that provide ways of substituting expressions in by conceptually splitting and managing the context around a position in a formula.
  *  - [[RenUSubst RenUSubst]] Renaming Uniform Substitutions that quickly combine and ultimately reduce to uniform renaming and uniform substitution of the kernel.
  *  - [[UnificationMatch] UnificationMatch] single-sided matching unification algorithms.
  *  - [[Augmentors Augmentors]]: Implicit convenience additions of helper functions to formulas, terms, programs, sequents that are useful for analysis and transformation.
  *  - [[ExpressionTraversal ExpressionTraversal]] generic functionality for traversing expressions for analysis or transformation.
  *
