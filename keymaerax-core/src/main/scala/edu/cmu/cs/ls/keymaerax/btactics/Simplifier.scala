 ...
 * Simplification is parameterized over a list of simplification steps. The default set of simplifications is guaranteed
 * to terminate (using the number of constructors in the term as a termination metric), and an optional set of rules is
 * provided for which termination is less clear. Any set of simplifications is allowed, as long as they terminate (the
 * termination metric need not be the number of constructors in the term). Created by bbohrer on 5/21/16.
 */
object Simplifier {
  ...
  val timesZeroLeft: Simplification = {
    case Times(n: Number, t: Term) =>
      if (n.value == 0) { Some(Number(0), useAt(Ax.zeroTimes)(Position(1, 0 :: Nil)) & byUS(Ax.equalReflexive)) }
      else None
    case _ => None
  }
  ...
  val timesZeroRight: Simplification = {
    case Times(t: Term, n: Number) =>
      if (n.value == 0) { Some((Number(0), useAt(Ax.timesZero)(Position(1, 0 :: Nil)) & byUS(Ax.equalReflexive))) }
      else None
    case _ => None
  }
  ...
  val timesOneLeft: Simplification = {
    case Times(n: Number, t: Term) =>
      if (n.value == 1) {
        Some((
          t,
          useAt(Ax.timesCommute)(1, 0 :: Nil) & useAt(Ax.timesIdentity)(Position(1, 0 :: Nil)) & byUS(Ax.equalReflexive),
        ))
      } else None
    case _ => None
  }
  ...
  val timesOneRight: Simplification = {
    case Times(t: Term, n: Number) =>
      if (n.value == 1) { Some((t, useAt(Ax.timesIdentity)(Position(1, 0 :: Nil)) & byUS(Ax.equalReflexive))) }
      else None
    case _ => None
  }
  ...
  def simp(simps: List[Simplification] = defaultSimps): DependentPositionTactic =
    anon((pos, sequent) => SaturateTactic(simpOnce(simps)(pos)))
}
