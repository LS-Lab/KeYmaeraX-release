...
object USubstRenChurch {
  ...
  /**
   * `true` for transpositions (replace `what` by `repl` and `what'` by `repl'` and, vice versa, `repl` by `what` etc)
   * or `false` to clash upon occurrences of `repl` or `repl'`.
   */
  private[infrastruct] val TRANSPOSITION: Boolean = URename(Variable("dummy"), Variable("ymmud"))(Variable("ymmud")) ==
    Variable("dummy")
}
 ...
 *
 * This implementation uses Church-style uniform substitution implementation a la [[USubstChurch]]. Liberal list of
...
final case class USubstRenChurch(private[infrastruct] val subsDefsInput: immutable.Seq[(Expression, Expression)])
  ...
  /** substitution part */
  private val subs: immutable.Map[Expression, Expression] = subsDefsInput
  ...
  private val matchHeads: immutable.Map[Function, (ApplicationOf, Expression)] = subs
...
//      case DifferentialSymbol(x) => DifferentialSymbol(renameVar(x, term))
  ...
  private def usubst(formula: Formula): Formula = {
    ...
    (
      r => r.kind == formula.kind && r.sort == formula.sort,
      "Uniform Substitution leads to same kind and same sort " + formula,
    )
  ...
  private def usubst(program: Program): Program = {
    ...
    (
      r => r.kind == program.kind && r.sort == program.sort,
      "Uniform Substitution leads to same kind and same sort " + program,
    )
  ...
  private def requireAdmissible(U: SetLattice[Variable], e: Expression, context: Expression): Unit =
    if (!admissible(U, e)) throw new SubstitutionClashException(
      toString,
      U.prettyString,
      e.prettyString,
   ...
   *   arXiv:1503.01981 Definition 12.
   */
  private def projection(affected: immutable.Set[NamedSymbol]): USubstRenChurch =
    new USubstRenChurch(subsDefsInput.filter(sp =>
      sp._1 match {
