...
class RingsLibrary(terms: Iterable[Term]) {
  ...
  val names = {
    ...
    if (emp.nonEmpty) throw ToolExecutionException("RingsLibrary does not handle non-constant function symbols: " + emp)
    vars ++ funcs
  }

  val ringsNames = names
    .zipWithIndex
    .map(p =>
      p._1 match {
  ...
  def toRing(term: Term): Ring = {
    term match {
      case Neg(l) => ring.negate(toRing(l))
      case Plus(l, r) => ring.add(toRing(l), toRing(r))
      case Minus(l, r) => ring.subtract(toRing(l), toRing(r))
      case Times(l, r) => ring.multiply(toRing(l), toRing(r))
      case Divide(l, r) =>
      ...
      case Power(l, Number(n)) if n.isValidInt && n >= 0 => ring.pow(toRing(l), n.toIntExact)
      case Power(l, r) => {
      ...
      case f: FuncOf if f.child == Nothing => ring(mapper(f.func))
  ...
  def fromRing(m: Monomial[Rational[BigInteger]]): Term = {
    ...
    val xis =
      (cof ::
        k.zipWithIndex
          .map(i =>
            if (i._1 == 0) Number(1) else if (i._1 == 1) unmapper(i._2) else Power(unmapper(i._2), Number(i._1))
    ...
    if (xis.isEmpty) cof else xis.tail.fold(xis.head)(cleverTimes)
  }
  ...
  private def multiringify(
      varname: String,
      upoly: UnivariatePolynomial[Ring],
      ring: MultivariateRing[Rational[BigInteger]],
  ): Ring = {
    val coeffs = upoly.iterator().asScala.toList
    coeffs
      .zipWithIndex
      .map(p => ring.multiply(p._1, ring(varname + "^" + p._2.toString)))
  ...
  def integrate(i: Int)(t: Ring): Ring = t.mapTerms(
    Q,
    { mon =>
      val p = mon.dv.exponents(i)
      mon.setCoefficient(mon.coefficient.divide(p + 1)).setDegreeVector(mon.dv.dvMultiply(i, 1))
    },
  )
  ...
  def internalQuotientRemainder(a: Ring, b: Ring): Option[(Ring, Ring)] = {
    ...
    val q = array(0)
    if (q.isZero) None else Some(q, array(1))
  }
    ...
    case x :: xs => {
      internalQuotientRemainder(t, x) match {
        ...
        case Some((q, r)) => {
          ...
          if (hr == Number(0)) prod else if (prod == Number(0)) hr else Plus(hr, prod)
        }
      }
    }
  }
  ...
  def substitutes(subst: Variable => Option[Ring])(t: Term): Ring = t match {
    ...
    case _ =>
      throw ToolExecutionException("Operation " + t.kind + " not (yet) supported by substitutes in RingsLibrary: " + t)
  }
  ...
  def normalizeLessEquals(qeTac: BelleExpr): DependentPositionTactic = anon { (pos: Position, seq: Sequent) =>
    seq.sub(pos) match {
      ...
      case Some(And(f1, f2)) => normalizeLessEquals(qeTac)(pos ++ PosInExpr(0 :: Nil)) &
  ...
  case class ODE(time: Variable, state: Seq[Variable], const0: Seq[FuncOf], rhs: Seq[Term]) {
    ...
    def applyODE(xs: Seq[Ring]) = { rhs.map(f_i => substitutes(state.lazyZip(xs).toMap.get)(f_i)) }
    ...
    def PicardIteration(tm0: Seq[Ring], order: Int) = {
      var ptm = tm0
      for (i <- 0 until order) { ptm = PicardOperation(tm0, ptm, order)._1 }
      ptm
    }
  }
}
