 ...
 *     - `[[edu.cmu.cs.ls.keymaerax.btactics.HilbertCalculus HilbertCalculus]]` Hilbert Calculus for differential
 *       dynamic logic
 *     - `[[edu.cmu.cs.ls.keymaerax.btactics.SequentCalculus SequentCalculus]]` Sequent Calculus for propositional and
 *       first-order logic
 *     - `[[edu.cmu.cs.ls.keymaerax.btactics.HybridProgramCalculus HybridProgramCalculus]]` Hybrid Program Calculus for
 *       differential dynamic logic
 *     - `[[edu.cmu.cs.ls.keymaerax.btactics.DifferentialEquationCalculus DifferentialEquationCalculus]]` Differential
 *       Equation Calculus for differential dynamic logic
 *     - `[[edu.cmu.cs.ls.keymaerax.btactics.UnifyUSCalculus UnifyUSCalculus]]` Unification-based Uniform Substitution
 *       Calculus
 *
 *   - Tactic tools
 *     - [[edu.cmu.cs.ls.keymaerax.btactics.AxIndex] AxIndex]: Axiom Indexing data structures for canonical proof strategies. - [[edu.cmu.cs.ls.keymaerax.btactics.DerivationInfoRegistry DerivationInfo]]:
 *       Meta-information for derivation steps such as axioms for user interface etc.
 *
 * All tactics are implemented in the [[edu.cmu.cs.ls.keymaerax.bellerophon Bellerophon tactic language]], including its
 * dependent tactics, which ultimately produce [[edu.cmu.cs.ls.keymaerax.core.Provable]] proof certificates by the
 * [[edu.cmu.cs.ls.keymaerax.bellerophon.Interpreter Bellerophon interpreter]]. The
 * [[edu.cmu.cs.ls.keymaerax.core.Provable Provables]] that tactics produce can be extracted, for example, with
 * [[edu.cmu.cs.ls.keymaerax.btactics.TactixLibrary.proveBy()]].
 *
 * =Proof Styles=
 * KeYmaera X supports many different proof styles, including flexible combinations of the following styles:
 *
 *   1. [[edu.cmu.cs.ls.keymaerax.core.Provable Explicit proof certificates]] directly program the proof rules from the
 *      core.
 *   1. [[edu.cmu.cs.ls.keymaerax.btactics.TactixLibrary.andR Explicit proofs]] use tactics to describe a proof directly
 *      mentioning all or most proof steps.
 *   1. [[edu.cmu.cs.ls.keymaerax.btactics.TactixLibrary.prop Proof by search]] relies mainly on proof search with
 *      occasional additional guidance.
 *   1. [[edu.cmu.cs.ls.keymaerax.btactics.UnifyUSCalculus.useAt() Proof by pointing]] points out facts and where to use
 *      them.
 *   1. [[edu.cmu.cs.ls.keymaerax.btactics.UnifyUSCalculus.CEat() Proof by congruence]] is based on equivalence or
 *      equality or implicational rewriting within a context.
 *   1. [[edu.cmu.cs.ls.keymaerax.btactics.UnifyUSCalculus.chase() Proof by chase]] is based on chasing away operators
 *      at an indicated position.
 *
 * ===Explicit Proof Certificates===
 * The most explicit types of proofs can be constructed directly using the [[edu.cmu.cs.ls.keymaerax.core.Provable]]
 * certificates in KeYmaera X's kernel without using any tactics. Also see [[edu.cmu.cs.ls.keymaerax.core]].
 *
 * {{{
 ...
 * [[edu.cmu.cs.ls.keymaerax.btactics.TactixLibrary TactixLibrary]]: The only actual difference is the order of the
 * branches, which is fixed to be from left to right in
 * [[edu.cmu.cs.ls.keymaerax.bellerophon.BelleExpr.<() tactic branching]]. Tactics also use more elegant signed integers
 * to refer to antecedent positions (negative) or succedent positions (positive).
 * {{{
 ...
 * )
 * }}}
 * Common tactics for proof by search include [[edu.cmu.cs.ls.keymaerax.btactics.TactixLibrary.prop()]],
 ...
 * [[edu.cmu.cs.ls.keymaerax.btactics.UnifyUSCalculus.useAt]] and
 * [[edu.cmu.cs.ls.keymaerax.btactics.UnifyUSCalculus.byUS]], which are also imported into
 * [[edu.cmu.cs.ls.keymaerax.btactics.TactixLibrary]]. The above proof by pointing implements directly in KeYmaera X:
 *
 * {{{
 * import TactixLibrary._
 ...
 * )
 * }}}
 * [[edu.cmu.cs.ls.keymaerax.btactics.TactixLibrary.stepAt]] also uses proof by pointing but figures out the appropriate
 * fact to use on its own. Here's a similar proof:
 * {{{
 *  import TactixLibrary._
 ...
 *
 * ===Proof by Congruence===
 * [[edu.cmu.cs.ls.keymaerax.btactics.UnifyUSCalculus.CEat() Proof by congruence]] is based on equivalence or equality
 * or implicational rewriting within a context. This proof style can make quite quick inferences leading to significant
 ...
 * further without critical choices. The canonical examples use
 * [[edu.cmu.cs.ls.keymaerax.btactics.UnifyUSCalculus.chase()]] to chase away differential forms:
 * {{{
 * import TactixLibrary._
 ...
 * )
 * }}}
 * Yet [[edu.cmu.cs.ls.keymaerax.btactics.UnifyUSCalculus.chase()]] is also useful to chase away other operators, say,
 * modalities:
 * {{{
 * import TactixLibrary._
 ...
 *       `Provable=>Provable`
 *     - [[edu.cmu.cs.ls.keymaerax.btactics.UnifyUSCalculus.ForwardPositionTactic]]: Positional forward Hilbert-style
 *       tactics `Position=>(Provable=>Provable)`
