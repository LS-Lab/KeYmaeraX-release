...
//          Sequent(immutable.IndexedSeq(pany), immutable.IndexedSeq(Box(Loop(a), pany))))),
  ...
  private def assertCheckAxiomFile(axs : Map[String, Formula]): Boolean = {
    ...
    insist(axs("refinement choice left") == Equiv(Refinement(Choice(sys, sysb), sysc), And(Refinement(sys, sysc), Refinement(sysb, sysc))) , "refinement choice left")
    insist(axs("refinement choice right") == Imply(Or(Refinement(sys, sysb), Refinement(sys, sysc)), Refinement(sys, Choice(sysb, sysc))) , "refinement choice right")
    insist(axs("refinement sequence") == Imply(And(Refinement(sys, sysc), Box(sys, Refinement(sysb, sysd))), Refinement(Compose(sys,sysb),Compose(sysc,sysd))), "refinement sequence")
    insist(axs("refinement loop left") == Imply(Box(Loop(sys), Refinement(Compose(sys,sysb),sysb)), Refinement(Compose(Loop(sys), sysb), sysb)), "refinement loop left")
    insist(axs("refinement loop right") == Imply(Refinement(Compose(sys,sysb),sys), Refinement(Compose(sys, Loop(sysb)), sys)), "refinement loop right")
    insist(axs("refinement test det") == Equiv(Refinement(Compose(Test(p0),sys), Compose(Test(p0),sysb)), Box(Test(p0), Refinement(sys, sysb))), "refinement test det")
    insist(axs("refinement assign det") == Equiv(Refinement(Compose(Assign(x, f0), sys), Compose(Assign(x, f0), sysb)), Box(Assign(x, f0),Refinement(sys, sysb))), "refinement assign det")
    insist(axs("refinement :* merge") == ProgramEquivalence(Compose(AssignAny(x), Compose(Test(PredOf(p,x)), AssignAny(x))), Compose(AssignAny(x), Test(Exists(immutable.Seq(y), PredOf(p,y))))), "refinement :* merge")
    insist(axs("refinement unloop") == Imply(Box(Loop(sys), Refinement(sys, sysb)), Refinement(Loop(sys), Loop(sysb))), "refinement unloop")
    insist(axs("refinement ode") == Equiv(Refinement(ODESystem(AtomicODE(DifferentialSymbol(x),FuncOf(f,x)), PredOf(p,x)),ODESystem(AtomicODE(DifferentialSymbol(x),FuncOf(g,x)), PredOf(q,x))), Box(ODESystem(AtomicODE(DifferentialSymbol(x),FuncOf(f,x)), PredOf(p,x)), And(Equal(FuncOf(f,x), FuncOf(g,x)), PredOf(q,x)))), "refinement ode")
    insist(axs("ode idempotent") == ProgramEquivalence(Compose(ODESystem(ode, pany),ODESystem(ode, pany)),ODESystem(ode, pany)), "ode idempotent")
    insist(axs("refinement DE") == ProgramEquivalence(ODESystem(DifferentialProduct(AtomicODE(DifferentialSymbol(x), fany), ode), pany), Compose(ODESystem(DifferentialProduct(AtomicODE(DifferentialSymbol(x), fany), ode), pany), Assign(DifferentialSymbol(x), fany))), "refinement DE")
    insist(axs("refinement DW") == ProgramEquivalence(ODESystem(ode, pany), Compose(ODESystem(ode, pany), Test(pany))), "refinement DW")
    insist(axs("refinement DX") == Refinement(Compose(Assign(DifferentialSymbol(x), FuncOf(f,x)), Test(PredOf(p,x))),ODESystem(AtomicODE(DifferentialSymbol(x), FuncOf(f,x)), PredOf(p,x))), "refinement DX")
    /* KAT AXIOMS */
    ...
    insist(axs("choice associative") == ProgramEquivalence(Choice(sys, Choice(sysb, sysc)), Choice(Choice(sys, sysb), sysc)), "choice associative")
    insist(axs("sequence identity left") == ProgramEquivalence(Compose(Test(True),sys), sys), "sequence identity left")
    insist(axs("sequence identity right") == ProgramEquivalence(Compose(sys, Test(True)), sys), "sequence identity right")
    insist(axs("sequence associative") == ProgramEquivalence(Compose(sys, Compose(sysb, sysc)), Compose(Compose(sys, sysb), sysc)), "sequence associative")
    insist(axs("distribute left") == ProgramEquivalence(Choice(Compose(sys,sysb), Compose(sys, sysc)), Compose(sys, Choice(sysb, sysc))), "distribute left")
    insist(axs("distribute right") == ProgramEquivalence(Choice(Compose(sys, sysc), Compose(sysb, sysc)), Compose(Choice(sys, sysb), sysc)), "distribute right")
    ...
    insist(axs("unfold left") == ProgramEquivalence(Choice(Test(True), Compose(sys, Loop(sys))), Loop(sys)), "unfold left")
    insist(axs("unfold right") == ProgramEquivalence(Choice(Test(True), Compose(Loop(sys), sys)), Loop(sys)), "unfold right")
    /* SKAT AXIOMS */
    insist(axs("assign commute") == ProgramEquivalence(Compose(Assign(x, f0), Assign(y, FuncOf(g, x))), Compose(Assign(y, FuncOf(g, f0)), Assign(x, f0))), "assign commute")
    insist(axs("assign substitution") == ProgramEquivalence(Compose(Assign(x, f0), Assign(y, FuncOf(g, x))), Compose(Assign(x, f0), Assign(y, FuncOf(g, f0)))), "assign substitution")
    insist(axs("assign test") == ProgramEquivalence(Compose(Assign(x, f0), Test(PredOf(p, x))), Compose(Test(PredOf(p, f0)), Assign(x, f0))), "assign test")
    insist(axs("assign merge") == ProgramEquivalence(Compose(Assign(x, f0), Assign(x, FuncOf(g, x))), Assign(x, FuncOf(g, f0))), "assign merge")
    insist(axs("nondet assign commute") == ProgramEquivalence(Compose(AssignAny(x), AssignAny(y)), Compose(AssignAny(y), AssignAny(x))), "nondet assign commute")
    insist(axs(":=* commute") == ProgramEquivalence(Compose(Assign(x, f0), AssignAny(y)), Compose(AssignAny(y), Assign(x, f0))), ":=* commute")
    insist(axs("nondet assign test") == ProgramEquivalence(Compose(AssignAny(x), Test(p0)), Compose(Test(p0), AssignAny(x))),"nondet assign test")
