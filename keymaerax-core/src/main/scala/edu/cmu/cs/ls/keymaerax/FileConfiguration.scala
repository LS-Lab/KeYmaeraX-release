...
object FileConfiguration extends Configuration {
  ...
  /**
   * The default KeYmaera X directory name in the user's home directory, if environment variable [[KEYMAERAX_HOME_PATH]]
   * is not set.
   */
  ...
  private def safeGet[T](key: String, getter: String => Any): Option[T] =
    if (contains(key)) Some(getter(key).asInstanceOf[T]) else None
  ...
  override def path(key: String): String = {
    ...
    if (p.startsWith(File.separator)) p else Configuration.sanitizedPath(Configuration.KEYMAERAX_HOME_PATH, p)
  }
