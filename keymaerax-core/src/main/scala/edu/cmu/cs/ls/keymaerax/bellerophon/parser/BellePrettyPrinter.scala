...
import edu.cmu.cs.ls.keymaerax.{bellerophon, core}
import edu.cmu.cs.ls.keymaerax.bellerophon._
import BelleOpSpec.op
import edu.cmu.cs.ls.keymaerax.btactics.{SerializationNames, TacticInfo}
...
//      println("Looking for a code name for " + e)
      ...
      case exn:Throwable =>
        e match {
          ...
          case nit: NamedInputTactic => {
            val theArg = nit.input match {
              case e: core.Expression => Left(e)
              case _ => throw new Exception("IDK what to do with this input.")
            }
            nit.name + "(" + argPrinter(theArg) + ")"
          }
          ...
          case it : InputTactic[_] => {
            ...
            it.name.toString + "(" + argPrinter(theArg) + ")"
          }
