  ...
  protected override def unify(s1: Sequent, s2: Sequent): List[SubstRepl] =
    if (!(s1.ante.length == s2.ante.length && s1.succ.length == s2.succ.length)) ununifiable(s1, s2)
      ...
      val antesubst = s1
        .ante
        .indices
        .foldLeft(List[SubstRepl]())((subst, i) => composeFolder(subst, s1.ante(i), s2.ante(i)))
  ...
  protected override def compose(after: List[SubstRepl], before: List[SubstRepl]): List[SubstRepl] =
    if (after.isEmpty) before
      ...
      try {
        ...
        val r = before.map(sp =>
          try { (sp._1, if (sp._1.isInstanceOf[Variable]) sp._2 else us(sp._2)) }
          catch {
            case e: ProverException =>
...
class FreshUnificationMatch extends SchematicComposedUnificationMatch {
  ...
  private def renameIfNeedBe(repl: List[SubstRepl], e: Expression): Expression = {
    ...
    if (ren.isEmpty) e else Subst(ren)(e)
  }

  /**
   * apply renamings of `repl` to the replacements of `input` (if there any renamings in `repl` at all). Filtering
   * identity renamings that may be caused by renamings.
   */
  ...
  protected override def unifier(e1: Expression, e2: Expression, us: List[SubstRepl]): Subst = {
    if (true)
      ...
      Subst(us.map(sp =>
        if (
          sp._1.isInstanceOf[UnitPredicational] || sp._1.isInstanceOf[UnitFunctional] ||
          sp._1.isInstanceOf[ProgramConst] || sp._1.isInstanceOf[SystemConst] ||
          sp._1.isInstanceOf[DifferentialProgramConst] || sp._1.isInstanceOf[PredicationalOf]
        ) (sp._1, ren(sp._2))
        else sp
      ))
    }
...
class RestrictedBiDiUnificationMatch extends FreshUnificationMatch {
  override protected def unify(e1: Term, e2: Term): List[SubstRepl] = e1 match {
    case _: Number => e2 match {
        case Number(_) => if (e1 == e2) id else ununifiable(e1, e2)
        case _: DotTerm | _: FuncOf => unify(e2, e1)
        case _ => ununifiable(e1, e2)
      }
    // homomorphic cases
    case Neg(t) => e2 match {
        case Neg(t2) => unify(t, t2)
        case fn: FuncOf => unify(fn, e1)
        case _ => ununifiable(e1, e2)
      }
  ...
  override protected def unify(e1: Program, e2: Program): List[SubstRepl] = e1 match {
    case Assign(x, t) => e2 match {
        ...
        case _: ProgramConst | _: SystemConst => unify(e2, e1)
        case _ => ununifiable(e1, e2)
      }
    ...
    case Dual(a) => e2 match {
        ...
        case _: ProgramConst => unify(e2, e1)
        case _ => ununifiable(e1, e2)
      }
    case _ => super.unify(e1, e2)
  }

  override protected def unifyApplicationOf(
  ...
  override def join(
      s: List[(Expression, Expression)],
      t: List[(Expression, Expression)],
...
private final object RenUnificationMatch extends UnificationMatchBase {
  ...
  override protected def unifyVar(x1: Variable, e2: Expression): List[SubstRepl] =
    if (x1 == e2) id
      ...
      case _: Variable => List(SubstRepl(x1, e2.asInstanceOf[Variable]))
      case _ => List(SubstRepl(x1, e2))
    }
  override protected def unifyVar(xp1: DifferentialSymbol, e2: Expression): List[SubstRepl] =
    if (xp1 == e2) id
    else e2 match {
      case _: DifferentialSymbol => List(SubstRepl(xp1.x, e2.asInstanceOf[DifferentialSymbol].x))
      case _ => List(SubstRepl(xp1, e2))
    }
...
class UnificationMatchUSubstAboveURen extends /*Insistent*/ Matcher with Logging {
  ...
  private val usubstUMatcher = new UnificationMatchBase {
    ...
    override protected def unifyVar(x1: Variable, e2: Expression): List[SubstRepl] = e2 match {
      case _: Variable => id
      case _ => ununifiable(x1, e2)
    }
    override protected def unifyVar(xp1: DifferentialSymbol, e2: Expression): List[SubstRepl] = e2 match {
      case _: DifferentialSymbol => id
  ...
  private def staple(e: Expression, ren: Subst, subst: Subst): Subst = {
    ...
    val posthocDottify: (Expression, Expression) => Expression = (what: Expression, repl: Expression) => {
      ...
      val r = repl match {
        case rhs: Term =>
          ren(replaceFree(replaceFree(rhs)(ren(Variable("x_")), dot))(ren(DifferentialSymbol(Variable("x_"))), dot))
    ...
    val inverseRename = (subst: RenUSubst) =>
      RenUSubst(
        subst
          .subsDefsInput
          .map(sp =>
...
object NonSubstUnificationMatch extends FreshUnificationMatch {
  ...
  override def unifyODE(e1: DifferentialProgram, e2: DifferentialProgram): List[(Expression, Expression)] = super
    .unifyODE(e1, e2)
  ...
  override def unifiesODE2(
    ...
    val u1 = unifyODE(s1, t1)
    try { compose(unifyODE(unifier(u1)(s2).asInstanceOf[DifferentialProgram], t2), u1) }
    catch {
  ...
  override def compose(after: List[SubstRepl], before: List[SubstRepl]): List[SubstRepl] = before ++
    transpose(before, after)

  private def transpose(repl: List[SubstRepl], input: List[SubstRepl]): List[SubstRepl] = {
    val ren = repl
      .filter({
        case (_: BaseVariable, _: BaseVariable) => true
        case _ => false
      })
      .map(_.swap)
    if (ren.isEmpty) input
  ...
  override def unifyVar(x1: Variable, e2: Expression): List[(Expression, Expression)] =
    if (x1 == e2) id else unifier(x1, e2)
}
