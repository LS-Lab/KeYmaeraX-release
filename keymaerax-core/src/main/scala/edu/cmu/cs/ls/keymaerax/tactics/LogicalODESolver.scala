...
object LogicalODESolver {
  ...
  /**
   * Moves ODE v' = ... to the front of a system using ", commute" axiom.
   * Used when comuting an inverse diff ghost b/c the axiom requires the ODE at hand to exist at the
   * end of an equation.
   * @todo might also need reassociation as well, not sure --
   * @todo 1) If so, there is a relevant todo in the ODESolver's inverse diff aux tactics (and in particular related to InverseDiffAuxHelpers)
   * @todo 2) If so, this tactic should be the one to do it at the end using a Formatter tactic...
   * @param v The ODE to move to the front.
   */
  def moveODE(v : Variable) = new PositionTactic("Move ODE to front of system") {
    override def applies(s: Sequent, p: Position): Boolean = s(p) match {
      case Box(ODESystem(product:DifferentialProduct, _), _) =>
        getPrimedVariables(product).contains(v)
      case _ => false
    }

    override def apply(p: Position): Tactic = new ConstructionTactic("Construct " + name) {
      override def constructTactic(tool: Tool, node: ProofNode): Option[Tactic] = {
//        val currPos : Position = vpos(TacticHelper.getFormula(node.sequent, p)) match {
//          case Some(toAppend) => p.subPos(toAppend)
//          case None => ???
//        }

        TacticHelper.getFormula(node.sequent, p) match {
          case Box(ODESystem(product:DifferentialProduct, _), _) => {
            if(ODETactics.InverseDiffAuxHelpers.toList(product).last.xp.x.equals(v)) Some(debugT("moveODE isn't necessary.") & NilT)
            else {
              ???
            }
          }
    ...
    /**
     * Goal is to eventually move v'=... to be the last element of the product. Concretely, ensures:
     *    ODETactics.InverseDiffAuxHelpers.toList(product).last.xp.x.equals(v)
     *
     * This tactic steps toward that goal by finding the first product:
     *    (X, Y)
     * such that Y contains v'=...
     *
     * This is achieved by moving the outermost formula containing v'=... to the right.
     * For instance, if we have ((v'=..., blah'=...) *,* (x'=..., z'=...)) then we pivot on the *,* comma.
     * And then saturating eventual gives ((x'=..., z'=...) , (blah'=..., v'=...))
     *
     * Saturating this tactic and the Formatting to a right associative form should be sufficient for
     * isolating v'=...
     */
    def stepT(v : Variable) : PositionTactic =
    new PositionTactic(name + " Step") {
      override def applies(s: Sequent, p: Position): Boolean = TacticHelper.getFormula(s, p) match {
        case Box(ODESystem(pi : DifferentialProduct, h), _) => ODETactics.InverseDiffAuxHelpers.isProductOfAtomics(pi)
        case _ => false
      }
    ...
    /**
     *
     * @param theF A formula containing v' = ...
     * @return the PosInExpr of v'=... in f
     */
    def vpos(theF : Formula) : Option[PosInExpr] = {
      val traversalFn = new ExpressionTraversalFunction {
        var thePos : Option[PosInExpr] = None
        override def preP(p : PosInExpr, pi : Program) = pi match {
          case AtomicODE(DifferentialSymbol(candidate),_) =>
      ...
      ExpressionTraversal.traverse(traversalFn, theF)
      traversalFn.thePos
    }
  }
