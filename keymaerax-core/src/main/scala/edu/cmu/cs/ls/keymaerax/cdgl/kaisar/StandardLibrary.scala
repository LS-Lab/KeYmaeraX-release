...
object StandardLibrary {
  ...
  def rN(n: Int): Sort = { if (n == 0) Unit else if (n == 1) Real else Tuple(Real, rN(n - 1)) }

  /** Tuple containing terms [[fs]] */
  def termsToTuple(fs: List[Term]): Term = {
    fs match {
      case Nil => Nothing
      case _ => fs.reduceRight[Term](Pair)
    }
  }
  ...
  def zip3[T1, T2, T3](xs: List[T1], ys: List[T2], zs: List[T3], astNode: ASTNode = Triv()): List[(T1, T2, T3)] = {
    (xs, ys, zs) match {
      ...
      case _ => throw new NodeException("Mismatched lengths in zip3") {
          override val node: ASTNode = astNode
        }
    }
  }
  ...
  /**
   * Elaborate fact binding patterns as in assertions ?pat:(fml); Vectorial patterns ?(x, y):(p & q) become lists of
   * bindings.
   */
  def factBindings(lhs: Expression, rhs: Formula, node: ASTNode = Triv()): List[(Option[Ident], Formula)] = {
    def loop(lhs: Expression, rhs: Formula): List[(Option[Ident], Formula)] = {
      (lhs, rhs) match {
        ...
        case _ => throw ElaborationException(
            s"Vectorial fact $lhs:$rhs expected right-hand-side to have shape p1 & ... & ... pn with one conjunct per fact name, but it did not.",
            node,
          )
      }
    }
  ...
  def unifyFml(p: Formula, q: Formula): Option[Formula] = {
    try {
      ...
      val r = subst(p)
      Some(r)
    } catch { case _: ProverException => None }
  }
