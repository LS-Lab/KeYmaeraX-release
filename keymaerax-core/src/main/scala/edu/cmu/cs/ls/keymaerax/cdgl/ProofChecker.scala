  ...
  def apply(G: Context, M: Proof): Formula = {
    ...
    M match {
      ...
      case DChoiceE(child, left, right) =>
        apply(G, child) match {
          case Diamond(Choice(a, b), p) =>
            ...
            if (pp != qq) {
              throw ProofException(s"Postconditions $pp and $qq do not match in <++>E", G)
            }
            pp
          case p => throw ProofException(s"Rule <++>E not applicable to subgoal $p", G)
        }
      ...
      case DGo(child) =>
        apply(G, child) match {
          case Diamond(a, Diamond(Loop(b), p)) =>
            if (a != b)
              throw ProofException(s"Programs $a and $b do not match in <*>G", G)
            Diamond(Loop(a), p)
      ...
      case DRepeatE(child, post, step) =>
        apply(G, child) match {
          case Diamond(Loop(a), p) =>
            ...
            if (p1 != p2)
              throw ProofException(s"Rule <*>E postconditions $p1 and $p2 did not match", G)
            p1
          case p => throw ProofException(s"Rule <*>E not applicable to subgoal $p", G)
        }
      case DDualI(child) =>
      ...
      case DSolve(ode, post, durPos, dc, child, s, t, ysOpt) =>
        ...
        apply(G, durPos) match {
          case GreaterEqual(dur, n: Number) =>
            if (n.value != 0) {
              throw ProofException(s"<'> duration must be proven >= 0, had ${n.value} instead", G)
            }
            ...
            if (postSub != expectedSub) {
              throw ProofException(s"<'> Post step with postcondition $post expected subgoal postcondition $expectedSub, got $postSub", G1)
            }
      ...
      case DV(const, dur, rate, post, t, ysOpt) =>
        ...
        apply(G1, const) match {
          case And(GreaterEqual(d1, n1: Number), GreaterEqual(f, Neg(Times(d2, v1)))) =>
            if (d1 != d2)
              throw ProofException(s"Durations $d1 and $d2 must be equal in DV", G1)
            else if (n1.value != 0)
              throw ProofException(s"Expected duration >=0 in DV, got $n1", G1)
            apply(G1, dur) match {
              case Diamond(ODESystem(tode@DifferentialProduct(AtomicODE(DifferentialSymbol(t1), one: Number), ode1), q1), td@GreaterEqual(Minus(t2, tInit), d3)) =>
                if (t1 != t || one.value != 1) {
                  ...
                  throw ProofException(s"DV duration $d1 must not mention time variables $t, $tOld or bound variables of $tode", G1)
                } else if (!StaticSemantics(v1).intersect(StaticSemantics(tode).bv.+(tOld)).isEmpty) {
                  throw ProofException(s"DV rate $v1 must be constant with respect to time $t, $tOld and bound variables of $tode", G1)
                }
                apply(G, rate) match {
                case Box(ODESystem(ode2, q2), GreaterEqual(fdiff, v2)) =>
                  ...
                  if (ode1 != ode2) {
                    throw ProofException(s"ODEs $ode1 and $ode2 must match in DV", G)
                  } else if (q1 != q2) {
                    throw ProofException(s"Constraints $q1 and $q2 must match in DV", G)
                  } else if (v1 != v2) {
                    throw ProofException(s"Rates $v1 and $v2 must match in DV", G)
      ...
      case BTestE(left, right) =>
        apply(G, left) match {
          case Box(Test(p1), q) =>
            ...
            if (p1 != p2)
              throw ProofException(s"Assumption $p2 does not match expected $p1 in [?]E", G)
            q
          ...
          case Box(ODESystem(ode, con), p) =>
            apply(G, right) match {
              case Box(ODESystem(ode1, And(con1, p1)), q) =>
                if (ode != ode1) {
                  throw ProofException(s"DC expects ODEs $ode and $ode1 to match", G)
                } else if (con1 != con) {
                  throw ProofException(s"DC expects constraint $con to match conjunct $con1", G)
                } else if (p1 != p) {
                  throw ProofException(s"DC expects postconditions $p and $p1 to match", G)
                }
      ...
      case DI(ode1, pre, step, ysOpt) =>
        ...
        apply(G1, step) match {
          case dp =>
            ...
            if (dp != df) {
              throw ProofException(s"Subgoal in DI should be derivative $df, was $dp", G1)
            }
      ...
      case OrE(child, left, right) =>
        apply(G, child) match {
          case Or(p, q) =>
            ...
            if (r1 != r2)
              throw ProofException(s"Postconditions $r1 and $r2 do not match in rule |E", G)
            else
              r1
          case p => throw ProofException(s"Rule |E not applicable to subgoal $p", G)
        }
      ...
      case ImplyE(left, right) =>
        apply(G, left) match {
          case Imply(p1, q) =>
            val p2 = apply(G, right)
            if (p1 != p2)
              throw ProofException(s"Assumption $p1 and $p2 mismatch in ->E", G)
            else
              q
          case p => throw ProofException(s"Rule ->E not applicable to formula $p", G)
        }
      case EquivI(Equiv(p, q), left, right) =>
        ...
        if (p1 != p) {
          throw ProofException(s"Left formula $p1 does not match $p in <->I", G)
        } else if (q1 != q) {
          throw ProofException(s"Right formula $q1 does not match $q in <->I", G)
        } else
          Equiv(p, q)
      case EquivEL(child, assump) =>
        apply(G, child) match {
          case Equiv(p, q) =>
            ...
            if (p != pp) {
              throw ProofException(s"Assumption $pp does not match expected $p in <->EL", G)
            } else {
              q
            }
          case p => throw ProofException(s"Rule <->EL not applicable to subgoal $p", G)
      ...
      case EquivER(child, assump) =>
        apply(G, child) match {
          case Equiv(p, q) =>
            ...
            if (q != qq) {
              throw ProofException(s"Assumption $qq does not match expected $q in <->EL", G)
            } else {
              p
            }
          case p => throw ProofException(s"Rule <->ER not applicable to subgoal $p", G)
      ...
      case NotE(left, right) =>
        apply(G, left) match {
          case Not(p) =>
            ...
            if (p != pp) {
              throw ProofException(s"Rule !E needed $p to contradict ${Not(p)}, got $pp", G)
            }
            False
          case p => throw ProofException(s"Rule !E not applicable to subgoal $p", G)
        }
      ...
      case Hyp(p) =>
        if (G.contains(p)) G(p)
        else throw ProofException(s"Proof variable $p undefined in rule hyp", G)
      case Mon(left, right, ysOpt) =>
        ...
        p match {
          ...
          case Diamond(a, p) =>
            ...
            Diamond(a, q)
          case _ =>
            throw ProofException(s"Montonicity not applicable to non-modal formula ${p}", G)
        }
      case QE(q, child) =>
        val p = apply(G, child)
        if (!Imply(p, q).isFOL) {
