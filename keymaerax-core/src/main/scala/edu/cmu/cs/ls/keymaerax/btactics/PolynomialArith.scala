...
object PolynomialArith extends Logging {
   ...
   * }}}
   *
   * Variables in the first half of * must be lexicographically > than the variable in the second by VarOrd ("Variables"
   * is meant a bit loosely in that nullary function symbols are allowed too) (n must be a Number)
   *
   ...
   * }}}
   *
   * monomials in the second half of + must be smaller than the first half by monomial ordering const must be a rational
   * coefficient represented by Divide(Number,Number), with both numerator != 0, denominator > 0 and they must be
   * co-prime
   *
  ...
  private lazy val plusAssoc1 = remember(
    "(F_() + G_()) + (A_() + B_()) = ((F_()+G_())+A_())+B_()".asFormula,
    useAt(Ax.equalCommute)(1) & byUS(Ax.plusAssociative),
    namespace,
  ).fact
  // todo: we might get this if the simplifier understood AC rewriting..
  private lazy val plusAssoc2 =
    remember("(F_() + K_()*M_()) + (A_() + L_()*M_()) = (F_()+A_())+(K_()+L_())*M_()".asFormula, QE & done, namespace)
      .fact

  private lazy val plusCoeff1 = remember(
    "K_() = 0 -> (F_() + K_()*M_() = F_())".asFormula,
    prop & exhaustiveEqL2R(-1) & SimplifierV3.simpTac()(1) & close,
    namespace,
  ).fact
  private lazy val plusCoeff2 =
    remember("K_() = L_() -> (F_() + K_()*M_() = F_() + L_()*M_())".asFormula, byUS(Ax.constCongruence), namespace).fact

  private lazy val onetimes =
  ...
  private lazy val timesAssoc1 = remember(
    "(F_() * G_()) * (A_() * B_()) = ((F_()*G_())*A_())*B_()".asFormula,
    useAt(Ax.equalCommute)(1) & byUS(Ax.timesAssociative),
    namespace,
  ).fact
  // todo: we might get this if the simplifier understood AC rewriting..
  private lazy val timesAssoc2 =
    remember("(F_() * M_()^K_()) * (A_() * M_()^L_()) = (F_()*A_())*M_()^(K_()+L_())".asFormula, QE & done, namespace)
      .fact
  private lazy val timesAssoc3 = remember(
    ("(P_() + C_() * M_()) * (D_() * N_()) = " + "P_() * (D_() * N_()) + (C_() * D_()) * (M_() * N_())").asFormula,
    QE & done,
    namespace,
  ).fact

  // QE has interesting ideas about X^0
  private lazy val timesCoeff1Lem =
    remember("F_() = F_() * M_() ^ 0".asFormula, SimplifierV3.simpTac()(1) & close, namespace).fact
  private lazy val timesCoeff1 = remember(
    "K_() = 0 -> (F_() * M_()^K_() = F_() )".asFormula,
    useAt(timesCoeff1Lem)(SuccPosition(1, 1 :: 1 :: Nil)) & byUS(Ax.constCongruence),
    namespace,
  ).fact
  private lazy val timesCoeff2 =
    remember("K_() = L_() -> (F_() * M_()^K_() = F_() * M_()^L_())".asFormula, byUS(Ax.constCongruence), namespace).fact
  ...
  private lazy val powLem5 =
    remember("K_() = L_() -> (M_()^K_() = M_()^L_())".asFormula, byUS(Ax.constCongruence), namespace).fact

  private lazy val negNormalise =
    remember("-P_() = P_() * (-1/1 * 1)".asFormula, SimplifierV3.simpTac()(1) & close, namespace).fact
  ...
  private lazy val var1Normalise =
    remember("P_() = 0 + (1/1) * (1 * P_()^1)".asFormula, SimplifierV3.simpTac()(1) & close, namespace).fact
  // Normalization for any variable (or power of variable)
  private lazy val varNormalise =
    remember("P_() = 0 + (1/1) * (1 * P_())".asFormula, SimplifierV3.simpTac()(1) & close, namespace).fact
  ...
  private lazy val plusGeMono: ProvableSig =
    remember("(f_() >= k_() & g_() >= 0) -> f_() + g_() >= k_()".asFormula, QE, namespace).fact
  private lazy val timesPos: ProvableSig =
    remember("(f_() >= 0 & g_() >= 0) -> f_() * g_() >= 0".asFormula, QE, namespace).fact

  private lazy val neGtSquared: ProvableSig =
    remember(" f_() != 0 & g_() > 0 -> f_()^2 * g_() > 0 ".asFormula, QE, namespace).fact
  private lazy val plusGtMono: ProvableSig =
    remember("(f_() > k_() & g_() >= 0) -> f_() + g_() > k_()".asFormula, QE, namespace).fact

  private lazy val gtNotZero: ProvableSig = remember(
    "f_() > 0 -> !(f_() = 0)".asFormula,
    prop & exhaustiveEqL2R(-2) & SimplifierV3.simpTac()(-1) & close,
    namespace,
  ).fact
  private lazy val axMov: ProvableSig = remember(
    "f_() + a_() * g_() = k_() -> (a_() = 0 -> f_() = k_())".asFormula,
    prop & exhaustiveEqL2R(-2) & SimplifierV3.simpTac()(-1) & close,
    namespace,
  ).fact
  ...
  private lazy val neAnteZ: ProvableSig = remember("f_() != g_() <-> !!(f_()-g_() !=0)".asFormula, QE & done, namespace)
    .fact
  private lazy val ltAnteZ: ProvableSig =
    remember("f_() < g_() <-> f_() <= g_() & f_() - g_() != 0 ".asFormula, QE, namespace).fact
  private lazy val gtAnteZ: ProvableSig =
    remember("f_() > g_() <-> f_() >= g_() & f_() - g_() != 0 ".asFormula, QE, namespace).fact

  private lazy val existsOr1 = remember(
    "(\\exists x_ p_(x_) | \\exists y_ q_(y_)) <-> (\\exists x_ (p_(x_) |  q_(x_)))".asFormula,
    prop &
      OnAll(existsL(Symbol("L")) & prop) <
      (
        existsR(Symbol("R")),
        existsR(Symbol("R")),
        existsR("y_".asTerm)(Symbol("R")),
        existsR("x_".asTerm)(Symbol("Rlast")),
      ) & OnAll(prop),
    namespace,
  ).fact

  private lazy val existsSame = remember(
    "(\\exists x_ p_(x_) | \\exists x_ q_(x_)) <-> (\\exists x_ (p_(x_) |  q_(x_)))".asFormula,
    prop &
      OnAll(existsL(Symbol("L")) & prop) <
      (
        existsR(Symbol("R")),
        existsR(Symbol("R")),
        existsR("x_".asTerm)(Symbol("R")),
        existsR("x_".asTerm)(Symbol("Rlast")),
      ) & OnAll(prop),
    namespace,
  ).fact

  private lazy val existsOr2 = remember(
    "\\exists x_ p_(x_) | q_() <-> (\\exists x_ (p_(x_) |  q_()))".asFormula,
    prop & OnAll(SaturateTactic(existsL(Symbol("L"))) & SaturateTactic(existsR(Symbol("R"))) & prop),
    namespace,
  ).fact

  private lazy val existsOr3 = remember(
    "q_() | \\exists x_ p_(x_) <-> (\\exists x_ (p_(x_) |  q_()))".asFormula,
    prop & OnAll(SaturateTactic(existsL(Symbol("L"))) & SaturateTactic(existsR(Symbol("R"))) & prop),
    namespace,
  ).fact

  private lazy val existsAnd1 = remember(
    "(\\exists x_ p_(x_) & \\exists y_ q_(y_)) <-> (\\exists x_ \\exists y_ (p_(x_) & q_(y_)))".asFormula,
    prop &
      OnAll(SaturateTactic(existsL(Symbol("L"))) & prop) <
      (existsR(Symbol("R")) & existsR(Symbol("R")) & prop, existsR(Symbol("R")) & prop, existsR(Symbol("R")) & prop),
    namespace,
  ).fact

  private lazy val existsAnd2 = remember(
    "(\\exists x_ p_(x_) & q_()) <-> (\\exists x_ (p_(x_) & q_()))".asFormula,
    prop & OnAll(SaturateTactic(existsL(Symbol("L"))) & SaturateTactic(existsR(Symbol("R"))) & prop),
    namespace,
  ).fact

  private lazy val existsAnd3 = remember(
    "(q_() & \\exists x_ p_(x_)) <-> (\\exists x_ (p_(x_) & q_()))".asFormula,
    prop & OnAll(SaturateTactic(existsL(Symbol("L"))) & SaturateTactic(existsR(Symbol("R"))) & prop),
    namespace,
  ).fact

  private lazy val existsRename = remember(
    "(\\exists x_ p_(x_) & \\exists x_ q_(x_)) <-> (\\exists x_ p_(x_) & \\exists z_ q_(z_))".asFormula,
    prop &
      OnAll(SaturateTactic(existsL(Symbol("L"))) & prop) <
      (existsR("x_".asTerm)(Symbol("R")), existsR("z_".asTerm)(Symbol("R"))) & OnAll(prop),
    namespace,
  ).fact
  ...
  private lazy val divNeq =
    remember("!(G_()=0) -> (F_()/G_() != 0) -> F_() != 0".asFormula, QE, namespace).fact // Derivable from the above
  ...
  def checkMono(t: Term, maxs: Option[Term] = None): Boolean = {
    ...
    t match {
      ...
      case Times(l, Power(s, n: Number)) if isVar(s) =>
        n.value.isValidInt && n.value.toInt > 0 && maxs.forall(t => VarOrd.compare(s, t) < 0) && checkMono(l, Some(s))
      case _ => false
    }
  }
  ...
  object MonOrd extends Ordering[Term] {
    ...
    def compare(l: Term, r: Term): Int = {
      ...
      if (ol < or) -1 else if (ol > or) 1 else lexMono(l, r)
    }
  }

  def gcd(a: BigDecimal, b: BigDecimal): BigDecimal = { if (b == 0) a else gcd(b, a % b) }

  def checkCoeff(t: Term): Boolean = {
    t match {
      case Divide(numer: Number, denom: Number) => numer.value != 0 && denom.value > 0 && gcd(numer.value, denom.value)
          .abs == 1
      case _ => false
    }
  }
  ...
  def checkPoly(t: Term, maxm: Option[Term] = None): Boolean = {
    ...
    t match {
      ...
      case Plus(l, Times(c, m)) => checkCoeff(c) && checkMono(m) &&
        (maxm match {
          case None => checkPoly(l, Some(m))
          case Some(n) => { MonOrd.compare(m, n) < 0 && checkPoly(l, Some(m)) }
        })
      case _ => false
    }
  }
      ...
      case (Plus(nl, Times(cl, ml)), Plus(nr, Times(cr, mr))) => {
        ...
        if (cmp < 0) {
          ...
          (
            ...
            prover(
              Equal(lhs, res),
              useAt(plusAssoc1)(SuccPosition(1, 0 :: Nil)) & useAt(pr)(SuccPosition(1, 0 :: 0 :: Nil)) &
          ...
          if (isZero) // Canceling out the 0
            (
              ...
              prover(
                Equal(lhs, rec),
                useAt(plusAssoc2)(SuccPosition(1, 0 :: Nil)) & useAt(pr)(SuccPosition(1, 0 :: 0 :: Nil)) &
      ...
      case (Times(nl, Power(sl, ml: Number)), Times(nr, Power(sr, mr: Number))) =>
        ...
        if (cmp < 0) {
          ...
          (
            ...
            prover(
              Equal(lhs, res),
              useAt(timesAssoc1)(SuccPosition(1, 0 :: Nil)) & useAt(pr)(SuccPosition(1, 0 :: 0 :: Nil)) &
                byUS(Ax.equalReflexive),
            ),
          )
        } else if (cmp == 0) {
          ...
          if (mnew == 0) // Canceling out the 0
            (
              ...
              prover(
                Equal(lhs, rec),
                useAt(timesAssoc2)(SuccPosition(1, 0 :: Nil)) & useAt(pr)(SuccPosition(1, 0 :: 0 :: Nil)) &
    ...
    l match {
      ...
      case Plus(nl, Times(cl, ml)) =>
        ...
        (
          ...
          prover(
            Equal(lhs, res),
            useAt(timesAssoc3)(SuccPosition(1, 0 :: Nil)) & useAt(pr1)(SuccPosition(1, 0 :: 0 :: Nil)) &
              useAt(pr2)(SuccPosition(1, 0 :: 1 :: 1 :: Nil)) & useAt(plusCoeff2, PosInExpr(1 :: Nil))(1)
    ...
    r match {
      ...
      case Plus(nr, Times(cr, mr)) =>
        ...
        (
          ...
          prover(
            Equal(lhs, res),
            useAt(Ax.distributive)(SuccPosition(1, 0 :: Nil)) & useAt(pr1)(SuccPosition(1, 0 :: 0 :: Nil)) &
              useAt(pr2)(SuccPosition(1, 0 :: 1 :: Nil)) & by(pr3),
          ),
        )
  ...
  def iterSquare(l: Term, p: Int, skip_proofs: Boolean = false): (Term, ProvableSig) = {
    ...
    if (p == 0) (Number(1), prover(Equal(lhs, Number(1)), byUS(powLem1)))
      ...
      (
        ...
        prover(
          Equal(lhs, rhs),
          useAt(pr, PosInExpr(1 :: Nil))(SuccPosition(1, 1 :: 0 :: Nil)) & useAt(powLem3)(SuccPosition(1, 1 :: Nil)) &
      ...
      (
        ...
        prover(
          ...
          useAt(pr, PosInExpr(1 :: Nil))(SuccPosition(1, 1 :: 0 :: 0 :: Nil)) &
            useAt(powLem4)(SuccPosition(1, 1 :: Nil)) & useAt(powLem5, PosInExpr(1 :: Nil))(1) & RCF,
        ),
      )
    }
  }
  ...
  def groundNormalise(t: Term): Option[Term] = {
    t match {
      ...
      case bop: BinaryCompositeTerm => (groundNormalise(bop.left), groundNormalise(bop.right)) match {
          case (Some(l), Some(r)) => bop match {
      ...
      case Neg(u) => groundNormalise(u).map(t => mulCoeff(Divide(Number(-1), Number(1)), t))
      case _ => None
    }
  }
  ...
  def isMono(t: Term): Boolean = {
    t match {
      case Times(l, r) => isMono(l) && isMono(r)
      case Power(l, r: Number) => isMono(l) && r.value.isValidInt
...
//    }
    val res = l match {
      ...
      case Plus(ln, rn) =>
        ...
        (
          ...
          prover(
            Equal(l, res),
            useAt(pr1)(SuccPosition(1, 0 :: 0 :: Nil)) & useAt(pr2)(SuccPosition(1, 0 :: 1 :: Nil)) & by(pr3),
          ),
        )
      case Times(ln, rn) =>
        val (rec1, pr1) = normalise(ln, skip_proofs)
      ...
      case Power(ln, n: Number) if n.value == 2 =>
        ...
        (
          ...
          prover(
            Equal(l, res),
            useAt(pr1)(SuccPosition(1, 0 :: 0 :: Nil)) & useAt(powNormalise)(SuccPosition(1, 0 :: Nil)) & by(pr2),
          ),
        )
      case Power(ln, n: Number) if n.value.isValidInt =>
        ...
        (res, prover(Equal(l, res), useAt(pr1)(SuccPosition(1, 0 :: Nil)) & by(pr2)))
      ...
      case Neg(ln) =>
        ...
        (
          ...
          prover(
            Equal(l, res),
            useAt(pr1)(SuccPosition(1, 0 :: 0 :: Nil)) & useAt(negNormalise)(SuccPosition(1, 0 :: Nil)) & by(pr2),
          ),
        )
      case Minus(ln, rn) =>
        ...
        (
          ...
          prover(
            Equal(l, res),
            useAt(minusNormalise)(SuccPosition(1, 0 :: Nil)) & useAt(pr1)(SuccPosition(1, 0 :: 0 :: Nil)) &
              useAt(pr2)(SuccPosition(1, 0 :: 1 :: Nil)) & by(pr3),
          ),
        )
        ...
        propt match {
          ...
          case Some((n, pr)) => {
            ...
            (
              ...
              prover(
                Equal(l, res),
                useAt(divNormalise)(SuccPosition(1, 0 :: Nil)) & useAt(pr)(SuccPosition(1, 0 :: 0 :: Nil)) & by(pr2),
              ),
            )
          }
        }
    ...
    (l, r) match {
      ...
      case (Times(nl, Power(sl, ml: Number)), Times(nr, Power(sr, mr: Number))) =>
        ...
        if (cmp < 0) None
          ...
          val mnew = ml.value - mr.value
          if (mnew < 0) None
          else divMono(nl, nr) match {
            case None => None
            case Some(q) => Some(if (mnew == 0) q else Times(q, Power(sr, Number(mnew))))
          }
        } else {
          divMono(nl, r) match {
            case None => None
  ...
  def divPolyMono(l: Term, r: Term): Option[(Term, Term)] = {
    l match {
      ...
      case Plus(nl, Times(cl, ml)) => divMono(ml, r) match {
  ...
  def negDivCoeff(cl: Term, cr: Term): Term = {
    (cl, cr) match {
      case (Divide(lnum: Number, lden: Number), Divide(rnum: Number, rden: Number)) =>
        ...
        else Divide(Number(-num / normalizer), Number(den / normalizer))
      case _ => ???
    }
  }
  ...
  def divPoly(l: Term, r: Term): Option[(Term, Term)] = {
    r match {
      ...
      case Plus(nr, Times(cr, mr)) => divPolyMono(l, mr) match {
  ...
  def reduction(l: List[Term], r: Term): List[(Int, Term)] = {
    firstDivisor(l, 0, r) match {
      case None => Nil
      case Some((i, d, q)) => (i, d) :: reduction(l, q)
    }
  }
    ...
    l match {
      ...
      case ((a, s) :: xs) =>
        ...
        (
          res,
          proveBy(
            GreaterEqual(res, Number(0)), // (foo) + x^2 >0
            useAt(plusGeMono, PosInExpr(1 :: Nil))(1) &
              andR(1) < (by(pr), useAt(timesPos, PosInExpr(1 :: Nil))(1) & andR(1) < (RCF, byUS(Ax.nonnegativeSquares))),
          ),
        )
    }
  }
  ...
  def neqGtZero(l: List[Int], sequent: Sequent): (Term, BelleExpr) = {
    ...
    val ineqPos = antes.indexWhere(_ match {
    ...
    val tac = l.foldRight(cohideR(1) & by(Ax.oneGreaterZero): BelleExpr)((e, n) => {
  ...
  def genWitness(gtz: Term, l: List[(Term, Term)]): (Term, ProvableSig) = {
    ...
    (
      trm,
      proveBy(
        Imply(Greater(gtz, Number(0)), Greater(trm, Number(0))),
  ...
  def genWitnessTac(
      ...
      instopt: Option[List[(Int, Term)]] = None,
  ): DependentTactic = anon((sequent: Sequent) => {
    ...
    cut(pf.conclusion.succ.head) <
      (
        implyL(Symbol("Llast")) < (tac & id, ident) & notL(Symbol("Llast")) &
          // Run the instructions
          inst.foldRight[BelleExpr](ident)((h, tac) =>
            implyRi(keep = true)(AntePos(h._1), SuccPos(0)) & useAt(
              axMov,
              PosInExpr(1 :: Nil),
              (us: Option[Subst]) => us.get ++ RenUSubst(("g_()".asTerm, h._2) :: Nil),
            )(1) & tac
