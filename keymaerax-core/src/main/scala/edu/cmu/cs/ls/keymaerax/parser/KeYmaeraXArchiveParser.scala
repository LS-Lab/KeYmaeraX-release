    ...
    StaticSemantics.symbols(expr).forall({
      case f:Function =>
        val (declaredDomain,declaredSort, interpretation, loc) = d.decls.get((f.name,f.index)) match {
          case Some(decl) => decl
          case None => throw ParseException("type analysis" + ": " + "undefined symbol " + f + " Make sure to declare ProgramVariable and other Definitions.", f)
        }
      ...
      case x: Variable =>
        val (declaredSort, declLoc) = d.decls.get((x.name,x.index)) match {
          ...
          case None => throw ParseException("type analysis" + ": " + "undefined symbol " + x + " with index " + x.index + " Make sure to declare ProgramVariable and other Definitions.", x)
        }
