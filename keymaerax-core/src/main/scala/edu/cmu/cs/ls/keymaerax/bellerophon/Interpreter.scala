...
trait Interpreter {
  ...
  /**
   * Returns the result of applying tactic `expr` to the proof value `v` (usually a provable). Interpreter must be
   * started before executing tactics.
   */
  ...
  protected def applySubDerivation(
      ...
      n: Int,
      subderivation: ProvableSig,
      subst: USubst,
  ): (Boolean, ProvableSig) = {
    assert(
      original.subgoals.length > n,
      s"$n is a bad index for Provable with ${original.subgoals.length} subgoals: $original",
    )
    val (substParent, substChild) =
      if (original.subgoals(n) == subderivation.conclusion) (original, subderivation)
        ...
        exhaustiveSubst(original, subst),
        exhaustiveSubst(
          subderivation,
          ...
          USubst(
            subst
              .subsDefsInput
              .filter({
                case SubstitutionPair(FuncOf(_, Nothing), _: Variable) => false
                case _ => true
              })
          ),
        ),
      ...
      r match {
        case (rmerged: Boolean, rp: ProvableSig) => (!rmerged && rp == subderivation) ||
          (rmerged && exhaustiveSubst(rp, subst).conclusion == exhaustiveSubst(original, subst).conclusion &&
            (if (subderivation.isProved) { rp.subgoals.size == original.subgoals.size - 1 }
             else {
               subst(rp.subgoals(n)) == subst(subderivation.subgoals(0)) &&
               rp.subgoals.takeRight(subderivation.subgoals.size - 1).map(subst(_)) ==
                 subderivation.subgoals.tail.map(subst(_))
             }))
      }
    )

  /**
   * Assert that the conclusion of provable `sub` matches the subgoal `n` of provable `parent` either verbatim or modulo
   * constification renaming that is assumed to be applied in the future. Constification renaming requires `parent` to
   * have exactly one single subgoal.
   */
  protected def assertSubMatchesModuloConstification(
      parent: ProvableSig,
      sub: ProvableSig,
      n: Int,
      subst: USubst,
    ...
    if (
      ProvableHelper.exhaustiveSubst(parent.underlyingProvable, subst).subgoals(n) != Try(
        ProvableHelper.exhaustiveSubst(sub.underlyingProvable, subst)
      ).toOption.getOrElse(sub.underlyingProvable).conclusion &&
  ...
  /**
   * Assert that the conclusion of provable `sub` matches the conclusion of provable `parent` either verbatim or modulo
   * constification renaming that is assumed to be applied in the future.
   */
  protected def assertConclusionsMatchModuloConstification(
      parent: ProvableSig,
      sub: ProvableSig,
      subst: USubst,
  ): Unit = {
  ...
  private def subMatchesModuloConstification(parent: Sequent, sub: Sequent, subst: USubst): Boolean = {
    if (subst(parent) != subst(sub)) {
      ...
      val constifications = subst
        .subsDefsInput
        .flatMap({
          case SubstitutionPair(f @ FuncOf(Function(fn, fi, Unit, Real, _), Nothing), v: Variable)
              if fn == v.name && fi == v.index => Some(f -> v)
          case _ => None
        })
