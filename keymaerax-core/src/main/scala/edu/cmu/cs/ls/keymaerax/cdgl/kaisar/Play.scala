...
class Environment[number <: Numeric[number, Ternary]](val factory: NumberFactory[Ternary, number]) {
  ...
  /**
   * Unindexed variable x always represents "current value," but indexed "x_i" also remembered for history. This sets
   * both.
   */
  def set(x: Ident, v: number): Unit = {
    val base = x match {
      case bv: BaseVariable => Variable(bv.name)
      case ds: DifferentialSymbol => DifferentialSymbol(Variable(ds.name))
    }
...
class Play[N <: Numeric[N, Ternary]](factory: NumberFactory[Ternary, N]) {
  ...
  def apply(env: Environment[number], as: AngelStrategy, ds: DemonStrategy[number]): Unit = {
    as match {
        ...
        try {
          ...
          if (fails || alwaysPrint) {
            ...
            val locStr = mapped match {
              case Some(it) => it
              case None => "at unknown source location"
            }
            ...
            if (fails) {
              ds.reportViolation()
              if (!Play.continueOnViolation) throw TestFailureException(as.nodeID)
            }
          }
        } catch { case v: NoValueException => throw NoValueException(as.nodeID, msg = v.msg) }
      case SAssign(x, f) =>
        try {
          val v = env.eval(f)
      ...
      case SChoice(l, r) => if (ds.readChoice(as.nodeID)) apply(env, r, ds) else apply(env, l, ds)
