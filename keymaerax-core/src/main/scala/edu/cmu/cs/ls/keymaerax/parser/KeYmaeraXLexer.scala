...
object KeYmaeraXLexer extends (String => List[Token]) with Logging {
  ...
  def normalizeNewlines(input: String): String = input
    .replace("\r\n", "\n")
    .replace("\r", "\n")
    .replaceAll(" *\n", "\n")

  /**
   * The lexer has multiple modes for the different sorts of files that are supported by KeYmaeraX. The lexer will
  ...
  @tailrec
  private def replaceAnything(
      output: scala.collection.mutable.ListBuffer[Token]
  ): scala.collection.mutable.ListBuffer[Token] = {
  ...
  private def consumeColumns(
      ...
      terminal: Terminal,
      location: Location,
  ): Option[(String, Token, Location)] = {
    assert(cols > 0, "Cannot move cursor less than 1 columns.")
    Some((s.substring(cols), Token(terminal, spanningRegion(location, cols - 1)), suffixOf(location, cols)))
  }

  private def consumeTerminalLength(
      s: String,
      terminal: Terminal,
      location: Location,
  ): Option[(String, Token, Location)] = consumeColumns(s, terminal.img.length, terminal, location)

  private def consumeUnicodeTerminalLength(
      s: String,
      terminal: Terminal,
      location: Location,
      replacementTerminal: Terminal,
  ): Option[(String, Token, Location)] = {
    consumeColumns(s, terminal.img.length, terminal, location).map({ case (st, t, l) =>
      (st, Token(replacementTerminal, t.loc), l)
    })
  }

  private val lexers: Seq[
    (
        Regex,
        (String, Location, LexerMode, String) => Either[(String, Location, LexerMode), Option[
    ...
    comment ->
      ((s: String, loc: Location, mode: LexerMode, theComment: String) => {
    ...
    newline ->
      ((s: String, loc: Location, mode: LexerMode, _) =>
        Left((
          s.tail,
          loc match {
            case UnknownLocation => UnknownLocation
    ...
    whitespace ->
      ((s: String, loc: Location, mode: LexerMode, spaces: String) =>
        Left((
    ...
    TURNSTILE.startPattern ->
      ((s: String, loc: Location, mode: LexerMode, _) =>
        mode match {
          case LemmaFileMode | StoredProvableMode => Right(consumeTerminalLength(s, TURNSTILE, loc))
          case _ =>
            throw new Exception("Encountered a turnstile symbol ==> in a non-lemma file or non-storedprovable input")
        }
      ),
    FORMULA_BEGIN.startPattern ->
      ((s: String, loc: Location, mode: LexerMode, _) =>
        mode match {
    ...
    DOT.startPattern ->
      ((s: String, loc: Location, _: LexerMode, dot: String) => {
        val (_, idx) = splitName(dot); Right(consumeTerminalLength(s, DOT(idx), loc))
      }),
    // File cases
    PERIOD.startPattern ->
      ((s: String, loc: Location, _: LexerMode, _) =>
        Right(consumeTerminalLength(s, PERIOD, loc))
      ), // swapOutFor(consumeTerminalLength(PERIOD, loc), DOT)
    ARCHIVE_ENTRY_BEGIN.startPattern ->
      ((s: String, loc: Location, mode: LexerMode, kind: String) =>
        mode match {
          ...
          case _ => throw new Exception(
              "Encountered ``" + ARCHIVE_ENTRY_BEGIN(kind).img + "`` in non-problem file lexing mode."
            )
        }
      ),
    FUNCTIONS_BLOCK.startPattern ->
      ((s: String, loc: Location, mode: LexerMode, _) =>
        mode match {
    ...
    DEFINITIONS_BLOCK.startPattern ->
      ((s: String, loc: Location, mode: LexerMode, _) =>
        mode match {
          case AxiomFileMode | ProblemFileMode | LemmaFileMode =>
            Right(consumeTerminalLength(s, DEFINITIONS_BLOCK, loc))
          case _ => throw new Exception("Definitions. should only occur when processing files.")
        }
      ),
    PROGRAM_VARIABLES_BLOCK.startPattern ->
      ((s: String, loc: Location, mode: LexerMode, _) =>
        mode match {
          case AxiomFileMode | ProblemFileMode | LemmaFileMode =>
            Right(consumeTerminalLength(s, PROGRAM_VARIABLES_BLOCK, loc))
          case _ => throw new Exception("ProgramVariables. should only occur when processing files.")
        }
      ),
    VARIABLES_BLOCK.startPattern ->
      ((s: String, loc: Location, mode: LexerMode, _) =>
        mode match {
    ...
    SHARED_DEFINITIONS_BEGIN.startPattern ->
      ((s: String, loc: Location, mode: LexerMode, _) =>
        mode match {
          case ProblemFileMode => Right(consumeTerminalLength(s, SHARED_DEFINITIONS_BEGIN, loc))
          case _ =>
            throw new Exception("SharedDefinitions./End. sections should only occur when processing .kyx files.")
        }
      ),
    // Lemma file cases (2)
    TOOL_VALUE_PAT.startPattern ->
      ((s: String, loc: Location, mode: LexerMode, str: String) =>
        mode match { // @note must be before DOUBLE_QUOTES_STRING
          case LemmaFileMode =>
            ...
            Right(consumeColumns(
              s,
              str.length,
              TOOL_VALUE(str.stripPrefix("\"\"\"\"").stripSuffix("\"\"\"\"").stripPrefix(" ").stripSuffix(" ")),
              loc,
            ))
    ...
    DOUBLE_QUOTES_STRING_PAT.startPattern ->
      ((s: String, loc: Location, mode: LexerMode, str: String) =>
        mode match {
          case AxiomFileMode | LemmaFileMode | ProblemFileMode =>
            ...
            Right(Some((
              ...
              Token(
                terminal,
                Region(loc.begin.line, loc.begin.column, loc.begin.line + n, c - 1),
              ), // without \n character
    ...
    GREATEREQ_UNICODE.startPattern ->
      ((s: String, loc: Location, _, _) => Right(consumeUnicodeTerminalLength(s, GREATEREQ_UNICODE, loc, GREATEREQ))),
    LESSEQ.startPattern -> ((s: String, loc: Location, _, _) => Right(consumeTerminalLength(s, LESSEQ, loc))),
    LESSEQ_UNICODE.startPattern ->
      ((s: String, loc: Location, _, _) => Right(consumeUnicodeTerminalLength(s, LESSEQ_UNICODE, loc, LESSEQ))),
    ...
    AND_UNICODE.startPattern ->
      ((s: String, loc: Location, _, _) => Right(consumeUnicodeTerminalLength(s, AND_UNICODE, loc, AMP))),
    ...
    OR_UNICODE.startPattern ->
      ((s: String, loc: Location, _, _) => Right(consumeUnicodeTerminalLength(s, OR_UNICODE, loc, OR))),
    EQUIV.startPattern -> ((s: String, loc: Location, _, _) => Right(consumeTerminalLength(s, EQUIV, loc))),
    EQUIV_UNICODE.startPattern ->
      ((s: String, loc: Location, _, _) => Right(consumeUnicodeTerminalLength(s, EQUIV_UNICODE, loc, EQUIV))),
    IMPLY.startPattern -> ((s: String, loc: Location, _, _) => Right(consumeTerminalLength(s, IMPLY, loc))),
    IMPLY_UNICODE.startPattern ->
      ((s: String, loc: Location, _, _) => Right(consumeUnicodeTerminalLength(s, IMPLY_UNICODE, loc, IMPLY))),
    REVIMPLY.startPattern -> ((s: String, loc: Location, _, _) => Right(consumeTerminalLength(s, REVIMPLY, loc))),
    REVIMPLY_UNICODE.startPattern ->
      ((s: String, loc: Location, _, _) => Right(consumeUnicodeTerminalLength(s, REVIMPLY_UNICODE, loc, REVIMPLY))),
    //
    FORALL.startPattern -> ((s: String, loc: Location, _, _) => Right(consumeTerminalLength(s, FORALL, loc))),
    FORALL_UNICODE.startPattern ->
      ((s: String, loc: Location, _, _) => Right(consumeUnicodeTerminalLength(s, FORALL_UNICODE, loc, FORALL))),
    EXISTS.startPattern -> ((s: String, loc: Location, _, _) => Right(consumeTerminalLength(s, EXISTS, loc))),
    EXISTS_UNICODE.startPattern ->
      ((s: String, loc: Location, _, _) => Right(consumeUnicodeTerminalLength(s, EXISTS_UNICODE, loc, EXISTS))),
    //
    EQ.startPattern -> ((s: String, loc: Location, _, _) => Right(consumeTerminalLength(s, EQ, loc))),
    UNEQUAL_UNICODE.startPattern ->
      ((s: String, loc: Location, _, _) => Right(consumeUnicodeTerminalLength(s, UNEQUAL_UNICODE, loc, NOTEQ))),
    ...
    //
    ANYTHING.startPattern ->
      ((s: String, loc: Location, _, _) =>
        Right(consumeTerminalLength(s, ANYTHING, loc))
      ), // @note this token is stripped out and replaced with (! !) in [[fin`dNextToken]].
    //
    ...
    //
    FORMULA_SEPARATOR.startPattern ->
      ((s: String, loc: Location, mode, _) =>
        mode match {
          case LemmaFileMode | StoredProvableMode => Right(consumeTerminalLength(s, FORMULA_SEPARATOR, loc))
          case _ => throw new Exception(
              "Encountered a formula separator symbol " + FORMULA_SEPARATOR.img + " in a non-storedprovable input"
            )
        }
      ),
    FROM.startPattern ->
    ...
    //
    IDENT.startPattern ->
      ((s: String, loc: Location, _, name: String) => {
        val (n, idx) = splitName(name)
        Right(consumeTerminalLength(s, IDENT(n, idx), loc))
      }),
    NUMBER.startPattern ->
      ((s: String, loc: Location, _, n: String) => Right(consumeTerminalLength(s, NUMBER(n), loc))),
    ...
    //
    EXERCISE_PLACEHOLDER.startPattern ->
      ((s: String, loc: Location, _, _) => Right(consumeTerminalLength(s, EXERCISE_PLACEHOLDER, loc))),
    //
    ...
    QUOTATION_MARK.startPattern ->
      ((s: String, loc: Location, _, _) => Right(consumeTerminalLength(s, QUOTATION_MARK, loc))),
  )
    ...
    while (next.isLeft) {
      ...
      if (cs.isEmpty) next = Right(None)
        ...
        val lexPrefix = lexers
          .view
          .map({ case (r, lexer) => r.findPrefixOf(cs).map(lexer(cs, cloc, cmode, _)) })
          .find(_.isDefined)
          .flatten
        lexPrefix match {
  ...
  private def splitName(s: String): (String, Option[Int]) =
    if (s.contains("_") && !s.endsWith("_")) {
      ...
      val (namePart, idxPart) = {
        ...
        (s.substring(0, finalUnderscoreIdx), s.substring(finalUnderscoreIdx + 1, s.length))
      }
