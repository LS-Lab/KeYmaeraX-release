...
/**
  ...
  * @TODO: Revisit design when library gets bigger
...
object TermTactics {
  ...
  def and(args: Proof*): Proof = {
    args.reduce(AndI)
  }

  def and(i: Int): Proof = {
    List.tabulate(i)(i => Hyp (i)).reduce(AndI)
  }

  def and(G: Context): Proof = {
    and(G.asIndexedSeq.length)
  }
}
