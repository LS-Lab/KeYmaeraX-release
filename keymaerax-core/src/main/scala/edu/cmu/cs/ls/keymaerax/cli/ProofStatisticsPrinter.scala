...
object CsvProofStatisticsPrinter extends BaseProofStatisticsPrinter {
  /** @inheritdoc */
  override def csvHeader: String = "Name,Tactic,Status,Timeout,Duration,QE duration,RCF duration,Proof steps,Tactic size"

  /** @inheritdoc */
  override def toCsv(ps: ProofStatistics): String = s"${ps.name},${ps.tacticName},${ps.status},${ps.timeout*1000},${ps.duration},${ps.qeDuration},${ps.rcfDuration},${ps.proofSteps},${ps.tacticSize}"
}
...
object ArchNLNCsvProofStatisticsPrinter extends BaseProofStatisticsPrinter {
  ...
  /** @inheritdoc */
  override def toCsv(ps: ProofStatistics): String = s"${ps.name},${ps.tacticName},${ps.status},${ps.duration},0,${ps.proofSteps}"
}
