...
import edu.cmu.cs.ls.keymaerax.btactics.{Kaisar => _, _}
...
object StrategyExtractorMain {
  ...
  def withTemporaryConfig(tempConfig: Map[String, String])(testcode: => Any): Unit = Configuration
  ...
  private val mathematicaProvider: Lazy[DelayedShutdownToolProvider] =
    new Lazy(new DelayedShutdownToolProvider(MathematicaToolProvider(ToolConfiguration.config(WOLFRAM.toLowerCase))))
  ...
  private def initQE(): Unit = {
    ...
    withTemporaryConfig(common) {
      ...
      val toolMap =
        Map("linkName" -> "C:\\Program Files\\Wolfram Research\\Mathematica\\SystemFiles\\Links\\JLink\\JLink.jar")
      val tool = provider.defaultTool() match {
        case Some(m: Mathematica) => m
        case _ =>
          throw new Exception("Illegal Wolfram tool, please use one of 'Mathematica' or 'Wolfram Engine' in test setup")
      }
      KeYmaeraXTool.init(Map(KeYmaeraXTool.INTERPRETER -> LazySequentialInterpreter.getClass.getSimpleName))
    }
  }
