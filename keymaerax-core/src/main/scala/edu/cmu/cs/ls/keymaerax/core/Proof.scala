...
final case class Sequent(ante: immutable.IndexedSeq[Formula], succ: immutable.IndexedSeq[Formula]) {
  ...
  def glue(s: Sequent): Sequent = {
    ...
    } ensures(r => this.subsequentOf(r) && s.subsequentOf(r)
...
object Provable {
  ...
  "edu.cmu.cs.ls.keymaerax.tools.Mathematica" :: "edu.cmu.cs.ls.keymaerax.tools.qe.Z3" ::
    (if (LAX_MODE) "edu.cmu.cs.ls.keymaerax.tools.Polya" :: "edu.cmu.cs.ls.keymaerax.tools.BigDecimalQETool$" :: Nil else Nil)
