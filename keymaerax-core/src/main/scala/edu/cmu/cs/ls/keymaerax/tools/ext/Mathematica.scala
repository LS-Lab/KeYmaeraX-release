...
class Mathematica(private[tools] val link: MathematicaLink, override val name: String) extends Tool
  ...
  override def sosSolve(polynomials: List[Term], variables: List[Term], degree: Int, timeout: Option[Int]): Option[(Term, List[Term])] = mSOSsolve.sosSolve(polynomials, variables, degree, timeout)
