...
case class RefinementFailure(s: Statement, g: Program, extraMsg: String = "", node: ASTNode = Triv())
...
object RefinementChecker {
   ...
   *   Hybrid program
   * @return
   *   (hd, tl) where hd is an initial block and tl is the remainder of the program, if any
   */
  private def unappend(game: Program): (Program, Program) = {
    ...
    def recombine(l: Program, x: Program, r: Program): (Program, Program) = {
      if (isPhiBlock(x)) (Compose(l, x), r) else (l, Compose(x, r))
    }
    game match {
      // ; is self-dual
      case Compose(Dual(Compose(l, x)), r) => unappend(Compose(Dual(l), Compose(Dual(x), r)))
   ...
   *   Statement
   * @return
   *   Statement s with any top-level [[Phi]] node stripped. Return value has same dynamics as [[s]]
   */
  ...
  private def commutativeMods(ss: List[Statement]): Boolean = {
    val mods = ss.flatMap({
      case mod: Modify => List(mod)
      case _ => Nil
    })
    if (mods.length < ss.length) return false // because some statement was not a Modify
  ...
  /**
   * Attempts to soundly reorder statement list so that statement corresponding to a game appears first. This is
   * particularly useful when auto-generated proof statements are generated in some unknown arbitrary order.
   * @param ss
   *   List of Kaisar statements to be reordered
   * @param ghd
   *   Game by which to reorder ss. Is "head" in list traversal of games
   * @return
   *   Statement list equivalent to ss which, if possible, begins with statement corresponding to game ghd
   */
  private def sortBy(ss: List[Statement], ghd: Program): List[Statement] = {
    ...
    (ss, ghd) match {
      ...
      case (Phi(Block(ss)) :: tail, as: Assign) if commutativeMods(ss) => sortBlock(ss, tail, as, isPhi = true)
      case (Block(ss) :: tail, as: Assign) if commutativeMods(ss) => sortBlock(ss, tail, as, isPhi = false)
      case _ => ss
    }
  }
  ...
  /**
   * @param s
   *   Kaisar proof statement
   * @param gvs
   *   Set of variables mentioned in games (non-ghost variable set)
   * @return
   *   s with no-op statements stripped: explicit ghosts, implicit ghost phi assignments, meta nodes
   */
  ...
  private def beIgnorant(s: Statement, gvs: Set[Variable]): List[Statement] = {
    ...
    KaisarProof.flatten(List(s)) match {
      ...
      case flat =>
        val dropped = flat.dropWhile(isIgnored)
  ...
  /**
   * @return
   *   unit if phi block and program make same phi assignments possibly in different order and with possible duplicate
   *   assignments to the loop index due to silly artifacts of SSA pass
   * @throws RefinementFailure
   *   if headers don't agree
   */
  private def checkForHeaderAgrees(pf: Phi, game: Program, loopIndex: Variable): Unit = {
    ...
    val ssAssigns = ss.map({
      case Modify(_, List((x, Some(f)))) => Assign(x, f)
      case _ => throw new Exception("Bad pattern match in RefinementChecker.forHeaderAgrees")
    })
    val lSet = ssAssigns.toSet
    val rSet = games.toSet
    val lDiff = lSet -- rSet
    val rDiff = rSet -- lSet
    val lAdmiss = lDiff.filter({
      case x if x.x == loopIndex => false
      case _ => true
    })
    if (rDiff.nonEmpty) {
        ...
        (unphi(shd), ghd) match {
          ...
          case (Block((phi: Phi) :: (fr @ For(metX, met0, metIncr, conv, guard, body, guardEps)) :: Nil), assignMetZ) =>
            ...
            val (loopGame, cont) = loopAndCont match {
              ...
              case _ => throw RefinementFailure(
                  fr,
                  loopAndCont,
                  "Refining for-loop, expected game to have Angelic loop  {{body}*}^@ but couldn't find it",
                )
            }
            val (gameBody, bodyAssigns) = loopGame match {
              ...
              case _ => throw RefinementFailure(
                  fr,
                  loopGame,
                  "Refining for-loop, loop body must have fixed format {body;^@; assigns}  or {body; assigns}^@",
                )
            }
            ...
            if (incr != Assign(metXFooter, metIncr)) {
              throw RefinementFailure(
                fr,
                loopGame,
                "Refining for-loop, loop body must have fixed format  {{body;}^@ incr;} where incr increments the loop index",
              )
            }
            if (assignMetZ != Assign(metX, met0)) {
              ...
              throw RefinementFailure(
                mod,
                ghd,
                "Refining for-loop, game must start with assignment of index variable to initial value",
              )
            }
            val (plainBody, Phi(proofBodyRawPhi)) = stripLoopFooter(body)
            val proofBodyPhi =
              Phi(KaisarProof.block(Modify(Nil, List((metX, Some(metXFooter)))) :: proofBodyRawPhi :: Nil))
            ...
            val theTail = cont match {
              case None => Test(True) // end of game reached
              case Some(Dual(Test(concl))) if theConcl == concl =>
                Test(True) // erase the test, which is also the whole game
              case Some(Compose(Dual(Test(concl)), rest)) if theConcl == concl => rest // erase the test, leave the rest
              case Some(k) =>
                k // there was no test of termination condition, whole continuation should be treated as "after the loop"
            }
          ...
          case _ => throw RefinementFailure(
              shd,
              ghd,
              s"Proof connective ${PrettyPrinter.short(shd)} does not match program $ghd",
              shd,
            )
        }
        // In lockstep cases, just recurse directly, else let the case handle recursion
        val oneToOne = shd match {
          case (Block((_: Phi) :: (_: For) :: _)) => false
          case _ => true
        }
        if (oneToOne) {
          val con = kc.:+(shd)
  ...
  def apply(s: Statement, game: Program): Unit = {
    ...
    } catch { case rf: RefinementFailure => throw ProofCheckException(s"Proof did not refine game $game", cause = rf) }
  }
}
