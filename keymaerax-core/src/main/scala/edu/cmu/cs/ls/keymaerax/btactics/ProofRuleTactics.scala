...
private object ProofRuleTactics extends TacticProvider with Logging {
  ...
  private[btactics] def requireOneSubgoal(provable: ProvableSig, msg: => String): Unit =
    if (provable.subgoals.length != 1) throw new IllFormedTacticApplicationException(
      s"Expected exactly one subgoal sequent in Provable but found ${provable.subgoals.length}\n" + msg
    )

  @inline
  private[btactics] def requireAtMostOneSubgoal(provable: ProvableSig, msg: => String): Unit =
    if (provable.subgoals.length > 1) throw new IllFormedTacticApplicationException(
      s"Expected at most one subgoal sequent in Provable but found ${provable.subgoals.length}\n" + msg
    )

  /** Executes `t` on the sole subgoal (if any). */
  @inline
  private[btactics] def onSoleSubgoal(provable: ProvableSig, t: Sequent => ProvableSig, msg: => String): ProvableSig = {
  ...
  def boundRenameAt(repl: Variable): DependentPositionWithAppliedInputTactic =
    inputanon { (pos: Position, sequent: Sequent) =>
      sequent.sub(pos) match {
        ...
        case ex => throw new TacticInapplicableFailure(
  ...
  def contextualize[T <: BelleExpr](tactic: AtPosition[T]): DependentPositionTactic = contextualize(
    tactic,
    (fml: Formula) => {
