...
object PegasusM2KConverter extends UncheckedM2KConverter {
  ...
  /** Decodes the formula generated by convertFormulaTermList back into a list of formulas. */
  def decodeFormulaList(listFml: Expression): immutable.Seq[(immutable.Seq[Formula], Formula)] = {
    listFml match {
      case And(Equal(Number(invListLength), _), dcChainList) =>
        //@note Pegasus returns lists of length 2:
        // - first entry is dc chain
        // - second indicates invariant (true) vs. candidate (false)
        assert(invListLength == 2, "Expected a list with two entries (DC chain and invariant/candidate indicator), but got " + invListLength)
        dcChainList match {
          case And(And(Equal(Number(dcChainLength), _), fml), q) =>
            (FormulaTools.leftConjuncts(fml, dcChainLength.toInt), q) :: Nil
          case _ => throw new IllegalArgumentException("Expected a conjunction of DC chain (length and formula) and an invariant/candidate indicator, but got " + dcChainList.prettyString)
        }
      case fml: Formula => (fml::Nil, True) :: Nil
...
/**
  ...
  * @author Andre Platzer
  */
class MathematicaAlgebraTool(override val link: MathematicaLink) extends BaseKeYmaeraMathematicaBridge[KExpr](link, new UncheckedK2MConverter, PegasusM2KConverter) with AlgebraTool {
