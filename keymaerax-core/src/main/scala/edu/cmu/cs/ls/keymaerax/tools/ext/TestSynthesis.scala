...
/**
  ...
  */
class TestSynthesis(mathematicaTool: Mathematica) extends BaseKeYmaeraMathematicaBridge[Either[KExpr,NamedSymbol]](mathematicaTool.link, CEXK2MConverter, CEXM2KConverter) {
  ...
  def synthesizeSafetyMarginCheck(fml: Formula, vals: Map[Term, Term]): Number = {
    ...
    val valsExpr = new MExpr(MathematicaSymbols.LIST, vals.map({case (k,v) =>
      new MExpr(new MExpr(Expr.SYMBOL, "Set"), Array(k2m.convert(Left(k)), k2m.convert(Left(v))))}).toArray)
  ...
  def getSafetyRange(fml: Formula): (Number, Number) = {
    ...
    val symbolsExpr = new MExpr(MathematicaSymbols.LIST, symbols.map(s => k2m.convert(Right(s))).toArray)
  ...
  private def findInstance(fml: Formula, amount: Int, timeout: Option[Int]): MExpr = {
    val fi = numeric(new MExpr(new MExpr(Expr.SYMBOL,  "FindInstance"),
      Array(k2m.convert(Left(fml)),
        new MExpr(
          MathematicaSymbols.LIST,
          StaticSemantics.symbols(fml).filter({ case Function(_, _, _, _, interpreted) => !interpreted case _ => true}).
            toList.sorted.map(s => CEXK2MConverter.convert(Right(s))).toArray),
        new MExpr(Expr.SYMBOL, "Reals"), k2m(Left(Number(amount))))))

    timeout match {
      case Some(to) => new MExpr(new MExpr(Expr.SYMBOL,  "TimeConstrained"), Array(fi, k2m(Left(Number(to)))))
      case None => fi
    }
  }
