...
import edu.cmu.cs.ls.keymaerax.btactics.TaylorModelTactics.{debugTac, TaylorModel}
...
object TaylorModelArith {
  ...
  private def tmFormula(elem: Term, poly: Term, lower: Term, upper: Term) = {
    ...
    Exists(Seq(err), And(Equal(elem, Plus(poly, err)), And(LessEqual(lower, err), LessEqual(err, upper))))
  }
  ...
  def destTaylorModelFormula(fml: Formula): (Term, Term, Term, Term) = fml match {
    case Exists(vars, And(Equal(elem, Plus(poly, ivl)), And(LessEqual(l, ivl1), LessEqual(ivl2, u))))
        if vars.length == 1 && vars(0) == ivl && ivl == ivl1 && ivl == ivl2 => (elem, poly, l, u)
  ...
  /**
   * data structure with certifying computations maintains the invariant
   * {{{
  ...
  case class TM(elem: Term, poly: Polynomial, lower: Term, upper: Term, prv: ProvableSig) {
    ...
    def checkCompatibleContext(other: TM) = if (context != other.context)
      throw new IllegalArgumentException("Incompatible contexts: " + context + " and " + other.context)
    ...
    def collectHigherOrderTerms(implicit options: TaylorModelOptions): TM = {
      val (polyLow, polyHigh, partitionPrv) = poly
        .resetTerm
        .partition { case (n, d, powers) => powers.degree <= options.order }
    ...
    def interval(implicit options: TaylorModelOptions): (Term, Term, ProvableSig) = {
      ...
      (
        ...
        u,
        useDirectlyConst(
          weakenWith(context, taylorModelIntervalPrv),
          Seq(("elem1_", elem), ("poly1_", poly1), ("l1_", lower), ("u1_", upper), ("rem_", rem), ("l_", l), ("u_", u)),
          Seq(prv, weakenWith(context, hornerPrv), newIvlPrv),
        ),
      )
    }
     ...
     *   }}}
     */
    def partition(newElem: Term, P: (BigDecimal, BigDecimal, PowerProduct) => Boolean)(implicit
      ...
      val assms = Seq(
        this.weakenContext(newElemEq).prv,
    ...
    def dropEmptyInterval: Option[ProvableSig] =
      if (lower == Number(0) && upper == Number(0)) Some {
        val poly1 = rhsOf(poly.representation)
        useDirectlyConst(
          weakenWith(context, taylorModelEmptyIntervalPrv),
          Seq(("elem1_", elem), ("poly1_", poly1)),
          Seq(prv),
        )
      }
      else None
    ...
    def *!(other: TM)(implicit options: TaylorModelOptions): TM = {
      checkCompatibleContext(other)
      val (polyLow, polyHigh, partitionPrv) = (poly.resetTerm * other.poly.resetTerm).partition { case (n, d, powers) =>
        powers.degree <= options.order
      }
      ...
      val newPrv = useDirectlyConst(
        weakenWith(context, taylorModelTimesPrv),
        Seq(
          ("elem1_", elem),
          ("poly1_", poly1),
          ("l1_", lower),
    ...
    def /!(const: BigDecimal)(implicit options: TaylorModelOptions): TM = {
      ...
      val numberPrv = useDirectly(
        divideNumber,
        Seq(("p_", elem), ("q_", q), ("i_", q), ("r_", r)),
        Seq(equalReflex(q), equalReflex(r)),
      )
      val other = Exact(ofTerm(inv), context)
      val res = this *! other
      val newPrv = useDirectlyConst(
        weakenWith(context, taylorModelDivideExactPrv),
        Seq(
          ("elem1_", elem),
          ("inv_", inv),
        ...
        Seq(res.prv, weakenWith(context, numberPrv)),
      )
    ...
    def squareExact(implicit options: TaylorModelOptions): TM = {
      val (polyLow, polyHigh, partitionPrv) = (poly.resetTerm ^ 2).partition { case (n, d, powers) =>
        powers.degree <= options.order
      }
      val hornerPrv = hornerForm(polyHigh)
      val rem = rhsOf(hornerPrv)
    ...
    def ^(n: Int)(implicit options: TaylorModelOptions): TM = n match {
      case 1 =>
        val newPrv = useDirectlyConst(
          weakenWith(context, taylorModelPowerOne),
          Seq(("elem1_", elem), ("poly1_", rhsOf(poly.representation)), ("l1_", lower), ("u1_", upper)),
          Seq(prv),
        )
    ...
    def approx(implicit options: TaylorModelOptions): TM = {
      ...
      val newPrv = useDirectlyConst(
        ...
        Seq(
          prv,
          weakenWith(context, polyApproxPrv),
          weakenWith(context, poly1.representation),
          weakenWith(context, poly2repPrv),
          ivlPrv,
        ),
      )
    ...
    def prettyPrv(implicit options: TaylorModelOptions): ProvableSig = {
      ...
      proveBy(
        ProvableSig
          .startPlainProof(
            Sequent(context, IndexedSeq(tmFormula(elem, rhsOf(prettyRepresentation), prettyLower, prettyUpper)))
          )
          .apply(CutRight(prv.conclusion.succ(0), SuccPos(0)), 0)
          .apply(CoHideRight(SuccPos(0)), 1)
          .apply(ImplyRight(SuccPos(0)), 1),
        Idioms.<(
          by(prv),
          existsL(-1) & existsR("err_".asTerm)(1) & andL(-1) & andL(-2) & andR(1) & Idioms.<(
            cohideOnlyL(-1) & useAt(prettyRepresentation, PosInExpr(1 :: Nil))(1, 1 :: 0 :: Nil) & id,
    ...
    def weakenContext(formula: Formula) = {
      ...
      val newPrv = ProvableSig
        .startPlainProof(Sequent(prv.conclusion.ante ++ Seq(formula), prv.conclusion.succ))
    ...
    def transElem(eqPrv: ProvableSig): TM = {
      require(
        eqPrv.isProved && eqPrv.conclusion.ante == context,
        "transElem requires proved equality with compatible context",
      )
      val elem1 = lhsOf(eqPrv)
      val newPrv = useDirectlyConst(
        weakenWith(context, taylorModelTransElem),
        Seq(("elem_", elem), ("elem1_", elem1), ("poly_", rhsOf(poly.representation)), ("l_", lower), ("u_", upper)),
        Seq(prv, eqPrv),
      )
  ...
  def evalFormula(fml: Formula, context: IndexedSeq[Formula], argumentMap: Map[Term, TM])(implicit
      options: TaylorModelOptions
  ): Option[ProvableSig] = {
    ...
    def doEval(fml: Formula): Option[ProvableSig] = fml match {
      case cmp: ComparisonFormula =>
        ...
        try {
          ...
          prv.conclusion.succ(0) match {
            case Equiv(_, True) => Some(useDirectly(
                weakenWith(context, lemma),
                Seq(("l_", l), ("u_", u), ("f_", f), ("g_", g)),
                Seq(fgIvlPrv, weakenWith(context, prv)),
              ))
            case _ => None
          }
        } catch { case _: IllegalArgumentException => None }
      case And(a, b) => doEval(a) match {
          case Some(prva) => doEval(b) match {
              case Some(prvb) =>
      ...
      case Or(a, b) => doEval(a) match {
          case Some(prv) => Some(proveBy(Sequent(context, IndexedSeq(fml)), orR(1) & hideR(2) & by(prv)))
          case None => doEval(b) match {
              case Some(prv) => Some(proveBy(Sequent(context, IndexedSeq(fml)), orR(1) & hideR(1) & by(prv)))
              case None => None
            }
        }
  ...
  def Exact(elem: Polynomial, context: IndexedSeq[Formula]): TM = {
    val newPrv = useDirectlyConst(
      weakenWith(context, taylorModelExactPrv),
  ...
  private def mkAndPrv(p1: ProvableSig, p2: ProvableSig): ProvableSig = {
    ...
    proveBy(
      Sequent(p1.conclusion.ante, IndexedSeq(And(p1.conclusion.succ(0), p2.conclusion.succ(0)))),
  ...
  private def equalTrans(p1: ProvableSig, p2: ProvableSig): ProvableSig = {
    ...
    proveBy(
      Sequent(p1.conclusion.ante, IndexedSeq(Equal(lhsOf(p1), rhsOf(p2)))),
      cutR(p1.conclusion.succ(0))(1) & Idioms.<(by(p1), implyR(1) & eqL2R(Llast)(1) & hideL(Llast) & by(p2)),
    )
  }
  ...
  def refineTaylorModelFormula(fml: Formula, assms: IndexedSeq[Formula])(implicit options: TaylorModelOptions) = {
    ...
    val iffPrv = proveBy(
      Equiv(tmFml, tmRepFml),
      useAt(polyPrettyPrv, PosInExpr(1 :: Nil))(1, 0 :: 0 :: 0 :: 1 :: 0 :: Nil) &
        useAt(poly.representation, PosInExpr(0 :: Nil))(1, 0 :: 0 :: 0 :: 1 :: 0 :: Nil) & byUS(Ax.equivReflexive),
    )
  ...
  class TemplateLemmas(ode: DifferentialProgram, order: Int) extends TaylorModel(ode, order) {
     ...
     * }}}
     */
    class TimeStep(x0: Seq[TM], r0: Seq[TM], t0: ProvableSig, h: BigDecimal)(implicit
        options: TaylorModelOptions,
      ...
      val instantiation: USubstOne = instantiateLemma(
        options.precision,
        rhsOf(t0),
        Number(h),
      ...
      val initialStateEqs = x0
        .lazyZip(initialConditionFmls1.tail)
        .map {
          case (x, Equal(y, t)) if x.elem == y =>
            val eq1 = x
              .dropEmptyInterval
              .getOrElse(throw new RuntimeException("intervals have been checked for emptiness"))
            val eq2 = weakenWith(
              x.context,
              x.poly
                .resetTerm
                ...
                .getOrElse(
                  throw new RuntimeException(
                    "this equality should hold by construction: " + x.poly.resetTerm.term + " = " + ofTerm(t).term
                  )
                ),
            )
            equalTrans(eq1, eq2)
      ...
      val numbericConditionPrv = proveBy(
        Sequent(context, IndexedSeq(numbericCondition1)),
        andR(1) & Idioms.<(
          debugTac("Initial Numberic condition") & SaturateTactic(andL(Symbol("L"))) &
            IntervalArithmeticV2.intervalArithmeticBool(options.precision, qeTool)(1) & done,
          SaturateTactic(allR(1)) & SaturateTactic(implyR(1)) & SaturateTactic(andL(Symbol("L"))) &
            debugTac("Numberic condition") & IntervalArithmeticV2.intervalArithmeticBool(options.precision, qeTool)(1) &
            done,
        ),
      )
       ...
       */
      def timeStepLemma: ProvableSig = proveBy(
        Sequent(context, IndexedSeq(boxTMEnclosure1)),
      ...
      val nonnegativeTimeStep = {
        ...
        weakenWith(
          context,
          proveBy(fml, useAt(ProvableSig.proveArithmetic(BigDecimalQETool, fml), PosInExpr(0 :: Nil))(1) & closeT),
        )
      }
       ...
       * @return
       *   (provable, TM_ivl, (TM_h, t1_eq)) where provable:
       *   {{{
      ...
      def timeStepLemma(P: Formula): (ProvableSig, Seq[TM], Seq[TM], (Seq[TM], Seq[TM], ProvableSig)) = {
        ...
        val hideOldInitialState = context
          .zipWithIndex
          .flatMap { case (fml, i) => if (vars.exists(subtermOf(_, fml))) Some(hideL(-i - 1)) else None }
          .reverse
        ...
        val prv = proveBy(
          ...
          useAt(timeStepLemma1, PosInExpr(1 :: Nil))(1) & andR(1) & Idioms.<(
            andR(1) & Idioms.<(
              andR(1) & Idioms.<(by(numericAssumptionPrv), by(nonnegativeTimeStep)),
              cut(t0.conclusion.succ(0)) &
                Idioms.<(eqL2R(-context.length - 1)(1) & hideL(Symbol("Llast")), hideR(1) & by(t0)) &
                ...
                cutL(taylorModelsIvl.map(_._1).reduceRight(And))(Symbol("Llast")) & Idioms.<(
                  ...
                  cohideOnlyR(2) &
                    taylorModelsIvl
                      .init
                      .foldRight[BelleExpr](useAt(taylorModelsIvl.last._3, PosInExpr(1 :: Nil))(1) & id) {
                        case ((_, _, implyPrv, _), tac) => useAt(refineConjunction, PosInExpr(1 :: Nil))(1) & andR(1) &
                            Idioms.<(useAt(implyPrv, PosInExpr(1 :: Nil))(1) & id, tac)
                      } & done,
                ),
            ),
            saveRightTaylorModels &
              ...
              cutL((t1Eq +: taylorModelsEq.map(_._1)).reduceRight(And))(Symbol("Llast")) & Idioms.<(
                ...
                cohideR(2) & useAt(refineConjunction, PosInExpr(1 :: Nil))(1) & andR(1) & Idioms.<(
                  ...
                  taylorModelsEq
                    .init
                    .foldRight[BelleExpr](by(taylorModelsEq.last._3)) { case ((_, _, implyPrv, _), tac) =>
                      useAt(refineConjunction, PosInExpr(1 :: Nil))(1) & andR(1) & Idioms.<(by(implyPrv), tac)
                    },
                ),
              ),
          ),
        ...
        val tmIvls = taylorModelsIvl.map { case (tmFml, tmRepFml, _, (elem, poly, l, u, iffPrv)) =>
          TM(
            ...
            l,
            u,
            proveBy(Sequent(contextIvl, IndexedSeq(tmRepFml)), useAt(iffPrv, PosInExpr(1 :: Nil))(1) & id),
          )
        }
        ...
        val tmEqs = taylorModelsEq.map { case (tmFml, tmRepFml, _, (elem, poly, l, u, iffPrv)) =>
          TM(
            ...
            l,
            u,
            proveBy(Sequent(contextEq, IndexedSeq(tmRepFml)), useAt(iffPrv, PosInExpr(1 :: Nil))(1) & id),
          )
        }
    ...
    /**
     * Params:
     *   - (x0, r0, t0) like for [[TimeStep]]
     *   - prv: Provable with one subgoal of the form `x = x0 o r0, t=t0 |- [{x'=ode(x)}]P(x)`
     * @return
     *   - (x1, r1, t1) suitable input to iterate this function (identity preconditioned)
     *   - t1: proof of "t = t0 + h"
     *   - prv1: Provable with one subgoal of the form `x = x1 o r1, t = t0 +h |- [{x'=ode(x)}]P(x)`
     */
    def timeStepAndPrecondition(x0: Seq[TM], r0: Seq[TM], t0: ProvableSig, h: BigDecimal, prv: ProvableSig)(implicit
        options: TaylorModelOptions,
        timeStepOptions: TimeStepOptions,
      ...
      val P = subgoal.succ(0) match {
        ...
        case _ => throw new IllegalArgumentException(
            "timeStepAndPrecondition must be applied on a suitable unconstrained ODE evolution"
          )
      }
      ...
      val prv4 =
        evalFormula(prvIvl.conclusion.succ(0), prvIvl.subgoals(0).ante, tms.map(t => (t.elem, t)).toMap) match {
          case Some(safetyPrv) => prv3.apply(safetyPrv, 0)
          case None =>
            throw new TacticInapplicableFailure("Could not prove safety for the past time step: " + prvIvl.subgoals(0))
        }
      (x1, r1, tEq1, prv4)
    }

    /** the iteration of [[timeStepAndPrecondition]] */
    def iterateTimeSteps(x0: Seq[TM], r0: Seq[TM], t0: ProvableSig, h: BigDecimal, prv: ProvableSig, n: Int)(implicit
        options: TaylorModelOptions,
        timeStepOptions: TimeStepOptions,
    ): (Seq[TM], Seq[TM], ProvableSig, ProvableSig) =
  ...
  def partition(xs: Seq[TM], newElems: Seq[Term], P: (BigDecimal, BigDecimal, PowerProduct) => Boolean)(implicit
      options: TaylorModelOptions
  ): (Seq[TM], Seq[TM], Seq[Equal]) = {
    def doPartition(
        xs: Seq[TM],
        rs: Seq[TM],
        newElemEqs: Seq[Equal],
        x0s: Seq[TM],
        newElems: Seq[Term],
    ): (Seq[TM], Seq[TM], Seq[Equal]) =
      if (x0s.length == 0) (xs.reverse, rs.reverse, newElemEqs.reverse)
      else {
        val (x, r, newElemEq) = x0s
          .head
          .partition(newElems.head, (n: BigDecimal, d: BigDecimal, pp: PowerProduct) => pp.degree == 0)
  ...
  def cutEq(prv: ProvableSig, eq: Formula) = eq match {
    case Equal(v: Variable, rhs) =>
      ...
      proveBy(
        prv.apply(Cut(exEq.conclusion.succ(0)), 0)(HideRight(SuccPos(0)), 1)(CoHideRight(SuccPos(0)), 1)(exEq, 1),
   ...
   * }}}
   */
  def identityPrecondition(x: Seq[TM], r: Seq[TM], prv: ProvableSig)(implicit
      options: TaylorModelOptions
  ): (Seq[TM], Seq[TM], ProvableSig) = {
    ...
    val hides = context
      .zipWithIndex
      .reverse
      .flatMap { case (fml, i) => if (hidevars.exists(subtermOf(_, fml))) Some(i) else None }
    ...
    val prv6 = (context5.length - x1.length until context5.length)
      .reverse
      .foldLeft(prv5) { case (prv, i) => prv(AndLeft(AntePos(i)), 0) }
