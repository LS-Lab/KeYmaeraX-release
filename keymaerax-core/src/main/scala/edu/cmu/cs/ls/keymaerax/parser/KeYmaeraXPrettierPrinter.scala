...
class KeYmaeraXPrettierPrinter(margin: Int) extends KeYmaeraXPrecedencePrinter {
  ...
  protected def wrapChildDoc(t: UnaryComposite, doc: Doc): Doc = if (skipParens(t)) doc else encloseText("(", doc, ")")
  protected def wrapChildDoc(t: Quantified, doc: Doc): Doc = if (skipParens(t)) doc else encloseText("(", doc, ")")
  protected def wrapChildDoc(t: Modal, doc: Doc): Doc = if (skipParens(t)) doc else encloseText("(", doc, ")")
  ...
  protected def docOf(term: Term): Doc = term match {
    ...
    case p: Pair =>
      ...
      wrapDoc(
        (Doc.lineBreak + flattenPairs(p)
          .map(docOf)
          .reduce[Doc]({ case (a, b) => a + Doc.text(ppOp(term)) + Doc.lineBreak + b + Doc.lineBreak }))
          .nested(2)
          .grouped,
        term,
      )
    ...
    case t: BinaryCompositeTerm => (wrapLeftDoc(t, docOf(t.left)) + Doc.space + Doc.text(ppOp(t)) + Doc.line +
        wrapRightDoc(t, docOf(t.right))).grouped
  ...
  protected def docOf(formula: Formula): Doc = formula match {
    ...
    case f: ComparisonFormula => (wrapLeftDoc(f, docOf(f.left)) + Doc.space + Doc.text(ppOp(formula)) + Doc.line +
        wrapRightDoc(f, docOf(f.right))).grouped
    case DifferentialFormula(g) => encloseText("(", docOf(g), ")" + ppOp(formula))
    // prints quantifiers with comma-separated list of variables
    case f: Quantified =>
      (Doc.text(ppOp(formula)) + Doc.space + Doc.intercalate(Doc.comma + Doc.space, f.vars.map(docOf(_))) +
  ...
  protected def docOf(program: Program): Doc = program match {
    ...
    case ODESystem(ode, f) =>
      wrapDoc(docOfODE(ode) + Doc.space + Doc.text(ppOp(program)) + Doc.line + docOf(f), program).grouped
    ...
    case t: Compose =>
      (pwrapLeftDoc(t, docOf(t.left)) + Doc.line + Doc.text(ppOp(t)) + pwrapRightDoc(t, docOf(t.right))).grouped
    // @note all remaining binary operators are infix, see [[OpSpec]]
    case t: BinaryCompositeProgram => (pwrapLeftDoc(t, docOf(t.left)) + Doc.line + Doc.text(ppOp(t)) + Doc.line +
        pwrapRightDoc(t, docOf(t.right))).grouped
  }
  ...
  def docOf(seq: Sequent): Doc = Doc.intercalate(
    Doc.line,
    (1 to seq.ante.length).map(i => Doc.text(s"${-i}: ") + (Doc.line + docOf(seq.ante(i - 1))).nested(2)),
  ) + Doc.line + Doc.text(" ==> ") + Doc.line + Doc.intercalate(
    Doc.line,
    (1 to seq.succ.length).map(i => Doc.text(s"$i: ") + (Doc.line + docOf(seq.succ(i - 1))).nested(2)),
  )

  override def stringify(seq: Sequent): String = docOf(seq).render(margin)

  def docOf(prv: ProvableSig): Doc = Doc.text("Conclusion:") + (Doc.line + docOf(prv.conclusion)).nested(2) + Doc.line +
    Doc.text(s"${prv.subgoals.length} subgoals" + (if (prv.subgoals.isEmpty) "." else ":")) + Doc.line +
    Doc.intercalate(
      Doc.line,
      prv.subgoals.zipWithIndex.map { case (seq, i) => Doc.text(s"Subgoal $i: ") + (Doc.line + docOf(seq).nested(2)) },
    )
