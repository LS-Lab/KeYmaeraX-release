 ...
 */
object ModelPlex extends TacticProvider with ModelPlexTrait with Logging {

  /** @inheritdoc */
  override def getInfo: (Class[_], universe.Type) = (ModelPlex.getClass, universe.typeOf[ModelPlex.type])

  private val NAMESPACE = "modelplex"

  // supporting-lemmas
  lazy val simplForall1: Lemma = AnonymousLemmas.remember(
    "p(f()) -> \\forall x_ (x_=f() -> p(x_))".asFormula,
    implyR(1) & allR(1) & implyR(1) & eqL2R(-2)(1) & close,
    NAMESPACE,
  )
  lazy val simplForall2: Lemma = AnonymousLemmas.remember(
    "p(f()) -> \\forall x_ (f()=x_ -> p(x_))".asFormula,
    implyR(1) & allR(1) & implyR(1) & eqR2L(-2)(1) & close,
    NAMESPACE,
  )
  ...
  def apply(
    ...
    val vars = StaticSemantics
      .boundVars(formula)
      .symbols
      .filter(v => v.isInstanceOf[Variable] && !v.isInstanceOf[DifferentialSymbol])
  ...
  def normalizeInputFormula(f: Formula): Formula = {
    proveBy(f, SaturateTactic(TactixLibrary.alphaRule)).subgoals match {
      case IndexedSeq(Sequent(assumptions, alternatives)) =>
        val (boxes, negAssumptions) = alternatives.partition({
          case _: Box => true
          case _ => false
        })
  ...
  def partitionUnobservable(unobservable: ListMap[_ <: NamedSymbol, Option[Formula]]): (
    ...
    val unobservableStateVars: ListMap[Variable, Option[Formula]] = unobservable
      .filter(_._1.isInstanceOf[Variable])
      .map({ case (k: Variable, v) => k -> v })
    val unknownParams: ListMap[(Function, Variable), Option[Formula]] = unobservable
      .filter({
        case (Function(_, _, domain, _, _), _) => domain == Unit
        case _ => false
      })
      .map({ case (k: Function, v) => (k, Variable(k.name, k.index, k.sort)) -> v })
    val unknownFunctions: ListMap[(Function, Variable), Option[Formula]] = unobservable
      .filter({
        case (Function(_, _, domain, _, _), _) => domain != Unit
        case _ => false
      })
  ...
  def createMonitorSpecificationConjecture(
    ...
    require(
      StaticSemantics.symbols(fml).intersect(vars.map(postVar).toSet).isEmpty,
    ...
    def conjectureOf(assumptions: Formula, prg: Program): (Formula, List[Formula]) = {
      ...
      assert(
        boundVars.forall(v => !v.isInstanceOf[Variable] || v.isInstanceOf[DifferentialSymbol] || vars.contains(v)),
        "All bound variables " + StaticSemantics.boundVars(prg).prettyString + " must occur in monitor list " + vars
          .mkString(", ") +
          "\nMissing: " + (StaticSemantics
            .boundVars(prg)
            .symbols
            .filterNot(_.isInstanceOf[DifferentialSymbol]) diff vars.toSet).mkString(", "),
      )
        ...
        .map({ case (v, Some(e)) =>
          (StaticSemantics.freeVars(e).toSet - v).toList match {
            ...
            case s => throw new IllegalArgumentException(
                "Sensor specifications with multiple variables (" + s.map(_.prettyString).mkString(",") +
      ...
      val varConjectureBody =
        if (preEstimator == True) Diamond(prg, postEqs) else And(preEstimator, Diamond(prg, postEqs))
      ...
      val nonboundAssumptions = FormulaTools
    ...
    normalizeInputFormula(fml) match {
      ...
      case _ => throw new IllegalArgumentException(
          "Unsupported shape of formula " + fml
            .prettyString + "; formula must be propositionally equivalent to A -> [prg;]P"
        )
    }
  }
  ...
  def createSlidingMonitorSpec(
    ...
    // @todo encode in dL
    val ModelPlexConjecture(_, spec, assms) =
    ...
    val windowMon = (2 to windowSize).foldLeft(ithMon(specFml, vars, 1))({ case (f, i) =>
      And(f, ithMon(specFml, vars, i))
    })
    (ctx(windowMon), assms)
  }
  ...
  private def replaceOld(fml: Formula, x0: Map[Variable, Term]): Formula = x0.foldRight(fml)({ case ((v, t), f) =>
    f.replaceFree(v, t)
  })
  ...
  private def combineDiffInvariants(
      diffInvariants: List[(DifferentialProgram, (Formula, Formula))],
      origOde: ODESystem,
      x0: Map[Variable, Term],
  ): Formula = {
    ...
    diffInvariants
      .map({ case (ode, (initial, di)) =>
        ...
        if (diffInvariants.size > 1) {
          ...
          val varStartValues = rhsStartValues
            .flatMap({ case Equal(orig, inst) =>
  ...
  def createNonlinearModelApprox(
      ...
      tactic: BelleExpr,
      defs: Declaration,
  ): Expression => (Formula, BelleExpr) = (model: Expression) =>
    model.exhaustiveSubst(USubst(
      defs
        .substs
        .filter(_.what match {
          case _: SystemConst | _: ProgramConst => true
          case _ => false
        })
    )) match {
      case fml @ Imply(init, Box(Loop(prg), safe)) =>
        ...
        LazySequentialInterpreter(pl :: /*qeDurationListener::*/ Nil)(
          ...
          case BelleProvable(proof, _) =>
            assert(proof.isProved, "Cannot derive a nonlinear model from unfinished proof")
          case _ => assert(assertion = false, "Cannot derive a nonlinear model from unfinished proof")
        }

        def pushOld(fml: Formula): Formula = ExpressionTraversal
          .traverse(
            new ExpressionTraversalFunction() {
              override def preT(p: PosInExpr, e: Term): Either[Option[StopTraversal], Term] = e match {
        ...
        (approx, skip)
      case _ =>
        throw new IllegalArgumentException("Unsupported model shape, expected shape init -> [(ctrl;plant;)*]safe")
    }

  /**
   * Creates a sandbox safety conjecture from a formula of the shape init->[?bounds;{ctrl;plant}*]safe. The sandbox
   * embeds an (unverified) external controller in monitor checks of `kind`. It approximates the external controller
   * behavior with nondeterministic values for each of the bound variables in `ctrl`. Input to the external controller
   * is measured with nondeterministic values for each of the bound variables in `plant`, but restricted to those
  ...
  def createSandbox(
    ...
    require(
      kind == Symbol("ctrl"),
      s"Unable to create a sandbox of kind $kind, so far only controller monitors supported",
    )
    ...
    val (
      ...
      case _ => throw new IllegalArgumentException(
          "Sandbox synthesis supports input formulas of the shape init -> [{ctrl;ODE}*]safe. Please use {} to group program statements into exactly two blocks, e.g. { {ctrl1;ctrl2;}{ODE} }*."
        )
    }
    ...
    val vars: List[BaseVariable] = StaticSemantics
      .symbols(expandedModel)
      .filter({
    ...
    val consts: Map[Term, Variable] = constVars
      ...
      .map({
        case s: Function => FuncOf(s, Nothing) -> Variable(s.name, s.index)
        case v: BaseVariable => v -> v
      })
      .toMap
    val bounds = FormulaTools
      .conjuncts(initCond)
    ...
    val initSensors = FormulaTools
      ...
      filter(
        !StaticSemantics
          .freeVars(_)
          .intersect(StaticSemantics.boundVars(plant) ++ StaticSemantics.boundVars(ctrlPrg))
          .isEmpty
      )
      .map(replace(_, consts))
      .reduceRightOption(And)
    ...
    assert(
      pl.invariant.isDefined,
      "Proof of model " + name + " does not provide a loop invariant. Please use tactic loop({`inv`},...) in the proof.",
    )
    assert(
      pl.diffInvariants.nonEmpty,
      "Proof of model " + name + " does not provide sufficient insight into invariant regions of the ODE dynamics. Please use differential cuts dC in the proof.",
    )
    assert(
      pl.odeLemmas.nonEmpty,
    ...
    val odeLemmas = pl
      .odeLemmas
      .map({ case (odeLemmaName, odeLemmaFml, odeLemmaTactic) =>
        (
          odeLemmaName,
          replace(replace(odeLemmaFml, sensePostVars), sensePreVars),
    ...
    val sandbox = Imply(
      replace(initCond, consts),
      Box(
        Compose(
          Compose(readConsts, Compose(readCtrls, Compose(readInitialSensors, Test(And(bounds, initSensors))))),
          Loop(Compose(ctrl, Compose(nonCtrlState, Compose(replace(act, consts), Compose(operationalize, sense))))),
        ),
        replace(safe, consts),
      ),
    )
  ...
  private def sandboxTacticPlantFirst(
      ...
      odeDomain: Formula,
      ctrl: Program,
      fallbackCtrl: Program,
      upsilon: Formula,
      senseVars: List[Variable],
  ): BelleExpr = {
    ...
    val chaseFallback = anon((pos: Position, seq: Sequent) =>
      seq.sub(pos) match {
      ...
      DebuggingTactics.print("Proving controllers") & chase(1) & andR(1) & Idioms.<(
          ...
          implyR(1) & cut(Box(ctrl, inv)) & Idioms.<(
            cut(Diamond(ctrl, And(inv, And(upsilon, odeDomain)))) & Idioms.<(
              hideL(Symbol("L"), Box(ctrl, inv)) & hideL(Symbol("L"), Diamond(ctrl, And(upsilon, odeDomain))) &
                ...
                notL(Symbol("Llast")) & abstractionb(Symbol("Rlast")) & allR(Symbol("Rlast")) * numCtrlVars & notR(
                  Symbol("Rlast")
                ) & SaturateTactic(andL(Symbol("L"))) &
                upsilonConjuncts
                  .filter({ case Equal(l, r) => l != r })
                  .map(c => exhaustiveEqL2R(Symbol("L"), c))
        ...
        DebuggingTactics.print("Proving fallback") &
          implyR(1) & hideL(Symbol("Llast")) & cut(Box(fallbackCtrl, monitor)) < (
            Idioms.searchApplyIn(
              Box(fallbackCtrl, monitor),
              useLemmaAt(name + "_MonitorCheck", Some(PosInExpr(0 :: Nil))),
              PosInExpr(1 :: Nil),
            ) &
              SaturateTactic(chaseFallback(Symbol("Llast"))) & cut(Box(ctrl, inv)) < (
                cut(Diamond(ctrl, And(inv, And(fallbackUpsilon, odeDomain)))) < (
                  hideL(Symbol("L"), Box(ctrl, inv)) & hideL(
                    Symbol("L"),
                    Diamond(ctrl, And(fallbackUpsilon, odeDomain)),
                  ) &
                    useAt(Ax.diamond, PosInExpr(1 :: Nil))(Symbol("Llast")) &
                    notL(Symbol("Llast")) & abstractionb(Symbol("Rlast")) & allR(Symbol("Rlast")) * numCtrlVars & notR(
                      Symbol("Rlast")
                    ) & SaturateTactic(andL(Symbol("L"))) &
                    fallbackUpsilonConjuncts
                      .filter({ case Equal(l, r) => l != r })
                ...
                implyR(1) & cut(Box(ctrl, inv)) < (
                  cut(Diamond(ctrl, And(inv, And(upsilon, odeDomain)))) < (
                    DebuggingTactics.print("Using external control actuation cuts") &
                      ...
                      hideL(Symbol("L"), Box(ctrl, inv)) & hideL(Symbol("L"), Diamond(ctrl, And(upsilon, odeDomain))) &
                      useAt(Ax.diamond, PosInExpr(1 :: Nil))(Symbol("Llast")) &
                      notL(Symbol("Llast")) & abstractionb(Symbol("Rlast")) & allR(
                        Symbol("Rlast")
                      ) * numCtrlVars & notR(Symbol("Rlast")) & SaturateTactic(andL(Symbol("L"))) &
                      upsilonConjuncts
                        .filter({ case Equal(l, r) => l != r })
                        .map(c => exhaustiveEqL2R(Symbol("L"), c))
                        .reduce[BelleExpr](_ & _) &
                    ...
                    cut(Box(ctrl, inv)) < (
                      cut(Diamond(ctrl, And(inv, And(fallbackUpsilon, odeDomain)))) < (
                        DebuggingTactics.print("Using fallback cuts") &
                          chase(1) & hideL(Symbol("L"), Box(ctrl, inv)) & hideL(
                            Symbol("L"),
                            Diamond(ctrl, And(fallbackUpsilon, odeDomain)),
                          ) &
                          useAt(Ax.diamond, PosInExpr(1 :: Nil))(Symbol("Llast")) &
  ...
  private val loopComposeIndex = (e: Expression) =>
    e match {
  ...
  private val skipODEIndex = (e: Expression) =>
    e match {
      // remove loops
      case Diamond(Loop(_), _) => Ax.loopApproxd :: Nil
  ...
  def eulerAllIn: DependentPositionTactic = anon((pos: Position, sequent: Sequent) => {
    val eulerAxiom =
      "<{x_'=f(x_)}>p(x_) <-> \\exists t_ (t_>=0 & \\forall e_ (e_>0 -> \\forall h0_ (h0_>0 -> \\exists h_ (0<h_&h_<h0_&<{x_:=x_+h_*f(x_);}*>(t_>=0 & \\exists y_ (abs(x_-y_) < e_ & p(y_))) ))))"
        .asFormula
    val positions: List[BelleExpr] = mapSubpositions(
      pos,
      sequent,
      {
        case (Diamond(_: ODESystem, _), pp) =>
          Some(useAt(ProvableSig.startPlainProof(eulerAxiom), PosInExpr(0 :: Nil))(pp))
        case _ => None
      },
    )
    positions.reduceRightOption[BelleExpr](_ & _).getOrElse(skip)
  ...
  def eulerSystemAllIn: DependentPositionTactic = anon((pos: Position, sequent: Sequent) => {
    ...
    def createEulerAxiom(ode: ODESystem, p: Formula): Formula = {
      ...
      s"<{c_&q_(||)}>p_(||) <-> \\exists t_ (t_>=0 & \\forall $e ($e>0 -> \\forall h0_ (h0_>0 -> \\exists $h (0<$h&$h<h0_&<{$systemApprox}*>(t_>=0 & $errorMargin) ))))"
        .asFormula
    }
    ...
    val positions: List[BelleExpr] = mapSubpositions(
      ...
      sequent,
      {
        ...
        case (Diamond(ode: ODESystem, p), dpos) =>
          Some(OnAll(useAt(ProvableSig.startPlainProof(createEulerAxiom(ode, p)), PosInExpr(0 :: Nil))(dpos) | skip))
        case _ => None
      },
    )
    positions.reduceRightOption[BelleExpr](_ & _).getOrElse(skip)
      ...
      val positions: List[BelleExpr] = mapSubpositions(
        ...
        sequent,
        {
          case (Diamond(_: Loop, _), pp) =>
            if (n == 1) Some(useAt(Ax.loopApproxd)(pp)) else Some(useAt(Ax.iterated, PosInExpr(0 :: Nil))(pp))
          case _ => None
        },
      )
    ...
    anon((pos: Position, sequent: Sequent) => {
      ...
      sequent.sub(pos) match {
        ...
        case Some(e) => throw new TacticInapplicableFailure(
            "Modelplex In-Place only applicable to diamond properties, but got " + e.prettyString
          )
  ...
  private def monitorAxioms = List(
    ...
    useAt(Ax.testd),
    anon((p: Position) =>
      TryCatch(useAt(Ax.assigndAxiom)(p), classOf[SubstitutionClashException], (_: SubstitutionClashException) => fail)
    ),
   ...
   * the outermost such sub-position of p. Formulas only.
   *
   * @example
   *   {{{
   ...
   * `defs` the functions that have at least one if their arguments bound. Returns the formula with abbreviated/expanded
   * functions, and the substitutions telling which functions were expanded/abbreviated how.
   */
  def mxAbbreviateFunctions(fml: Formula, defs: Declaration): (Formula, USubst) = {
    ...
    def replaceOrExpand[T <: Expression](expr: T): T = ExpressionTraversal
      .traverseExpr(
        new ExpressionTraversalFunction() {
          override def preF(p: PosInExpr, e: Formula): Either[Option[StopTraversal], Formula] = e match {
            case pn @ PredOf(f: Function, _) => defs
                .substs
                .find(_.what match {
                ...
                defs
                  ...
                  case None =>
                    if (InterpretedSymbols.preshipped.contains(n, i)) Left(None)
                      ...
                      "Function " + fn
                        .prettyString + " depends on quantified variable " + bv + " and so must be expanded, but got no definition how to expand; please make unobservable"
                    )
                  case Some(d) =>
                    val s = USubst(List(d))
                    subst = subst ++ s
  ...
  /**
   * Partial QE on a formula that first expands/abbreviates all uninterpreted symbols and then substitutes back in on
   * the result.
   */
  ...
  def mxPartialQE(
    ...
    tool.qe(goal, continueOnFalse = false) match {
      case (Atom(f), result) => abbreviated.get(f) match {
          case Some(subst) =>
            if (verified) tool.qe(f).fact(subst) else ProvableSig.startProof(Equiv(f, result), defs)(subst)
    ...
    SimplifierV3.formulaSimp(result, faxs = SimplifierV3.defaultFaxs, taxs = SimplifierV3.defaultTaxs) match {
      case (simplifiedResult, Some((True, simpProof))) =>
        ...
        val uncondSimpProof = ProvableSig.startProof(equiv, p.defs)(
          CEat(
            Ax.trueImply
              .provable(USubst(List(SubstitutionPair(PredOf(Function("p_", None, Unit, Bool), Nothing), equiv))))
          )(SuccPos(0)),
          0,
        )(simpProof, 0)
        ProvableSig.startProof(Equiv(orig, simplifiedResult), p.defs)(
          CEat(uncondSimpProof, PosInExpr(1 :: Nil))(SuccPosition.base0(0, PosInExpr(1 :: Nil))),
          0,
        )(p, 0)
      case _ => p
    }
  ...
  def stepwisePartialQE(
    ...
    case (ctx, exists @ Exists(x, p)) =>
      ...
      val componentResults = monitorComponents
        .zipWithIndex
        .map({ case (f, i) =>
          // @note backend QE seems to not always handle f[] well in the context of existential quantifiers
          preQE(f, i)
          val consts = StaticSemantics
            .symbols(f)
            .filter({
              case Function(_, _, Unit, Real, None) => true
              case _ => false
            })
            .map({ case fn @ Function(n, i, Unit, s, _) => FuncOf(fn, Nothing) -> Variable(n, i, s) })
            .toList
          ...
          val qfProof = consts.foldLeft(ProvableSig.startProof(fnified, defs))({ case (p, c) =>
            p(
              FOQuantifierTactics.allInstantiateInverse(c)(SuccPos(0)) andThen
                ProofRuleTactics.skolemizeR(SuccPos(0)),
              0,
            )
          })(qfVarifiedProof, 0)
          ...
          val merged = ProvableSig.startProof(Equiv(Exists(x, Or(p, q)), Or(pqf, qqf)), defs)(
            CEat(
              RenUSubst(List(
                ...
                (Variable("x_"), x.head),
              )).toForward(Ax.existsOr.provable),
              PosInExpr(List(0)),
            )(SuccPosition.base0(0, PosInExpr(List(0)))),
            0,
          )(CEat(lproof, PosInExpr(List(0)))(SuccPosition.base0(0, PosInExpr(List(0, 0)))), 0)(
            CEat(rproof, PosInExpr(List(0)))(SuccPosition.base0(0, PosInExpr(List(0, 1)))),
            0,
          )(
            Ax.equivReflexive
              .provable(USubst(
                List(SubstitutionPair(PredOf(Function("p_", None, Unit, Bool), Nothing), Or(pqf, qqf)))
              )),
            0,
          )
      ...
      val (reassociated, reassociateProof) = timed(
        {
          ...
          val proof = timed(
            ProvableSig.startProof(resultEquiv, defs)(
              CEat(lp, PosInExpr(List(0)))(SuccPosition.base0(0, PosInExpr(List(0, 0, 0)))),
              0,
            )(CEat(rp, PosInExpr(List(0)))(SuccPosition.base0(0, PosInExpr(List(0, 1)))), 0)(
              Ax.equivReflexive.provable(subst),
              0,
            ),
            "Applying reassociate subproofs",
          )
      ...
      val innerProof = timed(
        ProvableSig.startProof(Equiv(exists, qfResult), defs)(
          CEat(dnfProof, PosInExpr(List(0)))(SuccPosition.base0(0, PosInExpr(List(0, 0)))),
          0,
        )(PropositionalTactics.rightAssociate(SuccPosition.base0(0, PosInExpr(List(0, 0)))), 0)(subst)(
          CEat(reassociateProof(subst), PosInExpr(List(1)))(SuccPosition.base0(0)),
          0,
        )(mergedProof(subst), 0),
        "Inner proof",
      )
      ...
      val outerSubst = USubst(
        defs
          .substs
          .filter({ case SubstitutionPair(what, _) => StaticSemantics.symbols(what).intersect(outerExpand).nonEmpty })
      )

      val innerPos = PosInExpr(List(0)) ++ innermostPos
      timed(
        ProvableSig.startProof(Equiv(fml, outerQf), defs)(subst)(
          CEat(innerProof, PosInExpr(List(0)))(SuccPosition.base0(0, innerPos)),
          0,
        )(outerSubst)(outerProof, 0),
        "Combining inner and outer proof",
      )
    case (ctx, p: Forall) =>
      ...
      val innerSubst = USubst(
        defs
          .substs
          .filter({ case SubstitutionPair(what, _) => StaticSemantics.symbols(what).intersect(innerExpand).nonEmpty })
      )
      ...
      ProvableSig.startProof(Equiv(fml, outerQf), defs)(innerSubst)(
        CEat(innerProof, PosInExpr(List(0)))(SuccPosition.base0(0, innerPos)),
        0,
      )(outerSubst)(outerProof, 0)
  ...
  override def optimizationOneWithSearch(
    ...
    sequent.sub(pos) match {
      case Some(fml) => ExpressionTraversal.traverse(
          new ExpressionTraversalFunction() {
    ...
    positions
      .map(p => instantiateQuantifiers(tool, assumptions, unobservable, simplifier, postVar)(pos ++ p))
      .reduceRightOption[BelleExpr](_ & _)
      .getOrElse(skip) & simplifier.map(_(pos)).getOrElse(skip)
  })

  /**
   * Returns the polarity at `pos` in the `sequent`.
   * @return
   *   - `<0` for positive polarity if antecedent, negative polarity in succedent
  ...
  private def solutionQE(
      existsFml: Formula,
      qeFml: Formula,
      signature: Set[Function],
      assumptions: List[Formula],
      tool: Option[SimplificationTool],
  )(pp: Position) = tool match {
    case None => skip
    case Some(t) =>
  ...
  private def instantiateQuantifiers(
    ...
    def instantiateODESolution(odeShape: Exists): BelleExpr = {
      ...
      solutionQE(odeShape, transformed.subgoals.head.succ.head, signature, assumptions, tool)(pos) & simplifier
        .map(_(pos))
        .getOrElse(skip) |
        solutionQE(odeShape, transformed.subgoals.head.succ.head, signature, Nil, tool)(pos) & simplifier
          .map(_(pos))
          .getOrElse(skip) |
    ...
    sequent.sub(pos) match {
      ...
      case Some(e: Exists) if polarityInSeq(sequent, pos) > 0 /*&& StaticSemantics.boundVars(q).isEmpty*/ =>
        ...
        findWitness(sequent, pos, unobservableVars, simplifier, postVar) match {
          case Some(w) =>
            optimizationOneAt(unobservableVars, Some(w), postVar)(pos) & simplifier.map(_(pos)).getOrElse(skip)
          case None => skip
        }
  ...
  private def findWitness(
    ...
    quant match {
      case Exists(vars, And(Equal(l, r), q))
          if polarityInSeq(sequent, pos) > 0 && vars.contains(l) &&
            (unobservableVars ++ unobservableVars.map(postVar)).intersect(vars).isEmpty && StaticSemantics
              .boundVars(q)
              .intersect(StaticSemantics.freeVars(r))
              .isEmpty =>
        // case from deterministic assignments
        Some(l.asInstanceOf[BaseVariable] -> r)
      case Forall(vars, Imply(Equal(l, r), q))
          if polarityInSeq(sequent, pos) < 0 && vars.contains(l) &&
            (unobservableVars ++ unobservableVars.map(postVar)).intersect(vars).isEmpty && StaticSemantics
              .boundVars(q)
              .intersect(StaticSemantics.freeVars(r))
              .isEmpty => Some(l.asInstanceOf[BaseVariable] -> r)
      case Exists(vars, phi) if polarityInSeq(sequent, pos) > 0 =>
        ...
        val postEquality = {
          ...
          if (unobservableVars.contains(v) && !v.name.endsWith("_")) synonyms.find({
            case r: BaseVariable => NAMED_POST_VAR(v) == r
            case _ => false
          })
          else if (unobservableVars.filterNot(_.name.endsWith("_")).map(NAMED_POST_VAR).contains(v)) synonyms.find({
            case r: BaseVariable => NAMED_POST_VAR(r) == v
            case _ => false
          })
          else synonyms.find({
            case r: BaseVariable => postVar(v) == r || postVar(r) == v
            case _ => false
          })
        }
          ...
          case _ =>
            if (unobservableVars.intersect(vars).nonEmpty) {
              ...
              synonyms.find(
                StaticSemantics
                  .freeVars(_)
                  .intersect(unobservableVars.toSet -- StaticSemantics.boundVars(ctx(False)).toSet)
                  .isEmpty
              )
            }
        }
  ...
  private def optimizationOneAt(
      unobservable: List[Variable],
      inst: Option[(Variable, Term)] = None,
      postVar: Variable => Variable = NAMED_POST_VAR,
  ): DependentPositionTactic = anon((pos: Position, sequent: Sequent) => {
    // was "Optimization 1"
    sequent.sub(pos) match {
      case Some(Exists(vars, q)) if polarityInSeq(sequent, pos) > 0 =>
        inst match {
          case Some((what, repl)) =>
            if (StaticSemantics.boundVars(q).intersect(StaticSemantics.freeVars(repl)).isEmpty) existsR(what, repl)(pos)
              ...
              "Unable to instantiate because instance " +
      ...
      case Some(Forall(vars, q)) if polarityInSeq(sequent, pos) < 0 =>
        inst match {
          case Some((what, repl)) =>
            if (StaticSemantics.boundVars(q).intersect(StaticSemantics.freeVars(repl)).isEmpty) allL(what, repl)(pos)
            else throw new IllFormedTacticApplicationException(
              "Unable to instantiate because instance " + repl.prettyString + " is bound in " + q.prettyString
            )
          case None if (unobservable ++ unobservable.map(postVar)).intersect(vars).isEmpty =>
            require(vars.size == 1)
            val (v, post) = vars.map(v => (v, postVar(v))).head
      ...
      case Some(e) => throw new TacticInapplicableFailure(
          "Optimization 1 only applicable to existential quantifier in positive polarity or universal quantifier in negative polarity, but got " + e
            .prettyString + " with polarity " + polarityInSeq(sequent, pos)
        )
      case None => throw new IllFormedTacticApplicationException(
          "Position " + pos + " does not point to a valid position in sequent " + sequent.prettyString
        )
  ...
  private def toMetricT(fml: Formula): BelleExpr = {
    ...
    val propNF = chaseCustom({
      ...
      case And(_: BinaryCompositeFormula, _) =>
        (Ax.andRecursor.provable, PosInExpr(0 :: Nil), PosInExpr(0 :: Nil) :: Nil) :: Nil
      case And(_, _: BinaryCompositeFormula) =>
        (Ax.andRecursor.provable, PosInExpr(0 :: Nil), PosInExpr(1 :: Nil) :: Nil) :: Nil
      ...
      case Or(_: BinaryCompositeFormula, _) =>
        (Ax.orRecursor.provable, PosInExpr(0 :: Nil), PosInExpr(0 :: Nil) :: Nil) :: Nil
      case Or(_, _: BinaryCompositeFormula) =>
        (Ax.orRecursor.provable, PosInExpr(0 :: Nil), PosInExpr(1 :: Nil) :: Nil) :: Nil
      case _ => Nil
    })
