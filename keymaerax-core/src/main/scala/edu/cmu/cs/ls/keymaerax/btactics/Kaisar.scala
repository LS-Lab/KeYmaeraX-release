...
object Kaisar {
  ...
  case class RBSolve(t: Variable, fmlT: Formula, dc: Variable, fmlDC: Formula, sols: List[(Variable, Formula)])
      extends RuleSpec
  ...
  case class RDSolve(t: Variable, fmlT: Formula, dc: Variable, fmlDC: Formula, sols: List[(Variable, Formula)])
      extends RuleSpec
  ...
  case class History(steps: List[HistChange]) {
    def hasTimeStep(ts: TimeName): Boolean = {
      steps.exists({
        case hcts: HCTimeStep => hcts.ts == ts
        case _ => false
      })
    }
    ...
    def nextIndex(x: VBase): Int = {
      steps.count {
        case HCRename(b, _, _) => b.name == x
        case _ => false
      }
    }

    def update(x: VBase, e: Term): History = { History(HCAssign(Assign(Variable(x), e)) :: steps) }
    ...
    private def partBefore[A](xs: List[A], f: (A => Boolean)): (List[A], List[A]) = {
      xs match {
        ...
        case (x :: xs) =>
          if (f(x)) { (Nil, x :: xs) }
          else { partBefore(xs, f) match { case (xs, ys) => (x :: xs, ys) } }
      }
    }
    ...
    def update(x: VBase): History = {
      ...
      val hist = recent.map {
        case ch: HCAssign => ch.rename(from, to)
        case x => x
      } ++ old
    ...
    def resolveHelp(x: VBase, renSteps: List[HistChange] = Nil, subSteps: List[HistChange] = steps): Term = {
      ...
      val fullName = rs.reverse match {
        case (HCRename(_, to, _) :: _) => to
        case _ => Variable(x)
      }
      val xChanges = ss.filter(isThisAssign)
      val xxChanges =
        if (xChanges.length > 1) { xChanges.take(1) }
        else xChanges
  ...
  case class Context(
      gmap: Map[Variable, Position],
      defmap: Map[VBase, Expression],
      fundefmap: Map[VBase, (VBase, Expression)],
  ) {
    def concat(other: Context): Context =
      Context(gmap.++(other.gmap), defmap.++(other.defmap), fundefmap.++(other.fundefmap))
    def addFun(x: VBase, arg: VBase, e: Expression): Context = Context(gmap, defmap, fundefmap.+((x, (arg, e))))
    def add(x: Variable, at: AntePos): Context = { Context(gmap.+((x, at)), defmap, fundefmap) }
    def inter(other: Context): Context = Context(
      gmap.filter({ case (k, v) => other.gmap(k) == v }),
      defmap.filter({ case (k, v) => other.defmap(k) == v }),
      fundefmap.filter({ case (k, v) => other.fundefmap(k) == v }),
    )
    def usubst: USubst = {
      USubst(
        defmap
    ...
    def getAssm(ident: String, ante: immutable.IndexedSeq[Formula]): Formula =
      try { ante(gmap(Variable(ident)).checkAnte.index0) }
      catch {
    ...
    def getDef(ident: String): Expression = {
      ...
      else if (ident.last == '_' && defmap.contains(ident.dropRight(1))) { defmap(ident.dropRight(1)) }
      else { throw new Exception("Tried to get invalid definition for " + ident) }
    }
    def stripNominal(nom: VBase, e: Expression, at: Option[TimeName]): Expression = {
      val f: ExpressionTraversalFunction = new ExpressionTraversalFunction() {
        override def preT(p: PosInExpr, t: Term): Either[Option[StopTraversal], Term] = {
          t match {
            case FuncOf(Function(fname, _, _, _, _), args) if fname == nom =>
              at match {
                case Some(atTime) =>
                  Right(FuncOf(Function(atTime, domain = Real, sort = Real), args.asInstanceOf[Term]))
        ...
        override def preF(p: PosInExpr, f: Formula): Either[Option[StopTraversal], Formula] = {
          f match {
            case PredicationalOf(Function(fname, _, _, _, _), args) if fname == nom =>
              at match {
                case Some(atTime) =>
                  Right(PredicationalOf(Function(atTime, domain = Bool, sort = Bool), args.asInstanceOf[Formula]))
    ...
    def getFunDef(ident: String, at: Option[TimeName]): Expression = {
      val (tVar, body) =
        ...
        else if (ident.last == '_' && fundefmap.contains(ident.dropRight(1))) { fundefmap(ident.dropRight(1)) }
        else { throw new Exception("Tried to get invalid function definition for " + ident) }
  ...
  object Context {
    ...
    def ofDef(base: VBase, e: Expression): Context = { new Context(Map(), Map(base -> e), Map()) }
  }

  // proof-checking
  def eval(method: Method, pr: Provable): Provable = {
    val e: BelleExpr = method match {
  ...
  def eval(up: UP, h: History, c: Context, g: Provable): Provable = {
    ...
    val ante = seq.ante
    val fprvs = fps
      .foldLeft[(List[Provable], immutable.IndexedSeq[Formula])]((Nil, ante))({
        case ((prvs: List[Provable], ante2: immutable.IndexedSeq[Formula]), fp: FP) =>
          val prv: Provable = eval(fp, h, c, ante2)
          (prv :: prvs, ante2 ++ immutable.IndexedSeq(prv.conclusion.ante.head))
      })
      ._1
    val withFacts: Provable = fprvs.foldLeft[Provable](g)({ case (pr: Provable, next: Provable) =>
      pr(Cut(next.conclusion.succ.head), 0)(next, 1)
    })
    ...
    def matchesAssm(i: Int, pat: Expression): Boolean = {
      (patIdent(pat), patChild(pat)) match {
        ...
        case (Some("neg"), Some(p)) => !matchesAssm(i, p)
        case (Some(id), _) =>
          val hasOne = c.hasAssm(id)
          hasOne && c.getAssm(id, ante) == ante(i)
      }
    }
    val inds: immutable.IndexedSeq[Int] = withFacts
      .subgoals
      .head
      .ante
      .zipWithIndex
      .filter({ case (f, i) =>
        !pats.exists({ case pat =>
    ...
    val indss =
      if (use.isEmpty) { immutable.IndexedSeq[Int]() }
      else { inds }
  ...
  private def doTravel(h: History, c: Context, at: Option[TimeName]): ExpressionTraversalFunction = {
    new ExpressionTraversalFunction() {
      override def preT(p: PosInExpr, e: Term): Either[Option[StopTraversal], Term] = e match {
        case v: Variable => Right(h.resolve(v.name, at))
        ...
        case FuncOf(Function(fname, _, _, _, _), arg) if h.hasTimeStep(fname) => Right(expand(arg, h, c, Some(fname)))
        case FuncOf(Function(fname, _, _, _, _), arg) if c.hasFunDef(fname) =>
          Right(expand(c.getFunDef(fname, at).asInstanceOf[Term], h, c, at))
        case FuncOf(Function(fname, _, _, _, _), arg) if c.hasDef(fname) => Right(c.getDef(fname).asInstanceOf[Term])
        case _ => Left(None)
      }
      override def preF(p: PosInExpr, e: Formula): Either[Option[StopTraversal], Formula] = e match {
        case PredOf(Function(fname, _, _, _, _), arg) if c.hasDef(fname) => Right(c.getDef(fname).asInstanceOf[Formula])
        case PredicationalOf(Function(fname, _, _, _, _), arg) if c.hasDef(fname) =>
          Right(c.getDef(fname).asInstanceOf[Formula])
        case UnitPredicational(fname, arg) if c.hasDef(fname) => Right(c.getDef(fname).asInstanceOf[Formula])
      ...
      override def preP(p: PosInExpr, e: Program): Either[Option[StopTraversal], Program] = e match {
        case ProgramConst(fname, _) if c.hasDef(fname) => Right(c.getDef(fname).asInstanceOf[Program])
        case SystemConst(fname, _) if c.hasDef(fname) => Right(c.getDef(fname).asInstanceOf[Program])
        case DifferentialProgramConst(fname, _) if c.hasDef(fname) => Right(c.getDef(fname).asInstanceOf[Program])
        case _ => Left(None)
      }
    }
  }
  ...
  def expand(e: Term, h: History, c: Context, at: Option[TimeName]): Term = {
    ExpressionTraversal.traverse(doTravel(h, c, at), e).get
  }
  ...
  def uniqueMatch(pat: Expression, c: Context, ante: immutable.IndexedSeq[Formula]): Int = {
    val ident: Option[String] = pat match {
      case ns: NamedSymbol => Some(ns.name)
      case f: ApplicationOf => Some(f.func.name)
      case _ => None
    }
    ...
    ofIdent match {
      ...
      case None =>
        val matches: immutable.IndexedSeq[(Formula, Int)] = ante
          .zipWithIndex
          .filter({ case (fml, _) => doesMatch(pat, fml, c, ante) })
        ...
        goal match {
          case (Forall(xs, q)) =>
            try {
              ...
              val subst: Subst = UnificationMatch(
                AxiomInfo("all instantiate").provable.conclusion.succ.head.asInstanceOf[Imply].left,
                goal,
              )
              val pair = (FuncOf(Function("f", None, Unit, Real), Nothing), t2)
              val vpair = (Variable("x_"), xs.head)
              val renu: RenUSubst = RenUSubst(
                immutable
                  .IndexedSeq
                  .concat(
                    immutable.IndexedSeq[(Expression, Expression)](vpair, pair),
                    subst.usubst.subsDefsInput.map({ case x => (x.what, x.repl) }),
                  )
              )
  ...
  def polyK(pr: Provable, facts: List[Provable], onSecond: Boolean = false, impAtEnd: Boolean = false): Provable = {
    val doPrint = false
    facts match {
      case Nil => /*NoProofTermProvable(pr) */
        interpret(DebuggingTactics.debug("foobar", doPrint = doPrint) & close, pr)
      case fact :: facts =>
        val pos =
          if (impAtEnd) { pr.subgoals(if (onSecond) 1 else 0).ante.length - 1 }
          else 0
        val Box(a, Imply(p, q)) = pr.subgoals(if (onSecond) 1 else 0).ante(pos)
        val e: BelleExpr = cut(Imply(Box(a, Imply(p, q)), Imply(Box(a, p), Box(a, q)))) <
          (
            DebuggingTactics.debug("a", doPrint = doPrint) &
              ...
              (
                /* use */ close,
                implyL(-(pos + 2)) <
                  (hide(1) & useAt(ElidingProvable(fact, Declaration(Map.empty)), PosInExpr(Nil))(1), nil),
              ),
            DebuggingTactics.debug("e", doPrint = doPrint) & /* show */ cohideR(2) & useAt(Ax.K, PosInExpr(Nil))(1, Nil),
          ) & hide(-1) & DebuggingTactics.debug("before recur", doPrint = doPrint)
        polyK(
          interpret(
            if (onSecond) { nil < (nil, e) }
            else { e },
            pr,
          ),
          facts,
  ...
  def collectLoopInvs(
      ...
      hh: History,
      c: Context,
  ): (List[Variable], List[Formula], List[Formula], List[SP], List[SP], IP) = {
    ip match {
  ...
  case class DiffCutInfo(proofVar: Variable, bcFml: Formula, indFml: Formula, bcProof: SP, indProof: SP)
      extends OdeInvInfo
  ...
  def collectODEInvs(ip: IP, h: History, hh: History, c: Context): (List[OdeInvInfo], IP) = {
    ip match {
      case Inv(x, fml, pre, inv, tail) =>
        ...
        (
          DiffCutInfo(proofVar = x, bcFml = expand(fml, h, c, None), indFml = expand(fml, hh, c, None), pre, inv) ::
            infs,
          t,
        )
    ...
    (ip, goal) match {
      ...
      case (nextInv: Inv, Box(Loop(a), post)) => {
        ...
        def baseCase(fmlPres: List[(Formula, SP)], done: List[Formula] = Nil): BelleExpr = {
          fmlPres match {
            ...
            case (fml, pre: SP) :: Nil =>
              ...
              assert(
                tail.isProved,
                "Failed to prove first base case subgoal " + fml + " in subproof " + pre + ", left behind provable " +
                  tail.prettyString,
              )
              DebuggingTactics.debug("BASE0 I WANTED", doPrint = true) &
                useAt(ElidingProvable(tail, Declaration(Map.empty)), PosInExpr(Nil))(1) &
                DebuggingTactics.debug("BASE0B", doPrint = true)
            case (fml, pre: SP) :: fps =>
              val preSeq = Sequent(ante, immutable.IndexedSeq(fml))
              val tail = eval(pre, h, c, Provable.startProof(preSeq))
              assert(
                tail.isProved,
                "Failed to prove additional base case subgoal " + fml + " in subproof " + pre +
                  ", left behind provable " + tail.prettyString,
              )
              val e1 = useAt(ElidingProvable(tail, Declaration(Map.empty)), PosInExpr(Nil))(1)
              val e2 = baseCase(fps, fml :: done)
              andR(1) <
                (
                  DebuggingTactics.debug("left", doPrint = true) & e1,
                  DebuggingTactics.debug("right", doPrint = true) & e2,
                )
          }
        }
        def indCase(
            fmlPres: List[(Variable, (Formula, SP))],
            c: Context,
            done: immutable.IndexedSeq[Formula] = immutable.IndexedSeq(),
        ): BelleExpr = {
          fmlPres match {
            case Nil => ???
            case (x, (fml, inv: SP)) :: Nil =>
              ...
              assert(
                tail.isProved,
                "Failed to prove first inductive case subgoal " + fml + " in subproof " + inv +
                  ", left behind provable " + tail.prettyString,
              )
            ...
            case (x, (fml, pre: SP)) :: fps =>
              ...
              assert(
                tail.isProved,
                "Failed to prove additional inductive case subgoal " + fml + " in subproof " + pre +
                  ", left behind provable " + tail.prettyString,
              )
        ...
        val unpack: BelleExpr =
          if (invs.length > 1) { andL(AntePosition(1)) & andL(Symbol("Llast")) * (invs.length - 2) }
          else {
            def rot(pr: ProvableSig, p: SuccPosition): ProvableSig =
              ElidingProvable(rotAnte(pr.underlyingProvable), pr.defs)
            val e = TacticFactory.anon((pos: ProvableSig, seq: SuccPosition) => { rot(pos, seq) })(1)
            e
          }
        val e: BelleExpr = DLBySubst.loop(conj, pre = nil)(1) <
          (
            unsaveVars(gg, bvs, rewritePost = true) & baseCase(baseFmls.zip(pres)),
            nil,
            unpack & indCase(vars.zip(indFmls.zip(invs)), c),
          )
      ...
      case (firstInv, Box(ODESystem(ode, constraint), post)) if !firstInv.isInstanceOf[Finally] => {
        ...
        def proveInvs(pr: Provable, infs: List[OdeInvInfo], c: Context): (Provable, Context) = {
          infs match {
            ...
            case DiffCutInfo(proofVar, bcFml, indFml, bcProof, indProof) :: invTail =>
              val e: BelleExpr = dC(indFml)(1) <
                (
                  nil,
                  DebuggingTactics.debug("Blah", doPrint = true) &
                    (dI()(1) | (DebuggingTactics.debug("Blahhh", doPrint = true) & dW(1) & master())),
                )
        ...
        def flatAnd(f: Formula, n: Int): List[Formula] = {
          f match {
            case And(a, b) if n > 0 => flatAnd(a, n - 1) ++ List(b)
            case ff => List(ff)
          }
        }
        def unflatAnd(l: List[Formula]): Formula = l match {
          case List(l) => l
          case (x :: xs) => And(x, unflatAnd(xs))
        }
        ...
        val killAnds =
          if (nInvs > 0) { andL(Symbol("Llast")) * (nInvs) }
          else nil
        val killTrue =
          if (nInvs > 0) { hideL(AntePosition(anteSize + 1)) }
          else { hideL(Symbol("Llast")) }
      ...
      case (Finally(tail: SP), post) => { eval(tail, h, c, g) }
    }
  }
  ...
  def assertBranches(had: Int, wanted: Int): Unit = {
    if (had != wanted) throw BadBranchingFactorException(had, wanted)
  }

  private def saveVars(
      pr: Provable,
      savedVars: Set[Variable],
      invCurrent: Boolean = true,
      hide: Boolean = true,
  ): (Provable, List[Variable]) = {
    savedVars.foldRight[(Provable, List[(Variable)])]((pr, Nil))({ case (v: Variable, (acc, accVs)) =>
      ...
      val e: BelleExpr = discreteGhost(v, Some(vv))(1) & TactixLibrary.exhaustiveEqR2L(hide = false)(Symbol("Llast")) &
        TactixLibrary.eqL2R(-last)(1) &
        (if (invCurrent) { TactixLibrary.eqL2R(-last)(1 + accVs.length - last) }
         else { nil })
      (interpret(e, acc), vv :: accVs)
    })
  }

  private def unsaveVars(
      pr: Provable,
      bvs: Set[Variable],
      rewritePost: Boolean = false,
      hide: Boolean = true,
  ): BelleExpr = {
    ...
    poses.foldLeft[BelleExpr](nil)({ case (acc, pos) =>
      ...
      val rens = List
        .tabulate(numRens)({ case i => TactixLibrary.eqL2R(pos)(AntePosition(1 + i)) })
        .foldLeft[BelleExpr](nil)({ case (acc, e) => acc & e })
      val doHide =
        if (hide) { hideL(pos) }
        else nil
      val renss =
        if (rewritePost) {
          rens & DebuggingTactics.debug("Rewriting post " + pos, doPrint = true) &
            TactixLibrary.eqL2R(pos)(SuccPosition(1))
        } else rens
      acc & DebuggingTactics.debug("unsaving " + pos, doPrint = true) & /*(TactixLibrary.eqL2R(pos)('L)*)*/
        renss & DebuggingTactics.debug("unsave huh " + pos, doPrint = true) & doHide &
        DebuggingTactics.debug("unsaved " + pos, doPrint = true)
    })
  }
    ...
    goal match {
      case _ => (brule, goal) match {
          ...
          case (RBAssign(hp: Assign), Box(Assign(x, theta), p)) if hp.x == x =>
            ...
            try { gsub = Some(interpret(useAt(Ax.assignbAxiom)(1), g)) }
            catch {
              case e: Throwable =>
                ...
                val hh = h.updateRen(
                  Variable(base.name),
                  Variable(base.name, Some(h.nextIndex(base.name))),
                  AntePos(gg.subgoals.head.ante.length - 1),
                )
          ...
          case (RBMid(varName: Variable, fml: Formula), Box(a, Box(b, p))) =>
            ...
            val seq1: Sequent =
              Sequent(pr1prep.subgoals.head.ante, immutable.IndexedSeq(Box(a, fmlLate)) ++ sequent.succ.tail)
            val pr1: Provable = eval(sp(0), hh, c, Provable.startProof(seq1))
            assert(
              pr1.isProved,
              "Failed to prove subgoal in subproof " + sp(0) + ", left behind provable " + pr1.prettyString,
            )
            ...
            assert(
              pr2.isProved,
              "Failed to prove subgoal in subproof " + sp(1) + ", left behind provable " + pr2.prettyString,
            )
            ...
            val e = poses
              .foldLeft[BelleExpr](nil)({ case (acc, pos) => acc & TactixLibrary.eqR2L(pos)(-1) & hideL(pos) })
            //
            //
            //
            ...
            prr
          case (RBCase(pat1 :: pat2 :: Nil), Box(Choice(a, b), p))
              if doesMatch(pat1, a, c, ante) && doesMatch(pat2, b, c, ante) =>
            assertBranches(sp.length, 2)
            ...
            prr(pr1, 0)(pr2, 0)
          case (RBCase(pat1 :: pat2 :: Nil), Box(a, And(p, q)))
              if doesMatch(pat1, p, c, ante) && doesMatch(pat2, q, c, ante) =>
            assertBranches(sp.length, 2)
            val seq1: Sequent = Sequent(sequent.ante, immutable.IndexedSeq(Box(a, p)) ++ sequent.succ.tail)
          ...
          case (
                RBSolve(t: Variable, fmlT: Formula, dc: Variable, fmlDC: Formula, sols: List[(Variable, Formula)]),
                Box(ODESystem(ode, q), p),
              ) =>
          ...
          case (
                RDSolve(t: Variable, fmlT: Formula, dc: Variable, fmlDC: Formula, sols: List[(Variable, Formula)]),
                _,
              ) => ???
          case (RDAssignAny(x: Variable, xVal: Term), _) => ???
          case (_, Box(Compose(a, b), p)) => eval(brule, sp, h, c, interpret(useAt(Ax.composeb)(1), g))
          case (_, Box(Test(_), p)) => eval(brule, sp, h, c, interpret(useAt(Ax.testb)(1), g))
    ...
    sp match {
      case Show(phi: Formula, proof: UP) =>
        ...
        def ifilter[T, S](p: (T, Int) => Boolean, f: (T, Int) => S, xs: IndexedSeq[T]): IndexedSeq[S] = {
          xs.zipWithIndex
            .flatMap({ case (x, i) =>
              if (p(x, i)) { IndexedSeq(f(x, i)) }
              else { IndexedSeq() }
            })
        }
        val expanded = expand(phi, h, c, None)
        val hideInds = ifilter(
          { case (x, i) => !doesMatch(expanded, x, c, ante) }: ((Formula, Int) => Boolean),
          { case (x, i) => i }: ((Formula, Int) => Int),
          succ,
        )
        val upsucc = ifilter(
          { case (x, i) => !hideInds.contains(i) }: ((Formula, Int) => Boolean),
          { case (x, i) => x }: ((Formula, Int) => Formula),
          succ,
        )
        ...
        assert(
          pr.isProved,
          "Shown formula not proved: " + phi + " not proved by " + pr.prettyString + "\\n\\n at " + g.prettyString,
        )
        pr
      ...
      case Have(x: Variable, fml: Formula, sp: SP, tail: SP) =>
        ...
        assert(
          prOut.isProved,
          "Failed to prove subgoal " + x + ":" + fml + " in subproof " + sp + ", left behind provable " +
            prOut.prettyString,
        )
        val size = prOut.conclusion.ante.size
        val newPos = AntePos(size)
        val succSize = g.subgoals.head.succ.length
        def cohideR(g: Provable, n: Int): Provable =
          if (n == 0) { g }
          else { cohideR(g(HideRight(SuccPos(0)), 1), n - 1) }
  ...
  def collectVarPat(t: Term): Option[List[BaseVariable]] = {
    t match {
      case (Pair(x, y)) => (collectVarPat(x), collectVarPat(y)) match {
  ...
  case class PatternMatchError(msg: String) extends Exception {
    override def toString: String = "Pattern Match Error: " + msg
  }
  ...
  def pmatch(pat: Expression, e: Expression, c: Context, ante: immutable.IndexedSeq[Formula]): Context = {
    def patExn(pat: Expression, e: Expression, additional: String = "N/A"): PatternMatchError = {
      PatternMatchError("Expected expression " + e + " to match pattern " + pat + " Additional message?: " + additional)
    }
    val exn: PatternMatchError = patExn(pat, e)
    def atom(e1: Expression, e2: Expression, additional: String = "N/A") = {
      if (e1 == e2) { Context.empty }
      else {
        throw new PatternMatchError(
          "Expected atomic expression " + e2 + " to match atomic pattern " + e1 + " Additional message?: " + additional
        )
      }
    }
    def matchDef(id: String) = {
      if (e == c.getDef(id)) { Context.empty }
      else {
        throw PatternMatchError(
          "Bound variable pattern " + id + " only matches expression " + c.getDef(id) + " but instead got " + e
        )
      }
    }
    pat match {
      ...
      case PredOf(Function("p", _, _, _, _), args)
          if collectVarPat(args).isDefined || collectNegVarPat(args).isDefined =>
        (collectVarPat(args), collectNegVarPat(args)) match {
          case (Some(pos), _) =>
            ...
            if (posvs.subsetOf(fvs)) { Context.empty }
            else { throw patExn(pat, e, "Bad variable occurrence") }
          case (_, Some(neg)) =>
            val negvs: SetLattice[Variable] = SetLattice(neg.toSet.asInstanceOf[Set[Variable]])
            val fvs: SetLattice[Variable] = StaticSemantics.freeVars(e)
            if (negvs.intersect(fvs).isEmpty) { Context.empty }
            else { throw patExn(pat, e, "Bad negative variable occurrence") }
      ...
      case PredOf(Function("inter", _, _, _, _), Pair(pat1, pat2)) => pmatch(pat1, e, c, ante)
          .inter(pmatch(pat2, e, c, ante))
      ...
      case PredOf(Function("assm", _, _, _, _), which) if patIdent(which).isDefined && c.hasAssm(patIdent(which).get) =>
        ...
        if (e == f) { Context.empty }
        else { throw patExn(pat, e, "Expression did not match assumption " + ident) }
      case FuncOf(Function("f", _, _, _, _), args) => (collectVarPat(args), collectNegVarPat(args)) match {
          case (Some(pos), _) =>
            val posvs: SetLattice[Variable] = SetLattice(pos.toSet.asInstanceOf[Set[Variable]])
            val fvs: SetLattice[Variable] = StaticSemantics.freeVars(e)
            if (posvs.subsetOf(fvs)) { Context.empty }
      ...
      case FuncOf(Function("inter", _, _, _, _), Pair(pat1, pat2)) => pmatch(pat1, e, c, ante)
          .inter(pmatch(pat2, e, c, ante))
      ...
      case PredicationalOf(Function("assm", _, _, _, _), which)
          if patIdent(which).isDefined && c.hasAssm(patIdent(which).get) =>
        val ident = patIdent(which).get
        val f = c.getAssm(ident, ante)
        if (e == f) { Context.empty }
        else { throw patExn(pat, e, "Expression did not match assumption " + ident) }
      ...
      case BaseVariable(vname, _, _) =>
        if (vname.last == '_' && !c.hasDef(vname)) { Context.ofDef(vname.dropRight(1), e) }
        else if (pat == e) { Context.empty }
        else { throw exn }
      case FuncOf(fn: Function, Nothing) =>
        if (fn.name.last == '_' && !c.hasDef(fn.name)) { Context.ofDef(fn.name.dropRight(1), e) }
        else if (pat == e) { Context.empty }
        else { throw exn }
      case UnitFunctional(name: String, _, _) =>
        if (name.last == '_' && !c.hasDef(name)) { Context.ofDef(name.dropRight(1), e) }
        else if (pat == e) { Context.empty }
        else { throw exn }
      ...
      case _ => (pat, e) match {
          ...
          case (e1: Plus, e2: Plus) => pmatch(e1.left, e2.left, c, ante).concat(pmatch(e1.right, e2.right, c, ante))
          case (e1: Minus, e2: Minus) => pmatch(e1.left, e2.left, c, ante).concat(pmatch(e1.right, e2.right, c, ante))
          case (e1: Times, e2: Times) => pmatch(e1.left, e2.left, c, ante).concat(pmatch(e1.right, e2.right, c, ante))
          case (e1: Divide, e2: Divide) => pmatch(e1.left, e2.left, c, ante).concat(pmatch(e1.right, e2.right, c, ante))
          case (e1: Power, e2: Power) => pmatch(e1.left, e2.left, c, ante).concat(pmatch(e1.right, e2.right, c, ante))
          case (e1: Pair, e2: Pair) => pmatch(e1.left, e2.left, c, ante).concat(pmatch(e1.right, e2.right, c, ante))
          case (e1: Equal, e2: Equal) => pmatch(e1.left, e2.left, c, ante).concat(pmatch(e1.right, e2.right, c, ante))
          ...
          case (e1: LessEqual, e2: LessEqual) =>
            pmatch(e1.left, e2.left, c, ante).concat(pmatch(e1.right, e2.right, c, ante))
          case (e1: Less, e2: Less) => pmatch(e1.left, e2.left, c, ante).concat(pmatch(e1.right, e2.right, c, ante))
          case (e1: And, e2: And) => pmatch(e1.left, e2.left, c, ante).concat(pmatch(e1.right, e2.right, c, ante))
          case (e1: Or, e2: Or) => pmatch(e1.left, e2.left, c, ante).concat(pmatch(e1.right, e2.right, c, ante))
          case (e1: Imply, e2: Imply) => pmatch(e1.left, e2.left, c, ante).concat(pmatch(e1.right, e2.right, c, ante))
          case (e1: Equiv, e2: Equiv) => pmatch(e1.left, e2.left, c, ante).concat(pmatch(e1.right, e2.right, c, ante))
          case (e1: Box, e2: Box) => pmatch(e1.program, e2.program, c, ante).concat(pmatch(e1.child, e2.child, c, ante))
          case (e1: Diamond, e2: Diamond) =>
            pmatch(e1.program, e2.program, c, ante).concat(pmatch(e1.child, e2.child, c, ante))
          case (e1: Choice, e2: Choice) => pmatch(e1.left, e2.left, c, ante).concat(pmatch(e1.right, e2.right, c, ante))
          ...
          case (e1: ODESystem, e2: ODESystem) =>
            pmatch(e1.ode, e2.ode, c, ante).concat(pmatch(e1.constraint, e2.constraint, c, ante))
  ...
  def doesMatch(pat: Expression, e: Expression, c: Context, ante: immutable.IndexedSeq[Formula]): Boolean = {
    try { pmatch(pat, e, c, ante); true }
