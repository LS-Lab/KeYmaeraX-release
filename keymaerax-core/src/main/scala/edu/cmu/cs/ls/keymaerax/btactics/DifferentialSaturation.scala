...
object DifferentialSaturation extends Logging {
  ...
  def pQE(f: Formula): Formula = {
    val qe = ToolProvider
      .qeTool()
      .getOrElse(throw new ProverSetupException("partialQE requires a QETool, but got None"))
  ...
  def parametricCandAux(vars: List[Variable], deg: Int): List[Term] = {
    vars match {
      ...
      case (v :: vs) =>
        val ls = List
          .range(0, deg)
          .foldLeft(List[Term]())((t, i) => {
            val cands = parametricCandAux(vs, deg - i)
            if (i == 0) cands else cands.map(c => Times(Power(v, Number(i)), c)) ++ t
          })
        ls
    }
  }

  def freshConst(ind: Int): Variable = { Variable(freshConstName, Some(ind)) }
  ...
  def repeatedSubst(e: Expression, subst: Map[Variable, Term], limit: Int = 100): Option[Expression] = {
    val e2 = subst.foldLeft(e)((f, s) => SubstitutionHelper.replaceFree(f)(s._1, s._2))
    if (e == e2) Some(e2) else if (limit == 0) None else repeatedSubst(e2, subst, limit - 1)
  }

  def occamify(subs: List[Map[Variable, Term]], instVars: List[Variable]): List[Map[Variable, Term]] = {
    subs.map(s => s ++ instVars.diff(s.keySet.toList).map(v => (v, Number(1))).toMap)
  }
  ...
  def guessInstantiations(fml: Formula, instVars: List[Variable]): List[Map[Variable, Term]] = {
    ...
    if (pr.isProved) return List(Map())
    ...
    subs.filter(s => {
      ...
      if (fml2.isEmpty) false else { proveBy(fml2.get.asInstanceOf[Formula], Idioms.?(QE)).isProved }
    })
  }
  ...
  def parametricInvariant(
      ode: DifferentialProgram,
      domls: List[Formula],
      candSet: List[Variable],
      insts: List[Variable],
      fresh: Int,
      deg: Int,
      tool: Option[SimplificationTool],
      ineq: Boolean,
  ): Option[(Term, Map[Variable, Term], List[Variable], Int)] = {
    ...
    val invCand =
      if (!StaticSemantics.freeVars(invCandPre).isEmpty && tool.isDefined) { tool.get.simplify(invCandPre, domls) }
      else invCandPre
  ...
  def parametricInvariants(
      ode: DifferentialProgram,
      dom: List[Formula],
      candSets: List[List[Variable]],
      degLimit: Int,
      withIneq: Boolean = false,
      tool: Option[SimplificationTool] = None,
