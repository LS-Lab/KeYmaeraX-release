...
// @TODO: Replace assumptions with assertions
object Totalizer {
  ...
  def findController(s: Statement): Option[(Statement, Statement)] = {
    s match {
      // found the plant
      case pode: ProveODE => Some((Triv(), pode))
  ...
  def replaceControls(template: Statement, fallback: Option[Statement]): Statement = {
    ...
    KaisarProof.block(l :: (newMods ++ List(r)))
  }
  ...
  def splitAtAssigns(s: Statement): (Statement, List[Modify], Statement) = {
    s match {
      case Block(ss) =>
        ...
        val (mods, right) = (ss.takeWhile(isModify).asInstanceOf[List[Modify]], ss.dropWhile(isModify))
  ...
  def mainBody(s: Statement, fallback: Option[Statement]): Option[Statement] = {
    findController(s).flatMap({case (ctrl, plant) =>
      splitAtBranch(ctrl) match {
        case (ctrl, Triv(), Triv()) =>
          fallback match {
            case Some(fb) => Some(BoxChoice(ctrl, fb))
            case None => Some(replaceControls(ctrl, None))
          }
        case (head, branching, footer) =>
          ...
          Some(KaisarProof.block(head :: rebranch :: footer :: Nil))
      }
    })
  }
  ...
  def findLoop(s: Statement, fallback: Option[Statement]): Option[Statement] = {
    s match {
      case bl: BoxLoop => mainBody(bl.s, fallback)
      case Block(s :: ss) =>
