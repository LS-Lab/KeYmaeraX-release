...
object PolynomialArithV2
  ...
  ): PolynomialRing = TwoThreeTreePolynomialRing(variableOrdering, monomialOrdering)
  ...
  final case class NonSupportedOperationInapplicability(cause: NonSupportedOperationException)
      extends TacticInapplicableFailure(
        "Tactic inapplicable because of an operation that is not supported by polynomial arithmetic: " +
          cause.getMessage,
        cause,
      )
...
object MonomialOrders {
  val variableConstantOrdering: Ordering[Term] = Ordering.by {
    ...
    case t => throw new IllegalArgumentException(
        "variableConstantOrdering expects BaseVariable or FuncOf(_, Nothing) of sort Real, but got " + t
      )
  }

  /** reverse lexicographic order -- @note: strange legacy default */
  def revlex(variableOrdering: Ordering[Term]): Ordering[IndexedSeq[(Term, Int)]] =
    new Ordering[IndexedSeq[(Term, Int)]] {
  ...
  def grevlex(variableOrdering: Ordering[Term]): Ordering[IndexedSeq[(Term, Int)]] =
    new Ordering[IndexedSeq[(Term, Int)]] {
      override def compare(x: IndexedSeq[(Term, Int)], y: IndexedSeq[(Term, Int)]): Int = {
        val lx = x.length
        val ly = y.length
...
object PolynomialArithV2Helpers {
  ...
  def anyArgify(prv: ProvableSig): ProvableSig = {
    ...
    val us = USubst(
      StaticSemantics
        .signature(prv.conclusion)
        .flatMap {
          case f @ Function(n, None, Unit, Real, None) =>
            Some(SubstitutionPair(FuncOf(f, Nothing), UnitFunctional(n, AnyArg, Real)))
          case _ => None
        }
        .toIndexedSeq
    )
  ...
  def impliesElim(PQ: ProvableSig, P: ProvableSig): ProvableSig = {
    ...
    pq match {
      case Imply(pp, q) => ProvableSig.startProof(Sequent(PQ.conclusion.ante, IndexedSeq(q)), PQ.defs ++ P.defs)(
          CutRight(p, SuccPos(0)),
          0,
        )(PQ, 1)(P, 0)
...
/**
 * A polynomial is represented as a set of monomials stored in a 2-3 Tree, the ordering is lexicographic A monomial is
 * represented as a coefficient and a power-product. A coefficient is represented as a pair of BigDecimals for
 * num/denom. A power product is represented densely as a list of exponents
 *
 ...
 *   - power product [e1, ..., en] is "x1^e1 * ... * xn ^ en", where instead of "x^0", we write "1" in order to avoid
 *     trouble with 0^0, i.e., nonzero-assumptions on x or the like
 *
 * All operations on the representations update the proofs accordingly.
 */
case class TwoThreeTreePolynomialRing(
    variableOrdering: Ordering[Term],
    monomialOrdering: Ordering[IndexedSeq[(Term, Int)]],
  ...
  case class Coefficient(num: BigDecimal, denom: BigDecimal, prvO: Option[ProvableSig] = None) {
    ...
    def unary_- : Coefficient = {
      ...
      Coefficient(
        ...
        Some(useDirectly(
          coefficientNegPrv,
          Seq(("x_", lhs), ("xn_", numN), ("xd_", denomN), ("nxn_", Number(-num))),
          Seq(prv, negPrv),
        )),
      )
    }
    ...
    def +(that: Coefficient): Coefficient = {
      ...
      val numericPrv = ProvableSig.proveArithmetic(
        BigDecimalQETool,
        List(
     ...
     * }}}
     */
    def normalized: (ProvableSig, Term) =
    ...
    def split(newNum: BigDecimal, newdenom: BigDecimal): (ProvableSig, Coefficient, Coefficient) = {
      ...
      val numericCondition = ProvableSig.proveArithmetic(
        BigDecimalQETool,
      ...
      (
        ...
        Coefficient(num1, denom1),
        Coefficient(num2, denom2),
      )
    }
  ...
  case class Monomial(coeff: Coefficient, powerProduct: SparsePowerProduct, prvO: Option[ProvableSig] = None) {
    ...
    def powersString: String = {
      ...
      (if (coeff.num.compareTo(1) == 0 && coeff.denom.compareTo(1) == 0 && powers.exists(_._2 > 0)) ""
       ...
       else coeff.rhsString + sep) + powers.map { case (v, p) => Power(v, Number(p)) }.mkString(sep)
    }
        ...
        if (l >= 0 && r >= 0) {
          ...
          variableOrdering.compare(x, y) match {
            ...
            case c if c < 0 =>
              ...
              val newPrv = useDirectly(
                timesPowersRight,
                Seq(("xs_", xs), ("ys_", ys), ("xys_", xys), ("y_", Power(y, Number(j)))),
                Seq(recPrv),
              )
            ...
            case c if c > 0 =>
              ...
              val newPrv = useDirectly(
                timesPowersLeft,
                Seq(("xs_", xs), ("ys_", ys), ("xys_", xys), ("x_", Power(x, Number(i)))),
                Seq(recPrv),
              )
    ...
    def unary_- : Monomial = {
      ...
      val newPrv = useDirectly(
        negTimes,
        Seq(("l_", lhs), ("a_", coeff.rhs), ("b_", rhs.right), ("c_", newCoeff.rhs)),
    ...
    def +(that: Monomial): Option[Monomial] =
      if (that.powers == powers) Some {
        val newCoeff = coeff.forgetPrv + that.coeff.forgetPrv
    ...
    def normalizePowers(c: Coefficient, t: Term): (ProvableSig, Term) = t match {
      case Times(Number(one), t @ Power(v, Number(n))) =>
        ...
        if (c.num.compareTo(1) == 0 && c.denom.compareTo(1) == 0) {
          // c = 1
          if (n.compareTo(1) == 0) { (useDirectly(normalizePowers1V, Seq(("c_", c.lhs), ("v_", v)), Seq(cdPrv)), v) }
          else { (useDirectly(normalizePowers1R, Seq(("c_", c.lhs), ("t_", t)), Seq(cdPrv)), t) }
        } else {
          // c = d
          if (n.compareTo(1) == 0) {
            (useDirectly(normalizePowersCV, Seq(("c_", c.lhs), ("d_", d), ("v_", v)), Seq(cdPrv)), Times(d, v))
          } else { (useDirectly(normalizePowersCP, Seq(("c_", c.lhs), ("d_", d), ("t_", t)), Seq(cdPrv)), Times(d, t)) }
        }
      case Times(ps, t @ Power(v, Number(n))) =>
        val (cpsPrv, cps) = normalizePowers(c, ps)
        if (n.compareTo(1) == 0) {
          (
            useDirectly(normalizePowersRV, Seq(("c_", c.lhs), ("ps_", ps), ("cps_", cps), ("v_", v)), Seq(cpsPrv)),
            Times(cps, v),
          )
        } else {
          (
            useDirectly(normalizePowersRP, Seq(("c_", c.lhs), ("ps_", ps), ("cps_", cps), ("t_", t)), Seq(cpsPrv)),
            Times(cps, t),
          )
        }
      case Number(n) =>
    ...
    def normalized: ProvableSig = {
      ...
      if (coeff.num.compareTo(0) == 0)
        useDirectly(normalizeMonom0, Seq(("x_", lhs), ("c_", coeff.rhs), ("ps_", powersTerm)), Seq(prv, cnPrv))
      else if (coeff.num.compareTo(0) > 0) {
        val (cpsPrv, cps) = normalizePowers(coeff.forgetPrv, powersTerm)
        useDirectly(
          normalizeMonomCS,
          Seq(("x_", lhs), ("c_", coeff.rhs), ("ps_", powersTerm), ("cps_", cps)),
          Seq(prv, cpsPrv),
        )
      } else {
        ...
        useDirectly(
          normalizeMonomNCS,
          Seq(("x_", lhs), ("c_", coeff.rhs), ("m_", m.rhs), ("ps_", powersTerm), ("cps_", cps)),
    ...
    def approx(prec: Int): (ProvableSig, Monomial, Monomial) = {
      ...
      (
        useDirectly(
          splitMonomial,
          Seq(("c_", coeff.rhs), ("x_", powersTerm), ("c1_", c1.rhs), ("c2_", c2.rhs), ("m_", lhs)),
          Seq(cPrv, prv),
        ),
        Monomial(c1, powerProduct),
        Monomial(c2, powerProduct),
      )
    }
    ...
    def degree(include: Term => Boolean = _ => true): Int =
      if (coeff.num.compare(0) != 0) powers.map { case (t, p) => if (include(t)) p else 0 }.sum else 0
    ...
    case Plus(l, r) =>
      ...
      rhsOf(rPrv) match {
        case Number(n) if n.compareTo(0) == 0 =>
          ...
          useDirectly(
            reassocRight0,
            Seq(("t_", lhsOf(prv)), ("l_", l), ("r_", r), ("ll_", rhsOf(llPrv))),
            Seq(prv, rPrv, llPrv),
          )
        case Plus(rs, rr) =>
          ...
          useDirectly(
            reassocRightPlus,
            Seq(("t_", lhsOf(prv)), ("l_", l), ("r_", r), ("rs_", rs), ("rr_", rr), ("lrs_", rhsOf(lrsPrv))),
            Seq(prv, rPrv, lrsPrv),
          )
        case c =>
          val llPrv = reassoc(equalReflex(l))
          rhsOf(llPrv) match {
            case Number(n) if n.compareTo(0) == 0 =>
              useDirectly(
                ...
                Seq(("t_", lhsOf(prv)), ("l_", l), ("r_", r), ("c_", c)),
                Seq(prv, rPrv, llPrv),
              )
            case ll => useDirectly(
                reassocRightConst,
                Seq(("t_", lhsOf(prv)), ("l_", l), ("r_", r), ("c_", c), ("ll_", ll)),
                Seq(prv, rPrv, llPrv),
              )
          }
      }
  ...
  final case class UnknownPolynomialImplementationException(other: Polynomial)
      extends RuntimeException("only TreePolynomials are supported, but got " + other)
  ...
  sealed trait TreePolynomial extends Polynomial {
    ...
    override def prettyTerm: Polynomial = {
      ...
      val prettyPrv = proveBy(
        Equal(PolynomialArithV2Helpers.rhsOf(prettyRepr), PolynomialArithV2Helpers.rhsOf(repr)),
        useAt(repr, PosInExpr(1 :: Nil))(1, 1 :: Nil) & useAt(prettyRepr, PosInExpr(1 :: Nil))(1, 0 :: Nil) &
          byUS(equalReflexive),
      )
    ...
    def lookup(x: IndexedSeq[(Term, Int)]): Option[Monomial] = this match {
      ...
      case Branch3(left, v1, mid, v2, right, _) => monomialOrdering.compare(x, v1.powers) match {
          ...
          case c if c < 0 => left.lookup(x)
          case c if c > 0 =>
            monomialOrdering.compare(x, v2.powers) match {
              case 0 => Some(v2)
              ...
              case c if c < 0 =>
                mid.forgetPrv.insert(x) match {
                  ...
                  case Sprout(mx) =>
                    ...
                    Sprout(Branch2(
                      Branch2(left, v, mx.left, None),
                      mx.value,
                      Branch2(mx.right, w, right, None),
                      Some(newPrv),
                    ))
                }
              case c if c > 0 =>
    ...
    def *(other: Polynomial): TreePolynomial = other match {
      case other: TreePolynomial => this match {
          case Empty(_) =>
          ...
          case Branch3(left, value1, mid, value2, right, _) =>
            val sum = (left.forgetPrv * other) + (other * value1.forgetPrv) + (mid.forgetPrv * other) +
              (other * value2.forgetPrv) + (right.forgetPrv * other)
            val newPrv = useDirectly(
              timesBranch3,
              IndexedSeq(
                ("t_", lhs),
    ...
    def ^(n: Int): TreePolynomial = n match {
      case 0 => One.updatePrv(useDirectly(powerZero, IndexedSeq(("t_", lhs), ("one_", One.rhs)), Seq(One.prv)))
      case 1 => this.updatePrv(useDirectly(powerOne, IndexedSeq(("t_", lhs), ("s_", rhs)), Seq(prv)))
      case n =>
        if (n >= 0) {
          if (n % 2 == 0) {
            val m = n / 2
    ...
    def isConstant: Boolean = this match {
      case Empty(_) => true
      case Branch2(l, v, r, _) => v.isConstant && l.isConstant && r.isConstant
      case Branch3(l, v1, m, v2, r, _) => v1.isConstant && v2.isConstant && l.isConstant && m.isConstant && r.isConstant
    }
    ...
    def ^(other: Polynomial): TreePolynomial = other match {
      case other: TreePolynomial =>
        if (other.isConstant) {
          ...
          rhsOf(otherNormalized) match {
            case Number(i) if i.isValidInt =>
              ...
              val newPrv = useDirectly(
                powerPoly,
                Seq(("p_", lhs), ("q_", other.lhs), ("i_", Number(i)), ("r_", rhsOf(pi.prv))),
                Seq(otherNormalized, pi.prv),
              )
              pi.updatePrv(newPrv)
            case Number(bd) => throw NonSupportedExponentException("Exponent must be integer but normalizes to " + bd)
            case d @ Divide(l, r) =>
        ...
        } else { throw NonSupportedExponentException("Exponent must be a constant polynomial.") }
      case _ => throw UnknownPolynomialImplementationException(other: Polynomial)
    }
        ...
        if (other.isConstant) {
          ...
          rhsOf(otherNormalized) match {
            ...
            case Divide(Number(n), Number(d)) =>
              ...
              val newPrv = useDirectly(
                divideRat,
                Seq(("p_", lhs), ("q_", other.lhs), ("n_", Number(n)), ("d_", Number(d)), ("r_", rhsOf(pi.prv))),
                Seq(otherNormalized, pi.prv),
              )
              pi.updatePrv(newPrv)
            case Neg(_) =>
              val npq = -(this / (-other))
              val newPrv =
                useDirectly(divideNeg, Seq(("p_", lhs), ("q_", other.lhs), ("r_", rhsOf(npq.prv))), Seq(npq.prv))
              npq.updatePrv(newPrv)
            case _ => throw new RuntimeException("Constant polynomials must normalize to Number, Divide, or Neg.")
          }
        } else { throw NonSupportedDivisorException(s"Divisor ${other.term} must be a constant polynomial.") }
      case _ => throw UnknownPolynomialImplementationException(other: Polynomial)
    }
    ...
    /**
     * normalized to nicer rhs: drop 0 for empty leaves, normalized monomials, reassociated e.g., t = (0 + a + 0) + b +
     * (0 + c + 0 + d + 0) ~~> t = a + b + c + d
     */
    ...
    def zeroTest: Option[ProvableSig] = {
      ...
      rhsOf(normalizedPrv) match {
        case Number(n) if n.compareTo(0) == 0 => Some(normalizedPrv)
        case _ => None
      }
    }
    ...
    def partitionMonomials(
        P: Monomial => Boolean
    )(acc: (Seq[Monomial], Seq[Monomial])): (Seq[Monomial], Seq[Monomial]) = {
      def accumulate(m: Monomial)(acc: (Seq[Monomial], Seq[Monomial])): (Seq[Monomial], Seq[Monomial]) =
    ...
    def partition(P: (BigDecimal, BigDecimal, PowerProduct) => Boolean): (Polynomial, Polynomial, ProvableSig) = {
      ...
      val prv0 = (this - p1 - p2)
        .zeroTest
        .getOrElse(throw new RuntimeException("Runtime error in 0-proof for partitioning - this should never fail!"))
      val eqPrv = useDirectly(
        partition2,
        Seq(("t_", lhs), ("r_", rhs), ("t1_", p1.lhs), ("r1_", p1.rhs), ("t2_", p2.lhs), ("r2_", p2.rhs)),
    ...
    def approx(prec: Int): (ProvableSig, TreePolynomial, TreePolynomial) = this match {
      case Empty(_) => (useDirectly(splitEmpty, Seq(("t_", lhs)), Seq(prv)), Empty(None), Empty(None))
      case Branch2(left, value, right, _) =>
    ...
    override def divideAndRemainder(
    ...
    ): (Polynomial, Polynomial, ProvableSig) = {
      ...
      val quotRem = ringsLibrary
        .ring
        .divideAndRemainder(ringsLibrary.toRing(rep1), ringsLibrary.toRing(rep2))
        .map { p =>
    ...
    override def variables: Set[Term] = monomials.foldLeft[Set[Term]](Set()) { case (vars, mon) =>
      vars ++ mon.powerProduct.sparse.map(_._1)
    }
  ...
  def Var(term: Term): TreePolynomial = Branch2(
    Empty(None),
    Monomial(Coefficient(1, 1, None), wfPowerProduct(Seq((term, 1))), None),
    Empty(None),
    Some(varLemma(substAny("v_", term))),
  )
  def Var(term: Term, power: Int): TreePolynomial = Branch2(
    Empty(None),
    Monomial(Coefficient(1, 1, None), wfPowerProduct(Seq((term, power))), None),
    Empty(None),
  ...
  def Const(num: BigDecimal, denom: BigDecimal): TreePolynomial = Branch2(
    Empty(None),
    Monomial(Coefficient(num, denom, None), wfPowerProduct(Seq()), None),
    Empty(None),
    Some(useDirectly(rationalLemma, Seq(("n_", Number(num)), ("d_", Number(denom))), Seq())),
  )
  def Const(num: BigDecimal): TreePolynomial = Branch2(
    Empty(None),
    Monomial(Coefficient(num, 1, None), wfPowerProduct(IndexedSeq()), None),
    Empty(None),
  ...
  case class Branch3(
    ...
    override def degree(include: Term => Boolean): Int = left.degree(include) max value1.degree(include) max
      mid.degree(include) max value2.degree(include) max right.degree(include)
    override def resetRepresentation(newRepresentation: ProvableSig): Polynomial = {
      require(rhsOf(newRepresentation) == rhsOf(prv))
  ...
  val normalizeAt: DependentPositionTactic = anon { (pos: Position, seq: Sequent) =>
    seq.sub(pos) match {
      ...
      case Some(Equal(s, t)) => useAt(eqNormalize, PosInExpr(0 :: Nil))(pos) & normalizeAt(pos)
      case Some(t: Term) => useAt(PolynomialArithV2.reportBelleThrowables { normalize(t) }, PosInExpr(0 :: Nil))(pos)
      case Some(e) => throw new TacticInapplicableFailure(
          "normalizeAt only applicable to equalities or terms, but got " + e.prettyString
        )
      case None => throw new IllFormedTacticApplicationException(
          "Position " + pos + " does not point to a valid position in sequent " + seq.prettyString
        )
  ...
  def powerDivideLemma(i: Int) =
    if (i <= 100) powerDivideLemmas(i) else provePowerDivideLemma(i, 100, powerDivideLemmas)

  def ratForm(term: Term): (Polynomial, Polynomial, ProvableSig) = {
    lazy val ringsLibrary = new RingsLibrary(Seq(term))

    def gcdAndRemainders(
        a: ringsLibrary.Ring,
        b: ringsLibrary.Ring,
    ...
    def rec(term: Term): (Polynomial, ringsLibrary.Ring, Polynomial, ringsLibrary.Ring, ProvableSig) = term match {
        ...
        binop match {
          case Plus(_, _) | Minus(_, _) =>
            ...
            val prv = useDirectly(
              ...
              Seq(
                ("x_", binop.left),
                ("nx_", nx.term),
                ("dx_", rhsOf(dx.representation)),
                ("y_", binop.right),
                ("ny_", ny.term),
                ("dy_", rhsOf(dy.representation)),
                ("gcd_", gcd),
                ("rx_", rx),
                ("ry_", ry),
              ...
              Seq(xPrv, yPrv, dx.representation, dy.representation, nz.representation, dz.representation),
            )
          ...
          case Times(_, _) =>
            ...
            val prv = useDirectly(
              ...
              Seq(xPrv, yPrv, nz.representation, dz.representation),
            )
            (nz.resetTerm, nzR, dz.resetTerm, dzR, prv)
          case Divide(_, _) =>
          ...
          case Power(_, _) =>
            if (dyR.isOne && nyR.isConstant) {
              ...
              val ymPrv = (ny / dy)
                .equate(ofInt(m))
                .getOrElse(throw new RuntimeException("expected to prove that polynomial equals integer"))
              ...
              val prv = useDirectly(
                ...
                Seq(xPrv, yPrv, ymPrv, powerDividePrv, nz.representation, dz.representation),
              )
              (nz.resetTerm, nzR, dz.resetTerm, dzR, prv)
            } else
              throw NonSupportedExponentException("exponent does not normalize to a natural number: " + ny + "/" + dy)
        }
      case Neg(c) =>
        ...
        val prv = useDirectly(
          ratFormNeg,
          Seq(("x_", c), ("nx_", nx.term), ("dx_", dx.term), ("nz_", rhsOf(nz.representation))),
          Seq(xPrv, nz.representation),
        )
      ...
      case a: AtomicTerm =>
        ...
        (
          aP.resetTerm,
          ringsLibrary.toRing(a),
          ofInt(1).resetTerm,
          ringsLibrary.ring.getOne,
          useDirectly(
            divideIdentity,
            Seq(("x_", a), ("y_", rhsOf(aP.representation)), ("z_", rhsOf(ofInt(1).representation))),
            Seq(aP.representation, ofInt(1).representation),
          ),
        )
      case _ => throw NonPolynomialArithmeticException("Operation not supported by polynomial arithmetic: " + term)
    }
