...
/**
 * A context is used during proof-checking and elaboration or transformation passes in order to track assumptions. In
 * Kaisar, contexts also track changes to program variables, line labels, and local definitions of function symbols.
 ...
 *   1. During proof-checking, assertions and lemmas in a context have already been proven, so their proofs need not be
 *      re-executed.
 *   1. During elaboration or proof-checking, optional annotations may be automatically filled in for a context. For
 *      example, the result of a [[Note]] lemma is inferred in the [[edu.cmu.cs.ls.keymaerax.cdgl.kaisar.ProofChecker]]
 *   1. Contexts may introduce meta nodes which do not exist in proofs, but are helpful for proof checking. For example
 *      [[edu.cmu.cs.ls.keymaerax.cdgl.kaisar.BoxLoopProgress]] remembers the entirety of a loop while its body is being
 *      processed.
 *
 * @author
 *   Brandon Bohrer
...
case class Context(s: Statement) {
   ...
   * proving some branch of the switch. An exception is thrown if the difference does not exist.
   */
    ...
    if (this.s == other.s) Context.empty
      ...
      case (Block(ss1), Block(ss2)) =>
        if (ss2.length > ss1.length) fail
          ...
          if (ss1.length >= ss2.length) lrStatement.:+(Block(ss1.drop(ss2.length))) else lrStatement
        }
      case (Block(l), r) if l.nonEmpty =>
        val diff = Context(l.head) -- Context(r)
        if (diff.isEmpty) Context(Block(l.tail)) else Context(KaisarProof.block(diff.s :: l.tail))
      ...
      case (bc: BoxChoice, bcPr: BoxChoiceProgress) if bc == bcPr.bc =>
        if (bcPr.onBranch == 0) Context(bc.left) -- Context(bcPr.progress)
        else Context(bc.right) -- Context(bcPr.progress)
      case (l, r) => fail
    }
  }
  ...
  def add(x: Ident, fml: Formula): Context = { :+(Assume(x, fml)) }
  ...
  def getAssignments(x: Variable): List[Formula] = withOuter
    .withoutGhost
    .searchAll(QAssignments(x, onlySSA = false), Set(), Set())
    .formulas
    .map(elaborateStable)

  // Return all facts which mention any SSA-variant of x
  def getMentions(x: Variable): List[Formula] = withOuter
    .withoutGhost
    .searchAll(QProgramVar(x), Set(), Set())
    .formulas
    .map(elaborateStable)

  /**
   * Look up definitions of a proof variable, starting with the most recent. tabooProgramVars are for soundness,
   * tabooFactVars are to avoid duplicate lookup results
   */
  def searchAll(cq: ContextQuery, tabooProgramVars: Set[Variable], tabooFactVars: Set[Ident]): ContextResult = {
    def succeed(fmls: Set[(Option[Ident], Formula)], assigns: Set[Assign]): ContextResult = RSuccess(fmls, assigns)
      .admissiblePart(this, tabooProgramVars, tabooFactVars)
      .matchingPart(cq)
      .elaborated(this, cq)
    def matched(x: IdentPat, fml: Formula): ContextResult = Context
      .matchAssume(x, fml, s)
      .admissiblePart(this, tabooProgramVars, tabooFactVars)
      .matchingPart(cq)
      .elaborated(this, cq)
    s match {
      ...
      case Block(ss) =>
        def iter(
            ss: List[Statement],
            cq: ContextQuery,
            tabooProgramVars: Set[Variable],
            tabooFactVars: Set[Ident],
        ): ContextResult = {
          ss match {
            case Nil => ContextResult.unit
            case (s: Phi) :: ss =>
              val left = reapply(s.asgns)
                .searchAll(cq, tabooProgramVars, tabooFactVars)
                .filter({ case (yx, yf, yisAssign) =>
              ...
              leftMatches.foreach({ case ((yx, yf, isAssignment)) =>
                if (yx.nonEmpty && t.tabooFacts.contains(yx.get) && !isElaborationContext && !isInverseGhost)
                  throw ProofCheckException(
                    s"Inverse ghost fact $yx inaccessible outside of other inverse ghosts",
                    node = s,
                  )
              })
              val taboos = tabooProgramVars ++ VariableSets(s).boundVars
      ...
      case switch @ Switch(sel, pats) =>
        ...
        if (eachResult.isEmpty) ContextResult.unit else eachResult.reduce(RBranch)
      ...
      case po: ProveODE => findAll(po, po.ds, cq, tabooProgramVars, tabooFactVars)
          .++(findAll(po.dc, cq, tabooProgramVars, tabooFactVars))
      ...
      case fr @ For(metX, met0, metIncr, conv, guard, body, guardDeltaOpt) =>
        val convMatch = conv match {
          case Some(cnv) => matched(cnv.pat, cnv.f)
        ...
        guardDeltaOpt match {
          case Some(guardDelta) => convMatch ++ matched(guard.pat, Metric.weakNegation(guard.f, guardDelta))
          case _ => convMatch
        }
      case While(_, _, body) =>
        // @TODO
      ...
      case BoxChoiceProgress(bc, onBranch, progress) => reapply(progress).searchAll(cq, tabooProgramVars, tabooFactVars)
    }
  }
   ...
   *
   * @param mod
   *   A [[Modify]] statement which is searched for bindings
   * @param cq
   *   A user-supplied search query
   * @return
   *   all bindings which satisfy [[finder]], starting with the most recent binding (i.e. variable's current value)
   */
  private def findAll(
      mod: Modify,
      cq: ContextQuery,
    ...
    mod
      .asgns
      .map({
        case (Some(p), x, Some(f)) if (cq.matches(Some(p), Equal(x, f), isAssignment = false)) =>
        ...
        case (None, x, Some(f)) if (cq.matches(Some(x), Equal(x, f), isAssignment = true)) =>
          ...
          if (isGhost && !outer.isGhost) { ContextResult.unit }
          else succeed(Set(), Set(Assign(x, f)))
        case (Some(_), x, None) =>
   ...
   *
   * @param odeContext
   *   ODE proof in which ds appears
   * @param ds
   *   A [[DiffStatement]] statement which is searched for bindings
   * @param f
   *   A user-supplied search predicate
   * @return
   *   all bindings which satisfy [[finder]]
   */
  private def findAll(
      odeContext: ProveODE,
      ds: DiffStatement,
      ...
      case AtomicODEStatement(AtomicODE(xp, e), solIdent) if (!this.isInverseGhost) =>
        ...
        odeContext.bestSolutions match {
          case Some(sols) =>
            ...
            val (ident, isUnnamed) = solIdent match {
              case Nothing => (xp.x, true)
              case id: Variable => (id, false)
              case FuncOf(fn, arg) => (Variable(fn.name), false)
            }
            if (solMap.contains(xp.x)) {
              val eqFact = Equal(xp.x, solMap(xp.x))
              val fullFact = odeContext.timeVar match {
                case None => eqFact
                case Some(tvar) => And(eqFact, GreaterEqual(tvar, Number(0)))
              }
              // Search using full fact and explicit name if possible
              if (cq.matches(Some(ident), fullFact, isUnnamed)) succeed(Set((Some(ident), fullFact)), Set())
              ...
              // a fast one and return the whole fact
                ...
                succeed(Set(), Set(Assign(xp.x, solMap(xp.x))))
              else ContextResult.unit
            } else ContextResult.unit
          case None => ContextResult.unit
        }
      case AtomicODEStatement(dp, _) => ContextResult.unit
      case DiffProductStatement(l, r) => findAll(odeContext, l, cq, tabooProgramVars, tabooFactVars)
          .++(findAll(odeContext, r, cq, tabooProgramVars, tabooFactVars))
      case DiffGhostStatement(ds) => withGhost.findAll(odeContext, ds, cq, tabooProgramVars, tabooFactVars)
      case InverseDiffGhostStatement(ds) => withInverseGhost
          .findAll(odeContext, ds, cq, tabooProgramVars, tabooFactVars)
    }
  }
    ...
    getFor match {
      ...
      case Some(For(metX, met0, metIncr, conv, guard, body, None)) =>
        ...
        val assertionPosQuery: ContextQuery =
          if (assertionCandidates.nonEmpty) assertionCandidates
            .map(v => QUnify(Greater(v, Number(0))))
          ...
          case Nil =>
            ...
            val guardPosQuery: ContextQuery =
              if (guardCandidates.nonEmpty) {
                guardCandidates.map(v => QUnify(Greater(v, Number(0)))).reduce[ContextQuery](QUnion(_, _))
              } else QNil()
            val guardFound = findAll(guardPosQuery).formulas
            guardFound match {
              case Greater(v, _) :: _ => v
              case _ :: _ => throw new Exception("Bad pattern match - bug found")
              case Nil => throw ProofCheckException(
                  "Could not infer guard comparison delta in for loop, " +
   ...
   *
   * @param dc
   *   A [[DomainStatement]] statement which is searched for bindings
   * @param cq
   *   A user-supplied search query
   * @return
   *   all bindings which satisfy [[cq]]
   */
  private def findAll(
      dc: DomainStatement,
      cq: ContextQuery,
    ...
    dc match {
      case DomAssume(x, fml) if !isInverseGhost =>
        matchAssume(x, fml, dc).admissiblePart(this, tabooProgramVars, tabooFactVars).matchingPart(cq)
      case DomAssume(x, fml) => ContextResult.unit
      case DomAssert(x, fml, _) =>
        matchAssume(x, fml, dc).admissiblePart(this, tabooProgramVars, tabooFactVars).matchingPart(cq)
      case DomAnd(l, r) => findAll(l, cq, tabooProgramVars, tabooFactVars) ++
          findAll(r, cq, tabooProgramVars, tabooFactVars)
      case dw @ DomWeak(dc) => withInverseGhost.findAll(dc, cq, tabooProgramVars, tabooFactVars) match {
          // @TODO: Better message
          case cr if cr.nonEmpty =>
            throw ProofCheckException(
              s"Weakened domain constraint $dc selected result ${cq}, but selection should have been empty",
              node = dw,
            )
  ...
  def getHere(id: Ident, wantProgramVar: Boolean = false): Option[Formula] = get(id, wantProgramVar)
    .map(elaborateStable)

  /**
   * Used in proof statements such as ProveODE which have not only implicit assumptions but which need to consider
   * immediately preceding assignments. The lastBlock can include straight-line assignments and facts, which may appear
  ...
  private def lastStatements(allowProgress: Boolean, taboos: Set[Variable] = Set()): List[Statement] = {
    s match {
      ...
      case Ghost(s) =>
        ...
        val mapped = res.map({
          case g: Ghost => g
          case s: Statement => Ghost(s)
        })
        mapped
      case Block(ss) => ss
          .foldRight[(List[Statement], Set[Variable], Boolean)]((List(), taboos, false))({
            case (s, (acc, taboos, done)) =>
   ...
   * ghosted or unghosted.
   */
  def lastBlock: Block = { Block(lastStatements(allowProgress = true, taboos = Set())) }

  def lastBlockSmall: Block = { Block(lastStatements(allowProgress = false, taboos = Set())) }
  ...
  private def finishFact(f: Formula, phis: List[Phi]): Formula =
    destabilize(phis.foldLeft(f)((fml, phi) => Context.substPhi(phi, fml)))
  ...
  private def destabilize(f: Formula): Formula = SubstitutionHelper.replacesFree(f)(f => KaisarProof.getStable(f))
  ...
  def elaborateStable(e: Expression): Expression = e match {
  ...
  private def replaceFunctions(t: Term, node: ASTNode = Triv()): Option[Term] = {
    t match {
      case f: FuncOf => signature.find(fn => fn._1.name == f.func.name) match {
          case Some((fn, lf)) if fn == f.func =>
            ...
            if (elabArgs.length != lf.args.length) throw ElaborationException(
              s"Function ${f.func.name} called with ${elabArgs.length}, expected ${lf.args.length}",
              node = node,
            )
            ...
            Some(
              SubstitutionHelper
                .replacesFree(lf.e)({
                  case (v: Variable) => argMap.get(v)
                  case _ => None
                })
                .asInstanceOf[Term]
            )
          case Some((fn, lf)) => throw ElaborationException(
              s"Function ${f.func} found with type ${f.func.domain} -> ${f.func.sort}(interpreted?: ${f.func.interpreted}), expected ${fn
                  .domain}->${fn.domain} (interpreted?: ${fn.interpreted})",
              node = node,
            )
          case None => KaisarProof.getAt(f) match {
              case Some((trm, lr)) => Some(KaisarProof.makeAt(
                  elaborateFunctions(trm, node),
                  LabelRef(lr.label, lr.args.map(x => elaborateFunctions(x, node))),
                ))
              case None =>
                if (KaisarProof.isBuiltinFunction(f.func)) {
                  val elabChild = elaborateFunctions(f.child, node)
                  Some(FuncOf(f.func, elabChild))
                } else { throw ElaborationException(s"Unknown function ${f.func}", node = node) }
            }
        }
      case _ => None
    }
  ...
  private def replaceProgs(hp: Program, node: ASTNode = Triv()): Option[Program] = {
    hp match {
      case pc: ProgramConst => programSignature.get(pc) match {
  ...
  private def replacePreds(t: Formula, node: ASTNode = Triv()): Option[Formula] = {
    t match {
      case f: PredicationalOf => KaisarProof.getAt(f, node) match {
          case Some((trm, lr)) => Some(KaisarProof.makeAt(
              elaborateFunctions(trm, node),
              LabelRef(lr.label, lr.args.map(x => elaborateFunctions(x, node))),
            ))
          case None => throw ElaborationException(
              s"Predicational functions ${f.func} should only be used for internal data structures",
              node = node,
            )
        }
      case f: PredOf => signature.get(f.func) match {
          case Some(lf) =>
            val elabChild = elaborateFunctions(f.child, node)
            val elabArgs = StandardLibrary.tupleToTerms(elabChild, node)
            if (elabArgs.length != lf.args.length) throw ElaborationException(
              s"Predicate ${f.func.name} called with ${elabArgs.length}, expected ${lf.args.length}",
              node = node,
            )
            val argMap = lf.args.zip(elabArgs).toMap
          ...
          case None =>
            ...
            } else throw ElaborationException(s"Unknown predicate function ${f.func}", node = node)
        }
      case _ => None
    }
  }
  ...
  def elaborateFunctions(f: Term, node: ASTNode): Term = ExpressionTraversal
    .traverse(elabFunctionETF(node), f)
    .getOrElse(f)
  def elaborateFunctions(fml: Formula, node: ASTNode): Formula = ExpressionTraversal
    .traverse(elabFunctionETF(node), fml)
    .getOrElse(fml)
  def elaborateFunctions(prog: Program, node: ASTNode): Program = ExpressionTraversal
    .traverse(elabFunctionETF(node), prog)
    .getOrElse(prog)
  def elaborateFunctions(e: Expression, node: ASTNode): Expression = e match {
    case f: Term => elaborateFunctions(f, node)
    case fml: Formula => elaborateFunctions(fml, node)
  }
  def elaborateFunctions(pt: ProofTerm, node: ASTNode): ProofTerm = pt match {
  ...
  def unifyAll(pat: Expression): ContextResult = { searchAll(QUnify(pat), Set(), Set()) }
  ...
  def fresh(ident: String = ghostVar): Ident = {
    var i = 0
    while (contains(Variable(ident + i))) { i = i + 1 }
    Variable(ident + i)
  }
...
object Context {
  ...
  private def sameHead(e: Expression, f: Expression): Boolean = {
    (e, f) match {
      case (bcf1: BinaryCompositeFormula, bcf2: BinaryCompositeFormula) => bcf1.reapply(bcf2.left, bcf2.right) == bcf2
      case (ucf1: UnaryCompositeFormula, ucf2: UnaryCompositeFormula) => ucf1.reapply(ucf2.child) == ucf2
  ...
  /**
   * @param e
   *   the left-hand side of an assumption statement, which is a pattern that binds fact variables
   * @param f
   *   the right-hand side of an assumption statement, which is a formula expression
   * @return
   *   the set of fact bindings introduced by an assumption statement
   */
  private def matchAssume(e: Expression, f: Formula, node: ASTNode = Triv()): ContextResult = {
    e match {
      ...
      case _ =>
        ...
        if (!sameHead(e, f)) throw ProofCheckException(s"Pattern $e does not match formula $f", node = node)
        (e, f) match {
          case (bcf1: BinaryCompositeFormula, bcf2: BinaryCompositeFormula) => matchAssume(bcf1.left, bcf2.left, node)
              .++(matchAssume(bcf1.right, bcf2.right, node))
          case (ucf1: BinaryCompositeFormula, ucf2: BinaryCompositeFormula) => matchAssume(ucf1.left, ucf2.left, node)
