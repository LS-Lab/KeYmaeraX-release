...
object DerivationInfoPrinter {
  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      val out = DerivationInfo.allInfo.
        map({ case (k, v) => (k, v.persistentInputs, v.numPositionArgs) }).
        map({ case (codeName, argInfo, numPosArgs) => s"""case "$codeName" => DerivationInfo("$codeName", ${printArgInfo(argInfo)}, $numPosArgs)""" }).
        mkString("\n") + "\ncase t => throw ParseException(\"Unknown tactic \" + t)"
      println(out)
    } finally {
      KeYmaeraX.shutdownProver()
    }
  }
