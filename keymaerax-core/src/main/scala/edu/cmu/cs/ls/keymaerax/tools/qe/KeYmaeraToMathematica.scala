...
/**
 * Converts KeYmaeara X [[edu.cmu.cs.ls.keymaerax.core.Expression expression data structures]] to Mathematica Expr
 * objects.
 * @author
 *   Stefan Mitsch
 * @author
...
class KeYmaeraToMathematica extends K2MConverter[KExpr] {
  ...
  private[tools] def convert(e: KExpr): MExpr = {
    val convertInterpretedSymbols = Configuration
      .getBoolean(Configuration.Keys.QE_ALLOW_INTERPRETED_FNS)
      .getOrElse(false)
    insist(
      convertInterpretedSymbols ||
        StaticSemantics
          .symbols(e)
          .forall({
  ...
  protected[tools] def convertTerm(t: Term): MExpr = {
    require(
      ...
      "Mathematica can only deal with reals not with sort " + t.sort,
    )
    t match {
      ...
      case Number(n) =>
        ...
        n.toBigIntExact match {
          ...
          case None =>
            ...
            assert(
              n == num / denom,
              "Expected double to rational conversion to have value " + n + ", but got numerator " + num +
  ...
  protected def convertQuantified(f: Quantified, op: QuantifiedMathOpSpec): MExpr = {
    ...
    @tailrec
    def collectVars(vsSoFar: Array[NamedSymbol], candidate: Formula): (Array[NamedSymbol], Formula) =
      (f, candidate) match {
