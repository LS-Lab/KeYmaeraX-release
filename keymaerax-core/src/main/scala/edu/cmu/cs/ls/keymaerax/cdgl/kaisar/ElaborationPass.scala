...
/**
 * Reduces all other selectors to proof and program-variable selectors. Proof-term selectors are translated to
 * [[Note]]s. Variables, which are ambiguous in the source syntax, are disambiguated to program variables or proof
 * variables, and the "default" selector is lowered as well.
 */
...
class ElaborationPass() {
  ...
  /**
   * Generous notion of free variables: include variables explicitly mentioned in proof term as well as program
   * variables mentioned in any fact used in the proof.
   */
  ...
  private def collectPt(kc: Context, pt: ProofTerm): ProofTerm = {
    pt match {
      ...
      case ProofVar(x) =>
        ...
        got match {
          case Some(_) => pt
          case None if VariableSets(kc).allVars.contains(x) => locate(ProgramVar(x), pt)
          case _ => throw ElaborationException(s"Tried to use unknown fact: $x. Typo?", node = pt)
        }
      case ProofApp(m, n) => locate(ProofApp(collectPt(kc, m), collectPt(kc, n)), pt)
      case _: ProofInstance => pt
      case ProgramVar(x) => throw ElaborationException(
          "Did not expect program variable proof term in selector elimination pass.",
          node = pt,
        )
    }
  }
  ...
  private def collectPts(kc: Context, sel: Selector, goal: Formula): List[Selector] = {
    sel match {
      ...
      case PatternSelector(pat) => kc
          .unify(pat)
          .flatMap({
            case (Some(x), _) => Some(locate(ForwardSelector(ProofVar(x)), sel))
            case _ => None
          })
          .toList
  ...
  private def collectPts(
      kc: Context,
      kce: Context,
      asrt: Assert,
      m: Method,
      goal: Formula,
  ): (List[Selector], Method) = {
    m match {
      case Using(use, m) =>
      ...
      case _: ByProof | _: RCF | _: Auto | _: Prop | _: Solution | _: DiffInduction | _: Exhaustive | _: Hypothesis =>
        (List(), m)
      case GuardDone(fOpt) =>
  ...
  private def collectPts(kc: Context, kce: Context, dc: DomainStatement): DomainStatement = {
    dc match {
      ...
      case DomAssume(x, f) => StandardLibrary
          .factBindings(x, kc.elaborateFunctions(f, dc), dc)
          .map({ case (x, y) => locate(DomAssume(IdentPat(x), y), dc) })
          .reduce[DomainStatement]({ case (l, r) => locate(DomAnd(l, r), dc) })
      case DomAssert(x, f, m) =>
        if (x != Nothing && !x.isInstanceOf[Variable]) throw ElaborationException(
          s"Non-scalar fact patterns not allowed in domain constraint assertion ${dc}",
          node = dc,
        )
  ...
  def apply(s: Statement): Statement = {
    ...
    val ret = ProofTraversal.traverse(
      Context.empty.asElaborationContext,
      Context.empty.asElaborationContext,
      s,
      new TraversalFunction {
        ...
        private def checkAdmissibleAssignmentBlock(ss: List[Modify]): Unit = {
          val _ = ss.foldRight[Set[Variable]](Set())({ case (mod, acc) =>
            ...
            val fv = mod.freeVars
            if (acc.intersect(fv).nonEmpty) throw ElaborationException(
              s"Simultaneous vector assignments are not supported, but assignment $mod tries to use simultaneously-bound variable(s) ${acc
                  .intersect(fv)}. " +
                s"To fix this, assign each variable separately, and if you wish to use a simultaneous assignment, introduce temporary variables for the old value(s) of ${acc
                    .intersect(fv)}.",
              node = mod,
            )
            acc.++(bv)
        ...
        private def elabVectorAssign(modify: Modify): Statement = {
          if (modify.mods.length == 1) modify
          // If the assignment *vector* has a single name, then ghost in the assignment facts and note their conjunction.
            ...
            val (asgnIds, assignsRev) = modify
              .mods
              .foldLeft[List[(Option[Ident], Modify)]](Nil) {
                // * assignments do not introduce facts
                case (xfs, (x, None)) => (None, Modify(Nil, List((x, None)))) :: xfs
                case (xfs, (x, Some(f))) =>
        ...
        private def coerceFormula(e: ProofTerm): ProofTerm = {
          e match {
            ...
            case ProofInstance(FuncOf(Function(name, index, domain, sort, interpreted), arg)) =>
              ProofInstance(PredOf(Function(name, index, domain, Bool, interpreted), arg))
        ...
        private def coerceTerm(e: ProofTerm): ProofTerm = {
          e match {
            ...
            case ProofInstance(PredOf(Function(name, index, domain, sort, interpreted), arg)) =>
              ProofInstance(FuncOf(Function(name, index, domain, Real, interpreted), arg))
            case _ => throw new Exception(s"Expected formula, got $e")
          }
        }
        ...
        private def coercePTSorts(pt: ProofTerm): ProofTerm = {
          pt match {
            case ProofApp(ProofApp(ProofVar(BaseVariable("orIL", _, _)), pt1), pt2: ProofInstance) =>
              ProofApp(ProofApp(ProofVar(Variable("orIL")), coercePTSorts(pt1)), coerceFormula(pt2))
            case ProofApp(ProofApp(ProofVar(BaseVariable("orIR", _, _)), pt1: ProofInstance), pt2) =>
              ProofApp(ProofApp(ProofVar(Variable("orIR")), coerceFormula(pt1)), coercePTSorts(pt2))
            case ProofApp(ProofApp(ProofVar(BaseVariable("falseE", _, _)), pt1), pt2: ProofInstance) =>
              ProofApp(ProofApp(ProofVar(Variable("falseE")), coercePTSorts(pt1)), coerceFormula(pt2))
            case ProofApp(ProofApp(ProofVar(BaseVariable("implyW", _, _)), pt1: ProofInstance), pt2) =>
              ProofApp(ProofApp(ProofVar(Variable("implyW")), coerceFormula(pt1)), coercePTSorts(pt2))
            case ProofApp(ProofApp(ProofVar(BaseVariable("allE", _, _)), pt1), pt2) =>
              ProofApp(ProofApp(ProofVar(Variable("allE")), coercePTSorts(pt1)), coerceTerm(pt2))
            case ProofApp(ProofApp(ProofApp(ProofVar(BaseVariable("existsI", _, _)), pt1), pt2), pt3) => ProofApp(
                ProofApp(ProofApp(ProofVar(Variable("existsI")), coercePTSorts(pt1)), coerceTerm(pt2)),
                coercePTSorts(pt3),
              )
        ...
        override def preS(kc: Context, kce: Context, sel: Statement): Option[Statement] = {
          sel match {
            case mod: Modify =>
              ...
              val elabFuncs = locate(
                Modify(mod.ids, mod.mods.map({ case (x, fOpt) => (x, fOpt.map(x => kce.elaborateFunctions(x, mod))) })),
                sel,
              )
        ...
        private def requireTimeVarForSolution(ds: DiffStatement, clocks: Set[Variable]): Unit = {
          ds match {
            ...
            case AtomicODEStatement(dp, solFact) if clocks.isEmpty =>
              throw ElaborationException("ODE with solution label must have time variable")
            case AtomicODEStatement(dp, solFact) => ()
            case DiffProductStatement(l, r) =>
              requireTimeVarForSolution(l, clocks); requireTimeVarForSolution(r, clocks)
        ...
        private def elaborateODE(kc: Context, kce: Context, ds: DiffStatement): DiffStatement = {
          ds match {
            case AtomicODEStatement(AtomicODE(xp, e), solFact) =>
              AtomicODEStatement(AtomicODE(xp, kce.elaborateFunctions(e, ds)), solFact)
        ...
        override def postS(kc: Context, kce: Context, s: Statement): Statement = {
          s match {
            ...
            case LetSym(_, _, _: Program) =>
              throw ElaborationException("Lets of programs are only allowed in top-level declarations, not in proofs")
            ...
            case For(metX, metF, metIncr, guard, conv, body, metGuard) => locate(
                For(
                  metX,
                  kce.elaborateFunctions(metF, s),
                  kce.elaborateFunctions(metIncr, s),
                  guard,
                  conv,
                  body,
                  metGuard.map { f => kce.elaborateFunctions(f, s) },
                ),
                s,
              )
            ...
            case Switch(scrut, pats) =>
              locate(Switch(scrut, pats.map({ case (x, f: Formula, b) => (x, kce.elaborateFunctions(f, s), b) })), s)
            case s => s
          }
        }
      },
