...
object KaisarKeywordParser {
  val reservedWords: Set[String] =
    Set("by", "RCF", "auto", "prop", "end", "proof", "using", "let", "match", "print", "for")
  ...
  def using[$: P]: P[Using] = (P("using") ~ ws ~ selector.rep ~ ws ~ method).map({ case (sels, meth) =>
    Using(sels.toList, meth)
  })
  def byProof[$: P]: P[ByProof] = ("proof" ~ ws ~ proof ~ ws ~ "end").map(ByProof)

  def method[$: P]: P[Method] = rcf | auto | prop | using | byProof
  def modify[$: P]: P[Modify] = (ident ~ ws ~ ":=" ~ ws ~ term).map({ case (x: Ident, f: Term) =>
    Modify(Nil, List((x, Some(f))))
  })
  def bassignAny[$: P]: P[Modify] =
    ((ident ~ ws ~ ":=" ~ ws ~ "*").map({ case (x: Ident) => Modify(Nil, List((x, None))) }))
  def assert[$: P]: P[Assert] = ("assert" ~ ws ~ ident ~ ws ~ ":" ~ ws ~ formula ~ ws ~ method).map({
    case (ident, formula, method) => Assert(ident, formula, method)
  })
  def assume[$: P]: P[Assume] = ("assume" ~ ws ~ ident ~ ws ~ ":" ~ ws ~ formula).map({ case (ident, formula) =>
    Assume(ident, formula)
  })
  def label[$: P]: P[Label] = (identString ~ ":").map(c => Label(LabelDef(c)))
  def parseMatch[$: P]: P[Match] = ("match" ~ ws ~ term ~ ws ~ "=" ~ ws ~ expression).map({ case (e1, e2) =>
    Match(e1, e2)
  })
  def letFun[$: P]: P[LetSym] = ("let" ~ ws ~ ident ~ "(" ~ ident.rep(sep = ",") ~ ")" ~ ws ~ "=" ~ ws ~ expression)
    .map({ case (f, xs, e) => LetSym(f, xs.toList, e) })
  ...
  // @TODO update
  def branch[$: P]: P[(Term, Formula, Statement)] = (formula ~ ws ~ "=>" ~ ws ~ parseBlock).map({ case (e, blk) =>
    (Nothing, e, blk)
  })
  ...
  def ghost[$: P]: P[Ghost] = ("ghost" ~ ws ~ ident ~ ws ~ "=" ~ ws ~ term).map({ case (x, f) =>
    Ghost(Modify(Nil, List((x, Some(f)))))
  })
  def bsolve[$: P]: P[Statement] =
    (("solve" ~ ws ~ differentialProgram ~ ws ~ "domain" ~ ws ~ ident ~ ws ~ ":" ~ ws ~ formula ~ ws ~ "duration" ~ ws ~
      ident)).map({ case (dp, vdc, dcFml, vdur) =>
      // @TODO
      Block(List())
    })
  ...
  def parseFor[$: P]: P[Statement] =
    ("for" ~ ws ~ ident ~ ws ~ "=" ~ ws ~ term ~ ws ~ ";" ~ ws ~ ident ~ ws ~ ":" ~ ws ~ formula ~ ws ~ ";" ~ ws ~
      ident ~ ws ~ "-=" ~ ws ~ term).map({ case (ident1, f0, vj, j, ident2, f2) =>
      // @TODO
      Block(List())
    })

  def statement[$: P]: P[Statement] = ws ~
    (modify | assert | assume | label | parseMatch | letFun | note | parseBlock | boxChoice | patternMatch | printGoal |
