...
object AngelStrategy {
  private def ofODE(pode: ProveODE): AngelStrategy = {
    ...
    val assignT = pode.duration match {case Some((_, f)) => DAssign(tv, f) case None => NDAssign(tv)}
    ...
    pode.solutions match {
      case None =>
        ...
        val conds = pode.timeVar match {case Some(t) if !pode.isAngelic => And(GreaterEqual(t, Number(0)), invs) case _ => invs}
  ...
  def apply(pf: Statement): AngelStrategy = {
    pf match {
      ...
      case BoxChoice(left, right) => DCompose(apply(left), apply(right))
      case InverseGhost(s) => apply(s)
      case Modify(ids, mods) =>
        mods.map({case (x, None) => NDAssign(x) case (x, Some(f)) => DAssign(x, f)})
            .reduceRight(DCompose)
      case Block(ss) => ss.map(apply).reduceRight(DCompose)
      case Switch(scrutinee, pats) => ASwitch(pats.map({case (x, f, b) => (f, apply(b))}))
      case While(x, j, s) => ALoop(j, apply(s))
      case BoxLoop(s, ih) => DLoop(apply(s))
