...
/**
 * Executable representation for Angelic strategies. Translations to Angelic strategies and to simplified Demon subset.
 * @author
 *   Brandon Bohrer
 */
...
object IDCounter {
  ...
  def setSourceFile(str: String): Unit = {
    sourceFile = (str: StringOps).linesIterator.toList; fullSourceFile = Some(str)
  }
  ...
  def set(i: Int, as: AngelStrategy): Unit = {
    ...
    count = (count + 1).max(i)
    as.location match {
      case Some(loc) => sourceLocationMap = sourceLocationMap.+(i -> loc)
      case None => ()
    }
  }
  ...
  def idMapString: String = {
    idMap
      .toList
      .sortBy({ case (k, v) => k })
      .map({ case (k, v) => s"$k$MAP_SEPARATOR${StrategyPrinter(v)}" })
      .mkString("\n")
  }
...
/* Angel for loop */
case class AForLoop(idx: Ident, idx0: Term, conv: Formula, body: AngelStrategy, idxUp: Term, guardDelta: Option[Term])
...
object Composed {
  ...
  def apply(l: AngelStrategy, r: AngelStrategy): AngelStrategy = { apply(l :: r :: Nil) }
  ...
  def apply(l: SimpleStrategy, r: SimpleStrategy): SimpleStrategy = { apply(l :: r :: Nil) }
}

object SimpleStrategy {

  /**
   * Demonization pass which inlines Angelic strategies into Demon strategies. The resulting Demon strategy is more
   * complex in the sense that Demon must provide (or simulate) choices which Angel previously made.
   * @see
   *   [[BasicDemonStrategy]] wrapper which automatically handles the added complexity of a Demonized strategy, so the
  ...
  def apply(fs: AngelStrategy): SimpleStrategy = {
    fs match {
      ...
      case AForLoop(metX, met0, guard, body, metIncr, guardEpsilon) =>
        val loop = SLoop(Composed(
          List(STest(guard).rememberSourceStrategy(fs), apply(body), SAssign(metX, metIncr).rememberSourceStrategy(fs))
        )).rememberSourceStrategy(fs)
        IDCounter.setOriginal(loop.nodeID, fs)
        // @TODO: Also have guard even if not specified in model somehow
        Composed(List(
          SAssign(metX, met0).rememberSourceStrategy(fs),
          loop,
      ...
      case ASwitch(branches) =>
        ...
        val (choice, _) = xs.foldLeft[(SimpleStrategy, ASwitch)]((x._1, ASwitch(x._2 :: Nil)))({
          case ((accStrat, accOrig), (thisStrat, thisOrig)) =>
...
object AngelStrategy {
    ...
    (pode.solutions, hasAsserts && isDemon) match {
      ...
      case (None, _) | (_, true) =>
        ...
        val assignDX = pode
          ...
          .map(_.dp)
          .toList
          .map({ case AtomicODE(xp, e) => SAssign(xp, e).rememberSourceStatement(pode) })
        ...
        Composed(
          STest(conds).rememberSourceStatement(pode) :: assignT ::
            (assignX ++ assignDX.+:(STest(conds).rememberSourceStatement(pode)))
        )
      case (Some(xfs), _) =>
        val setT = Composed(
          SAssignAny(tv).rememberSourceStatement(pode),
          STest(GreaterEqual(tv, Number(0))).rememberSourceStatement(pode),
        )
  ...
  private def body(pf: Statement, isPhi: Boolean): AngelStrategy = {
    pf match {
      ...
      case Modify(ids, mods) => Composed(mods.map({
          case (x, None) => SAssignAny(x).rememberSourceStatement(pf)
          case (x, Some(f)) =>
            val res = SAssign(x, f).rememberSourceStatement(pf)
            if (isPhi) IDCounter.setOriginal(res.nodeID, STest(True).rememberSourceStatement(pf))
            res
        }))
      case Block(ss) => Composed(ss.map(body(_, isPhi)))
      case Switch(scrutinee, pats) => ASwitch(pats.map({ case (x, f, b) =>
          (f, body(b, isPhi).rememberSourceStatement(b))
        })).rememberSourceStatement(pf)
