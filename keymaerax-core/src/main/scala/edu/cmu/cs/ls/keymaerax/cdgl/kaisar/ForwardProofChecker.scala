...
object ForwardProofChecker {
  ...
  private val binaryBuiltin: Map[String, (String, (ForwardArg, ForwardArg) => Formula)] = Map(
    ...
    "allI" ->
      (
        "allI (x: Variable, proof: P)",
        { case (ExpressionArg(v: Variable), ProvedArg(p)) if !StaticSemantics(p).fv.contains(v) => Forall(List(v), p) },
      ),
    "allE" ->
      (
        "allE (proof: (forall x, P), f: Term)",
        { case (ProvedArg(Forall(xs, p)), ExpressionArg(f: Term)) => subst(xs.head, f, p) },
      ),
    "existsE" ->
      (
        "existsE (proofL: (exists x, P), proofR: (forall y, (P -> Q)))",
        {
          case (ProvedArg(Exists(List(x), p)), ProvedArg(Forall(List(y), Imply(pp, q))))
              if p == pp && x == y && !StaticSemantics(q).fv.contains(x) => q
        },
      ),
  )
  private val ternaryBuiltin: Map[String, (String, (ForwardArg, ForwardArg, ForwardArg) => Formula)] = Map(
    "orE" ->
      (
        "orE (proof: (A | B), proofL: (A -> C), proofR: (B -> C))",
        {
          case (ProvedArg(Or(a, b)), ProvedArg(Imply(aa, c)), ProvedArg(Imply(bb, cc)))
              if a == aa && b == bb && c == cc => c
        },
      ),
    "existsI" ->
      (
        "existsI (x: Variable, f: Term, proof: P(f))",
        { case (ExpressionArg(x: Variable), ExpressionArg(f: Term), ProvedArg(p)) =>
          Exists(List(x), invSubst(x, f, p))
        },
      ),
  )

  val allBuiltins: Set[String] = nullaryBuiltin.keySet ++ unaryBuiltin.keySet ++ binaryBuiltin.keySet ++
    ternaryBuiltin.keySet
  ...
  def apply(con: Context, pt: ProofTerm): Formula = {
    pt match {
      case ProgramVar(x) =>
        ...
        else mentions.map(fml => con.elaborateFunctions(fml, pt)).reduce(And)
      case ProgramAssignments(x, onlySSA) =>
        val asgns = con.getAssignments(x)
        if (asgns.isEmpty) { throw ProofCheckException(s"No assignments found for program variable $x", node = pt) }
      ...
      case app @ ProofApp(ProofVar(s), pt1) if unaryBuiltin.contains(s.name) =>
        unary(s.name, ptToForwardArg(con, pt1), app)
      ...
      case ProofVar(s) => con.getHere(s) match {
          case Some(fml) => con.elaborateFunctions(fml, pt)
          case None => throw ProofCheckException(s"Undefined proof variable $s", node = pt)
        }
      case fullPt @ ProofApp(pt, ProofInstance(e)) => apply(con, pt) match {
      ...
      case ProofApp(pt1, pt2) => (apply(con, pt1), apply(con, pt2)) match {
          case (Imply(p, q), r) if p == r =>
            throw ProofCheckException(s"Argument $r in proof term does not match expected $p", node = pt2)
          case (Imply(p, q), r) => q
