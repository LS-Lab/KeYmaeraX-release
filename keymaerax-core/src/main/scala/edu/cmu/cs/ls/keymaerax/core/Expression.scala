 ...
 *
 *   1. [[Term terms]] are of type [[edu.cmu.cs.ls.keymaerax.core.Term]] of kind
 *      [[edu.cmu.cs.ls.keymaerax.core.TermKind]]
 *   1. [[Formula formulas]] are of type [[edu.cmu.cs.ls.keymaerax.core.Formula]] of kind
 *      [[edu.cmu.cs.ls.keymaerax.core.FormulaKind]]
 *   1. [[Program hybrid programs]] are of type [[edu.cmu.cs.ls.keymaerax.core.Program]] of kind
 *      [[edu.cmu.cs.ls.keymaerax.core.ProgramKind]]
 *   1. [[DifferentialProgram differential programs]] are of type [[edu.cmu.cs.ls.keymaerax.core.DifferentialProgram]]
 *      of kind [[edu.cmu.cs.ls.keymaerax.core.DifferentialProgramKind]]
 ...
 */
case class UnitFunctional(name: String, space: Space, sort: Sort)
...
case class Forall(vars: immutable.Seq[Variable], child: Formula) extends Quantified {
  def reapply = copy
}

/** \exists vars existentially quantified formula for some real value of vars. */
case class Exists(vars: immutable.Seq[Variable], child: Formula) extends Quantified {
  def reapply = copy
}
...
case class Box(program: Program, child: Formula) extends Modal {
  def reapply = copy
}

/** diamond modality some run of program satisfies child: ⟨program⟩child. */
case class Diamond(program: Program, child: Formula) extends Modal {
  def reapply = copy
}
 ...
 */
case class ProgramConst(name: String, space: Space = AnyArg)
