...
/**
 * Wrappable interface for Demon strategies. While this interface is not "simpler" than [[DemonStrategy]], it is
 * designed to work seamlessly with the output of [[SimpleStrategy.apply]]. i.e., if you wish to write a raw
 * [[DemonStrategy]], you will have to be familiar with the [[SimpleStrategy.apply]] implementation, but not if you
 * write [[BasicDemonStrategy]]. Also simplifies initial-state computation.
 * @see
 *   [[DemonStrategy]]
 * @see
 ...
 * [[SimpleStrategy.apply]]. The major contribution is that [[WrappedDemonStrategy]] automatically remembers the Angelic
 * choices from the ambient Angelic strategy, and makes Demon simulate those Angelic choices automatically, leaving you
 * to implement only the true Demon operations.
 */
class WrappedDemonStrategy[number <: Numeric[number, Ternary]](bds: BasicDemonStrategy[number])(
    val env: Environment[number]
  ...
  override def readLoop(id: NodeID): Boolean = (IDCounter.getOriginal(id), IDCounter.get(id)) match {
    case (Some(ALoop(conv, body)), _) => env.holds(conv) == KnownTrue() // if formula holds, continue
    case (Some(AForLoop(idx, idx0, conv, body, inc, guardEpsilon)), _) =>
      env.holds(conv) == KnownTrue() // if formula holds, continue
    case (Some(_), _) => throw new DemonException(
        "Demon expected to be given loop, but was not. Are you playing an angel strategy against an incompatible Demon?"
      )
    case (None, Some(SLoop(_body))) => bds.readDemonLoop(id)
    case (None, _) => throw new DemonException(
        "Demon expected to be given loop, but was not. Are you playing an angel strategy against an incompatible Demon?"
      )
  }

  override def readChoice(id: NodeID): Boolean = (IDCounter.getOriginal(id), IDCounter.get(id)) match {
    ...
    case (Some(_), _) => throw new DemonException(
        "Demon expected to be given choice, but was not. Are you playing an angel strategy against an incompatible Demon?"
      )
  ...
  override def readAssign(id: NodeID, x: Ident): number = {
    val base = x match {
      case bv: BaseVariable => BaseVariable(bv.name)
      case ds: DifferentialSymbol => DifferentialSymbol(BaseVariable(ds.name))
    }
    val name = base match {
      case bv: BaseVariable => bv.name
      case ds: DifferentialSymbol => ds.name + "'"
    }
