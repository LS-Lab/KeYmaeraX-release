...
object ArithmeticSpeculativeSimplification extends TacticProvider {
  ...
  private lazy val smartHideQE: BelleExpr =
    (DebuggingTactics.debug("Bound", DEBUG) & hideNonmatchingBounds & smartHide &
      DebuggingTactics.debug("Bound-hidden", DEBUG) & SimplifierV3.fullSimplify &
      DebuggingTactics.debug("Bound-simplified", DEBUG) & QE & done) |
  ...
  private lazy val speculativeQENoAbs: BelleExpr = anon((_: Sequent) => {
    (DebuggingTactics.debug("Trying orIntro and smart hiding...", DEBUG) & orIntro(smartHideQE) &
      DebuggingTactics.debug("... orIntro and smart hiding successful", DEBUG)) |
      (DebuggingTactics.debug("orIntro failed, trying smart hiding...", DEBUG) & smartHideQE &
        DebuggingTactics.debug("...smart hiding successful", DEBUG)) |
      (DebuggingTactics.debug("All simplifications failed, falling back to ordinary QE", DEBUG) &
        SimplifierV3.fullSimplify & QE & done)
  })
  ...
  def orIntro(finish: BelleExpr): BelleExpr = anon((sequent: Sequent) => {
    def toSingleSucc(retain: Int): BelleExpr = {
      sequent
        .succ
        .indices
        .patch(retain, List.empty, 1)
        .reverse
        .map(i => hideR(SuccPos(i)))
  ...
  lazy val proveOrRefuteAbs: BelleExpr = anon((sequent: Sequent) => {
    ...
    if (symbols.contains(InterpretedSymbols.absF)) exhaustiveAbsSplit & OnAll(
      (SaturateTactic(hideR(Symbol("R"))) & expandAllDefs(Nil) & ToolTactics.assertNoCex & SimplifierV3.fullSimplify &
        QE & done) | speculativeQENoAbs
    )
  ...
  def exhaustiveAbsSplit: BelleExpr = anon((sequent: Sequent) => {
    val absTerms =
      scala
        .collection
        .mutable
        .Set[Term]() // remember which abs are expanded already (absExp tactic expands same term everywhere)
    ...
    val absTactic = (anteAbs ++ succAbs)
      ...
      .map({ case (_, p) =>
        p.map(pos => OnAll(abs(pos) & orL(Symbol("Llast")))).reduceLeftOption[BelleExpr](_ & _).getOrElse(skip)
      })
      .reduceLeftOption[BelleExpr](_ & _)
      .getOrElse(skip)

    absTactic & OnAll(SaturateTactic(andL(Symbol("_")))) &
  ...
  def autoMonotonicityTransform: BelleExpr = anon((sequent: Sequent) => {
    ...
    def transformOnConsistentSign(v: Variable, b: Term, i: Int) = {
      ...
      val canTransform = !vSigns.contains(Sign.Unknown) && vSigns.nonEmpty && bSigns.nonEmpty &&
        vSigns.intersect(bSigns) == vSigns.union(bSigns)
      // @todo on unknown bounds, split into x<=0 | x>=0 and transform on subsequent branches?
      if (canTransform && StaticSemantics.symbols(b).size <= 1) Some(v -> b, i) else None
    }
    val varBounds = sequent
      .ante
      .zipWithIndex
      .flatMap({
    ...
    val bounds = varBounds
      .groupBy({ case (_, p) => p })
      .map({ case (k, v) => k -> v.flatMap({ case ((v, b), _) => v :: b :: Nil }).toSet })
    // first transform formulas that use the bound (are not a bounds formula themselves)
    val transformUses = varBounds
      .map({ case ((v: Variable, b: Term), _) =>
        val bs = StaticSemantics.symbols(b)
        sequent
          .ante
          .zipWithIndex
          .filter({ case (_, i) => !bounds.get(i).exists(_.contains(b)) })
          .map({ case (fml, i) =>
            if (StaticSemantics.symbols(fml).intersect(bs).nonEmpty)
              Idioms.?(transformEquality(Equal(b, v))(AntePos(i)))
            else skip
          })
          .reduceLeftOption[BelleExpr](_ & _)
          .getOrElse(skip)
