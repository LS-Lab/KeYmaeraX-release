...
import edu.cmu.cs.ls.keymaerax.tools.{CounterExampleException, Mathematica, Tool}
...
object ArithmeticTacticsImpl {
      ...
      val t: Tactic = new ConstructionTactic(tacticName) {
        ...
        override def constructTactic(tool: Tool, node: ProofNode): Option[Tactic] = {
          try {
            tool match {
              case qe: QETool =>
                ...
                lemma.fact.conclusion.succ.head match {
                  ...
                  case lemmaFml@Equiv(res, False) =>
                    if(toolId.equals("Mathematica")) {
                      try {
                        val counterExample = tool.asInstanceOf[Mathematica].getCounterExample(res)
                        //                      println("Cannot prove: \n" + res.prettyString + "\nCounter example generated by " + toolId + " is: " + counterExample)
                        throw new CounterExampleException(counterExample)
                      } catch {
                        case e: Exception => println("Cannot prove:\n" + res.prettyString + "\nbut unable to find a counter example with " + toolId)
                      }
                    } else {
                      println("Only apply QE if the result is true, have " + res.prettyString)
                    }
                    None
                }
            }
            ...
            case ex: CounterExampleException => Some(counterExampleT(ex.getMessage) & stopT)
          }
        }
      }
  ...
  /**
   * Prints counter example for some open goal, similar to debugT
   */
  def counterExampleT(s: => Any): Tactic = new Tactic("Counter example") {
    override def applicable(node: ProofNode): Boolean = true

    override def apply(tool: Tool, node: ProofNode): Unit = {
      node.tacticInfo.infos += ("counter example" -> s.toString)
      println("*****\n" + "Cannot prove:\n" + node.sequent + "\n" + "Generated counter example is:\n" + s + "\n***** ")
