              ...
              case qe: QETool =>
                val f = node.sequent.succ.head
                    if(toolId.equals("Mathematica")) {
                      try {
                        val counterExample = tool.asInstanceOf[Mathematica].getCounterExample(f)
                        if(counterExample.equals("false"))
                          println("No counter example exists for\n" + f.prettyString +"\nIt is probably provable.")
                        else
                          println("Cannot prove: \n" + f.prettyString + "\nCounter example generated by " + toolId + " is:\n" + counterExample)
                      } catch {
                        case e: NoCountExException => println(toolId + " cannot find a counter example for\n" + f.prettyString)
                      }
                    }
                    None
            }
