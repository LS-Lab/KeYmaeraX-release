      ...
      val (result, resultCtx) = tactic match {
        ...
        case BranchTactic(children) if children.nonEmpty => goal match {
          case BelleProvable(p, labels) =>
            ...
            } else new BelleDelayedSubstProvable(resultProvable, resultLabels, substs.reduce(_++_),
                  ...
                  if (provable.subgoals.size > 1) {
                    ...
                    runningInner(tactic, BelleProvable(provable.sub(0), labels)) match {
                      ...
                      case BelleProvable(innerProvable, innerLabels) =>
                        ...
                        val result = (BelleProvable(provable(innerProvable, 0), resultLabels), ctx.store(tactic))
                        runningInner = null
                        result
                    }
                  }
