...
object SimplifierV3 extends TacticProvider {
  ...
  private def termAx(ctor: (Term, Term) => Term): (ProvableSig, ProvableSig, ProvableSig) = {
    ...
    val lAx = remember(
      Imply(limp, Imply("A_()".asFormula, Equal(lhs, ctor("LL_()".asTerm, "R_()".asTerm)))),
      prop & exhaustiveEqL2R(Find.FindLPlain("L_()=LL_()".asFormula)) & cohideR(1) & byUS(Ax.equalReflexive),
      namespace,
    ).fact
    val rAx = remember(
      Imply(rimp, Imply("B_()".asFormula, Equal(lhs, ctor("L_()".asTerm, "RR_()".asTerm)))),
      prop & exhaustiveEqL2R(Find.FindLPlain("R_()=RR_()".asFormula)) & cohideR(1) & byUS(Ax.equalReflexive),
      namespace,
    ).fact
    val lrAx = remember(
      Imply(And(limp, rimp), Imply("A_() & B_()".asFormula, Equal(lhs, ctor("LL_()".asTerm, "RR_()".asTerm)))),
      prop & exhaustiveEqL2R(Find.FindLPlain("L_()=LL_()".asFormula)) &
        exhaustiveEqL2R(Find.FindLPlain("R_()=RR_()".asFormula)) & cohideR(1) & byUS(Ax.equalReflexive),
      namespace,
    ).fact
    (lAx, rAx, lrAx)
  }
  ...
  /**
   * Walks a pair and applies the given function everywhere, remembering where it applied that function Used for
   * predicate arguments and formula arguments
   */
  private def pairWalk[A](
      t: Term,
      fn: Term => (Term, A),
      prefix: List[Int] = List[Int](),
  ): (Term, List[(A, List[Int])]) = t match {
    case Pair(a, b) =>
  ...
  private lazy val negAx = remember(
    "(A_() -> (L_() = LL_())) -> A_() -> (-L_() = -LL_())".asFormula,
    prop & exhaustiveEqL2R(Find.FindLPlain("L_()=LL_()".asFormula)) & cohideR(1) & byUS(Ax.equalReflexive),
    namespace,
  ).fact

  private lazy val equalTrans = remember(
    "(P_() -> (F_() = FF_())) & (Q_() -> (FF_() = FFF_())) -> (P_() & Q_() -> (F_() = FFF_())) ".asFormula,
    prop & exhaustiveEqL2R(Find.FindLPlain("F_()=FF_()".asFormula)) &
      exhaustiveEqL2R(Find.FindLPlain("FF_()=FFF_()".asFormula)) & cohideR(1) & byUS(Ax.equalReflexive),
    namespace,
  ).fact

  /**
   * An index is a function from a term/formula and the current formula context (i.e., assumptions) to a list of
   * simplification lemmas to try on that term/formula TODO: think more about the type used to represent the current
   * context
   */
  ...
  def composeIndex[A <: Expression](is: ((A, context) => List[ProvableSig])*)(f: A, ctx: context): List[ProvableSig] =
    is.flatMap(axs => axs(f, ctx)).toList

  /**
   * Dependent term simplification workhorse
   *
   * Given provable `A -> (t = t') or (t = t')`, a context, and a term `x`, checks if x unifies with t. If applicable,
   * checks for the `A[unif]` in the context. Then unifies the conclusion appropriately.
   */
  private def applyTermProvable(t: Term, ctx: context, pr: ProvableSig): Option[(Term, Formula, ProvableSig)] = {
    ...
    pr.conclusion.succ(0) match {
      ...
      case r => throw new IllegalArgumentException(
          "Illegal shape of rewrite: expected equality or conditional equality, but got " + r.prettyString
        )
    }
  }
  ...
  def termSimp(t: Term, ctx: context, taxs: termIndex): (Term, Option[(Formula, ProvableSig)]) = {
    val (rect, recpropt) = t match {
      case bop: BinaryCompositeTerm =>
        ...
        (lpropt, rpropt) match {
          ...
          case (Some((lprem, lpr)), None) =>
            assert(
              rf == bop.right,
              "Missing proof: simplified r=" + bop.right.prettyString + " to " + rf.prettyString + ", but got no proof",
            )
            val fml = Imply(lprem, Equal(t, concl))
            val subst = USubst(List(
              SubstitutionPair(pA, lprem),
              SubstitutionPair(fL, bop.left),
              SubstitutionPair(fLL, lf),
              SubstitutionPair(fR, bop.right),
            ))
          ...
          case (None, Some((rprem, rpr))) =>
            assert(
              lf == bop.left,
              "Missing proof: simplified l=" + bop.left.prettyString + " to " + lf.prettyString + ", but got no proof",
            )
            val fml = Imply(rprem, Equal(t, concl))
            val subst = USubst(List(
              SubstitutionPair(pB, rprem),
              SubstitutionPair(fL, bop.left),
              SubstitutionPair(fR, bop.right),
              SubstitutionPair(fRR, rf),
            ))
          ...
          case (Some((lprem, lpr)), Some((rprem, rpr))) =>
            ...
            val pr = ProvableSig.startPlainProof(fml)(CEat(lem._3(subst), PosInExpr(1 :: Nil))(SuccPos(0)), 0)(
              AndRight(SuccPos(0)),
              0,
            )(rpr, 1)(lpr, 0)
        ...
        upropt match {
          ...
          case Some((uprem, upr)) =>
            ...
            (
              ...
              Some((
                uprem,
                ProvableSig.startPlainProof(fml)(CEat(negAx(subst), PosInExpr(1 :: Nil))(SuccPos(0)), 0)(upr, 0),
              )),
            )
        }
      ...
      case FuncOf(fn, c) if c != Nothing =>
        ...
        val pr = ProvableSig.startPlainProof(Imply(premise, Equal(t, nt)))(ImplyRight(SuccPos(0)), 0)(
          times(andL(Symbol("Llast")), proofs.length - 1),
          0,
        )(cuts, 0)(cohideR(SuccPos(0)), 0)(Ax.equalReflexive.provable(subst), 0)
        (nt, Some(premise, pr))
      ...
      case Some((tt, prem, pr)) =>
        ...
        recpropt match {
          ...
          case Some((prem2, pr2)) =>
            ...
            val subst = USubst(List(
              SubstitutionPair(pP, prem2),
              SubstitutionPair(pQ, prem),
              SubstitutionPair(fF, t),
              SubstitutionPair(fFF, rect),
              SubstitutionPair(fFFF, tt),
            ))
            val prr = (ProvableSig.startPlainProof(fml)(CEat(equalTrans(subst), PosInExpr(1 :: Nil))(SuccPos(0)), 0)(
              AndRight(SuccPos(0)),
              0,
            )(pr, 1)(pr2, 0))
            (tt, Some(premise, prr))
  ...
  private def fmlAx(ctor: (Term, Term) => Formula): (ProvableSig, ProvableSig, ProvableSig) = {
    ...
    val lhs = ctor("L_()".asTerm, "R_()".asTerm)
    val lAx = remember(
      Imply(limp, Imply("A_()".asFormula, Equiv(lhs, ctor("LL_()".asTerm, "R_()".asTerm)))),
      implyR(1) & implyR(1) & implyL(-1) < (id, exhaustiveEqL2R(-1) & cohideR(1) & byUS(Ax.equivReflexive)),
      namespace,
    ).fact
    val rAx = remember(
      Imply(rimp, Imply("B_()".asFormula, Equiv(lhs, ctor("L_()".asTerm, "RR_()".asTerm)))),
      implyR(1) & implyR(1) & implyL(-1) < (id, exhaustiveEqL2R(-1) & cohideR(1) & byUS(Ax.equivReflexive)),
      namespace,
    ).fact
    val lrAx = remember(
      Imply(And(limp, rimp), Imply("A_() & B_()".asFormula, Equiv(lhs, ctor("LL_()".asTerm, "RR_()".asTerm)))),
      implyR(1) & implyR(1) & andL(-1) &
        implyL(-2) <
        (
          andL(-1) & id,
          implyL(-3) < (andL(-1) & id, exhaustiveEqL2R(-2) & exhaustiveEqL2R(-3) & cohideR(1) & byUS(Ax.equivReflexive)),
        ),
      namespace,
    ).fact
    (lAx, rAx, lrAx)
  ...
  private lazy val andAxL =
    remember("(A_() -> (L_() <-> LL_())) -> (A_() -> (L_() & R_() <-> LL_() & R_()))".asFormula, prop & done, namespace)
      .fact
  // only R changes
  private lazy val andAxR = remember(
    "(B_() -> (R_() <-> RR_())) -> ((L_() -> B_()) -> (L_() & R_() <-> L_() & RR_()))".asFormula,
    prop & done,
    namespace,
  ).fact
  // both changed
  private lazy val andAxLR = remember(
    ("(A_() -> (L_() <-> LL_())) & (B_() -> (R_() <-> RR_())) ->" +
      " ( (A_() & (LL_() -> B_())) -> (L_() & R_() <-> LL_() & RR_()) )").asFormula,
    prop & done,
    namespace,
  ).fact
  ...
  // Imply
  // only L changes
  private lazy val impAxL = remember(
    "(A_() -> (L_() <-> LL_())) -> (A_() -> (L_() -> R_() <-> LL_() -> R_()))".asFormula,
    prop & done,
    namespace,
  ).fact
  // only R changes
  private lazy val impAxR = remember(
    "(B_() -> (R_() <-> RR_())) -> ((L_() -> B_()) -> (L_() -> R_() <-> L_() -> RR_()))".asFormula,
    prop & done,
    namespace,
  ).fact
  // both changed
  private lazy val impAxLR = remember(
    ("(A_() -> (L_() <-> LL_())) & (B_() -> (R_() <-> RR_())) ->" +
      " ( (A_() & (LL_() -> B_())) -> (L_() -> R_() <-> LL_() -> RR_()) )").asFormula,
    prop & done,
    namespace,
  ...
  private lazy val orAxL =
    remember("(A_() -> (L_() <-> LL_())) -> (A_() -> (L_() | R_() <-> LL_() | R_()))".asFormula, prop & done, namespace)
      .fact
  // only R changes
  private lazy val orAxR = remember(
    "(B_() -> (R_() <-> RR_())) -> ((!(L_()) -> B_()) -> (L_() | R_() <-> L_() | RR_()))".asFormula,
    prop & done,
    namespace,
  ).fact
  // both changed
  private lazy val orAxLR = remember(
    ("(A_() -> (L_() <-> LL_())) & (B_() -> (R_() <-> RR_())) ->" +
      " ( (A_() & (!LL_() -> B_())) -> (L_() | R_() <-> LL_() | RR_()) )").asFormula,
    prop & done,
    namespace,
  ...
  // Equiv
  // only L changes
  private lazy val equivAxL = remember(
    "(A_() -> (L_() <-> LL_())) -> (A_() -> ( (L_() <-> R_()) <-> (LL_() <-> R_())))".asFormula,
    prop & done,
    namespace,
  ).fact
  // only R changes
  private lazy val equivAxR = remember(
    "(B_() -> (R_() <-> RR_())) -> (B_() -> ( (L_() <-> R_()) <-> (L_() <-> RR_())))".asFormula,
    prop & done,
    namespace,
  ).fact
  // both changed
  private lazy val equivAxLR = remember(
    ("(A_() -> (L_() <-> LL_())) & (B_() -> (R_() <-> RR_())) ->" +
      " ( (A_() & B_()) -> ((L_() <-> R_()) <-> (LL_() <-> RR_())) )").asFormula,
    prop & done,
    namespace,
  ).fact

  private lazy val equivTrans = remember(
    "(P_() -> (F_() <-> FF_())) & (Q_() -> (FF_() <-> FFF_())) -> (P_() & Q_() -> (F_() <-> FFF_())) ".asFormula,
    prop & done,
    namespace,
  ).fact
  ...
  /**
   * Dependent formula simplification workhorse Given provable `A -> (P <-> P') or (P <-> P')`, a context, and a formula
   * `f`, checks if `P` unifies with `f`. If applicable, checks for the `A[unif]` in the context. Then unifies the
   * conclusion appropriately.
   *
   * Note: can avoid unifying again in the proof?
   */
  private def applyFormulaProvable(
      f: Formula,
      ctx: context,
      pr: ProvableSig,
  ): Option[(Formula, Formula, ProvableSig)] = {
    // todo: Add some kind of unification search? (that precludes fast HashSet lookups though)
    pr.conclusion.succ(0) match {
  ...
  private def addCtx(ctx: context, f: Formula, ctr: Int = 0): (Int, context) = {
    if (ctx.contains(f) | f.equals(True) | f.equals(Not(False))) { return (ctr, ctx) }
    f match {
      case And(l, r) =>
  ...
  def formulaSimp(
    ...
    val (recf, recpropt) = f match {
      case And(l, r) =>
        ...
        (lpropt, rpropt) match {
          ...
          case (Some((lprem, lpr)), None) =>
            assert(
              rf == r,
              "Missing proof: simplified r=" + r.prettyString + " to " + rf.prettyString + ", but got no proof",
            )
            val fml = Imply(lprem, Equiv(f, concl))
            val subst = USubst(List(
              SubstitutionPair(pA, lprem),
              SubstitutionPair(pL, l),
              SubstitutionPair(pLL, lf),
              SubstitutionPair(pR, r),
            ))
          ...
          case (None, Some((rprem, rpr))) =>
            assert(
              lf == l,
              "Missing proof: simplified l=" + l.prettyString + " to " + lf.prettyString + ", but got no proof",
            )
          ...
          case (Some((lprem, lpr)), Some((rprem, rpr))) =>
            ...
            val pr = ProvableSig.startPlainProof(fml)(CEat(andAxLR(subst), PosInExpr(1 :: Nil))(SuccPos(0)), 0)(
              AndRight(SuccPos(0)),
              0,
            )(rpr, 1)(lpr, 0)
            (concl, Some((premise, pr)))
      ...
      case Imply(l, r) =>
        ...
        (lpropt, rpropt) match {
          ...
          case (Some((lprem, lpr)), Some((rprem, rpr))) =>
            ...
            val pr = ProvableSig.startPlainProof(fml)(CEat(impAxLR(subst), PosInExpr(1 :: Nil))(SuccPos(0)), 0)(
              AndRight(SuccPos(0)),
              0,
            )(rpr, 1)(lpr, 0)
            (concl, Some((premise, pr)))
      ...
      case Or(l, r) =>
        ...
        (lpropt, rpropt) match {
          ...
          case (Some((lprem, lpr)), Some((rprem, rpr))) =>
            ...
            val pr = ProvableSig.startPlainProof(fml)(CEat(orAxLR(subst), PosInExpr(1 :: Nil))(SuccPos(0)), 0)(
              AndRight(SuccPos(0)),
              0,
            )(rpr, 1)(lpr, 0)
            (concl, Some((premise, pr)))
      ...
      case Equiv(l, r) =>
        ...
        (lpropt, rpropt) match {
          ...
          case (Some((lprem, lpr)), Some((rprem, rpr))) =>
            ...
            val pr = ProvableSig.startPlainProof(fml)(CEat(equivAxLR(subst), PosInExpr(1 :: Nil))(SuccPos(0)), 0)(
              AndRight(SuccPos(0)),
              0,
            )(rpr, 1)(lpr, 0)
            (concl, Some((premise, pr)))
      ...
      case cf: ComparisonFormula =>
        ...
        (lpropt, rpropt) match {
          ...
          case (Some((lprem, lpr)), None) =>
            assert(
              rt == r,
              "Missing proof: simplified r=" + r.prettyString + " to " + rt.prettyString + ", but got no proof",
            )
            val fml = Imply(lprem, Equiv(f, concl))
            val subst = USubst(List(
              SubstitutionPair(pA, lprem),
              SubstitutionPair(fL, l),
              SubstitutionPair(fLL, lt),
              SubstitutionPair(fR, r),
            ))
            val pr = ProvableSig.startPlainProof(fml)(CEat(lem._1(subst), PosInExpr(1 :: Nil))(SuccPos(0)), 0)(lpr, 0)
            (concl, Some((lprem, pr)))
          case (None, Some((rprem, rpr))) =>
            assert(
              lt == l,
              "Missing proof: simplified l=" + l.prettyString + " to " + lt.prettyString + ", but got no proof",
            )
            val fml = Imply(rprem, Equiv(f, concl))
            val subst = USubst(List(
              SubstitutionPair(pB, rprem),
      ...
      case q: Quantified =>
        ...
        upropt match {
          ...
          case Some((uprem, upr)) =>
            val premise =
              Forall(q.vars, uprem) // Implicitly, we are doing a forall quantification over the smaller simps
            val fml = Imply(premise, Equiv(f, nf))

            val seq = q match {
              case _: Forall => (
                (_: ProvableSig)(FOQuantifierTactics.allSkolemize(SuccPos(0)), 0)(
                  FOQuantifierTactics.allInstantiate(None, None)(AntePos(0)),
                  0,
                )(FOQuantifierTactics.allInstantiate(None, None)(AntePos(1)), 0)
              )
              case _: Exists => (
                (_: ProvableSig)(FOQuantifierTactics.existsSkolemize(AntePos(1)), 0)(
                  FOQuantifierTactics.allInstantiate(None, None)(AntePos(0)),
                  0,
            ...
            val pr = (ProvableSig.startPlainProof(fml)(ImplyRight(SuccPos(0)), 0)(EquivRight(SuccPos(0)), 0)
            // right branch
            (seq, 1)(implyRi()(AntePos(1), SuccPos(0)), 1)(EquivifyRight(SuccPos(0)), 1)(
              CommuteEquivRight(SuccPos(0)),
              1,
            )(implyRi, 1)(upr, 1)
            // left branch
            (seq, 0)(implyRi()(AntePos(1), SuccPos(0)), 0)(EquivifyRight(SuccPos(0)), 0)(implyRi, 0)(upr, 0))
      ...
      case m: Modal =>
        ...
        upropt match {
          ...
          case Some((uprem, upr)) =>
            ...
            // |- p <-> q
            val upr2 = (ProvableSig.startPlainProof(concl)(CEat(upr, PosInExpr(1 :: Nil))(SuccPos(0)), 0)(
              fastCloser(HashSet.empty, uprem),
              0,
            ))
      ...
      case PredOf(fn, c) if c != Nothing =>
        ...
        val cuts = proofs
          ...
          .map({
            case ((None, _), _) => ident
            case ((Some(prf), _), i) => CEat(prf._2)(AntePos(i)) andThen
                eqL2R(AntePosition.base0(i))(SuccPosition.base0(0))
          })
          .foldLeft(_: ProvableSig)({ case (p, t) => p(t, 0) })
        val subst = USubst(List(SubstitutionPair(PredOf(Function("p_", None, Unit, Bool), Nothing), nf)))
        val pr = ProvableSig.startPlainProof(Imply(premise, Equiv(f, nf)))(ImplyRight(SuccPos(0)), 0)(
          times(andL(Symbol("Llast")), proofs.length - 1),
          0,
        )(cuts, 0)(cohideR(1), 0)(Ax.equivReflexive.provable(subst), 0)
        (nf, Some(premise, pr))
      // Differentials
      case _ => (f, None)
    }
    ...
    rw match {
      ...
      case Some((ff, prem, pr)) => recpropt match {
          ...
          case Some((prem2, pr2)) =>
            ...
            val subst = USubst(List(
              SubstitutionPair(pP, prem2),
              SubstitutionPair(pQ, prem),
              SubstitutionPair(pF, f),
              SubstitutionPair(pFF, recf),
              SubstitutionPair(pFFF, ff),
            ))
            val prr = (ProvableSig.startPlainProof(fml)(CEat(equivTrans(subst), PosInExpr(1 :: Nil))(SuccPos(0)), 0)(
              AndRight(SuccPos(0)),
              0,
            )(pr, 1)(pr2, 0))
            (ff, Some(premise, prr))
  ...
  private def fastCloser(hs: context, f: Formula): ProvableSig => ProvableSig = (pr: ProvableSig) => {
    ...
    if (f == True) closeT.result(pr)
      ...
      f match {
        case And(l, r) => pr(AndRight(SuccPos(0)), 0)(fastCloser(hs, r), 1)(fastCloser(hs, l), 0)
        case Imply(l, r) =>
          val (_, newctx) = addCtx(hs, l)
          pr(ImplyRight(SuccPos(0)), 0)(andSplit, 0)(fastCloser(newctx, r), 0)
        case Forall(_, f) => FOQuantifierTactics.allSkolemize(SuccPos(0)).computeResult(pr)(fastCloser(hs, f), 0)
        case _ => pr
      }
    }
  }
  ...
  private def termSimpWithDischarge(ctx: IndexedSeq[Formula], t: Term, taxs: termIndex): (Term, Option[ProvableSig]) = {
    ...
    (
      ...
      recpropt match {
        case None => None
        case Some((prem, recpr)) => Some(
            ProvableSig.startPlainProof(Sequent(ctx, IndexedSeq(Equal(t, recf))))(Cut(prem), 0)
            // show
            (HideRight(SuccPos(0)), 1)(fastCloser(hs, prem), 1)
            // use
            (CoHide2(AntePos(ctx.length), SuccPos(0)), 0)(implyRi, 0)(recpr, 0)
          )
      },
    )
  }
  ...
  def simpWithDischarge(
    ...
    (
      ...
      recpropt match {
        case None => None
        case Some((prem, recpr)) => Some(
            ProvableSig.startPlainProof(Sequent(ctx, IndexedSeq(Equiv(f, recf))))(Cut(prem), 0)
            // show
            (HideRight(SuccPos(0)), 1)(fastCloser(hs, prem), 1)
            // use
            (CoHide2(AntePos(ctx.length), SuccPos(0)), 0)(implyRi, 0)(recpr, 0)
  ...
  def simpTac(
      ths: List[ProvableSig] = List.empty,
      faxs: formulaIndex = defaultFaxs,
      taxs: termIndex = defaultTaxs,
  ): BuiltInPositionTactic = anon { (provable: ProvableSig, pos: Position) =>
    ...
    sequent.sub(pos) match {
      case Some(f: Formula) =>
        ...
        if (pos.isTopLevel) {
          ...
          simpWithDischarge(ctx, f, augmentFaxs, augmentTaxs) match {
            ...
            case (ff, Some(pr)) => (
              provable(cutAtFw(ff)(pos), 0)(cohideOnlyR(cutPos), 1)(equivifyR(SuccPos(0)), 1)(commute, 1)(pr, 1)
            )
          }
        } else {
      ...
      case Some(t: Term) => termSimpWithDischarge(IndexedSeq(), t, augmentTaxs) match {
          case (_, None) => provable
          case (_, Some(pr)) =>
            try { provable(CEat(useFor(Ax.equalCommute)(SuccPos(0))(pr))(pos), 0) }
            catch {
              case _: InfiniteTacticLoopError => throw new UnsupportedTacticFeature(
                  "Unable to simplify term " + t + " because some of its variables appear in a maybe-bound context"
                )
            }
        }
      case _ => provable
  ...
  def fullSimpTac(
    ...
    val succ =
      if (simpSuccs) List.range(1, seq.succ.length + 1, 1).foldRight(antes)((i, pr) => pr(simps(i), 0)) else antes
...
//  }
  ...
  private lazy val mulArith: List[ProvableSig] = Try(Ax.zeroTimes.provable).toOption.toList ++
    Try(Ax.timesZero.provable).toOption.toList ++ Try(Ax.timesIdentity.provable).toOption.toList ++
    Try(useFor(Ax.timesCommute, PosInExpr(0 :: Nil))(SuccPosition(1, 0 :: Nil))(Ax.timesIdentity.provable))
      .toOption
      .toList ++ Try(Ax.timesIdentityNeg.provable).toOption.toList ++
    Try(useFor(Ax.timesCommute, PosInExpr(0 :: Nil))(SuccPosition(1, 0 :: Nil))(Ax.timesIdentityNeg.provable))
      .toOption
  ...
  lazy val divArith: List[ProvableSig] = Try(Ax.zeroDivNez.provable).toOption.toList ++
  ...
  def arithGroundIndex(t: Term, ctx: context = emptyCtx): List[ProvableSig] = {
    ...
    res match {
      ...
      case Some(v) if v.isExactDouble && v.isValidInt =>
        ...
        if (pr.isProved) List(pr) else List()
      case _ => List()
    }
  }
  ...
  private lazy val eqSymmetricImp = remember(
    "F_() = G_() -> G_() = F_()".asFormula,
    prop & exhaustiveEqL2R(-1) & hideL(-1) & byUS(Ax.equalReflexive),
    namespace,
  ).fact
    ...
    ctx
      .collectFirst({
        ...
        case Equal(n: Number, tt) if !tt.isInstanceOf[Number] && tt == t =>
          eqSymmetricImp(USubst(
            SubstitutionPair(FuncOf(Function("F_", None, Unit, Real), Nothing), n) ::
    ...
    ctx
      .collectFirst({
        ...
        case Equal(ttt, tt) if tt.equals(t) =>
          eqSymmetricImp(USubst(
            SubstitutionPair(FuncOf(Function("F_", None, Unit, Real), Nothing), ttt) ::
              SubstitutionPair(FuncOf(Function("G_", None, Unit, Real), Nothing), t) :: Nil
          ))
      })
      .toList
  ...
  private def propProof(f: String, ff: String, precond: Option[String] = None): ProvableSig = {
    precond match {
      case Some(pre) => remember(Imply(pre.asFormula, Equiv(f.asFormula, ff.asFormula)), prop & done, namespace).fact
      case None => remember(Equiv(f.asFormula, ff.asFormula), prop & done, namespace).fact
    }
  }
  ...
  private lazy val eqs = Ax.equalSym.provable ::
    qeSearch(Equal.apply, List(NotEqual.apply, Greater.apply, GreaterEqual.apply, Less.apply, LessEqual.apply))
  private lazy val neqs = Ax.notEqualSym.provable ::
    qeSearch(NotEqual.apply, List(Equal.apply, Greater.apply, GreaterEqual.apply, Less.apply, LessEqual.apply))
  private lazy val gts = Ax.greaterNotSym.provable ::
    qeSearch(Greater.apply, List(Equal.apply, NotEqual.apply, GreaterEqual.apply, Less.apply, LessEqual.apply))
  private lazy val ges =
    qeSearch(GreaterEqual.apply, List(Equal.apply, NotEqual.apply, Greater.apply, Less.apply, LessEqual.apply))
  private lazy val lts = Ax.lessNotSym.provable ::
    qeSearch(Less.apply, List(Equal.apply, NotEqual.apply, Greater.apply, GreaterEqual.apply, LessEqual.apply))
  private lazy val les =
    qeSearch(LessEqual.apply, List(Equal.apply, NotEqual.apply, Greater.apply, GreaterEqual.apply, Less.apply))
  ...
  def cmpIndex(f: Formula, ctx: context): List[ProvableSig] = {
    ...
    f match {
      ...
      // Closing by search
      case bop: ComparisonFormula => bop match {
          case Less(_, _) => lts
  ...
  private def to_NNF(f: Formula): Option[(Formula, ProvableSig)] = {
    ...
    if (nnff != f) {
      val prv =
        chaseNeg(Position(1, 1 :: Nil))(Ax.equivReflexive.provable(USubst(Seq(SubstitutionPair("p_()".asFormula, f)))))
  ...
  private def atomicTermIndex(t: Term, ctx: context): List[ProvableSig] = {
    t match {
      ...
      case Power(_, _) => List()
      case _ => throw new IllegalArgumentException("cannot normalize term:" + t + " rejecting immediately")
    }
  }

  /**
   * Normalize a formula By default, this normalizer builds in:
   *   1. NNF conversion
   *   1. Term checking: no min, max, abs, functions (except consts) -- if checkTerms = true
   */
  ...
  private lazy val neqNorm: ProvableSig =
    remember(" f_() != g_() <-> f_() - g_() > 0 | g_() - f_() > 0 ".asFormula, QE, namespace).fact
  ...
  private lazy val minGeqNorm: ProvableSig = remember("f_()>=0&g_()>=0<->min((f_(),g_()))>=0".asFormula, QE, namespace)
    .fact
  private lazy val maxGeqNorm: ProvableSig = remember("f_()>=0|g_()>=0<->max((f_(),g_()))>=0".asFormula, QE, namespace)
    .fact
  ...
  private lazy val andEqNorm: ProvableSig = remember("f_()=0 & g_()=0 <-> f_()^2 + g_()^2=0".asFormula, QE, namespace)
    .fact
  ...
  private def algNormalizeIndex(f: Formula, ctx: context): List[ProvableSig] = {
    f match {
      ...
      case _ => throw new IllegalArgumentException(
          "cannot normalize " + f + " to have 0 on RHS (must be a conjunction/disjunction of atomic equations)"
        )
    }
  }
  ...
  private def semiAlgNormalizeIndex(f: Formula, ctx: context): List[ProvableSig] = {
    f match {
      ...
      case False => List(falseEqNorm)
      case _ => throw new IllegalArgumentException(
          "cannot normalize " + f + " to have 0 on RHS (must be a conjunction/disjunction of atomic comparisons)"
        )
    }
  }
  ...
  private def disAlgNormalizeIndex(f: Formula, ctx: context): List[ProvableSig] = {
    f match {
      ...
      case Or(l, r) => Nil
      case _ => throw new IllegalArgumentException(
          "cannot normalize " + f + " to have 0 on RHS (must be a conjunction/disjunction of =s or !=s comparisons)"
        )
    }
  }
  ...
  private def ineqNormalizeIndex(f: Formula, ctx: context): List[ProvableSig] = {
    f match {
      ...
      case Greater(l, r) => ()
      case _ =>
        throw new IllegalArgumentException("cannot normalize " + f + " to have 0 on RHS (must be inequality >=,>,<=,<)")
    }
  ...
  private def atomNormalizeIndex(f: Formula, ctx: context): List[ProvableSig] = {
    f match {
      ...
      case NotEqual(l, r) => ()
      case _ => throw new IllegalArgumentException(
          "cannot normalize " + f + " to have 0 on RHS (must be atomic comparison formula >=,>,<=,<,=,!=)"
        )
    }
    semiAlgNormalizeIndex(f, ctx)
  }
  ...
  private def maxMinGeqNormalizeIndex(f: Formula, ctx: context): List[ProvableSig] = {
    f match {
      ...
      case _ => throw new IllegalArgumentException(
          "cannot normalize " + f + " to max/min >=0 normal form (must be a conjunction/disjunction of >= 0s or =0s)"
        )
    }
  }
  ...
  private def maxMinGtNormalizeIndex(f: Formula, ctx: context): List[ProvableSig] = {
    f match {
      ...
      case _ => throw new IllegalArgumentException(
          "cannot normalize " + f + " to max/min >0 normal form (must be a conjunction/disjunction of > 0s)"
        )
    }
  }
  ...
  val semiAlgNormalizeUnchecked: Formula => (Formula, Option[ProvableSig]) =
    doNormalize(semiAlgNormalizeIndex, checkTerms = false)(_)
  val maxMinGeqNormalize: Formula => (Formula, Option[ProvableSig]) = doNormalize(maxMinGeqNormalizeIndex)(_)
  val maxMinGeqNormalizeUnchecked: Formula => (Formula, Option[ProvableSig]) =
    doNormalize(maxMinGeqNormalizeIndex, checkTerms = false)(_)
  val maxMinGtNormalize: Formula => (Formula, Option[ProvableSig]) = doNormalize(maxMinGtNormalizeIndex)(_)
  val maxMinGtNormalizeUnchecked: Formula => (Formula, Option[ProvableSig]) =
    doNormalize(maxMinGtNormalizeIndex, checkTerms = false)(_)
}
