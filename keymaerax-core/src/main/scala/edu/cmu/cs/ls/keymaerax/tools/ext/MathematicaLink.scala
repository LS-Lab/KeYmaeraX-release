...
abstract class BaseKeYmaeraMathematicaBridge[T](
    ...
    val k2m: K2MConverter[T],
    val m2k: M2KConverter[T],
) extends KeYmaeraMathematicaBridge[T] {
  ...
  private[tools] def run[S](cmd: MExpr, localm2k: M2KConverter[S]): (String, S) = {
    val commandRunner = link match { case j: JLinkMathematicaLink => JLinkMathematicaCommandRunner(j.ml) }
    (
      cmd.toString,
      link.run(
        () => {
          try { commandRunner.run(memoryConstrained(timeConstrained(cmd)), localm2k)._2 }
          catch {
  ...
  protected def timeConstrained(cmd: MExpr): MExpr =
    if (timeout < 0) cmd else MathematicaOpSpec.timeConstrained(cmd, MathematicaOpSpec.int(timeout))

  protected def timeConstrained(cmd: String): String =
    if (timeout < 0) cmd else s"TimeConstrained[$cmd, $timeout, ${MathematicaOpSpec.timedOut.op}]"
  ...
  protected def memoryConstrained(cmd: String): String =
    if (memoryLimit < 0) cmd else s"MemoryConstrained[$cmd, ${memoryLimit * 1000000}]"
}
...
class JLinkMathematicaLink(val engineName: String) extends MathematicaLink with Logging {
  ...
  private def startKernel(linkName: String, port: String): Option[Process] = {
    val cmd = linkName :: "-mathlink" :: "-linkmode" :: "listen" :: "-linkname" :: port :: "-linkprotocol" :: "tcpip" ::
      Nil
  ...
  def init(linkName: String, jlinkLibDir: Option[String], tcpip: String, remainingTrials: Int = 5): Boolean = {
    ...
    if (jlinkLibDir.isDefined) {
      System.setProperty(
        "com.wolfram.jlink.libdir",
        jlinkLibDir.get,
      ) // e.g., "/usr/local/Wolfram/Mathematica/9.0/SystemFiles/Links/JLink"
    }
    try {
      ml =
                ...
                case Some(process: Process) =>
                  ...
                  if (process.isAlive()) mathProcess = Some(process)
                    ...
                    throw new IllegalStateException(
                      engineName + " terminated with exit code " + process.exitValue() +
                        "; check that your license is valid and your computer is online for license checking"
                    )
                  }
      ...
      case e: UnsatisfiedLinkError =>
        ...
        logger.error(
          s"""Shutting down since $engineName J/Link native library was not found in:
             |$jlinkLibDir
             |Or this path did not contain the native library compatible with ${Os.Name} ${Os.Version}
             |$diagnostic
      ...
      case Some(Right(throwable)) => throwable match {
          ...
          case ex: ToolCriticalException =>
            ...
            try { restart() }
              ...
              case ex: Throwable => throw ToolExecutionException(
                  "Restarting " + engineName +
                    " failed. Please restart KeYmaera X. If the problem persists, try Z3 instead of " + engineName +
                    " (KeYmaera X->Preferences). " + engineName + " error that triggered the restart:\n" +
                    ex.getMessage,
                  ex,
                )
            }
            throw ToolCommunicationException(
              "Restarted " + engineName + ", please rerun the failed command (error details below)",
              ex,
            )
        }
      case None =>
  ...
  private def isActivated(version: Version): Option[(Boolean, LocalDate)] = {
    ...
    try {
      ...
      def licenseExpiredConverter(licenseExpirationDate: MExpr): Option[(Boolean, LocalDate)] =
        try {
          version match {
...
class WolframScript extends MathematicaLink with Logging {
      ...
      case Some(Right(throwable)) => throwable match {
          ...
          case ex: ToolExternalException =>
            ...
            try { restart() }
              ...
              case restartEx: Throwable => throw ToolExecutionException(
                  "Restarting Wolfram Engine failed. Please restart KeYmaera X. If the problem persists, try Z3 instead of Wolfram Engine (Help->Tools). Wolfram Engine error that triggered the restart:\n" +
                    ex.getMessage,
                  restartEx,
                )
            }
            throw ToolCommunicationException(
              "Restarted Wolfram Engine, please rerun the failed command (error details below)",
              throwable,
            )
        }
      case None =>
  ...
  private def isComputing: Option[Boolean] = {
    ...
    } catch { case e: Throwable => throw ToolExecutionException("Error running test computation", e) }
  }
  ...
  def evaluate(cmd: String): MExpr = {
    ...
    if (exitVal == 0) {
      ...
      if (rs.contains("malloc")) evaluate(cmd) else parseExpressionJSON(rs)
    } else { throw ToolExecutionException(s"Error executing Wolfram Engine, exit value $exitVal") }
  }
  ...
  private def parseExpressionJSON(expr: String): MExpr = {
    def convertJSON(v: JsValue): MExpr = v match {
      ...
      case JsNumber(n) if n.isWhole =>
        new MExpr(
          n.toBigIntExact
            .getOrElse(throw ConversionException("Unexpected: whole BigDecimal cannot be converted to BigInteger"))
            .bigInteger
        )
    ...
    val json =
      try { JsonParser(expr) }
        ...
        case ex: JsonParser.ParsingException => throw ConversionException(
            "Error parsing Wolfram Engine response; checking license may have failed, please double-check Wolfram Engine is activated and retry",
            ex,
          )
      }
    convertJSON(json)
