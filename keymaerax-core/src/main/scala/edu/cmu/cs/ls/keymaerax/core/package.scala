 ...
 * [[edu.cmu.cs.ls.keymaerax.btactics proof tactics]] that KeYmaera X provides give you a more powerful and flexible and
 * easier way of constructing and searching for proofs, but they internally reduce to what is shown here.
 *
 * ===Constructing Proofs===
 ...
 * begins a new proof of a [[edu.cmu.cs.ls.keymaerax.core.Sequent]] containing the conjectured differential dynamic
 * logic formula. A proof rule of type [[edu.cmu.cs.ls.keymaerax.core.Rule]] can be applied to any subgoal of a
 * [[edu.cmu.cs.ls.keymaerax.core.Provable]] by
 * [[edu.cmu.cs.ls.keymaerax.core.Provable.apply(rule:edu\.cmu\.cs\.ls\.keymaerax\.core\.Rule,subgoal:edu\.cmu\.cs\.ls\.keymaerax\.core\.Provable\.Subgoal):edu\.cmu\.cs\.ls\.keymaerax\.core\.Provable*]]
 * to advance the proof until that Provable has been proved since [[edu.cmu.cs.ls.keymaerax.core.Provable.isProved]] is
 * true. Subgoals are identified by integers.
 * {{{
 ...
 * }}}
 * Of course, [[edu.cmu.cs.ls.keymaerax.btactics]] make it much easier to describe proof search procedures compared to
 * the above explicit proof construction. The tactics internally construct proofs this way, but add additional
 * flexibility and provide convenient ways of expressing proof search strategies in a tactic language.
 *
 * ===Combining Proofs===
 * Multiple Provable objects for subderivations obtained from different sources can also be merged into a single
 * Provable object by substitution with
 * [[edu.cmu.cs.ls.keymaerax.core.Provable.apply(edu.cmu.cs.ls.keymaerax.core.Provable,Int)]]. The above example can be
 * continued to merge proofs as follows:
 * {{{
 ...
 * [[edu.cmu.cs.ls.keymaerax.core.Expression syntax]] and
 * [[edu.cmu.cs.ls.keymaerax.core.StaticSemantics static semantics]].
 *
 * ===Syntax===
 * The immutable algebraic data structures for the expressions of differential dynamic logic are of type
 * [[edu.cmu.cs.ls.keymaerax.core.Expression]]. Expressions are categorized according to their kind by the syntactic
 * categories of the grammar of differential dynamic logic:
 *
 *   1. [[edu.cmu.cs.ls.keymaerax.core.Term terms]] are of type [[edu.cmu.cs.ls.keymaerax.core.Term]] of kind
 *      [[edu.cmu.cs.ls.keymaerax.core.TermKind]]
 *   1. [[edu.cmu.cs.ls.keymaerax.core.Formula formulas]] are of type [[edu.cmu.cs.ls.keymaerax.core.Formula]] of kind
 *      [[edu.cmu.cs.ls.keymaerax.core.FormulaKind]]
 *   1. [[edu.cmu.cs.ls.keymaerax.core.Program hybrid programs]] are of type [[edu.cmu.cs.ls.keymaerax.core.Program]] of
 *      kind [[edu.cmu.cs.ls.keymaerax.core.ProgramKind]]
 *   1. [[edu.cmu.cs.ls.keymaerax.core.DifferentialProgram differential programs]] are of type
 *      [[edu.cmu.cs.ls.keymaerax.core.DifferentialProgram]] of kind
 *      [[edu.cmu.cs.ls.keymaerax.core.DifferentialProgramKind]]
 *
 * See [[https://doi.org/10.1007/s10817-016-9385-1 Section 2.1]]
 *
 * ===Static Semantics===
 * The static semantics of differential dynamic logic is captured in [[edu.cmu.cs.ls.keymaerax.core.StaticSemantics]] in
 * terms of the
 * [[edu.cmu.cs.ls.keymaerax.core.StaticSemantics.freeVars(edu.cmu.cs.ls.keymaerax.core.Expression) free variables]] and
 * [[edu.cmu.cs.ls.keymaerax.core.StaticSemantics.boundVars(edu.cmu.cs.ls.keymaerax.core.Expression) bound variables]]
 * that expressions have as well as their
 * [[edu.cmu.cs.ls.keymaerax.core.StaticSemantics.signature(edu.cmu.cs.ls.keymaerax.core.Expression) signatures]] (set
 * of occurring symbols). See [[https://doi.org/10.1007/s10817-016-9385-1 Section 2.4]]
 *
 * ==Theorem Prover==
 * The KeYmaera X Prover Kernel provides [[edu.cmu.cs.ls.keymaerax.core.USubst uniform substitutions]],
 * [[edu.cmu.cs.ls.keymaerax.core.UniformRenaming uniform]] and
 * [[edu.cmu.cs.ls.keymaerax.core.BoundRenaming bound variable renaming]], and
 * [[edu.cmu.cs.ls.keymaerax.core.Provable.axioms axioms]] of differential dynamic logic. For efficiency, it also
 * directly provides propositional sequent proof rules and [[edu.cmu.cs.ls.keymaerax.core.Skolemize Skolemization]].
 *
 * ===Axioms===
 ...
 * All available axioms are listed in [[edu.cmu.cs.ls.keymaerax.core.Provable.axioms]], all available axiomatic rules
 * are listed in [[edu.cmu.cs.ls.keymaerax.core.Provable.rules]] which both ultimately come from the file
 * [[edu.cmu.cs.ls.keymaerax.core.AxiomBase]]. See [[https://doi.org/10.1007/s10817-016-9385-1 Sections 4 and 5.0]]
 * Additional axioms are available as derived axioms and lemmas in [[edu.cmu.cs.ls.keymaerax.btactics.Ax]].
 *
 * ===Uniform Substitutions===
 * [[edu.cmu.cs.ls.keymaerax.core.USubst Uniform substitutions]] uniformly replace all occurrences of a given predicate
 * p(.) by a formula in (.) and likewise for function symbols f(.) and program constants. Uniform substitutions and
 * their application mechanism for differential dynamic logic are implemented in
 * [[edu.cmu.cs.ls.keymaerax.core.USubst]]. See [[https://doi.org/10.1007/s10817-016-9385-1 Section 3]] and
 ...
 * as well as efficient propositional sequent proof rules and Skolemization [[edu.cmu.cs.ls.keymaerax.core.Skolemize]]
 * are all of type [[edu.cmu.cs.ls.keymaerax.core.Rule]], which are the only proof rules that can ever be applied to a
 * proof. See [[https://doi.org/10.1007/s10817-008-9103-8 sequent calculus]]
 *
 ...
 * arithmetic decision procedures are defined in [[edu.cmu.cs.ls.keymaerax.core.Provable.fromStorageString()]] and
 * [[edu.cmu.cs.ls.keymaerax.core.QETool]].
 *
 * ===Error Reporting===
 * Errors from the prover core are reported as exceptions of type [[edu.cmu.cs.ls.keymaerax.core.ProverException]] whose
 * main responsibility is to propagate problems in traceable ways to the user by augmenting them with contextual
 * information. The prover core throws exceptions of type [[edu.cmu.cs.ls.keymaerax.core.CoreException]].
 *
 * ===Set Lattice===
