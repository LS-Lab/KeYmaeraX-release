...
object Transitivity extends TacticProvider {
  ...
  val closeTransitive: DependentTactic = anon((s: Sequent) => {
    ...
    val transitiveInequalities = search(s) match {
      ...
      case None => throw new TacticInapplicableFailure(
          s"Could not find a set of transitive inequalities that imply the postcondition of ${s}"
        )
    }
  ...
  def instantiations(formulas: List[Formula]): List[(Variable, Term)] = {
    ...
    ("TRANS0".asVariable, inequalities.head._2) ::
      inequalities.map(_._3).zipWithIndex.map(x => (s"TRANS${x._2 + 1}".asVariable, x._1))
  }

  def transitivityLemma(formulas: List[Formula]) = {
    assert(formulas.length >= 2)
    val inequalities = formulas
      .zipWithIndex
      .map(fi => {
        val (f, i) = fi
  ...
  def searchResultToPositionList(s: Sequent, fs: List[Formula]) = fs
    .map(f => s.ante.zipWithIndex.find(_._1 == f).get._2)
    .map(x => -x - 1)
  ...
  def search(s: Sequent): Option[List[Formula]] = {
    ...
    val (direction, start, end) = decomposeInequality(s.succ.head) match {
      case Some(x) => x
      case None => throw new TacticInapplicableFailure(
          s"[closeTransitive] Expected succedent of form a ~ b but found ${s.succ(0).prettyString}"
        )
    }
  ...
  private def initialSearchSet(s: Sequent, direction: TransitivityDirection, start: Term) = s
    .ante
    .filter(f => {
  ...
  /**
   * Used to indicate the "direction" of an inequality `a ~ b`. E.g., by mapping `(a >= b)` and `(a > b)` to
   * `(GreaterDir, a, b)`
   */
