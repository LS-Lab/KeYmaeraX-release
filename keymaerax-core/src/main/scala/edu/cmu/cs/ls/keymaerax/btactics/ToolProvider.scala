...
class PreferredToolProvider[T <: Tool](val toolPreferences: List[T]) extends ToolProvider {
  ...
  /**
   * Ensures that the tool `t` is initialized (= not yet shutdown) before returning it; returns None for uninitialized
   * tools.
   */
...
case class MultiToolProvider(providers: List[ToolProvider])
    extends PreferredToolProvider(providers.flatMap(_.tools())) {
