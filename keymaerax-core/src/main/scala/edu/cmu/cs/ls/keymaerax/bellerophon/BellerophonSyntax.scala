 ...
 * are of type [[edu.cmu.cs.ls.keymaerax.bellerophon.BelleExpr]], which provides the following tactic combinators
 *
 *   - `s ; t` alias `s & t` [[edu.cmu.cs.ls.keymaerax.bellerophon.SeqTactic sequential composition]] executes t` on the
 *     output of `s`, failing if either fail.
 ...
 *   Andre Platzer
 * @see
 *   [[edu.cmu.cs.ls.keymaerax.bellerophon.Interpreter]]
 * @see
