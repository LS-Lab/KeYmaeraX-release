...
case class Rational(n: BigInt, d: BigInt) {
  ...
  def reciprocal: Rational = if (signum < 0) Rational(-d, -n) else Rational(d, n)
  ...
  def +(r: Rational): Rational = {
    ...
    if (dgcd == 1) { Rational(r.d * n + r.n * d, r.d * d) }
    else {
      val lden: BigInt = d / dgcd
      val rden: BigInt = r.d / dgcd
      val num: BigInt = rden * n + r.n * lden
      val ngcd: BigInt = num.gcd(dgcd)
      if (ngcd == 1) Rational(num, lden * r.d) else Rational(num / ngcd, (r.d / ngcd) * lden)
    }
  }
  ...
  def floor: Rational =
    if (isWhole) this else if (n.signum >= 0) Rational(n / d, BigInt(1)) else Rational(n / d - 1, BigInt(1))

  def ceil: Rational =
    if (isWhole) this else if (n.signum >= 0) Rational(n / d + 1, BigInt(1)) else Rational(n / d, BigInt(1))
  ...
  def pow(exp: Int): Rational =
    if (exp == 0) Rational(1, 1) else if (exp < 0) Rational(d pow -exp, n pow -exp) else Rational(n pow exp, d pow exp)

  def compareToOne: Int = n compare d

  def compare(r: Rational): Int = {
    val dgcd = d.gcd(r.d)
    if (dgcd == 1) (n * r.d) compare (r.n * d) else ((r.d / dgcd) * n) compare ((d / dgcd) * r.n)
  }
