...
object ODEInvariance {
  ...
  /** Given a polynomial, generates the formula p*>=0
    * along with a tactic that proves <x'=f(x)&p>=0>O
    * TODO - implement
    * List of optimizations
    * TODO - Ignore "DRI" case when the basis simplifies to "1"
    * TODO - Reduce rank by adding |f(x)|!=0 assumption
    *
    * @param ode   : the ODEs under consideration
    * @param p     : the polynomial p to generate p*>0
    */
  private def pStarGeq(ode: ODESystem,p:Term) : Formula = {
    True
  }
  ...
  */
        ...
        dC(gtz)(pos) < (
          ...
          boxAnd(pos) & andR(pos) < (
            ...
            diffInd('diffInd)(pos) <(
              ...
              DebuggingTactics.debug("First Vdbx QE") &
              ...
              cohideOnlyR('Rlast) & SaturateTactic(Dassignb(1)) &
                ...
                cut(Equal(lie,Times(two,dot_prod(matvec_prod(Gco,ps),ps))))<(
                  ...
                  DebuggingTactics.debug("Second Vdbx QE") &
                    hideR(1) & QE
                )
              )
            )
  ...
  def dRI : DependentPositionTactic = "dRI" by ((pos:Position,seq:Sequent) => {
    ...
    diffUnpackEvolutionDomainInitially(pos) &
      DebuggingTactics.debug("dgVdbx") &
      dgVdbx(cofactors,groebner)(pos) &
      DebuggingTactics.debug("Final QE") &
