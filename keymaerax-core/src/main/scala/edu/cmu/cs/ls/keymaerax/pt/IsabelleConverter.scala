...
object IDMap extends Logging {
  ...
  def ofFuncl(name: String, repl: Expression, acc: IDMap): IDMap = { ofExp(repl, acc).addFuncl(name) }
  ...
  def ofUnitPred(name: String, repl: Expression, acc: IDMap): IDMap = { ofExp(repl, acc).addUnitPred(name) }

  def ofCon(name: String, arg: Formula, repl: Expression, acc: IDMap): IDMap = {
    ofExp(repl, ofExp(arg, acc)).addCon(name)
  }

  def ofProg(name: String, repl: Expression, acc: IDMap): IDMap = { ofExp(repl, acc).addProg(name) }

  def ofDiffConst(name: String, repl: Expression, acc: IDMap): IDMap = { ofExp(repl, acc).addDiffProg(name) }

  def ofSubst(us: USubst, acc: IDMap): IDMap = {
    us.subsDefsInput
      .map({ case SubstitutionPair(what, repl) => (what, repl) })
      .foldLeft(acc) {
...
//        println("Translating functional replaced with: " + repl)
  ...
  def ofProofTerm(pt: ProofTerm, acc: IDMap): IDMap = {
    pt match {
          ...
          try { Provable.rules(name) }
            ...
            case _: NoSuchElementException =>
              try {
                ...
                DerivedRuleInfo
                  .allInfo
                  .find({ case (name, info) => info.codeName.toLowerCase() == name.toLowerCase() })
                  .get
                  ._2
                  .provable
                  .underlyingProvable
                ...
                case e: NoSuchElementException =>
                  logger.warn("Couldn't find rule: " + name, e)
                  throw e
              }
          }
        ofProvable(r, acc)
      case UsubstProvableTerm(child: ProofTerm, substitution: USubst) => ofSubst(substitution, ofProofTerm(child, acc))
      ...
      case Sub(child: ProofTerm, sub: ProofTerm, idx: Int) => ofProofTerm(child, ofProofTerm(sub, acc))
...
case class IDMap(
    varMap: Map[(String, Option[Int]), String],
  ...
  def addVar(name: String, ind: Option[Int]): IDMap = {
    if (varMap.contains((name, ind))) { this }
      ...
      IDMap(
        varMap.+(((name, ind), ISABELLE_IDS(maxVar))),
        funMap,
        predMap,
        conMap,
        progMap,
  ...
  def addProg(name: String): IDMap = {
    if (progMap.contains(name)) { this }
      ...
      IDMap(
        ...
        predMap,
        conMap,
        progMap.+((name, ISABELLE_IDS(maxProg))),
        odeMap,
        fArity,
        pArity,
        maxVar,
  ...
  def addDiffProg(name: String): IDMap = {
    if (odeMap.contains(name)) { this }
      ...
      IDMap(
        ...
        conMap,
        progMap,
        odeMap.+((name, ISABELLE_IDS(maxOde))),
        fArity,
        pArity,
        maxVar,
        maxFun,
      ...
      throw ConversionException(
        "Need more Isabelle identifiers, not enough to convert differential program identifier: " + name
      )
    }
  }
  ...
  def addUnitPred(name: String): IDMap = {
    if (conMap.contains(Right(name))) { this }
      ...
      IDMap(
        ...
        funMap,
        predMap,
        conMap.+((Right(name), ISABELLE_IDS(maxCon))),
        progMap,
        odeMap,
        fArity,
        pArity,
      ...
      throw ConversionException(
        "Need more Isabelle identifiers, not enough to convert nullary predicational identifier: " + name
      )
    }
  }
  ...
  def addCon(name: String): IDMap = {
    if (conMap.contains(Left(name))) { this }
      ...
      IDMap(
        ...
        funMap,
        predMap,
        conMap.+((Left(name), ISABELLE_IDS(maxCon))),
        progMap,
        odeMap,
        fArity,
        pArity,
      ...
      throw ConversionException(
        "Need more Isabelle identifiers, not enough to convert unary predicational identifier: " + name
      )
    }
  }

  def addFunc(name: String, arity: Int): IDMap = {
    if (funMap.contains(Left(name))) { this }
    else if (maxFun < ISABELLE_IDS.size) {
      IDMap(
        varMap,
        funMap.+((Left(name), ISABELLE_IDS(maxFun))),
        predMap,
        conMap,
        progMap,
        odeMap,
  ...
  def addFuncl(name: String): IDMap = {
    if (funMap.contains(Right(name))) { this }
    else if (maxFun < ISABELLE_IDS.size) {
      IDMap(
        varMap,
        funMap.+((Right(name), ISABELLE_IDS(maxFun))),
        predMap,
        conMap,
        progMap,
        odeMap,
  ...
  def addPred(name: String, arity: Int): IDMap = {
    if (predMap.contains(name)) { this }
    else if (predMap.size < ISABELLE_IDS.size) {
      IDMap(
        varMap,
        funMap,
        predMap.+((name, ISABELLE_IDS(maxPred))),
        conMap,
        progMap,
        odeMap,
        fArity,
...
object IaxiomaticRule {
  def apply(n: String): IaxiomaticRule = {
    n match {
      ...
      case "goedel" => IG()
      case "monb" => Imonb()
      case _ => throw ConversionException("Unrecognized axiomatic rule: " + n)
    }
  }
}
...
case class Isubst(
    ...
    SContexts: List[Option[Iformula]],
    SPrograms: List[Option[Ihp]],
...
class IsabelleConverter(pt: ProofTerm) extends Logging {
  ...
  def apply(name: String, seqPos: Seq[SeqPos], expArgs: Seq[Expression]): IruleApp = {
    (name, seqPos.toList, expArgs.toList) match {
      ...
      case ("cut Left", (a: AntePos) :: Nil, (f: Formula) :: Nil) =>
        IApplyLrule(ICutLeft(apply(f, NonSubst())), a.getIndex)
      ...
      case ("cut Right", (s: SuccPos) :: Nil, (f: Formula) :: Nil) =>
        IApplyRrule(ICutRight(apply(f, NonSubst())), s.getIndex)
      ...
      case _ => throw ConversionException(
          "Unrecognized non-axiomatic rule: " + name + "," + seqPos.toList + ", " + expArgs.toList
        )
    }
  }

  private def sortSubs[T](
      seq: Seq[(Expression, Expression)],
      f: (Expression => String),
      g: ((Expression, Expression) => T),
  ): List[(String, T)] = {
  ...
  def apply(sub: USubst, defun: Boolean = false, depred: Boolean = false, space: Ispace = IAllSpace()): Isubst = {
    ...
    val (fun, t1) =
      if (defun) {
        pairs.partition({
          case (_: FuncOf, _) => true
          case (_: UnitFunctional, _) => true
          case _ => false
        })
      } else {
        pairs.partition({
    ...
    val (pred, t2) =
      if (depred) {
        t1.partition({
          case (_: PredOf, _) => true
          case (_: UnitPredicational, _) => true
          case _ => false
        })
      } else {
        t1.partition({
    ...
    val (prog, t4) = t3.partition({
      case (_: ProgramConst, _) => true
      case (_: SystemConst, _) => true
      case _ => false
    })
    val (ode, t5) = t4.partition({
      case (_: DifferentialProgramConst, _) => true
      case _ => false
    })
    val (unitFun, t6) =
      if (defun) { t5.partition(_ => false) }
      else {
        t5.partition({
          case (_: UnitFunctional, _) => true
          case _ => false
    ...
    if (
      unitFun.exists({
        case (_, FuncOf(Function("V", _, _, _, _), _)) => true
        case _ => false
      })
    ) { val 2 = 1 + 1 }
    ...
    val res = Isubst(
      ...
      extendSub(sortSubs(
        fun,
        {
          case FuncOf(Function(name, _, _, _, _), _) => m.funMap(Left(name))
          case UnitFunctional(name, _, _) => m.funMap(Right(name))
        },
        {
          case (_: UnitFunctional, e: Term) => apply(e, sm = if (defun) DefunSubst() else FunSubst())
          case (_: FuncOf, e: Term) => apply(e, sm = FunSubst())
        },
      )),
      extendSub(sortSubs(
        unitFun,
        { case UnitFunctional(name, _, _) => m.funMap(Right(name)) },
        { case (_, e: Term) => apply(e, NonSubst()) },
      )),
      // @TODO: Not clear what mode
      extendSub(sortSubs(
        pred,
        {
          case PredOf(Function(name, _, _, _, _), _) => m.predMap(name)
          case UnitPredicational(name, _) => m.conMap(Right(name))
        },
        {
      ...
      extendSub(sortSubs(
        con,
        {
          case PredicationalOf(Function(name, _, _, _, _), _) => m.conMap(Left(name))
          case UnitPredicational(name, _) => m.conMap(Right(name))
        },
        { case (_, e: Formula) => apply(e, sm = ConSubst()) },
      )),
      extendSub(sortSubs(
        prog,
        {
          case ProgramConst(name, _) => m.progMap(name)
          case SystemConst(name, _) => m.progMap(name)
        },
        { case (_, e: Program) => apply(e, NonSubst()) },
      )),
      extendSub(sortSubs(
        ode,
        { case DifferentialProgramConst(name, _) => m.odeMap(name) },
        { case (_, e: DifferentialProgram) => apply(e, NonSubst()) },
      )),
      space,
    )
    res
  ...
  private def isDiffFormulaChase(pt: ProofTerm): Boolean = {
    pt match {
      case Sub(
            Sub(
              RuleApplication(StartProof(reflFml), _ /*"cut Right"*/, _),
              ForwardNewConsequenceTerm(
                ForwardNewConsequenceTerm(
                  ProlongationTerm(
                    UsubstProvableTerm(AxiomTerm(geqPrimeName /*">=' derive >="*/ ), _),
                    UsubstProvableTerm(RuleTerm(ceName /*"CE Equiv"*/ ), _),
                  ),
                  _,
                  _: EquivifyRight,
                ),
            ...
            UsubstProvableTerm(AxiomTerm(equivReflName), _),
            _,
          ) =>
        val 2 = 1 + 1
        true
      case Sub(
            ...
            UsubstProvableTerm(AxiomTerm(equivReflName), equivReflSubst),
            where,
          ) =>
  ...
  private def translateDiffFormulaChase(pt: ProofTerm): Ipt = {
    pt match {
      case Sub(
            Sub(
              RuleApplication(StartProof(reflFml), _: CutRight, _),
              ForwardNewConsequenceTerm(
                ForwardNewConsequenceTerm(
                  ProlongationTerm(
                    UsubstProvableTerm(AxiomTerm(">=' derive >="), _),
                    UsubstProvableTerm(RuleTerm("CE congruence"), _),
                  ),
                  _,
                  _: EquivifyRight,
                ),
            ...
            UsubstProvableTerm(AxiomTerm("<-> reflexive"), equivReflSubst),
            where,
          ) =>
  ...
  private def isDiffEffectSysInstance(pt: ProofTerm): Boolean = {
    pt match {
      ...
      case UsubstProvableTerm(Sub(RuleApplication(StartProof(seq), _, _), _, _), _) if seqNeedsDefun(seq) => true
      case _ => false
    }
  }
  ...
  private def or(p: Iformula, q: Iformula): Iformula = { INot(IAnd(INot(p), INot(q))) }

  private def implies(p: Iformula, q: Iformula): Iformula = { or(q, INot(p)) }

  private def dpredl(id: String): Iformula = { IProp(IDEnum(id), ISABELLE_IDS.map(i => IVar(IDEnum(i))).toList) }

  private def box(a: Ihp, p: Iformula): Iformula = { INot(IDiamond(a, INot(p))) }

  private def diGeqConclusion(o: IODE, t1: Itrm, t2: Itrm): Iformula = {
    implies(
      implies(
        dpredl("i1"),
        IAnd(IGeq(t1, t2), box(IEvolveODE(o, dpredl("i1")), IGeq(IDifferential(t1), IDifferential(t2)))),
      ),
  ...
  private def translateDiffTermChase(pttt: ProofTerm): Ipt = {
    pttt match {
      case rp @ RuleApplication(
            Sub(a, Sub(b, UsubstProvableTerm(AxiomTerm("DI differential invariant"), sub), c), d),
            rule,
            subgoal,
          ) =>
          ...
          case GreaterEqual(l: Term, r: Term) =>
            ...
            val bigSubst = USubst(
              collection
                .immutable
                .Seq(
                  SubstitutionPair(csym, cc),
                  SubstitutionPair(qsym, q),
                  SubstitutionPair(fsym, l),
                  SubstitutionPair(gsym, r),
                )
            )
            ...
            IRuleApplication(
              ISub(apply(a), ISub(apply(b), result, c), d),
              apply(rule.name, rp.positions, rp.args),
              subgoal,
            )
          /*case LessEqual(r,l) =>
            val fsym = UnitFunctional("f", AnyArg, Real)
  ...
  private def isDiffTermChase(pt: ProofTerm): Boolean = {
    pt match {
      case RuleApplication(Sub(a, Sub(b, UsubstProvableTerm(AxiomTerm("DI differential invariant"), _), _), _), _, _) =>
        true
      case _ => false
    }
  }
  ...
  private def axiomNeedsDefunctionalization(pt: ProofTerm): Boolean = {
    pt match {
      ...
      case Sub(RuleApplication(StartProof(seq), _, _), _, _) if seqNeedsDefun(seq) => true
  ...
  private def axiomNeedsDepredicationalization(pt: ProofTerm): Boolean = {
    pt match {
      ...
      // commuted DESys... yechh
      case Sub(RuleApplication(StartProof(seq), _, _), _, _) if seqNeedsDepred(seq) => false // true
      case AxiomTerm(n) => /*println("Normal ax: " + pt); */ false
      case _ => false
    }
  }
  ...
  def apply(pt: ProofTerm): Ipt = {
    ...
    else if (isDiffEffectSysInstance(pt)) { translateDiffEffectSysInstance(pt) }
    else {
      pt match {
        ...
        case StartProof(seq) =>
          ...
          val sm0 = NonSubst()
          val sm1 =
            if (seqNeedsDefun(seq)) { Defun(sm0) }
            else sm0
          val sm2 =
            if (seqNeedsDepred(seq)) { Depred(sm0) }
            else sm1
          val space =
            if (seqNeedsBanana(seq)) { INBSpace("i1") }
            else { IAllSpace() }
          val sm3 =
            if (seqNeedsBanana(seq)) {
              logger.trace("Bananizing startproof: " + seq)
              BananaODE(sm2)
            } else sm2
          if (sm3 != NonSubst()) { logger.trace("Interesting startproof: " + sm3) }
  ...
  private def seqNeedsDefun(phi: Sequent): Boolean = {
    ...
    val specials: List[Sequent] = List(new Sequent(
      collection.immutable.IndexedSeq(),
      collection.immutable.IndexedSeq(reverse(AxiomInfo("DE differential effect (system)").formula)),
    ))
  ...
  def apply(f: Formula, sm: SymMode): Iformula = {
    (f, sm.isDepred) match {
      ...
      case (PredOf(Function(name, _, _, _, _), arg), _) =>
        val propId =
          if (sm.base == FunSubst()) { IDLeft(IDEnum(m.predMap(name))) }
          else { IDEnum(m.predMap(name)) }
      ...
      case (PredicationalOf(Function(name, _, _, _, _), child), _) =>
        val predId =
          if (sm.base == ConSubst()) { IDLeft(IDEnum(m.conMap(Left(name))), "myvars", "Unit") }
          else { IDEnum(m.conMap(Left(name))) }
      ...
      case (UnitPredicational(name, _), true) => {
        val predId =
          if (sm.base == ConSubst()) { IDLeft(IDEnum(m.conMap(Right(name))), "myvars", "Unit") }
          else { IDEnum(m.conMap(Right(name))) }
      ...
      case (DifferentialFormula(Greater(t1, t2)), _) =>
        IGeq(IDifferential(apply(t1, ddefun(sm))), IDifferential(apply(t2, ddefun(sm))))
      case (DifferentialFormula(LessEqual(l, r)), _) =>
        IGeq(IDifferential(apply(r, ddefun(sm))), IDifferential(apply(l, ddefun(sm))))
      case (DifferentialFormula(Less(l, r)), _) =>
        IGeq(IDifferential(apply(r, ddefun(sm))), IDifferential(apply(l, ddefun(sm))))
  ...
  def apply(t: Term, sm: SymMode): Itrm = {
    (t, sm.isDefun) match {
      ...
      case (DifferentialSymbol(BaseVariable(x, ind, _)), _)
          if (sm.base == DefunSubst() /*|| sm.base == DepredSubst()*/ ) => ???
      case (DifferentialSymbol(BaseVariable(x, ind, _)), _) => IDiffVar(IDEnum(m.varMap((x, ind))))
      case (Number(n), _) =>
        if (n.isValidInt) { IConst(n.intValue, sm) }
        else { throw ConversionException("Can't convert non-integer literal: " + n) }
      case (FuncOf(Function(name, _, _, _, _), arg), _) =>
        ...
        val funId =
          ...
          } else { IDEnum(m.funMap(Left(name))) }
      ...
      case (Divide(l, r), _) =>
        throw ConversionException("Converter currently does not support conversion of divisions")
  ...
  def apply(o: DifferentialProgram, sm: SymMode): IODE = {
    o match {
      ...
      case DifferentialProgramConst(c, _) if sm.isBananaODE => IOVar(IDEnum(m.odeMap(c)), INBSpace("i1"))
  ...
  def apply(hp: Program, sm: SymMode): Ihp = {
    hp match {
      ...
      case Assign(DifferentialSymbol(BaseVariable(x, ind, _)), e) =>
        IDiffAssign(IDEnum(m.varMap((x, ind))), apply(e, ddefun(sm)))
  ...
  def apply(pr: Provable): Irule = { (pr.subgoals.map(apply(_, NonSubst())).toList, apply(pr.conclusion, NonSubst())) }
  ...
  private def writeObjects(sb: StringBuilder, objName: String, fieldName: String, mainName: String): Unit = {
    ...
    val defs = List(
      ...
      s"def s(t:trm[myvars,myvars]):(myvars =>trm[myvars,myvars]) = {case ${ISABELLE_IDS
          .head}() => t ${ISABELLE_IDS.tail.map(i => s"case $i() => z").mkString(" ")}}",
      s"def sst(t:trm[sum[myvars,myvars],myvars]):(myvars =>trm[sum[myvars,myvars],myvars]) = {case ${ISABELLE_IDS
          .head}() => t ${ISABELLE_IDS.tail.map(i => s"case $i() => zst").mkString(" ")}}",
...
abstract class SourceBuilder(sb: StringBuilder) {
  ...
  private def emptyElse(args: List[Itrm], f: (() => Unit), sm: SymMode): Unit = {
    if (
      args.forall({
        case IConst(0, sm) => true
        case _ => false
      })
    ) {
      sm match {
        case _: FunSubst => sb.++=("est")
        case _ => sb.++=("e")
      }
    } else if (
      args.length >= 1 &&
  ...
  def nat(i: Int): Unit = { b1("Nata", () => sb.++=(i.toString)) }

  def apply(br: Int): Unit = { b1("int_of_integer", () => sb.++=(br.toString)) }
  ...
  def apply(pt: Ipt): Unit = {
    pt match {
      ...
      case ISub(child, sub, branch) =>
        b3("Sub", () => apply(child), () => apply(sub), () => nat(branch)) /*;sb.++=("\n")*/
    }
  }
}
...
class ScalaBuilder(sb: StringBuilder) extends SourceBuilder(sb) {
  ...
  override def b7(
    ...
    sb.++=(name); sb.++=("("); f1(); sb.++=(","); f2(); sb.++=(","); f3(); sb.++=(","); f4(); sb.++=(","); f5();
    sb.++=(","); f6(); sb.++=(","); f7(); sb.++=(",())")
  }

  override def btup(f: (() => Unit), g: (() => Unit)): Unit = { sb.++=("("); f(); sb.++=(","); g(); sb.++=(")") }
  ...
  override def bff[T](l: List[T], f: (T => Unit)): Unit = {
    ...
    sb.++=("{")
    cases.foreach({ case (v, id) => sb.++=("case "); sb.++=(id); sb.++=("() => "); f(v); sb.++=(" ") })
    sb.++=("}")
  }
...
class SexpBuilder(sb: StringBuilder) extends SourceBuilder(sb) {
  ...
  override def b7(
    ...
    sb.++=("("); sb.++=(name); sb.++=(" "); f1(); sb.++=(" "); f2(); sb.++=(" "); f3(); sb.++=(" "); f4(); sb.++=(" ");
    f5(); sb.++=(" "); f6(); sb.++=(" "); f7(); sb.++=(")")
  }
  ...
  // finite functions over identifiers
  override def bff[T](l: List[T], f: (T => Unit)): Unit = { blist(l, f) }
