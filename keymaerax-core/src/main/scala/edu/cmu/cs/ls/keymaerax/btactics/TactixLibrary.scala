 ...
 * [[edu.cmu.cs.ls.keymaerax.core.Rule prover kernel]].
 *
      ...
      anon((pos: Position, seq: Sequent, defs: Declaration) =>
        (gen, seq.sub(pos)) match {
          case (cgen: ConfigurableGenerator[GenProduct], Some(Box(Loop(_), _))) if cgen(seq, pos, defs).nonEmpty =>
            ...
            ChooseSome(
              () =>
                try { gen(seq, pos, defs).iterator.map(_._1) }
                catch {
                  case err: Exception =>
  ...
  def loop(gen: Generator[GenProduct]): DependentPositionTactic = new DependentPositionTactic("I gen") {
    override def factory(pos: Position): DependentTactic = new SingleGoalDependentTactic(name) {
      override def computeExpr(sequent: Sequent, defs: Declaration): BelleExpr = loop(nextOrElse(
        gen(sequent, pos, defs).map(_._1).iterator,
        ...
        case Some(Box(Loop(_), _) | Diamond(Loop(_), _)) =>
          ...
          (invSupplier, seq.sub(pos)) match {
            case (cgen: ConfigurableGenerator[GenProduct], Some(Box(Loop(_), _))) if cgen(seq, pos, defs).nonEmpty =>
              ...
              ChooseSome(
                () =>
                  try { invSupplier(seq, pos, defs).iterator.map(_._1) }
                  catch {
                    case err: Exception =>
                      logger.warn("ChooseSome: error listing options " + err, err)
                      List[Formula]().iterator
