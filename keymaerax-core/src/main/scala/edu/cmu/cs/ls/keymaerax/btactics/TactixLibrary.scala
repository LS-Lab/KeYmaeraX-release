...
object TactixLibrary extends HilbertCalculus
  ...
  def explore(gen: Generator[GenProduct]): InputTactic = inputanon {autoImpl(anon ((pos: Position, seq: Sequent, defs: Declaration) => (gen, seq.sub(pos)) match {
    case (cgen: ConfigurableGenerator[GenProduct], Some(Box(Loop(_), _))) if cgen(seq, pos, defs).nonEmpty =>
      ...
      ChooseSome(
        () => try {
          gen(seq, pos, defs).iterator.map(_._1)
        } catch {
          case err: Exception =>
...
//  def US(subst: USubst, origin: Sequent): BuiltInTactic = ProofRuleTactics.US(subst, origin)
  ...
  def loopauto(gen: Generator[GenProduct] = loopInvGenerator): DependentPositionTactic =
      anon ((pos: Position, seq: Sequent, defs: Declaration) => seq.sub(pos) match {
    case Some(Box(Loop(_), _) | Diamond(Loop(_), _)) =>
      ...
      (invSupplier, seq.sub(pos)) match {
        case (cgen: ConfigurableGenerator[GenProduct], Some(Box(Loop(_), _))) if cgen(seq, pos, defs).nonEmpty =>
          ...
          ChooseSome(
            () => try {
              invSupplier(seq, pos, defs).iterator.map(_._1)
            } catch {
              case err: Exception =>
                logger.warn("ChooseSome: error listing options " + err, err)
                List[Formula]().iterator
...
//@todo     loopPostMaster(gen)(pos) & done ||
  ...
  private lazy val ODEImpl: DependentPositionTactic = anon ((pos: Position, seq: Sequent, defs: Declaration) => must({
    // use and check invSupplier (user-defined annotations from input file)
    val invs = invSupplier(seq, pos, defs).toList
