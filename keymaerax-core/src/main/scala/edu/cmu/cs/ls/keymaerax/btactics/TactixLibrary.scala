...
object TactixLibrary extends HilbertCalculus with SequentCalculus {
  ...
  /** auto: automatically try hard to prove the current goal if that succeeds.
    * @see [[master]] */
  def auto: BelleExpr = "auto" by master(loopauto(invGenerator), ODE, keepQEFalse=true) & done
...
//  def US(subst: USubst, origin: Sequent): BuiltInTactic = ProofRuleTactics.US(subst, origin)
  ...
  /** loop: prove a property of a loop automatically by induction, trying hard to generate loop invariants.
    * @see [[loop(Formula)]] */
  def loopauto(gen: Generator[Formula] = invGenerator): DependentPositionTactic = "loopauto" by ((pos: Position, seq: Sequent) => {
    ...
    (invGenerator, seq.sub(pos)) match {
      ...
      case _ =>
        logger.debug("LoopAuto uses loopPostMaster")
        loopPostMaster(gen)(pos)
    }
  })
  ...
  lazy val ODE: DependentPositionTactic = "ODE" by ((pos: Position) =>
    odeInvariant(pos) | solve(pos) | DifferentialTactics.ODE(pos))
  ...
  lazy val odeInvariant: DependentPositionTactic = "odeInvariant" by ((pos:Position) =>
    DifferentialTactics.DconstV(pos) & DifferentialTactics.domSimplify(pos) &
      ((cohide(pos) & boxTrue(1)) | ODEInvariance.sAIclosedPlus(3)(pos) | ODEInvariance.sAIRankOne(true)(pos)))
