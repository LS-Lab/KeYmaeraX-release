  ...
  def doStep(index: Boolean => Expression => Option[DerivationInfo]): DependentPositionTactic = anon ((pos: Position, seq: Sequent) =>
    ...
    TryCatch(
      ...
      classOf[Throwable], (ex: Throwable) => seq.sub(pos) match {
        ...
        case _ => ex match {
          // StackOverflowErrors were swallowed by assignb so far; translate only in 4.9.4 for compatibility with old proofs
          // they are caused by the deeply nested structures generated by tacticChase (unfold etc.)
          case e: IllFormedTacticApplicationException => e.getCause match {
            case _: StackOverflowError =>
              throw new TacticInapplicableFailure("Step-attempt not terminating within stack limits", ex)
            case _ =>
              throw ex
          }
          case _ =>
